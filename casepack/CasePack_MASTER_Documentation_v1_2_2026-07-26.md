...
**Version:** 1.2 · **Date:** 2026-07-26 *(supersedes v1.1 same-day — discard any earlier v1.1 download)* · **Status of live artifact:** `casepack_page.html` md5 `2d18d1c3873675f48e250275a564f2f4` (277,641 bytes) — **DEPLOYED & VERIFIED 2026-07-22**
*(History: md5 `82e41230…` deployed 2026-07-21, verified, superseded 2026-07-22 by the thrneck/extension/attest build.)*
...
Hindi consent assembled from the doctor's own library: procedure-specific opening (tokens {NAME}{AGE}{RELM}{RELN}{RES}{SIDE}{PART}{BONE}{DUR}) + shared clauses (consultation, understanding, re-operation possibility, infection, anaesthesia, no-guarantee, physiotherapy, non-union where relevant) + closing + attestation. Print layout v4: 3-line repeating letterhead (title / credentials / patient-निवासी-date), no in-body title, borderless consent table, side-by-side dashed end-signature boxes, doctor's dashed signing box in attestation, auto "पृष्ठ X/Y". Title is print-only. **`CS_ATTEST` wording is LOCKED — text edits need explicit doctor OK.** (Locked text updated 2026-07-22 with doctor approval: inline "हस्ताक्षर/दिनांक" blanks trimmed to "हस्ताक्षर व दिनांक/समय नीचे बॉक्स में:", pairing with the dashed dr-box.) **New 2026-07-22:** procedure entry `thrneck` — "THR for fracture neck femur / कूल्हा प्रत्यारोपण — गर्दन फ्रैक्चर" (doctor-authored fix-vs-replace opening; dialect word **गुल्ले** retained deliberately; same body/risks as elective THR; `thrneck` present in both joint-recognition lists). New tick-box add-on **`extension`** "ऑपरेशन में बदलाव/विस्तार की मंजूरी" (doctor's wording), printed after the risk paragraph before the आयुष्मान clause; co-exists with the older "आपात निर्णय" add-on; tick-box only, never auto-added. `csLastMeta` carries `res` for the header.
...
2. `CS_ATTEST` wording locked (current locked text = 2026-07-22 trimmed version).
...
8. Extension-of-surgery clause stays a tick-box add-on — never default-on (doctor decision D3, 2026-07-22).
9. गुल्ले stays in the thrneck opening — patients' dialect, deliberate (doctor decision, 2026-07-22).
...
- **2026-07-22 (Session c):** consent engine opened with approval — `thrneck` procedure entry (from live case C-2026-000003), `extension` add-on (Phase 1k delivered), CS_ATTEST trim. New md5 `2d18d1c3…`, deployed & verified. **Clinic Hub v1 built & live** (`D:\clinic_hub\`): static `clinic_hub.html` (4 cards, live port dots 5058/5000/5057, GMB Review Assist by relative file link) + self-contained `open_clinic_hub.bat` (direct `start /D` python launches with port+file checks; tracker deliberately excluded to preserve its CSV-archive ritual). Path truth: Case Pack folder = `D:\casepack tool` (old bat header comment stale). Accepted quirk: hub launch also opens vitals+casepack self-opened tabs (fix designed: HUB_LAUNCH env guard; deferred, needs the two app .py files).
- **2026-07-26 (cross-project):** Clinic Hub **expanded to 5 cards** — new **CC Statements → Tally** card, port **5059**, `D:\Scripts\statements_app.py` (built in the Clinic Systems & Automation project; auto-runs once daily on hub start, on-demand button inside). Launcher gained a matching third start block. **Governance decision: the hub is now a shared surface across both projects; the canonical copy is the GitHub `clinic-hub/` folder — whichever project edits the hub must commit there, and always expand from the repo copy, never from memory.**

## 6. Open backlog
- [ ] Hub tab-quirk fix: HUB_LAUNCH env guard in `casepack_app.py` + `vitals_app.py` (doctor to upload both; full-file replacements, .py only).
- [ ] Repair `open_casepack.bat` + `open_vitals.bat` with `cd /d "%~dp0"` (rename-proof); fix stale header comments.
- [ ] GMB Review Assist upgrade pass: variation pools + shuffle, WhatsApp message = draft + direct long Google review URL, catalogue refresh. **Blocked on doctor supplying the review URL (GMB dashboard → Ask for reviews).** Shortlink & QR rejected.
...
- [ ] HBP-2022 ingestion (source document still not in project).
- [ ] Doctor 30-sec item: desktop shortcut to `open_clinic_hub.bat` named "Clinic Tools".
...
- `clinic_hub.html` + `open_clinic_hub.bat` + repo copy of `GMB_Review_Assist_DrManojAgarwal.html` — Clinic Hub (live at `D:\clinic_hub\`; 5 cards as of 2026-07-26: Case Pack :5058 · Tracker :5000 · Vitals :5057 · GMB file · CC Statements :5059). Canonical copy = GitHub `clinic-hub/`.
- `Consent_Update_Preview_2026-07-21c.html` — the approved design preview behind the thrneck/extension/attest changes.
