...
def api_day(date_iso):
    """One day: its own drawer, its own tenders, its own scans. The PAYLOAD is
    unchanged by D2 -- it was already correctly scoped. What it never had was a
    role check of any kind (F-127), so a viewer seat could read it. Gated now;
    the maker needs it and keeps it."""
    u, err = require("maker", "checker")
    if err:
        return err
    try:
        ...
        if rv and rv["fingerprint"] != _money_fingerprint(total_p, upi_p, expenses,
            ...
            if not con.execute("SELECT 1 FROM data_flag WHERE unit=? AND business_date=? "
                ...
                con.execute("INSERT INTO data_flag (unit, business_date, day_entry_id, "
                            ...
                            (UNIT, iso, eid,
                             "the figures were changed after the bank/Marg check was "
                             "shown for this day. Not an error and not blocked (D326) "
                             "\u2014 recorded so the cross-check stays honest."))
...
def api_day_mirror(date_iso):
    ...
    _mirror_tbl(con)
    if not con.execute("SELECT 1 FROM day_mirror_reveal WHERE day_entry_id=?",
                       (eid,)).fetchone():
    ...
    return jsonify(ok=True, date=iso, status=st.get("status"),
                   declared=dict(total=rupees(declared_total_p),
                                 ...
                                 cash=st.get("cash"),
                                 opening=st.get("opening"),
                                 closing=st.get("closing")),
...
def selftest():
    ...
    for _pid in ("bdate", "manned", "opening", "total", "upi", "cashCalc", "cashReal",
                 "ncList", "addNc", "expList", "addExp", "movList", "addMov",
                 "closingBox", "closing", "scanReason", "btnDraft", "btnSubmit",
