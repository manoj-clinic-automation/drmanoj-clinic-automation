...
# S188: the live box has NO unit_role row for the smoke user. Seeding a
# checker row here turned every "a maker cannot X" assertion green by
# accident -- the F-106 trap, in the rehearsal harness itself. The header
# role alone decides, exactly as it does on the box.
con.execute("DELETE FROM unit_role WHERE lower(username)='selftest'")
con.commit()
