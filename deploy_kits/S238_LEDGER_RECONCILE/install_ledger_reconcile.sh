#!/bin/bash
# =============================================================================
#  install_ledger_reconcile.sh · kit S238_LEDGER_RECONCILE
#  Run by:  bash /root/deploy/vps_deploy.sh S238_LEDGER_RECONCILE
#
...
[ "$(md5sum "$MODULE" | awk '{print $1}')" = "$PIN" ] \
  || fail "$MODULE is not the pinned v3.6 rev 3 ($PIN) this tool was built and proven against"
