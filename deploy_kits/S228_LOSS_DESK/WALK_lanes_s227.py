...
# names[43] "LS BELT" (index 43 -> item 44): an orthotic -> the ORTHOTICS lane (S227 STAFF).  names[44]: dead, shelf = Marg -> sitting dead (A).
...
WANT = {X: "marg", names[1]: "loss", names[2]: "over", names[5]: "allowance", names[7]: "loss", names[9]: "recount",
        names[12]: "recount", names[17]: "bill", names[40]: "dead", names[41]: "dead", names[42]: "dead", names[43]: "ortho"}
KIND = {names[40]: "D", names[41]: "C", names[42]: "B"}
...
with sync_playwright() as pw:
    ...
    chk("lane totals: recount 2, dead 3 (+11 matched; the LS BELT is an orthotic now), salt 1 pair, every lane with lines / short / over / unpriced",
        L1["lines"] == 2 and L8["lines"] == 3 and L8["dead_matched"] == 11 and L7["pairs"] == 1 and all(k in L1 for k in ("short_p", "over_p", "unpriced", "decided", "major")), [(l["key"], l["lines"]) for l in j["lanes"]])
    ...
    # ------------------------------------------------------------ 2 the page (read-only since the desk split: the document, not the buttons)
    pg.goto(HOST + "/finance/stock/page/report?count=1"); pg.wait_for_timeout(1400)
    head = pg.inner_text("#head"); body = pg.inner_text("#body")
    chk("THE PAGE: the running totals, the search box, the owner's card pointing at the desk", "written off" in head and "still open" in head and pg.locator("#q").count() == 1
        and pg.locator("#s0.owner").count() == 1 and "Open the decision desk" in body and pg.locator("#s0 a.desk").get_attribute("href").endswith("/page/desk?count=1"), body[:200])
    chk("...the lanes as folding cards with titles and totals, ALL folded on arrival (the owner: the page must not daunt)",
        pg.locator(".lane").count() >= 6 and pg.locator(".lane .lb:not([hidden])").count() == 0 and "Recount first" in body and "No movement since April" in body,
        (pg.locator(".lane").count(), pg.locator(".lane .lb:not([hidden])").count()))
    pg.locator('.lane[data-lane="allowance"] button.tg').click(); pg.wait_for_timeout(200)
    chk("...a folded lane opens on its Show button and the button now reads Hide", pg.locator('.lane[data-lane="allowance"] .lb').get_attribute("hidden") is None
        and pg.locator('.lane[data-lane="allowance"] button.tg').inner_text().startswith("Hide"))
    pg.evaluate("()=>document.querySelectorAll('.lane .lb[hidden]').forEach(e=>e.hidden=false)"); pg.wait_for_timeout(200)
    ...
    chk("...the dead lane shows kind badges and the two Amir lists", pg.locator('.lane-dead .badge.kind').count() == 3 and "Amir's check list (Excel)" in body and "Marg cleanup list for Amir (Excel)" in body)
    chk("...NO decision buttons on the document (they live on the desk)", pg.locator("button.act").count() == 0 and pg.locator("input.sel").count() == 0)
    ...
    # ------------------------------------------------------------ 3 the owner's word through the decide route; the document shows it
    def say(items, act):
        return pg.request.post(HOST + "/finance/stock/api/pad/decide", data=json.dumps(dict(count_id=1, items=items, action=act, note="")), headers={"Content-Type": "application/json"}).json()
    say([names[42]], "PARKED"); j = pg.request.get(HOST + "/finance/stock/api/pad/report/1").json()
    ...
    say([d["item"] for d in j["differences"] if d["lane"] == "allowance"], "WRITE_OFF"); j = pg.request.get(HOST + "/finance/stock/api/pad/report/1").json()
    chk("WRITE OFF the whole allowance lane in one call: every line of it written off, the totals move",
    ...
    say([names[7]], "RECOVER"); j = pg.request.get(HOST + "/finance/stock/api/pad/report/1").json()
    w7 = [d for d in j["differences"] if d["item"] == names[7]][0]["word"]; w1 = [d for d in j["differences"] if d["item"] == names[1]][0]["word"]
    chk("ONE LINE marked to pursue: only that line, the other loss line untouched", w7 and w7["action"] == "RECOVER" and not w1, (w7, w1))
    ...
    pg.reload(); pg.wait_for_timeout(1400); pg.evaluate("()=>document.querySelectorAll('.lane .lb[hidden]').forEach(e=>e.hidden=false)"); pg.wait_for_timeout(200)
    rt = pg.locator('.row[data-item="%s"]' % names[7]).inner_text()
    chk("...THE DOCUMENT SHOWS THE WORD on the line, by whom and when", "to pursue" in rt and "by walk" in rt, rt)
    say([names[7]], "OPEN"); j = pg.request.get(HOST + "/finance/stock/api/pad/report/1").json()
    chk("...a change of mind is a new row: Reopen puts it back to open (no word again), the history stays", not [d for d in j["differences"] if d["item"] == names[7]][0]["word"]
    ...
    chk("AMIR'S NO-MOVEMENT LIST: every dead item with its kind and what to do (A x11, B, C, D; the orthotic has its own lane now)",
        sorted(set(x[0] for x in rows)) == ["A", "B", "C", "D"] and len(rows) == 14 and any("expiry" in (x[6] or "") for x in rows) and any("WAIT for the owner" in (x[6] or "") for x in rows), rows)
