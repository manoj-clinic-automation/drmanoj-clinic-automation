...
def _systemd(unit, now):
    ...
    try:
        ...
        pass
    try:
        r = subprocess.run(["systemctl", "show", unit + ".service",
                            "-p", "ExecMainStartTimestamp"],
                           capture_output=True, text=True, timeout=20)
        val = (r.stdout.split("=", 1)[-1] or "").strip()
        if val:
            for fmt in ("%a %Y-%m-%d %H:%M:%S %Z", "%a %Y-%m-%d %H:%M:%S"):
                try:
                    when = dt.datetime.strptime(val, fmt)
                    break
                except ValueError:
                    when = None
            if when is not None:
                when = when.replace(tzinfo=dt.timezone.utc).astimezone(IST)
...
def render(rows, err, now):
    order = {"SILENT": 0, "OFF": 1, "NO TRACE": 2, "LATE": 3, "SEEN": 4, "ALIVE": 5}
    ...
    L.append("")
    L.append("%-9s %-34s %-12s %-10s %s" % ("verdict", "job", "last wrote",
                                            "age", "trace"))
    L.append("-" * 96)
    for r in rows:
        L.append("%-9s %-34s %-12s %-10s %s" % (
            r["verdict"], r["job"][:34], r["last_seen"],
            human_age(r["age_min"]), (r.get("trace") or "(names no log)")[:34]))
    L.append("")
    bad = [r for r in rows if r["verdict"] in ("SILENT", "OFF", "NO TRACE")]
