...
TOL = "TOLTRIS PLUS TAB"; TYR = "TYRO BR TAB"; SYP = "LACTOVAX SYP"
...
MARG = {BIO: 400, REM: 60, AXI: 300, PLAIN: 50, TOL: 189, TYR: 290, SYP: 9}
for r in ITEMS:
...
seq = 500
for nm in (BIO, REM, AXI, PLAIN, TOL, TYR, SYP):
    for k in range(3):
        seq += 1
...
for nm in names:
    ...
    tgt = {BIO: q - 60, AXI: q - 286, PLAIN: q - 7, TOL: q - 18, TYR: q - 230, SYP: q - 2}.get(nm, q)
...
with sync_playwright() as pw:
    ...
    chk("AUDIT: loose-is-total instances: 1971 (6, was 12), 2767 (480, was 960), 50428 (300, was 600), 53875 (165, was 315), PO-9 (300, was 600) -- nothing else",
        lt == [("1971", 6, 12), ("2767", 480, 960), ("50428", 300, 600), ("53875", 165, 315), ("PO-9", 300, 600)], lt)
    chk("AUDIT: the odd free quantity is flagged (4 free on 2 paid)", [x["bill_no"] for x in a["odd_free"]] == ["BM-2"], a["odd_free"])
    chk("AUDIT: lines checked = the 9 live-or-duplicate lines + 1 superseded = 10", a["lines"] == 10, a["lines"])
    ...
    chk("AUDIT WORKBOOK: %d rows, the four kinds, a WHY column and a tick column" % len(rows),
        len(rows) == 8 and kinds == ["DOUBLE ENTRY", "FREE TOO HIGH", "LOOSE = TOTAL", "OLD EXPORT"] and ws.cell(row=4, column=11).value.startswith("WHY"), (len(rows), kinds))
    ...
    chk("...PINPOINTED to the span 27-08 -> 02-09 and EXPLAINED: bill 2767 of 27-08-2026 (48 strips) keyed after that day's export; residue left 0",
        ...
        and "48 strips" in sp[0]["explained"]["sentence"] and "keyed in Marg after that day's export" in sp[0]["explained"]["sentence"] and t["residue"] == 0 and not t["lookups"], sp)
    chk("...the sentence speaks in strips, never units", "units" not in t["sentence"] and "+20 strips 1 tab" in t["sentence"] and "is explained (" in t["sentence"], t["sentence"])
    ...
    chk("TYRO BR: the -40 strips is PINPOINTED to 02-09 -> 03-09 and turned into ONE question for Amir",
        ...
        and sp[0]["lookup"].startswith("Marg > item ledger > TYRO BR TAB > 02-09-2026 to 03-09-2026: what voucher moved 40 strips out?") and t["lookups"] == [sp[0]["lookup"]], (t["residue"], sp))
    chk("...in words: '-40 strips moved with NO document behind them, between 02-09-2026 and 03-09-2026'", "-40 strips moved with NO document behind them, between 02-09-2026 and 03-09-2026" in t["sentence"], t["sentence"])
    ...
    chk("LANES: TOLTRIS is NOT a 'Marg moved' line any more -- its move is the late-keyed bill; it lands in loss with the note",
        lane[TOL]["lane"] in ("loss", "allowance") and "bill 2767" in lane[TOL]["why"] and "keyed after that day's export" in lane[TOL]["why"], (lane[TOL]["lane"], lane[TOL]["why"]))
    chk("LANES: TYRO BR is the Marg lane, the reason names the span and the strips, and carries its lookup",
        lane[TYR]["lane"] == "marg" and "-40 strips" in lane[TYR]["why"] and "between 02-09-2026 and 03-09-2026" in lane[TYR]["why"] and "units" not in lane[TYR]["why"] and lane[TYR]["lookups"], (lane[TYR]["lane"], lane[TYR]["why"]))
    ...
    chk("REPORT, TOLTRIS opened: the span line in green with the bill named; no 'units' anywhere in the life",
        "27-08-2026 → 02-09-2026" in life and "bill 2767" in life and "keyed in Marg after that day's export" in life and "units" not in life.lower() and "Marg loose" not in life, life[:900])
    pg.screenshot(path=os.path.join(HERE, "_shot_pin_toltris.png"), full_page=True)
    ...
    chk("REPORT, TYRO opened: the open span with 'For Amir: Marg > item ledger > ...'", "For Amir: Marg > item ledger > TYRO BR TAB > 02-09-2026 to 03-09-2026" in life, life[:900])
    pg.screenshot(path=os.path.join(HERE, "_shot_pin_tyro.png"), full_page=True)
    ...
    card = pg.inner_text("#card")
    chk("DESK: the last card is 'No movement since April -- shelf matches Marg', 20 items carried forward, nothing to decide, Amir's list",
        "NO MOVEMENT SINCE APRIL" in card.upper() and "20 items carried forward" in card and "nothing to decide" in card and pg.locator('#card a[href$="/api/pad/amir/1.xlsx"]').count() == 1, card[:300])
    pg.locator("button[data-list]").click(); pg.wait_for_timeout(200)
    chk("...See the 20 items lists them with 'Marg N strips = shelf'", pg.locator("#list .li").count() == 20 and "= shelf" in pg.inner_text("#list"))
    pg.screenshot(path=os.path.join(HERE, "_shot_pin_deadcard.png"), full_page=True)
