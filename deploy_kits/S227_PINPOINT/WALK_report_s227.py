...
with sync_playwright() as pw:
    ...
    chk("THE DETECTOR: Marg moved %+d, the documents explain %+d (bought %d, sold %d, returned %d), residue %+d" % (MARG_DELTA, DOCS_DELTA, WIN_P_UNITS, WIN_SOLD, WIN_RET, RESIDUE),
        ...
        and t["docs_delta"] == DOCS_DELTA and t["residue"] == RESIDUE and ("NO document" in t["sentence"] if RESIDUE else "accounted for by a document" in t["sentence"]), t)
    ...
    chk("...THE RESIDUE, in one sentence, and what a residue means",
        (("%+d strip" % (RESIDUE // PSX) in lt and "moved with NO document behind them" in lt and "For Amir: Marg > item ledger > " + X in lt) if RESIDUE else "accounted for by a document" in lt) and "A residue is a stock change no document on the server explains" in lt)
