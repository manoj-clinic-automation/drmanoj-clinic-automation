...
WANT = {"item": ("ITEM",), "packing": ("PACKING",), "pack": ("PACK SIZE", "PACKSIZE"),
        "strips": ("STRIPS", "STRIP", "RECOUNT STRIPS"),
        "loose": ("LOOSE", "TABLETS", "PIECES", "RECOUNT LOOSE"),
        "total": ("TOTAL UNITS", "TOTAL", "RECOUNT TOTAL"),
        "remarks": ("REMARKS", "REMARK", "NOTE"),
        "ref": ("SHEET ROW",),
        # S227 DIFF SHEET: the DIFFERENCES tab is a fill-in tab too
        "reason": ("REASON", "REASON (1-7)", "REASON NO", "REASON NUMBER")}
...
def read_pad(data):
    ...
    for name, g in all_g:
        ...
        # S227: a table with a REASON column is the DIFFERENCES tab. Its rows are
        # items ALREADY counted; a blank row there means "no change", and only a
        # row with a recount, a reason or a remark is handed on.
        is_diff = "reason" in cols
        started = False
        for r in range(hdr + 1, maxr + 1):
            ...
            reason = g.get((r, cols.get("reason", -1))) if is_diff else None
            if isinstance(reason, float):
                reason = str(int(reason)) if reason == int(reason) else str(reason)
            reason = (str(reason).strip() or None) if reason is not None else None
            if is_diff and st is None and lo is None and tl is None and not reason and not rem:
                continue
            ...
            rows.append({"row": r, "sheet": name, "item": nm,
                         "kind": ("diff" if is_diff else "count"), "reason": reason,
