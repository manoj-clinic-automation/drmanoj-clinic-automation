...
def result_workbook(R):
    ...
    if mm:
        line("", ""); line("VALUE OF THE STOCK MISMATCH (at MRP, the shop's selling price)", "")
        line("Short on the shelf, at MRP", "%s  (%d line%s)" % (rupees(mm["mrp_short_p"]), mm["short_lines"],
                                                                 "" if mm["short_lines"] == 1 else "s"))
        line("Over on the shelf, at MRP", "%s  (%d line%s)" % (rupees(mm["mrp_over_p"]), mm["over_lines"],
                                                                "" if mm["over_lines"] == 1 else "s"))
        line("Net, at MRP", rupees(mm["mrp_net_p"]))
        line("Lines with no MRP on record", "%d of %d  (the shop has never sold them here)" % (mm["mrp_unpriced"], mm["lines"]))
        line("At cost (last purchase rate), for reference",
             "short %s, over %s, %d line%s without a rate" % (rupees(mm["cost_short_p"]), rupees(mm["cost_over_p"]),
                                                            mm["cost_unpriced"], "" if mm["cost_unpriced"] == 1 else "s"))
    ...
    # ---- DIFFERENCES
    # S227 DIFF SHEET: the third fill-in tab. Type only in the shaded cells:
    # RECOUNT STRIPS / RECOUNT LOOSE after checking the shelf again, REASON as
    # the number from the legend, REMARKS in words. A blank row = no change.
    sh = Sheet()
    sh.text(1, 0, "DIFFERENCES -- counted against Marg. Check these again; write what you find.", TITLE)
    sh.text(2, 0, "Type only in the shaded cells. RECOUNT = count the shelf again and write strips / loose "
                  "(leave blank if the first count was right). REASON = a number: 1 count error, 2 not billed, "
                  "3 breakage, 4 expiry, 5 sample / given to doctor, 6 return, 7 don't know. "
                  "A blank row means no change. Save and upload at step 3.", NOTE)
    sh.merges.append("A2:K2")
    sh.text(4, 2, "AS PER MARG", GROUPGREY)
    sh.text(4, 3, "COUNTED ON THE SHELF", GROUPGREY)
    sh.text(4, 4, "DIFFERENCE = counted minus Marg", GROUPGREY)
    sh.text(4, 7, "YOUR ANSWER -- shaded cells", GROUPYELLOW)
    heads = ["ITEM", "PACKING", "MARG STOCK (strips & tabs)", "PHYSICAL COUNT (strips & tabs)",
             "DIFFERENCE (strips & tabs)", "AT MRP", "AT COST",
             "RECOUNT STRIPS", "RECOUNT LOOSE", "REASON (1-7)", "REMARKS"]
    for i, h in enumerate(heads):
        sh.text(5, i, h, HEADER)
    rr = 6
    diffs = sorted(R["differences"], key=lambda d: ((d[6] if len(d) > 6 else None) is None,
                                                    -abs((d[6] if len(d) > 6 else 0) or 0),
                                                    -abs(d[5] or 0), -abs(d[4] - d[3])))
    for d_ in diffs:
        item, packing, ps, marg, counted, value_p = d_[:6]
        mrp_p = d_[6] if len(d_) > 6 else None
        d = counted - marg
        ...
        sh.text(rr, 5, rupees(mrp_p) if mrp_p is not None else "no MRP on record", BOX)
        sh.text(rr, 6, rupees(value_p) if value_p is not None else "no rate on record", BOX)
        sh.num(rr, 7, None, INPUT); sh.num(rr, 8, None, INPUT)
        sh.text(rr, 9, "", INPUT); sh.text(rr, 10, "", INPUT)
        rr += 1
    sh.widths = {0: 30, 1: 10, 2: 22, 3: 22, 4: 22, 5: 16, 6: 16, 7: 9, 8: 9, 9: 12, 10: 30}
    sh.freeze = "A6"
    if rr > 6:
        sh.filter = "A5:K%d" % (rr - 1)
