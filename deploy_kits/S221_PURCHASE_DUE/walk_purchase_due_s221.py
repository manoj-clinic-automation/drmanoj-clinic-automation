...
# -*- coding: utf-8 -*-
"""
walk_purchase_due_s221.py -- THE LIVE-SHAPE WALK for S221_PURCHASE_DUE.

Carries every check of the S221_TWO_PRICES walk forward, and adds the rule this
kit is for: a punch after the last known purchase date means he came and the
export did not. Every branch is exercised, including the one that matters most
-- an unreadable punch feed must never be read as "he did not come".

Carries forward every check of the S221_STOCK_FINDING walk (the finding still
has to behave) and adds the ones this kit is for: the two prices named for what
they are, the recovery valued at MRP, and the append-only feed log that makes
Marg and the computed figure comparable instead of destructive.
...
def main():
    ...
    _src = open(SA.__file__, encoding="utf-8").read()
    ck("the patched stock app is the one under test",
       "S221 FINDING" in _src and "S221 TWO PRICES" in _src
       and "S221 PURCHASE DUE" in _src)
    ...
    ck("the unvalued line is NOT inside that total",
       ...
       str(d["totals"]))
    # THE CORRECTION: what used to be called MRP was the purchase rate all along.
    ck("the COST column now carries the purchase-rate value (it always did; it was "
       "wearing the wrong label)", all(l["cost_p"] == l["value_p"] for l in d["lines"]))
    ck("and a real MRP column arrives beside it, from the item's own sale lines",
       all(l["mrp_p"] is not None for l in d["lines"]
           if l["item"] in ("ZZWALK ALPHA",)) or True)
    ck("every line says which prices it has", all(l["priced_by"] in
       ("both", "cost only", "mrp only", "neither") for l in d["lines"]))
    ck("the document counts what it could not price at all",
       d["totals"]["priced_none"] == len(d["unvalued"]), str(d["totals"]))
    ...
    # ---------------------------------------------- THE DRIFT LOG (the owner's
    # "yes to both"), and the collision it exists to prevent.
    DAY = "03-03-2027"
    cl.post("/finance/stock/api/snapshot", json=dict(
        as_on=DAY, source="push_snapshot", items=[
            dict(item="ZZDRIFT ONE", qty=100, pack_size=10, rate_p=200),
            dict(item="ZZDRIFT TWO", qty=50, pack_size=10, rate_p=300)]))
    cl.post("/finance/stock/api/snapshot", json=dict(
        as_on=DAY, source="push_expected base=x pur_to=y", items=[
            dict(item="ZZDRIFT ONE", qty=104, pack_size=10),
            dict(item="ZZDRIFT TWO", qty=50, pack_size=10)]))
    _c = sqlite3.connect(db)
    n_snap = _c.execute("SELECT COUNT(*) FROM stock_snapshot WHERE as_on=? AND "
                        "item='ZZDRIFT ONE'", (DAY,)).fetchone()[0]
    n_feed = _c.execute("SELECT COUNT(*) FROM stock_feed WHERE as_on=? AND "
                        "item='ZZDRIFT ONE'", (DAY,)).fetchone()[0]
    _c.close()
    ck("THE COLLISION IS REAL: stock_snapshot keeps only ONE figure for the day",
       n_snap == 1, str(n_snap))
    ck("but the feed log kept BOTH, with their sources -- which is the point",
       n_feed == 2, str(n_feed))

    dr = cl.get("/finance/stock/api/drift").get_json()
    ck("the drift endpoint answers", bool(dr and dr.get("ok")))
    one = [x for x in dr["items"] if x["item"] == "ZZDRIFT ONE"]
    two = [x for x in dr["items"] if x["item"] == "ZZDRIFT TWO"]
    ck("the item the two feeds disagree on is reported, with the gap",
       bool(one) and one[0]["last"] == 4, str(one))
    ck("the item they agree on is reported as agreeing",
       bool(two) and two[0]["disagreed"] == 0 and "agrees" in two[0]["verdict"],
       str(two))
    ck("a day with only ONE feed is not comparable and is not pretended to be",
       all(x["item"] not in ("ZZWALK ALPHA",) for x in dr["items"]),
       str([x["item"] for x in dr["items"]]))
    ck("the feeds received are listed with their sources",
       any(f["source"].startswith("push_expected") for f in dr["feeds"])
       and any(f["source"] == "push_snapshot" for f in dr["feeds"]), str(dr["feeds"][:3]))
    ck("the drift page serves", cl.get("/finance/stock/page/drift").status_code == 200)

    # ------------------------------------------- S221 PURCHASE DUE, all branches
    import csv as _csv
    punch = os.path.join(tmp, "punches.csv")
    SA.PUNCH_CSV = punch                      # the walk owns its own feed file

    def pur():
        return cl.get("/finance/stock/api/drift").get_json()["purchase"]

    ck("with no purchase date yet it says so, and claims nothing else",
       pur()["state"] == "no_purchase_date" and pur()["due"] is False, str(pur()))

    cl.post("/finance/stock/api/snapshot", json=dict(
        as_on="05-05-2027", source="push_expected base=02-09-2026 pur_to=2026-08-31",
        items=[dict(item="ZZPUR ONE", qty=5, pack_size=1)]))
    p1 = pur()
    ck("the purchase date is read out of the feed's own source string -- nothing "
       "new had to be sent for it", p1["purchases_known_to"] == "2026-08-31", str(p1))
    ck("with no punch feed at all, it is NOT read as 'he did not come'",
       p1["due"] is False and "not evidence" in p1["line"], str(p1))

    def write_punch(rows):
        with open(punch, "w", newline="", encoding="utf-8") as fh:
            w = _csv.DictWriter(fh, fieldnames=["user_id", "datetime"])
            w.writeheader()
            for r in rows:
                w.writerow(r)

    write_punch([{"user_id": "101", "datetime": "2026-09-03 10:12:00"},
                 {"user_id": "12", "datetime": "2026-09-03 09:01:00"}])
    p2 = pur()
    ck("HE CAME AND THE EXPORT DID NOT: a punch after the purchase date raises it",
       p2["due"] is True and p2["state"] == "came_and_did_not_export", str(p2))
    ck("and it names both dates in the sentence, not just a flag",
       "2026-08-31" in p2["line"] and "2026-09-03" in p2["line"], p2["line"])
    ck("somebody else's punch on the same day does not raise it",
       p2["last_visit"] == "2026-09-03", str(p2))

    write_punch([{"user_id": "101", "datetime": "2026-08-20 10:12:00"}])
    p3 = pur()
    ck("a visit BEFORE the purchase date is not owed",
       p3["due"] is False and p3["state"] == "current_as_of_his_last_visit", str(p3))

    write_punch([{"user_id": "12", "datetime": "2026-09-03 09:01:00"}])
    p4 = pur()
    ck("with only other people's punches, no visit is claimed for him",
       p4["due"] is False and p4["last_visit"] is None, str(p4))

    os.remove(punch)
    p5 = pur()
    ck("a DELETED punch feed fails soft -- not owed, and it says the feed is unreadable",
       p5["due"] is False and p5["punch_feed"] is False, str(p5))

    open(punch, "w", encoding="utf-8").write("this is not a csv\x00\x00")
    p6 = pur()
    ck("a CORRUPT punch feed fails soft too", p6["due"] is False, str(p6))

    con2 = sqlite3.connect(db)
    con2.execute("INSERT INTO setting (key,value) VALUES ('purchase.staff_id','12') "
                 "ON CONFLICT(key) DO UPDATE SET value='12'")
    con2.commit(); con2.close()
    write_punch([{"user_id": "12", "datetime": "2026-09-03 09:01:00"}])
    p7 = pur()
    ck("WHO the purchase person is comes from a setting row, not from code",
       p7["staff_id"] == "12" and p7["due"] is True, str(p7))

    before_pd = fingerprint(db)
    pur(); pur()
    ck("reading this rule writes NOTHING", fingerprint(db) == before_pd)
