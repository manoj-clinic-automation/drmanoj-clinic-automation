...
def _r_stock(con):
    ...
    try:
        ...
        for r in con.execute(
                "SELECT as_on, source, COUNT(*), MIN(received_at), MAX(received_at) "
                "FROM stock_feed GROUP BY as_on, source"):
            k = _feed_kind(r[1])
            if k not in ("marg", "expected"):
                continue
            cur = newest.get(k)
            if cur is None or (str(r[0]), str(r[4])) > (cur["as_on"], cur["processed_at"]):
                newest[k] = dict(as_on=str(r[0])[:10], source=str(r[1]),
                                 items=int(r[2] or 0), processed_at=str(r[4] or ""))
