#!/bin/bash
# =============================================================================
#  install_v1b.sh · kit S183_V1b — the F-97 structural fix.
#
#  WHAT THIS INSTALLS
#     /root/deploy/verify_live_pins.py   the live-code pin checker (read-only)
#     /root/deploy/live_pins.txt         the pins, generated from KB Register v5.4
#
...
&& { [ -f "$DEST/live_pins.txt" ] && cp -f "$DEST/live_pins.txt" "$DEST/live_pins.txt.bak_$KIT_NAME" || true; } \
