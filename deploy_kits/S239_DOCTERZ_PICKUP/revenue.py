...
def split_payment(amount_collected, mode_of_payment) -> tuple[float, float]:
    ...
    S239 (the S223 parser fix, proven offline on every retained export): Docterz writes
    SEVEN tender names inside the bracket -- Cash, Credit Card, Debit Card, Net Banking,
    Online Payment, Patient APP, Wallet -- and the old reader looked for two, so a
    "1100 (Wallet: 600, Online Payment: 500)" came out as (0, 500) and Rs 600 walked past.
    Now every "Name: amount" pair is read; `online` is EVERY non-cash tender, so
    cash + online is what the patient actually paid. A tender name never seen before is
    counted as non-cash and printed loudly -- never silently dropped."""
    raw = "" if amount_collected is None else str(amount_collected)
    pairs = _TENDER_PAIR.findall(raw)
    if pairs:
        cash = online = 0.0
        for k, v in pairs:
            key = " ".join(k.strip().casefold().split())
            amt = _num(v)
            if key == "cash":
                cash += amt
            else:
                if key not in _TENDER_NAMES:
                    print("  [revenue] LOUD: unknown tender name %r in %r -- counted as non-cash"
                          % (k.strip(), raw))
                online += amt
        return cash, online
    ...
    if "cash" in mode:
        return total, 0.0
    if mode and total:
        return 0.0, total          # any named non-cash mode (online, UPI, card, wallet, app, bank)
    # Unknown/blank mode (usually a ₹0 free visit) — nothing collected.
    return 0.0, 0.0


_TENDER_PAIR = re.compile(r"([A-Za-z][A-Za-z .]*?)\s*:\s*([\d,]+(?:\.\d+)?)")
_TENDER_NAMES = {"cash", "credit card", "debit card", "net banking", "online payment",
                 "patient app", "wallet"}
...
def parse_day_revenue(filepath: str) -> tuple[pd.DataFrame, str]:
    ...
    # S239 (F-93): only a real Patient UID makes a patient row -- the same guard the
    # processor has applied since S135. Docterz's payment-mode footer below the "Total"
    # row carries cells such as "Credit Card" and "0" in the UID column, and those used
    # to reach the Day Revenue sheet as phantom visits.
    raw = raw[raw.get("Patient UID", "").astype(str).str.strip()
              .str.fullmatch(r"[A-Z0-9]{8,14}", na=False)].reset_index(drop=True)
