...
def find_workbook():
    ...
    if not cands:
        return None
    # S239: the LATEST CALL DAY by the date in the name, not the file written last -- on 11-Sep an
    # old day re-run at 08:08 (08-Sep) was the newest file and replaced today's call list.
    return max(cands, key=lambda p: (os.path.basename(p)[19:29], os.path.getmtime(p)))
