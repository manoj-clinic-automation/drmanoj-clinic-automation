...
set BASELINE=03-09-2026

REM  S260 (C.3): the baseline now lives with the machine's other settings, in
REM  D:\Downloads\margsync\_config\machine.conf, so a re-baseline is one line
REM  in one file. The date above is what this file has always used, so a
REM  missing file or a blank value changes nothing.
...
REM  S259 -- THE OFF SWITCH IS A FILE. Either marker stops this run; delete it
REM  and the next run goes ahead. Nothing to restart, nothing to re-register.
if exist "%OFFDIR%\ALL_OFF.txt" goto :off
if exist "%OFFDIR%\PUSH_STOCK_OFF.txt" goto :off
...
:off
echo.>> "%LOG%"
echo ======== %DATE% %TIME%   OFF - a marker in %OFFDIR% is present; nothing sent >> "%LOG%"
echo.
echo  OFF - the stock push is switched off.
echo  A marker file in this folder is what does it:
echo     %OFFDIR%
echo  Double-click this to switch everything back on:
