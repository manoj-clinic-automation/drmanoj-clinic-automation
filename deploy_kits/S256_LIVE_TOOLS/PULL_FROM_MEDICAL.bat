...
REM  S260 (C.3): and now that one place is the MACHINE's, not this file's.
REM  D:\Downloads\margsync\_config\machine.conf holds it; the numbers below
REM  are what this file has always used, so a missing file changes nothing.
set "MEDHOST=100.119.151.40"
set "MEDSHARE=DDrive"
set "S260CONF=D:\Downloads\margsync\_config\machine.conf"
...
REM ---- S225 (06-Sep-2026): PUSH ON CAPTURE, IN THE SAME CYCLE ----------------
REM  The owner: "for the morning flow they will not wait at all". Until now the
REM  two on-capture pushers ran on their own 15-minute clocks, so a report
REM  archived by THIS cycle could wait up to 15 more minutes before Marg's
REM  stock figure and our computed figure reached the server. Now they run
REM  here, right after the archive and the outbox, so everything a cycle
REM  captured is on the server before the cycle ends. Both are quiet when
REM  nothing is new, and neither can fail this pull -- their own logs in
REM  _analysis\ say what they did. The 15-minute tasks stay as backstops.
echo.
echo  Pushing on capture: Marg's stock figure, then our computed figure...
if exist "%HERE%snapshot_on_capture.py" "%PY%" -B "%HERE%snapshot_on_capture.py" >> "D:\Downloads\margsync\_analysis\snapshot_on_capture_console.log" 2>&1
if exist "%HERE%expected_on_capture.py" "%PY%" -B "%HERE%expected_on_capture.py" >> "D:\Downloads\margsync\_analysis\expected_on_capture_console.log" 2>&1
