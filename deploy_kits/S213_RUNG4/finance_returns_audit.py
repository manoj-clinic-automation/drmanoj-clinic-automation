...
v2 · S213: ladder rung 4 reworked through finance_money (it compared a sum of
RATES against a bill's MONEY -- the confusion behind the withdrawn Rs 38,157).
Everything else is byte-identical to the S212 version.
"""
import collections
import os
import re
...
def find_return_lines(con, bill_no, business_date, amount_p=None):
    ...
    if business_date and amount_p is not None:
        # last rung: the same day, flagged a return, and the bill's MONEY is
        # reproduced by the lines. S212 shipped this rung comparing a SUM OF
        # RATES against money -- the exact confusion finance_money.py exists
        # to end, and the same arithmetic that produced the withdrawn
        # Rs 38,157. Reworked at S213 (v2): every candidate's lines are
        # VALUED through bill_gross_p, and a candidate with ANY unreadable
        # line is skipped rather than compared on a total that silently
        # omits it. Still accepted only when exactly ONE bill fits (Rs 1).
        cand = collections.defaultdict(list)
        for c0 in con.execute(
                "SELECT bill_no, qty_raw, pack, amount_p FROM sale_line_item "
                "WHERE is_return=1 AND business_date=?", (business_date,)):
            cand[c0["bill_no"]].append(dict(qty_raw=c0["qty_raw"],
                                            pack=c0["pack"],
                                            amount_p=c0["amount_p"]))
        fit = []
        for bno, lns in cand.items():
            gross, bad = bill_gross_p(lns)
            if bad:
                continue          # a partial total must not identify a bill
            if abs(gross - abs(amount_p)) <= 100:
                fit.append(bno)
        if len(fit) == 1:
            r = con.execute("SELECT %s FROM sale_line_item WHERE bill_no=? ORDER BY seq"
                            % LINE_COLS, (fit[0],)).fetchall()
            if r:
                return r, "same day and the money agrees (bill %s)" % fit[0]
    return [], ""
