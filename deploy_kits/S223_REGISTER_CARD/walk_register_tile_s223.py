#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""walk_register_tile_s223.py -- who gains the Daily Register tile, measured over every combination."""
import io, os, sys
...
ck("the only thing gained anywhere is Daily Register",
   all(set(after[k]) - set(before[k]) == {"Daily Register"} for k in changed))
ck("nothing is lost anywhere", all(not (set(before[k]) - set(after[k])) for k in changed))
print("\n-- 2  the owner's list, exactly -------------------------------------")
got_staff = {u for u in USERS if "Daily Register" in after[(u, "staff", False)]}
...
ck("the owner and Dr Bhawna have it as doctors",
   all("Daily Register" in after[(u, "doctor", False)] for u in ("manoj", "bhawna")))
ck("darpan does NOT have it (he was not on the list)",
   "Daily Register" not in after[("darpan", "staff", False)])
ck("amir does NOT have it", "Daily Register" not in after[("amir", "staff", False)])
ck("a brand-new staff login does NOT have it",
   "Daily Register" not in after[("nobody", "staff", False)])
print("\n-- 3  fail-safe: grants file gone -----------------------------------")
fb = snap(NEW_P, None)
ck("no staff login sees it without the grants file (fail CLOSED)",
   not any("Daily Register" in fb[(u, "staff", False)] for u in USERS))
ck("the owner still sees it without the grants file (never locked out)",
   "Daily Register" in fb[("manoj", "doctor", False)])
print("\n-- 4  where it lands ------------------------------------------------")
