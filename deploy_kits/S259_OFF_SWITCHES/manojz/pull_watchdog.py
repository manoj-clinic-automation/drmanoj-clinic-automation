...
the server has every channel and a state of 'asleep' is what it acts on.
    ...
    OFF switch (S259): D:\Downloads\margsync\_off\ALL_OFF.txt  or  PULL_WATCHDOG_OFF.txt
...
DEF_OFFDIR = os.path.join(MARGSYNC, "_off")
OFF_MARKERS = ("ALL_OFF.txt", "PULL_WATCHDOG_OFF.txt")
MAX_AGE_MIN = 35
...
def off_marker(offdir):
    """S259 -- THE OFF SWITCH IS A FILE. Its presence stops this job; deleting it
    starts it again. Nothing is restarted either way: the next 15-minute run
    reads the folder and obeys what it finds. The same shape as the server's
    own /root/finance/AUTOAPPLY_OFF (D488).

    Returns the marker's full path, or None."""
    for name in OFF_MARKERS:
        p = os.path.join(offdir, name)
        if os.path.isfile(p):
            return p
    return None
...
def main(argv=None):
    ...
    ap.add_argument("--off-dir", default=DEF_OFFDIR)
    ...
    off = off_marker(a.off_dir)
    if off:
        msg = "%s OFF -- %s is present; nothing checked, nothing sent" % (
            now.strftime("%Y-%m-%d %H:%M"), off)
        print(msg)
        if not a.dry_run:
            log(a.logdir, msg)
        return 0
