...
OFF SWITCH (S259) -- D:\SendToClinic\_off\ALL_OFF.txt  or  MARG_PUSH_OFF.txt. Read at the top of
    every pass, so switching off or back on needs no restart. Capture is NOT stopped by it.
"""
import argparse
import hashlib
import json
...
OFF_MARKERS = ("ALL_OFF.txt", "MARG_PUSH_OFF.txt")
...
def off_marker(offdir=None):
    """S259 -- THE OFF SWITCH IS A FILE. Its presence stops the sending; deleting
    it starts it again. It is read at the top of EVERY pass, so neither
    direction needs the watcher restarted. Capture is not affected by this
    switch at all -- see D:\\SendToClinic\\_off\\READ_ME.txt.

    Returns the marker's full path, or None."""
    d = offdir or OFFDIR
    for name in OFF_MARKERS:
...
def run_once(spool=SPOOL, out=None, limit=PER_PASS):
    ...
    off = off_marker()
    if off:
        say("  pusher: OFF -- %s is present; nothing sent" % off)
        return 0, 0, 0
    s = state_read()
    tok = token()
    if not tok:
...
def loop(spool=SPOOL, out=None, every=EVERY, stop=None, wake=None):
    ...
    while True:
        ...
        if bool(off) != was_off:
            if off:
                say("  pusher: OFF -- %s" % off)
                log("pusher OFF (%s)" % os.path.basename(off))
            else:
                say("  pusher: on again")
                log("pusher ON")
            was_off = bool(off)
...
def selftest():
    ...
    od = _tf.mkdtemp()
    ck("no marker means the pusher is on", off_marker(od) is None)
    open(os.path.join(od, "MARG_PUSH_OFF.txt"), "w").write("off")
    ck("its own marker switches the pusher off", off_marker(od) is not None)
    os.remove(os.path.join(od, "MARG_PUSH_OFF.txt"))
    ck("removing the marker switches it on again", off_marker(od) is None)
    open(os.path.join(od, "ALL_OFF.txt"), "w").write("off")
    ck("ALL_OFF switches the pusher off too", off_marker(od) is not None)
