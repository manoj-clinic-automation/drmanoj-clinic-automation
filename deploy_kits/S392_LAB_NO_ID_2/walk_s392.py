#!/usr/bin/env python3
"""walk_s392.py -- kit S392_LAB_NO_ID_2 (S391 walk + originals that arrive after their resends). The real S391 slip_log.py over a SCRATCH COPY of the live finance.db, on the
...
The live slip_log.py (S391) is the negative control. Prints IDs and counts only.
...
# S392: the ORIGINALS of 21/22-Sep reach the server AFTER today's resends (the mailbox reads newest first)
A0, S0 = build(live, "old")
ok(S0.noid_match(con, "MRS. CHANDA", D1 + " 19:26:00") == "",
   "negative control: the live slip_log (S391) cannot place an original once its resend took the order -- 24-Sep's 9 unsure lines")
ok(S.noid_match(con, "MRS. CHANDA", D1 + " 19:26:00") == "909301", "S392 follows the later copy")
orig = [("walk391r", "MRS. CHANDA", D1 + " 19:26:00", "909301"), ("walk391s", "MR. BHUPENDRA KUMAR", D1 + " 19:27:00", "909302"),
        ("walk391t", "Mr. .KAMLESH DEVI", D1 + " 19:27:30", "909308"), ("walk391u", "MR. PRAMOD KUMAR", D1 + " 19:28:00", "909304"),
        ("walk391v", "MRS. SAVITA", D1 + " 19:28:30", "909303"), ("walk391w", "MRS. TAHSEENA", D2 + " 20:05:00", "909309"),
        ("walk391x", "MRS. TAHSEENA", D2 + " 20:05:30", "909309"), ("walk391y", "MR. RAJVEER PRATAP", D2 + " 20:05:40", "909306"),
        ("walk391z", "MR. ANUJ", D2 + " 20:06:00", "909307"), ("walk391zz", "MR. LAKHANWALK", D1 + " 19:28:10", "")]
g3 = {x["msg_id"]: x["clinic_id"] for x in c.post("/finance/slips/api/lab-noid", headers=TOK, json={"reports": [
    {"msg_id": m, "name": n, "received_at": t} for m, n, t, _w in orig]}).get_json()["reports"]}
ok(g3 == {m: w for m, _n, _t, w in orig}, "all nine originals join their patient; a name with no order at all still waits", g3)
ok(all(r["received_at"][:10] in (D1, D2) for r in (S._report_for(con, cid, d) for cid, d in (("909301", D1), ("909309", D2)))),
   "the line's report date is now the original's day, not the resend's")
con.execute("DELETE FROM lab_noid WHERE msg_id='walk391zz'"); con.commit()
...
# negative control
ok({r.rule for r in A.url_map.iter_rules()} == {r.rule for r in A0.url_map.iter_rules()}, "routes identical to S391")
for t in ("blood_order", "lab_report", "patient_ref"):
    con.execute("DELETE FROM %s WHERE clinic_id LIKE '9093%%'" % t)
con.execute("DELETE FROM lab_noid WHERE msg_id LIKE 'walk391%'")
con.commit()
