...
def api_cash_position():
    ...
    held = {r["party"]: int(r["held_p"] or 0)
            for r in con.execute("SELECT party, held_p FROM v_cash_custody_balance "
                                 "WHERE unit=?", (UNIT,))}
    reserve_p = max(0, held.get("dr_bhawna", 0))
    manoj_p = max(0, held.get("dr_manoj", 0))
    ...
    rows = con.execute("SELECT business_date, closing_p FROM v_cash_ledger "
                       "WHERE unit=? ORDER BY business_date", (UNIT,)).fetchall()
    days = [dict(date=r["business_date"],
                 drawer=rupees(int(r["closing_p"] or 0) - parked_p),
                 unbanked=rupees(int(r["closing_p"] or 0)))
            for r in rows]
    latest = int(rows[-1]["closing_p"] or 0) if rows else 0
    return jsonify(ok=True,
                   drawer=rupees(latest - parked_p),
                   ...
                   unbanked=rupees(latest),
                   as_of=(rows[-1]["business_date"] if rows else None),
                   days=days[-30:])
