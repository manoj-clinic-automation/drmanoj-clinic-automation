#!/bin/bash
# =============================================================================
#  install_S342_CHECK_KAREIN_2.sh · kit S342_CHECK_KAREIN_2 (session 273, 20-Sep-2026)
#  S342 = S340's payload, byte-identical, with the walk corrected: S340 was REFUSED at its own walk on the box
#  (nothing installed) because the live data already held a real unknown-ID lab report and the walk counted
#  "exactly one". Seed/patch scripts keep their s340 names -- they are S340's files, unchanged.
#
#  Run by (one line on the VPS):
#    cd /root/deploy/repo && git pull --ff-only && bash /root/deploy/repo/deploy_kits/S342_CHECK_KAREIN_2/install_S342_CHECK_KAREIN_2.sh
#
...
"$SPY" -m py_compile records.py seed_s340.py walk_s342.py "$WALK/finance_app.py" && "$VPY" -m py_compile portal.py \
...
WOUT="$( cd "$WALK/app" && FINANCE_DB="$WALK/walk.db" FINANCE_ALLOW_HEADER_AUTH=1 RECORDS_DRIVE_STUB="$WALK/stub" \
         FINANCE_SSO_DIR="$POR" PETTY_UPLOAD_DIR="$WALK/uploads" timeout 170 "$VPY" -B "$KDIR/walk_s342.py" "$WALK/app" 2>&1 | tail -1 )"
echo "$WOUT" | grep -q "^WALK OK" || { say "!! [4/8] walk red: $WOUT - nothing installed"; rm -rf "$WALK"; exit 1; }
say "[4/8] $WOUT"
declare -A BAK
for f in "${ORDER[@]}"; do BAK[$f]="${DEST[$f]}.bak_S342_$(m5 "${DEST[$f]}" | cut -c1-8)"; \cp -p "${DEST[$f]}" "${BAK[$f]}" || { say "!! [5/8] backup failed - nothing placed"; rm -rf "$WALK"; exit 1; }; done
restore() {
...
say "[5/8] placed, backups beside each file (.bak_S342_<from8>)"
