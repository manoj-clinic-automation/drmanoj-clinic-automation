...
Records step 3 (owner, S271_BUILD_BRIEF §2.5). **Check karein** — one reception queue in Hindi (Alisha, Shivani, reception; Shavez the checker), one-tap answers with who and when, oldest first. Items now: a blood test ordered with no report after 2 days (not tested / asked the lab — "asked" comes back after 3 days if nothing arrives) · a lab report for an ID the clinic never issued (confirm, or type the right ID — the file moves to that patient) · a lab e-mail with no PDF. X-ray and WhatsApp items join when those steps go live. The doctors see **one collapsed line** on the records page — "Patient records: all clear" or "N need a look · oldest D days" — opening to the items and the last 7 days' answers. Walk: 131 checks. **S342 = S340 unchanged except the walk**: S340 refused itself on the box (nothing installed) because live data already held a real lab report for an unknown ID and its walk counted "exactly one"; the walk now finds its own rows by key.
