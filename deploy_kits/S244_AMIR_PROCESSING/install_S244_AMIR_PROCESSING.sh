#!/bin/bash
# =============================================================================
#  install_S244_AMIR_PROCESSING.sh -- kit S244_AMIR_PROCESSING
#  Amir's step 4: processing -> processing done -> what is still to be made.
#
#  Run by:  bash /root/deploy/repo/deploy_kits/S244_AMIR_PROCESSING/install_S244_AMIR_PROCESSING.sh
#
#  A  /root/finance/amir_day.py   FULL-FILE replacement.  Refuses unless the live file is
#                                 EXACTLY the S243_AMIR_VISIT pin bf7d9826...  The kit's file
...
#  62-check walk against the kit file ON THIS BOX -> .bak_S244_<pin8> -> place -> py_compile
...
AD_FROM_PIN="${MOCK_AD_PIN:-bf7d9826119edab48652fd959e1e8da5}"
AD_MARK="S244_AMIR_PROCESSING"
...
"$PY" -B "$KDIR/patch_amir_day_processing_s244.py" --selftest "$ADF" "$KDIR/amir_day.py" >/dev/null 2>&1 || \
...
WALK_RC=0
WALK_OUT="$("$PY" -B "$KDIR/walk_amir_processing_s244.py" "$KDIR/amir_day.py" 2>&1)" || WALK_RC=$?
[ "$WALK_RC" -eq 0 ] || red "the kit's own walk failed ON THIS BOX; NOTHING changed:
$(echo "$WALK_OUT" | tail -12)"
echo "-- walk on this box: $(echo "$WALK_OUT" | grep -E '^== [0-9]+ checks' | tail -1)"

# ---- backup, then place
AD_BAK="$ADF.bak_S244_${LIVE_AD:0:8}"
\cp -f "$ADF" "$AD_BAK"
...
print('import ok: amir_day mounted, every route still registered, grace %d min' % amir_day.EXPORT_GRACE_MIN)" 2>&1)" || SMOKE_RC=$?
...
echo "   read next: Amir's step 4 at /finance/amir/step/4 -- it now says 'ban rahi hai' while a file is in transit"
exit 0
