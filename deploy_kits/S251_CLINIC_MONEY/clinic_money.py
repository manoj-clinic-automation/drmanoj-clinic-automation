...
_unit = "clinic"
PHYSIO_UNIT = "physio"
APP_VERSION = "S249-CLINIC-MONEY-1.0"
...
def match_day(con, d, now=None):
    ...
    if reg is None:
        ...
        res["lines"] = ["Counter sheet not filled — nothing can be matched until it is."]
        return res
    if not doc["known"]:
        ...
        res["lines"] = ["Waiting for the Docterz export; nothing to do yet."]
        return res
    ...
    head_diffs = [(h, x) for h, _a, _b, x in p2 if x]
    ...
    if diff1 == 0:
        ...
        where = ", ".join("%s %s₹%s" % (_HEAD[h], "+" if x > 0 else "−", _r(abs(x))) for h, x in head_diffs) or "no head differs"
        how = ", ".join("%s %s₹%s" % (_TENDER[t], "+" if x > 0 else "−", _r(abs(x))) for t, x in tender_diffs) or "no tender differs"
        hint = ""
        wf = _whole_fee(diff1)
        if wf:
            hint = " That is exactly %d × ₹%s — the counter is %s consultation%s %s." % (
                wf[0], _r(wf[1]), wf[0] if wf[0] > 1 else "one", "s" if wf[0] > 1 else "", "ahead" if diff1 > 0 else "behind")
        text = ("The counter sheet says ₹%s, Docterz says ₹%s — counter is ₹%s %s. Where: %s. How: %s.%s"
                % (_r(r_total), _r(d_total), _r(abs(diff1)), "higher" if diff1 > 0 else "lower", where, how, hint))
    ...
    if not flags:
        ...
        res["lines"] = ["Nothing to do — the counter, Docterz and the bank agree" + (", and every difference is explained." if explained else ".")]
    else:
        res["verdict"] = "flags"
        n = len(flags)
        res["lines"] = ["%d thing%s need%s a person; everything else is explained." % (n, "s" if n > 1 else "", "" if n > 1 else "s")]
    return res
...
def _float_block(con, d):
    ...
    comp = {}
    for ev in con.execute("SELECT * FROM clinic_float_event WHERE kind='issue' ORDER BY id"):   # the agreed composition
        for k in FLOAT_KEYS:
            comp[k] = comp.get(k, 0) + (ev[k] or 0)
    if not any(comp.values()):
        comp = dict(FLOAT_DEFAULT)
    cells = []
    for rs, k in FLOAT_NOTES:
        v = row[k] if row is not None else comp.get(k, 0)
        cells.append("<tr><th class='sec'>&#8377; %d</th><td><input name='%s' value='%s' inputmode='numeric' pattern='[0-9]*' "
                     "autocomplete='off' class='amt'></td><td class='mut'>standing %s</td></tr>"
                     % (rs, k, _esc("" if not v else v), comp.get(k, 0) or "—"))
    status = ""
    if row is not None:
        short = standing - row["kept_p"]
        status = ("<p class='verdict %s'>Kept aside for tomorrow: &#8377; %s of &#8377; %s%s.%s</p>"
                  % ("all_agree" if short <= 0 else "waiting_bank", _r(row["kept_p"]), _r(standing),
                     (" — &#8377; %s short, made up next time" % _r(short)) if short > 0 else "",
                     (" Change needed: <b>%s</b>" % _esc(row["need"])) if row["need"] else ""))
    return """
      <div class="card" id="float"><h2>The float — kept aside for tomorrow</h2>
        <p class="mut">Reception ka chutta (&#8377; %s: %s). Din ke ant mein yeh kal ke liye alag rakho — kitne note rakhe, yahan likho.
        Yeh din ki kamai ka hissa <b>nahi</b> hai. Opened today with &#8377; %s.</p>
        <form method="post" action="/finance/clinic/money/%s/float">
        <table class="grid entry"><thead><tr><th>Note</th><th>How many kept</th><th></th></tr></thead><tbody>%s</tbody></table>
        <p><label class="mut" for="need">Need change? <span class="hint">(e.g. 5 × &#8377;50)</span></label><br>
           <input id="need" name="need" class="note" maxlength="120" value="%s"></p>
        <button type="submit" class="save">Save the float count</button>
        </form>%s
      </div>""" % (_r(standing), ", ".join("%d × &#8377;%d" % (comp[k], rs) for rs, k in FLOAT_NOTES if comp.get(k)),
                   _r(open_p), d, "".join(cells), _esc(row["need"] if row is not None else ""), status)
...
def post_float(date):
    ...
    if bad:
        return _shell("The float", "<div class='card'><div class='bad'><b>Nothing was saved.</b> Whole numbers of notes only.</div>"
                      "<p class='navrow'><a class='btn' href='/finance/clinic/register/%s#float'>back</a></p></div>" % date), 400
    kept = sum(rs * 100 * vals[k] for rs, k in FLOAT_NOTES)
    who, now = u.get("user", ""), _stamp()
    open_p = float_open_p(con, date)
    need = (request.form.get("need") or "").strip()[:120]
    ...
    if before is None:
        con.execute("INSERT INTO clinic_float_day (business_date, %s, kept_p, open_p, standing_p, need, note, by_whom, at) "
                    ...
                    [date] + [vals[k] for k in FLOAT_KEYS] + [kept, open_p, standing, need, note, who, now])
    else:
        con.execute("UPDATE clinic_float_day SET %s, kept_p=?, open_p=?, standing_p=?, need=?, note=?, by_whom=?, at=? WHERE business_date=?"
                    % ",".join("%s=?" % k for k in FLOAT_KEYS),
                    [vals[k] for k in FLOAT_KEYS] + [kept, open_p, standing, need, note, who, now, date])
    # the handover expectation stored with the drawer count moves with the float
    try:
    ...
    _audit_safe(con, "clinic_float_day", date, "update" if before is not None else "insert",
                dict(before) if before is not None else None, dict(kept_p=kept, need=need), who)
    return redirect("/finance/clinic/register/%s#float" % date)
...
def _match_html(con, date, u, named, m, rows, st, msg):
    ...
    head = ["""
      <div class="card"><h2>Morning match — %s</h2>
        <p class="navrow noprint"><a class="btn" href="/finance/clinic/match/%s">&lsaquo; %s</a>
           <a class="btn" href="/finance/clinic/match/%s">%s &rsaquo;</a>
           <a class="btn" href="/finance/clinic/register/%s">the counter sheet</a>
           <a class="btn" href="/finance/clinic/day/%s">Docterz entries</a>
           <a class="btn" href="/finance/clinic/day/%s/mpr">bank MPR</a></p>
        %s
        <p class="verdict %s">%s</p>
        <p class="mut">%s</p>
      </div>""" % (_human(date), prev, _human(prev), nxt, _human(nxt), date, date, date, msg, verdict_cls,
                   _esc(" ".join(m["lines"])), _day_line(st, con))]
    # the numbers, small
    if m.get("p1"):
        p1, p2, p3 = m["p1"], m["p2"], m["p3"]
        head.append("""<div class="card"><h2>The numbers</h2><table class="grid nums"><thead><tr><th></th><th class="r">Counter</th>
          ...
          </tbody></table><p class="mut">Bank expects = Docterz online − other UPI (₹%s). Bank: %s%s.</p></div>""" % (
    ...
    # explained
    if m["explained"]:
        head.append("<div class='card'><h2>Explained — nothing to do</h2><ul class='lines'>%s</ul></div>"
                    % "".join("<li>%s</li>" % _esc(e["text"]) for e in m["explained"]))
    if m["notes"]:
        head.append("<div class='card'><h2>Notes</h2><ul class='lines mut'>%s</ul></div>"
                    % "".join("<li>%s</li>" % _esc(n) for n in m["notes"]))
    # flags
    live = [r for r in rows if r["status"] != "cleared"]
    if live:
        items = []
        for r in live:
            items.append(_flag_html(r, named, status, who))
        head.append("<div class='card'><h2>Flags — these need a person (%d)</h2>%s</div>" % (len(live), "".join(items)))
    # the pass buttons
...
def _owner_html(con, ym, msg, u):
    ...
    q = owner_queue(con)
    out = ["""<div class="card"><h2>Clinic money — what needs you</h2>%s
    ...
    out.append("""<div class="card"><h2>The float%s</h2>
      <p class="mut">Clinic cash placed at the counter and still clinic cash: not revenue, not an expense, never part of any
      collection. Reception keeps it aside at close on their sheet; only a shortfall or a change request shows up here.</p>
      <form method="post"><input type="hidden" name="act" value="float">
      <table class="grid entry"><tbody>%s
        <tr><th class="sec">This is</th><td><select name="kind" class="note"><option value="issue">issued (first time)</option>
            <option value="topup">a top-up (making up a shortfall)</option><option value="return">taken back</option></select></td></tr>
      ...
      </tbody></table><button type="submit" class="save">Record</button></form></div>""" % (
        (" — standing ₹ %s" % _r(standing)) if standing else " — not issued yet", cells, _now().date().isoformat()))
    return "".join(out)
...
details.card summary{cursor:pointer;font-size:19px}
