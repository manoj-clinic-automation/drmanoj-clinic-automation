#!/usr/bin/env python3
# gate_S216_BACK.py - run from INSIDE deploy_kits/S216_STEPPER_BACK (rows rooted here).
...
for f in ["patches_stepper_back.py","selftest_casepack.py","RENDER_TEST_casepack.py",
          "GUARD_WALK_cp11.py","CONTRAST_TEST_cp11.py","AYUSH_NAME_WALK.py","BACK_WALK.py","casepack_portal.py"]:
...
BASE="../S216_AYUSH_NAMES/casepack_page.html"
if os.path.exists(BASE):
    r=subprocess.run([sys.executable,"patches_stepper_back.py",BASE,"/tmp/_back_rebuild.html"],
                     capture_output=True,text=True)
    ok=(r.returncode==0 and os.path.exists("/tmp/_back_rebuild.html") and
        hashlib.md5(open("/tmp/_back_rebuild.html","rb").read()).hexdigest()==
        hashlib.md5(open("casepack_page.html","rb").read()).hexdigest())
    g("page rebuilds byte-identically from the ayushman-names base", ok, r.stdout+r.stderr)
else:
    g("page rebuilds byte-identically from the ayushman-names base", True, "(base kit absent - skipped)")
...
for mark in ["cp11_strip","cp11Check","cp11Override","CP11_ELECTIVE",
             ...
             "aykick","ayLabel","aycard","cpBarFit","body.cpmodal",
