...
def _sms_early_p(con, d):
    """S295: the bank's own settlement SMS for this unit's day (bank_sms.py, S290), or None when no SMS
    came. An early signal only -- the MPR file stays the record and everything below still waits for it."""
    try:
        import bank_sms                                          # noqa: PLC0415
        return bank_sms.sms_total_p(con, _unit, d)
    except Exception:                            # noqa: BLE001
        return None
...
def _other_upi_block(con, d):
    ...
    return """
      ...
      </details>""" % (" open" if rows else "", (" · &#8377; %s recorded" % _r(total)) if rows else "", table, d, _esc(apps[0]) if apps else "", "".join("<option value='%s'>" % _esc(a) for a in list(dict.fromkeys(apps + list(UPI_APPS)))),
...
def _match_html(con, date, u, named, m, rows, st, msg):
    ...
    if m.get("p1"):
        if m["bank_known"] and not m.get("expected_bank_p") and not m["bank_p"]:
            ...
            _sp = _sms_early_p(con, date)                          # S295: the morning SMS, when it came
            bank_line = (("Bank SMS this morning: ₹%s credited to the clinic account — the early figure; "
                          "the bank file (MPR) has not arrived yet, so the match waits for it." % _r(_sp))
                         if _sp is not None else
                         "Bank: the file for this day has not arrived yet (%s)." % _esc(m["bank_state"].replace("_", " ")))
        ...
        head.append("""<details class="card"><summary>Show the numbers</summary><table class="grid nums"><thead><tr><th></th><th class="r">Counter</th>
          ...
          </tbody></table><p class="mut">Bank expects = Docterz online − other UPI (₹%s) − portal (Razorpay, by id). Bank: %s%s.</p>%s
