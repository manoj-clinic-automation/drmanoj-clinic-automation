...
# S295: Drive files that carry NO 'Day Revenue' sheet (the two June workbooks made before the tracker
# wrote one) are remembered by id and Drive mtime, so the all-day schedule (S291) does not download and
# fail them every ten minutes. A file that changes on Drive is read again.
NOSHEET_FILE = os.environ.get("DOCTERZ_NOSHEET_FILE", "/root/finance/docterz_nosheet.json")
...
def main():
    ...
    done = skipped = failed = nosheet = 0
    try:
        with open(NOSHEET_FILE, "r", encoding="utf-8") as fh:
            nosheet_known = json.load(fh)
    except (OSError, ValueError):
        nosheet_known = {}
    nosheet_changed = False
    for title, fid, mtime in files:
        if not a.all and known.get(fid) == mtime:
            skipped += 1
            continue
        if not a.all and nosheet_known.get(fid) == mtime:
            nosheet += 1
            continue
        try:
            d = parse_bytes(fetch_bytes(fid))
        except Exception as e:                              # noqa: BLE001
            if str(e).startswith("no 'Day Revenue' sheet"):
                nosheet += 1
                print("  NO SHEET %-37s remembered, not retried (%s)" % (title, e))
                if nosheet_known.get(fid) != mtime:
                    nosheet_known[fid] = mtime
                    nosheet_changed = True
                continue
            failed += 1
    ...
    if not a.dry_run:
        ...
        if nosheet_changed:
            try:
                tmp = NOSHEET_FILE + ".part"
                with open(tmp, "w", encoding="utf-8") as fh:
                    json.dump(nosheet_known, fh, indent=1, sort_keys=True)
                os.replace(tmp, NOSHEET_FILE)
            except OSError as e:
                print("no-sheet memory not saved (%s) -- those files will be tried again next run" % e)
    try:
    ...
    print("read %d, skipped %d unchanged, %d with no Day Revenue sheet, failed %d; %s now holds %d days and %d lines%s"
          % (done, skipped, nosheet, failed, TABLE, n, ln,
