...
def _item_life(con, item, ps, since_iso, upto_iso):
    ...
    if len(marg) >= 2:
            ...
            if res:
                ...
                if lines:
                    ...
                    span["explained"] = dict(
                        ...
                        sentence="%s is %s -- a purchase entry keyed in Marg after that day's export was taken; the documents agree, nothing to chase."
                    ...
                    span["lookup"] = ("Marg > item ledger > %s > %s to %s: which voucher took %s? No bill, return or sale on the server for it."
                                      % (item, _r_dmy(a[0]), _r_dmy(b[0]), _qwd(res, ps)))
            spans.append(span)
        # a bill keyed late shows twice: MISSING from the span that holds its date (a minus residue) and
        # PRESENT in the later span where it was found (a plus residue). Both are the same event (ROSIKA
        # FORTE, 06-Sep-2026: -60 strips 27-08..02-09, +60 strips 02-09..03-09, bill 545 of 01-09). Settle both.
        for x in spans:
            if not x["explained"]:
                continue
            for b_ in x["explained"]["bills"]:
                bd = next((p["bill_date"] for p in purchases if p["bill_no"] == b_["bill_no"]), None)
                if not bd:
                    continue
                for y in spans:
                    if y["lookup"] and y["from_date"] < bd <= y["to_date"] and y["residue"] == -x["residue"]:
                        y["explained"] = dict(bills=[b_], sentence="%s: bill %s of %s was not yet in Marg at the %s export (keyed later); it appears in the next span. Same entry, nothing to chase."
                                              % (y["residue_text"], b_["bill_no"], b_["bill_date_text"], y["to_text"]))
                        y["lookup"] = None
                        explained_total += y["residue"]
        # two spans that cancel each other with no bill: an entry made and reversed -- net zero
        opn = [x for x in spans if x["lookup"]]
        for x in opn:
            for y in opn:
                if x is not y and x["lookup"] and y["lookup"] and x["residue"] == -y["residue"] and x["to_date"] <= y["from_date"]:
                    for z in (x, y):
                        z["explained"] = dict(bills=[], sentence="%s here and %s later cancel out -- an entry made and reversed in Marg; net zero, nothing to chase."
                                              % (x["residue_text"], y["residue_text"]))
                        z["lookup"] = None
                    explained_total += x["residue"] + y["residue"]
        a, b = marg[0], marg[-1]
        ...
        # THE CONCLUSION -- one line, the owner's ask: "I need only your conclusion; the decision metrics expandable"
        ...
        for x in spans:
            details.append("%s -> %s: Marg %d -> %d (%s); documents %s; residue %s%s" % (
                x["from_text"], x["to_text"], x["from_qty"], x["to_qty"], x["moved_text"], _qws(x["docs_delta"], ps), _qwd(x["residue"], ps) if x["residue"] else "none",
                (" -- " + x["explained"]["sentence"]) if x["explained"] else ((" -- " + x["lookup"]) if x["lookup"] else "")))
