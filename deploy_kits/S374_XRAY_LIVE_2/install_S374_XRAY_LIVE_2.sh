#!/bin/bash
# =============================================================================
#  install_S374_XRAY_LIVE_2.sh · kit S374_XRAY_LIVE_2 (session 279, 23-Sep-2026) -- the X-ray LIVE filing
#
#  S374 = S373 with ONE change: S373 went red at its own probe -- xray-plan without the token answers 302 (the
#  finance app's fail-closed gate sends every token-less request to the login before the route runs), not the
#  route's own 401. Either answer means the door is locked; the probe now accepts both. records.py is byte-identical.
#
...
#    cd /root/deploy/repo && git pull --ff-only && bash /root/deploy/repo/deploy_kits/S374_XRAY_LIVE_2/install_S374_XRAY_LIVE_2.sh
#
...
BAK="$FIN/records.py.bak_S374_07ec9b41"
...
say "[6/7] healthz $H · xray-plan without the token $PL (302/401 expected: locked) · /finance/checks without login $CK (302/401/403 expected)"
[ "$H" = 200 ] && { [ "$PL" = 302 ] || [ "$PL" = 401 ]; } && { [ "$CK" = 302 ] || [ "$CK" = 401 ] || [ "$CK" = 403 ]; } || restore
