...
def _report_html(con, m, u):
    ...
    n_emr, old_emr = 0, None                              # S379: the Docterz upload list is retired (the owner)
    if n_emr:
...
def pending_counts(con):
    ...
    return {"blood": len([o for o in b if o["st"] != "later"]), "blood_red": len([o for o in b if o["st"] in red]),
            "xray": len(x), "xray_red": len([o for o in x if o["st"] in red]), "orphans": len(orph)}
