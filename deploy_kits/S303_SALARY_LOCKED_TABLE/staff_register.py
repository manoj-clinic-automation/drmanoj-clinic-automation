...
v0.18(S238) — Step 0 condensed for print: three dates per row, tighter rows, the two
              explanatory paragraphs removed (the owner, on the first real 3-page print).
v0.17(S238) — settings page: "Cover staff: a punch-out at/after this time is a cover day"
              (cover_auto_from, 20:00; salary_policy v1.12 verifies cover by the punch).
v0.16(S238) — Step 0 is a proper A4 document (a row per date, P/L/A tick boxes, Sundays
...
v0.15(S238) — the settings page carries "Overtime payable enters the net (1/0)" --
              salary_policy v1.10 shows OT on Sheet 1 and pays it only when set.
v0.14(S238) — STEP 0 of the month-end flow: the machine-absent days, staff-wise,
...
v0.13(S238) — the Lock refuses until the STAFF LEDGER has closed the month: the
...
v0.12(S200/R7) — approve WHERE YOU READ: Sheet 1/2 carry their own approve
              strips, the lock desk shows open/approve doors per sheet, the
              flow approve returns you to the page you came from; fix-absents
              staff blocks are expandable; scenario/early-big explained; fonts up;
v0.11(S200) — D346 go-live: the lock records COLLECT when a suspended charge
              is taken (no improvement); settings page carries the D345 ramp
              and the D341 Sunday-weight override in place of the dead flat fine;
v0.10(S200) — D339 second pass (owner's live use): the reason survives a
              submit (an empty required box at the top of a long page made
              the browser refuse to send, which read as 'nothing happened'),
              corrections are LISTED, and a correction can be UNDONE;
v0.9 (S200) — D339 the FIX-ABSENTS desk: one month page listing every
              machine-absent staff-day with a live per-staff counter, so a
              month's corrections are one pass instead of one page per day;
...
S289 (17-Sep-2026) — MARK MY EXIT (owner ruling, same day): a staff member who
leaves late when the punch machine cannot be reached raises "Mark my exit" on
/register/me AT THE MOMENT OF LEAVING. Server-authoritative guards: today only,
the machine (or a present request) must already show them in, the time must be
at or after their shift end (SR_EXIT_AFTER when no shift is known), one per day,
a reason and who they told are chosen by tapping. The SERVER receipt time IS
the punch-out. Owner: staff are usually in front of the doctors when leaving, so
the approver may decide at once or later — from pending or verified, no
deadline; a checker may still verify first. Approved exits fold into
att_month_report as synthetic punch-outs (OT candidate, as any punch-out).
Nothing else on the self page changes: today only.

S303 (17-Sep-2026 night) — F-519, THE LOCK DESK SHOWS ONE TOTAL. Once a month is
LOCKED, the desk's table is the FROZEN table saved at the lock (locked_run.report_html,
Sheet 3 rows + each own-sheet person's NET PAYABLE), so its TOTAL equals the card — the
approved, paid run. Today's recompute is no longer shown as a second table: it moves
after a lock (the lock itself closes last month's holds; a leaver made inactive drops
out of every month; a ledger correction), so only a short "today's recompute differs"
list is shown per staff, naming what moved, for information. The frozen sheets open
read-only at /register/salary/locked. If the frozen table cannot be read, or its rows
do not add up to the card, the desk says so and shows no second total. Nothing is
written anywhere; salary_policy, the ledger and the hold ledger are untouched.

S300 (17-Sep-2026 evening) — D540, THE SELF PAGE AS THE OWNER CORRECTED IT
(his words, folded; "past days show absents and leaves only" was then DROPPED
by him: "not needed", so the page stays today only):
  * the self page speaks Hinglish (D536/D540 rule: Hindi/Hinglish on staff
    pages, English with the owner) and is TAP-ONLY;
  * "Mark me present": the reason is one of PRESENT_REASONS, tapped, never
    typed; the server accepts only a listed key and stores its English label
    (the doctors' review card reads English);
  * "Mark my exit" becomes "Mark overtime": two buttons, "Told Dr Bhawna" and
    "Told Dr Manoj" — one tap raises it; reason stored as 'overtime'; the
    server refuses any other reason key or any other "told";
  * the "My month" link is removed from the self page (F-516). The route
    itself is untouched and no longer linked from anywhere a staff member sees.
  * the review board's card is titled "Overtime requests"; older exit rows
    keep their stored reason/told labels.
...
# S289: exit requests. Earliest time when the staff member's shift end is unknown.
SR_EXIT_AFTER = _cfg("SR_EXIT_AFTER", "20:00")
EXIT_REASONS = [("overtime", "Overtime"),                     # S300: the only one offered now
                ("late_patients", "Late patients / work"),
                ("doctor_asked", "Doctor asked me to stay"),
                ("machine_closed", "Machine off or locked"),
                ("other", "Other")]
EXIT_TOLD = [("manoj", "Dr Manoj"), ("bhawna", "Dr Bhawna"), ("reception", "Reception")]
# S300 (D540): the self page offers exactly these two buttons, in the owner's order;
# EXIT_TOLD above stays as the label dictionary for rows raised before S300.
EXIT_TOLD_OFFERED = [("bhawna", "Dr Bhawna"), ("manoj", "Dr Manoj")]
# S300 (D540): mark-me-present reasons, tapped. (key, what the staff member reads,
# what is stored and what the doctors read on the review card).
PRESENT_REASONS = [("finger", "Machine ne finger nahi padha", "Machine did not read finger"),
                   ("machine_off", "Machine band thi / chal nahi rahi", "Machine off or not working"),
                   ("clinic_work", "Clinic ke kaam se bahar tha / thi", "Out on clinic work"),
                   ("forgot", "Punch karna bhool gaya / gayi", "Forgot to punch"),
                   ("other", "Koi aur karan (reception ko bataya)", "Other reason (told reception)")]
...
def approved_exit_requests(con, d):
    """{staff_id: req_ts} of APPROVED exit requests for date d (S289). Missing
    table (pre-init DB) reads as none."""
    out = {}
    try:
        for r in con.execute("SELECT staff_id, req_ts FROM exit_request "
                             "WHERE reg_date=? AND status='approved'", (d,)):
            out[r["staff_id"]] = r["req_ts"]
    except Exception:
        pass
...
def machine_day(con, d):
    ...
    for sid, ts in approved_present_requests(con, d).items():
        ...
        times[sid] = lst
        via_req.add(sid)
    for sid, ts in approved_exit_requests(con, d).items():   # S289: punch-outs
        try:
            dt = datetime.datetime.strptime(ts.strip(), "%Y-%m-%d %H:%M:%S")
        except (ValueError, AttributeError):
            continue
...
CREATE TABLE IF NOT EXISTS exit_request (          -- S289: "I left late, machine unreachable"
    id          INTEGER PRIMARY KEY,
    reg_date    TEXT NOT NULL,                     -- always the SERVER's today
    staff_id    INTEGER NOT NULL,
    req_user    TEXT NOT NULL,
    req_ts      TEXT NOT NULL,                     -- SERVER receipt time = the punch-out
    reason      TEXT NOT NULL,                     -- chosen from EXIT_REASONS
    told        TEXT NOT NULL,                     -- who they told (EXIT_TOLD)
    status      TEXT NOT NULL DEFAULT 'pending',   -- pending|verified|approved|rejected
    verify_user TEXT, verify_ts TEXT,
    decide_user TEXT, decide_ts TEXT, decide_note TEXT,
    UNIQUE (reg_date, staff_id)
);
"""
...
<div class="wrap" style="max-width:480px">
  <div class="head"><h1>&#129465; Meri attendance</h1>
    <span class="sub">{{ name }} &middot; login: {{ who.user }}</span></div>
  {% if msg %}<div class="msg {{ msgcls }}">{{ msg }}</div>{% endif %}

  <div class="card" style="text-align:center">
    <div style="font-size:13px;color:var(--muted)">Aaj</div>
    <div style="font-size:22px;font-weight:700;margin:4px 0 12px">{{ d }}</div>
    {% if holiday %}
      <div class="msg ok">&#127958;&#65039; Aaj clinic ki chhutti hai.</div>
    {% elif feed_down %}
      <div class="msg err">Punch machine ki jaankari abhi nahi mil rahi.
        Reception se baat karein.</div>
    {% elif times %}
      <div style="font-size:13px;color:var(--muted);margin-bottom:6px">Aaj aapka punch</div>
      {% for t in times %}
        <div style="font-size:26px;font-weight:700;letter-spacing:.03em">{{ t }}</div>
      {% endfor %}
      <div class="note" style="margin-top:10px">Sab darj hai. Kuch karna nahi hai.</div>
    {% else %}
      <div class="msg err" style="text-align:center">Aaj aapka punch nahi aaya.</div>
      {% if req %}
        {% if req.status == 'approved' %}
          <div class="msg ok">&#9989; Present maan liya &mdash; aapki request ka time
            <b>{{ req.req_ts[11:16] }}</b> hi aapka punch time hai.</div>
        {% elif req.status == 'rejected' %}
          <div class="msg err">&#10060; Aapki request manzoor nahi hui.
            {% if req.decide_note %}Karan: {{ req.decide_note }}{% endif %}</div>
        {% else %}
          <div class="msg ok">&#8987; Request <b>{{ req.req_ts[11:16] }}</b> par bheji
            &mdash; {{ 'doctor' if req.status=='verified' else 'check' }} ka intezaar.
            Manzoor hone par {{ req.req_ts[11:16] }} hi punch time hoga.</div>
        {% endif %}
      {% elif can_request %}
        <form method="POST" action="{{ prefix }}/me/request" style="margin-top:8px">
          <div style="font-size:15px;font-weight:600;margin:4px 0 6px">Punch kyun nahi aaya? Ek chuniye:</div>
          {% for k, lbl in present_reasons %}
            <label style="display:block;text-align:left;padding:12px;margin:6px 0;border:1px solid var(--blue);border-radius:10px;font-size:16px">
              <input type="radio" name="reason" value="{{ k }}" required> {{ lbl }}</label>
          {% endfor %}
          <button class="btn" type="submit" style="width:100%;font-size:16px;margin-top:8px"
                  onclick="return confirm('Abhi present lagayein? Abhi ka time hi punch time hoga.');">
            &#128075; Mujhe present lagao (abhi ka time = punch time)</button>
        </form>
        <div class="note">Request ka time hi punch time maana jaata hai; clinic check
          karke manzoor karta hai. Jaate samay machine par punch karein.</div>
      {% endif %}
    {% endif %}
  </div>
  {% if ex and (ex.row or ex.can or ex.why == 'too_early') %}
  <div class="card" style="text-align:center">
    <div style="font-size:13px;color:var(--muted);margin-bottom:6px">Der tak ruke &mdash; machine nahi mili?</div>
    {% if ex.row %}
      {% if ex.row.status == 'approved' %}
        <div class="msg ok">&#9989; Overtime manzoor &mdash; <b>{{ ex.row.req_ts[11:16] }}</b> aapka jaane ka punch hai.</div>
      {% elif ex.row.status == 'rejected' %}
        <div class="msg err">&#10060; Aapka overtime manzoor nahi hua.
          {% if ex.row.decide_note %}Karan: {{ ex.row.decide_note }}{% endif %}</div>
      {% else %}
        <div class="msg ok">&#8987; Overtime <b>{{ ex.row.req_ts[11:16] }}</b> par bheja &mdash; doctor ka intezaar.
          Manzoor hone par {{ ex.row.req_ts[11:16] }} jaane ka punch hoga.</div>
      {% endif %}
    {% elif ex.can %}
      <form method="POST" action="{{ prefix }}/me/exit">
        <input type="hidden" name="reason" value="overtime">
        <div style="font-size:17px;font-weight:700;margin:4px 0 6px">&#128682; Mark overtime</div>
        <div style="font-size:13px;margin:0 0 6px">Kisko bataya? Button dabate hi abhi ka time jaane ka punch banega.</div>
        {% for k, lbl in exit_told %}
          <button class="btn" type="submit" name="told" value="{{ k }}" style="width:100%;font-size:17px;margin:6px 0"
                  onclick="return confirm('Abhi overtime lagayein? Abhi ka time jaane ka punch hoga.');">Told {{ lbl }}</button>
        {% endfor %}
      </form>
    {% else %}
      <div class="note">{{ ex.opens }} baje se khulega (aapki shift khatam hone par).</div>
    {% endif %}
  </div>
  {% endif %}
  <div class="foot"><a href="{{ prefix }}/logout">Logout</a></div>
</div></body></html>
"""
...
def request_present(user, reason):
    ...
    S300 (D540): the reason is a PRESENT_REASONS key, tapped; the English label
    is stored. Anything else (typed words included) is refused."""
    reason = {k: en for k, _hi, en in PRESENT_REASONS}.get((reason or "").strip(), "")
    con = get_db()
    try:
...
def _msg_safe(s):
    """Message text for a redirect query string. Deliberately blunt — the
    characters that would break the URL are dropped, not escaped."""
    out = []
    for ch in str(s)[:300]:
        if ch in "&#%?\r\n":
            continue
...
def punch_month_index(ym):
    """{'YYYY-MM-DD': set(staff_id)} in ONE pass over punches.csv for the whole
    month — the day-at-a-time reader would re-read the file 31 times for one
    page. None = feed unreadable, exactly like punch_times_for_day (callers
    fail soft; a missing feed must never be read as 'everybody was absent')."""
    p = SR_PUNCH_CSV
    if not p or not os.path.exists(p):
        return None
    out = {}
        ...
        with open(p, newline="", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ...
                try:
                    ...
                    continue
                out.setdefault(ds[:10], set()).add(uid)
    except Exception:
        return None
    return out


def absent_worklist(con, ym):
    """D339: every CORRECTABLE machine-absence in month ym, grouped per staff.

    A staff-day is offered only when all of these hold — the same conditions
    correct_present() enforces at write time, applied here so the page never
    offers a row the write would refuse:
      * the date is past or today (never the future);
      * it is not a clinic holiday;
      * the staff member is active that day AND rostered ON (duty_shift_for
        returns a start — a rostered-off Sunday is not an absence);
      * the machine has NO punch for them that day;
      * no present_request exists for that staff-day yet (any status).

    Returns (blocks, feed_ok). feed_ok False = punch feed unreadable, in which
    case blocks is empty and the page must say so rather than show a clean
    list. Each block: {sid, name, absent, corrected, rows:[{d, dow, sun,
    prefill}]} where `absent` counts every rostered no-punch day in the month
    (corrected ones included) so the owner can see 9 -> 7 as he works."""
    if not _valid_ym(ym):
        return [], True
    idx = punch_month_index(ym)
    if idx is None:
        return [], False
    year, mon = int(ym[:4]), int(ym[5:7])
    ndays = calendar.monthrange(year, mon)[1]
    today = _today()
    shifts = load_staff_shifts() or {}
    reqs = {}
    try:
        for r in con.execute("SELECT reg_date, staff_id, status, req_ts, decide_note "
                             "FROM present_request WHERE reg_date LIKE ?", (ym + "-%",)):
            reqs[(r["reg_date"], r["staff_id"])] = (r["status"], r["req_ts"] or "",
                                                    r["decide_note"] or "")
    except Exception:
        pass
    acc = {}
    for day in range(1, ndays + 1):
        ...
        for s in staff_for_date(con, d):
            ...
            if sh is not None:
                try:
                    s_start, _s_end = duty_shift_for(date, sh)
                except Exception:
                    s_start = None
                if s_start is None and not sh["minutes_exempt"]:
                    continue                      # rostered OFF — not an absence
                if s_start is not None:
                    prefill = s_start.strftime("%H:%M")
            b = acc.setdefault(sid, {"sid": sid, "name": s["name"], "absent": 0,
                                     "corrected": 0, "rows": [], "done": []})
            b["absent"] += 1
            got = reqs.get((d, sid))
            if got is not None:
                st, ts, note = got
                if st == "approved":
                    b["corrected"] += 1
                    b["done"].append({"d": d, "dow": date.strftime("%a"),
                                      "sun": date.weekday() == 6,
                                      "in": ts[11:16],
                                      "undoable": ("D338" in note or "D339" in note)})
                continue                          # already has a request row
            b["rows"].append({"d": d, "dow": date.strftime("%a"),
                              "sun": date.weekday() == 6, "prefill": prefill})
    blocks = sorted(acc.values(), key=lambda b: b["name"].lower())
    return [b for b in blocks if b["rows"] or b["done"]], True


def uncorrect_present(actor, d, sid):
    """D339b (S200): reverse a past-day presence correction — the day goes back
    to whatever the machine says, i.e. absent. Deliberately NARROW: it removes
    ONLY a row this desk (or the D338 day card) wrote, proven by its decide_note.
    A staff member's own D334 request carries a real approval trail and is never
    silently deleted here — that is rejected through the normal decide door."""
    if actor not in PRESENT_APPROVERS:
        raise PermissionError("only a present-request approver may undo a correction")
    if not _valid_date(d):
        raise PermissionError("invalid date")
    try:
        sid = int(sid)
    ...
    con = get_db()
    try:
        r = con.execute("SELECT * FROM present_request WHERE reg_date=? AND staff_id=?",
                        (d, sid)).fetchone()
        if not r:
            raise PermissionError("no correction on that day to undo")
        note = r["decide_note"] or ""
        if not ("D338" in note or "D339" in note):
            raise PermissionError("that day is a staff request, not a correction — "
                                  "use the request decide door")
        con.execute("DELETE FROM present_request WHERE id=?", (r["id"],))
        _audit(con, "present_request", "%s/%s" % (d, sid), "undo_past_day",
               "approved in=%s" % (r["req_ts"] or "")[11:16], "removed", actor)
        con.commit()
        return True
    finally:
        con.close()
...
# S289 — MARK MY EXIT
# ---------------------------------------------------------------------------
def _exit_row(con, d, sid):
    try:
        return con.execute("SELECT * FROM exit_request WHERE reg_date=? AND staff_id=?",
                           (d, sid)).fetchone()
    except Exception:
        return None


def _exit_open_at(sid, d):
    """datetime from which an exit request is allowed: the staff member's duty
    shift end today, else SR_EXIT_AFTER. None = OFF today by roster."""
    date = datetime.datetime.strptime(d, "%Y-%m-%d").date()
    fallback = _parse_hhmm(SR_EXIT_AFTER) or datetime.time(20, 0)
    try:
        sh = (load_staff_shifts() or {}).get(sid)
    except Exception:
        sh = None
    if sh and not sh["minutes_exempt"]:
        s_start, s_end = duty_shift_for(date, sh)
        if s_start is None:
            return None
        if s_end:
            return datetime.datetime.combine(date, s_end)
    return datetime.datetime.combine(date, fallback)


def exit_state(con, user):
    """What the self page needs for the exit block:
    {"row": exit row or None, "can": bool, "why": str, "opens": "HH:MM"}."""
    sid, name, d, times, req = _me_context(con, user)
    st = {"row": None, "can": False, "why": "", "opens": ""}
    if not sid:
        return st
    st["row"] = _exit_row(con, d, sid)
    if st["row"]:
        return st
    if is_holiday(con, d):
        st["why"] = "holiday"
        return st
    in_ok = bool(times) or bool(req and req["status"] != "rejected")
    if not in_ok:
        st["why"] = "no_in"
        return st
    opens = _exit_open_at(sid, d)
    if opens is None:
        st["why"] = "off"
        return st
    st["opens"] = opens.strftime("%H:%M")
    if datetime.datetime.strptime(_now(), "%Y-%m-%d %H:%M:%S") < opens:
        st["why"] = "too_early"
        return st
    st["can"] = True
    return st
...
def request_exit(user, reason, told):
    """Raise a mark-my-exit request for TODAY (S289). Guards are the server's:
    no date parameter exists; the staff member must already be in (a machine
    punch today, or a present request not rejected); now must be at or after
    their shift end (SR_EXIT_AFTER when unknown); one per day; reason and told
    must be one of the offered choices. The SERVER receipt time is stored and,
    on approval, IS the punch-out."""
    reason = (reason or "").strip()
    told = (told or "").strip()
    if reason != "overtime":                     # S300 (D540): the only reason offered
        raise PermissionError("choose a reason")
    if told not in dict(EXIT_TOLD_OFFERED):      # S300: Told Dr Bhawna / Told Dr Manoj
        raise PermissionError("choose who you told")
    con = get_db()
    try:
        sid, name, d, times, req = _me_context(con, user)
        if not sid:
        ...
        if times is None:
            raise PermissionError("punch feed unavailable — ask at reception")
        st = exit_state(con, user)
        if st["row"]:
            raise PermissionError("an exit request for today already exists (%s)" % st["row"]["status"])
        if not st["can"]:
            raise PermissionError({
                "holiday": "today is a clinic holiday",
                "no_in": "no punch-in for you today — use Mark me present first",
                "off": "you are off duty today by roster",
                "too_early": "exit request opens at %s" % st["opens"],
            }.get(st["why"], "not allowed now"))
        now = _now()
        con.execute("INSERT INTO exit_request(reg_date,staff_id,req_user,req_ts,reason,told,"
                    "status) VALUES(?,?,?,?,?,?,'pending')", (d, sid, user, now, reason, told))
        _audit(con, "exit_request", "%s/%s" % (d, sid), "raise", "",
               "req_ts=%s reason=%s told=%s" % (now, reason, told), user)
        con.commit()
        return now
    finally:
        con.close()
...
def verify_exit(rid, actor):
    """Optional checker step — never one's own request."""
    con = get_db()
    try:
        r = con.execute("SELECT * FROM exit_request WHERE id=?", (rid,)).fetchone()
        if not r:
            raise PermissionError("no such request")
        if r["status"] != "pending":
            raise PermissionError("request is already %s" % r["status"])
        if r["req_user"] == actor:
            raise PermissionError("you cannot verify your own request")
        con.execute("UPDATE exit_request SET status='verified',verify_user=?,verify_ts=? "
                    "WHERE id=?", (actor, _now(), rid))
        _audit(con, "exit_request", rid, "verify", "pending", "verified", actor)
        con.commit()
    finally:
        con.close()


def decide_exit(rid, actor, action, note=""):
    """PRESENT_APPROVERS only, from pending OR verified — the owner may approve at
    once (the staff member is in front of him) or later. Never one's own."""
    if action not in ("approve", "reject"):
        raise PermissionError("bad action")
    if actor not in PRESENT_APPROVERS:
        raise PermissionError("only the doctor decides exit requests")
    con = get_db()
    try:
        r = con.execute("SELECT * FROM exit_request WHERE id=?", (rid,)).fetchone()
        if not r:
        ...
        if r["req_user"] == actor:
            raise PermissionError("you cannot decide your own request")
        new = "approved" if action == "approve" else "rejected"
        con.execute("UPDATE exit_request SET status=?,decide_user=?,decide_ts=?,decide_note=? "
                    "WHERE id=?", (new, actor, _now(), (note or "").strip()[:200], rid))
        _audit(con, "exit_request", rid, action, r["status"], new, actor)
        con.commit()
    finally:
        con.close()
...
def me_view():
    ...
    return render_template_string(
        ...
        ex=ex, exit_told=EXIT_TOLD_OFFERED,
        present_reasons=[(k, hi) for k, hi, _en in PRESENT_REASONS],
        msg=request.args.get("m", ""), msgcls=request.args.get("c", "ok"))


def _q(text):
    """S300: a message for the self page's address, safely quoted (Hinglish carries ':' and '/')."""
    import urllib.parse
    return urllib.parse.quote_plus(text)


# S300 (D540): the server's refusals, as the staff member reads them. Unknown text passes through.
_ME_HI_EXACT = {
    "this login is not mapped to a staff member": "Yeh login kisi staff se juda nahi hai — Dr Manoj se kahein",
    "today is a clinic holiday": "Aaj clinic ki chhutti hai",
    "punch feed unavailable — ask at reception": "Punch machine ki jaankari nahi mil rahi — reception se poochhein",
    "a reason is required": "Ek karan chuniye",
    "choose a reason": "Ek karan chuniye",
    "choose who you told": "Kisko bataya — button chuniye",
    "no punch-in for you today — use Mark me present first": "Aaj aapka aane ka punch nahi hai — pehle 'Mujhe present lagao' karein",
    "you are off duty today by roster": "Roster mein aaj aapki chhutti hai",
    "not allowed now": "Abhi nahi ho sakta",
}
_ME_HI_PREFIX = [
    ("machine already has your punch at ", "Machine mein aapka punch pehle se hai: "),
    ("a request for today already exists", "Aaj ki request pehle se bheji hui hai"),
    ("an exit request for today already exists", "Aaj ka overtime pehle se bheja hua hai"),
    ("exit request opens at ", "Overtime is time se khulega: "),
]


def _me_hi(e):
    t = str(e)
    if t in _ME_HI_EXACT:
        return _ME_HI_EXACT[t]
    for en, hi in _ME_HI_PREFIX:
        if t.startswith(en):
            rest = t[len(en):]
            return hi + (rest if hi.endswith(": ") else "")
    return t
...
def me_request():
    ...
    try:
        ...
        return redirect(APP_PREFIX + "/me?m=" + _q("Request %s par bheji" % ts[11:16]) + "&c=ok")
    except PermissionError as e:
        return redirect(APP_PREFIX + "/me?m=%s&c=err" % _q(_me_hi(e)))


@app.route(APP_PREFIX + "/me/exit", methods=["POST"])
@require(None)
def me_exit():
    u = current_user(request)
    try:
        ts = request_exit(u["user"], request.form.get("reason"), request.form.get("told"))
        return redirect(APP_PREFIX + "/me?m=" + _q("Overtime %s par bheja" % ts[11:16]) + "&c=ok")
    except PermissionError as e:
        return redirect(APP_PREFIX + "/me?m=%s&c=err" % _q(_me_hi(e)))


@app.route(APP_PREFIX + "/exit/verify", methods=["POST"])
@require("check")
def exit_verify():
    u = current_user(request)
    try:
        verify_exit(request.form.get("rid"), u["user"])
        return redirect(APP_PREFIX + "/review?m=Exit+verified&c=ok")
    except PermissionError as e:
        return redirect(APP_PREFIX + "/review?m=%s&c=err" % str(e).replace(" ", "+"))


@app.route(APP_PREFIX + "/exit/decide", methods=["POST"])
@require(None)
def exit_decide():
    u = current_user(request)
    ...
    try:
        decide_exit(request.form.get("rid"), u["user"], request.form.get("action"),
                    request.form.get("note"))
        return redirect(APP_PREFIX + "/review?m=Exit+decided&c=ok")
    except PermissionError as e:
        return redirect(APP_PREFIX + "/review?m=%s&c=err" % str(e).replace(" ", "+"))
...
@app.route(APP_PREFIX + "/fixabsents")
@require(None)
def fixabsents():
    # D339: the month desk. Approver-only, exactly like the D338 day card.
    u = current_user(request)
    if not u["caps"].get("present"):
        return render_template_string(NO_ACCESS_HTML, who=u,
                                      url_prefix=APP_PREFIX), 403
    ...
    try:
        ...
        con.close()
    return render_template_string(
        FIXABS_HTML, who=u, prefix=APP_PREFIX, ym=ym, blocks=blocks,
        feed_ok=feed_ok,
        reason_default=(request.args.get("r", "").replace("+", " ").strip()
                        or "machine missed the punch"),
        msg=request.args.get("m", ""),
        msgcls=("ok" if request.args.get("c") == "ok" else
                ("err" if request.args.get("c") == "err" else "")))


@app.route(APP_PREFIX + "/fixabsents/apply", methods=["POST"])
@require(None)
def fixabsents_apply():
    # Every row goes through correct_present() — the SAME guarded write the day
    # card uses. This page only saves the owner the walk; it grants nothing.
    u = current_user(request)
    if not u["caps"].get("present"):
        return render_template_string(NO_ACCESS_HTML, who=u,
                                      url_prefix=APP_PREFIX), 403
    ...
    ok, errs = 0, []
    for key in request.form.getlist("pick"):
        d, _sep, sid = key.partition("|")
        try:
            correct_present(u["user"], d, sid,
                            request.form.get("t_" + key, ""), reason)
            ok += 1
        except (PermissionError, ValueError) as e:
            errs.append("%s: %s" % (d, e))
    if not ok and not errs:
        msg, cls = "Nothing was ticked - no day was changed.", "err"
    else:
        msg = "%d day(s) marked present." % ok
        cls = "ok" if not errs else "err"
        if errs:
            msg += " %d refused - %s" % (len(errs), "; ".join(errs[:3]))
    return redirect("%s/fixabsents?ym=%s&m=%s&c=%s&r=%s"
                    % (APP_PREFIX, ym, _msg_safe(msg), cls, _msg_safe(reason)))


@app.route(APP_PREFIX + "/fixabsents/undo", methods=["POST"])
@require(None)
def fixabsents_undo():
    # D339b: put ticked days back to absent by removing the correction row.
    u = current_user(request)
    if not u["caps"].get("present"):
        return render_template_string(NO_ACCESS_HTML, who=u,
                                      url_prefix=APP_PREFIX), 403
    ...
    if not _valid_ym(ym):
        ym = _today()[:7]
    ok, errs = 0, []
    for key in request.form.getlist("undo"):
        d, _sep, sid = key.partition("|")
        try:
            uncorrect_present(u["user"], d, sid)
            ok += 1
        except (PermissionError, ValueError) as e:
            errs.append("%s: %s" % (d, e))
    if not ok and not errs:
        ...
        msg = "%d day(s) put back to absent." % ok
        cls = "ok" if not errs else "err"
        if errs:
            msg += " %d refused - %s" % (len(errs), "; ".join(errs[:3]))
    return redirect("%s/fixabsents?ym=%s&m=%s&c=%s"
                    % (APP_PREFIX, ym, _msg_safe(msg), cls))
...
table{border-collapse:collapse;width:100%;font-size:16px;margin:12px 0}
...
.note{color:#93a4b8;font-size:15px;margin:8px 0}
...
.datelinks a{display:inline-block;margin:3px 8px 3px 0;padding:5px 11px;border-radius:8px;
  background:#0b1b29;border:1px solid #7f1d1d;color:#ffd0d0;text-decoration:none;font-size:15px}
...
<div class="btnbar">
  ...
  <a class="btn" href="{{ prefix }}/salary/scenario?ym={{ ym }}" title="What-if simulator — try different rule numbers on this month reality; changes NOTHING">&#128202; What-if</a>
...
{% else %}
  <div class="lockcard {{ 'ready' if can_lock else 'block' }}">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin:4px 0">
      {% if s1 %}<span class="okline">&#10003; Sheet 1 approved ({{ s1.approved_by }})</span>{% else %}
        <span class="badline">&#10007; Sheet 1</span>
        <a class="btn" style="padding:6px 12px;font-size:14px" href="{{ prefix }}/salary/flow/sheet1?ym={{ ym }}">open &amp; approve</a>
        <form method="POST" action="{{ prefix }}/salary/flow/approve" style="margin:0"
              onsubmit="return confirm('Approve Sheet 1 (attendance) for {{ ym }} WITHOUT opening it?')">
          <input type="hidden" name="ym" value="{{ ym }}"><input type="hidden" name="sheet" value="sheet1">
          <input type="hidden" name="back" value="desk">
          <button class="btn" style="padding:6px 12px;font-size:14px;background:#16794a;border-color:#16794a;color:#fff">approve</button></form>
      {% endif %}
      &nbsp;&middot;&nbsp;
      {% if s2 %}<span class="okline">&#10003; Sheet 2 approved ({{ s2.approved_by }})</span>{% else %}
        <span class="badline">&#10007; Sheet 2</span>
        <a class="btn" style="padding:6px 12px;font-size:14px" href="{{ prefix }}/salary/flow/sheet2?ym={{ ym }}">open &amp; approve</a>
        <form method="POST" action="{{ prefix }}/salary/flow/approve" style="margin:0"
              onsubmit="return confirm('Approve Sheet 2 (money) for {{ ym }} WITHOUT opening it?')">
...
{% if locked %}
  {% if snap_rows %}
<p class="note">&#128274; <b>The locked table &mdash; as approved and paid on {{ locked.locked_ts[:10] }}.</b> Frozen at the lock; its TOTAL is the card's.
  <a href="{{ prefix }}/salary/locked?ym={{ ym }}" style="color:#bfe3ff">Open the locked sheets (as printed at the lock)</a></p>
<table><thead><tr><th class="nm">Staff</th><th>Base</th><th>Advance ded.</th><th>Leave amt</th>
<th>Late collect</th><th>Hold</th><th>Prev hold deducted</th><th>Prev hold written off</th><th>Fines</th><th>D+I</th><th>Duty +</th><th>OT paid</th>
<th>Incentive&rarr;pot</th><th>NET</th></tr></thead><tbody>
{% for r in snap_rows %}
<tr><td class="nm"><b>{{ r.name }}</b>{% if r.own %} <span class="note">(own sheet)</span>{% endif %}</td><td>{{ r.base }}</td><td>{{ r.adv_ded }}</td><td>{{ r.leave_amt }}</td>
<td>{{ r.collect }}</td><td>{{ r.held }}</td><td>{{ r.prior_collect }}</td><td>{{ r.release }}</td><td>{{ r.fines }}</td><td>{{ r.di }}</td>
<td>{{ r.duty_credits }}</td><td>{{ r.ot_paid }}</td><td>{{ r.incentive }}</td><td><b>{{ r.net }}</b></td></tr>
{% endfor %}
<tr class="tot"><td class="nm">TOTAL</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>{{ snap_pot }}</td><td><b>{{ locked.total_fmt }}</b></td></tr>
</tbody></table>
    {% if diffs %}
<div class="lockcard info"><div style="font-weight:700;margin-bottom:6px">Today's recompute differs for {{ diffs|length }} staff &mdash; for information only; nothing here is paid or changed.</div>
<div class="note" style="margin:0 0 6px">Figures move after a lock when data behind them changes: the lock itself closes last month's holds, a person marked as left drops out, or a ledger entry is corrected.</div>
<table><thead><tr><th class="nm">Staff</th><th>Locked NET</th><th>Today</th><th>Difference</th><th class="nm">What moved (locked &rarr; today)</th></tr></thead><tbody>
{% for d in diffs %}<tr><td class="nm">{{ d.name }}</td><td>{{ d.locked }}</td><td>{{ d.today }}</td><td>{{ d.diff }}</td><td class="nm" style="white-space:normal">{{ d.moved }}</td></tr>{% endfor %}
</tbody></table></div>
    {% elif rows %}
<p class="note">&#10003; Today's recompute matches the locked table.</p>
    {% endif %}
  {% else %}
<div class="lockcard block">The locked table could not be read back ({{ snap_err }}). The card above is the locked total; no second total is shown.
  <a href="{{ prefix }}/salary/locked?ym={{ ym }}" style="color:#ffd0d0">Open the locked sheets</a></div>
  {% endif %}
{% elif rows %}
...
body{font-size:16px} table{font-size:15.5px!important} td,th{padding:9px 10px!important}
h1{font-size:21px} .note,small{font-size:14.5px}</style>
<div class="btnbar">
...
def _exit_board(con, ym, actor, can_check):
    """S289: open exit requests for ym (pending + verified) for the review page."""
    out = []
    try:
        rs = dict(EXIT_REASONS); tl = dict(EXIT_TOLD)
        for r in con.execute(
                "SELECT er.*, s.name AS staff_name, "
                "(SELECT COUNT(*) FROM exit_request e2 WHERE e2.staff_id=er.staff_id "
                " AND e2.reg_date LIKE ? AND e2.status!='rejected') AS month_n "
                "FROM exit_request er JOIN staff s ON s.staff_id=er.staff_id "
                "WHERE er.reg_date LIKE ? AND er.status IN ('pending','verified') "
                "ORDER BY er.reg_date, er.req_ts", (ym + "%", ym + "%")):
            out.append({
                "id": r["id"], "d": r["reg_date"], "name": r["staff_name"],
                "t": (r["req_ts"] or "")[11:16],
                "reason": rs.get(r["reason"], r["reason"]), "told": tl.get(r["told"], r["told"]),
                "status": r["status"], "month_n": r["month_n"],
                "verify_user": r["verify_user"] or "",
                "can_verify": bool(can_check and r["status"] == "pending" and r["req_user"] != actor),
                "can_decide": bool(actor in PRESENT_APPROVERS and r["req_user"] != actor),
            })
    except Exception:
        out = []
    return out
...
def review_board(con, ym, actor=None, is_override=False, can_check=False):
    ...
    return {"ym": ym, "draft": draft, "to_approve": len(draft),
            ...
            "xreq": _exit_board(con, ym, actor, can_check), }


FIXABS_HTML = HEAD + """
<div class="wrap">
  <div class="head"><h1>&#128203; Fix absents &mdash; {{ ym }}</h1>
    <span class="sub">Signed in as {{ who.user }} ({{ who.role }})</span></div>
  {% if msg %}<div class="msg {{ msgcls }}">{{ msg }}</div>{% endif %}

  <div class="datebar">
    <form method="GET" action="{{ prefix }}/fixabsents" style="display:flex;gap:8px;align-items:center">
      <input type="month" name="ym" value="{{ ym }}"
             style="background:#0b1b29;border:2px solid var(--blue);color:#fff;border-radius:10px;padding:8px 10px;font-size:15px">
      <button class="btn" type="submit">Show</button>
    </form>
    <a href="{{ prefix }}/salary/flow/sheet1?ym={{ ym }}">Sheet 1 &rarr;</a>
  </div>

  {% if not feed_ok %}
    <div class="msg err">The punch feed is unavailable, so absences cannot be
      proved. Nothing is listed &mdash; this page will not guess.</div>
  {% elif not blocks %}
    <div class="msg ok">No correctable absences in {{ ym }}. Every rostered day
      has a punch or already carries a correction.</div>
  {% else %}
  <div class="msg">Ticked days are marked <b>present</b> for that person, with
    the in-time shown (their shift start &mdash; change it if they came late).
    Rostered-off days, holidays, days the machine already has a punch for, and
    days already corrected are not listed. Sundays are marked <b>SUN</b>.</div>

  <form method="POST" action="{{ prefix }}/fixabsents/apply">
    <input type="hidden" name="ym" value="{{ ym }}">
    <div class="allclear">
      <label style="flex:1">Reason (applies to every day you tick)
        <input name="reason" required maxlength="200" value="{{ reason_default }}"
               style="width:100%;background:#0b1b29;border:1px solid var(--line);color:#fff;border-radius:8px;padding:8px 10px;font-size:14px">
      </label>
    </div>
    <div class="note">The reason is pre-filled and kept after each save &mdash; if you
      clear it the browser will refuse to send the form.</div>

    {% for b in blocks %}
    <details {% if b.rows %}{% if loop.first %}open{% endif %}{% endif %} style="margin:10px 0;background:var(--card);border:1px solid var(--line);border-radius:12px;padding:4px 12px">
    <summary style="cursor:pointer;padding:9px 2px;font-size:16px;font-weight:700;color:#fff">{{ b.name }}
      <span class="sub" style="font-weight:400;font-size:13px">&mdash; absent {{ b.absent }}
        {% if b.corrected %}&middot; corrected {{ b.corrected }}
          &middot; <b>still absent {{ b.absent - b.corrected }}</b>{% endif %}
        {% if not b.rows %}&middot; nothing left to tick{% endif %}</span></summary>
    {% if b.rows %}
    <table>
      <tr><th style="width:34px"><input type="checkbox" title="tick all for {{ b.name }}"
            onclick="var t=this.closest('table'),k=t.querySelectorAll('input.pk');
                     for(var i=0;i<k.length;i++)k[i].checked=this.checked;"></th>
          <th>Date</th><th>In-time</th></tr>
      {% for r in b.rows %}
      <tr>
        <td><input class="pk" type="checkbox" name="pick" value="{{ r.d }}|{{ b.sid }}"></td>
        <td>{{ r.d }} &middot; {{ r.dow }}{% if r.sun %}
            <span class="pill holiday">SUN</span>{% endif %}</td>
        <td><input name="t_{{ r.d }}|{{ b.sid }}" value="{{ r.prefill }}"
                   size="6" maxlength="5" inputmode="numeric"
                   style="background:#0b1b29;border:1px solid var(--line);color:#fff;border-radius:8px;padding:6px 8px;font-size:14px;width:76px"></td>
      </tr>
      {% endfor %}
    </table>
    {% else %}
      <div class="sub" style="padding:0 0 8px">Nothing left to correct for {{ b.name }}.</div>
    {% endif %}
    </details>
    {% endfor %}

    <div style="margin:18px 0 24px"><button class="btn" type="submit">Mark the ticked days present</button></div>
  </form>

  {% set anyundo = blocks | selectattr("done") | list %}
  {% if anyundo %}
  <h2 style="font-size:16px;margin:26px 0 6px;color:#fff">Corrections already made</h2>
  <div class="note">These days are counted <b>present</b>. Tick any that should go
    back to <b>absent</b> and press Undo &mdash; the day returns to whatever the
    machine says.</div>
  <form method="POST" action="{{ prefix }}/fixabsents/undo">
    <input type="hidden" name="ym" value="{{ ym }}">
    {% for b in blocks %}{% if b.done %}
    <details style="margin:10px 0;background:var(--card);border:1px solid var(--line);border-radius:12px;padding:4px 12px">
    <summary style="cursor:pointer;padding:9px 2px;font-size:15px;font-weight:700;color:#fff">{{ b.name }}
      <span class="sub" style="font-weight:400;font-size:13px">&mdash; {{ b.done | length }} corrected</span></summary>
    <table>
      <tr><th style="width:34px"></th><th>Date</th><th>Marked in at</th></tr>
      {% for r in b.done %}
      <tr>
        <td>{% if r.undoable %}<input class="cb" type="checkbox" name="undo"
              value="{{ r.d }}|{{ b.sid }}">{% endif %}</td>
        <td>{{ r.d }} &middot; {{ r.dow }}{% if r.sun %}
            <span class="pill holiday">SUN</span>{% endif %}</td>
        <td>{{ r.in }}{% if not r.undoable %}
            <span class="gate">staff request &mdash; undo on the day page</span>{% endif %}</td>
      </tr>
      {% endfor %}
    </table>
    </details>
    {% endif %}{% endfor %}
    <div style="margin:18px 0 40px"><button class="btn ghost" type="submit">Undo the ticked corrections</button></div>
  </form>
  {% endif %}
  {% endif %}
</div></body></html>
...
<div class="wrap">
  ...
  {% if bd.xreq %}
  <div class="card">
    <h2 style="margin:0 0 8px;font-size:15px;color:#fff">&#128682; Overtime requests
      <span class="pill draft">{{ bd.xreq|length }}</span></h2>
    <div class="note" style="margin:0 0 8px">Raised at the moment of leaving &mdash; the request time IS the punch-out.
      Approve now or later; only the doctor's approval counts it.</div>
    <div class="tblwrap"><table>
      <tr><th>Date</th><th>Staff</th><th>Left at</th><th>Why</th><th>Told</th><th>This month</th><th>Status</th><th></th></tr>
      {% for p in bd.xreq %}
      <tr>
        <td>{{ p.d }}</td><td class="nm">{{ p.name }}</td><td><b>{{ p.t }}</b></td>
        <td>{{ p.reason }}</td><td>{{ p.told }}</td><td>#{{ p.month_n }}</td>
        <td><span class="pill {{ 'approved' if p.status=='verified' else 'draft' }}">{{ p.status|upper }}</span>
            {% if p.verify_user %}<span class="gate">by {{ p.verify_user }}</span>{% endif %}</td>
        <td style="white-space:nowrap">
          {% if p.can_verify %}
          <form method="POST" action="{{ prefix }}/exit/verify" style="display:inline">
            <input type="hidden" name="rid" value="{{ p.id }}">
            <button class="btn ghost" type="submit">Verify</button></form>
          {% endif %}
          {% if p.can_decide %}
          <form method="POST" action="{{ prefix }}/exit/decide" style="display:inline"
                onsubmit="return confirm('Approve — the request time becomes the punch-out?');">
            <input type="hidden" name="rid" value="{{ p.id }}"><input type="hidden" name="action" value="approve">
            <button class="btn green" type="submit">Approve</button></form>
          <form method="POST" action="{{ prefix }}/exit/decide" style="display:inline">
            <input type="hidden" name="rid" value="{{ p.id }}"><input type="hidden" name="action" value="reject">
...
# S303 (F-519): read the frozen run back out of locked_run.report_html. Sheet 3's columns, as
# salary_policy.sheets34_html writes them, mapped to the engine's own field names.
_S3_COLS = [("base", "Salary"), ("adv_ded", "Advance ded."), ("leaves_total", "Leaves"),
            ("leave_amt", "Leave amt"), ("late_min", "Late min"), ("marks", "Marks"),
            ("late_charge", "Late charge"), ("collect", "Collect now"), ("held", "Hold"),
            ("prior_collect", "Prev hold deducted"), ("release", "Prev hold written off"),
            ("dress_days", "Dress"), ("icard_days", "I-card"), ("di", "D+I fine"),
            ("fines", "Fines"), ("duty_credits", "Duty credits"), ("ot_paid", "OT paid"),
            ("incentive", "Incentive (to pot)"), ("net", "NET PAYABLE")]
_S3_MONEY = ("adv_ded", "leave_amt", "late_charge", "collect", "held", "prior_collect", "release",
             "di", "fines", "duty_credits", "ot_paid", "net")
...
def locked_snapshot(report_html, total_payout):
    """[{name, own, vals{field: float}}] from a frozen report, or (None, why). Read-only.
    Accepted only when every NET is readable and the NETs add up to the locked total."""
    import re as _re
    doc = report_html or ""
    rows = []
    m = _re.search(r"<table class='s3t'>(.*?)</table>", doc, _re.S)
    if not m:
        return None, "the frozen Sheet 3 table was not found"
    for tr in _re.findall(r"<tr>(.*?)</tr>", m.group(1), _re.S):
        nm = _re.match(r"<td><b>(.*?)</b></td>", tr)
        if not nm:
            continue                                  # header row
        cells = _re.findall(r"<td class='n'>(.*?)</td>", tr)
        if len(cells) != len(_S3_COLS):
            return None, "a frozen Sheet 3 row has %d figures, not %d" % (len(cells), len(_S3_COLS))
        vals = {}
        for (key, _lbl), c in zip(_S3_COLS, cells):
            v = _num(html.unescape(c))
            if v is None:
                return None, "a frozen figure could not be read"
            vals[key] = v
        rows.append({"name": html.unescape(nm.group(1)), "own": False, "vals": vals})
    for nm, net in _re.findall(r"SHEET 5 · (.*?) — FULL WORKING — .*?<tr class='net'><td><b>NET PAYABLE</b></td>"
                               r"<td class='n'><b>(.*?)</b></td></tr>", doc, _re.S):
        v = _num(html.unescape(net))
        if v is None:
            return None, "a frozen own-sheet NET could not be read"
        rows.append({"name": html.unescape(nm).title(), "own": True, "vals": {"net": v}})   # Sheet 5 prints it in capitals
    if not rows:
        return None, "the frozen table has no staff rows"
    tot = int(round(sum(r["vals"]["net"] for r in rows)))
    if tot != int(total_payout):
        return None, "the frozen rows add up to %s, not the locked %s" % ("{:,}".format(tot),
                                                                         "{:,}".format(int(total_payout)))
    return rows, ""


def locked_differences(snap, res_staff, money):
    """Per staff: the locked NET against today's recompute, and which Sheet 3 figures moved."""
    now = {str(st["name"]).strip().lower(): st for st in (res_staff or [])}
    seen, out = set(), []
    for r in snap:
        ...
        st = now.get(key)
        if st is None:
            out.append({"name": r["name"], "locked": money(r["vals"]["net"]), "today": "not in today's recompute",
                        "diff": "", "moved": "no longer computed (e.g. marked as left)"})
            continue
        today = {"base": st["base"], "adv_ded": st["adv_ded"], "leaves_total": st["leaves_total"],
                 "leave_amt": st["leave_amt"], "late_min": st["late_min"], "marks": st["marks"],
                 "late_charge": st["late_charge"], "collect": st["collect"], "held": st["held"],
                 "prior_collect": st.get("prior_collect", 0), "release": st["release"],
                 "dress_days": st["dress_days"], "icard_days": st["icard_days"],
                 "di": st["dress_rs"] + st["icard_rs"], "fines": st["fine_uninf"] + st["fine_exc"],
                 "duty_credits": st["duty_credits"], "ot_paid": st.get("ot_paid", 0),
                 "incentive": st["incentive"], "net": st["net"]}
        if abs(float(today["net"]) - r["vals"]["net"]) < 0.005:
            continue
        labels = dict(_S3_COLS)
        moved = ["%s %s → %s" % (labels[k], money(r["vals"][k]), money(today[k]))
                 for k, _l in _S3_COLS if k != "net" and k in r["vals"]
                 and abs(float(today[k]) - r["vals"][k]) >= 0.005]
        out.append({"name": r["name"], "locked": money(r["vals"]["net"]), "today": money(today["net"]),
                    "diff": money(float(today["net"]) - r["vals"]["net"]),
                    "moved": "; ".join(moved) or ("own sheet — only the NET is frozen" if r["own"] else "—")})
    for key, st in now.items():
        if key not in seen:
            out.append({"name": st["name"], "locked": "not in the locked run", "today": money(st["net"]),
                        "diff": "", "moved": "added since the lock"})
    return out
...
def _render_salary(ym, u, msg="", msg_kind="block"):
    ...
    # S303 (F-519): a locked month shows its frozen table and one total -- the card's.
    snap, snap_err, diffs, snap_rows, snap_pot = None, "", [], [], "0"
    if is_locked:
        try:
            snap, snap_err = locked_snapshot(lr.get("report_html"), lr["total_payout"])
        except Exception as e:                                  # never let the desk fall over
            snap, snap_err = None, "%s: %s" % (type(e).__name__, e)
        if snap:
            _m = P.money if P is not None else (lambda x: "%g" % x)
            for r in snap:
                v = r["vals"]
                snap_rows.append(dict(
                    name=r["name"], own=r["own"],
                    **{k: (_m(v[k]) if k in v else "") for k in ("base", "adv_ded", "leave_amt", "collect", "held",
                                                                 "prior_collect", "release", "di", "fines",
                                                                 "duty_credits", "ot_paid", "incentive", "net")}))
            snap_pot = _m(sum(r["vals"].get("incentive", 0) for r in snap))
            if res_staff:
                try:
                    diffs = locked_differences(snap, res_staff, _m)
                except Exception as e:
                    snap_err = "today's recompute could not be compared: %s" % e
    ...
    return render_template_string(
        ...
        snap_rows=snap_rows, snap_err=snap_err, diffs=diffs, snap_pot=snap_pot,
...
@app.route(APP_PREFIX + "/salary/locked")
@require("salary")
def salary_locked_view():
    """S303 (F-519): the frozen sheets exactly as saved at the lock. Read-only."""
    ym = (request.args.get("ym") or "").strip()
    con = get_db()
    try:
        lr = con.execute("SELECT report_html, status, locked_by, locked_ts FROM locked_run WHERE ym=?",
                         (ym,)).fetchone()
    finally:
        con.close()
    if not lr:
        return "No locked run for this month.", 404, {"Content-Type": "text/plain; charset=utf-8"}
    return lr["report_html"], 200, {"Content-Type": "text/html; charset=utf-8"}
...
<h1 style="font-size:19px">What-if simulator &mdash; {{ ym }}</h1>
<div style="background:#13233b;border:1px solid #2e4a6e;border-radius:10px;padding:10px 14px;
     font-size:15px;max-width:760px;margin:6px 0">This page is a SANDBOX: it re-runs the
month's REAL punches under whatever rule numbers you try, so you can see what a policy
change would cost before ruling it. <b>Nothing here is saved and nothing touches pay</b> —
the live rules live in <a href="{{ prefix }}/salary/policy-settings" style="color:#ffd868">Settings</a>.</div>
{{ inner|safe }}
</body>"""
...
<style>
 body{font-family:"Segoe UI",Arial,sans-serif;margin:16px auto;max-width:900px;color:#111;background:#fff;font-size:13px;
      -webkit-print-color-adjust:exact;print-color-adjust:exact}
 .bar{margin:0 0 10px} .bar a{margin-right:16px;color:#05c;font-weight:bold;text-decoration:none}
 .hdr{text-align:center;border-bottom:2px solid #222;padding-bottom:4px;margin-bottom:6px}
 .hdr h1{font-size:18px;margin:0} .hdr .t{font-size:13.5px;font-weight:700;margin:2px 0 0;letter-spacing:.3px}
 .hdr .m{font-size:11.5px;color:#444;margin-top:2px}
 table.v{border-collapse:collapse;width:100%;table-layout:fixed;border:1.5px solid #333}
 table.v th{background:#e9e9e9;font-size:10.5px;padding:3px 2px;border:1px solid #777;text-align:left}
 table.v td{border:1px solid #888;padding:3px 4px;font-size:12px;vertical-align:middle;white-space:nowrap;overflow:hidden}
 ...
 col.nm{width:16%} col.dt{width:9%} col.ck{width:14%} col.fx{width:5%}
 td.nm{font-weight:700;font-size:12.5px;background:#fafafa;white-space:normal}
 td.nm small{display:block;font-weight:400;color:#444;font-size:10.5px}
 td.dt.sun{background:#d9ccff;color:#3b0f8a;font-weight:800;border-left:3px solid #3b0f8a}
 td.gap{border-left:2px solid #555}
 td.fx{text-align:center}
 table.v td.done{color:#444;font-style:italic;background:#e6e6e6;font-size:10px;white-space:normal;line-height:1.15}
 .bx{display:inline-block;width:13px;height:13px;border:1.5px solid #333;border-radius:2px;
     vertical-align:-3px;margin:0 2px 0 5px} .bx.first{margin-left:0}
 .donly{margin:8px 0 0;border:1px solid #999;border-radius:6px;padding:5px 8px;font-size:12px}
 .sunk{color:#3b0f8a;font-weight:800;text-decoration:underline}
 .sign{display:flex;justify-content:space-between;margin-top:18px;font-size:12px}
 .err{border:2px solid #b00;padding:8px;border-radius:6px}
 @page{size:A4 portrait;margin:8mm}
 @media print{.noprint{display:none!important} body{margin:0;max-width:none}
...
<div class="hdr"><h1>Dr. Manoj Agarwal Clinic</h1>
 ...
 absent &middot; {{ ndone }} already corrected &middot; tick P / L / A from the register, then Fixed</div></div>
...
{% elif not blocks %}<div class="m">No machine-absent days in {{ month }}.</div>
{% else %}
<table class="v"><colgroup><col class="nm">{% for _ in range(3) %}<col class="dt"><col class="ck"><col class="fx">{% endfor %}</colgroup>
<thead><tr><th>Staff</th>{% for _ in range(3) %}<th{% if not loop.first %} class="gap"{% endif %}>Date</th><th>P / L / A</th><th style="text-align:center">Fixed</th>{% endfor %}</tr></thead>
{% for b in blocks %}{% set rows = b["items"]|batch(3)|list %}<tbody>
{% for row in rows %}<tr>
 {% if loop.first %}<td class="nm" rowspan="{{ rows|length }}">{{ b.name }}<small>{{ b.absent }} day(s){% if b.corrected %} &middot; {{ b.corrected }} corrected{% endif %}</small></td>{% endif %}
 {% for r in row %}<td class="dt{{ ' sun' if r.sun else '' }}{{ ' gap' if not loop.first else '' }}">{{ r.d[8:10] }}-{{ r.d[5:7] }} {{ 'Sun' if r.sun else r.dow }}</td>
  {% if r.done %}<td class="done" colspan="2">&#10003; already marked present{% if r["in"] %} {{ r["in"] }}{% endif %}</td>
  {% else %}<td class="ck"><span class="bx first"></span>P<span class="bx"></span>L<span class="bx"></span>A</td><td class="fx"><span class="bx first"></span></td>{% endif %}
 {% endfor %}{% for _ in range(3 - row|length) %}<td class="gap"></td><td></td><td></td>{% endfor %}</tr>
{% endfor %}</tbody>
{% endfor %}</table>{% endif %}
{% if donly %}<div class="donly"><b>Punch dates only (not on the grid):</b>
...
def salary_flow():
    ...
    try:                                   # STEP 0 counts; the flow page never dies on them
        _vb, _vok, _vd = _verify_blocks(ym)
...
def salary_flow_approve():
    ...
    dest = {"desk":   "/salary?ym=%s" % ym,
            "sheet1": "/salary/flow/sheet1?ym=%s" % ym,
            "sheet2": "/salary/flow/sheet2?ym=%s" % ym}.get(back)
    if dest:
        return redirect(APP_PREFIX + dest)
...
def _flow_doc(kind):
    ...
    try:
        ...
        def _apv(sheet, label):
            """The approve-where-you-read strip (S200/R7). Sign-off itself stays a
            deliberate human press — D337 — it just no longer needs a page hop."""
            row = st.get(sheet)
            if row:
                return ('<div class="apvbar"><span class="done">&#9989; %s approved '
                        'by %s · %s</span> <a class="door" href="%s/salary?ym=%s">'
                        'Lock desk &rarr;</a></div>'
                        % (label, html.escape(row["approved_by"] or ""),
                           html.escape((row["approved_ts"] or "")[:16]), APP_PREFIX, ym))
            return ('<div class="apvbar"><span>&#9997; %s — reviewed and correct?</span>'
                    '<form method="POST" action="%s/salary/flow/approve">'
                    '<input type="hidden" name="ym" value="%s">'
                    '<input type="hidden" name="sheet" value="%s">'
                    '<input type="hidden" name="back" value="%s">'
                    '<button type="submit" onclick="return confirm(\'Approve %s for %s?\')">'
                    'APPROVE %s</button></form>'
                    '<a class="door" href="%s/salary?ym=%s">Lock desk &rarr;</a></div>'
                    % (label, APP_PREFIX, ym, sheet, sheet, label, ym,
                       label.upper(), APP_PREFIX, ym))

        if kind == "sheet1":
            doc = P.sheet1_html(res, doors=(not request.args.get("print")),
                                prefix=APP_PREFIX, back=back,
                                print_=bool(request.args.get("print")),
                                approve_html=_apv("sheet1", "Sheet 1 (attendance)"))
        elif kind == "sheet2":
            doc = P.sheet2_html(res, doors=True, back=back, prefix=APP_PREFIX,
                                staff=(request.args.get("staff") or None),
                                approve_html=_apv("sheet2", "Sheet 2 (money)"))
        else:
            lr_row = None
            try:
                _c = get_db()
                lr_row = _c.execute("SELECT * FROM locked_run WHERE ym=?", (ym,)).fetchone()
                _c.close()
            except Exception:
                pass
            if lr_row and lr_row["status"] == "locked":
                s34 = ('<div class="apvbar"><span class="done">&#128274; FINALISED — '
                       'locked by %s · %s</span></div>'
                       % (html.escape(lr_row["locked_by"] or ""),
                          html.escape((lr_row["locked_ts"] or "")[:16])))
            else:
                s34 = ('<div class="apvbar"><span>&#9888;&#65039; NOT FINAL — working '
                       'preview; the month is not locked yet.</span>'
                       '<a class="door" href="%s/salary?ym=%s">Lock desk &rarr;</a></div>'
                       % (APP_PREFIX, ym))
...
SETTINGS_LABELS = [
    ...
    ("collect_now_pct", "Late charge collected now (%); rest is SUSPENDED"),
    ("improve_pct", "Improvement (%) that CANCELS last month's suspended charge"),
    ...
    ("fine_ramp_step", "Absence ramp step Rs (k-th excess day = k x this, D345)"),
    ("sunday_weight_override", "Sunday absence weight (-1 = derive from shift, D341)"),
    ...
    ("ot_daily_min", "…a day's overtime under this many minutes is not counted (when switched on)"),
...
def salary_lock():
    ...
    if not res.get("ledger_closed", False):
        return _render_salary(ym, u,
        ...
        if st["release"] > 0:
            ...
            if ph and ph["status"] == "HELD":
                P.append_hold({"id": secrets.token_hex(6), "action": "RELEASE",
                               ...
                               "by": u["user"], "ts": _now()})
        # D342a/D346: no improvement -> the suspended charge was COLLECTED in
        # this month's deductions; close the hold so it can never charge twice.
        if st.get("prior_collect", 0) > 0:
            ph = existing.get((st["name"], pm))
            if ph and ph["status"] == "HELD":
                P.append_hold({"id": secrets.token_hex(6), "action": "COLLECT",
                               "staff": st["name"], "ym": pm,
                               "amount": st["prior_collect"],
                               "reason": st["release_note"],
                               "by": u["user"], "ts": _now()})
    con = get_db()
    if already:
...
def _selftest():
    ...
    save_profile(5, "2026-05-01", "", "Lab technician, Receptionist, Night desk",
                 ...
                 "Wife", "EXAMPLE-0000", "Father Name", "Father", "shavez")
    con = get_db()
    ...
    assert b"punch nahi aaya" in r.data and b"Mujhe present lagao" in r.data
    # S300 (D540): no My month link; the reasons are taps, in Hinglish
    assert b"My month" not in r.data and b"me/month" not in r.data, "S300: My month link removed"
    assert b'type="radio" name="reason" value="finger"' in r.data and b"Machine ne finger nahi padha" in r.data
    assert b'<input name="reason"' not in r.data, "S300: no typed reason box"
    ...
    rT = c5.post(APP_PREFIX + "/me/request", data={"reason": "machine did not read finger"})
    assert "c=err" in rT.headers.get("Location", ""), "S300: typed words are refused"
    con = get_db()
    assert con.execute("SELECT COUNT(*) FROM present_request WHERE staff_id=6").fetchone()[0] == 0, \
        "S300: a typed reason writes nothing"
    con.close()
    c5.post(APP_PREFIX + "/me/request", data={"reason": "finger"})
    con = get_db()
    ...
    assert pr["req_ts"][:10] == _today() and pr["reason"] == "Machine did not read finger", \
        "S300: the tapped key is stored as its English label"
    con.close()
    c5.post(APP_PREFIX + "/me/request", data={"reason": "machine_off"})
    con = get_db()
    assert con.execute("SELECT COUNT(*) FROM present_request WHERE staff_id=6")\
        .fetchone()[0] == 1, "one request per day"
    con.close()
    r = c5.get(APP_PREFIX + "/me")
    assert b"par bheji" in r.data
    ...
    c6.post(APP_PREFIX + "/me/request", data={"reason": "finger"})
    con = get_db()
    ...
    con.close()
    c3.post(APP_PREFIX + "/me/request", data={"reason": "machine_off"})
    con = get_db()
    ...
    con.close()
    r = c5.get(APP_PREFIX + "/me")
    assert b"Present maan liya" in r.data
    ...
    con = get_db()
    assert con.execute("SELECT reason FROM present_request WHERE staff_id=9").fetchone()[0] == "Forgot to punch"
    con.close()
    con = get_db()
    ...
    assert b"manzoor nahi hui" in r.data and b"was not seen at clinic" in r.data
    # ---- S289: mark my exit -------------------------------------------------
    # the opening time is the roster's shift end; fallback SR_EXIT_AFTER
    assert _exit_open_at(7, "2026-08-06").strftime("%H:%M") == "18:00", "weekday shift end"
    assert _exit_open_at(2, "2026-09-06") is None, "B on the 1st roster Sunday is OFF"
    assert _exit_open_at(4, "2026-08-06").strftime("%H:%M") == SR_EXIT_AFTER, \
        "minutes_exempt uses the fallback"
    assert _exit_open_at(99, "2026-08-06").strftime("%H:%M") == SR_EXIT_AFTER, \
        "no shift row uses the fallback"
    _real_now, _real_open = _now, _exit_open_at
    _g = globals()
    _g["_exit_open_at"] = lambda sid, d: datetime.datetime.combine(
        datetime.datetime.strptime(d, "%Y-%m-%d").date(), datetime.time(18, 0))
    try:
        _g["_now"] = lambda: _today() + " 17:59:00"
        r = c6.get(APP_PREFIX + "/me")
        assert b"18:00 baje se khulega" in r.data and b"Mark overtime" not in r.data, "too early: no button"
        c6.post(APP_PREFIX + "/me/exit", data={"reason": "overtime", "told": "manoj"})
        con = get_db()
        assert con.execute("SELECT COUNT(*) FROM exit_request").fetchone()[0] == 0, \
            "before shift end the server refuses"
        con.close()
        _g["_now"] = lambda: _today() + " 22:40:00"
        r = c6.get(APP_PREFIX + "/me")
        assert b"Mark overtime" in r.data and b"Told Dr Bhawna" in r.data and b"Told Dr Manoj" in r.data
        assert b"Told Reception" not in r.data and b"Doctor asked me to stay" not in r.data, \
            "S300: two buttons only, no reason list"
        assert r.data.index(b"Told Dr Bhawna") < r.data.index(b"Told Dr Manoj"), "S300: the owner's order"
        r = c6.post(APP_PREFIX + "/me/exit", data={"reason": "typed words", "told": "manoj"})
        assert "c=err" in r.headers.get("Location", ""), "reason must be a choice"
        r = c6.post(APP_PREFIX + "/me/exit", data={"reason": "late_patients", "told": "manoj"})
        assert "c=err" in r.headers.get("Location", ""), "S300: an old reason key is no longer offered"
        r = c6.post(APP_PREFIX + "/me/exit", data={"reason": "overtime", "told": "nobody"})
        assert "c=err" in r.headers.get("Location", ""), "told must be a choice"
        r = c6.post(APP_PREFIX + "/me/exit", data={"reason": "overtime", "told": "reception"})
        assert "c=err" in r.headers.get("Location", ""), "S300: reception is not a button"
        con = get_db()
        assert con.execute("SELECT COUNT(*) FROM exit_request").fetchone()[0] == 0, \
            "S300: every refusal wrote nothing"
        con.close()
        r = c6.post(APP_PREFIX + "/me/exit", data={"reason": "overtime", "told": "bhawna"})
        import urllib.parse as _up
        _loc = _up.unquote_plus(r.headers.get("Location", ""))
        assert "Overtime 22:40 par bheja" in _loc and "c=ok" in _loc, _loc
        c6.post(APP_PREFIX + "/me/exit", data={"reason": "overtime", "told": "manoj"})
        con = get_db()
        xr = con.execute("SELECT * FROM exit_request WHERE staff_id=7").fetchall()
        assert len(xr) == 1 and xr[0]["req_ts"] == _today() + " 22:40:00" \
            and xr[0]["status"] == "pending" and xr[0]["told"] == "bhawna", "one per day, server time"
        assert 7 in machine_day(con, _today()) and machine_day(con, _today())[7]["n"] == 1, \
            "a PENDING exit is not a punch"
        con.close()
        r = c6.get(APP_PREFIX + "/me")
        assert b"Overtime <b>22:40</b> par bheja" in r.data
        # sandip: present request rejected, no machine punch -> refused
        r = c7.post(APP_PREFIX + "/me/exit", data={"reason": "overtime", "told": "manoj"})
        assert "Pehle" in _up.unquote_plus(r.headers.get("Location", "")) or \
            "pehle" in _up.unquote_plus(r.headers.get("Location", "")), "S300: the refusal reads Hinglish"
        con = get_db()
        assert con.execute("SELECT COUNT(*) FROM exit_request WHERE staff_id=9").fetchone()[0] == 0, \
            "no punch-in -> no exit"
        con.close()
        # sukhveer: in by an APPROVED present request -> may raise an exit
        c5.post(APP_PREFIX + "/me/exit", data={"reason": "overtime", "told": "manoj"})
        con = get_db()
        x6 = con.execute("SELECT * FROM exit_request WHERE staff_id=6").fetchone()
        con.close()
        assert x6 and x6["status"] == "pending", "a present request counts as being in"
        # the board: checker may verify, maker may not decide, doctor decides from PENDING
        r = c.get(APP_PREFIX + "/review")
        assert b"Overtime requests" in r.data and b"Ranjeet K" in r.data
        assert b"<td>Overtime</td><td>Dr Bhawna</td>" in r.data, "S300: the doctors read the labels"
        rX = c2.post(APP_PREFIX + "/exit/decide", data={"rid": xr[0]["id"], "action": "approve"})
        assert rX.status_code == 403, "maker cannot decide an exit"
        c3.post(APP_PREFIX + "/exit/verify", data={"rid": x6["id"]})
        c.post(APP_PREFIX + "/exit/decide", data={"rid": xr[0]["id"], "action": "approve"})
        c.post(APP_PREFIX + "/exit/decide", data={"rid": x6["id"], "action": "reject",
                                                   "note": "left at 20:00"})
        con = get_db()
        assert con.execute("SELECT status FROM exit_request WHERE id=?", (xr[0]["id"],))\
            .fetchone()[0] == "approved", "approve straight from pending"
        x6b = con.execute("SELECT * FROM exit_request WHERE id=?", (x6["id"],)).fetchone()
        assert x6b["status"] == "rejected" and x6b["verify_user"] == "shavez"
        mX = machine_day(con, _today())
        assert mX[7]["n"] == 2, "an APPROVED exit is a punch on the day picture"
        assert con.execute("SELECT COUNT(*) FROM audit_log WHERE entity='exit_request'")\
            .fetchone()[0] >= 4, "raise/verify/decide audited"
        con.close()
        r = c6.get(APP_PREFIX + "/me")
        assert b"Overtime manzoor" in r.data and b"22:40" in r.data
        r = c5.get(APP_PREFIX + "/me")
        assert b"manzoor nahi hua" in r.data and b"left at 20:00" in r.data
        try:
            decide_exit(xr[0]["id"], "manoj", "reject")
            raise AssertionError("a decided exit cannot be decided again")
        except PermissionError:
            pass
    finally:
        _g["_now"], _g["_exit_open_at"] = _real_now, _real_open
    ...
    # ---- D339 (S200): the month FIX-ABSENTS desk ----
    rF = c2.get(APP_PREFIX + "/fixabsents?ym=2026-08")
    assert rF.status_code == 403, "maker must not reach the fix-absents desk"
    rF = c.get(APP_PREFIX + "/fixabsents?ym=2026-08")
    ...
    assert b'name="pick" value="2026-08-04|6"' not in rF.data, \
        "a staff-day already corrected must not be offered for correction again"
    assert b'name="pick" value="2026-08-04|1"' in rF.data, \
        "an uncorrected absence must be offered"
    con = get_db()
    _blocks, _feed = absent_worklist(con, "2030-01")
    assert all(not b["rows"] for b in _blocks), "future days must never be offered"
    _old_csv = SR_PUNCH_CSV
    SR_PUNCH_CSV = os.path.join(tmp, "no_such_punches.csv")
    _bl, _fk = absent_worklist(con, "2026-08")
    assert _fk is False and _bl == [], "a dead feed must yield no list, not a clean one"
    SR_PUNCH_CSV = _old_csv
    con.close()
    rF = c.post(APP_PREFIX + "/fixabsents/apply",
                data={"ym": "2026-08", "reason": "machine missed them",
                      "pick": ["2026-08-04|1"], "t_2026-08-04|1": "09:15"})
    assert rF.status_code in (301, 302) and "c=ok" in rF.headers.get("Location", "")
    con = get_db()
    pb = con.execute("SELECT * FROM present_request WHERE reg_date=? AND staff_id=1",
                     ("2026-08-04",)).fetchone()
    assert pb and pb["status"] == "approved" and pb["req_ts"] == "2026-08-04 09:15:00", \
        "the desk must write through correct_present"
    con.close()
    rF = c.post(APP_PREFIX + "/fixabsents/apply",
                data={"ym": "2026-08", "reason": "again",
                      "pick": ["2026-08-04|1"], "t_2026-08-04|1": "09:15"})
    assert "c=err" in rF.headers.get("Location", ""), \
        "a duplicate must be reported as refused, not silently swallowed"
    rF = c.post(APP_PREFIX + "/fixabsents/apply", data={"ym": "2026-08", "reason": "x"})
    assert "c=err" in rF.headers.get("Location", ""), "nothing ticked must say so"

    # ---- D339b (S200): the reason survives, corrections list, undo ----
    rF = c.get(APP_PREFIX + "/fixabsents?ym=2026-08")
    assert b'value="machine missed the punch"' in rF.data, \
        "an empty reason box is what silently blocked the whole form — must default"
    assert b"Corrections already made" in rF.data and b"2026-08-04|1" in rF.data
    rF = c.get(APP_PREFIX + "/fixabsents?ym=2026-08&r=cover+duty")
    assert b'value="cover duty"' in rF.data, "the reason must survive a save"
    rF = c.post(APP_PREFIX + "/fixabsents/apply",
                data={"ym": "2026-08", "reason": "kept text",
                      "pick": ["2026-08-05|1"], "t_2026-08-05|1": "09:20"})
    assert "r=kept+text" in rF.headers.get("Location", ""), \
        "the redirect must carry the reason back to the page"
    rU = c2.post(APP_PREFIX + "/fixabsents/undo",
                 data={"ym": "2026-08", "undo": ["2026-08-04|1"]})
    assert rU.status_code == 403, "maker must not undo a correction"
    rU = c.post(APP_PREFIX + "/fixabsents/undo",
                data={"ym": "2026-08", "undo": ["2026-08-04|1"]})
    assert "c=ok" in rU.headers.get("Location", "")
    con = get_db()
    assert con.execute("SELECT 1 FROM present_request WHERE reg_date=? AND staff_id=1",
                       ("2026-08-04",)).fetchone() is None, "the undo must remove the row"
    assert con.execute("SELECT 1 FROM audit_log WHERE action='undo_past_day'").fetchone(), \
        "an undo must leave an audit trail"
    # a staff's OWN request (D334) is not a correction and must survive an undo attempt
    _own = con.execute("SELECT reg_date, staff_id FROM present_request "
                       "WHERE decide_note IS NULL OR decide_note NOT LIKE '%D33%'").fetchone()
    con.close()
    if _own:
        try:
            uncorrect_present("manoj", _own["reg_date"], _own["staff_id"])
            raise AssertionError("a staff request must not be deletable as a correction")
        except PermissionError:
            pass
    rU = c.post(APP_PREFIX + "/fixabsents/undo", data={"ym": "2026-08"})
    assert "c=err" in rU.headers.get("Location", ""), "nothing ticked must say so"

    # feed-down: no punch file -> page says so and the request refuses loudly
    SR_PUNCH_CSV = os.path.join(tmp, "missing_punches.csv")
    r = c6.get(APP_PREFIX + "/me")
    assert b"jaankari abhi nahi mil rahi" in r.data
    try:
    ...
    print("SELFTEST OK — all-clear, exceptions, Arjun (D276), late+approver, "
          ...
          "v0.9/D339: fix-absents desk (approver-only, roster/holiday/future/corrected exclusions, dead-feed refusal, bulk write through correct_present), "
          "v0.10/D339b: reason defaulted+carried (the silent-form-block fault), "
          "corrections listed, undo (approver-only, correction-rows only, audited), "
          "S289: mark my exit (shift-end opening + fallback + roster OFF, tap-only choices, "
          "server-time punch-out, one/day, in-punch or present request required, verify optional, "
          "doctor decides from pending, approved exit = punch on the day picture, audited), S300/D540: self page Hinglish + tap-only (present reasons as keys, typed words refused, English label stored), Mark overtime = two buttons Told Dr Bhawna / Told Dr Manoj (old reason keys and reception refused, refusals write nothing), My month link gone, review card Overtime requests.")
    return True
