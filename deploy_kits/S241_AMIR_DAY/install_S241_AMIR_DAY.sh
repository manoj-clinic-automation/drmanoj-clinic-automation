#!/bin/bash
# =============================================================================
#  install_S241_AMIR_DAY.sh
#
#  KIT   S241_AMIR_DAY -- Amir's day as seven steps on a phone (D472).
#  RUN   bash /root/deploy/vps_deploy.sh S241_AMIR_DAY
#
#  WHAT CHANGES
...
red() {
  if [ "$RESTARTED" = "1" ] && [ -n "$PATCH_BAK" ] && [ -f "$PATCH_BAK" ]; then
    \cp -f "$PATCH_BAK" "$FA"
    systemctl restart "$SVC" >/dev/null 2>&1
    echo "   finance_app.py put back and the service is up again"
  fi
  echo "!! RED -- $*"
  exit 1
}
...
if [ -f "$FIN/amir_day.py" ]; then
  ...
  \cp -f "$FIN/amir_day.py" "$FIN/amir_day.py.bak_S241_$OLD8"
fi
...
echo "$KIT GREEN -- Amir's day is live. Open it on a phone:"
