...
# =============================================================================
#  clinic_state_backup.py  ·  Session 233  ·  S233_SHEETS_BACKUP  ·  v3
#  (v1 S230 · v2 S230 widened the store walk after F-355 · v3 S233 adds the
#   Google Sheets export directory as a source · one source row, and the
#   kit tag in the state file so a restore knows which kit shipped it)
#
...
SRC_DIRS = [
    "/root/staff_register",
    "/root/staff_ledger",
    # v3, 08-Sep-2026 (S233) — THE SEVEN SHEETS. S232 measured the Google plane
    # and found the live Google Sheets had zero copies anywhere: not here, not
    # in the repository, not on the SSD. They are the data; the code can be
    # rewritten. sheets_pull.py (cron 01:45, five minutes before this job)
    # writes one CSV per tab under this directory, plus _TAKEN_AT.json so the
    # export can state its own age from inside a restored bundle. Nothing else
    # about this job changes: the same walk, the same tar, the same key, the
    # same two owner-owned slot files, the same proven restore. The owner's
    # instruction was to put them inside the mechanism that already works and
    # to invent no second one.
...
def run(drive_cls=Drive, session_maker=make_session, now=None):
    ...
    try:
        ...
        st = {
            ...
            "kit": "S233_SHEETS_BACKUP",   # v3; the mechanism is still S230's
        }
        save_state(conf, st)
