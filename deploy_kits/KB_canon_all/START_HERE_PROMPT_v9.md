# START-HERE PROMPT — v9 — the project's custom instructions
...
## HOW TO HAND ME ANYTHING — earned rules, S206–S258
...
**3 ·** Open **`CANONICAL_MANIFEST.md`** (Tier 0 · the linchpin).
...
## THE CAP AND THE TRANCHE

Project knowledge is capped at 2 MB. **One reduction tranche moves at every close, measured — never
projected — before and after.** `KB_EXTENSION_PLAN` gives the order; nine documents never move, and
**no document moves that is its own only copy.** The durable answer remains branching
Sanjeevni/Marg into its own project.
...
---
*START_HERE_PROMPT — v9 · supersedes v8 · adopted at the S258 close, 15-Sep-2026. This evergreen
prompt is the custom-instructions template; `START_HERE_SESSION_###` entry points are regenerated at
each close and carry the numbers and the current canon.*
