...
| `/root/finance/marg_backfill.py` | `e101c595619dc39a19397abb040d64c9` | **S182 NEW (placed, not yet run to completion)** — Marg fortnight backfill driver. DRY RUN BY DEFAULT; surveys filed vs unfiled days, backs up `finance.db` before the first write, refuses an inactive source or a column map that does not match `marg_report.py`'s real headers, and ABORTS the run if rows read ≠ rows in the CSV (the silent-zero-rows trap). Writes `sale_item` only — the drug-line half (`finance_returns.load_lines` → `sale_line_item`) is the S183 v2 |
...
## FINDINGS — F-0 … F-99 (next free F-100).
...
**END OF KB REGISTER v5.4 (S182). Current-state + index only; all session history is in KB History Archive v1.30 (§S65–§S182). If this end-marker or the CURRENT LIVE FILE VERSIONS table is absent, this file is truncated and must not be used as canonical.**
