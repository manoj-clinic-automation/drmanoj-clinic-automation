# KB REGISTER — v5.1 (Tier 0 · current state · rides the session loop) — S179 finance subsystem
...
> **How to use.** *Now* → this file. *History* → **KB History Archive v1.27** (Tier 1). *Live backlog* → **HANDOFF_RUNBOOK §2** (v113). *Findings register* → **Fault_Action_Register v2.16** (Tier 1; consolidated F-82+F-83 append owed → v2.17, entry text in `Fault_Register_append_F82_F83_S177.md`; **F-84 append owed too — `Fault_Register_append_F84_S179.md`**). *Doc manifest + hashes* → **CANONICAL_MANIFEST.md**.
...
| File | md5 | live as of |
|---|---|---|
| `/root/finance/finance_app.py` | `61e36d5522e4e99e1e65e159ef50c85e` | S179 — maker entry + checker review, month close, parked cash, cutover, scanner host, UPI endpoint (smoke 176/176) |
| `/root/finance/finance_ingest.py` | `872ec33ef7c628cd474224b0c6c78ba5` | S179 — pluggable line adapters by column map (selftest 30/30) |
...
**Next free clinic decision: D314.** Asset-app decisions **A-D1–A-D24** are *sub-project scope* (next free A-D25) — indexed in `KB_Asset_Register` + Archive §S173–§S177, and never consume clinic D-numbers.
...
## FINDINGS — F-0 … F-84 (next free F-85).
...
*Post-split Register line = v2.0 (S147, the D247 split) → v5.0 (S178, compaction) → v5.1 (S179, finance subsystem). v1.38–v1.72 = the pre-split monolith, now the KB History Archive.*


---

**END OF KB REGISTER v5.1 (S179). Current-state + index only; all session history is in KB History Archive v1.27 (§S65–§S179). If this end-marker or the CURRENT LIVE FILE VERSIONS table is absent, this file is truncated and must not be used as canonical.**
