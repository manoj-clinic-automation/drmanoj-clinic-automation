...
| `START_HERE_PROMPT_v6` | **S207 close** | `dfad82a24b331096a4c0a2ab79844ca7` | **SUPERSEDED by v7 (S208 close)** — evergreen prompt. **⚠ TWO v6 TEXTS EXIST:** this pinned `dfad82a2…` and the ClaudeCowork S207-snapshot copy `a1d47a2d…` (back-filled into `KB_canon_all` at the S208 close, since the folder had never held the file the manifest pinned). Divergence RECORDED, not reconciled — it falls under the owner's still-open S207 manifest-divergence ruling; v7 supersedes both texts / custom-instructions template. Names the four stores with the **real** ClaudeCowork path, the F-185 rule, the standing KB reduction, and **Phase 0 step 1 — check the folders AND the SSD and PROMPT the owner** rather than working around a missing one. Carries the cold-kit cadence, the SSD structure and the retention rule. Supersedes v5. *(Re-hashed after the same-session amendment; the earlier `3e436b19…` never left this close.)* |
