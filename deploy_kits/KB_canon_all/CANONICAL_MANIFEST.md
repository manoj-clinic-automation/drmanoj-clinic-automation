...
|---|---|---|---|
| `CANONICAL_MANIFEST.md` | **S203 close** | *(self — recomputed last, each EOS)* | this file; the linchpin. **Its own Version cell read `S179` from S179 until S203** — a claim about itself that had gone stale exactly as the STATUS block did (F-116/F-45). Corrected here, visibly. |
| `START_HERE_SESSION_204` | **S203 close** | `3281b7192b9f153a4e8fbf28fd63e3f4` | **CURRENT** session entry point. Phase 0 is **ALL TIERS by md5**, and gains **project-knowledge headroom** as a standing report (it hit 98% at the S202 close with nothing watching it). Standing checks F-88 · F-107 inverse · **F-119** (`md5sum -c` must exit 0) · F-123 · A8 · **F-190** (`.gitattributes` still pins `*.md text eol=lf`). `verify_live_pins.py` expects **GREEN against `live_pins_S203close.txt`** — and the START_HERE now states what that checker **cannot** see: the medical PC and manojz are **blind to it by construction** (F-186). Build discipline hardened: **`py_compile` AND `pyflakes`, every time** — `py_compile` passed a `NameError` twice at S203 (F-204). **A copy block is the default for EVERY machine** (owner ruling), and **where Claude has write access it installs directly**. ⭐1 order: **D350 §2/§3/§4 — the reinstall kits FIRST** (Marg and its data before the pipeline) · **F-195** make the pipeline-gate check actually bite · **F-200** the inverse check across stores · **F-206/AF-1**. |
...
| `KB_Register` | **v5.55** | `5cb438b301adb610aee99aadee73eaa9` | **CURRENT — S203 CLOSE.** The session that looked at the machines themselves. **D351 minted** (the Marg/medical three-file documentation model). **F-194 … F-206 appended**, five of them the assistant's own. **A ruling amended by a ruling: D347's “Tailscale is NOT load-bearing” is CORRECTED here — Tailscale is the SOLE transport**, proven by the 26-Aug outage; the amendment sits in the decisions index. **AF-1's proposed strike was REFUSED on measurement — AF-1 is LIVE**, armed in `SEND_TO_CLINIC.bat` on the medical PC. D348's conflict with `MARG_INGESTION_REFERENCE_v1` §9 item 5 is recorded, flagged and NOT resolved. Live-file table carries the seven pins that moved and the first medical-PC rows. Next free **D352 · F-207 · A-D25**. **The live-pin list is generated from THIS version** (A8). |
...
| `HANDOFF_RUNBOOK` | **v137** | `7f106816db8698f731d0afc777c217f0` | **CURRENT.** S203 close — §0 the session that looked at the machines themselves (the three-fault chain; the first medical-PC pins; the backup measured and the record found wrong; sixty-nine documents to three) · §1 the mental models added this session · §2 the live backlog, with `OWNER_TODO_LIVE.md` named as the maintained copy and this a close-time snapshot · §3 install discipline updated (`pyflakes` beside `py_compile`; copy blocks everywhere; install where you have write access) · §4 the EOS automation boundary, moved deliberately and recorded. |
...
| Doc | Version | md5 | Notes |
|---|---|---|---|
| `KB_History_Archive` | **v1.50** | `7c0834bcfc25e2a7e6e0585915536a42` | **CURRENT.** **§S203 appended at the S203 close.** §S203 is the last section; all earlier history verbatim and untouched. |
...
| `MARG_WALL_CARD` | S203 · D351 · **printed, not read** | `dd0185630ffa2e339f71c2c20ef6807c` | one page of HTML, printed and kept **beside the medical PC**: what the machine does, what must stay on, the 60-second check, who to call. The only canonical document whose delivery surface is paper. |
...
| `Fault_Action_Register` | **v2.42** | `ef5cea3e11c997edc8cf3ef8f2bd7c8a` | **CURRENT** findings register (F-0 … F-206). **v2.42 (S203 close): F-194 … F-206 appended, and AF-1's proposed strike REFUSED on measurement.** F-194 the B2 gate (`/finance/api/pipeline-status` never added to `_gate()`'s exemption list — **B2 had never once reported**; the third instance of AF-2's born-dead shape) · **F-195 OPEN** the smoke suite posts that route on a **signed-in** client, so the token clause was never exercised — and the check added at S203 to fix it does NOT bite, recorded as green-and-meaningless rather than left looking like coverage · F-196 `-- ok` written unconditionally · F-197 the pull kept no log · F-198 `marg_router.py` lost an unreadable file for ever · F-199 manojz's mirror is `robocopy /E` with no `/PURGE` · **F-200 OPEN** project knowledge was the stale store, not the repo · F-201 F-191(c) was wrong — the backup was **never scheduled** · **F-202 OPEN** the token lives in FIVE stores, not three · F-203 the manojz runbook copy was the S201 version · F-204 `py_compile` passed a `NameError` twice · F-205 thirteen documents produced while consolidating sixty-nine away · **F-206 — AF-1 is LIVE**, armed in `SEND_TO_CLINIC.bat`; the claim earlier in the session that it was dead was **false and refused on evidence**. Next free **F-207**. |
