...
**STATUS: canonical — current at the **S228 CLOSE** (07-Sep-2026 · THE LOSS DESK, THE ITEM LEDGER READ, AND THE 581 UNDATED PURCHASE LINES — `S228_LOSS_DESK` installed to five predicted pins; Marg's batch-wise item ledger read and proven on three exports with the archive router taught to accept it; `S228_THREE_WAY` built and held back; and then, on the owner's instruction, the data audit that found **F-340 — 581 purchase lines, every one of April, May and June, undated on the server and dropped silently by every stock query**, the root of the S227–S228 disagreement with Marg. **F-342: `finance.db` was readable from the owner's Drive all along.** Two documents written this session were wrong and were withdrawn (F-347). D390–D397; F-340 … F-350). Register **v5.77** · Archive **v1.74** (+14,775 → 1,196,849, prefix proven) · Fault **v2.59** · Runbook **v160** · **START_HERE_SESSION_229** · `S229_BUILD_BRIEF` · `live_pins_S228close.txt`. Next free: **D398 · F-351 · Session 229.****
...
| `live_pins_S228fold.txt` | **S228 post-close fold** | `2aa4c0340e1add4975e1fdf226f1fde4` | **CURRENT** pin list, generated by `gen_live_pins.py` v1.2 from `KB_Register_v5_78_S228fold` against this manifest — `register_pin_verified: yes`. **93 VPS rows · 71 BLIND.** `stock_app.py` **55cd610e** and `stock_report.html` **bd750dc8** are now in the VPS set as PASSES — the owner read all six back off the box and every one matched. Nothing in this list is a prediction. |
...
| `KB_Register` | **v5.78** | `49e91b58a8868243256bd6141a26d594` | **CURRENT — S228 post-close fold.** `S228_THREE_WAY` is live and read back from the box; F-350's remedy is in place. The live-shape read of the installed page is OWED at the S229 open (F-242 login loop). |
...
| `Fault_Action_Register` | **v2.59** | `43d892103d508c322e433233cf4f8974` | **CURRENT** findings register. F-0 … F-350; **next free F-351**. S228 close minted **F-340 … F-350** — 581 undated purchase lines · three missing sale days · the server database readable from Drive all along · Marg's negative balances printed without their minus · mismatched units on one row · a money sort in a moneyless copy · CSS widths summing to 109 per cent · two documents withdrawn · Marg's Excel-TIME ledger · the router refusing every item ledger · last-write-wins on `stock_snapshot`. |
