...
| `KB_Register` | **v5.53** | `c93782251a1004f2103f457c9625e2cf` | **CURRENT.** **v5.53 (S202, kit `S202_PICTURE`):** two faults on manojz, **both found by the owner USING the system rather than by any test.** Every report ever pushed reached the server called **`REPORT_1.XLS`** — Marg's slot name — so the approvals card listed them all identically and only a hash fragment distinguished them; the upload now carries the archived filename. And **one deliberately-generated 12-June report made `MARG_PICTURE.txt` claim 56 MISSING DAYS**, where the day before it read 0, because the coverage window ran from the earliest report ever seen. Fixed in two steps — an operational window with older reports shown as **backfill, not gaps**, and then a **DECLARED** start date (`MargArchive/_coverage_from.txt`, 2026-08-17) because the machine must not guess when coverage began. **56 false alarms became one real one: 25-Aug, a trading day whose report was never produced.** Selftest **49 → 53, +4 exactly**. Pins `margpull/marg_gate.py` → **`af2c3ca507136f3f82ec7cf64e8aae34`** and the new `_coverage_from.txt`. Next free **D350 (contract WRITTEN, unsigned) · F-188 · A-D25**. |
...
| `HANDOFF_RUNBOOK` | **v135 (S201 close)** | `716c6f54329f43a56c7a1b78b3cc9392` | **CURRENT.** §0 the S201 narrative (the outbox with no consumer; three kits; D347/D348; the three assistant faults recorded not softened) · §1 mental models (+10: *a queue with no consumer is a hole* · *an unmaintainable dependency is a scheduled outage* · *reading what the server SENT is a distinct check* · *when the count is right and the shape is wrong, no counting test will see it* · *a component that must not self-update must self-report* · *a threshold imported from another domain measures nothing here* · *a number that is never zero* · *the owner's own words are the better label* · *don't mix a behaviour change into a labelling fix* · *ask a register what it does NOT list*) · §2 the live backlog (**⭐1 is `/ops`, with B2 as its prerequisite**) · §3 install discipline (+ verify against the LIVE file never the repo copy · an installer prints what it VERIFIED · stop before touching files on Windows · a supervisor reports drift · deliver one double-click, never a console paste · always the complete path and the machine) · §4 the boundary (now including **A10**). File `HANDOFF_RUNBOOK_2026-08-25_Session201close_v135.md`. |
...
| Doc | Version | md5 | Notes |
|---|---|---|---|
| `KB_History_Archive` | **v1.48** | `d17b2d26adb5a9e49f84e6b448231cc7` | **CURRENT.** **§S201 appended at the S201 close** as a **pure append** — the **860,641 characters** before v1.47's END marker **proven byte-identical by direct comparison** (+16,258); END marker present (§S201 last); **the v1.16 header line deliberately carried forward unchanged**, because bumping it would break the very property the next close must prove. Carries the full Marg-pipeline narrative, the D347/D348 texts, the capture table, and the three assistant faults. |
...
| `Fault_Action_Register` | **v2.40** | `7a14604e3ab3dccfcf17ba363ec13a78` | **CURRENT** findings register (F-##). **v2.40 (S202 open): F-185 + F-186 appended, both found by MEASURING what a previous ruling had only estimated.** F-185 — **real patient data, including named patients with their DIAGNOSES, in the PUBLIC repository**; F-96 recorded 7 mobiles / 2 names / 1 clinic ID across 48 files and **D320 ruled on that evidence**, while measurement found **133 distinct mobile-shaped numbers across ~190 files**, because F-96 examined the canonical DOCUMENT set and never code or `test-data`. **OPEN pending the owner's re-ruling.** F-186 — the live-pin discipline reaches only the VPS; a real PC-side drift sat in the gap and the record is corrected from the box. §7 index F-184 → **F-186**; next free **F-187**. Reverse-application-proven onto the `78f231a1…` pin. |
