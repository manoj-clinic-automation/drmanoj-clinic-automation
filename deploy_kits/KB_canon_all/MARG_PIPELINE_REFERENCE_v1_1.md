...
## 4 · WHERE THE TOKEN LIVES — **five copies, not three**

A rotation performed from any one of these breaks the others. No previous doc listed them together.

> **CORRECTED AT S205 (F-202).** This section said **three** copies from S201 until now, and
> **three is wrong** — the machines were measured at S203 and they carry **five**. The two
> below that this document never listed are the dangerous ones, because a rotation that
> reaches the three named copies leaves two live tokens in the world and nothing says so.
> The count was never measured when it was written; it was believed.
...
4. **manojz, a SECOND loose copy** — `D:\Downloads\MARG_TOKEN_S187.txt`. Written during the
   S187 install and never removed. Nothing reads it; it is simply a live secret sitting in a
   Downloads folder.
5. **manojz, a THIRD loose copy** — under `margsync\_to_delete\S201_20260825\loose\`. Parked
   there by a cleanup that deliberately deletes nothing (D-rule: nothing is auto-deleted), so
   the token was parked rather than destroyed.

**Copies 4 and 5 are not read by anything.** That does not make them harmless: they are the
same secret, in plain text, on a machine, and a rotation that does not reach them leaves them
valid.

**Rotation is the oldest open item in the project (21-Aug).** Both tokens transited chat during the
S195 diagnosis. **Rotation is parked at the owner's instruction (26-Aug); when it is unparked it
must reach FIVE stores.**
...
## 9 · KNOWN GAPS — **updated at S205; two of these are closed and are marked so**

- ~~**No PC-side live pins.**~~ **PARTLY CLOSED at S203:** the first medical-PC pins were taken
  (8 files), and `S205_LIVE_TOOLS` now holds 24 manojz + medical files each verified **against
  its live source**, not against its own `SUMS.md5`. **The structural gap remains (F-186):**
  `verify_live_pins.py` still runs on the VPS and is blind to both PCs by construction, so
  nothing on a schedule notices PC-side drift — a person has to look.
- **The medical guard cannot run** — its Python has no spreadsheet reader at all.
- **Deep verification** exists only for sale reports.
- ~~**Health checks cannot see this side of the line.**~~ **CLOSED, in three steps.** B2 (S202)
  built the reporter and the health-page checks; S203 fixed the gate that was refusing every
  post (F-194); **S205 added the LINK itself** — Tailscale at both ends, a *performed* share
  test, which point is down in words, which transport carried the cycle, and whether a
  credential exists at all. **The lesson that forced it: on 26-Aug both endpoints were healthy
  and the wire between them was dead for 8h40m.** Every check before S205 watched an endpoint.
- **`xlsx_stdlib.py` is not yet on the medical PC** — not needed until verification runs there.

---
*MARG_PIPELINE_REFERENCE **v1.1** · S201, CORRECTED S202 and **S205** · verified against the
running systems. No patient identifiers
