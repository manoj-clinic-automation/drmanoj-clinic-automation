# KB REGISTER — v5.4 (Tier 0 · current state · rides the session loop) — S182 portal tiles LIVE + a fail-open identity default closed
...
**Next free clinic decision: D321.** Asset-app decisions **A-D1–A-D24** are *sub-project scope* (next free A-D25) — indexed in `KB_Asset_Register` + Archive §S173–§S177, and never consume clinic D-numbers.
...
## FINDINGS — F-0 … F-104 (next free F-105).
...
**END OF KB REGISTER v5.5 (S183). Current-state + index only; all session history is in KB History Archive v1.30 (§S65–§S182). If this end-marker or the CURRENT LIVE FILE VERSIONS table is absent, this file is truncated and must not be used as canonical.**
