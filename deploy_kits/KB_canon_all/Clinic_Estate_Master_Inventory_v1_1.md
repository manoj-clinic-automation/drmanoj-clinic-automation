...
**Created:** 07 Aug 2026 · **Version:** v1.1 doc-series (canonical pin lineage: v1.7 S157 → **v1.1 S177**) — S177 update: Asset Register row advanced to **v1.11.0** (S173–S177 build arc) + NEW `/root/shared/` shared-libs row. All other rows carried verbatim from the v1.7 pin (`3668a915…`).
...
| **Asset Register** | `assets.dr-manoj.in` :8030 | Website/SEO | **3 roles (owner/manager/reception)** — SSO doctor→owner · manager→manager · staff→reception (fail-closed) | Doctor + Manager + Reception (intake only) | V |
...
| 12 | `assetapp.service` (2 workers) | 8030 | `assets.dr-manoj.in` | **Asset Register** — full estate app: taxonomy (entity×zone) · contract/period engine · service logs + parts-warranty · purchase-bill ledger (`bills`+`bill_items`, maker-checker) · reception scan-first intake w/ shadow-flatten scanner · Sarvam OCR auto-fill · vendor directory · spend analytics · supplier backfill. `asset_register.py` **v1.11.0 `0cd8fc3b…`** (arc S173–S177: 1.3.0→1.4.2→1.7.0→1.8.1→1.10.3→1.11.0; entrypoint `asset_register:app`, `app.py` dead/removed). Runs on **system python3** (F-53). Smoke 342/0. | V |
| 13 | `/root/shared/` (shared VPS libs — not a service) | — | — | **Shared modules imported by multiple apps** via `SHARED_LIB_DIR` sys.path shim (A-D16): `sarvam_ocr.py` **`b1cc567b…`** (Sarvam Doc-AI digitise/extract, SDK-based, graceful-skip; consumers: asset app; scanner app queued). One file, no copies — repo home `shared/`. NEW S175, verified single-copy S177. | V |
