# KB REGISTER — v5.2 (Tier 0 · current state · rides the session loop) — S180 Marg feed + sale returns
...
| File | md5 | live as of |
|---|---|---|
| `/root/finance/finance_app.py` | `7b62b7ae661914505c864d71cc6c9abc` | **S180** — S179 base + the review-resolve fix: a queued sale RETURN is stored signed and must become a magnitude + `_return` service on the way to `sale_item`, which forbids negatives (was HTTP 500). Smoke **179/179** |
...
**Next free clinic decision: D317.** Asset-app decisions **A-D1–A-D24** are *sub-project scope* (next free A-D25) — indexed in `KB_Asset_Register` + Archive §S173–§S177, and never consume clinic D-numbers.
...
## FINDINGS — F-0 … F-89 (next free F-90).
...
**END OF KB REGISTER v5.2 (S180). Current-state + index only; all session history is in KB History Archive v1.28 (§S65–§S180). If this end-marker or the CURRENT LIVE FILE VERSIONS table is absent, this file is truncated and must not be used as canonical.**
