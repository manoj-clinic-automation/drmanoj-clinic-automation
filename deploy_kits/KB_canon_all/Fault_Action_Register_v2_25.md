# FAULT → ACTION REGISTER — v2.25 (CONSOLIDATED, SELF-CONTAINED)
...
*Next free finding: F-134.* *(This line read "F-115" through v2.20 and v2.21 while the index rows above it were current — the F-45/F-108 family; corrected at v2.22, not silently, and advanced with the index ever since.)*
...
**What this pair says about the session.** Five findings came from building the next thing on the
last. These two came from **the owner looking at his own system through his staff's eyes** — and
they found the one thing five layers of gates could not, because every gate was checking that the
code did what the record said, and the record itself was wrong.

---

**END OF FAULT → ACTION REGISTER v2.25 — §7, §7.1 and the CHANGELOG are the last sections. If any of the three is absent, this file is truncated and must not be used as canonical.**
