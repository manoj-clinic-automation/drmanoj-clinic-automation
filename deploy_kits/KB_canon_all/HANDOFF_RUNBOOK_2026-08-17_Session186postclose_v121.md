# HANDOFF RUNBOOK — v121 (Session 186 POST-CLOSE · 17 Aug 2026)
...
## §0A — WHAT HAPPENED AFTER THE CLOSE (Session 186 post-close)

**The close was published and immediately failed its own first check.** `PUSH.bat` cannot publish a
close-out — it stages only its own kit folder, and `KB_canon_all` is not a kit — so the "successful"
push carried a mid-session kit while the whole canon sat uncommitted (**F-115**; fixed by
`PUBLISH_CLOSE.bat`, whose own v1 then cried wolf on deliberate ignores, **F-121**).

**Then the first live-pin run went RED**, and it was right: `finance_workbench.html` was pinned to the
superseded `S186_R2a` build while the box carried the newer `S186_I1a` one. At the close, the
duplicate-path guard fired and the conflict was resolved **from the documents instead of from the box** —
the inverse of D321(d) (**F-118**). **This is the first RED in the project caused by the record rather
than the box, and the first proof the pin checker works in that direction.**

Verifying it opened three more: a **phantom hash in the manifest's own footer** that no file in the repo
carries (**F-116**); a pin list attesting to a **manifest md5 that exists nowhere**, printed by the
checker as "VERIFIED" without ever being compared (**F-117**); and a `MD5SUMS_ALL.txt` row for a file
that is not on disk, which made **Phase 0's only command exit non-zero** (**F-119**) — alongside three
rival checksum files for one folder (**F-120**).

All record-only. **The box was not touched and no live code changed.** Canon now: Register **v5.12** ·
Archive **v1.35** · Fault Register **v2.21** · this Runbook **v121** · manifest rebuilt from real bytes ·
`MD5SUMS_ALL.txt` the single checksum authority, **74/74, exit 0**. Retired files went to
`deploy_kits/_attic_S186/` — **moved, not deleted.**

> **The lesson worth keeping.** Six of these seven were found only because one RED was taken seriously
> instead of explained away. Every one of them was invisible to a check that passed. **A record that is
> internally consistent is not the same as a record that is true** — F-88 said that about kits; this
> session said it about the canon itself.
...
**⭐ 0. OWED TOOL FIX — `verify_live_pins.py` must verify the manifest it names (F-117).** It reads the
word `register_pin_verified: yes` and then **prints** `source : VERIFIED against the manifest … (md5 …)`
without ever hashing that manifest. A false attestation therefore survived an entire close and was
displayed to the owner as a pass. The fix is about three lines: hash the named manifest, compare, refuse
on mismatch. **Do this before trusting any GREEN.**
