# END OF SESSION — full session close-out (v9)
...
**v9 in one line (S206, owner directive):** the knowledge base outgrew its container. Project knowledge is capped at **2 MB and reached 78.9 %** at the S206 close, with the KB History Archive alone at **46.9 %** — a Tier-1 file Phase 0 never opens, carrying half the cap. The owner's answer was a fourth store: **`D:\Downloads\ClaudeCowork\`**, reachable by Cowork and the agent on manojz, mirrored one-way to Drive. **v9 adds step A12** — the folder is refreshed and its manifest rebuilt as a numbered step, because everything this routine has ever lost was lost by living in narrative instead of a numbered step. Same lesson as A8, A9 and A10, for the fourth time.
...
**A12. THE COWORK FOLDER AND ITS MIRROR — `D:\Downloads\ClaudeCowork\` (added at v9 — S206).**

**Four stores now hold this project's knowledge, and exactly one rule keeps them honest:**
...
| **GitHub** | code + `deploy_kits/KB_canon_all/` (the folder `verify_live_pins.py` reads) | patient data, secrets |
| **`D:\Downloads\ClaudeCowork\`** | everything canon excludes + dated frozen snapshots | **any live, editable copy of a canonical document** |
| **Google Drive** | a one-way mirror of that folder, so it is reachable from phone and browser | nothing originates there |

**NO DOCUMENT MAY BE LIVE AND EDITABLE IN TWO STORES** (D202 · F-201: *neither store is
authoritative by position; compare by md5, never by where a file sits*). Frozen snapshots are
exempt — a cold backup is a copy by definition, and `KB_canon_all/` is a sanctioned second copy the
pin checker requires. **Working papers are safe to hold there because D351 rule three already
freezes them at birth.**

**At every close:**

1. **File this session's working papers** into `03_WORKING_PAPERS\S###\`, and its kit into
   `02_SESSION_KITS\` if one was built. **Copy → verify → only then delete from project
   knowledge. Never the other way round.**
2. **Refresh `00_INDEX.md`** with what was added, and **rebuild `MANIFEST.md5`.**
...
4. **Push the Drive mirror**, or say in the close report that it was not pushed. `D:\` is
   unreachable from phone and browser; **the mirror is the only thing that makes this material
   portable**, and a mirror nobody refreshes is a copy that lies.

*Why this is a numbered step: the S206 close measured the cap at 78.9 % and found the Archive
occupying 46.9 % of it — a file Phase 0 is explicitly told not to read. Nothing in the routine had
ever asked how full the container was, so nothing had ever noticed. **A limit nobody measures is a
limit you meet by surprise.***
...
*v9 changes from v8 (S206): **A12** added — the `D:\Downloads\ClaudeCowork\` folder, its Drive mirror and the four-store ownership rule become a numbered step, and every close now reports how full project knowledge is. Written after the S206 close found the cap at 78.9 % with the Archive alone at 46.9 %, and after the S205 close was found to have skipped A1 and A2 entirely — a close is only as good as its numbered steps.*
