# KB REGISTER — v5.6 (Tier 0 · current state · rides the session loop) — S184 cash books corrected LIVE + the D322 holiday classifier; S185 folded three sessions of canonical debt
...
> **How to use.** *Now* → this file. *History* → **KB History Archive v1.33** (Tier 1; §S183 · §S184 · §S185 appended at the S185 close). *Live backlog* → **HANDOFF_RUNBOOK §2** (v119). *Findings register* → **Fault_Action_Register v2.19** (Tier 1; the four owed appends APPLIED at S185 — F-90…F-108 now in the register itself, §7 index extended from F-89 to F-108, full text §7.1). *Doc manifest + hashes* → **CANONICAL_MANIFEST.md**.
...
| `/root/deploy/verify_live_pins.py` + `gen_live_pins.py` + `live_pins.txt` | `ce36dbf10e7d5bbd5310507add41f3cb` (checker) · pins generated from THIS Register | **S183 NEW — the F-97 structural fix (D321).** Read-only live-code pin checker on the box; run `python3 /root/deploy/verify_live_pins.py` at every session open + close. `live_pins.txt` is generated from this Register's live-file table (`gen_live_pins.py`), never hand-kept. Kits S183_V1a→V1c. |
...
## FINDINGS — F-0 … F-108 (next free F-109).

> **S184 raised two, S185 two. Full text now in Fault_Action_Register v2.19 §7.1 (the four owed appends were applied at the S185 close) and in Archive §S184 / §S185.**
...
> *Recorded unminted:* the **`finance_app.py` full md5 was never written down at the S184 close** — only `c66bec2b76…` — and was NOT invented at the S185 fold-in; complete it from the box at the S186 open (D321). The **opening-float question** remains open and is a measurement, not a finding.
...
**END OF KB REGISTER v5.6 (S185). Current-state + index only; all session history is in KB History Archive v1.33 (§S65–§S185). If this end-marker or the CURRENT LIVE FILE VERSIONS table is absent, this file is truncated and must not be used as canonical.**
