# FAULT → ACTION REGISTER — v2.57 (CONSOLIDATED, SELF-CONTAINED) *(H1 corrected of record at the S224 close: v2.53 still read v2.52 in this line — the F-45 family again, in the file that mints it; recorded, not silent. The S223-open correction is preserved next: H1 corrected of record at the S223 open: it had read v2.48 through v2.49–v2.51 — the F-45 family again, in the file that mints it; recorded, not silent. The S219-open correction is preserved in this note rather than deleted.)*
...
**v2.56, Session 225 CLOSE, 06 September 2026 (04:00 IST) — F-322 and F-323 appended; F-321 CLOSED.** Two findings from the stock-check night, both in the manojz pusher (`push_snapshot.py`, S208 kit): the same-date rule that reads a legitimate item merge as a category filter, and a token read from the medical share with no timeout that hung the nightly's catch-up run and, under the task's default 72-hour limit, swallowed the next night's start. Both answered structurally the same night (the on-capture pushers ride the pull cycle); the code fixes are owed to the S208 kit's next revision together with F-319. Three catches by existing gates (F-185 twice, the publish's `.gitignore` check) and one F-233 breach are recorded in the Archive §S225 §4, not minted — the gates did their job. Append proven: everything from the v2.55 version paragraph to the v2.55 END marker is unchanged; the S225-close section sits above the END marker.

**v2.55, Session 225, 04 September 2026 — F-320 and F-321 appended** (F-321 added in-session at 15:50, the assistant's own).** One finding, the assistant's own, raised while folding the S224 pin readback: a declared pin that does not hold on the box, and a readback that recorded it live without comparing it to the pin it existed to test. Append proven: from the v2.53 version paragraph to the v2.54 END marker the text is byte-identical to v2.54; only the H1 line and this version note differ above it, and §S225 is appended below. Next free: **F-321 · D371**.

**v2.54, Session 224 CLOSE, 04 September 2026 (midday) — F-310 … F-319 appended.** Ten findings from the Marg-lane session: six the assistant's own, three the owner's finds on the first screens after an install, one Marg's own behaviour. F-314 is the one that had cost the most for the longest — every scheduled task on manojz created *start only on AC*, the pull asleep three times in a month. F-312 and F-313 were found by the owner within minutes of a screen that had passed 133 and 115 green checks. Append proven: from the v2.52 version paragraph to the v2.53 END marker the text is byte-identical to v2.53 (444,019 bytes in all); only the H1 line and these two version notes differ above it, and §S224 is appended below. Next free: **F-320 · D371**.**

**v2.53, Session 223 CLOSE, 04 September 2026 (dawn) — F-302 … F-309 appended** *(this paragraph was absent from v2.53 itself, whose H1 also still read v2.52; both corrected of record at the S224 close — the F-45 family).*

**v2.52, Session 223 OPEN (the S222 close debt), 03 September 2026 (night) — F-297 … F-301 appended.** Four findings from the PWA session, minted here as the deferred close owed (the S211 / S219-open precedent). **All five are the assistant’s own**, and four are the same fault in four coats: a RECORD was trusted about a file instead of the file being read. F-297 is the one that cost the most — the Register’s pin table has held `darpan_corrections.html` to a hash it has not had since S209, and `verify_live_pins.py` is generated from that table. F-299 is the sharpest — a stale repo copy quoted to the owner as a checked fact, and he corrected it. Next free: **F-301**. See the section at the foot of this file.
...
# MINTED AT THE S220 CLOSE — F-280 … F-285 · F-277 CLOSED

**Six findings from the intent session, authored from Archive §S220, never from memory. Three are
the assistant's own (F-280 in the repo, F-284 a false finding it made and caught, F-285 on the owner's disk). F-281 and
F-282 were found by the OWNER on the first live day through the S220 system — two screens finally
reading the same day disagreed, and the code confirmed both faults to the rupee. F-277, minted at
S219 and still live then, is CLOSED: kit `S220_F277_NAMECHECK`, live 21:30 IST 02-Sep, four pins
predicted and matched, live-shape walk 12/12 through the real `ingest_day` on the box.**

| code | finding | the rule it leaves behind |
|---|---|---|
| **F-277 → CLOSED (S220)** | `resolve_patient_checked()` attaches by ID only and RECORDS a name that disagrees with the master's as an `identity_dispute`; the audit says **"identity disputed"** — amber, a question, never a money verdict, never escalated. The evidence CSV's 43 verdicts reproduced exactly (28 agree · 4 differ · 11 unknown); the first draft of the brief mis-read one row (CN00180) as needing no question and was corrected visibly. | *(as minted at S219)* — and: **calibrate a tolerant match on the master's own spellings before shipping it**; a stricter test manufactures false disputes. |
| **F-280** | **A REPOSITORY COPY THAT IS NOT THE LIVE FILE READS AS THE LIVE FILE.** `finance\finance_ingest.py` in the repo (15-Aug, 29,575 b) is 101 lines from the pinned live `6cb83302…` (27-Aug, 34,569 b, in `deploy_kits\S204_VPS_LIVE\`). The assistant read the repo copy first and its line numbers were wrong; `resolve_patient` happened to be byte-identical in both, so the reading held — by luck, not by method. | **Read the PINNED bytes, never a file whose name matches.** The pin list says which file is live; a repo path is not provenance (D188). A copy that can be mistaken for live is retired or labelled. |
| **F-281** | **A TOTAL THAT FETCHES THE SIGN AND IGNORES IT.** `finance_app.py`'s "Marg — total" was `sum(amount_p)` over the day's sale rows; the query fetched `service` and never read it, so a sale return (stored POSITIVE, S180) was **added**: 16,644 + 1,030 = 17,674 on 02-Sep against a true 15,614 for those rows. `v_day_attribution` had subtracted returns correctly since S180; the helper never read the view. | **One source, one rule (D349): a figure the schema already computes is READ, never re-derived.** A column fetched and unused is a bug, not a style issue. |
| **F-282** | **MONEY WITH NO NAME ON IT VANISHED FROM BOTH SCREENS.** Seven bills the ingest parked for review (a name, no clinic ID, confidence 0.5 — ₹2,030 net, one a return) were sale rows on neither screen, so the portal and Darpan's card both left them out; the declared cash (17,644) was the only correct figure, and the portal's "variance 30 ✓" was two errors nearly cancelling. Both screens now read attributed + parked from the view; the card names the parked bills and expands them (D363: with the full mobile). | **Attribution never moves the books: a bill Marg sold is in the day's total whether or not its patient is known.** Two errors that cancel are two errors. |
| **F-283** | **A CONFIDENCE GATE PARKS THE BILLS D355 SAID TO LOOK UP.** `ingest.min_confidence` 0.70 parks every NAMED bill without a clinic ID (confidence 0.5) into `sale_item_review` — 7 of 27 on 02-Sep (26 %). D355 rules identity by lookup, never by a generated confidence; the D355 ladder (name + mobile against the master) is not run at ingest. Recorded, not fixed: the parked money now counts (F-282); the parking is Marg-backlog design. | **A parked bill is a lookup not yet run, not a verdict.** Run the ladder before parking; park only what the ladder cannot resolve. |
| **F-284** | **A CLAIM CONVICTED ON A SLICE OF THE FILE — the assistant's own, the F-208 shape again.** At the Phase-0 open the assistant grepped a 280-line extract of `CANONICAL_MANIFEST.md` (lines 120–400, pulled to save tokens), found no `Fault_Action_Register` row past v2.16, and reported to the owner that the manifest had carried no Fault-Register row through thirty-three versions. The row exists — `v2.49 · dfdbdd67…` at the lineage table, line 445 — and was found by the close's own edit pass. The only stale cell is the cold-kit inventory's `v2.16 · Tier 1 CURRENT` label (S171), which is a label, not a missing row. The false finding stood in the session's report for four hours and was corrected before it reached canon. | **A check runs on the WHOLE file or it says which part it read.** Token-lean reading is a method for finding, never for convicting (A0); a claim of absence needs `grep` over the file, not over an extract. |
| **F-285** | **A CHECK RUN ON THE OWNER'S DISK LEFT WHAT IT COULD NOT DELETE.** `py_compile` run inside a kit folder on `D:\dr-manoj-git` wrote `__pycache__`; the device shell's `rm` is a no-op there, so the leftover reached the publish gate, which refused correctly. Companion to F-233: the same shell's `git status` leaves `.git\index.lock` for the same reason — the origin of every `_stale_git_locks_S###` folder. `git --no-optional-locks` leaves none; compile checks run in the workspace. | **Nothing that writes runs on the owner's disk unless its residue is a deliverable.** Compile, test and hash in the workspace; move the result once. |

**Next free: F-286 · D367.** The S214, S215 and S216 candidate sets and F-244 remain RECORDED, NOT
MINTED — they await the owner's ruling, as before.

**END OF FAULT → ACTION REGISTER v2.50 — F-280 … F-285 are the last minted findings; F-277 is closed. The v2.49 end-marker above is retained in place as a truncation-proof. If this marker is absent, this file is truncated and must not be used as canonical.**

---
*Minted at the S220 close, 02-Sep-2026. F-281 and F-282 are the owner's findings on the first live
day; F-280, F-284 and F-285 are the assistant's own — F-284 being a false claim of its own, caught by the evidence rule at this close.*

---

# MINTED AT THE S221 CLOSE — F-286 … F-296

**Eleven findings from the ladder session, authored from Archive §S221, never from memory. NINE are
the assistant's own, and two of those are F-142 repeated in the project that recorded F-142. Two are
the OWNER'S — F-295, found when a man stood in front of him and the credentials did not work, and
F-296, found by his reading a permission he had not asked for. F-296 is the only one carried
forward unbuilt, by his instruction; it opens the next session.**

| code | finding | the rule it leaves behind |
|---|---|---|
| **F-286** | **A RUNG DEAD IN PRODUCTION AND GREEN IN THE WALK.** Rung 3 of the D355 ladder delegated to `match_bill()`, which reads its rows by column name. Ingest calls it on a plain `sqlite3` connection, so it raised `TypeError` on every bill and the fail-soft swallowed it whole. The walk passed because **the walk built its own connection with a Row factory** — it asserted a property of the test harness, not of the system. | **A walk that supplies its own scaffolding proves the scaffolding.** Exercise the real call path with the real caller's objects; where a helper's contract is invisible (a row factory, an encoding, a cwd), cross-check the two implementations against each other on real rows. |
| **F-287** | **A NEW COLUMN NEARLY CARRIED THE FULL MOBILE INTO A TABLE.** The adapter stores the whole CSV row as `sale_item_review.raw_text`. D363 added a `mobile` column to that CSV the night before, so the next parked bill would have written ten digits into a review table in a project whose standing rule is last-four only. | **When an upstream feed gains a field, every store that keeps the row WHOLE has gained it too.** A schema change upstream is a privacy review downstream — fingerprint in memory, strip from what is stored. |
| **F-288** | **A DEFAULT THAT REACHED BACK ELEVEN WEEKS.** The jaankari list's window read `returns.act_from`, a setting that does not exist; the fallback reached to 18-Jun and drew 22 rows in the workspace copy — **121 on the live box**, on a counter phone. | **A setting read that can miss must fail to the SMALL answer, never the large one.** And size every list against live volumes, not workspace ones. |
| **F-289** | **A HEADING THAT RENDERED AS THE DEVANAGARI LETTER "ग".** A wrong code point where an emoji was intended: invisible to `grep`, to the compile check, to the walk, to every text-level gate. Only the browser saw it. | **Nothing but a renderer can prove what a page shows.** The standing browser-render rule earns its cost on exactly this class of fault. |
| **F-290** | **THE ASSISTANT LABELLED THE MONEY BACKWARDS.** The owner was told cost was not computable and MRP was; the reverse is true. `stock_rate.rate_p` is the **last purchase rate** (PURCHASE_ITEMWISE); `sale_line_item.amount_p` is a **strip rate**, not an amount — MRP per unit is that rate ÷ pack. An audit finding was nearly valued on an inverted pair of prices. | **Before a column becomes money on a page, prove what it is on real rows** — a ratio, a spread, a sanity range. Here: median MRP ÷ cost **1.40 over 147 items** is what proved the direction, and it is what should have been run first. |
| **F-291** | **`\| tail -1` SWALLOWED THE WALK'S EXIT CODE — in the assistant's own install paste.** The chain read the walk's last line and restarted the service past a RED walk. **This is F-142, in the project that recorded F-142.** | **Never pipe a gate.** A gate's exit code is the gate; a pipeline's status is its last stage. Check `$?` before the `&&`, or the gate is decoration. |
| **F-292** | **A PAGE HARD-CODED `/stock/...` WHILE THE BLUEPRINT SITS AT `/finance/stock`.** The S209 fault, exactly, four sessions later — a whole console killed by a mount prefix written into a page. | **A page never spells its own mount.** Derive the base from `location.pathname`; the server owns the prefix. |
| **F-293** | **A RENDER TEST THAT REPORTED THE STUB'S OWN GAPS AS PAGE FAULTS.** The harness fed a partial fixture and then blamed the page for the fields the fixture lacked. F-142's family for the third time in one session. | **A test's own incompleteness must not be reportable as the subject's fault.** Run it as a DIFFERENTIAL against the unpatched live artefact: only what the change moved may be reported. |
| **F-294** | **A PIN PREDICTED IN LF, READ BACK IN CRLF.** Predicted `1110af86`, the box read `7722c33a` — same content, two line endings, and ten minutes lost proving it. | **Predict PC-side pins as CRLF and VPS-side as LF, and say which convention a prediction uses.** A pin is a promise about bytes, so it must name the byte convention. |
| **F-295** | **THE JOINER PAGE DISPLAYS CREDENTIALS IT NEVER CREATES.** *"AMIR LOGGED IN FRONT OF ME, I ENTERED HIS CREDENTIALS, GOT REPLY THEY'RE WRONG."* The page prints `amir / amir1234` and no code anywhere calls `clinic_users.py adduser`. The same page shows a WhatsApp message with no way to send it. **The owner found both, standing beside the new joiner.** | **A page that SHOWS a credential must have CREATED it, or must say it has not.** An onboarding step that ends in a screen rather than in a state is not a step; test a joiner flow by joining somebody. |
| **F-296** | **A ROLE THAT IS A DESK'S KEY GRANTS THAT DESK TO EVERYONE WHO EVER HOLDS IT — and the assistant wrote the over-grant into its own walk as a PASS.** The owner: *"Why have you added him to the medicine return वापसी desk?"* Amir needed `viewer` for corrections and stock count; `viewer` **is** the Vaapsi desk's key, so a purchase man gained cash refunds. The cause is older than Amir: S214's "named reception staff" was never named in code — the desk asks *are you a viewer?*, never *are you one of them?*. **Anyone ever granted viewer gets the returns desk.** Fix designed and approved (opt-in `returns.desk_users`, inert when unset so reception cannot break) and **deliberately not built at this close** — it is the first item of the next session, by the owner's instruction. | **A permission walk must list what a grant OPENS ELSEWHERE, not only what it was asked to open.** A role is not scope: the moment a second desk keys off the same role, the role has become a group, and a group needs a membership list. Enumerate every route that keys on a role before widening who holds it. |

**Next free: F-297 · D367.** The S214, S215 and S216 candidate sets remain RECORDED, NOT MINTED —
they await the owner's ruling. **F-244 is RETIRED by the owner's ruling at this session** (its
remedy was already executed at S211); the §S205 narrative is **STRUCK**, settled 27-Aug.

**END OF FAULT → ACTION REGISTER v2.51 — F-286 … F-296 are the last minted findings. The v2.50 end-marker above is retained in place as a truncation-proof. If this marker is absent, this file is truncated and must not be used as canonical.**

---
*Minted at the S221 close, 03-Sep-2026. Nine of eleven are the assistant's own; F-295 and F-296 are
the owner's, both found by watching the system meet a real person. F-296 is carried into the next
session as its first item, unbuilt and named.*

---

# MINTED AT THE S222 CLOSE — F-297 … F-301
### *(minted at the S223 open, 03-Sep-2026 night, as the S222 close debt — `S222_CLOSE_DEBT.md` row 2)*

**Five findings, authored from `S222_BUILD_BRIEF.md`, `S222_DARPAN_CORRECTIONS_PIN_CONFLICT.md`,
`S222_CLOSE_DEBT.md`, `live_pins_S221close.txt` and `live_pins_S222close.txt` — never from memory.
ALL FIVE ARE THE ASSISTANT’S OWN. Four of them are one fault wearing four coats: a RECORD was trusted
about a file instead of the file being read. F-299 is the sharpest, because the owner had to be the
one who corrected it. **F-301 was found by this fold, not by the session it belongs to**, and it is
the largest of the five: an entire wave of eleven pins missing from the Tier-0 Register while all
four of its self-referential lines said the wave was there.**

| code | finding | the rule it leaves behind |
|---|---|---|
| **F-297** | **THE REGISTER CONTRADICTED ITSELF ABOUT A LIVE FILE, AND THE PIN CHECKER ENFORCED THE WRONG HALF.** `darpan_corrections.html`: the Register’s pin table says `b3cfd86f…` — the S208 bytes, from before the transfer controls existed — while the Register’s own narrative says the page moved at S209 and again at S210. The box reads **`f2f6f60e…`**. `verify_live_pins.py` is generated FROM the pin table, so **it has been holding that file to a hash it has not had since S209**: fourteen sessions of a check that could only ever be wrong, whose RED nobody re-derived. The narrative was over-broad in the other direction — S210 never touched the file. **Only the box settled it.** | **When a canonical record disagrees with itself, neither half is evidence — go to the artefact.** And a generated checker inherits the errors of whatever generates it: a RED from the pin checker is a claim about a TABLE until that row has been re-derived from the box. |
| **F-298** | **TWO KITS CHAINED ON ONE FILE WERE HANDED TO THE OWNER IN THE WRONG ORDER.** `S222_TILE_GRANTS` was built against `portal.py` `23824f92…` — the state *after* `S222_SCANAPP_INAPP`’s tile patch. The owner was told to install the grants kit first, and at 17:05 its from-pin gate **refused, correctly, on the live login system**. Nothing was harmed and nothing was half-applied. The gate turned a planning error into a refusal — and the planning error was the assistant’s. | **A kit chained on another kit’s output names its predecessor in its own INSTALL, and the order is stated in one line beside both.** A from-pin gate is the last defence, never the plan: anything it has to catch was mis-sequenced upstream. |
| **F-299** | **A STALE COPY WAS QUOTED TO THE OWNER AS A CHECKED FACT.** The assistant told him there was no PWA. There is one: `/portal/manifest.webmanifest`, two icons cut from the real clinic logo, `display: standalone`, and a deliberate **no service worker** ruling kept from ATT2. It had grepped `launcher/portal.py` (`d74aa3f9…`) — a repo copy — instead of the live file. **This is F-280 one step further down the pipeline:** F-280 was a stale copy READ; this is a stale copy PUBLISHED as a conclusion, to the owner, who then had to correct his own assistant about his own system. | **A file’s provenance is checked before it is QUOTED, not only before it is edited.** Reading is not the risky act — asserting is. Any statement of fact about a live file names the pin it was read from, or it is a hypothesis and must be said as one. |
| **F-300** | **A BLANKET `.gitignore` RULE NEARLY SHIPPED A KIT WITH ITS PAYLOAD MISSING.** `tile_grants.json` is the entire substance of `S222_TILE_GRANTS`, and `.gitignore`’s blanket `*.json` excluded it. The publish gate refused rather than dropping it in silence — but unrefused, that kit would have fetched, passed `md5sum -c` green on the files it *did* carry, installed nothing that mattered, and looked exactly like a success. | **A kit’s own payload is never subject to a repo-wide ignore rule.** And a gate verifies the kit’s file LIST against its manifest, not only the bytes of the files that arrived: a kit missing a file is not a kit with fewer files. |
| **F-301** | **A CLOSE UPDATED THE FOUR SELF-REFERENTIAL LINES AND NEVER WROTE THE WAVE THEY DESCRIBED.** `KB_Register_v5_69_S221` carries **no S221 wave block and not one S221 pin** — not the ladder, not Amir’s access, not the stock finding, none of the eleven — while its H1, its narrative blockquote, its lineage row and its END marker all say the wave is there (*“the S221 wave rows above”*). Every mechanical check on that file passes, because the four lines the routine checks are exactly the four lines that were written. Found at the S223 open by grepping the whole file for each S221 hash: **zero of eleven present, against two-of-two present for every S219 and S220 pin tested.** It is load-bearing because `live_pins.txt` is *generated from this Register* — its own header reads *“DO NOT HAND-EDIT the VPS rows. Change the Register, re-run the generator.”* Regenerating it from v5.69, as the close routine (A8) instructs, **would have erased the S221 truth and held eleven live files to pre-S221 hashes.** The surviving record is `live_pins_S221close.txt`, manifest-pinned and gate-green; every row was carried into v5.70 from it, two as full md5s, eight as the 8-char prefixes the S221 close held (still PENDING, still not passes) and one BLIND. Nothing live is affected — every one of those files is running the right bytes. **The generated pin list also cannot have come from the Register it names**: its header asserts `source: KB_Register_v5_69_S221.md` and `register_pin_verified: yes`, and the S221 rows it carries are not in that file. | **The four self-referential lines are a checklist, not a proof — a close is verified against the CONTENT it claims to have added, not against the pointers that describe it.** A version bump asserts a wave exists; the close must show the rows. And any generated artefact states its source: **regenerating it must be provably lossless against the previous generation, or the generation is a deletion.** |


**Next free: F-302 · D367.** The S214, S215 and S216 candidate sets remain RECORDED, NOT MINTED —
they await the owner’s ruling, as before. **The `darpan_app.py` divergent-copies candidate has
awaited his ruling since the S210 close and is now load-bearing** — three divergent full copies and
fourteen patchers, none of which reproduces the live `c98f0c24…`, which is why the S222
corrections-page fix had to be a page-level hide rather than a source fix. It is restated in
`OWNER_TODO_LIVE` ⭐0-6.

**END OF FAULT → ACTION REGISTER v2.52 — F-297 … F-301 are the last minted findings. The v2.51 end-marker above is retained in place as a truncation-proof. If this marker is absent, this file is truncated and must not be used as canonical.**

---
*Minted at the S223 open, 03-Sep-2026 (night), as the S222 close debt. All five are the assistant’s
own. F-297 is also the reason the debt mattered: the canon trailing the box by one session is the
exact condition that produced it — and F-301, found while paying that debt, is the same condition one
session earlier and eleven times over.*


---

# S223 — 03/04-Sep-2026 · F-302 … F-309

*Eight findings. **Four are the assistant's own**, and one of those took the owner's money system
down. Two are findings about the clinic's own records, minted because the rule each leaves is worth
more than the incident.*

| code | finding | the rule it leaves behind |
|---|---|---|
| **F-302** | **A PASTE PUT THE PAYLOAD IN PLACE BEFORE ITS OWN GATED INSTALLER RAN.** The `S223_LAUNCH_TILES` paste `\cp`'d `tile_grants.json` into its destination and *then* ran the gated installer that was supposed to place it. The installer found the file already correct, reported **ALREADY DONE**, and therefore **never took its backup**. The gate did not fail — it was answered before it was asked. | **An idempotence check placed after the copy proves nothing.** A gated installer must be the ONLY thing that writes its payload; if a paste also writes it, the gate degrades from a safeguard to a comment. Order the steps so the gate runs against the state it was designed to judge. |
| **F-303** | **A MODULE OPENED THE DATABASE AT IMPORT TIME AND TOOK THE WHOLE FINANCE APP DOWN.** `clinic_register.init()` called `db_getter()` while mounting. `finance_app.db()` uses `flask.g`, so at import there is no application context: `RuntimeError: Working outside of application context` → gunicorn *Worker failed to boot* → **the entire finance app 503 at 04:35**, on the owner's money system, in the dark. Two aggravations, both the assistant's: **`stock_app` has done this correctly since S213** — its `init()` was read for the mount signature and not for how it creates its tables; and **the render test passed** because it handed the module a plain sqlite connector that works anywhere — **F-286 exactly: a test that supplies its own scaffolding proves the scaffolding.** | **Mounting must touch nothing.** `init()` registers a blueprint and stores references; schema creation belongs on first request. And **when copying a pattern from a working module, read what it does, not only what it looks like.** A test's fixture must be *harder* than production, never easier: the register test now uses a getter that **raises** outside an app context. |
| **F-304** | **AN INDEX-SLICE REPLACEMENT RAN BACKWARDS AND LEFT TWO COPIES OF A FUNCTION.** Replacing `_section` by slicing between two `index()` marks, without checking the marks were in order, wrote the new function **above the shebang** while the original survived below it. Every request raised `TypeError`. | **Any slice-based edit asserts its own bounds before it writes:** `assert a < b`, and a check that the slice has not swallowed a neighbouring definition. An edit that can silently produce a *valid-looking* file is more dangerous than one that crashes. |
| **F-305** | **FOUR TEST FAILURES IN ONE SESSION WERE THE TEST'S FAULT, NOT THE CODE'S — F-293 four times over.** (1) The test hunted the word `"short"`, which appeared inside the page's own promise never to say anyone was short. (2) It looked for a phrase the HTML had wrapped across a line break. (3) Its arithmetic asserted 3×500 + 1×200 = 1,900; it is 1,700. (4) Its fixture modelled a **correct** day when the assertion needed a **mis-marked** one, so the difference it checked was always zero. | **A red test is a claim about the code that must itself be verified.** Match on whitespace-normalised text and on phrases specific enough not to collide with prose; compute fixture arithmetic in the test rather than typing the answer; and **a fixture must contain the condition the assertion is about.** |
| **F-306** | **A MANUAL COLUMN IS ALWAYS ABANDONED — the clinic's own X-ray register proves it twice.** `X-RAY DETAIL RAGISTER` carries film-size columns `8*10` and `11*14`, **filled for roughly 515 rows and then dropped**, and an `UPLOADTO DOCTERZ` column marked OK **549 times and then dropped** — while the register itself kept running to 6,177 entries. Nobody decided to stop; the columns that cost an extra thought simply lost. | **Do not design a field a human must remember to fill.** Either the system already knows it (pre-list it) or it is one tap in the flow they are already in. **The abandoned column is not a discipline failure — it is a design failure, and it is visible in the data years before anyone asks.** |
| **F-307** | **FREE TEXT AT A COUNTER PRODUCED 1,197 SPELLINGS OF ABOUT TWENTY THINGS.** 6,177 X-ray entries, 1,197 distinct typed forms, 882 of them occurring exactly once. LAT is typed **LET 664 times**; SPINE as SPAINE 162; ELBOW as ELWOB; CLAVICLE as CLAVICAL 83; FOREARM as FORAM. The vocabulary is small and stable — the typing is not. | **Anything that will be counted later must be chosen, not typed.** And when replacing free text with a picklist, **carry the misspellings in as aliases**: the history is only readable through them, and 6,177 rows of it are worth having. |

| **F-308** | **THE MANIFEST CARRIED NINE STALE `CURRENT` CLAIMS ACROSS FIVE CANON FAMILIES, AND THE PIN GENERATOR READ ONE OF THEM.** Running `gen_live_pins.py` at this close — the command step A8 prescribes — **refused**: *"the register you gave me is not the one the manifest pins… manifest pins 4d6e404a (v5.68, CURRENT)."* v5.68 is three closes old. Nine rows in all still said **CURRENT**: `KB_Register` v5.68 and v5.60 · `KB_History_Archive` v1.64 and v1.57 · `Fault_Action_Register` v2.45 · `HANDOFF_RUNBOOK` v151 · `START_HERE_SESSION` 216, 205 and 188. **Several contradict their own first cell** — `| KB_Register_v5_60_S210 | S210 close (superseded at S212) | … | **CURRENT** |`. The manifest's own history records this family biting before: *"one of the five duplicate-CURRENT rows"*. | **A document family may have exactly ONE row claiming CURRENT, and the close must MEASURE that, not assert it.** A stale label is not cosmetic: the generator picks a row, and it picked a Register three closes old — so the refusal was the only thing standing between this project and a pin list holding the box to a record that no longer exists (F-110's exact shape). **The tool was right and the canon was wrong**, which is the reverse of the usual assumption and the reason the refusal must never be overridden with `--allow-unpinned-register` to "get the close finished". A duplicate-CURRENT sweep is now part of A7. |

| **F-309** | **THE PHONE GATE GIVES THREE DIFFERENT VERDICTS ON THE SAME BYTES, AND THE ALARMING ONE IS THE WRONG ONE.** `NO_PHONE_NUMBERS.py` on `KB_History_Archive_v1_69_S223.md`: `--files-from` (**what the publish uses**) → **clean**; a single file path → **clean**; a folder walk → **"12 FOUND. Do not publish."** The twelve are carried-forward history, byte-identical in v1.68 and earlier. **The carried-forward baseline — the owner's 28-Aug ruling that history stays and only a number NEVER before in canon refuses — is implemented ONLY in the staged-files branch.** The walk has none of it, and the file's own header invites that walk: *"Pass a path to walk a folder instead, for a deliberate audit."* Found while auditing the S223 cold kit, which ships the gate inside itself so a restored copy still enforces F-185. | **A gate that is invited to run two ways must reach the same verdict both ways.** The audit mode is the one a frightened operator uses at 3 a.m. on a restored kit, and it is the one that says *do not publish* about twelve numbers the owner ruled may stay. Until the walk is made history-aware, **the cold kit's `00_READ_FIRST.md` carries the warning in full.** And the wider rule: **before believing a tool's alarm, establish which invocation produced it** — the first diagnosis at this close (that the kit's folder layout stripped the history signal) was wrong, and was withdrawn after the controlled comparison. |

**Next free: F-310 · D368.**

The S214, S215 and S216 candidate sets remain RECORDED, NOT MINTED. **The `darpan_app.py`
divergent-copies candidate has now awaited the owner's ruling since the S210 close**, and remains
load-bearing for the same reason as at S222. The four Docterz candidates in §7 of the working paper
also await a ruling. All are restated in `OWNER_TODO_LIVE`.

**END OF FAULT → ACTION REGISTER v2.53 — F-302 … F-309 are the last minted findings. The v2.52 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**

---
*Minted at the S223 close, 04-Sep-2026. F-303 is the one that matters: it is the first time this
project's own work took a live money system down, it happened at 04:35 with the owner awake, and it
was caught by him and not by any gate. The rule it leaves is now enforced in the module and in its
test.*


---

# S224 — 04-Sep-2026 · F-310 … F-319

*Ten findings. **Six are the assistant's own**, three are the owner's finds on the first screens after
an install, and one is Marg's own behaviour — minted because the supersede rule it leaves is worth
more than the incident. Sources: the S224 working papers in `03_WORKING_PAPERS\S224\` and the
owner's box readbacks recorded in `S224_MARG_CLUBBED_BUILD_LIST`.*

| code | finding | the rule it leaves behind |
|---|---|---|
| **F-310** | **A PIN WAS WRITTEN BACKWARDS INTO THE MANIFEST AT S213 AND STOOD FOR ELEVEN CLOSES.** Found at Phase 0 of S224: the `MARG_WALL_CARD` row had been re-pinned at S213 to `dd018563…` on the belief that the row had kept the S203 hash through the S205 update — the S213 conclusion was backwards. `dd018563…` is the **pre-S205** page: `D:\Downloads\margsync\MARG_WALL_CARD.html.bak_S205`, the backup taken *before* the S205 edit, hashes to it exactly. The true S205 page is `3a11a49a57ce0e6f1dfb597727b61efd` (7,545 bytes, 26-Aug-2026 23:47), whose own sub-line reads *"true as at 27 Aug 2026 (S205)"* and which carries the section the pinned bytes do not — *"If you ever run SEND_TO_CLINIC.bat by hand — read this first"* (AF-1 / F-206). Both kit copies (`MARG_MEDICAL/`, `S203_MARG_CANON/`) were refreshed from the margsync original and both `SUMS.md5` gates re-run from inside their folders (5/5, 67/67). **The page on the wall beside the medical PC needs reprinting.** The row was corrected in the manifest of record at S224, visibly, with the fault named beside it. *(assistant's own, S213)* | **A before/after pair is a claim about two files; each half is hashed, never transcribed from the other.** And a correction to canon is made in the file, named, and never silent — the F-45 family rule, applied to the manifest itself. |
| **F-311** | **THE FINANCE SERVICE'S PORT WAS QUOTED AS 8099 FROM A STALE REPO MAP; THE BOX LISTENS ON 8106.** The session's discovery map of the finance app was built from the repository copy, not from the live file or the running box. The first `healthz` check after the rev 1 install hit 8099, got a 404, and reported it as the app's. F-299 wearing a new coat — a repo-derived fact quoted as live. *(assistant's own)* | **A port, a path or a mount is read from the box that serves it, never from a copy.** A map built from a stale repo copy is F-299 whatever it is called; the live file is the only source for a live fact. |
| **F-312** | **GROSS WAS SUMMED WHERE NET WAS THE FIGURE.** The purchase hub summed `purchase_line.amount_p` — Marg's gross line value before discount — and set it beside the bill-wise report, which is net. Every month "differed": Sep ₹72,438 vs ₹80,352, Aug ₹3,54,879 vs ₹2,97,644, Jul ₹4,76,393 vs ₹5,08,062. **The owner found it on the first screen**, not the 133-check selftest, which had asserted the gross sum it was given. With `net_amount_p` July agrees on 101 of 103 bills and September on all 11. *(the owner's find)* | **Net is the figure; gross is the noise.** When a report of Marg's is placed beside a sum of ours, the selftest must assert the *report's* total, not our own arithmetic — a test that checks a sum against the same sum proves nothing (F-286's shape). Gross appears only in a column labelled *gross*. |
| **F-313** | **PAGE LINKS WENT OUT WITHOUT THEIR MOUNT PREFIX BECAUSE `bp.url_prefix` IS None WHEN THE PREFIX IS GIVEN AT REGISTER TIME.** Every link and the page's JS base were built from `bp.url_prefix`; Flask leaves that None when the prefix is passed to `register_blueprint`, so `/page/month/2026-08` was emitted bare and the portal answered Not Found. **The owner found it on the second screen.** F-292 family: a page that forgot its own mount. *(the owner's find)* | **A blueprint carries its own mount as a module-level value set in `init()`**, and the render test refuses any own href or `const P` that lacks the prefix (130/130 after the fix). A page is proven at the URL the owner will open, through the real gate, never only at the blueprint's root. |
| **F-314** | **EVERY SCHEDULED TASK ON MANOJZ HAD BEEN CREATED "START ONLY ON AC POWER", AND THE PULL SLEPT WHENEVER THE LAPTOP RAN ON BATTERY.** `schtasks /Create` sets the battery conditions by default and none of the session's lines cleared them. The 10-minute pull slept 06:40 → 08:37 on 04-Sep — the third time this month — and **it explains S219's 46-minute sleep**, which was recorded then as unexplained. Found in the field by the A3 watchdog work, which was itself about to be registered the same way. The owner cleared the flags on all three tasks with one PowerShell line. *(assistant's own, from S208 on)* | **A task created by `schtasks` starts only on AC until told otherwise.** Every task registration line carries the battery flags cleared, and the watchdog measures the pull *against the PC being awake*, so a sleeping laptop is never blamed on the pull and a sleeping pull is never excused by the laptop. |
| **F-315** | **THE S221 SPOT COUNT STORED A QUANTITY WITHOUT THE BILL IT WAS COUNTED AGAINST.** The Vaapsi desk's **गिनती करनी है** list was built at S221 as a *question* to staff, not as a *count*, so it posted `{kind:'spot', value}` and the server stored it — skipping the rule the counting page has obeyed since S207 and `stock_app /api/count` enforces (no count without the last sale bill number). A quantity with no anchor is unusable the moment a sale follows it. **The owner found it, in capitals:** *"THE STOCK CHECK SECTION DOES NOT HAVE ANY LAST SALE BILL NUMBER ENTRY BOX."* Parked at S223 dawn as "has no anchor"; un-parked and fixed at S224 on page and server (400 `anchor_required`). *(assistant's own, S221; the owner's find)* | **A count is a count wherever it is taken.** Any surface that records a stock quantity carries the last-sale-bill anchor as a compulsory field, refused on the page and again on the server, stored in its own column (D367). A question and a count are different objects; a list that collects quantities is a count. |
| **F-316** | **`_may_decide` READ THE BROKER ROLE AND DENIED THE OWNER HIS OWN DECISION BUTTONS.** `_may_decide(u)` looked at `u["role"]` alone. Under the live `finance_app.require` the login is `dict(u, roles=sorted(have))`: `role` is the broker's clinic-wide role (`doctor` for the owner) and the unit roles sit in `roles`. So `/api/finding` had been telling the live owner `may_decide=false` since S221 — the finding page offered him the staff's reason buttons and never *write off / recover / no loss*. The S221 walks never saw it because their fake login was `{"role": "checker"}`; `stock_finding.html` is a BLIND pin never read back live, so nobody saw it either. Found on the way to the diffs-page fix. *(assistant's own)* | **A test login has the LIVE shape or it is not a test of the live gate.** Role checks look in `roles` and `role` both (`_has_role`); walk fixtures are built as the broker builds them. And a BLIND pin is a page nobody has looked at — the render test now drives the pages' own JS with live-shaped logins for every role. |
| **F-317** | **MARG RE-KEYED AN AUGUST BILL BETWEEN EXPORTS AND A CONTAINED-PERIOD EXPORT KEPT THE GHOST ALIVE: +₹10,641.** Between 02-Sep and 04-Sep bill 02 of 18-Aug was moved in Marg to its corrected supplier. The old key lived on through the 30-Aug "01–29 Aug" bill-wise export — a different period from the full-month export, so the same-period supersede rule never retired it — and the hub would have shown an 85th bill and ₹3,65,520 where Marg's own total is ₹3,54,879. That bill's item lines, stored under the old key, then showed as an orphan set. *(Marg's behaviour; found by the assistant on the live data before the owner saw the page)* | **An export supersedes every live export of its type whose period it contains** and whose stamp is older — not only the one with the same period. A line set keyed to a supplier that names no live bill of the month is a *stray*, matched by (bill no, date) when unambiguous, never listed as an orphan; undated lines and orphan sets are shown and never block. Marg edits its past; the later, wider export is the truth. |
| **F-318** | **TWO KIT TEXT FILES WERE EDITED AFTER THEIR `SUMS.md5` HAD BEEN SUMMED, AND THE GATE REFUSED THE INSTALL.** The kit's own `md5sum -c` on the box reported the two rows FAILED and stopped — correctly. F-142 family, in the project that recorded F-142, on the day its five-kit installer was being handed over. The kit was re-summed from inside its folder, re-published, and went green. *(assistant's own)* | **Edit, then sum. Never sum, then edit.** `SUMS.md5` is generated LAST, from inside the folder, after the final byte of every file in it, and verified there before the kit is named to the owner. A gate that refuses its own author is doing its job; the answer is never to loosen it. |
| **F-319** | **`push_snapshot.py`'s "CATEGORY FILTER (F-235)" SKIP MESSAGE IS WRONG FOR EQUAL-SIZED DUPLICATES.** The three 02-Sep closing-stock exports carry the identical 377-item set (`same_items_as_ref=true`, measured at S224 A4); the nightly's skip line called the duplicates *"377 rows vs 377 — fewer items: a category filter (F-235)"*. The pick was right; the wording is not. Observed at S221, recorded open at S219's item, now with evidence; the message lives in the `S207_STOCK_VPS` kit on the VPS side and was **not touched** by the manojz lane. *(assistant's own; OPEN)* | **A skip message names the rule that actually fired.** Equal-sized identical sets are *duplicates*; a strict subset ≤ 60 % of the universe is a *subset*; only a named category is a *filter*. The router now records the distinction in `index.csv` `notes`; the VPS-side wording is owed and named in Runbook v156 §2. |

**Next free: F-320 · D371.** *(D368 · D369 · D370 minted at S224 from the owner's rulings of
04-Sep — supplier-wise is the month's figure and Marg's returns are normal; staff pages may be
all-English; the staff purchase-order flow is the S225 build — full text Archive §S224 §11.)*

The S214, S215 and S216 candidate sets remain RECORDED, NOT MINTED. **The `darpan_app.py`
divergent-copies candidate has awaited the owner's ruling since the S210 close**, and remains
load-bearing. The four Docterz candidates in §7 of the S223 working paper also await a ruling. All
are restated in `OWNER_TODO_LIVE`.

**END OF FAULT → ACTION REGISTER v2.54 — F-310 … F-319 are the last minted findings. The v2.53 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**

---
*Minted at the S224 close, 04-Sep-2026. F-314 is the one that had been costing the most for the
longest: a default flag on a scheduler, set at S208 and never questioned, that put the Marg pull to
sleep three times in one month and was blamed on everything except the laptop's power cord. F-312
and F-313 were both found by the owner within minutes of opening a screen that had passed 133 and
115 green checks — the first screen the owner sees is the walk nobody can script.*

---

# S225 — 04-Sep-2026 · F-320 (raised, explained and CLOSED in one sitting) · F-321

*One finding, the assistant's own, raised at the S225 open while folding `live_pins_S224close_READBACK`
into the Register's pin tables. Source: the readback paper itself, the kit `deploy_kits\S223_UPI_MATCH\`,
and a sweep of all five stores.*

| code | finding | the rule it leaves behind |
|---|---|---|
| **F-320** | **THE S223 `clinic_upi_check.py` PIN DOES NOT HOLD, AND THE READBACK THAT SHOULD HAVE CAUGHT IT RECORDED IT AS LIVE.** The box was read on 04-Sep at 13:05 as `0a2ec5ab0b80f8947a9d55f8901ba452`. The kit `deploy_kits\S223_UPI_MATCH\clinic_upi_check.py` hashes `33d9a7bd4d83c4a62388e485523e9640`, and that value is what v5.71, v5.72, `live_pins_S223close` and `live_pins_S224close` all carry. `0a2ec5ab…` occurs in exactly ONE place across the five stores — the S224 readback paper. It is not a paste-order slip: the other eight files in the same `md5sum` line all landed on their recorded values or prefixes. It is not a line-ending artefact: CRLF, trailing-newline and strip variants of the kit file were computed and none reach `0a2ec5ab…`. No later kit patches the file — `deploy_kits\` was swept for every reference. **EXPLAINED at the S225 open, from the owner's one line and git.** The box file is 11,005 bytes, mtime 04-Sep-2026 04:11:25 IST. Commit `a8849c4` (04:11 IST) published the kit with `clinic_upi_check.py` = `0a2ec5ab…`, 11,005 bytes — **that is the file the owner installed, and the mtime matches to the minute.** Commit `de72fb6`, five minutes later (04:16 IST), republished the kit with a revised `clinic_upi_check.py` = `33d9a7bd…`, 11,909 bytes — **rev 2, never reinstalled.** The Register then pinned rev 2, the kit as committed, not rev 1, the kit as installed. The bytes were never lost; they sit in git history at `a8849c4`. **The delta matters:** rev 2 adds the THIRD state — *bank feed has NO clinic transaction for this date while our lines show online money = a MISSING STATEMENT* (five such days measured, June and early July) — so since S223 the 09:40 batch has been filing those days under the two-state wording (*"our lines ABOVE the bank feed — Razorpay or not-yet-ingested"*). Wrong explanation on ~5 exception rows; no money moved and none is missing. **The second half of the finding is the worse half:** the S224 readback listed this row's verdict as *"READ — record as live"* — it recorded the number it had just been given rather than comparing it to the pin the row existed to test — so the divergence crossed a full close unseen. The same family as F-45 and F-301: a record that asserts where it was supposed to compare. Behaviour is not implicated; the file is a 09:40 cron batch that writes only `recon_exception` rows with `kind='docterz_vs_upi'`. *(assistant's own; **CLOSED the same sitting, 13:28 IST** — the owner re-ran the kit's STEP 1 copy against the current clone and pasted `33d9a7bd…`; the `--write` pass compared 60 days, 16 agreeing to the rupee, 44 exceptions filed or refreshed, 0 closed — F-275 made the pass idempotent.)* | **A DECLARED-PENDING row is closed by a COMPARISON, never by a reading — and a kit republished after its install is a kit NOT installed.** When a kit is re-published within a session, the Register pins the value the owner's paste actually landed, or the row is DECLARED-PENDING against the *new* kit and says so. A readback value is folded only after it is set against the pin it was captured to test, and a row where the two disagree is raised as a finding in the same breath — it is never written into the pin table as the new truth. The pin tables now carry the box's value with the divergence on its face, so every run of the checker restates it until F-320 is explained or the kit is reinstalled. |

| **F-321** | **AN INSTALL LINE THAT COPIES THE NEW FILE INTO THE LIVE FOLDER *BEFORE* ITS WALK LEAVES THAT FILE FOR THE NEXT RESTART TO LOAD.** The `S225_PHONEBOOK` line (15:30) copied `purchase_app.py` rev 7a into `/root/finance/`, then ran the live-shape walk, which failed (rev 7a read a column it had not yet created) and stopped the line before its own restart — correctly. But the file was already on disk, and the NEXT restart (`S225_DAY_MPR_LINE`, 15:38, an unrelated kit) loaded it: from 15:38 the running purchase app was rev 7a, read back `0b77cc53…` at 15:47, and the staff order page was faulty until rev 7b went in. The walk's design assumption — *a failed walk leaves nothing live changed* — held for the process and not for the disk. *(assistant's own; **CLOSED 16:20 IST 04-Sep — rev 7b `e4e5a75b…` read back, then revs 8–12 each walked on a probe copy**; no money, no data touched)* | **A walk runs on a COPY in a probe folder, never on the live folder.** Every install line from S225 on: copy the live modules and the new file to `/tmp/<kit>_probe/`, walk there (`FIN_DIR=/tmp/<kit>_probe`), and copy into the live folder ONLY after the walk is green. A file in the live folder is live at the next restart, whoever restarts. |


# S225 CLOSE — 06-Sep-2026 · F-322 · F-323 (the stock-check night; both in the manojz pusher, both answered structurally the same night)

| # | Finding | Action |
|---|---|---|
| **F-322** | **`push_snapshot.py` READS ANY SAME-DATE EXPORT WITH FEWER ROWS AS A CATEGORY FILTER — SO A LEGITIMATE ITEM MERGE IS SKIPPED AND THE OLDER TWIN RE-SENT.** 06-Sep 03:04: the owner merged two Marg items (`LACTOVAX SYP 200ML` / `LACTOVAX SYP 200ML.`) and re-exported the closing stock as on 05-09 — 438 rows, one fewer. `newest_full()` kept the 439-row pre-merge twin ("373 rows vs 374 — same as-on date, fewer items: a category filter, not the shop (F-235)") and sent it again; the on-capture pusher marked the new md5 as done. No harm this time — the pusher merges the dot-duplicate by name, so both files read as the same 373 items — but the rule is wrong: a filter is a *subset by category*, and the router's own test since S224 A4 is a strict subset ≤ 60 % of the 45-day universe. *(assistant's own — the rule predates S224 and was never re-aligned; **OPEN**; no money, no data touched)* | **One rule for "is this the whole shop": the router's `stock_variant()` test, imported or copied verbatim into `push_snapshot.py`.** A same-date export that is not a SUBSET by that test is a re-export, and **the later-taken one wins** (the S221 tiebreaker already says so for equal sets). Ship with F-319's wording fix and F-323's timeout in one S208-kit revision, walked against the archive's real twins (02-Sep ×3, 05-Sep ×3). |
| **F-323** | **THE NIGHTLY'S CATCH-UP RUN HUNG ON A TOKEN READ WITH NO TIMEOUT, AND THE HUNG INSTANCE SWALLOWED THE NEXT NIGHT.** 05-Sep 08:46 (Task Scheduler's *run when available* for the 04-Sep 22:30 miss): `push_stock_log.txt` shows the NIGHTLY header and nothing more; `push_snapshot_lastrun.txt` is 0 bytes. `read_token()` opens `\\<medical>\DDrive\SendToClinic\token.txt` **first** and falls back to the cache only after — a UNC `open()` against a host that is off has no timeout in Python, and at 08:46 the medical PC was not yet on. The task's default 72-hour execution limit left the instance "running", so 05-Sep 22:30 never started (manojz was asleep 21:38–01:04 as well — watchdog log); the drift page reported *"our computed figure: never received"*, honestly. Found 06-Sep 01:35 from the logs alone. *(assistant's own — the read order was chosen at S207 when the medical PC was assumed always-on; **OPEN for the code, ANSWERED for the flow**)* | **Answered the same night, structurally:** `expected_on_capture.py` (12/12) and the S225 snapshot pusher now run **inside every 10-minute pull cycle**, a minute after the export arrives — when the medical PC is certainly on — so nothing waits for 22:30 or for a PC to be awake at a fixed hour; the 15-minute tasks are backstops with a 10-minute limit. **Owed in the S208 kit:** cache-first token read, or a bounded read (a thread with a deadline) of the share; and the nightly re-registered with a 30-minute limit (the script's `Set-ScheduledTask` was refused — the task belongs to another account; the owner's one PowerShell line, when convenient). |

**Next free: F-324 · D377** *(D371–D376 minted at the S225 close; Archive §S225 §2 carries the full text).*

**END OF FAULT → ACTION REGISTER v2.56 — F-323 is the last minted finding. The v2.55 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**

---
*Minted at the S225 open, 04-Sep-2026. F-320 is the first finding this project has raised against its own readback discipline rather than against a build: the capture was done correctly and by the right hand, and the fold is where it failed. Explained the same hour from one owner line — a five-minute republish at 04:16 that nobody reinstalled.*


# S226 CLOSE — 06-Sep-2026 · F-324 … F-333 (the first live stock count — eight the assistant's own, one live since S213 and found by a counter's tap, one a stub found by use)

*Every finding below was surfaced by the people using the page — Darpan on the shelf, Amir at the keyboard, the owner reading over his shoulder — within minutes of a kit going live. The rule this session leaves behind is the one it kept proving: a walk proves the kit, the counter proves the join. Archive §S226 §1 carries the hour of each.*

| # | Finding | Action |
|---|---|---|
| **F-324** | **THE READINESS HEADER SUMMED EVERY PUSH OF A DAY AND SHOWED ROW COUNTS WHERE ITEM COUNTS BELONGED.** `stock_feed` is append-only by design; the header read 746 and 1,492 for a shop of 373 items because two pushes of 05-Sep had both been counted, and the one-day drift comparison, having picked an arbitrary push, said *"agrees"* about a day it had not compared. The owner read it on his first look. | `_feed_latest()` — the newest push per day is the day's figure everywhere the header and the drift page read it; the verdict wording says *"first day compared"* until there are two. **Rule: an append-only table is read by its newest row per key, never by its sum, and a comparison that has one side says so.** Kit `S226_DRIFT_TRUTH`. |
| **F-325** | **`push_snapshot.py`'s F-322 REMEDY WAS STILL WRONG, AND A SECOND DEFECT HID BEHIND IT: SAME-NAME MASTERS WERE SILENTLY DROPPED.** The S225 fix used the row count as a tiebreaker between same-date exports, so the owner's legitimate merge of the two LACTOVAX masters (one fewer row) was still beaten by the older twin; and the loader keyed on the item name, so before the merge one of the two masters had never reached the server at all — the +7 was real and had two causes. | Row count is a GUARD (fewer than 60% of the fullest export = a category subset, refused) and never a tiebreaker — the newest export wins; masters with the same normalised name are SUMMED. Proven against the real archive (`EVIDENCE_dryrun_real_archive`). **Rule: a heuristic that decides between two files must be stated as a guard with a threshold, not as a preference; and a loader that keys on a name must say what it does with a duplicate.** Kit `S226_F322_SUBSET`; `03a84524…` hashed on manojz. |
| **F-326** | **`push_expected.py` NEVER READ PURCHASE BILL ITEM WISE, SO PURCHASES STOPPED AT 03-Sep WHILE THE PAGE SAID *373/373 AGREES*.** Marg has two reports with near-identical names — PURCHASE ITEM WISE and PURCHASE **BILL** ITEM WISE — and only the second carries the bill date per line; the sender read the first and dated nothing after the 03-Sep baseline. The owner's *"check data of purchase exports in file, its there"* was right; the assistant's first fix double-counted where the two exports overlapped. The *"373/373"* agreement on the drift page was a FALSE agreement: both sides were missing the same purchases. | `billitem_after()` reads BILLITEMWISE, one export per bill date, ITEMWISE skipped where BILLITEMWISE covers it; Marg's own 06-Sep closing export then matched item-for-item. **Rule: two totals that agree prove nothing when they share an input — name the shared input before calling it a match. And a report name in Marg is read to the letter.** Kit `S226_PURCHASE_BILLITEM`; `a0e47a98…` hashed on manojz; wall-card wording owed. |
| **F-327** | **THE COUNT PAGE CRASHED ON EVERY CHIP BUT *TO DO*, AND HAD SINCE S213.** `render()` empties the list and appends its rows at the END, so while it runs `list.children.length` is 0; a differing row is rendered with its entry open → `openEntry()` → `calc()` → `bump()` → *"the list is empty"* → `render()` → … until the stack blew. *Differences*, *All*, *Has stock*, *Nil* all did it; only *To do* escaped because it removes the row instead. Found while building the search box, one tap away from a counter who had just marked his first item Not OK. | A re-entry guard on `render()`; `bump()` never re-renders during a render. **Rule: a page proven on one filter is proven on one filter — the walk taps every chip.** Kit `S226_COUNT_SEARCH`, walk 34/34. |
| **F-328** | **A SENT COUNT FORGOT IT HAD BEEN SENT ON RELOAD, AND TOLD THE TIME IN UTC.** *"It was sent"* lived in a variable; the owner had been told to reload for a fix, and the page came back as 0 / 373 with an enabled Send button — the next honest thing to do was count again and send a second count of the same morning. The banner then showed 04:46 for a count sent at 10:16 in Bareilly. | `S.sent` persisted with the record; a banner naming the count number and the wall-clock time; *"Start a genuinely new count"* is the one way forward. **Rule: any state that changes what the next tap means survives a reload; and a time shown to a person is the clock on the wall in front of them.** Kit `S226_SENT_STICKY`. |
| **F-329** | **THE REVISIT KIT MADE SEARCHING FOR A COUNTED ITEM SAVE ZERO OVER ITS FIGURE — IN SILENCE — FOR TWENTY MINUTES OF A LIVE COUNT.** An *OK* entry stored only the total; `S226_REVISIT` opened a revisited item filled in, found no breakdown, opened two EMPTY boxes, and `calc()` read them, computed 0 and SAVED it. 358 became 0 with the row still showing 358 from the render before. The owner: *"After reload, the previous entry was not showing … his entry wasn't there."* The assistant's own kit, installed the same hour. | `S226_ZEROFIX`: the breakdown is derived from the total that IS there; `calc(typed)` never writes on open — belt and braces. The owner moved the counters to the Excel pad and put the page on hold; NO count had been submitted. **Rule: a screen that opens a record never writes it; a value on record is never opened as a blank; and a fix installed during a live count is walked on the REVISIT path before, not after.** |
| **F-330** | **`save()` AND `load()` FAILED IN SILENCE, AND THE RECOVERY PATH THREW ON A `let` DECLARED BELOW IT.** `save()` was `try{}catch{}` — a browser refusing the write dropped every entry quietly; `load()` returned an EMPTY count on any parse error, so a damaged record started a blank count and the next save wrote over the bytes that might have been recovered. The hardening's own flags sat below `load()` in `let`s; `function load()` hoists, `let` does not, so the one path the code existed for threw *"Cannot access before initialization"* and killed the page. Found by driving it. | Read-back after every write; a `.bak` beside the record; anything unreadable set aside under `.rescue.<ts>` before anything is written; the flags declared above `load()`; the warning painted at the TOP of the page, not in a hidden section. **Rule: a store that can fail says so, loudly and permanently; and the walk drives the failure path, not only the happy one.** Kit `S226_SAFESTORE`, 13/13 + regressions. |
| **F-331** | **`S226_PAD_IMPORT` SHIPPED `padreader.py` dd32617b — WITHOUT THE `SHEET ROW` COLUMN — WHILE ITS OWN WALK HAD PROVED THE LATER COPY 38637a94 THAT HAS IT.** The walk imported from the folder it ran in, which held the newer file; the kit folder held the older; `SUMS.md5` was green on the wrong bytes. Live, a row corrected on SENT BACK TO FIX with a changed name could never be marked resolved and would be asked for again on every result sheet. Found at the next kit's open, by hashing. | The reader with `ref` shipped in `S226_PAD_ONPAGE` (`f617d7d5…`); the walk imports from `HERE` = the kit folder and is run FROM the kit folder. **Rule: the walk imports the file the kit ships, from the kit folder, and the kit's sums are computed from the folder the walk ran in. Intact is not identical.** |
| **F-332** | **F-233 BREACHED ONCE MORE: a read-only `git status` against the mounted repo, to look for `__pycache__` before a publish.** No lock was left this time; the rule exists because it has left one three times. `find` answers the same question. | Recorded; `find -name __pycache__` is the check, written into this session's brief. **Rule stands: never run `git` against the mounted repo, for any reason, read-only included.** |
| **F-333** | **THE JOINER PAGE'S RESET-PASSWORD BUTTON IS A STUB.** Amir's first login needed a password; the button on the joiner page does nothing; the password was set from the box (`/root/wa/venv/bin/python3 /root/portal/clinic_users.py passwd amir`). | Set from the box for now; the `/api/reset_password` route owed. **Rule: a button that does nothing is removed or finished before a person meets it.** |

**Next free: F-334 · D383** *(D377–D382 minted at the S226 close; Archive §S226 §2 carries the full text).*

**END OF FAULT → ACTION REGISTER v2.57 — F-333 is the last minted finding. The v2.56 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**

---
*Minted at the S226 close, 06-Sep-2026. F-329 is the finding this close is remembered for: a kit built to answer a counter's complaint destroyed a counter's work, and the owner's reload-and-look was the test that found it. The page was on hold within minutes and the Excel took the count — which is the fallback rule (standing ruling 7) doing exactly what it is for.*

---

## S227 findings — minted at the S227 close, 06-Sep-2026 (Archive §S227 §2 carries the full text)

| # | finding | action / rule |
|---|---|---|
| **F-334** | **MARG'S PURCHASE "LOOSE" COLUMN IS THE WHOLE QUANTITY IN UNITS, NOT A REMAINDER.** BIO D3 MAX "20 strips + 300 loose" was read as 600 units; 368 purchase lines of the financial year carried the shape. Found by the owner reading one item's purchase list. | `_purchase_units`: when the loose figure covers the paid strips it IS the units (free strips added only when loose equals the paid strips exactly); every line says which reading it got; the PURCHASE DATA AUDIT lists every instance. **Rule: a column's meaning is read from the data, never assumed from its name.** |
| **F-335** | **OVERLAPPING PURCHASE EXPORTS LEAVE THE SAME BILL LINE TWICE.** `purchase_export.superseded_by` marks a replacement only for the same type AND period; two exports with overlapping periods both stay live; 150 double entries in count #1 doubled the purchases again. | Dedupe on (vendor, bill, date, normalised item, batch, qty, free, loose, direction); lines of a replaced export dropped; counted once in the item life and the lanes; kept in the JSON audit, off the staff sheet. **Rule for the next run: one non-overlapping purchase export per period; the readiness gate to check it (kit C).** |
| **F-336** | **A CLOSING-STOCK EXPORT TAKEN MID-DAY SHOWS A LATE-KEYED BILL TWICE, WITH OPPOSITE SIGNS.** TYRO BR "74 strips 3 tabs OUT, then 34 strips 3 tabs IN"; ROSIKA FORTE −60 / +60; ASTOFEN SP; the first lookup list carried each item twice and the owner called it confusing. | Neighbouring spans whose residues have opposite signs are merged until none is left; pinpointing runs on the merged span; a residue against the window's net or under one strip is boundary noise and says so; ONE row per item on the lookup list. **Rule: an export day is a boundary with a width, not a line; the next run's exports are taken after the day's last entry.** |
| **F-337** | **AFTER AN ITEM MERGE IN MARG THE CLOSING-STOCK EXPORT IS DEFICIENT** — VINTAZ P 4500 INJ: two items merged, then the new stock report sent; Marg 24, shelf 0, no document on the server. Reported by the owner at the close. | Recorded; the item-ledger export for the window is the remedy, analysed on the server (S228 §1). **Rule: an item merge in Marg is an event the server must be told about, like a packing change.** |
| **F-338** | **THE ORTHOTICS WORD LIST HELD "CAP " AND SWEPT EVERY CAPSULE INTO THE ORTHOTICS LANE** (PANTO CAP, BIO D3 MAX CAP) in the first build of `S227_STAFF`. Caught by the desk and lanes regression walks before packaging. | The word removed; the walks keep the lane expectations for ordinary medicines. **Rule: a word list that classifies stock is tested against the whole shop's names, not the words it was written for.** |
| **F-339** | **A STAFF SHEET CARRIED THE SERVER'S OWN BUSINESS.** The first PURCHASE DATA AUDIT workbook went to Amir with 518 rows — double entries and loose-column arithmetic the server had already handled. The owner: *"show only the ones that need Amir and the exact reason … any staff worksheet has to be user friendly with only the required data."* | The workbook now carries FREE TOO HIGH and PACKING DIFFERS only, each with the exact thing to do; the rest stays in the JSON. **Rule: a staff worksheet carries only what needs that person, one exact instruction per line; the server's findings about itself never leave the server.** |

**Next free: F-340 · D390** *(D383–D389 minted at the S227 close; Archive §S227 §3 carries the full text).*

**END OF FAULT → ACTION REGISTER v2.58 — F-339 is the last minted finding. The v2.57 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**

---
*Minted at the S227 close, 06-Sep-2026. F-334 is the finding this close is remembered for: the analytics layer was built on a purchase quantity read the wrong way, and it was the owner reading one item's purchase list — not any walk — that found it. Two closes running, the human using the system found what the tests could not; the walks now carry both shapes.*

---

## S228 findings — minted at the S228 close, 07-Sep-2026 (Archive §S228 carries the full text)

| # | finding | action / rule |
|---|---|---|
| **F-340** | **581 OF 1,148 PURCHASE LINES HAVE NO BILL DATE, AND ALL 581 ARE APRIL, MAY AND JUNE.** Every stock query filters `WHERE bill_date IS NOT NULL`, so `_purchases_since()` sees 567 lines and silently drops the rest. An ITEMWISE export prints no bill date; `_store_lines()` dates each line from `purchase_bill`, which only BILLWISE / SUPPLIERWISE fills, and the earliest of those on the server is July. TRAMATEE exposed it: all 17 lines present, eleven undated. **This is the root of the whole S227–S228 "our stock disagrees with Marg" saga — the arithmetic was never wrong, it was running on a third of the purchase history.** | Export **PURCHASE BILLWISE 01-04-2026 → 30-06-2026**; `_redate_lines()` runs on every push and dates all 581 on arrival — no code change. **Rule: a row the system cannot date is a row the system does not have. An ingest that accepts an undated line must count it out loud, not drop it silently.** |
| **F-341** | **THREE SALE DAYS NEVER REACHED THE SERVER** — 19-04-2026 (2 lines), **04-05-2026 (220)**, **27-05-2026 (175)**; 397 lines. Every other day from 01-04 to 05-09 matches the Marg exports line for line. Found only by comparing the server's own `sale_line_item` against the exports on the PC, day by day. | Push those three days. **Rule: coverage is proven day by day against the source, never by a month total — two missing days inside a month that otherwise ties are invisible to any summary.** |
| **F-342** | **THE SERVER DATABASE WAS READABLE ALL ALONG AND THE SESSION WAS ASKING FOR PASTES.** The VPS is unreachable from every shell here (`healthz` http=000 from the device shell *and* the cloud container) — but `finance.db` is backed up nightly to Google Drive (`FinanceDB_Backups/finance_nightly.db.gz`), and that connector is attached. The owner had to say so: *"the data is backed up at connected google drive, so you have access to server db files too."* | `S228_CENSUS` retired. **Rule: before asking the owner to run anything, enumerate every connected source — a store that is attached is a store that was never checked if it was not opened.** *(Mechanism worth keeping: a tool result too large for the context is written to a file the container can read, so a 2.8 MB gzip arrived as base64 without entering the conversation.)* |
| **F-343** | **MARG'S NEGATIVE BALANCES PRINTED WITHOUT THEIR MINUS.** The comparison sheet used `_qw()`, which is unsigned by design, for the Marg column; twelve items where Marg is below zero read as a full shelf, and their differences read as stock *found*. Caught by a sub-agent screen read, by no check. | Sign restored as the word "minus"; the twelve are called out at the top of the sheet as a bookkeeping gap, not stock found. **Rule: an unsigned formatter may never render a figure that can be negative. `_qw()` is for magnitudes; the caller owns the sign, and a caller that forgets prints a lie.** |
| **F-344** | **THE TWO SIDES OF ONE ROW WERE WRITTEN IN DIFFERENT UNITS.** "Marg 1 strip" against "counted 2 tabs", difference "short 8 tabs" — the reader has to know the pack and do the arithmetic. Eight rows. | One shape per row: if either side reaches a whole strip both sides print in strips and tabs, otherwise both print in tabs; zero prints as "nil" on both sides. **Rule: two numbers a reader is asked to compare are written in the same unit, always.** |
| **F-345** | **A SHEET SORTED BY MONEY IN THE COPY WHERE THE MONEY IS HIDDEN.** Rows were ordered by value, so in the staff copy the order looked random — and BLADE (short 100 pcs), DISPO SYRINGE NIPRO 3ML (62) and GLOVES SURGICAL 7 (47), the three largest physical losses, sat near the bottom of Consumables because they carry no price. | Alphabetical inside each section. **Rule: a page's sort order must be legible in the state the reader is in. A hidden column may not decide a visible order.** |
| **F-346** | **COLUMN WIDTHS SUMMING TO 109 % ARE SILENTLY SCALED BY THE BROWSER**, so the widening applied to fix an overflow was cancelled: four cells still collided on paper (`minus 1 strip 3 tab18 strips 4 tabs`) and 202 on screen. Nothing errored. | Widths verified to sum to exactly 100 in both states, and the check added to the screen read. **Rule: a `table-layout:fixed` width set is an arithmetic claim — add it up before believing it.** |
| **F-347** | **TWO DOCUMENTS WRITTEN THIS SESSION WERE WRONG AND THE OWNER CORRECTED BOTH.** `S228_PURCHASE_HISTORY_HOLE` concluded April–July purchases were never exported (they were; only BILLITEMWISE had been looked at) and carried a bogus fake-vendor flag from a crude scan. The first `S228_EXACT_DATA_INVENTORY` named two missing Marg reports; **sale returns are inside the item-wise sale reports — 376 credit-note lines across all 182 CNs, with item, pack and quantity — and purchase returns are inside the item-wise purchase reports.** A "~40 strips residue" figure quoted to him earlier came from the S227 walk's synthetic fixture, not his data. | All three withdrawn in writing, in the same session, naming the error. **Rule: a report is named missing only after reading the report that would contain it. And a figure quoted to the owner must be traced to his data before it is spoken — a fixture is not evidence.** |
| **F-348** | **MARG'S BATCH-WISE ITEM LEDGER IS EXCEL TIME, AND AGGREGATING IN THAT DOMAIN CARRIES LOOSE AT 60 INSTEAD OF AT THE PACK.** Three further shapes in the same file: a column off-by-one on the balance, a running balance seeded in minutes while the accumulator was in tablets (one row contributing −2,880), and **negative balances arriving as TEXT** (`-2:10`), which cost 40 tablets on MEG QCS. | Everything converted to tablets at read time; nothing downstream ever adds minutes; the footer is found by its marker, not a fixed column. All three exports now walk exactly and tie to Marg's own Received/Issued totals. **Rule: convert a foreign encoding at the door. A quantity that is still in the source's units one function later will be added to something in ours.** |
| **F-349** | **THE ARCHIVE ROUTER REFUSED EVERY ITEM-LEDGER EXPORT** — `read_preamble()` assumed the header row starts in column A, and `RE_RANGE` demanded "FROM a TO b" where Marg prints "FROM a - b". The files were arriving and being filed as `_UNKNOWN`. | Both fixed on the medical PC; new signature `STOCK_ITEM_LEDGER / BATCHWISE`, `uploadable: false`. Regression over 152 archived files: **148 identical, 4 changed — all four the previously-refused ledgers, now filing.** **Rule: a router change is proven by re-running the whole archive and counting what moved, not by the file that prompted it.** |
| **F-350** | **A COUNT COULD BE MEASURED AGAINST OUR OWN ARITHMETIC WHILE EVERY SCREEN CALLED IT MARG.** `stock_snapshot` is keyed `(as_on, item)` and **last write wins**; two senders push to it (`push_snapshot` = Marg's export, `push_expected` = ours) and nothing recorded which one a count's figures came from. *(For the 06-09 count the answer is Marg — `push_snapshot`, 373 items — so that count is sound; the exposure is general.)* | `S228_THREE_WAY` names the sender and the hour on the header, refuses to call a count trustworthy unless it was Marg's, and carries a red banner when it was not. **Built, not yet installed.** **Rule: a figure with two possible authors must record its author, or every page that quotes it is guessing.** |

**Next free: F-351 · D398** *(D390–D397 minted at the S228 close; Archive §S228 carries the full text).*

**END OF FAULT → ACTION REGISTER v2.59 — F-350 is the last minted finding. The v2.58 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**

---
*Minted at the S228 close, 07-Sep-2026. **F-340 is the finding this close is remembered for**: two sessions of stock arithmetic were run against a third of the purchase history, and nothing anywhere said so — the lines were on the server, dateless and silent, and every screen that quoted them was honest about a number that was built on a hole. F-342 is its twin: the database that would have shown it in five minutes was in the owner's Drive the whole time, and it took him saying so.*

---

## S229 findings — minted at the S229 close, 07-Sep-2026 (Archive §S229 carries the full text)

| # | finding | action / rule |
|---|---|---|
| **F-351** | **THE RESCUE TOOL HAD BEEN DEAD FOR ELEVEN HOURS AND SAID SO ONLY IN A LOG.** At the S228 close `read_preamble()` gained a fourth return value — `c0`, the indent fix for Marg's batch-wise stock report. **`marg_rescan.py` still unpacked three**, so it raised `ValueError` on the FIRST quarantined file of every run. The ten-minute pull recorded it as `PROBLEM: rescan=1` — **69 consecutive cycles, 06-09-2026 23:40:51 → 07-09-2026 11:00:32 IST.** Nothing else reported it. It is why the quarantine never cleared even though the registry already knew three of the files. | Two lines: unpack four, pass `c0` on to `dates_from` exactly as the router does. First green cycle **`07-09-2026 11:10:32.52  ok`**, and the owner's own scheduler then rescued 11 reports unaided. **Rule: a pipeline that reports its own failures only into a log file has not reported them. A component failing twice in a row belongs on a screen, not in a log — `rescan=1` was true 69 times and was seen zero times.** Second rule, narrower: **when a shared function's signature changes, every caller is a caller — the router was fixed and its own rescue tool was not.** |
| **F-352** | **THE PIPELINE PRINTS A REMEDY THAT CANNOT WORK, AND HAS DONE FOR WEEKS.** `push_expected.py`, on every run where any item computes below zero: *"That is not missing stock — it is missing PURCHASE data… Export PURCHASE > BILL ITEM WISE for the range in Marg."* Three faults in one sentence. **(a)** It cannot distinguish OUR arithmetic going negative from **MARG'S OWN FIGURE ARRIVING** negative — all twelve current cases are the second, printed as negatives in Marg's own closing-stock export, identical on 03-09 and 05-09, eleven unmoved for four days. **(b)** *"for the range"* means *since the baseline*, but reads as *the range of the problem* — which is how it became a request to the owner for an April–June export that could not have moved the figure by one unit, the baseline being 03-09-2026. **(c)** It names an export when the remedy is a **STOCK RECEIVE adjustment voucher** in Marg, and **no purchase is pending for any of them** — the owner's correction. | The message must first ask whether the item was already negative in the baseline export. If it was: *"Marg's own figure for this item is below zero; it is corrected by a stock adjustment voucher, not by any report"* — and never name an export. Only a genuinely computed negative may mention purchase data, and only for the window since the baseline. **Fix specified; queued with the pipeline work; nothing changed this session.** **Rule: a message a pipeline prints is a hypothesis, not a finding. Repeating it to the owner without checking the numbers behind it turns a machine's guess into a person's work.** |

**Recorded, not minted — the assistant's own errors this session, four of them, each corrected by the owner and each checkable in data already on his disk:** the export that could not have worked (F-352(b)); a *"clearing themselves"* trend asserted from falling totals when the items behind them were two purchase entries and **two of his own merges**, and when there was no observation after 06-09 morning to draw a trend from; `DISPO SYRINGE NIPRO` called a duplicate record when it is the 20-letter clip of `DISPO SYRINGE NIPRO 3ML`; and *"twelve purchase entries"* when no purchase is pending. **Rule, and it is the one to keep: check what the owner says against the data before restating what a script says.**

**Also recorded, not minted — the spine's own six defects**, all found by an adversarial review against real data AFTER 44 self-tests passed green, and all fixed before install: a rename would have minted a second item (373 → 397, twenty-four nameless, silent); a name-clash signal returned and discarded by every caller; a clip test measuring the incoming string's length rather than the index key, blocking real clips and admitting false ones; `_facts` resolving at width 20 for tables declared at 27, so one build could call a name unknown and write a cost from it in the same minute; `first_seen`/`last_seen` last-writer-wins over mixed date formats, the last source wiping 230 of 423; and a task's money frozen at first sight while the report moved on. **Rule: a green self-test suite proves the cases you thought of. S208's two defects behind 65 green checks, again.**

**Next free: F-353 · D413** *(D398–D412 minted at the S229 close; Archive §S229 carries the full text).*

**END OF FAULT → ACTION REGISTER v2.60 — F-352 is the last minted finding. The v2.59 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**

---
*Minted at the S229 close, 07-Sep-2026. **F-351 is the finding this close is remembered for**: a tool whose whole purpose is to rescue what the system refused had itself been failing every ten minutes for half a day, and the only thing that knew was one word in a log file. F-352 is its companion in the same shape — a sentence printed on every run for weeks, believed because it was printed, and acted on by the owner twice before anyone read the arithmetic behind it.*

---

# FOLDED IN AT THE S230 CLOSE — F-353 AND F-354, MINTED AT THE S229 POST-CLOSE

*These two were minted in `S229_F353_PARI_AND_THE_EXPORT.md` after the S229 close was already
written, and the Register never received them. The close that finds a gap like this is the close
that closes it — **canon was one number behind reality for a whole session, and the next free number
recorded here was wrong.***

**F-353 · When two items differ by a number, that number IS the item.** `PARI CR 12.5` and
`PARI CR 25` are two medicines — different companies, different rates, both in stock. They were
written into `OWNER_TODO_LIVE` side by side as a single instruction, which reads as *merge these two
with each other*, and described as "not on the shelf" — true of the retired **spelling** and false of
the **medicine**. The code was correct throughout; the sentence was not. **A note about a NAME must
never stand where the reader will take it as a note about the THING.**

**F-354 · Marg names the same rows more than one way. Search by the CONTENT you need — the columns —
never by the type name you expect.** The April–June item-level purchase export was reported missing.
It had been made on 27-Aug under Marg's *other* name for the same rows (SUPPLIER/ITEM WISE rather
than BILL/ITEM WISE), archived as `PURCHASE_ITEMWISE`, and pushed to the VPS on 04-Sep at 08:29 with
HTTP 200. Nothing was owed. `signatures.json` had carried both names as separate types since S205
precisely because one folder never mixes two groupings of the same rows: **the design was right and
the search was not.**

---

# MINTED AT THE S230 CLOSE — F-355 … F-362

## ⭐ F-355 · A GREEN SELF-TEST SUITE CANNOT CATCH A WRONG ASSUMPTION, BECAUSE ITS FIXTURES SHARE IT

The off-box state backup discovered what to ship **by file extension** — `.db`, `.csv`, `.jsonl`,
`.json`, `.html`. It shipped 46 files and **86 self-tests passed green.**

`/root/staff_ledger/ledger.jsonl` — 21,912 bytes, **the only record of what each member of staff
owes** — was not in the bundle. Neither were `advance_pct.json`, the waivers file, `users.json`,
`applications/`, `hold_ledger.jsonl`, the manual advances or the salary register.

The extension rule was written from an assumption about how the ledger stores were laid out. **Every
fixture in the self-test suite was built from the same assumption, so the suite agreed with the code
and proved only that the code did what it had been told to do.** No number of green checks could
have found this. Reading the live machine found it in one listing.

**Fixed** to directory-content discovery with an explicit skip list. Re-run: **46 → 55 files, with
`secrets_skipped=2` unchanged** — the second half of that sentence is the point, because it proves
that widening what is *found* did not widen what is *sent*.

**THE RULE: a self-test proves the kit against your idea of the world. Only the live machine can
tell you the idea is wrong. Walk the live shape before declaring any discovery rule correct.**
*(This is S208's two-defects-behind-65-green-checks and S209's four-green-gates, for the third
time. It is the most expensive recurring lesson in this project.)*

## F-356 · A WATCHER KIT IS NOT ARMED ON ITS FIRST INSTALL

The freshness layer's 26 legs were declared from reading the repository, and 115 self-tests were
green. Its first unarmed read on the live box reported `Docterz ingest — file has never been
created`. The path was plausible, the code that would write it exists, and **the log has never
existed**: there is no log and no timer for `docterz_ingest.py` anywhere on the box or in the
repository. It is run by hand.

Corrected to watch the **data it lands** — `sqlite_max` on `clinic_day_revenue.taken_at` — rather
than a log that was never written. **Watching data is strictly better than watching a log: a log
file can be touched by a job that then fails.**

**THE RULE: any kit that watches things performs one unarmed read against the live machine before it
is armed, and that read is treated as the step most likely to find an error — because it is.**

## F-357 · A DEAD SIGNPOST — THE LOG STOPPED AND THE WORK CARRIED ON

`C:\followup_tracker_local_test_kit\local_test_kit\followup_tracker\watch_followups.log` stops dead
at **01-Jul-2026**, on a line reading `Follow-up auto-push watcher STARTED`. The files it names have
kept arriving on the VPS ever since — `/root/wa/followup-inbox/` was 38 hours old when read, and
`Staff_Action_Today_2026-09-07.xlsx` was generated on schedule.

So the watcher named in that log **was replaced by another route more than two months ago and
nothing anywhere said so.** A `.tmp.driveupload` folder beside the outputs suggests the route is now
Drive sync. Nothing is broken; the signpost is a lie.

**Fault A in miniature. Mitigated rather than fixed: the VPS-side leg watches the OUTCOME — files
arriving — which is route-independent. Whichever mechanism delivers them, its death is now visible
the next morning.**

## F-358 · A PRIVATE ALERT TOPIC HARD-CODED IN THE REPOSITORY

`clinic_watchdog.py` carries the live ntfy topic URL as a default argument, in **two copies** in the
repository. Anyone holding that URL can read every alert this estate sends and publish convincing
fake ones to the owner's phone.

**Not touched — rotating it means changing every publisher at once, and it is live.** Severity
moderate, not urgent: the repository is private and the alerts carry no patient data by design.
**Carried to 1.6, credential consolidation, which is where it belongs.**

## F-359 · `*.json` IN `.gitignore` SILENTLY SWALLOWS ANY KIT THAT SHIPS SETTINGS

The blanket `*.json` rule excludes a kit's configuration file with no message. `PUBLISH_ALL.bat`
then refuses the whole publish and names nothing. It cost a cycle this morning on three
`appsscript.json` files and would have cost another tonight on `legs.json`.

**Caught before the publish this time**, by running `git --no-optional-locks check-ignore` over every
file in the kit and adding an exact-path negation. **This check now belongs in every kit that ships
a `.json`, and it costs one line.**

## F-360 · THE RENEWALS REGISTER DECAYS TO EMPTY BY DESIGN

`nagRenewals` computes `days = dateISO − today` and returns if `days > 30 || days < −45`. Nothing
ever rolls `dateISO` forward by `cycleMonths`, which is recorded on every entry and used by nothing.
So every item's life ends: quiet → nagged → **OVERDUE for 45 days → silently dropped, for good.**

Two entries are inside the drop window now, one of them the domain **every screen in this estate runs
on**. It plainly renewed and the array was never updated — so the nag has been crying wolf for a
domain that is fine, and in five weeks will stop mentioning it at all.

**The register also contains no technical vendor at all** — not Tailscale, whose key expiry can stop
the entire Marg lane, nor the API vendors, nor the Google account everything is built on. Fix is
1.7: roll the date forward, or record `lastDoneISO` and compute from it.

## F-361 · A BACKUP SCHEDULER THAT HAS NEVER PRODUCED A FILE

CyberPanel's configured backup jobs produce nothing, and have produced nothing for as long as there
is evidence. Marg's own automatic backup is the same story from the other end: `E:\auto` and
`E:\MARGBCKUP\auto` have been **empty since October 2025** while the scheduler still looks
configured. In both cases the configuration screen is healthy and the output does not exist.

**The real floor was the Hostinger weekly snapshot alone.** That is what the S230 shipper and the
freshness layer were built to raise. **A configured backup is not a backup. The file is the backup.**

## F-362 · THE MEDICAL PC'S CAPTURE CHAIN RUNS ONLY INSIDE ONE LOGON SESSION

`MargAgent.cmd` lives in `C:\Users\SET\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup`,
so the watcher, the heartbeat and the hourly backup sweep all run **only while the owner is logged
in over RDP**. Nobody logging in means no capture, no heartbeat and no backup — **and the only signal
is an absence.** A reboot on 29-Aug cost 21½ hours of uncaptured trading.

Fix prepared, not applied: a scheduled task at system boot running as SYSTEM — with the caveat that
SYSTEM changes which account writes to the Drive folder, so the offsite copy may need a real path on
`C:` instead. **Slotted after the freshness layer deliberately: a chain that cannot say when it last
ran should not be rewired first.**

---

**Next free: F-363 · D419** *(D413–D418 minted at the S230 close; Archive §S230 carries the narrative.)*

**END OF FAULT → ACTION REGISTER v2.61 — F-362 is the last minted finding. The v2.60 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**

---
*Minted at the S230 close, 07-Sep-2026. **F-355 is the finding this close is remembered for**: eighty-six green self-tests could not see that the only record of what each person owes was outside the backup, because the tests and the code shared one assumption. The live machine said so in a single listing. Every other finding in this close is a variation on the same sentence — **the check that agrees with you proves nothing.***

---

# MINTED IMMEDIATELY AFTER, AT THE SAME CLOSE — F-363

## F-363 · A CANON DOCUMENT'S DISK COPIES ARE SHORTER THAN THE PROJECT COPY, AND NOBODY WOULD HAVE KNOWN

The S230 reduction tranche moved 41 documents, each verified byte-for-byte before deletion. One
refused to verify and was **held back**: `HANDOFF_RUNBOOK_…Session229close_v161.md`.

| copy | bytes |
|---|---|
| project knowledge | **7,607** |
| `deploy_kits/KB_canon_all/` | 7,589 |
| `00_CANON_SNAPSHOT_S229/` | 7,589 |

The two disk copies are byte-identical to each other (`ce048f05…`) and both are **eighteen bytes
shorter** than the project copy. The difference is not only punctuation flattened to ASCII: §2 item 7
reads *"The junk"* on disk where the project copy reads *"The junk (D400-adjacent)"*.

**The manifest pinned the disk copy. The manifest was therefore pinning the shorter document, and
every hash check across three stores agreed with itself — because they were all checking the same
degraded copy.** The only thing that could have caught it was comparing across stores, which is
precisely what the copy-verify-then-delete rule forced.

**Held back rather than deleted.** The complete text survives only in project knowledge; deleting it
on the strength of a filename would have destroyed the only full copy. *That is the exact failure the
routine warns about: "a retirement plan once claimed 22 documents were already safely in a kit; five
were."*

**What is not yet known, and matters more than this one file:** whether the writing path that
shortened it has shortened anything else. v161 is already superseded by v162, so this document's own
loss costs little — **the mechanism is the finding, not the file.**

**OWED AT THE S231 OPEN:** compare project-knowledge and disk copies of the canon set by byte count,
not by name, and establish which write path transliterates. Until then, **no canon document is
retired from project knowledge on the strength of a same-named disk copy alone.**

## ALSO RECORDED — not a fault, a privacy fact

`claude/S195_medical_kit/marg_report.py`, moved in this tranche, carries **real patient names and
phone numbers** in its self-test fixtures — the repository copy was scrubbed to dummies under F-185
and the project-knowledge copy never was; the two are otherwise byte-identical. It now sits in
`D:\Downloads\ClaudeCowork\03_WORKING_PAPERS\S194_S202_moved_at_S230\S195_medical_kit\`, which is
**not** the repository, so **F-185 is not breached**. Recorded so it is never published from there,
and noted in that folder's `WHY_SAFE.txt`.

---

**Next free: F-364 · D419.**

**END OF FAULT → ACTION REGISTER v2.62 — F-363 is the last minted finding. The v2.61 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**


---

# MINTED AT THE S231 CLOSE — F-364 … F-368

## F-364 · A CANON DOCUMENT WAS RETYPED INSTEAD OF COPIED, AND THE SAME HAND IS STILL AT WORK

F-363 recorded `HANDOFF_RUNBOOK…v161` as eighteen bytes short with punctuation *"flattened to
ASCII"*. **That diagnosis was wrong.** The disk copy retains `·`×29, `—`×20, `⭐`×8 and `§`×5 fully
intact. Every one of the eighteen bytes is now accounted for:

| line | disk | project | delta |
|---|---|---|---|
| 25 | `quarantine 28 to 18` | `quarantine 28 → 18` | −1 |
| 98 | `The junk` | `The junk (D400-adjacent)` | **−16** |
| 113 | `offline, ``py_compile``, then` | `offline → ``py_compile`` → ` | −1 |
| 41, 128, 130 | `...` | `…` | 0 |

**No tool renders an arrow as the word "to" in one line and as "," and ", then" in another.** The
disk copy was **typed by hand**, and in the retyping a parenthetical was lost. Git corroborates: one
commit, born at 7,589 bytes, never longer.

**Scope, measured:** 56 documents live in both project knowledge and `KB_canon_all`. **54 identical,
2 different, neither a retype** — both are project knowledge lagging disk. F-363 is a single-file
event and has **not** spread.

**But the habit has not stopped.** Inside the disk `CANONICAL_MANIFEST.md`, the rows written at the
**S230 close** use ASCII `...` and ` - ` where the rows around them use `…` and `—`. Same hand, in
text authored by the current close routine, in a file that is otherwise healthy. It has not yet
dropped a phrase.

**ACTION — D423: canon documents are COPIED, never retyped.** If a document must reach two stores,
it is transferred as bytes, never re-authored. `project_write(local_path=…)` and a file copy are the
only two permitted routes.

**AND THE LESSON THAT OUTLIVES THE FILE, from the same day (see F-363's successor finding):** the
owner read the freshly *corrected* `S190_Staff_Advance_Policy_D331.md` and said it is still wrong —
the 50% capping and the waive-the-application-as-pending path are both live and absent from it.
**Both stored copies agree with each other, the newer one says SIGNED AND EXECUTED, and both are
wrong about what runs.** A byte sweep can only catch two stores DISAGREEING. It is structurally
blind to a document that agrees with itself everywhere and has been overtaken by reality. **Only the
owner, or the live code, can catch that.** OWED: rebuild that document from `staff_ledger.py` and
the D331 → D332 → later trail; trust neither stored copy.

## F-365 · A LIVE WHATSAPP CREDENTIAL SAT IN THE PUBLIC REPOSITORY FOR 84 DAYS

`followup-tracker/python test_send.py` — **note the literal space in the filename**, an accidental
save from a mistyped shell command — 799 bytes, created **16-Jun-2026**, carrying a MyOperator/WABA
`Authorization: Bearer` token (42 chars), the company ID, the phone-number ID and one real 10-digit
mobile number. **A complete, runnable credential set**, in a repository confirmed **public**
(`private: false, visibility: public`, read unauthenticated from the box).

**It is the CURRENT token, not a dead one.** `API_QUICK_REFERENCE_CARD.md` records the live WABA
token's masked hint as `lHCx…`; the census read `lH…Hn` from the leaked file. Same value. Anyone
who read the repository could send WhatsApp messages as the clinic.

**Contained, not closed.** The file was moved to `D:\dr-manoj-git\_to_delete_S231\EXPOSED_TOKEN_FILE\`
with a `WHY_SAFE.txt`, and five further mobile numbers were masked in `.env.example` (F-185).
**Removing it does not un-publish it. Only revocation closes this**, and the owner has emailed
Ms. Khushi Jain for the regeneration procedure and — the question that decides everything — whether
old and new tokens overlap.

**THE DURABLE FIX IS NOT THE ROTATION.** The rest of the codebase is disciplined: 69 secret-shaped
candidates had empty defaults, every auth path reads from environment or config, and 24 distinct
literals were all ruled out in context. This was not a design failure; **it was an accident that
nothing was watching for.** A pre-publish gate refusing a `Bearer <literal>`, a `-----BEGIN` block
or a bare 10-digit number would have caught it the same afternoon. **The F-185 phone gate already
exists — this is that machinery, one pattern wider.** That gate is the first build of S232.

## F-366 · THE SCAN LOOKED FOR A URL; THE TOPIC WAS STORED AS A BARE NAME

The ntfy rotation's first scan searched for `https://ntfy.sh/<topic>`. The **WhatsApp notifier**
stores its topic as `Environment=NTFY_TOPIC=<bare name>` in its unit file — no URL, no match. The
kit as first written would have moved the watchman, the health report and freshness to a new topic
and **left the WhatsApp notifier publishing to the old, public one.**

**A half-rotation is the worst of the three outcomes**, because the owner would have believed he had
moved. It was caught only because he remarked, in passing, that other devices were also subscribed.
**ACTION:** a rotation enumerates its consumers by VALUE, never by the shape the value happens to
take in one file — and it searches unit directories (`/etc/systemd/system`), not just the data root.

## F-367 · ROTATING A SHARED SECRET SILENTLY DARKENED FOUR DEVICES

One topic carried everything. Four devices subscribed to it — the owner's PC, the assistant PC, the
reception PC and the assistant's personal phone. **The rotation killed the old topic and all four
went dark at 06:45**, with reception no longer being nudged about inbound WhatsApp and no error
anywhere to say so.

Two consequences: those staff devices had also been receiving **the owner's system alerts** all
along (→ **D420**, the split); and had the full-message push shipped before the split, **every
patient message would have appeared on the assistant's personal phone.**

**ACTION:** before rotating any shared secret, enumerate **who consumes it and on what device**, not
merely which files hold it. A credential's blast radius includes people.

## F-368 · SIX STALE COPIES OF THE SECRETS FILE, AND STALE CODE, IN LIVE FOLDERS

`/root/wa/` holds `.env.bak_20260707_162509`, `.env.bak.20260708-102630`, `.env.preswap.102831`,
`.env.bak_s126_20260708_212316`, `.env.bak_s127_pre_rotation_20260708_232229` and
`.env.bak_s127_step2_20260709_085801` — each with `MYOP_AUTH_TOKEN` at line 29 and
`MYOP_LOGS_TOKEN` at line 12. After the WABA rotation their WhatsApp token is dead, **but they still
hold every other secret in that file, including ones never rotated.** Three more
(`_backup_S231_*/env.bak`) were created by this session's own work and are current.

Also in live folders: `/root/wa/waba.py.bak.20260814_172324`,
`/root/wa/portal_console.py.bak-20260812-0729`, `/root/wa/wa_logs/portal_console.py.new`. **These are
the files a future sweep finds and believes.** OWED, not urgent, and not to be touched mid-rotation.

---

**Next free: F-369 · D425.**

**END OF FAULT → ACTION REGISTER v2.63 — F-368 is the last minted finding. The v2.62 and earlier end-markers above are retained in place as truncation-proofs. If you cannot see this line, you are reading a truncated copy.**

---

# MINTED AT THE S232 CLOSE — F-369 · F-370 · F-371

## F-369 · FOUR LIVE FILES THAT NO PIN LIST HAS EVER CARRIED — and a manifest naming two CURRENT Registers

**The task was one line of records housekeeping.** `OWNER_TODO_LIVE` said the `S232_FRESHNESS_GREEN`
kit had moved `/root/finance/freshness.py` to `66d93a63…` and that its Register and manifest rows
were stale. **The pin had no row to be stale in.**

`gen_live_pins.py` reads the Register between `## CURRENT LIVE FILE VERSIONS` and the `§12 STATE`
heading — the CONSOLIDATED table. The four S230 resilience/freshness files
(`freshness.py` · `freshness_legs.json` · `clinic_state_backup.py` · `verify_restore.py`) were
written only into a per-session WAVE table *below* that heading, which the generator never reaches.
**So they appeared in no `live_pins` list — not S230's, not S231's — and `verify_live_pins.py` had
never held the box to one of them. Live since S230; unchecked since S230.**

**Second half:** the manifest named **two CURRENT Registers.** The S231 close advanced the bare-name
version-chain row (`KB_Register | v5.81 | CURRENT`) and never touched the strict-name Tier-0 row,
which still read `KB_Register_v5_80_S230` **CURRENT** — and there was no strict-name row for v5.81
at all. **F-116/F-308 family, a fifth occurrence.**

**Neither could ever halt Phase 0**, because Phase 0 verifies bytes and these are labels and
absences. 413/413 was green over both this morning, as at the S231 and S230 closes.

**RULE.** **A hash gate proves that the rows it HAS are true. It says nothing about the rows it does
not have.** Every check in this estate is of that kind, and an absent row is a live file nobody is
checking — the exact fault `gen_live_pins.py` v1.1 was built to prevent, arriving through the one
door that tool does not watch. **A file is not pinned because a record mentions its hash; it is
pinned when the generator emits it and the checker holds the box to it.** Adding a live file and
regenerating the pin list is ONE step, and the VPS row count moving is the proof it worked.

**CLOSED the same session.** One atomic move across four files; VPS rows **95 → 99**, diffed
path-by-path; `MD5SUMS_ALL` 413 → 414 green. **The owner then read all four back from the box in one
command and all four matched**, so `freshness.py` is a pass and not a declaration.

---

## F-370 · A LIVE-CODE COMMENT STATING TWO RETIRED RULES — and the same file contradicting itself

`staff_ledger.py`'s D331 header block above `advance_pct()` — **the block anyone reads to learn the
rule** — carried both of the errors that made the policy document wrong:

1. *"default 50; **Darpan 75, the owner's ruling**"* — **retired by D352 (S204)**, and confirmed
   retired by **D425 (S232)**.
...
**`decide()`, forty lines below in the same file, has carried the correct D374 note since S225.**
The file has said two different things for four sessions, and the wrong one is at the top.

**No behaviour was ever wrong.** The box has held `{"Darpan": 50}` throughout — which is how D352 was
able to measure the divergence in the first place. **This is a records fault inside a code file**,
and D352's own lesson is why it matters: *an exception belongs in a settings row, never in a code
fallback* — and this comment was the last place that retired exception still lived.

**PREPARED, NOT APPLIED.** `deploy_kits/S232_F370_COMMENT/` has no installer on purpose: installing
it alone would spend a VPS trip, a restart and an owner action to change a comment. **It rides with
the next `staff_ledger.py` kit, and its file is the new BASE.** Proved harmless by compiling both
files and comparing all **312** code objects recursively — identical signature `cc74f70b…`, and a
non-comment diff that is empty.

---

## F-371 · SILENCE WAS LOAD-BEARING

*Reserved at the S231 post-close and minted here. **The number moved**: `S232_NOTIFICATION_LAYER_PLAN`
§6 reserved F-369 for this, and the session spent F-369 and F-370 in flight on other findings.*

**The freshness layer was designed so that quiet meant healthy.** Twenty-six legs, and a push only
when something was stale. **But quiet also meant "the job never ran", and the two were
indistinguishable to the only person who would have noticed.** A missing 08:05 line was itself the
finding, and nothing was watching for a missing line except the owner remembering to look.

**The owner found it in one sentence** — *"phone stayed quiet, better if it lists all it checks with
check marks"* — the second time in one day that he caught something a byte-level check structurally
could not.

**CLOSED at S231 and proven the same hour.** The daily-green line collapses to one title on a phone
and expands to 26 ticks, at most once a day; a stale leg still shouts exactly as before. **So the
ABSENCE of the morning line is now readable, which is the whole point.** And it earned its keep on
its first run: it showed `clinic day revenue ingest` sitting at 4 days inside an 8.3-day window — a
leg that could be dark for a week and still tick.

⚠ **That carries an expiry, recorded so it is not forgotten:** the wide windows are teaching the
owner what normal looks like. **Once the baseline is known the windows must come in, or a tick stops
meaning "healthy" and starts meaning "inside a tolerance nobody remembers setting". A green is only
worth reading if it could have been red.**

---

**Next free: F-372 · D428.**


**END OF FAULT → ACTION REGISTER v2.64 — F-371 is the last minted finding. The v2.63 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S232 close: the first 513423 bytes of v2.64 are byte-identical to v2.63 (md5 f1eead67a0119ae8fc23dcb7ae14f4a3). If you cannot see this line, you are reading a truncated copy.**


---

# §S233 — 08 September 2026 · FIVE FINDINGS, ALL FIVE THE ASSISTANT'S OWN

## F-372 · `sheets_pull.py` v1 had no pacing and no retry, and Google refused five books of eight

**Found by:** the owner's first live run, 15:20 IST, minutes after a preflight that reached all eight.
**Shape:** preflight made ~16 Google calls spread over eight seconds; the run made ~34 in under one
second, because v1 fetched every tab of every book back to back. Sheets quota is counted per minute
and this box has other writers on the same Google project — the call-hook receiver writes
`Call_Durations` all day. Five books returned `APIError`.
**Class:** the assistant's own. A backup job that hammers its source is not a backup job.
**Fix (v2, shipped and live):** every call goes through one paced wrapper — `API_MIN_INTERVAL_S`
default 1.2 s, about 50 calls a minute from this job — and a rate refusal is retried with
exponential backoff, `API_MAX_RETRIES` default 5. **A permission refusal is never retried: a retried
403 is a slower 403.** Walk checks 30–33 make Google refuse twice and prove the retry carries
through, then make the refusal permanent and prove the previous export survives.
**Status: CLOSED**, proven live — the four-book run completed in about a minute with no refusals.

## F-373 · a rate refusal wore a sharing refusal's words, and would have sent the owner to Google for nothing

**Found by:** the same run. **Shape:** v1 caught every exception in one `except` and printed one
message — *"share this sheet with the service account"*. Five sheets that were **already shared
correctly** were named that way. Had the owner acted on it he would have re-shared five sheets,
found nothing wrong, and lost trust in the tool's next honest message.
**Class:** the assistant's own, and **the F-352 family: a pipeline message that causes a wrong
action.** F-352 was minted at S229 for exactly this and the lesson had not been carried into new
code.
**Fix (v2):** `is_rate()` / `is_permission()` read the HTTP status three ways and fall back to the
message text; the two get **different exit codes — 41 permission, 43 rate** — and different words,
with 43 saying explicitly *"DO NOT re-share anything"*. **Walk check 28 asserts that a rate refusal
never contains the word "share"**, so the regression cannot return quietly.
**Status: CLOSED.**

## F-374 · a canonical document described the hash gate wrongly, and rebuilding to that description NARROWED it

**Found by:** the assistant, while adding one row to `MD5SUMS_ALL.txt` — caught only by diffing the
new row list against the shipped one.
**Shape:** `README_VERIFY.md` stated that exactly two files are excluded from `MD5SUMS_ALL.txt`,
*"`MD5SUMS_ALL.txt` itself and `KIT_ID.txt`, which carries its hash."* **Both halves are false.**
`KIT_ID.txt` **is** in the gate and always has been, and it carries no hash — it is a session
identity card holding the close's version numbers and standing warnings. A rebuild that trusted the
prose produced **428 rows where 429 were owed**, dropping `KIT_ID.txt` silently.
**Why it matters:** the rebuilt gate **exits 0**. A byte gate cannot see a row that is not there —
**the F-369 shape, three sessions after F-369 was minted for it.** Had it shipped, `KIT_ID.txt`
would have sat inside canon, unhashed, indefinitely.
**Fix:** the paragraph corrected in place and visibly, with the rule written beside it — *when you
rebuild `MD5SUMS_ALL.txt`, do not rebuild from a description of what it should contain; diff the new
row list against the old and prove nothing was dropped.* The S233 rebuild was accepted only after
reporting **added: one · dropped: none**.
**Status: CLOSED. The general lesson stands open as a habit, not a code fix.**

## F-375 · the one-line installer could not be run before the pull that fetches it

**Found by:** the owner, pasting the line twice and getting *No such file or directory* twice.
**Shape:** the handover promised **one line** — `bash /root/deploy/repo/deploy_kits/S233_SHEETS_BACKUP/install.sh`
— but the installer lives **inside the deploy clone**, and its own stage 2 is the git pull. It cannot
pull the repository that contains it. Chicken-and-egg, in the assistant's own instruction.
**Class:** the assistant's own; a handover defect, not a code defect. **The install is two lines: the
deploy pull, then the installer.**
**Fix:** stated to the owner immediately; to be written into the kit's install file and into any
future one-line entry point. **Status: CLOSED for this kit; the pattern is the thing to remember.**

## F-376 · `py_compile` always writes a `.pyc`, and it refused the owner's publish

**Found by:** the owner's own publish gate — *"REFUSING - at least one file was silently excluded by
`.gitignore`. Nothing committed or pushed."*
**Shape:** the project's build discipline is *build/test offline → `py_compile` → then install*. The
assistant ran `python3 -m py_compile` inside the kit folder. **`python -B` does not prevent it:**
writing the bytecode is what `py_compile` is for. The resulting `deploy_kits/S233_SHEETS_BACKUP/__pycache__/`
is blocked by `.gitignore` line 74, so the kit would have shipped with four files silently missing
from the commit — and the gate stopped it. `PUBLISH_ALL.bat`'s own comments warn about this exact
trap for the phone gate; the same trap took the assistant.
**Fix:** the compile check now writes its output to a temp path —
`py_compile.compile(f, cfile='/tmp/chk.pyc', doraise=True)` — leaving no `__pycache__` behind. The
stray files were moved to `D:\\dr-manoj-git\\_to_delete_S233\\` with a `WHY_SAFE.txt`.
**Status: CLOSED. The gate worked exactly as designed and the refusal was the finding.**

---

**Next free: F-377 · D433.**


**END OF FAULT → ACTION REGISTER v2.65 — F-376 is the last minted finding. The v2.64 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S233 close: the first 519460 bytes of v2.65 are byte-identical to v2.64 (md5 e8630e17f22964fa231dde15294b34d6). If you cannot see this line, you are reading a truncated copy.**

---

# F-377 · THE PAYMENT REGISTER SHEET HAS SILENTLY RE-DATED 95 OF ITS OWN 250 ROWS

**Session 234 · 08-Sep-2026 · NOT the assistant's · the estate's own data · CONTAINED, residue OPEN**

**Where:** the owner's **Payment Register** Google sheet, personal account, created and appended by
`registerSheet_()` in the Inbox Janitor.

**Found by:** building the VPS table for it and walking the code against all 250 real rows before
handing anything over. **No gate could have found this** — every file involved was intact.

**Shape.** The Janitor appends the date as TEXT, day-first, always:
`Utilities.formatDate(m.getDate(), Session.getScriptTimeZone(), 'dd-MM-yyyy')` — read from the
project's own exported source, not recalled. But of the 250 data rows on 08-Sep-2026:

* **155 are still text, and every one of them has a day above 12**
* **95 are real date cells, and every one of them has a day of 12 or less**

155/155 and 95/95 is not chance. **The spreadsheet parses an appended string as a date whenever it
CAN, reading it MONTH-FIRST.** `19-07-2026` has no 19th month and stays text; `09-07-2026` is
accepted — as **7 September**, when **9 July** was meant.

**Why the pipeline is nevertheless correct.** Those 95 cells are formatted `mm-dd-yyyy` (94) and
`m-d-yyyy` (1), so the sheet **displays** the exact characters the Janitor wrote, and `sheets_pull.py`
takes **displayed** values. Reading every row **day-first** recovers the intent for all 250 —
measured, 250 of 250 parsed. **A month-first read would put 91 of 250 rows wrong by up to two
months, in silence.**

**Two heuristics tested and both dead.** No row anywhere proves month-first, so a whole-file
order detector learns nothing. And the sheet is **not chronological** — 129 inversions in 249
consecutive pairs — so a "neighbour fits between" rule has nothing to stand on.

**Containment.** `payments_register.py` reads day-first uniformly and carries **EXIT 44**: if more
than 5% of dated rows stop parsing as day-month-year, the ingest refuses and writes nothing. A
re-format of that column in a browser breaks the round-trip, and month-first strings (`07-19-2026`)
do not parse at all — so the tripwire fires rather than the table filling with wrong dates.

**Status: CONTAINED for everything downstream of the displayed text. The residue is OPEN and it is
the owner's:** those 95 cells are **stored** in Google with day and month swapped, so sorting that
sheet by date **inside Google**, or reading it with unformatted values, gives the swapped dates. The
fix is upstream in a project under the standing *read, never act* hold and was not taken.

**The lesson, and it is not about dates.** A file can be byte-perfect, hash-green and complete, and
still mean something different from what its author wrote — because a system in the middle
reinterpreted it. **Intact is not valid** (A0), extended: *intact is not even unchanged.*

---

# F-378 · THE DRIFT CHECK'S SECOND COMPARISON DID NOTHING, AND SAID SO IN WORDS THAT READ AS FINE

**Session 234 · 08-Sep-2026 · THE ASSISTANT'S OWN · found on the first live install · CLOSED**

**Where:** `gas_export.py` v1, kit `S234_GAS_WEEKLY`.

**Found by:** **the owner running it on his own box.** Behind **24 green selftests and a 26-check
install walk.**

**Shape.** The kit exists to answer two questions: *what changed in Google this week* and *how stale
is the copy everybody reads*. v1 learned a folder's contents by reading its `_PROJECT.json`. **The
repository copy it compares against — `deploy_kits/S230_GAS_EXPORT/` — was written BY HAND at S230
and has no `_PROJECT.json`.** So v1 returned nothing for it, and the first real run printed, three
times:
...
**The copy was right there.** The half of the kit that answers the harder question was **inert**, and
its output read as a mild note rather than a failure.

**Why every gate passed.** The selftests built both sides with the exporter's own writer, so both
sides always had a meta file. **The fixture shared the defect's blind spot.** This is the S208/S209
family: *a green selftest proves the kit, not the join.*

**Fix (v2).** `read_meta()` builds the metadata by reading the files on disk when there is no meta
file, counting lines and hashing exactly as the exporter does. **Five new checks**, including one
asserting that a *matching* hand-made folder reports **nothing** rather than "no copy" — the check
whose absence hid this. Selftest 24 → **31, 0 failures**. Proved against the three real folders
(977 / 796 / 923 lines, all matching) and then live.

**A second fix rode with it.** A file differing only in trailing whitespace is now reported as
`changed_whitespace` — *"not an edit"* — because one S230 file has no final newline and **a weekly
false alarm is how a real alarm gets ignored.**

**Status: CLOSED, v2 live, `072a941177fdf58f2c311aa164ea4e0d`.**

**The lesson.** A refusal that reads like a note is worse than a crash. *"There is no X to compare
against"* was true of the metadata and false of the world, and it sat in the calm part of the output
where nobody looks. **When a check reports "nothing to do", make it say WHY there was nothing — and
put a test on the case where there was something.**

---

**Next free: F-379 · D437.**


**END OF FAULT → ACTION REGISTER v2.66 — F-378 is the last minted finding. The v2.65 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S234 close: the first 525609 bytes of v2.66 are byte-identical to v2.65 (md5 c9d1cef3832163fac96d6d561824ca11). If you cannot see this line, you are reading a truncated copy.**

---

## §7.5 — S235 and S236 findings: F-379 … F-384

*Appended at the S236 open, 09-Sep-2026. **F-379 … F-382 were minted in prose at the S235 close and
the register row was OWED** — the close report named them exactly for this reason, and §5 of
`START_HERE_SESSION_236.md` carried the debt so it could not be lost. **F-383 and F-384 are S236's own — F-383 the
assistant's, F-384 the linchpin's, found by measuring it.***

---

### **F-379 (S235 · OPEN, deliberately not patched · `/finance/approvals`)** — a floor labelled as a total.

**The Orthotics "Amount" column sums the pack price ONCE PER LINE regardless of packs sold.** A belt
sold 3 packs at Rs 450 contributes Rs 450, not Rs 1,350.

> **Rs 1,01,167.74 shown · Rs 1,04,917.74 true · understated by Rs 3,750, about 3.7%.**

The same fault sits in the per-day orthotics figure beside it. **Nothing else in the estate sums that
column unweighted** — `orthotics_value_p` in `S235_DERIVE` is the correct number.

**Why it is OPEN and not fixed.** The live page is deliberately NOT patched. The fix happens **once,
in one place**, when a screen is pointed at the derived table. Patching the page now would mean two
implementations of one number, which is the fault this whole derivation pass exists to end.

**The shape worth carrying: it is not wrong, it is LOW — and low reads as plausible.** A total that
crashed would have been found in a day. A total that is 96.3% of the truth survives every glance,
and it *widens as multi-pack sales grow*, so the error is a function of business success.
**RULE: a money column that can only ever be under-stated is not a rounding question — assert it
against a second computation, because no reader will ever see it as wrong.**

---

### **F-380 (S235 · CLOSED at our end, OPEN as a vendor question · Marg reports)** — the report clips the name, and the SIZE is what falls off.

**Sale report: 20 characters. Purchase reports: 27.** Measured, not guessed, on all **373** products:
**22 are ambiguous at 20 characters, 8 at 27** — and **every one of them is an appliance.**

**Consequence:** size-level stock cannot be proved from the reports alone. Two different sizes of one
product arrive as the same string, and any join that trusts the name silently merges them.

**Fixed at our end** by the naming rule (D439): `WHAT IT IS · BRAND · SIZE`, twenty characters or
fewer, size last. All 93 orthotic products renamed under it, **zero collisions at either width, no
duplicates.** **Asked of the vendor** as a report-width or item-code question, in the message to Mr
Ram Singh.

**RULE: measure a truncation against the whole catalogue before designing around it.** The instinct
was to fix the names that looked long; the measurement said which 22 actually collide, and that is a
different and much smaller list.

---

### **F-381 (S235 · OPEN, owner/vendor · Yuvika bill 571 of 01-Sep)** — goods received do not match the bill.

**Marg received exactly what the bill said** — LF L x2, LF M x2, RT L x2. **The shelf holds LF M 2 and
RT M 2, and RT M was never purchased at all**; only two wrist splints have sold all year.

**This is a supply or shelving error, not an entry error.** That distinction is the whole finding: the
data entry is provably correct, so no amount of checking Marg would ever have found it. It took a
physical count to see it.

**A wrong reading of mine, corrected from data at the owner's insistence, rides with this finding.**
I had warned that three surpluses might be a purchase-entry timing artefact. Two Marg stock reports
are held — 04-09 and 06-09 — and **every purchase line of Yuvika bills 571 and 585 shows as an
increase between them.** Timing ruled out; the surpluses are real. **RULE (and it is D-worthy): never
prune a stock report. Two consecutive reports turned an unanswerable question into a proof in one
query, and the owner was right to refuse my theory until it was tested.**

---

### **F-382 (S235 · CLOSED by design · Marg voucher printing)** — six to eight lines, and the rest vanish silently.

**Marg prints only six to eight lines on a voucher.** A 47-line correction is therefore **nine
vouchers** — six for the short set, three for the excess.

**The dangerous half is not the limit, it is the silence.** A longer voucher is truncated and **the
missing lines cannot be seen on the printed page.** The operator has no symptom to notice.

**So the batching is done FOR the operator, never left to care.** One batch = one voucher in Marg,
never mixed, never split; after each batch it is printed, read against the batch line by line, and
the Marg voucher number written into the batch summary. That read-back is the only thing that catches
a truncation.

**RULE: when a device silently drops output, the design must make the dropped part visible somewhere
else — a count to check against — because "be careful" is not a control.**

---

### **F-383 (S236 · CLOSED same hour · the assistant's own · `ORGANISE_DOWNLOADS.ps1`)** — a helper named `Mv`, and a silent SUCCESS reported as a zero.

**A one-off tidy script for `D:\Downloads` defined `function Mv($n,$d)` and called it 218 times.
`Mv` is a BUILT-IN ALIAS for `Move-Item` in Windows PowerShell, and in PowerShell's command
resolution an ALIAS BEATS A FUNCTION.** Every call went to `Move-Item` instead, with the function's
logging, its counters and its skip-and-collision guards bypassed entirely.

**What the owner saw: `MOVED 0 | SKIPPED 0 | FAILED 0`.** What had actually happened: **all 218 items
moved, correctly, to the right destinations** — because the alias received the same two arguments and
the working directory happened to make the relative paths resolve. The log file held a header, a
blank line, and a summary of nothing.

**This cost him two extra double-clicks and me a wrong diagnosis** — I read the zero as "nothing
moved", shipped a v2, and only found the truth by listing the destination folders and counting what
was in them against the plan.

**Why the sandbox passed it.** The script was proven in a Linux PowerShell 7 sandbox: 218 moved, 218
restored by the undo, folder byte-identical. **PowerShell does not define the `mv`/`ls`/`cp` aliases
on non-Windows, because they would shadow the real tools.** So the fixture could not contain the
defect. **This is the S208/S209 family exactly — a green gate on the wrong shape — and it is now the
third time a proof has been run somewhere the fault cannot live.**

**Fix.** Helpers renamed `MoveOneItem` / `RestoreOneItem`; each script now runs
`Remove-Item -Path Alias:<name> -Force -ErrorAction SilentlyContinue` before use, so a shadowing
alias cannot win even if one exists; and each script **counts itself** — it compares handled against
an expected total baked in at build time and prints a red warning if they disagree. Re-proven in the
sandbox **with a hostile alias deliberately defined**: 218 moved, 218 restored, byte-identical.

**Two rules, and the second is the bigger one.**

1. **Never name a helper with a short verb.** `Mv`, `Cp`, `Ls`, `Rm`, `Sl`, `Gc` are all aliases in
   Windows PowerShell and all of them silently outrank your function.
2. **⭐ A SILENT SUCCESS IS AS DANGEROUS AS A SILENT FAILURE.** The register is full of findings where
   a thing reported success and had done nothing (F-155, F-161, F-179, F-378). **This is the mirror
   image and it had never been minted: the work was done and the report said nothing happened.** It
   sent me to rebuild something that was already correct — the exact cost of F-352, arriving from
   the other direction. **A report that disagrees with the world is a fault whichever way it leans.**
   Therefore: **a script that acts must reconcile its own count against what it intended**, and say
   so out loud when they differ.

---

### **F-384 (S236 · CLOSED same session · `CANONICAL_MANIFEST.md`, the linchpin itself)** — THREE families with TWO rows each claiming CURRENT, and a byte gate that cannot see it.

**Found by measurement, not by reading.** While rowing the S235 canon debt, every manifest row was
parsed and asked one question: *does this row claim to be CURRENT?* The answer should be exactly one
row per document family — **F-308, minted for precisely this, and the warning is printed in
`KIT_ID.txt` where every session sees it.**

**Three families answered twice:**

| family | row A | row B |
|---|---|---|
| `KB_Register` | line 343 · `KB_Register_v5_83_S233` · **CURRENT** | line 504 · bare-name · **CURRENT — S234 close** (v5.84) |
| `KB_History_Archive` | line 351 · `KB_History_Archive_v1_81_S234` · **CURRENT** | line 614 · bare-name · **CURRENT — S233 close** (v1.80) |
| `START_HERE_SESSION` | line 367 · `_235` · **CURRENT** | line 369 · `_233` · **CURRENT** |

**The mechanism, and it is the interesting part.** The manifest carries **two indexes of the same
documents** — a filename-keyed block (`KB_Register_v5_83_S233`) and a bare-name family block
(`KB_Register` | **v5.84**) — and **the S234 close updated them inconsistently, in opposite
directions for two families:**

- For `KB_Register` it wrote the new **bare-name** row and demoted the old bare-name row, but **never
  added a filename-keyed row for `KB_Register_v5_84_S234` at all** and never demoted
  `_v5_83_S233`. The filename index is a whole version behind.
- For `KB_History_Archive` it did the exact opposite: it added the **filename-keyed** v1.81 row and
  demoted its predecessor, but **left the bare-name family row at v1.80 still saying "CURRENT — S233
  close"** — one close stale in the row the pin generator keys on.
- `START_HERE_SESSION_233` was simply never demoted when 234 and 235 arrived.

**Why every gate was green throughout.** `MD5SUMS_ALL.txt` carries a correct row for every one of
these files, and every file is present and unmodified. **The gate proves the BYTES; it has no opinion
about what a row SAYS.** Phase 0 halts on a hash mismatch and not on a false claim — the same blind
spot that let the STATUS line stand two closes stale (recorded at the S234 close) and the same family
as **F-369**, *a byte gate cannot see a row that is not there*, arriving now as **a byte gate cannot
see a row that lies**.

**Why it matters, concretely.** The START_HERE prompt states that **the manifest WINS on what is
current** — it is the reason versions are not hard-coded anywhere else. A future session obeying that
rule and reading the bare-name Archive row would have opened **v1.80**, silently missing §S234; one
reading the filename block for the Register would have opened **v5.83**, missing §S234 again. **Two
of the three would have been read as authoritative and both were wrong by one close.** This is
`Diagnostics_v1_7` (F-23) in a different costume: *a document that claims to carry the current state
and does not.*

**Fix, applied in this same bump.** All three families reduced to exactly one CURRENT row; the
filename-keyed rows for `KB_Register_v5_84_S234`, `_v5_85_S236`, `KB_History_Archive_v1_82_S236` and
`START_HERE_SESSION_236` added; `_v5_83_S233`, `_v1_81_S234`, `START_HERE_SESSION_233` and
`START_HERE_SESSION_235` demoted with the generator's keyword deliberately absent per F-308.

**⭐ THE INVARIANT, stated so it can be checked by machine rather than by eye:**

> **For every document family: at most ONE row claiming CURRENT in each index, and both indexes must
> name the SAME version.**

That phrasing matters, because the first attempt at this check was wrong in an instructive way. Asking
only *"how many rows claim CURRENT?"* flags the healthy state too — a family correctly rowed in both
indexes has two, and should. **The fault was never the count; it was that the two indexes named
DIFFERENT versions** (`KB_Register` v5.83 against v5.84, `KB_History_Archive` v1.81 against v1.80),
and for `START_HERE_SESSION` that **one index carried two live claims two sessions apart**. A check
that cannot tell the healthy shape from the broken one will be switched off the first week it cries
wolf.

**⭐ AND THE CLOSE-STEP: the close must MEASURE this, not assume it.** `KIT_ID.txt` has said exactly this since F-308 — *"a document family may
have exactly ONE row claiming CURRENT in the manifest, and the close must MEASURE that, not assume
it"* — and it was measured for `KB_Register` at the S234 close and reported as *exactly one*. **It
was one, by the generator's key. The measurement was too narrow: it asked about one family through
one index, and three families were broken through the other.** A check that is scoped to the place
the last fault occurred will keep passing while the next one grows beside it. **From this close the
count is measured for EVERY family across BOTH indexes.**

---

**Next free: F-385 · D442.**

**END OF FAULT → ACTION REGISTER v2.67 — F-384 is the last minted finding. The v2.66 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S236 open: the first 531383 bytes of v2.67 are byte-identical to v2.66 (md5 230b499c2891a34f7d53ddea1b170aed). If you cannot see this line, you are reading a truncated copy.**

---

## §7.6 — S236 CLOSE: F-385 … F-388

*Appended at the S236 close, 09-Sep-2026. Four findings, and **three of the four are about a report
that said success while the world said otherwise** — the same shape as F-383 earlier the same day,
arriving in three unrelated systems.*

---

### **F-385 (S236 · CAUSE REMOVED BY THE VENDOR THE SAME EVENING · the sale ingest)** — the discount was real, matched the owner's register, and our ingest threw it away.

**Found by putting two numbers side by side for the first time.** With `marg_item_discount` built
(T1), the owner's ruled price could be compared with the observed one. **27 of the 93 ruled products
have sale history. 22 of 27 record the FULL printed M.R.P. Zero record the discounted price.**

**The assistant's first reading was that the counter was not honouring the owner's discounts. THE
OWNER SAID OTHERWISE** — *"sale billwise with item details has a discount for the entire bill, as
found by you last session"* — **and he was right.**

**Measured across 446 unique bills from 19 archived exports, 17-Aug → 08-Sep.** Six bills contain
nothing but a single ruled appliance, so each is an exact reading:

| product | ruled | actually given |
|---|---|---|
| BRACE TYLOR UNISON | 30% | 31.89% / 29.76% / 29.76% |
| LEUKOCREPE 10CM*4IN | 25% | **25.00%** |
| ARM SLING M UNISON | 25% | 25.53% |
| LEUKOBAND 4 INCH | M.R.P. | **0.00%** |

And on mixed bills the arithmetic closed: of the four carrying a discounted appliance beside
medicines, **all four** had a bill DISCOUNT equal to the ruled item discount — three to the rupee.
A003333's whole bill discount of Rs 153.75 is exactly 25% of the LEUKOCREPE's Rs 615.00.

**This also answered a question S235 left explicitly open** — *"does an item-level discount reduce the
printed line rate, or fill the bill-level DISCOUNT column? Only answerable by opening one bill in
Marg."* **It fills the bill-level column.** Answered from 446 archived bills without opening Marg.

**The fault was ours:** `sale_line_item` stored gross line amounts and the bill number; the header's
`GROSS / DISCOUNT / NET / CASH` was read for the day's cash-UPI split and **not retained per bill.**
Every orthotic revenue figure we computed was therefore GROSS, overstating takings by up to 43% on a
30% product.

**⭐ CLOSED BY THE VENDOR, HOURS LATER.** Marg support added **a per-item discount column and a net
amount column** to the sale bill. The planned attribution build is superseded before it was written;
the work becomes *teach the importer three new columns*. See `S236_MARG_ADDED_THE_TWO_COLUMNS`.

**RULE: when a number is missing, find out where it went before deciding who is at fault.** This was
one short step from telling the owner his counter was cheating him. The report had the number all
along, one level up. **And the owner's correction was the thing that got there — for the fourth time
in two sessions, his challenge to the assistant's reading was right.**

---

### **F-386 (S236 · CLOSED · acme.sh on the VPS)** — a certificate renewal that reported success every time, against a fake authority, while the public website sat expired.

**`drmanojagarwal.com` — the public clinic website — served an EXPIRED certificate for a full day**
(expired 08-Sep 10:07 GMT). **The first signal was the owner hitting the browser warning by accident
on his phone**, while chasing an unrelated login problem.

**Three failures stacked, each hiding the next:**

**a · The domain was configured against Let's Encrypt STAGING.** From the owner's own run:
`Renewing using Le_API=https://acme-staging-v02.api.letsencrypt.org/directory`. Staging issues *test*
certificates signed by a fake authority — **every browser rejects them.** `followup.dr-manoj.in`
correctly used production; `drmanojagarwal.com` and `drmanojagarwal.in` did not.

**b · And acme.sh then advanced its own clock.** Having "renewed" against staging it recorded
`Next renewal time is: 2026-11-01` and **skipped the domain from then on**, while the live certificate
stayed expired. `--cron` printed a clean successful run every single time.

**c · And when a real certificate finally existed, nothing loaded it.** A valid production certificate
was issued at **20:56 IST**; at **21:53** the server was still handing out the June one. The fix was
`systemctl restart lsws` — **the correct certificate sat on disk, unused, for an hour.**

**A trap recorded on the way, still unresolved:** a forced re-issue failed with `404` on the ACME
challenge **even though the webroot was right** (`docRoot` is `$VH_ROOT/public_html`, where the files
were written). Two test files — one with an extension, one without, because real ACME tokens have no
extension — **both 404'd**, and fixing ownership from `root` to `drman1045:nobody` did not help.
**It did not matter that night because a valid certificate already existed, but it will block the
December renewal.**

**RULE: a renewal job that reports success is not evidence of a valid certificate. Assert what the
world can see — what the server actually serves — never what the tool says it did.**

---

### **F-387 (S236 · OPEN · the whole estate)** — six of nine sites were in no renewal list at all, and nothing anywhere watches an expiry date.

The `--cron` run named **three** domains. The server has **nine** vhosts: `assets`, `attendance`,
`drmanojagarwal.com`, `drmanojagarwal.in`, `fit`, `followup`, `health`, `rx`, `srv1746119.hstgr.cloud`.
**Six never appeared.** A domain acme.sh holds no record for is never renewed — it simply expires,
exactly as `.com` did. **`attendance.dr-manoj.in` is the nearest, at about 22 days.**

**And no certificate-expiry check exists anywhere in the estate.** `clinic_watchdog.py` already runs
**every five minutes** and guards eleven services; it watches processes and says nothing about the
certificates those processes serve. This is **F-369's family — a gate cannot see a row that is not
there** — applied to domains rather than files.

**It would have caught this on 08-Sep**, and `drmanojagarwal.in` too, and would give thirty days'
warning on every domain instead of none. **OPEN: the check, and the six missing renewal records.**

---

### **F-388 (S236 · CLOSED · four scheduled tasks on manojz)** — a fix that was found once and never travelled to the jobs that came after it.

**A console window flashed on the owner's screen three times every fifteen minutes**, for months,
until he finally said so: *"your files keep annoyingly flashing in my and medical pc."*

**The fix already existed.** At S201, `FIX_POPUP.bat` repointed the 10-minute Marg pull at
`PULL_HIDDEN.vbs` and it has been silent ever since. **Four jobs registered later — `MargPullWatchdog`,
`MargExpectedOnCapture` and `MargSnapshotOnCapture` (every 15 minutes) and `Clinic stock nightly` —
were each pointed straight at their `.bat`.** Nobody was wrong at the time; **the pattern simply did
not travel.**

Fixed by `RUN_HIDDEN.vbs` — **generic where `PULL_HIDDEN.vbs` was hard-coded to one job** — and
`STOP_THE_FLASHING`, which **discovers** the tasks rather than being told their names (they were
registered across three sessions under four names, and `Clinic stock nightly` carries no "Marg" in
its name at all), writes the UNDO before changing anything, and **reads every task back from Windows**
rather than trusting the write. Two runs: 2 changed, then 2 more under administrator rights. **4 of 4
verified hidden**, and the watchdog's own timestamp file moved on afterwards, proving the jobs still
run.

**Two defects caught in testing, both the S208/S209 shape — a fixture that cannot contain the
defect.** A task pointing straight at `python.exe` would also flash and the first filter only looked
for `.bat`. And `[System.IO.Path]::GetFileName` follows the *platform's* separator, so on the Linux
box where this was written and checked it did not split a Windows path at all — **the filter silently
matched nothing and the test could not have said so.** Replaced with a hand-written split.

**RULE: when a fix is found for one job, ask what else has the same shape — and make the fix generic
enough that the next one cannot repeat it.** A hard-coded remedy protects exactly one victim.

---

**Next free: F-389 · D442.**


**END OF FAULT → ACTION REGISTER v2.68 — F-388 is the last minted finding. The v2.67 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S236 close: the first 544584 bytes of v2.68 are byte-identical to v2.67 (md5 c8a342b41511e11c2038d1dd3fb0532e). If you cannot see this line, you are reading a truncated copy.**

---

# S237 — 09/10-Sep-2026 — F-389 … F-411

*Twenty-three findings. Seven from the certificate and Rung-1 builds; sixteen from the staff ledger,
where August's salary could not be finalised because the advances could not be read. The ledger
findings share one shape: **machinery that was built correctly, did nothing, and said nothing.***

---

### **F-389 (S237 · CLOSED · `cert_watch.py` v1)** — fifty green checks and wrong about every live site.

The first cut passed **50 unit checks** and reported every one of seven live certificates
incorrectly. `ssl.getpeercert()` returns **`{}`** — empty, not an error — on a connection that was
not verified, so every site read as having no certificate at all. The unit tests fed it dictionaries
and could not contain the defect.

Fixed with a two-pass design: verified connection first, and the unverified DER read only when the
first is refused. **Caught by the live-shape walk and by nothing else.**

**RULE: a library call that returns an empty container on failure is indistinguishable from success
against a fixture. Walk the live shape.**

---

### **F-390 (S237 · CLOSED · `install_cert_watch.sh`)** — a red path that deleted a file it had never written.

The installer's failure path ran `rm -f $TARGET` on **any** gate failure, including gates that fire
before anything is placed. A refused install would have deleted a working file it did not put there.
Fixed with `PLACED` / `UNITS_PLACED` flags so the red path undoes only what that run did.

**RULE: a rollback must undo this run, not the world.**

---

### **F-391 (S237 · CLOSED · threshold reasoned from the wrong layer)** — 21 days, from acme.sh's 30, on a box that renews at 15.

The warning threshold was set at 21 days by reasoning from acme.sh's 30-day default. **CyberPanel
drives acme.sh here and renews at 15**, read from its own `renew.py`. The watcher would have cried
wolf on `attendance` the following morning. Retuned to 10/4.

**RULE: read the layer that actually acts, not the layer underneath it.**

---

### **F-392 (S237 · CLOSED · `sale_bill.py --locate`)** — a locator that ignored the variable it was told to follow.

`--locate` used hard-coded `/root/finance/...` roots, so with `FINANCE_DIR` set elsewhere it
reported "no exports here" — a confident, wrong, empty answer. Caught by the installer walk.
**An empty result that would be pleasant if true gets checked harder, not less (A0).**

---

### **F-393 (S237 · CLOSED · a selftest fixture)** — a fake phone number that would have tripped the F-185 gate.

A fixture contained a ten-digit dummy number (masked here — quoting it verbatim in this register would reproduce the very breach the entry describes; caught by the F-185 self-check at the S237 close and recorded as part of F-413). F-185 is enforced as *no number at all*, so the publish would
have refused the kit and the reason would have looked like a false positive. Replaced with
`"PHONE-NOT-STORED"`.

**RULE: a fixture is repository content. It obeys every repository gate.**

---

### **F-394 (S237 · CLOSED · F-300's FOURTH recurrence, and mine)** — "Refusing json", on a morning its own kit warned me about.

A `.jsonl` fixture inside a kit needs its `.gitignore` line **when the kit is built**, not when the
publish refuses. `KIT_ID.txt` carried that warning and I read it that morning. Converted to
`_proof/make_fixture.py`, proven to reproduce all 19 rows exactly.

**RULE: a fault recorded four times is not a fault, it is a missing gate. F-300 belongs in the kit
builder, not in the register.**

---

### **F-395 (S237 · CLOSED · the orthotic row scan)** — a keyword filter that missed a row by spelling.

A scan for orthotic rows missed `WALKER FOLDIND UVISON` — a typo in the item name defeated the
keyword. Corrected by listing every not-done row before writing the park command.

**RULE: when the population is small enough to list, list it. A filter over twelve rows is a way to
be wrong about twelve rows.**

---

### **F-396 (S237 · OPEN · `close_month()`)** — the close collects less than the schedules require, and says nothing.

**August was meant to take ₹13,000 from Darpan. It took ₹5,000.** Nothing on any screen, in any
file, or in the close's own CSV said so. The owner found it three weeks later by asking why the
numbers would not add up.

The mechanism is understood and is not itself a bug: the recovery budget is the head-of-waterfall
instalment, and with a ₹5,000 loan at the head an interest-free advance behind it collects nothing.
D332 §4 named this exactly and built the schedule lane to fix it. **The lane works. It had nothing
to collect (F-397).** What remains defective is the silence.

**OPEN: the close must report, by name and by rupee, every open schedule it could not satisfy.**

**RULE: a job that reports success is not evidence (F-386). It must assert what the world can see —
including what it failed to move.**

---

### **F-397 (S237 · OPEN · Darpan's ₹20,000 of 17-Aug)** — a schedule ruled, minuted, contracted, and never attached.

S191 ruled the ₹20,000 as **₹8,000 (Aug) + ₹4,000 × 3**. D332 §6 listed it as a gated data
correction. S225 added the schedule box to the entry form. **The row exists carrying
`instalment: 20000` and no schedule at all**, so the schedule lane never saw it and August collected
nothing against it.

Nothing anywhere reconciles a schedule that was *ruled* against a row that *exists*.

**OPEN: the reconciler, and a check that an advance above the ceiling carries a schedule.**

**RULE: a design signed off is not a design built, and a design built is not a design used.**

---

### **F-398 (S237 · OPEN · `create_app()` advance routes — UNREAD)** — an advance issued without its schedule may have no repair path.

The schedule box is on the **new-entry** form. `make_contra` refuses an `ADVANCE_ISSUE` that has
children — *"advance already active (recovery/interest/skip recorded)"*. So a row created without a
schedule that has since collected anything appears to have no route back.

**Not yet established.** The advance routes were named this session and never read. **This is the
first thing the next session reads**, because everything about the ₹20,000 waits on the answer.

---

### **F-399 (S237 · OPEN · a closed month has no correction door)** — three doors, all bolted.

- `make_contra`: *"system rows (interest/instalment/capitalise/skip) are never contra'd by hand."*
- `record_skip`: *"{month} was already closed with an instalment — cannot skip it now."*
- `record_defer`: *"{month} was already closed with a collection — it cannot be deferred now."*

And **`close_month()` cannot reach back**: it collects against `open_advances()` — the advances open
*today* — and D331 §3e's filter skips any advance whose `against_month` falls later than the month
being closed. Both of Darpan's tranches carry `against_month` 2026-08, so closing April would skip
them and generate nothing. **There is no as-at-date replay in the engine.**

The only instrument left is an OTHER adjustment, which moves salary money and not a balance — the
wrong instrument for a wrong balance.

**OPEN: read `approve_salary()` for an actual-paid facility; if absent, that is the highest-value
addition in the staff-money system.**

---

### **F-400 (S237 · OPEN · no duplicate detection at entry)** — three duplicate rows in one month, found by a human a fortnight later.

Shivani's ₹1,100 was entered **three times on 31 August**. Surendra's ₹3,000 was entered **twice on
25 August**. Same staff, same amount, same day, no warning at entry and none at approval. The owner
found them by reading a statement in September, and they were the reason he could not close August.

**OPEN: warn on same staff + same amount + same date. Smallest change on the S237 list and it
removes a whole class of the owner's reading work.**

---

### **F-401 (S237 · OPEN · `contra_of` carries two opposite meanings)** — the reason the raw ledger is unreadable.

On a contra row, `contra_of` names **the row being reversed**. On a system row it names **the advance
the row belongs to**. Both are "the row this points at" and they mean opposite things.

Measured on the real file: **ten rows carrying `contra_of` cancelled only four entries.** The other
six were instalments and interest. Any reader — human or tool — must disambiguate by category and
amount before the ledger says anything true. `ledger_statement.py` had to be given an `is_reversal()`
that checks category and equal-and-opposite amount before it could produce a readable page.

**OPEN, and largest. The statement rewrite (F-409) makes the ledger readable meanwhile.**

**RULE: one field, one meaning. A field that means two things is a bug with a deferred trigger.**

---

### **F-402 (S237 · OPEN · the ceiling is set where every real advance breaks it)** — the exception is the normal path.

Darpan's ceiling is **₹10,000**; August held **₹35,000**. So every advance he takes is SPECIAL:
two steps instead of one, PENDING even for a checker, and an "application owed" flag standing on
almost every entry the owner makes.

**The gate is working correctly. The number is wrong for the traffic.** A settings change
(`advance_pct.json`), not a build — but it is friction he meets every single time.

---

### **F-403 (S237 · OPEN · four copies of the ledger app, and the obvious one is a fossil)** — this cost the session three wrong answers.

| path | md5 | what |
|---|---|---|
| `/root/staff_ledger.py` | `80257711…` | **live**, matches the pin, v3.6-S225-LOANS-D374 |
| `/root/deploy/repo/deploy_kits/S225_LOANS_VIEW/staff_ledger.py` | `80257711…` | its source, correct |
| `/root/deploy/repo/staff_ledger/staff_ledger.py` | `90de4cf1…` | **v3.0 fossil — and this is the copy on the owner's PC** |
| `/root/wa/staff_ledger.py` | `06bf03cb…` | v3.1 stray in the venv folder |
| `/tmp/s225loans_probe/staff_ledger.py` | `80257711…` | probe leftover |

Anyone opening the folder **named** `staff_ledger/` gets code from S155/S156. The service's own
`ExecStart` is the only authority on which file runs.

**OPEN: remove or repoint `/root/deploy/repo/staff_ledger/` and the `/root/wa/` stray.**

---

### **F-404 (S237 · OPEN · the file's own history stops six revisions short)** — a version log that lies by omission.

`APP_VERSION = "3.6-S225-LOANS-D374"` while the module docstring's version history **ends at v3.0**.
Everything from the ceiling through the signed-application upload to DEFER and the capacity rule
went in without a line in the file's own record. **A reader who trusts the docstring concludes the
features were never built — which is exactly what happened (F-411).**

---

### **F-405 (S237 · OPEN · the PC clone is missing kits the VPS clone has)** — `deploy_kits/S225_LOANS_VIEW/` is on the box and absent from `D:\dr-manoj-git\`.

Cause not established. **Settle it before the next publish**, because a publish from a clone that is
behind is how a deletion travels.

---

### **F-406 (S237 · OPEN · the August close ran eleven days early)** — 20 August, and it cannot be re-run.

`approved_adjustments_2026-08.csv` is stamped **20-Aug 00:19 IST**. `close_month()` is idempotent and
refuses a re-close. So **everything entered after the 20th is stranded**: Surendra's advances of
25-Aug, Shivani's of 31-Aug, Darpan's ₹15,000. For advances the practical effect is nil —
`ADVANCE_ISSUE` is in `SALARY_EXCLUDED` and never touched salary — but anything else entered after
the 20th is displaced into whichever month closes next.

Only two closes have ever run: **2026-07** (7-Aug) and **2026-08** (20-Aug). April and June were
never closed at all.

**OPEN: nothing should be able to close a month before that month has ended.**

---

### **F-407 (S237 · OPEN · no salary month has ever been locked)** — the salary side has never completed one cycle.

There is **no `salary_final_*.csv` on the box for any month**. The APPROVE & LOCK path built at S156
(D259) has never been exercised on a real month. Good news for August — nothing is frozen — and a
standing risk, because a path that has never run is a path nobody has proved.

**OPEN: August 2026 is the first real lock. Treat it as a first run, not a routine one.**

---

### **F-408 (S237 · OPEN · a SPECIAL advance requires no narration)** — the row that most needs a reason is the one that does not ask for one.

`narr_req` is **false** for `ADVANCE_ISSUE`. So an above-ceiling advance — the class that by policy
requires a signed written application — saves with an empty explanation. **Darpan's ₹14,000 went in
with a blank narration** and reads as a bare number on the statement forever.

**OPEN: narration compulsory when `special` is set.**

---

### **F-409 (S237 · OPEN · the statement page is unreadable to its own subject)** — measured on Darpan's twenty-one rows.

Six rows that cancel each other are shown in full · both migrated tranches are dated 7 August so
₹363,000 of opening balance reads as August advances · machine-written rows carry row ids and
decision numbers (*"auto instalment for loan b1eb7a8e… (balance after: 175000)"*, *"CONTRA of
540acd6b… (D332 S2.3)"*) · two school-expense perks sit in the same column as advances and read as
₹19,000 of debt · every month shows at once with mixed date shapes sorting oddly.

**The fix belongs at the display layer. Stored narrations are the audit trail and are never
rewritten** — the page knows the category and the parent advance and can say what a row *is*, with
the id kept underneath.

---

### **F-410 (S237 · OPEN · three rows dated after the day the money moved)** — the narration argues with its own row.

`ADVANCE_ISSUE 5000` dated **18-08** narrating *"cash given 17 Aug"*. `ADVANCE_ISSUE 15000` dated
**19-08** narrating *"given 17 Aug"*. Their contras inherit both wrong dates. A statement that has to
be read past its own date column is not a record, it is a puzzle.

**OPEN: on the reconciler's list.**

---

### **F-411 (S237 · CLOSED as a rule · MINE, and it cost three wrong answers in a row)** — I reasoned from a file without hashing it against the manifest.

I read `D:\dr-manoj-git\drmanoj-clinic-automation\staff_ledger\staff_ledger.py`, found a v3.0
docstring and no ceiling, no `special`, no application upload, no DEFER — and told the owner **three
times** that features he uses every day "were never built". The manifest pins the live app at
`80257711…`; that file hashes `90de4cf1…`. **It was the fossil (F-403), and the manifest was open in
front of me.**

Then I compounded it twice: I gave a path for the live app that I had guessed
(`/root/staff_ledger/staff_ledger.py` — it is `/root/staff_ledger.py`), and my `find` took `head -1`
and I reported the first hit as live when the authority is the service's own `ExecStart`.

The owner corrected all three from his own knowledge and from one paste each.

**RULE, and it is the S237 rule: hash a file against `CANONICAL_MANIFEST.md` before reasoning from
it. Every time. There is no exception for "the obviously named folder" — that is precisely where the
fossil lives. This is F-116's family: never write a hash that came from anything but a file, extended
to — never draw a conclusion from a file you have not hashed.**

---

### **F-412 (S237 · OPEN · the Fault Register has had no manifest row since v2.47)** — the document that records missing rows is itself a missing row.

Found while updating `CANONICAL_MANIFEST.md` at this very close. The manifest's live row for this
file reads **v2.47 (S219)**. **Eighteen versions have shipped since**, every one of them canon, none
of them manifested. Phase 0 verifies every manifest row by md5 and has therefore been verifying a
Fault Register that stopped being current in early September — **and reporting green.**

This is **F-184's shape exactly** (`KB_canon_all` holding files nobody listed) and **F-369's**
(a gate says nothing about a row that is not there), landing on the register that records both.

**Fixed at this close**: a CURRENT row for v2.69 added, and the eighteen-version gap recorded in the
row itself rather than quietly closed.

**RULE: a close that bumps a document's version and not its manifest row has not finished. The
manifest is updated LAST and it is updated for EVERY document the close touched — including the ones
that are only ever appended to.**

---

### **F-413 (S237 · OPEN · REAL NUMBERS ARE IN THE KB HISTORY ARCHIVE, IN GIT)** — F-185's own rule, breached in canon, found by running the check on my own work.

**Found at the S237 close**, running the F-185 gate by hand over everything this close touched —
which for the first time included the whole inherited Archive, because v1.84 is a new file and a new
file is staged whole.

**Five ten-digit numbers that do not look like test values, and one of them carries a patient name**,
sit in `KB_History_Archive` and have been carried forward, version after version, into the
repository:

| Archive line | what is there |
|---|---|
| ~2548 | a number **with a patient name and a clinic ID beside it** |
| ~2548 | a second number on the same line |
| ~3669–3670 | two numbers, in a classifier's first production run, with call times and durations |
| ~3697 | a fourth number, in a verdict record |

*(A fifth value, `98…78`, appears at lines 1517, 1518, 1732 and 2545 and is plainly a **dummy** —
the sequential test value used to document a JavaScript quoting defect. It is not a breach.)*

**F-185 is enforced as "no number at all."** These are numbers, in the repository, in the document
that is the project's own history. They predate this session by many versions; **v1.84 carries them,
and I am the one publishing v1.84.**

**Why this was not caught before:** the phone gate runs on **staged** files. Once the Archive is
committed, later closes append to it and the gate sees the whole file each time — so it either has
never flagged these, or the refusal was read as noise. Either answer is worse than the other.

**What was NOT done, deliberately.** They are not masked. Masking text inside the inherited prefix
would break the one property the Archive is built on — **pure append, proven byte-identical to the
previous version** — and that is exactly the shape of F-23, the document that claimed to carry a
version forward unchanged and silently dropped sixteen lines. **Repairing published history is a
ruling, not a tidy-up.**

**OPEN, and it needs the owner's word:**

1. **Mask in place**, accepting that the append proof is broken once and recording the break loudly
   — the honest option, and the one F-185's own `--fix` was written for; or
2. **Freeze the Archive** at v1.84, mask a v2.0 forward, and keep v1.84 as the last provable append.

**RULE, earned tonight: run the privacy gate over your own output before you publish it — including
the parts you only appended to. I found my own breach in the very entry describing that breach
(F-393), and the inherited five only because I looked at the whole file rather than my diff.**

---

**Next free: F-414 · D447.**


**END OF FAULT → ACTION REGISTER v2.69 — F-413 is the last minted finding. The v2.68 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S237 close: the first 553282 bytes of v2.69 are byte-identical to v2.68 (md5 315715ed96a7d8dae643e1d421626dd8). If you cannot see this line, you are reading a truncated copy.**


---

## §S238 — 10 and 11-Sep-2026 · SIX FINDINGS, THREE OF THEM THE ASSISTANT'S

### **F-414 (S238 · CLOSED IN PRACTICE · ASSISTANT'S OWN) — A PROBE COPY OF A `BASE`-RELATIVE MODULE READS NO SETTINGS AND NO HOLDS, AND REPORTS A CHANGE THAT IS NOT THERE**

`salary_policy.py` sets `BASE = dirname(__file__)`; its settings file, settings audit and hold ledger
are all derived from it. Both S238 probes loaded a **copy** from the probe folder, so the copy read
defaults and no holds, and the "net NEW" column showed every staff member's net changed. **Nothing had
changed.** The first explanation given (in-process state) was wrong. Also mine in the same probe: a
`%` in the label "30%->20%" crashed the `%`-format string.
**Fix, standing:** after loading a copy, re-point `M.BASE`, `M.SETTINGS_PATH`, `M.SETTINGS_AUDIT`,
`M.HOLD_LEDGER` (every BASE-derived path) at `/root/staff_register`, and restrict which diffs a probe
may report. **RULE: a probe that differs from live must first prove it read the same inputs.**

### **F-415 (S238 · CLOSED) — A CRON REDIRECT INTO A MISSING DIRECTORY KILLS THE COMMAND SILENTLY**

The S223 Docterz line `… >> /root/finance/logs/docterz_ingest.log 2>&1` pointed into a directory that
did not exist. The shell cannot open the redirect target, so **the command never runs and nothing is
logged anywhere** — not the log, not cron's mail. The Day Revenue screen stood still from 03-Sep to
11-Sep. Fixed by `S238_DOCTERZ_LIVE` (`mkdir`, catch-up to 10-Sep, 73 days).
**RULE: an installer that writes a cron line with a redirect creates the target directory and proves
one run landed in the log before it reports GREEN.** Candidate guard: a freshness leg on the log file
itself (see the Day Revenue leg, 200 h → 26 h, still owed).

### **F-416 (S238 · CLOSED) — "NOTHING TO READ" IS NOT "FAILED"**

The catch-up exited 1 because June workbooks carry no Day Revenue sheet; `S238_DOCTERZ_LIVE` rev 1
therefore refused to schedule. Rev 2 accepts exactly that reason and no other. **RULE: an exit code
that conflates an absent input with a failure makes every gate downstream wrong in one direction or the
other — name the benign reason and accept only it.**

### **F-417 (S238 · CLOSED) — A RECORD THAT NAMES LEDGER ROWS BY ID CANNOT SURVIVE A REVERSED-AND-RE-ENTERED ROW**

`S238_RECONCILE_AUG2` went RED on the box, safely, nothing written: Sukhveer's advance `…ca19` had been
reversed and re-entered after the walked copy was taken. Reconciler v1.1 adds the `month_advances`
kind — find the person's month advances at run time — proven on four ledger shapes.
**RULE: a reconciler states intent (whose advances, which month), not row ids; the ledger moves under it.**

### **F-418 (S238 · CLOSED · ASSISTANT'S OWN) — DOUBLE HTML ESCAPING IN A SHARED HEADER**

The printed money sheets showed `&amp;` in the clinic name: `_clinic_hdr` escaped a string its caller
had already escaped. Fixed in `S238_SHEETS_V4`. **RULE: escape once, at the boundary; a shared helper
states in its name whether it expects raw or escaped text.**

### **F-419 (S238 · OPEN) — DOCTERZ DAY REVENUE: 07-SEP READS ₹0 WITH NO LINES, 08-SEP HAS NO ROW**

After the catch-up. Either the tracker produced an empty sheet for 07-Sep and none for 08-Sep, or the
reader skipped them. **Put to the owner at the S238 close; to be read from the Drive folder's own files
at the S239 open before anything is changed.**

**Also recorded, not minted:** the assistant twice produced syntax errors by nesting `'''` inside a
heredoc Python patch; fixed by writing additions to temp files and appending them. And the owner once
ran a Windows `.bat` path in the VPS shell, then glued two commands into one paste — **both are the
hand-over's fault, not his: one line per copy block, the machine named above it.**

---

**Next free: F-420 · D455.**


**END OF FAULT → ACTION REGISTER v2.70 — F-419 is the last minted finding. The v2.69 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S238 close: the first 572901 bytes of v2.70 are byte-identical to v2.69 (md5 3299aafb03ea249e413fdf046954c7e5). If you cannot see this line, you are reading a truncated copy.**


---

## §S239 — 11-Sep-2026 — F-420 … F-428 MINTED · F-419, F-242 AND AF-12 CLOSED

*v2.71, Session 239 CLOSE, 11 September 2026 (IST). Nine findings: four the assistant's own (F-422,
F-426, F-427, F-428), two found in live data by the new pickup (F-420, F-423), one the owner's report
(F-421), two open for later work or his ruling (F-424, F-425). Pure append: the first 577,145 bytes
are byte-identical to v2.70.*

### **F-419 (S238) — CLOSED at S239.** 06-Sep was a Sunday; 07-Sep ₹0 is correct (the owner did not
work — bookings only); 08-Sep was re-exported by the owner at 08:08 IST on 11-Sep (₹22,200: cash
₹16,100 + online ₹6,100) and the reader dates by the sheet's own header. Nothing was wrong in code.

### **F-242 (S209) — CLOSED at S239 by `S239_PORTAL_F242`** (`/root/portal/portal.py` `7bc59115…` →
`ed558b3663c3dd100a24f58aafc32363`, backup `portal.py.bak_S239_F242_20260911_113255`, read back GREEN).
The trap was worse than recorded at S209: **every broker login planted the ten-year device cookie**,
and "Sign out everywhere" sat on every staff home screen, so one staff tap could lock the whole clinic
into the loop (Amir lost a day's entries). Fix: the login page always shows the form and clears the
device cookie; logins no longer plant it; in broker mode only the SSO token is a login; `/portal/logout`
exists; forget-devices and sign-out-everywhere are doctor-only (403 for others). **The S209 RULE stood
and was honoured: both doors were changed together, and the case-pack and WhatsApp doors with them.**

### **AF-12 (audit) — CLOSED at S239** by the same kit: the device cookie no longer opens the patient
case pack or the clinic WhatsApp sender (the F-98 fix completed on the last two doors).

### **F-420 (S239 · CLOSED) — `Day_Tenders.csv` HELD 18 DUPLICATE LEGS: NINE BILLS DOUBLED**
The S223 file on the PC, which the VPS ingest replaces its table from, carried the April–June backfill
export overlapping the June dailies plus same-day re-exports. Those days' split blocks showed doubled
tender legs. Rebuilt by the pickup, **newest export wins per invoice**: 194 legs, 64 days, 0 duplicates.
**RULE: a file assembled from overlapping exports needs a key and a winner, stated in code.**

### **F-421 (S239 · CLOSED) — THE CALL LIST WAS "THE WORKBOOK WRITTEN LAST", NOT "THE LATEST DAY"**
Both pushers (`push_followups_today.py` on the PC, `/root/wa/push_followups_vps.py`) chose
`max(mtime)`. The owner re-ran a missed 08-Sep on 11-Sep and reception was shown 08-Sep's calls.
Fixed: list made current at 09:10 IST (`--refresh-latest`), then both pickers choose by the date in
the name (PC `7693a29a…` → `74122185…`; VPS patched by the owner's line, backup `.bak_S239`; **its new
md5 is to be read at the S240 open**). **RULE: "latest" means the business date the file is about, never
when it was written.**

### **F-422 (S239 · CLOSED · ASSISTANT'S OWN) — A "TOTAL ROW" STOP DROPPED A REAL PATIENT**
The first pickup cut stopped the Day Revenue parse at the first row reading like a total. In the full
April–June export a real patient sits below such a row; one patient was lost in the offline comparison.
Replaced by the UID-shape guard (F-93's own rule): 175 phantom rows removed across 85 exports, 0 real
patients lost, bills unchanged. **RULE: a parser keeps rows by what they are, not by where it thinks the
table ends.**

### **F-423 (S239 · OPEN, MITIGATED) — `Staff_Action_Today_<date>` IS NAMED FROM THE LATEST DATE IT
MENTIONS.** max(consult date, follow-up due dates) — so the 07-Sep run and an 08-Sep re-export both
wrote `…_2026-09-08.xlsx`, and the second replaced the first in Drive. Mitigated: the pickup copies any
workbook before a run could overwrite it (`_outputs_before_run\`). The tracker's naming is not changed
(live, not rebuilt without the owner's OK).

### **F-424 (S239 · OPEN — the owner's ruling) — THE TRACKER'S `data\` FOLDER CARRIES A
`.tmp.driveupload` MARKER.** Drive for Desktop appears to sync the tracker's patient data, against the
tracker's own rule that PHI never goes to Google. Not touched.

### **F-425 (S239 · OPEN) — THE DIAGNOSIS SEEDER READ 0 ROWS AND SAID "REFRESHED"**
On 10-Sep 08:44 the owner fed the clinical data report to `/diagnosis`; `seed_diagnosis.detect_format()`
does not know that shape, so 0 diagnoses were seeded and the page reported success. Owed with the Ask 2
build (adapter + a refusal when a file yields 0 rows). **RULE: zero rows from a non-empty file is a
failure, never a refresh.**

### **F-426 (S239 · CLOSED · ASSISTANT'S OWN) — A PREVIEW NOT RENDERED FROM THE LIVE GRANTS**
The first staff-app preview showed the pre-change screens (the owner: *"Your preview HTML is not
updated"*); the regenerated one missed that Shavez is role *manager* with one more tile. **RULE: a
preview of a permission change is rendered from the grants file and the role table it will install,
never from a description of them.**

### **F-427 (S239 · CLOSED · ASSISTANT'S OWN) — GRANTS WRITTEN AGAINST GUESSED LOGIN NAMES**
The v9 hold used pravesh / sandip / vikki; the logins are parvesh / sandeep / vikky. The installer's own
read-back listed them and flagged the three; v10 fixed them by one owner line. **RULE: read the login
list off the box before writing a per-user rule.**

### **F-428 (S239 · ASSISTANT'S OWN) — "AUGUST IS READY TO LOCK" WAS SAID TWICE AND WAS WRONG**
Said at the S238 close and again after the S239 Money-page kits. The owner: *"not ready for lock … the
attendance verifier from physical sheet going into the staff today … their approvals … corrections …
then the salary sheet lock."* **RULE: "ready" names the next human step, not the state of the code.**
Now D460.

**Also recorded, not minted:** the device shell mounted nothing all session (Windows update of 8-Sep);
every file moved by the bridge and was verified by md5 after staging back. `project_write` timed out
twice and succeeded on retry.

---

**Next free: F-429 · D463.**


**END OF FAULT → ACTION REGISTER v2.71 — F-428 is the last minted finding. The v2.70 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S239 close: the first 577145 bytes of v2.71 are byte-identical to v2.70 (md5 9ff7c11c6512f7f3064174dbe49dc647). If you cannot see this line, you are reading a truncated copy.**

---

## §S240 — 11 and 12-Sep-2026 · F-429 … F-438

### **F-429 (S240) — AN EXPORT WAS REFUSED AS TRUNCATED BECAUSE MARG CHANGED ITS FOOTER**
`PURCHASE_ITEMWISE` began failing the completeness check on 09-Sep: Marg had changed the advertisement
line in its footer, and the router's end-marker rule was reading that line as the proof the file was
whole. Nothing was wrong with the data. The rule is now a **row-matched GRAND TOTAL** — the file is whole
when its own total agrees with its own rows. **RULE: prove a file complete by its content, never by a
string the vendor is free to change.** Fixed in `S240_MANOJZ_FIXES`.

### **F-430 (S240) — THE DOCTERZ PICKUP WENT SILENT AT 11:55 BECAUSE THE LAPTOP WAS ON BATTERY**
A task created with `schtasks` defaults to **AC-power-only**, so the pickup simply did not run once the
machine was unplugged. This is **F-314 repeating** on a different task. The flag is cleared in
`REGISTER_DOCTERZ_PICKUP.bat` (f2ae1169). **RULE: every `schtasks` task this project creates clears the
battery flag in the same breath, and the close checks it.**

### **F-431 (S240) — A RUNNING PULL WAS REPORTED ASLEEP EVERY NIGHT**
The purchases hub read the pull's START time and, finding it old, said *manojz pull asleep* — while the
pull was running. A monitor that cries wolf nightly is a monitor nobody reads (the S202 lesson, again).
**RULE: liveness is proven by the last COMPLETION, never by the last start.**

### **F-432 (S240 · ASSISTANT'S OWN) — A KIT FILE WAS SILENTLY EXCLUDED BY `.gitignore`**
The session's first publish refused: `legs_add_s240.json` had never been committed because the
repository's `.gitignore` carries a blanket `*.json`. The kit's own `SUMS.md5` listed the file, so from
inside the kit everything looked right. **RULE adopted as a close step: run `git check-ignore` over every
new kit file before naming a publish.** `.gitignore` 5ddfea0f → 911345d9.

### **F-433 (S240) — `_newest_snapshot` CAN RETURN OUR OWN COMPUTED PUSH**
The newest stock "snapshot" is not necessarily Marg's: a day written only by our own computed push has
the same shape and wins the sort. Same family as **F-350**. **RULE: a comparison is only a comparison
when both sides have independent provenance — stamp the source and filter on it.**

### **F-434 (S240 · ASSISTANT'S OWN, WITHDRAWN IN PUBLIC) — A BILL WAS USED TO EXPLAIN A COUNT IT DID NOT EXPLAIN**
RAVI bill 7617 was offered as the explanation for the DECA INSTABOLIN 50 difference. It was not. The
owner's own knowledge of the bill is what caught it. **The +5 remains unexplained and is recorded as
unexplained.** **RULE: an explanation that merely fits the number is a hypothesis; say so, and never
close a count on one.**

### **F-435 (S240 · FIXED THE SAME HOUR) — THE SALARY SHEET'S FOOTNOTE SAID ÷30 WHILE THE ENGINE DIVIDES BY 30.5**
The owner's suspicion — *"the leave ded amt is not looking correct"* — was right about the page and wrong
about the money. **Every rupee was correct**; the printed sentence under Sheet 3 was not, and the rows
that looked odd were Sunday-weighted absences (D340/D341) behaving exactly as designed. The footnote now
prints the live divisor (`%g` of `day_divisor`, D343) and cannot go stale again.
`salary_policy.py` c7577174 → aabd90fb (`S240_SALARY_NOTE`). **RULE: a number and the sentence that
explains it are the same deliverable — a wrong sentence is a wrong sheet.**

### **F-436 (S240 · OPEN, PARKED AT THE OWNER'S WORD) — MARG'S PURCHASE SERIAL IS IN NO EXPORT**
Marg gives every purchase entry a serial in chronological order for the financial year, and the owner
writes it on the bill and in the physical purchase register. **It is in no export and in no table:**
`purchase_bill.bill_no` is the SUPPLIER's invoice number and repeats across suppliers. The serial our
pages show is therefore the keying order we infer, not Marg's number. Capturing it is a data-entry
question (Amir types it against each bill), not a parsing one. Parked until the salary work closed.

### **F-437 (S240 · ASSISTANT'S OWN) — A SCRIPT RUN BY ITS ABSOLUTE PATH PUTS ITS OWN FOLDER ON `sys.path`, NOT THE CWD**
`S240_SALARY_NOTE`'s first live run went RED on `ModuleNotFoundError: No module named 'salary_policy'`
while sitting in the right folder, because python had put the KIT's folder on the import path, not the
one the installer had `cd`'d into. Fixed by passing the directory as `argv[1]` and inserting it
explicitly. The rollback worked; nothing was left changed. **RULE: a helper that must import the live
module is told where the live module is — never left to infer it from where it was launched.**

### **F-438 (S240 · ASSISTANT'S OWN) — A COMPARATOR ASSERTED MORE THAN THE CHANGE PROMISED**
`S240_DARPAN_OWN_SHEET`'s first live run reported NOT PROVED because the comparator demanded the name
appear **nowhere** on the page. It appears twice in notes outside the tables — which was never what the
change was about. Corrected to assert on the ROWS and the MONEY (no Sheet 3 row, no Sheet 4 row, every
total down by exactly his own figure) and to report remaining mentions as a count. **RULE: a proof
asserts exactly what the change claims — no more, or a correct change is rolled back by its own guard.**
The rollback again worked correctly.

**Also recorded, not minted:** the device shell mounted nothing for the third session running (Windows
update of 8-Sep); every file moved through the bridge and was verified by md5 after staging back. A Drive
upload of `marg_push.py` was corrupted by the assistant typing `&lt;&lt;` for `<<` — caught by an exact
byte count, fixed by removing the shift, and confirmed live by the heartbeat's md5. The owner twice
recorded that the assistant's output was too long and too demanding (*"you are getting too demanding and
very confusing for me… I want a step by flow"*; *"please ask only what is compulsory"*) — answered by
drawing the screens instead of describing them.

---

**Next free: F-439 · D478.**

**END OF FAULT → ACTION REGISTER v2.72 — F-438 is the last minted finding. The v2.71 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S240 close: the first 583551 bytes of v2.72 are byte-identical to v2.71 (md5 4926959a787a3dcfb6b8bff45e500da4). If you cannot see this line, you are reading a truncated copy.**


---

## §S241 — 12-Sep-2026 — five findings

**F-439 — THE CLINIC AGENT RAN ONLY WHILE THE OWNER WAS SIGNED IN.**
The medical PC's own heartbeat names the account it runs in: `user: SET` — the owner's login, reached by
RDP, not the account the counter staff use. The agent starts from that profile's Startup folder, so
capture lived and died with his sessions. Measured on 12-Sep: last heartbeat 11:08, six hours dark, back
at 17:09 only because he logged in. **Captures that day to that point: zero** — an item-wise sale report
exported that morning never reached the server and had to be taken again. Found by reading the heartbeat
rather than by asking. **Fixed** by signing the machine in at power-on under SET (D480) and proven by a
restart with nobody connected: agent up on its own at 17:31:28. **RULE: an agent's real trigger is the
account it starts in, not the folder it starts from — read the heartbeat's `user` before believing any
claim about when it runs.**

**F-440 — THE POWER-ON FIX TOOK THE COUNTER'S OWN ACCOUNT OFF THE SIGN-IN SCREEN.**
With auto-logon pointing at SET, the PC no longer offered the staff account at startup, and for part of
the evening Darpan served the counter inside the owner's own login while sales ran. Nothing was deleted or
disabled — the account and its files were untouched — but nobody had been told that the counter would need
to switch users afterwards, and that omission was the assistant's. The vendor restored it. **The standing
answer is Ctrl+Alt+Del → Switch user:** SET stays signed in and keeps the agent alive while the staff work
in their own account. Verified afterwards: agent alive, all five kit files matching, two captures that day,
the newest already on the server. **RULE: a change that alters which account a shared machine lands in is
a change to how the people at it work — say so at the time, not after.**

**F-441 — THE SALTS PAGE COMPARES TICKS AGAINST A LIST OLDER THAN THE TICKS.**
Read live: 60 of 113 ticked, and beside them a "Marg says" column read from Marg's own SALT WISE ITEM LIST
**exported 04-Sep** — two days before the 06-Sep ticks. It therefore reports *not yet* against work that
may well be done, and can confirm nothing done after the 4th. Not a code fault: the comparison is correct
and its input is stale. **One fresh export settles 38 rows at once.** **RULE: a column that judges work
must show the age of what it judges by, or it will be read as a verdict when it is only an old photograph.**

**F-442 — GOOGLE DRIVE'S LETTER ON THE MEDICAL PC MOVED `F:` → `G:` ACROSS A REBOOT.**
The agent found it either way — it resolves the path rather than assuming a letter — and the heartbeat,
the kit check and the offsite backup all continued. But every instruction and every paper that hard-codes
`F:\My Drive\...` for that machine is now wrong as often as it is right, including the one handed over
earlier the same day. **RULE: never hard-code a Drive-for-Desktop letter in anything a person will be
asked to open.**

**F-443 — A FOURTH CONSECUTIVE CLOSE RAN WITHOUT A DEVICE SHELL.**
The Windows update of 8-Sep still prevents the workspace mounting the connected folders on manojz, so this
close could not run `gen_live_pins.py`, could not rebuild `MD5SUMS_ALL.txt`, and could not verify the
manifest row-by-row by md5. The file-transfer tools read and write those folders perfectly, so every
document below was still written, and every kit was still hashed and gated — but **three rows of the A17
checklist are BLOCKED by this and are recorded as such rather than passed over.** They are carried into
`OWNER_TODO_LIVE`. **RULE: a step that could not run says so by name; silence is never DONE.**

---

**Next free: F-444 · D481.**

**END OF FAULT → ACTION REGISTER v2.73 — F-443 is the last minted finding. The v2.72 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S241 close: the first 589959 bytes of v2.73 are byte-identical to v2.72 (md5 f0ed6c0772307caf4e9b88c143456978). If you cannot see this line, you are reading a truncated copy.**

---

## §S242 — 12-Sep-2026

**F-444 — THE DAILY REGISTER OPENED ON YESTERDAY, BECAUSE IT WAS READING DOCTERZ'S CALENDAR.**
The owner: *"its showing of 11 sept, shd be of today, to be used at end of day."* `register_index`
in `clinic_register.py` built its candidate days from one table, `clinic_day_revenue` — Docterz's
record of a day. Docterz sends a day AFTER it happens, so at the hour the screen is actually used
the newest row in that table is yesterday, and yesterday is where the page opened. No figure was
wrong and nothing was broken: the counter's own record was waiting on somebody else's calendar.
Fixed with one helper at two call sites; the day page needed nothing, because it already accepts any
ISO date and reads an unsent day as *not known* rather than zero. `93a31e68…` → `c6b87682…`.
**RULE: a screen that a person fills at a particular hour must take its date from the clock, not
from the newest row some other system has got around to sending.**

**F-445 — A PAGE WAS LIVE, WALKED, PINNED — AND HAD NO DOOR.**
`/finance/amir`, Amir's seven steps, went live at S241 with no tile anywhere in `portal.py`. It
could be opened only by typing the address, and it stayed that way until the owner asked a question
about something else. Found by reading the owner's live portal, which renders every tile there is.
Nobody had checked, because every gate the build did have came back green: the code was right, the
walk was right, the pin was right. **"Live" is not "reachable".** **RULE: a build for one named
person is not finished until that person's own screen has been asked what it shows — the delivery
is part of the deliverable, not follow-up.**

**F-446 — THE ASSISTANT'S OWN: AN OPTIONAL SECTION WAS REPORTED AS A MISSED STEP.**
Reading the first register entry ever made, the assistant saw the end-of-day drawer count blank,
called it something Alisha had left undone, and proposed a word to reception. The owner had told her
to leave it: the nine boxes are the record and they reconcile against the overnight report and the
bank MPR without a note being counted; the drawer count is a convenience, offered and never
demanded. Nothing was broken by the error, but it would have sent a correction to a member of staff
who had done her job exactly right. Ruled as **D484**. **RULE: before calling an empty field a
failure, ask what fills it and who required it. An empty field is a question, not a verdict** — and
this was the second time in one evening, the first being a tile withdrawn for having no entries when
the people who fill it could not see it.

**F-447 — THE ASSISTANT'S OWN, AND FOUR CLOSES OLD: ONE BLOCKER WAS GENERALISED OVER THREE STEPS.**
Since S238 the close has recorded `gen_live_pins.py`, `MD5SUMS_ALL.txt` and the manifest's
row-by-row md5 verification as BLOCKED by the dead device shell on manojz (F-443). **Only the third
ever needed that shell.** The generator is a standalone script over two files; the sums file is md5
over a folder of files; the file-transfer tools have been reading and writing those folders
faithfully the whole time — they wrote every kit of every one of those sessions. All three ran at
this close, in the workspace, and the manifest rows were verified by staging the files and hashing
them. This is S209's lesson wearing different clothes — *"does not mount" is not "unreachable"*,
applied to a shell instead of a disk. **RULE: when a step is blocked, name the RESOURCE it needs,
not the tool that failed; and re-ask that question at every close a blocker survives, because a
blocker nobody re-examines becomes a fact.**

**F-448 — THE FOLDER'S OWN HASH GATE HAS BEEN EXITING 1 SINCE THE S241 CLOSE, AND NOBODY RAN IT.**
`README_VERIFY.md` names one mechanical Phase-0 command — clone the repo, `cd deploy_kits/KB_canon_all`,
`md5sum -c MD5SUMS_ALL.txt` — and says **the command must exit 0**. Run at this close for the first
time in four sessions, it exited **1**: 484 rows OK and `CANONICAL_MANIFEST.md` **FAILED**, because
S241 appended its block to the manifest and did not rebuild the sums file (recorded BLOCKED under
F-443). The inverse check found the other half of the same omission: **three S241 canon files —
`KB_History_Archive_v1_88_S241close.md`, `Fault_Action_Register_v2_73.md`,
`KB_Register_v5_91_S241close.md` — were on disk and in no row at all**, so the gate was not merely
red, it was not even looking at the session's own canon. No row was listed-but-missing (not the
F-119 shape), and no file was wrong: nothing had drifted, the gate had simply been left holding a
stale expectation for a document that legitimately moved. **Rebuilt at this close: rows 485 → 495,
added 10, dropped 0, and the gate now exits 0 on all 495** — the ten being this close's seven
documents and the three S241 files that had never been listed. **RULE: a verification command that nobody runs
decays into a lie in one close. If a close cannot rebuild the gate, it must RUN the gate and record
the red — an unrun gate and a green gate are indistinguishable in a close report, which is exactly
what "silence is never DONE" was written for.**

---

**Next free: F-449 · D485.**

**END OF FAULT → ACTION REGISTER v2.74 — F-448 is the last minted finding. The v2.73 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S242 close: the first 594091 bytes of v2.74 are byte-identical to v2.73 (md5 45f09695fe5cdb1c67ecda049fee5e4d). If you cannot see this line, you are reading a truncated copy.**

---

## §S243 — 12/13-Sep-2026 · F-449 … F-458, three of them the assistant's

### F-449 — THE MANIFEST CARRIED TWO CURRENT ROWS PER FAMILY, AND HAD SINCE S240 (OPEN)
Found at the Phase 0 verification of `CANONICAL_MANIFEST`, before any work: the Tier 0 table still
marked the S240 artifacts **CURRENT** while the §S242 rows marked their successors **CURRENT** — two
answers to "which Register is current" in one file, and a reader who stopped at the first table got
the wrong one. The F-384 family, which found the same shape at S236 for two Register rows. **Ten rows
demoted visibly at the S243 open**, the manifest moving `82bacfc3…` → `2e862b6a…` with a §S243 OPEN
block; `MD5SUMS_ALL` rebuilt to 495 rows, exit 0. Stays OPEN until the close routine carries the check.
**RULE: for every family exactly one row reads CURRENT — a one-line grep for the close routine, run
before the manifest is published, not after a reader trips over it.**

### F-450 — THE PIN LIST AND THE REGISTER PINNED `finance_app.py` AT A HASH THE RECORD'S OWN S241 BLOCK CONTRADICTS
`live_pins_S242close.txt` row 21 and the Register's consolidated table both pinned
`/root/finance/finance_app.py` at `81db4854…` (8-char, "read back at install", S240) while the record's
own S241 block says `72bc8323…` — the S241 kits moved the file and the table row was never advanced
(S241 recorded, not silent, that the table was not edited row by row). Confirmed on the box at the
AUTOAPPLY install: the live file read `72bc8323…`, as the S241 block said.
The F-369 family — a live file whose pin the record holds wrong is, for the installer, an unpinned
file. Corrected in the Register at this close (the S243 wave row names the correction). **RULE: when a
session records that a table was not edited, the next close's first job is to edit it — a "recorded,
not silent" deferral is a row owed, not a row closed.**

### F-451 — A CRON LINE A KIT DECLARED WAS NEVER IN THE CRONTAB
Kit `S223_UPI_MATCH` declared `40 9 * * * clinic_upi_check.py --days 60 --write`. The crontab read at
the S243 study has no such line; the file is present on the box. Whether the line was never installed
or was lost is not recoverable from the record. Owed: install it or retire `clinic_upi_check` (the CA
ruling D492 may make the second the right answer). **RULE: a crontab claim in a kit is a row to verify
against `crontab -l`, not a fact — the estate register carries the crontab, and every kit's declared
line is checked against it at the next open.**

### F-452 — `stock_snapshot` HAD NO SOURCE COLUMN, AND THE COMPUTED FEED OVERWROTE MARG'S CLOSING FIGURES
`stock_snapshot` was keyed (as_on, item) and nothing else: Marg's closing figure and the server's
computed expected figure landed on the same row, last writer wins. The computed feed won on 05, 11 and
12-Sep — **373 items each day** — so the snapshot that the count gate reads as "what Marg says" was in
fact "what we computed", and a real difference between the two could auto-close itself with no one
looking. Found while reading the stock tables for the study, not by any gate. Fixed by
`S243_SNAPSHOT_SOURCE` and its migration (D490): the snapshot holds Marg's figures only, the computed
figure has its own table `stock_expected`; **1,119 Marg rows restored on three days, 2,245 computed
rows moved**; database backup `finance.db.bak_S243_SNAPSHOT_20260913_084756`. **RULE: two writers,
two columns or two tables — a figure that can be overwritten must carry who wrote it, and a check that
compares A to B must never read A and B from the same cell.**

### F-453 — A MANOJZ-ONLY SCRIPT PINNED AT A VPS PATH
`/root/finance/push_purchases.py` is pinned in the Register's S240 wave at a VPS path though it runs
only on manojz; the VPS has no such job and the pin describes a file where it is not used. Pin
correction owed with F-450 and F-454. **RULE: a pin names the machine that runs the file — a path is
not a location until the machine is in front of it.**

### F-454 — THE WATCHDOG'S PIN WAS TWELVE SESSIONS STALE, AND THE INSTALLER REFUSED CORRECTLY
The pin list's `/root/wa/clinic_watchdog.py` row read `01ca6591…` — the S204 capture — while the live
file was `389afcfe…`: the S231 ntfy change (the hard-coded default removed, the topic read from the env
file) was recorded in prose and never re-pinned. `S243_WATCHDOG_FINANCE` was built against the pinned
bytes, and its installer **refused its first run on the md5 guard** — the correct outcome, and the
reason the guard exists. Rebased on the repository's followup-vps copy, which matched the live file,
and installed: now `35e40626…`, fourteen units watched. **RULE: a patch recorded in prose ("patched —
default removed") without a re-pin leaves the pin list lying; every install, including a sed line,
ends with the hash read back and the row moved.**

### F-455 — AMIR'S STEP 4 COMPARED A STAMP TO A DAY AND COULD NEVER MATCH
`amir_day.py` compared `substr(export_stamp,1,10)` to an ISO day (`YYYY-MM-DD`) while the ingest door
stores its stamp as `YYYYMMDD-HHMMSS`; the first ten characters of one are never equal to the other.
So Amir's step 4 could never see a real export, and today's bills always read *pichhla baaki* —
yesterday's remainder — however fresh the export. Live since S241, found by reading the code against
the door's own format during the study; the owner informed. Fixed in `S243_AMIR_VISIT`
(`ae2c8939…` → `bf7d9826…`). **RULE: before comparing two stamps, print one of each from their real
sources side by side — a walk that fabricates both sides of a comparison in the same format proves
nothing about the join.**

### F-456 — THE ASSISTANT'S OWN: THE FIRST CODE BUNDLE SHIPPED THE SETTINGS FILES, SECRETS AND ALL, TO DRIVE
`code_bundle.py` v1.0, the nightly code copy, ran at 07:19 and shipped `/root/portal/portal_config.py`
and `/root/att_config.py` — a live token seed, the SSO secret, the MyOperator token, the SMTP password —
to Drive. Found on inspection of the bundle's own listing, not by any gate; nothing had been asked to
look. v1.2 excludes settings files by name and applies the repository gate's own secret heuristic to
everything else before it is packed; the Drive copy was trashed and the slot recreated. **Rotation of
the exposed keys is OWED**, together with the MyOperator rotation already open. The F-365 family —
the same estate, a different vehicle. **RULE: a backup that copies configuration copies secrets; the
exclusion list and the secret scan are written before the first run, and the first run's listing is
read line by line before the job is put on cron.**

### F-457 — THE ASSISTANT'S OWN: TWO LIVE 401s BEFORE THE GATE WAS READ
`salts_refresh.py` posted to `/finance/purchase/api/salts`, which `purchase_app` does expose — but the
finance front gate opens only three prefixes to the machine token, the S224 list: push / vendors / feed.
Two live 401s were spent suspecting the key before the gate's code was opened, where the answer was one
tuple long. Rerouted through the `/vendors` machine door (D489); live since. Rule 6 of the working
protocol, reaffirmed against the assistant: *when a screen is wrong, read the screen's own code FIRST.*
**RULE: a 401 from a service whose gate is a list in one file is a question for that file, not for the
credential — read the gate before blaming the key, and never spend a second live call on a guess.**

### F-458 — THE REPOSITORY GATE BLOCKED ARITHMETIC AND PASSED A LITERAL SECRET
`NO_PHONE_NUMBERS.py` refused the byte-exact live capture for the public repository on a **false
positive: an arithmetic expression of two five-digit figures** in `finance_app.py` read as a mobile
number. In the same pass it **did not flag `att_config.py`'s literal password and secret** — the very
shape F-365 built the secret layer to catch — because the assignments there do not match its heuristic.
The gate was therefore both too strict and too loose in one run. The capture is held privately
(ClaudeCowork `02_SESSION_KITS` and the SSD) rather than masked; a gate improvement is owed — the
numeric rule to ignore operator-separated figures, the secret rule widened to the settings files'
shapes. **RULE: a gate's false positive is a finding, not a nuisance to work around — and a gate that
is tested only on what it catches is never tested on what it misses; the selftest carries one live-shape
miss for every rule.**

---

**Next free: F-459 · D496.**

**END OF FAULT → ACTION REGISTER v2.75 — F-458 is the last minted finding. The v2.74 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S243 close: the first 599653 bytes of v2.75 are byte-identical to v2.74 (md5 9e0840962b34b2dcf5b504c2544b016e). If you cannot see this line, you are reading a truncated copy.**


---

## §S244 — 13-Sep-2026 · F-459 … F-464 (two the assistant's: F-460, F-463)

### F-459 — TWO "OUR ONLINE" FIGURES FOR ONE DAY
`finance_clinic_day.py` computed the clinic's online takings in two places with two rules: the day card
summed the online lines, the MPR page summed online lines **and** the online legs of split bills. On a
day with one split bill the two pages disagreed with each other, and the reconciler would have had to
pick one. Fixed in `S251_CLINIC_MONEY`: one function, `our_online_p()`, read by the day card, the MPR page
and the reconciler. **RULE: a figure that two screens show is computed in one function that both call;
a second implementation of the same sum is a defect the day it is written, not the day it disagrees.**

### F-460 — THE ASSISTANT'S OWN: A PUBLISHED, INSTALLED KIT REBUILT IN PLACE
`S245_AMIR_BILLTAP` was published, installed (`aad400fa…` → `79eb701f…`), and minutes later — when the
owner's next ruling arrived — **rebuilt in the same folder** with the new content. The owner pulled and
ran it; the installer's pin gate refused (*live already carries the S245 mark but is not this kit's
file*) and changed nothing. Nothing was damaged; the owner spent a step, and for a moment the repository
said something other than what was live. The live bytes were recovered from the commit before the
rebuild (`git show <commit>:deploy_kits/S245_AMIR_BILLTAP/amir_day.py`, verified `79eb701f`), and the
change was re-issued as `S246_AMIR_LIST_REOPEN` against the pin actually live. The S245 folder carries a
SUPERSEDED README and its installer still refuses on the pin, which is correct. *(The S246 build document
labelled this F-458 — see F-463.)* **RULE: a published kit is immutable. Any further change, however
small and however soon, takes the next kit number and is built against the pin that is live. A published
commit is the record of what is live; that is what makes a clean re-base possible.**

### F-461 — AMIR'S BILL LIST COUNTED A BILL ONCE PER EXPORT IT RODE IN
A purchase bill rides in every cumulative 1st-to-date BILLWISE export after the one that first carried
it, and `_bills()` joined bill to export without grouping. With more than one export, one unanswered
bill was listed — and **counted in the gate that decides whether the day can close** — once per export.
It had barely shown because step 4 could not verify before S243 (F-455). Fixed in S246: grouped by the
bill, `seen_day` the earliest export that carried it; the old rolling 45-day cutoff removed, since a
window would have hidden an unresolved flag once it aged out. *(Labelled F-457 in the build document —
F-463.)* **RULE: a list that feeds a gate is de-duplicated at the key the gate counts, and the walk
carries a fixture with two exports of the same bill.**

### F-462 — TWO SAME-DAY SESSIONS MOVED THE SAME LIVE FILE; THE INSTALL REFUSED CORRECTLY
`S249_CLINIC_MONEY` was built on `tile_grants.json` v14 `0efad736…`, the S243 close pin. Between its
build and its install, the owner's second chat installed `S250_STAFF_REGISTER_TILE`, which moved the
same file to v15 `932f7bd0…`. The S249 installer's currency gate refused — nothing installed, nothing
touched — and the kit was re-issued as `S251_CLINIC_MONEY` on v15 → v16, installed GREEN. The gate
worked; the cost was one refused install and one re-issue. **RULE: when two sessions run at once, the
file that both will move is named at the start, and the kit that reaches the box second is re-based on
what the first one left — the pin gate is the last line, not the plan. The record of a parallel chat is
folded into the same close, under a name the owner can find it by.**

### F-463 — THE ASSISTANT'S OWN: THE KIT COUNTER RAN AHEAD OF THE SESSION, AND NUMBERS WERE RE-USED
Session 244 issued kits S244 … S254 as if each kit were a session, and the session's own build documents
(`S244_AMIR_PROCESSING_BUILT` … `S248_SALARY_RAISE_BUILT`, `S246_AMIR_LIST_REOPEN_BUILT`) minted
decisions **D478 … D484** and findings **F-457, F-458** — numbers already minted at S241 and S243 —
because the documents were written from the kit's number, not from the Register's reserved line. The
Register's "next free" line was right the whole time (D496 · F-459); it was not read. Corrected at this
close, recorded not silent: the rulings are re-minted as **D497 … D503**, the findings as **F-460 and
F-461**; the build documents keep their wrong labels with a one-line note; **the next session is declared
S255**, so the session counter and the kit counter meet. **RULE: decision and finding numbers come from the
Register's reserved line, read at the moment of minting — never from a kit number, a document name, or
memory. A kit number is a kit number.**

### F-464 — THE DEPLOY CLONE DOES NOT PULL ITSELF AFTER THE OWNER'S PUBLISH
The first S244 install line — the bare `bash …/install_S244_AMIR_PROCESSING.sh` — met *No such file or
directory* on a kit that had been published correctly: `/root/deploy/repo` is a plain clone that nobody
pulls between sessions. The working line is `cd /root/deploy/repo && git pull --ff-only && bash …`, and
every later line this session carried it. *(The S244 build document called this an "F-456 candidate";
F-456 was already the code-bundle secrets finding.)* **RULE: every VPS install line carries the pull;
the owner never meets a bare error for a kit that was published correctly.**

---

**Next free: F-465 · D510.**

**END OF FAULT → ACTION REGISTER v2.76 — F-464 is the last minted finding. The v2.75 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S244 close: the first 608542 bytes of v2.76 are byte-identical to v2.75 (md5 fe78ea6c63d7f1383dbeec48d3d69eec). If you cannot see this line, you are reading a truncated copy.**

## §S255 — 13/14-Sep-2026 · F-465 … F-471 (four the assistant's: F-466, F-468, F-469, F-471)

### F-465 — THE TWO MACHINE TOKENS HAVE A RESTING PLACE NO INVENTORY COUNTED
`/etc/systemd/system/clinic-finance.service` sets `FINANCE_CRON_TOKEN` (48 chars) and `FINANCE_MARG_TOKEN` (32 chars) as plain `Environment=` lines. The S243 study ("one token … cached in five places on two PCs") and the F-456 rotation list did not include the unit file. Found by the Club A capture (13-Sep). **Rule:** the F-456 rotation must edit this unit and restart `clinic-finance`, or the app returns with the old token; the repository copy of the unit carries `DUMMY-TOKEN-PUT-HERE`, never the value; Club C.4 (one token per sender) treats the unit file as a token store.

### F-466 — TWO CANON DOCUMENTS DESCRIBED A FIX AS OPEN THAT HAD BEEN INSTALLED THE SAME NIGHT (assistant's)
`S243_CURRENT_CONDITION_AND_ARCHITECTURE_PLAN` §A2 and `SANJEEVNI_SYSTEM_BOOK_v1_1` line 216 say `clinic-finance` is not on the watchdog; `S243_WATCHDOG_FINANCE` had been installed at S243 and the pin list said so. The assistant planned Club A from the paper and told the owner an installed thing was open; then found three Club B items in the same state. The same START_HERE carried a stale F-242 browser warning. **Rule:** a plan item is verified against the live pins before it is built or reported open; a dated analysis paper is not the record — the pin list is.

### F-467 — THE DOCTERZ MODE FIELD DOES NOT CARRY THE PAYMENT CHANNEL; D507'S PREMISE WAS FALSE
`S244_BUILD_BRIEF` (D507) states that Wallet / Patient APP / Net Banking rows are Razorpay payments. Live evidence: the one payment with a Razorpay receipt (29-Aug, `pay_TVSz…`) is recorded as *Online Payment*; a *Wallet* payment (12-Sep, ₹600) paired to the ICICI MPR; the owner's personal-phone money is *Online Payment*. Raised by the owner ("if they were from portal, there must be an email"). **Rule:** D510 — the channel comes from the independent record (ICICI MPR · the sheet's other-UPI box · the Razorpay id on the line), never from the mode word. A sentence in a brief is a claim to test, not a premise to build on.

### F-468 — A LABEL-BASED SPLIT REINTRODUCED THE F-459 CONTRADICTION (assistant's)
`S255_PORTAL_CHANNEL` split the entries in `clinic_money._docterz` but passed only a figure to `finance_clinic_day._mpr_card`; the MPR page kept pairing an entry the reconciler had removed as portal, and the same ₹600 read two ways on two screens. Installed at ~14:0x IST, rolled back the same hour, both pages verified. **Rule:** one entries list, every reader filters it the same way; a change to *which entries count* is made where the list is built and nowhere else. S257 does it so.

### F-469 — AN INSTALLER REPORTED A SUCCESSFUL BACKFILL AS FAILED (assistant's)
`install_S257_PORTAL_EVIDENCE.sh` judged `docterz_ingest.py --all` by its exit code; the re-read had done 76 of 78 files (two Drive files fail to parse, as before) and the script exits non-zero on any failure — the installer printed "backfill: FAILED" over a job that had succeeded. Immutable kit; recorded. **Rule:** a wrapper judges a batch by its own counts ("read N, failed M"), never by the exit code alone.

### F-470 — NINE PLAIN TOKEN COMPARES ACROSS THREE FILES; ONE CONSTANT-TIME COMPARE IN THE ESTATE
The S243 study said "two of the apps compare with `==`". The Club C map found seven `==` and three `!=` sites (`finance_app.py` ×7 including two doubled, `stock_app.py`, `purchase_app.py`) and exactly one `hmac.compare_digest` (`marg_door.py`). Fixed by `S258_CONSTANT_TIME` through one helper; an empty expected token now never matches (before, `"" != ""` on the two `!=` sites let a headerless request through a token-less server). **Rule:** every machine-door check goes through `_tok_ok()`; a new door copies the helper, not the `==`.

### F-471 — KIT FILENAMES CONTAINING "TOKEN" ARE SILENTLY IGNORED BY THE REPOSITORY, AND A RENAME LEAVES THE OLD FILE FOR THE GATE TO FIND (assistant's)
`.gitignore` line 36 (`*token*`) excluded `patch_tokens_s258.py` and `selftest_tokens_s258.py`; `PUBLISH_ALL.bat` refused, correctly. After the rename the old files still sat in the folder (no device shell to remove them) and the gate — which scans the folder, not the kit's `SUMS.md5` — refused again; one `del` line from the owner cleared it. **Rule:** no "token" or "secret" in any kit filename; `git check-ignore` on every kit file before hand-over (the F-432 close step, applied at build time); a rename before first publish must remove the old file from the owner's folder.

---

**Next free: F-472 · D511.**

**END OF FAULT → ACTION REGISTER v2.77 — F-471 is the last minted finding. The v2.76 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S255 close: the first 614412 bytes of v2.77 are byte-identical to v2.76 (md5 2e8dc5c3ca2bb05246270ce78c361fab). If you cannot see this line, you are reading a truncated copy.**


---

## §S256 — F-472 … F-478 (14-Sep-2026)

**F-472 (S256 · CLOSED by install · the assistant's) — A PREDICTED PIN WAS RECORDED AS THE LIVE PIN.**
`/root/finance/amir_day.py` read `79eb701f2df14e58c77e130a77b972ba` — the exact value
`S246_AMIR_LIST_REOPEN` was built *over*, per `EVIDENCE_S246.txt:115`. **The kit had never been
installed**, and the Register carried its predicted to-pin as though the box had printed it. Found by
hashing the box instead of trusting the record. *Action:* a predicted pin is never written into the
canon as the live pin. **A lost read-back is BLIND, not SHORT** — SHORT means a truncated value that
was read; BLIND means no read happened, and the two must not share a label. The owner installed
S246 the same night (156 checks; box read back `a9f2062267ebac59e02fdb0f88e775de`).

**F-473 (S256 · CLOSED · the assistant's) — BYTES THAT MUST BE EXACT DO NOT PASS THROUGH THE
ASSISTANT'S OWN OUTPUT.** A base64 hand-relay of a kit to the medical PC's Drive channel produced
17,560 bytes on the first attempt and 17,562 on the second, against a required 17,561. A deliberate
4-byte probe round-tripped exactly, **proving the transport was sound and the assistant was the
faulty link**. *Action:* exact bytes move by the file-transfer tools or by a synced folder — never
re-typed, never re-encoded through a reply. Resolved by writing the kit into the Drive folder manojz
already syncs.

**F-474 (S256 · CLOSED · the assistant's) — A KIT IS NOT PUBLISHED UNTIL THE PUBLISH RUNS AFTER IT
IS WRITTEN.** The `S261_VENDOR_PAY` install line failed on the VPS with *No such file or directory*.
The publish the owner had run listed thirty files, all S259 and S260: **it predated the kit by an
hour.** *Action:* before naming an install line, confirm the kit's files are in the publish that
actually ran — the publish output names what it carried, and a kit absent from it is a kit the VPS
cannot see. Re-published; installed 42/0.

**F-475 (S256 · CLOSED by S263 · the assistant's) — AN IMPORT KEYED ON A NAME IS NOT AN IMPORT.**
The 22 verified vendor bank accounts imported at S225 were filed under **the name the bank knows**;
Marg's bills carry **the name the vendor prints**. For 14 vendors those are different strings, so
**only 8 of the 24 imported rows had ever been matched by a single bill**. The remaining 16 sat on
the server, correct and unreachable, while their vendors fell to the cheque lane —
**₹2,22,608 of August's purchases**. Nothing was stale: all four Apr–Jul sheets agree with each other
and with the server. *Action:* S263 links the bill's name to the account row (one account, one row,
a pointer); a future rename is one more row, not a re-import. Every pair was put to the owner and
confirmed before it was built.

**F-476 (S256 · CLOSED by S264) — A PAGE THAT OPENS ON ONE MONTH WITH NO WAY TO ANOTHER IS NOT
NAVIGABLE, AND TWO SCREENS DOING DIFFERENT JOBS MUST NAME EACH OTHER.** The tile landed on the newest
month and the payment sheet printed no link to any other, so the only route to August was the hub —
whose month link opens the purchase AUDIT page. The owner asked for August's payment sheet, got
August's purchase audit, and reasonably read the old screen as the new flow misbehaving. *Action:*
a month strip on the sheet; the audit page now opens *"This is the purchase audit … To pay the month,
open the payment sheet for August 2026"*, and the sheet points back in a sentence that reads.

**F-477 (S256 · CLOSED in build · the assistant's) — A BANK ACCOUNT NUMBER WAS WRITTEN INTO A KIT.**
The first `S263_VENDOR_LINK` build carried DAANSHI PHARMA's account number in `block.py`. Caught
before publish against the standing rule that bank numbers do not enter the repository. *Action:*
rebuilt so a new vendor's account travels **on the install line**, is written straight to the
database by a helper that prints it back masked to the last four, and **never overwrites an account
already on file** — replacing a live account is a decision, not an install step. Same pattern reused
for the shop's printed mobile (S265) and the firm's debit account (S266).

**F-478 (S256 · CLOSED by S266 · the assistant's) — PRETTIER THAN THE SOURCE IS STILL DRIFT.**
The S265 advice card printed amounts as `353,410`. The bank's own sheets carry **no separators at
all** — the July file says `97930`, the letter says `Rs. 472527/`. A cosmetic improvement to a
document whose whole value is that it matches what has always been sent is a defect. *Action:* S266
removed the separators from the advice and its total and left `_rupees_s266` as the single place a
rupee figure becomes text, for the advice, the letter and the `.xlsx` alike.

---

**Next free: F-479 · D521.**

**END OF FAULT → ACTION REGISTER v2.78 — F-478 is the last minted finding. The v2.77 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S256 close: the first 619449 bytes of v2.78 are byte-identical to v2.77 (md5 961287b20500b6c191b448f8c7a4db98). If you cannot see this line, you are reading a truncated copy.**

---

## S257 — F-479 … F-484

**F-479 · A PIN TAKEN THREE MINUTES BEFORE THE BYTES WERE FINAL.** *(the assistant's — S256's, found at the S257 open)*
`MD5SUMS_ALL.txt` pinned `S256_CLOSE_REPORT.md` at `7611c73b…`. The file is `3b8d217a…` and is
**byte-identical in `D:\dr-manoj-git` and on GitHub** (9,873 bytes, `cmp` clean). The sums file was
written at 12:21:17 IST; the close report was saved again at 12:24:19 IST. **The document was never
damaged — the row was wrong.** This is the **F-472 family in the session immediately after F-472 was
minted**, and the rebuilt Cowork manifest showed the same shape on five more files at once: every
S256 canon snapshot row had been appended from an intermediate draft and now hashes identical to the
published canon. *Action:* the row corrected in `MD5SUMS_ALL.txt` at this close. **The standing
repair is ordering: `MD5SUMS_ALL` and every snapshot row are written AFTER the last document is
final, never alongside it.**

**F-480 · A MANIFEST THAT CONTRADICTED ITSELF.** *(the assistant's, accumulated)*
`D:\Downloads\ClaudeCowork\MANIFEST.md5` carried **`00_INDEX.md` twice, at two different hashes** —
line 557 `d37cd762…` from a full build, line 2684 `29ada2a6…` appended at the S256 close. `md5sum -c`
would report **the same file both OK and FAILED**. Append-only upkeep, forced by a dead shell since
08-Sep, had made the gate self-contradicting; nothing reads a gate that disagrees with itself.
*Action:* `S268_MANIFEST_TOOL` rebuilds the whole tree from the bytes and resolves it to one row;
a nightly task at 03:10 keeps it current, so the rebuild is owed at no future close.

**F-481 · A DELIVERY REPORTED AS WRITTEN THAT HAD NOT LANDED.** *(the assistant's)*
`device_commit_files` returned `written` for `selftest_rebuild_manifest_s268.py` while the file on
manojz was still the previous version — 7,070 bytes, `b551aaab…`. The owner ran it and got the old
behaviour. It was caught only by **staging the file back off the machine and hashing it**;
re-committed and verified at `c98766d6…`, matching its own `SUMS.md5`. **The tool's own success
report is not evidence** — the F-473 family, one layer further out. *Action:* every commit in this
session was followed by a stage-back and a hash, and that is now the standing practice: **a delivery
is proven by reading the bytes back, never by the tool saying it wrote them.**

**F-482 · A LIVE-PIN ROW NAMING A VPS PATH FOR A FILE THAT HAS NEVER BEEN ON THE VPS.**
`md5sum` answered **`/root/finance/push_purchases.py: No such file or directory`**. The Register rows
it as a VPS file at `a316c345` *(8-char)* — **and the same Register already says it is not one**, in
the S243 wave header: *"push_purchases.py (F-453) is NOT re-rowed: it runs on manojz, not the VPS."*
The correction was made and **the row survived it and three closes after it**, and `gen_live_pins.py`
faithfully emitted it as a VPS row, because the generator reads the Register and the Register was
wrong. The file is real and correctly rowed PC-side at `13a5cb8e…`. **One file, two rows in two
stores, one naming a path that does not exist — the F-449 family (*exactly one CURRENT per family*).**
**Why the shorthand hid it:** an 8-character pin looks like a pin. Only asking the box whether the
file exists could expose it, and in three closes nothing had asked. *Action:* the VPS row struck from
the Register at this close; the PC-side row stands. `a316c345` is **not** carried forward — it cannot
be tied to anything on the box, because there is nothing there.
*(Recorded alongside, and NOT a fault: `/root/finance/walk_staff_gate_s225.py` was also absent, and
correctly so — the Register says plainly it is a walk tool, not a live file. Written down so no
future session mints a finding for it.)*

**F-483 · THE KIT WAS RIGHT; THE WALK AND THE PACKAGING WERE NOT — FOUR PARTS.** *(all the assistant's)*
S270 was built correctly at the first attempt and failed twice on the box for reasons that had
nothing to do with the cheque register.
**(a) A copy of the app cannot run from `/tmp`.** `purchase_app.py` resolves `purchase_schema.sql`
from its **own folder**, so a patched copy in `/tmp` made every page a 500 looking for
`/tmp/purchase_schema.sql`. *Action:* the walk refuses a `--file` outside the app's folder and names
the reason.
**(b) Four checks went GREEN on a 500 body.** They asserted the *absence* of a string, and an error
page lacks it too — so *"the old card's wording is gone"* and *"the duplicate is refused"* both
passed while the page was an exception. **This is the F-438 family: a proof must assert exactly what
the change claims.** *Action:* every page read goes through `need200()`, which fails loudly; a check
that cannot run is named as skipped and never counted as a pass (F-443).
**(c) A real drift, and the walk caught it — F-478's shape again.** `/hub`, `/scans`, `/orders` and
`/book` each grew by **exactly 1,287 bytes**: the cheque CSS, appended to the page-wide stylesheet by
an early draft, landing on four screens with nothing to do with cheques. *Action:* **fixed at the
source, not by relaxing the check** — the style travels with the card and the register alone, the
patcher **refuses** any block that appends to the shared stylesheet, and an offline check asserts the
shared sheet is untouched.
**(d) A refusal judged by its status code.** A correct refusal, carrying exactly the right message,
came back **403** — what `_refuse()` returns on this box — and the walk demanded 200. *Action:* a
refusal is judged by **what it says**, not by the code it rides on.

**F-484 · A WALK ASSERTS WHAT ITS OWN KIT CLAIMS.** *(the assistant's)*
S271's walk was built from S270's and inherited the check *"it did not exist before this kit"*,
asserting 404 on the previous file. True when the register was new; **false for S271, whose "before"
file IS the live S270 and rightly answers 200.** The run failed, the `&&` chain stopped, and nothing
reached the live file — the design working. *Action:* corrected to the claim S271 actually makes.
**The general rule, and the session's lesson: a walk is code, it carries claims, and its claims go
stale exactly as a document's do. Three times in one evening a check — not the code — was what was
wrong, and each was corrected in the walk rather than relaxed.**

---

**Next free: F-485 · D522.**

**END OF FAULT → ACTION REGISTER v2.79 — F-484 is the last minted finding. The v2.78 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S257 close: the first 624591 bytes of v2.79 are byte-identical to v2.78 (md5 c2abbf77f4921f5119de370c392a778c). If you cannot see this line, you are reading a truncated copy.**


---

# §S258 — F-485 … F-492 · 15-Sep-2026

**Eight findings. Six of the eight are the assistant's own.**

---

## F-485 · A FILE ENTERS CANON AND THE GATE STOPS COVERING IT — WITHOUT SAYING SO

**Twice in one morning.** `SANJEEVNI_SYSTEM_BOOK_v1_2_S258.md` was published into
`deploy_kits\KB_canon_all\` and `MD5SUMS_ALL.txt` gained **no row for it**. `md5sum -c` then reports
**535 of 535 OK** — and is not checking the new file at all. It happened again an hour later when the
Book was corrected for F-487.

**Measured rather than assumed: 537 files in canon, 535 rows.** Two files unwatched — the Book, and
`deploy/push_kit.bat`, which had been ungated for some time and which nobody had noticed because
nothing looks.

**This is F-369's shape and its third instance in three days.** The nightly unattended check had
already reported the same for `live_pins_S256close.txt` and `live_pins_S257close.txt`: *two
consecutive closes generated a pin list and neither got its row.*

> **A gate that quietly stops covering a file still exits 0.** Green is not the same as complete, and
> a count is only meaningful against the count it ought to be. **Ask a gate what it is guarding, not
> just whether it passed.**

**Repaired twice by hand, which is not a repair.** Each time by adding exactly the missing rows, with
every existing row byte-identical and in its existing position (`diff` showed `411a412` and `481a483`
and nothing else). **The actual repair is two-part:** `S275_CANON_SUMS` owns the file nightly, and
**`END_OF_SESSION_PROMPT_v14` A8b makes the close own it before the publish** — because the nightly
runs at 03:10 and the publish happens at the close, so the nightly can only ever be the safety net.

---

## F-486 · THE ASSISTANT'S — A STORE DECLARED EMPTY ON THE STRENGTH OF ONE OF ITS TWO LISTS

`/root/staff_master.csv` was written up as one of the files with **no off-box copy in any store**,
and that claim reached the Sanjeevni System Book and survived one publish.

**It is not true.** The check read `clinic_state_backup.py`'s **`SRC_DIRS`** and stopped there. The
same file also has **`SRC_FILES`**, and `/root/staff_master.csv` is its **fifth line**, directly under
the heading *"INCLUDED — the data with no other off-box copy."* It has been backed up nightly, and
encrypted, the whole time.

**How it was caught, and the part that matters:** on the **second reading of the same file** — while
scoping the repair, not while checking the finding. The verification step never ran; the build step
happened to do it.

> **A STORE IS NOT CHECKED UNTIL EVERY ONE OF ITS INCLUSION LISTS HAS BEEN READ.**
> Reading one of two is how a false gap is minted. **A false gap costs what a missed one does** — it
> spends the owner's trust and it sends the next session hunting something that is not there.

Now an evidence rule in `END_OF_SESSION_PROMPT_v14` A0 and `START_HERE_PROMPT_v9`. The retraction is
written into the Book at the row it touched, not deleted from it.

---

## F-487 · THE ASSISTANT'S — A PAGE READ AS TEXT IS NOT THE DATA

Reading `/finance/purchase/page/pay/2026-09` as plain text, one vendor appeared to read
**`AGARWAL SURGICALS AND MEDICALS CHEQUE`**, and the Book reported that the payment mode had been
typed into the supplier's name in Marg — with a recommendation about where the repair belonged.

**`CHEQUE` is a chip.** `purchase_app.py` line 2215:
`chips += ' <span class="chip warn">CHEQUE</span>'` — a lane badge rendered beside the name. A
plain-text extraction runs the two together. Checked against the data afterwards: `purchase_bill`
holds the supplier as `AGARWAL SURGICALS AND MEDICALS` across **nine bills**, clean, and
`purchase_vendor_contact` **has a row for it**, added 14-Sep 22:30 with a phone. Nothing is wrong
with the name. What is missing is narrower and was already ⭐0 item 2: the row has no `acct_no`, no
`ifsc`, no `acct_name`.

> **A page tells you where to look. The store tells you what is true.**
> Confirm any finding taken off a screen against the table, the file or the export behind it — *the
> rendering is not the record.*

**Second retraction of the same session, and the pattern is the finding**: both F-486 and F-487 came
from asserting on a partial reading. Both are now evidence rules; both retractions are written into
the Book rather than tidied away.

---

## F-488 · THE ASSISTANT'S — A NIGHTLY THAT CANNOT CATCH UP, AND A COMMAND THAT LIED ABOUT FIXING IT

**Two faults in one.**

**First:** the 03:10 task `KB Manifest Rebuild` was registered with
`schtasks /create /tn … /sc daily /st 03:10 /f`, which does **not** set *"run task as soon as possible
after a scheduled start is missed"*. manojz is the owner's **personal PC** and is not always on
overnight. **So any night the machine was off, the manifest rebuild, the SSD mirror and the canon
check were simply skipped — and nothing said so.** It ran on 15-Sep only because the machine happened
to be awake. *Three maintenance jobs were moved onto a schedule that could silently not happen.*

The owner found it, by asking the obvious question nobody had asked: *"it is my personal PC and does
it have to be switched on and running at 3:10 a.m.?"*

**Second, and worse:** the first fix —
`$t.Settings.StartWhenAvailable=$true; Set-ScheduledTask -InputObject $t` — **returned no error and
changed nothing.** Read back four minutes later, all three flags stood exactly as before. The reason
is in the BEFORE line of the tool that eventually worked: **`StartWhenAvailable` was `None` — the
element was absent from the task definition entirely**, so there was no property on the Settings
object for PowerShell to set, and it said nothing.

> **It was caught only because the command had been written to put its result in a file.**
> A command that reports success on the screen and a command that succeeded are different claims.
> **Make a change prove itself by reading the thing back — from the thing, not from the return code.**

`S276_TASK_CATCHUP` does it the way that works: export the task XML, edit three flags, **confirm the
edit landed in the file before importing**, import, then **query the task again from scratch** and
fail loudly if any flag did not take. The original definition is saved dated, with the one line that
restores it. Proven on the box 06:58 IST — `StartWhenAvailable` **None → true**,
`DisallowStartIfOnBatteries` **true → false**, `StopIfGoingOnBatteries` **true → false**, and the
result read back off the disk independently rather than taken from the pasted screen.

Battery flags have bitten this project before (F-314); they came off in the same edit.

---

## F-489 · TWO SURFACES, SAME NIGHT, SAME DATA, DIFFERENT COUNTS

The Marg Purchases hub reads **"373 items compared, 364 agree, 9 differ"**. `sh_run` row 7 — the
shadow's own record of the very run the hub is describing, 14-Sep 23:20 — reads **374 items, 368
same, 6 differ**.

**Different on all three numbers.** Neither is obviously wrong and this finding does not pick one.

**Why it matters beyond the arithmetic:** the shadow's agreement is the stated witness for retiring
the manojz senders (§11.4 item 3f), whose entry condition is *seven clean shadow nights*. **Until the
page and the table agree on how many items differ, neither figure can be used to decide anything.**
A number that two surfaces report differently is not a measurement yet.

Recorded and bounded rather than diagnosed — see the S258 evidence paper for the six named items and
the proof that each difference equals exactly one purchase line.

---

## F-490 · A HALF-WRITTEN TEMP FILE LIVES IN CANON AND IS GATED AS A DOCUMENT

`deploy_kits\KB_canon_all\MD5SUMS_ALL.txt.new` is a **260-row, 17,250-byte, half-written copy of the
sums file** — and it has a row in `MD5SUMS_ALL.txt`. **The gate is carrying it as though it were a
canonical document.** It is old: the live sums file has 537 rows.

**And it very nearly cost a file.** `canon_sums.py` (S275) was first written to stage its output at
`MD5SUMS_ALL.txt.new` — the same name — and `os.replace()` would have destroyed the existing file
without a word. **The kit's own test caught it** on a copy of the real canon folder, before the tool
ran anywhere real. The temp name is now unique per run and refuses a collision.

> **A tool's scratch name is part of its blast radius.** *The second-safest place to write a temp
> file is the folder you are about to rewrite; the safest is anywhere else.* And a test that runs on
> a fixture tree would not have found this — **it was found because the test ran against a copy of
> the real thing.**

The stray file is **reported, not deleted** — deleting is the owner's (⭐0).

---

## F-491 · THE PAPER POLICY'S ONE NUMBER WAS KILLED BY A DIFFERENT TOOL DOING ITS JOB CORRECTLY

`PAPER_POLICY_v1.1`, adopted at S257, is binding and its one measurable requirement is:

> *"Report the never-cited percentage at every close. It was **51 % (193 of 375)** at S257. **If it
> does not fall, the policy did not take — say so.**"*

**At this close the shelf reads `0` never cited, of 380 papers.** 193 → 0 in a single night.

**It is an artefact, and not one paper became more read.** `build_papers_index.py`'s `load_haystack()`
builds the corpus a paper is searched for in from exactly two files:

```
00_INDEX.md   +   MANIFEST.md5
```

**And since S268, `MANIFEST.md5` is rebuilt nightly over the WHOLE of `D:\Downloads\ClaudeCowork\`
— 2,802 rows, every file in the tree, working papers included.** So every paper's name now appears
in the haystack by construction. **The test can never again return true for any paper that exists.**

**Nothing was done wrong.** The manifest tool was built at S257 to rebuild the whole tree and it does;
the shelf was built at S269 to ask whether a paper is referred to anywhere and it does. **The two were
correct separately and, put together one night later, one silently destroyed the other's meaning.**

> **A metric whose corpus is a machine-generated listing of everything measures nothing.**
> And the shape to watch for: **this fault reports itself as GOOD NEWS.** 51 % → 0 % reads as the
> policy working perfectly, on a dashboard, with no error anywhere. **A0's rule — *an empty or small
> result is not good news; a result that would be pleasant if true gets checked harder* — is the only
> thing that caught it.** It was checked precisely because 193 → 0 in one night was too good.

**What the number actually is at this close: UNKNOWN, and it must be reported as unknown, not as 0.**
The last trustworthy reading is **S257's 51 % (193 of 375)**, taken before the nightly rebuild existed.

**The repair, for S259 and named in `START_HERE_SESSION_259`:** the haystack must exclude
`MANIFEST.md5` — a gate's inventory is not a citation — and a *citation* must mean a paper's name
appearing in a document somebody wrote: an index, a brief, a close report, a register. Until that is
done **the shelf's never-cited figure is not to be quoted**, and the close report says so rather than
printing a zero.

*(A second, smaller instance of the same shape was already visible and is recorded here beside it:
`S257_NEVER_CITED_193.md`, written at the S257 close, names all 193 files — so even with the manifest
excluded, those 193 would read as cited by the document that exists only to list them. **A list of
the unread is not a reading of them.** Whatever replaces this metric has to survive both.)*

---

## F-492 · THE ASSISTANT'S — FIVE KITS BUILT THE SAME WAY, AND ONE OF THEM HAD NO GATE

**`S276_TASK_CATCHUP` shipped with no `KIT_ID.txt`, no `README.md` and no `SUMS.md5`.** Three files
— the batch file, the python, and a stray `__pycache__\*.pyc` — and nothing that could verify any of
them. **The only one of this session's five kits with no self-gate.** S272, S273, S274 and S275 each
carry all three.

**And the `.pyc` had been picked up into the cold kit and given a row in its `SUMS.md5`** — a
compiled artefact gated as though it were a deliverable, which is F-490's shape at one remove.

**How it was found, and it is the only part worth keeping:** while assembling the cold kit, by
**counting the files in each kit against its source folder, one kit at a time.** Not by a gate —
there was no gate to fire. The kit had been built, walked, installed on manojz and proven live at
06:58, and its output was read back off the disk. **Every check this session ran on S276 passed,
and every one of them was about the task, not about the kit.**

> **A kit that proves what it DOES is not the same as a kit that can prove WHAT IT IS.**
> S276's proof of the scheduled task was the strongest in the session — export, edit, confirm the
> edit in the file, import, re-query from scratch. It still shipped unable to say whether its own
> four files were the four files it was built from.
> And the reason nobody noticed: **four kits built correctly are an argument that the fifth was
> too.** *Consistency is an expectation, not evidence.* **Count the set; do not sample it.**

**Repaired at this close**, before the cold kit was sealed: the `__pycache__` removed, `KIT_ID.txt`
and `README.md` written (both carrying F-488's lesson, which is what the kit exists for), and
`SUMS.md5` generated and verified from inside the folder — **4 files, 4 rows, 4 OK.**

**The durable repair belongs to the kit template, not to S276**, and is named for S259: *no kit is
published until it carries its own `KIT_ID.txt`, `README.md` and `SUMS.md5`, and a close counts the
three per kit rather than assuming them.* `PYTHONDONTWRITEBYTECODE=1` before python runs in a kit
tree keeps the `.pyc` from existing in the first place — A13.5 already says so for the drive roots,
and it was not applied here.

---

**Next free: F-493 · D524.**

**END OF FAULT → ACTION REGISTER v2.80 — F-492 is the last minted finding. The v2.79 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S258 close: the first 631365 bytes of v2.80 are byte-identical to v2.79 (md5 cf494367b65b839b5b6bdcd24a244b97). If you cannot see this line, you are reading a truncated copy.**

---

## F-493 · TWO SURFACES ANSWERING DIFFERENT QUESTIONS UNDER THE SAME THREE WORDS — S259

**Minted:** 15-Sep-2026, S259. **Class:** measurement / canon. **Severity:** it stopped a plan item.

S258 recorded that the stock hub read **373 compared · 364 agree · 9 differ** and the shadow's own
table read **374 · 368 · 6**, same box, same night, same underlying subject, and refused to pick one.

**Both were reproduced at S259 from the same nightly database**, each by the live code's own logic,
and they are **not the same comparison**:

| surface | what it compares | over what |
|---|---|---|
| the hub's Stock verification card — `_stock_card()`, `/root/finance/purchase_app.py` | the PC's **computed expected** figure against **Marg's own closing export** (`push_expected` vs `push_snapshot` in `stock_feed`) | the 373 items carrying both sides on the newest comparable day |
| `sh_run` / `shadow_last.json` — `marg_shadow.py` | the **server's own re-run** of the PC's code against **what manojz pushed** | the baseline's 374 items |

Marg's snapshot is not in the second comparison at all. **Neither figure is wrong.**

**THE FAULT IS THAT BOTH ARE WORDED "compared / agree / differ", AND CANON COPIED ONE INTO THE
OTHER'S ROW.** Sanjeevni Book v1.1 recorded *"the shadow at 373 compared, 370 agree, 3 differ"* —
373 is the hub's population, not the shadow's. A reader comparing across closes was comparing two
different measurements and could not know it.

**Consequence, and it is the expensive part.** Plan item **3f** — retiring the manojz senders — has
its entry condition written as *seven clean shadow nights*, and that has been read off the hub card.
**The hub card can never reach zero while two spellings of one product exist**, so the condition as
read is unmeetable. Three of the hub's nine differences are exactly that: KNEE IMMOBILIS**E**R
UNISON M (+1), KNEE IMMOBILI**Z**ER UNISON L (−1), SHOULDER IMMOBILISE UNISON M (−1).

**THE RULE.** *A number carries the question it answers, or it carries nothing.* Two surfaces
measuring different things may not share a vocabulary, and a condition written on a number must
name the surface it is read from. **3f's witness is `sh_run.differ = 0` on seven consecutive runs.**

**Also eliminated at S259**, of S258's two candidates for the six: every purchase export the server
holds for the range was captured on 13-Sep and every one of those stamps precedes run 7's cutoff of
`20260914-223006`, so `as_the_pc_saw_it` is **not** what excludes them.

---

## F-494 · A PURCHASE BILL THAT BELONGS TO NOBODY — S259

**Minted:** 15-Sep-2026, S259. **Class:** data attribution. **Money affected:** ₹17,777, September.

**Bill 160 of 1 September exists twice** — one from **DAANSHI PHARMA**, one from **KEDAR
PHARMACEUTICAL** — both dated the same day. `purchase_line` attributes a line by **bill number and
date**; when both collide the lines cannot be placed and were written with **no supplier at all**:
**5 lines, ₹17,777**, present in September's month total and on **no vendor's payment sheet**.

**Bounded, and the bound is the useful part.** Sixteen bill numbers are shared by more than one
supplier across April–September; the other fifteen resolve correctly **because their dates differ**.
One occurrence in **1,551 lines**. August is untouched, so the August close is not held up.

> **A key that is unique per supplier is not unique across suppliers.** Marg numbers bills per
> vendor; our table keyed them as though the number were global and leaned on the date to separate
> collisions. The date is not a tiebreaker — it is a coincidence that usually holds.

**Repair:** attribute by supplier as well as bill and date. **Assistant's, not owner's.**

---

## F-495 · THIRTY OF THIRTY-EIGHT JOBS COULD NOT PROVE THEY HAD RUN — S259

**Minted:** 15-Sep-2026, S259. **Class:** observability. **Severity:** it made every green reading
unfalsifiable.

Four of this session's six open faults were one shape — a check built correctly, wired correctly,
and then either never switched on or never looked at (F-485, F-491, the seven silent legs,
`darpan_kal_day` with zero rows). The common root: **a job that is quiet looks exactly like a job
that is passing.**

Measured on the box: **38 clinic jobs** — 46 cron lines resolving to 34, plus 4 systemd timers.
**8 left a run record anyone could read back. Thirty did not — and all five nightly backups were
among the thirty.**

**The loop that closes into is the fault, not the count.** `finance_backup.sh`, `code_bundle.py`,
`finance_drive_backup.py`, `sheets_pull.py` and `clinic_state_backup.py` leave no readable run
record; the health page has a `backup` leg meant to watch them; and that leg is **one of the seven
that have never once fired in fourteen days**. The thing that would save the clinic writes no proof,
the watchman over it has never spoken, and **neither state can be distinguished from the other.**
Nothing here said a backup had failed. The fault is that nothing could say either way.

**The fix that was refused, and why it matters that it was refused.** Adding a "record that you ran"
line to thirty live files is thirty chances to break something that works, for a change that adds no
function. **45 of the 46 cron lines already name their own log.** `S279_JOB_PULSE` reads each path
**out of the crontab at run time** — never inferred, never baked into a list that can go stale — and
asks systemd directly for the timers. It modifies no job and opens no database.

> **Measure liveness from the evidence a system already leaves. A watchman that requires every
> guard to be rebuilt will not be built.** (**D527**)

**First reading: 35 alive · 1 quiet · 2 no trace · 1 late.** Nothing dead.

---

## F-496 · THE BACKUP CARRIES WHAT A JOB IS, NOT WHETHER IT IS SWITCHED ON — S259

**Minted:** 15-Sep-2026, S259. **Class:** backup coverage. **The assistant's to fix.**

`code_bundle.py` copies `/etc/systemd/system/*.service` and `*.timer` and the root crontab — **what
each scheduled job is**. It does not copy the `*.wants/` symlinks, which are **whether each one is
enabled**. `clinic-certwatch.timer` is in the bundle, correct and complete, and its enabled state is
not.

**Cost, measured the same day:** S259 could read the certificate watch's code, its schedule and its
thresholds, and still could not say whether it had ever run — a question that then needed a command
from the owner. **A backup that cannot answer "is this switched on?" cannot answer the only question
that matters about a watchman.**

One line in `code_bundle.py`. Named for S260.

---

## F-497 · AN AGE THAT COULD NOT BE TRUE, REPORTED AS HEALTHY — S259 · THE ASSISTANT'S

**Minted:** 15-Sep-2026, S259. Found by the tool's **own first live run**, within minutes.

`job_pulse.py` v1.0 parsed systemd's `ExecMainStartTimestamp`, **forced UTC onto it and converted to
IST**. systemd prints in the box's own zone, so the shift was applied twice: `clinic-followup-push`
and `clinic-watchdog` reported a run **in the future**, ages of **−170** and **−329 minutes** — and
v1.0 printed both as **ALIVE**.

> **A quantity with an impossible value must never pass a check.** The bug was the timezone; the
> fault was that nothing refused a negative age. v1.1 reports **AHEAD?** and counts it as a problem.

Corrected in `S280_JOB_PULSE_FIX` the same hour: systemd is asked for a unix epoch, which carries no
zone to get wrong. A test now asserts it.

---

## F-498 · AN EMPTY LOG AND A STALE LOG TREATED AS THE SAME FACT — S259 · THE ASSISTANT'S

**Minted:** 15-Sep-2026, S259. Also found by the first live run.

`marg_ingest.py` read **SILENT at 2.3 days** while its own `mi_run` table showed a run at **01:00
that morning**. It runs every five minutes and prints nothing on a quiet pass, so its log does not
move. v1.0 took a log's mtime as the authority — the limit was written on the face of every report,
and it still produced a wrong verdict on the very first run.

**v1.1 reads the run table for the seven jobs that keep one**, and falls back to the log only where
there is none; every line now says which it read.

**The second half is not yet fixed and is named for S260.** A log that is **empty** and a log that is
**old** are different facts — an empty one means the job has **never once spoken**, which is a far
stronger signal — and the tool still treats them alike. That conflation cost this session an hour on
`salts_refresh`, which turned out to be healthy.

---

## F-499 · FOURTEEN VENDORS THAT WERE THREE — S259 · THE ASSISTANT'S · RETRACTED IN PUBLIC

**Minted and retracted:** 15-Sep-2026, S259, within hours, on the owner's board.

Reported: *"14 of 39 vendors have no bank account on file."* Read from `purchase_vendor_contact`
alone. **Thirteen of the fourteen are the same firms under a second spelling**, already aliased by
`S263_VENDOR_LINK` to a register row that carries the details.

Held instead against **every supplier that has ever appeared on a bill** — the population the
question is actually about — the answer is **three**: AGARWAL SURGICALS AND MEDICALS (in the
register, no bank details, 9 bills), RAMA MEDICOSE (not in the register at all, 1 bill) and
**KUSHAGRA MEDICAL AGENCY** (not in the register at all, 1 bill, **and on no list of the owner's**).

**This is F-486's shape inside twenty-four hours** — a store declared on one of its lists. And it was
caught only because **the owner asked for the list rather than the number.**

> **A count over the wrong population is not an approximation of the right answer. It is a different
> question, answered correctly, and presented as this one.**

Corrected on the board with the error stated, not quietly replaced.

---

## F-500 · A SILENT WATCHMAN RECORDED AS A BROKEN ONE — S259 · THE ASSISTANT'S · RETRACTED

**Minted and retracted:** 15-Sep-2026, S259.

The Sanjeevni Book and the S259 board carried *"`S237_CERT_WATCH` has sent nothing in eleven days"*
as a watch gap. **Reading its code shows silence is exactly what it is built for:** `run()` alerts
only on a problem that is **new** (`keys - already`), and on a resolved one. A healthy month is a
silent month. It covers all eight live sites, `drmanojagarwal.com` first.

`job_pulse` then settled it from the box: **it ran at 06:40 on 15-Sep and runs daily.**

> **Absence of alarm from an alarm built to speak only on change is not evidence of anything.**
> The question was never "why is it silent" but "is it switched on" — and **F-496** is why that could
> not be answered from any backup.

The uptime monitor watching the `.in` rather than the `.com` is a separate item and **remains true**.

---

**Next free: F-501 · D528.**

**END OF FAULT → ACTION REGISTER v2.81 — F-500 is the last minted finding. The v2.80 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S259 close: the first 645611 bytes of v2.81 are byte-identical to v2.80 (md5 33e700e291f0182512bbe374aba2ed14). If you cannot see this line, you are reading a truncated copy.**

---

## F-501 · THE CAP MODEL WAS WRONG: A REPLACE IS CHARGED AS AN ADDITION — S259 close, minted S260 · THE ASSISTANT'S

**Found:** 15-Sep-2026, at the very end of the S259 close, after v2.81 was sealed. **Minted:** S260 open, 15-Sep-2026, as owed.

Project knowledge refused `claude/CANONICAL_MANIFEST.md` with *"this write would exceed the project's maximum size"* at **1,886,887 of 2,000,000**. Every close had assumed a canon swap is size-neutral — delete v(n−1), write v(n). **It is not: the store charges the replacement before it releases the old copy**, so at ~94 % full the one document Phase 0 opens on no longer fits. The manifest was refused three times; **fourteen documents were deleted mid-close to make it fit** — not a tranche, the cap making the decision.

**What was actually wrong** is that the linchpin lived in two stores at once. The repository copy at `deploy_kits/KB_canon_all/CANONICAL_MANIFEST.md` is byte-authoritative and is verified by md5 at every Phase 0; the project-knowledge copy was a convenience duplicate costing ~140 KB of a 2 MB cap at every single close.

**Action (D528, S260):** the project-knowledge copy is removed; Phase 0 step 3 reads the manifest from the clone. Measured before/after in the S260 build brief. **The durable fix remains the Sanjeevni/Marg split (S260 JOB 2).**

> **A store's cap is measured at the moment of the write, not after it. Plan every canon swap as an addition.**

---

## F-502 · A FILE WRITE REPORTED SUCCESS AND LEFT THE OLD BYTES ON DISK — S259 close, minted S260 · THE TOOL'S, THE ASSISTANT'S FOR NOT READING BACK

**Found:** 15-Sep-2026, S259 close. **Minted:** S260 open, as owed.

`deploy_kits\S280_JOB_PULSE_FIX\SUMS.md5` was rewritten from three rows to four so the kit's gate would cover the README it had shipped without. The commit returned that path in its `written` list. **The file on disk was still the three-row version**; the publish — which carried the README and not the sums — exposed it. The README committed in the same call landed byte-perfect. Five other `.md5` files written that close were checked afterwards and all five were correct: **no rule would have predicted which write to distrust.**

**The lesson was already a project rule and was not applied to this file:** a write to a store is not done until it is read back and hashed. Fourteen canon documents were read back; this one was not, because it was a two-line housekeeping file. F-383 family.

**Action:** every file committed to any store in S260 and after is read back and hashed in the same turn — the housekeeping files first, because that is where the exception lives.

> **Read back everything, or the exception is where the fault lives.**

---

## F-503 · "A PROJECT-KNOWLEDGE DOCUMENT CANNOT BE HASHED" — RETRACTED: IT CAN — S260 · THE ASSISTANT'S · A LIMITATION THAT WAS NEVER TESTED

**Found and closed:** 15-Sep-2026, S260 open.

`START_HERE_SESSION_260` carried, as an open method fault, that *"a project-knowledge document still cannot be hashed by the tools, so provenance for a moved document is by construction, not by byte comparison."* **Tested at the S260 open and found false.** Reading `claude/CANONICAL_MANIFEST.md` from project knowledge returned the document as a local file: **559,757 bytes, md5 `a893da8c28797f531bb41d8c7e8e036c` — byte-identical to the repository copy** whose row the canon gate had just passed (563 rows, 563 OK).

**What this changes:** a document leaving project knowledge can be proven against its surviving copy **by hash**, not by construction. F-208's caution stands for a document that comes back inline as text (small documents) — that path is tested per tranche, and the result recorded, before it is relied on.

> **A limitation is a claim. Measure it before it governs a session.**

---

**Next free: F-504 · D529.**

**END OF FAULT → ACTION REGISTER v2.82 — F-503 is the last minted finding. The v2.81 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S260 open: the first 656761 bytes of v2.82 are byte-identical to v2.81 (md5 0710979ad8d3e6283435f2373a24729c). If you cannot see this line, you are reading a truncated copy.**

---

## F-504 · 184 DOCUMENTS EXISTED IN PROJECT KNOWLEDGE AND NOWHERE ELSE — S260 · THE PROJECT'S, EVERY CLOSE SINCE S205

**Found:** 15-Sep-2026, S260 open, by the "ask what no store holds" pass. **Minted:** 17-Sep-2026, S260 resumed.

**The finding.** Of 314 project-knowledge documents, **184 had no copy in any other store** — checked by basename against the repository (every commit, 677 commits, 2,617 basenames ever committed), `D:\Downloads\ClaudeCowork\MANIFEST.md5` (2,802 rows), and all nine zips in `01_COLD_BACKUPS` and `03_WORKING_PAPERS\S212` (606 entries). The F: mirror is a zip of ClaudeCowork and holds nothing more. Among them: every S207–S242 build paper, every UNATTENDED run report, `START_HERE_PROMPT_v8`, `END_OF_SESSION_PROMPT_v13`, the S241 runbook v172, four START_HERE_SESSION entry points, the four SANJEEVNI_* registers and records, `MARG_REPORT_REGISTER`, `KB_EXTENSION_PLAN`.

**Why.** The close routine wrote the build documents into project knowledge and the canon into the repository, and the ClaudeCowork mirror carried the canon snapshot and the close report — never the build papers. Each store was correct about what it excluded; the gap lived between them, exactly as §1.5 warned. Meanwhile the cap pressure of F-501 was pushing closes to *delete* project documents to make room — at S259, 35 were removed and checked one by one; the check happened to hold, but nothing in the routine required it to.

**The rescue.** 180 of the 184 (two `.docx` and two `templates_snapshot*.json` excluded) were copied to `D:\Downloads\ClaudeCowork\01_RESCUED_FROM_PROJECT_KNOWLEDGE\S260_RESCUE\`, each staged back and re-hashed against `S260_RESCUE_SUMS.md5` there: **180 of 180 OK.**

**The method, stated so it is not over-trusted.** Project knowledge returns TEXT, not a file, so this was a transcription by sub-agents, not a byte copy. Ten documents were transcribed twice independently: **nine byte-identical; one had two paragraphs re-wrapped at different line breaks with every word intact** (`S219_M1_LIVE_APPLY_PATH_READ`; the original was read a third time and the correct wrapping kept). So the rescued copies are word-faithful, and byte-faithful in the large majority; a minority may differ from the original in paragraph line breaks. **They are backups. No project-knowledge original is deleted on their strength.** Provenance for a document leaving project knowledge is by hash against a survivor (F-503) or by a second full transcription diffed against the first — never by this folder alone.

**Action.** (1) The close routine gains a row: every document written to project knowledge is also written to `ClaudeCowork\03_WORKING_PAPERS\S###\` in the same close, and the "no store holds" comparison runs against the project document list at every open. (2) `START_HERE_PROMPT_v10` carries the rule. (3) The Sanjeevni/Marg split (S260 JOB 2) stages from this folder.

> **A document that exists only in the store with the cap is the first thing the cap will eat.**

---

**Next free: F-505 · D529.**

**END OF FAULT → ACTION REGISTER v2.83 — F-504 is the last minted finding. The v2.82 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at S260: the first 660997 bytes of v2.83 are byte-identical to v2.82 (md5 076061b3e784654505e39ee5ca0e4247). If you cannot see this line, you are reading a truncated copy.**

---

## F-505 · ONE CANON FILE, TWO LINE ENDINGS: THE GATE ON THE PC REFUSED WHAT THE GATE ON GITHUB PASSED — S260 close · THE TOOLING'S

**Found:** 17-Sep-2026 05:06 IST, by the canon-sums nightly (`CANON_SUMS_LATEST.txt`): *HASH MISMATCH deploy/push_kit.bat — row 01095763…, now 0e6d8058…*, and it refused to rewrite, as it should.

**What it was.** The text of the two files is identical. The repository copy (and the row) carry CRLF line endings; the copy in `D:\dr-manoj-git\…\KB_canon_all\deploy\` carries LF, with a modification time of 18-Aug-2026. The S260 open's gate ran on a fresh clone of GitHub and passed 563 of 563; the nightly ran on the PC's working tree and failed one. **Two stores, one text, two hashes — and each gate was right about the bytes in front of it.**

**Action.** The repository bytes (CRLF, `01095763f49d263aafa9232972443f12`) were written onto the PC at this close and read back. A `.bat` runs identically either way; nothing live was affected.

> **A hash gate compares bytes, not text. When two gates disagree on one file, look at the line endings before the content.**

---

**Next free: F-506 · D529.**

**END OF FAULT → ACTION REGISTER v2.84 — F-505 is the last minted finding. The v2.83 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S260 close: the first 664411 bytes of v2.84 are byte-identical to v2.83 (md5 729bf447e79e3b66b7f20267677f9358). If you cannot see this line, you are reading a truncated copy.**

## F-506 · THE SHELF PICKED ITS "NEWEST REGISTER" BY STRING SORT AND READ A SUPERSEDED ONE FROM v5.100 ON — S261 · THE TOOLING'S (S277), THE ASSISTANT'S FOR WRITING IT

**Found:** 17-Sep-2026 07:40 IST, by running the shelf (`build_papers_index.py`, S277) at the S261 open: its corpus line named `KB_Register_v5_99_S259close.md` as *the newest KB Register* while `v5_100` and `v5_101` sat in the same folder.

**What it was.** The tool chose the newest Register as the **last name in a string sort** of `KB_Register_v5_*.md`. `"v5_99"` sorts after `"v5_100"` because `'9' > '1'`. From the moment the Register reached v5.100 (the S260 close) the shelf silently read v5.99 — a superseded document — and printed it on the page as the newest. The figure happened not to change this session (205 of 406 either way), which is exactly why nobody would have noticed: **the same family as F-491 — a tool whose output looks right while it reads the wrong input.**

**Action.** One line changed: the sort key is now the number after `v5_`. Applied on manojz at S261 through the file tools (backup `build_papers_index.py.bak_S261_dfcf659c`), `py_compile` clean, re-run — corpus now names `KB_Register_v5_101_S261close.md`. The fixed source is in the repository as `deploy_kits/S281_PAPERS_SORT/` with its proof; **nothing for the owner to run.** `_kbtools\SUMS.md5` row updated; the S277 kit's own sums left as published.

> **Any "newest by name" pick over versioned files must sort on the number. A string sort is right until the number gains a digit, and then it is wrong in silence.**

---

**Next free: F-507 · D529.**

**END OF FAULT → ACTION REGISTER v2.85 — F-506 is the last minted finding. The v2.84 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at S261: the first 665936 bytes of v2.85 are byte-identical to v2.84 (md5 aa47eff135442a8e55437d324d2874a0). If you cannot see this line, you are reading a truncated copy.**


## F-507 · A LIVE CONFIGURATION FILE DRIFTED FROM ITS REGISTER PIN AND NOTHING SAID SO FOR TWO WEEKS — S262 · THE ESTATE'S

**Minted:** 17-Sep-2026, S262 open. **Class:** canon / measurement. **Severity:** the health page's own configuration was unrecorded.

`/root/finance/freshness_legs.json` is pinned in the Register at `ad223bc613d192c0c8774f0c78c65296` (S230, *"read back from the box and confirmed at the S231 open"*). Its pin note says the file is *"configuration, not code: a window is widened or a leg retired HERE, never in `freshness.py`."* The 17-Sep nightly bundle — the first that carries the file (S273) — holds it at `eaa976911a5d80d618f3b4015e90fdf7`. **Someone widened a window or retired a leg on the box, and the Register did not hear.** Until S273 the file had no off-box copy at all, so no check could have seen it; S273 made the comparison possible and S262's open was the first to run it: 84 pins matched, this one did not, 0 absent.

**Action.** The Register pins the bundle's bytes from this close (the box is the authority for a file that is edited on the box). **The diff — which legs moved — is the parent project's** (the file is clinic-core; D525's seven never-fired checks live in it) and is named in its backlog. **The rule:** a file whose pin note says *edited here* must be re-pinned at every close that can read it, because *edited here* is a promise that the edit will be recorded, and a promise is not a mechanism.

---

## F-508 · AMIR'S BILL LIST KEPT BILLS THAT MARG NO LONGER HAD — S262 · THE ASSISTANT'S (S246)

**Minted:** 17-Sep-2026, S262. **Class:** design. **Severity:** a corrected bill could never leave his screen.

S246 built the list Amir answers *Theek hai / Theek nahi* on by joining `purchase_bill` to every `purchase_export` that carried it, and grouped by the bill so a bill riding in five cumulative exports showed once. It never asked whether a **live** export still carried the bill. A bill Marg re-keyed, or one Amir corrected under the right supplier, lives on in the superseded export's rows — so the old wrong bill stayed on his list as *pichhla baaki* for ever, with the corrected bill beside it as a new one. Nobody saw it because every September bill happened to be answered; the S285 rehearsal against the nightly database found no bill in that state today, which is luck, not design.

**Repair — S285:** the list shows only bills a live BILLWISE export still carries; the earliest export still decides *seen_day*, so *pichhla baaki* keeps its meaning. Proven against the untouched module: before, a bill existing only in a superseded export haunted the list; after, it is gone and nothing else moved.

**The rule:** *superseded* must reach every reader of a table, not only the month total. S225 taught the month to forget a superseded export; the list that a person answers was left remembering.

---

## F-509 · TWO SESSIONS OPENED FROM ONE START_HERE AND BOTH WERE ENTITLED TO THE SAME NUMBER — S262 · THE ROUTINE'S

**Minted:** 17-Sep-2026, S262. **Class:** canon / numbering. **Severity:** one series, two writers, no lock.

`START_HERE_SESSION_261` said *"Next free: … Session 261"*. The parent project's session read it at 06:41 and became S261. The Sanjeevni project's first session read the same file at 06:48 and would have become S261 too — the split (D528) had just created a second reader of one number series. It became S262 only because it read the owner's board *before* the reserved numbers and found the parent's status line there.

**Action — the rule, now written:** **a session claims its number on the board as its first act** (`_claude_status.session`, with the time), **and reads the board before it reads the reserved numbers.** Kit numbers are claimed the same way (this session claimed S282 and S284 there, later S285, and the parent took S281 and S283 — no collision). `START_HERE_SESSION_263` carries the order; this entry makes it a fault so it survives the next rewrite of that file.

---

**Next free: F-510 · D532.**

**END OF FAULT → ACTION REGISTER v2.86 — F-509 is the last minted finding. The v2.85 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at S262: the first 667919 bytes of v2.86 are byte-identical to v2.85 (md5 0774bd8f195d4d9fec430132b100f606). If you cannot see this line, you are reading a truncated copy.**


## F-510 · THE ASSISTANT WROTE CLOCK TIMES FROM ITS SENSE OF ELAPSED TIME, NOT FROM A CLOCK — AND PUBLISHED THEM — S262 close · THE ASSISTANT'S

**Minted:** 17-Sep-2026 08:45 IST, S262, thirty minutes after its own close. **Class:** measurement (the F-472 / F-479 family). **Severity:** every timestamp of a close was wrong by one and a half to three hours, in canon, published.

The S262 close wrote *"installed ~09:25"*, *"~09:30"*, *"~10:10"*, *"06:48 → ~11:15"*, *"closed 11:30"* and *"live by noon"* into the Register, the runbook, START_HERE_264, the brief, the close report, `OWNER_TODO_LIVE`, the manifest and the owner's board. The commit log says the kits were published at **07:35:56** (`e216066`) and **07:53:34** (`a633a78`), the installs followed within minutes, and the close itself was published at **08:38:09** (`2214298`). The shell's own clock had been read once, at the open (06:48), and never again; every later time was the assistant's impression of how long the work had taken — and the work had gone faster than it felt. Nothing checked the figures because nothing in the close routine asks for a clock read at the moment a time is written.

**Action.** Every file corrected in place at 08:45 with a visible note; the Archive carries an erratum (§S262a, v1.100); a second publish owed. **The rule, into the next `END_OF_SESSION_PROMPT` revision:** a clock time written into canon is READ — `date` in the shell, a commit stamp, a file mtime — at the moment of writing, never estimated; and A16b's read-back of the publish compares the commit stamp with the close's own stated times.

---

**Next free: F-511 · D532.**

**END OF FAULT → ACTION REGISTER v2.87 — F-510 is the last minted finding. The v2.86 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at S262 (08:45): the first 672332 bytes of v2.87 are byte-identical to v2.86 (md5 149bb6df9930ae396be97ba0cb756295). If you cannot see this line, you are reading a truncated copy.**


## F-511 · GIT RUN IN THE PC SHELL LEFT ITS LOCK FILES INSIDE THE REPOSITORY — AND ONE WOULD HAVE FAILED THE OWNER'S NEXT PUBLISH — S263 · THE ASSISTANT'S

**Minted:** 17-Sep-2026, S263 close (found 08:13 IST). **Class:** tooling (the F-233 family — *never run git against the mounted repo*). **Severity:** would have made `PUBLISH_ALL.bat` refuse with "index.lock exists"; caught before the owner's double-click.

`device_bash` may create files in a connected folder but may not unlink them. `git fetch` at the S263 open (07:43) left `.git/objects/maintenance.lock`; `git status` at 08:13 left `.git/index.lock`. git reported both as "unable to unlink … Operation not permitted" and carried on. The routine already said *never run git against the mounted repo* (A16, F-233) — that rule was written for the file-transfer era and was not re-read when the shell came back at S261; the S263 open ran git twice under it.

**Action.** Both locks (empty) moved — not deleted — to `D:\dr-manoj-git\_to_delete\S263_git_locks\` at 08:14; `find .git -name "*.lock"` = 0 before every publish named since. The publish record is now read without git: `.git/logs/HEAD`, `refs/heads/main`, `refs/remotes/origin/main` and, where a file's committed bytes are needed, the object read with zlib (used at 09:06 to restore S286). **Rule, into `END_OF_SESSION_PROMPT_v15`: no git command in `device_bash` against a connected folder, ever; read the repository's files, not its commands.**

---

## F-512 · A KIT FOLDER WAS EDITED AFTER IT COULD BE PUBLISHED — THE OWNER PUBLISHED AND INSTALLED THE HALF-BUILT VERSION, AND THE EDITS THEN OVERWROTE THE PUBLISHED BYTES — S263 · THE ASSISTANT'S

**Minted:** 17-Sep-2026, S263 close. **Class:** process (the F-460 family — a published file is never rewritten). **Severity:** no harm to the box — the version installed was complete and verified for what it claimed — but the repository folder on manojz briefly held bytes that matched neither GitHub nor the server.

`S286_ASSETAPP_BACKUP` was built, gated and announced at ~08:55 as a local-only backup. The owner then asked for the off-site copy, and the assistant began rewriting the script **inside `deploy_kits/S286_ASSETAPP_BACKUP/`**. The owner, meanwhile, double-clicked the publish (commit `4701361`, 09:02) and installed from GitHub (09:04) — the first, local-only version, `9fdc66a3…`. The in-progress v2 files then overwrote that folder's published bytes on the PC. **A deploy-kit folder is the publish's pickup area: anything written there is one double-click from GitHub, and once published it is frozen.**

**Action.** The four published files restored from git's own objects (zlib read of `4701361`'s tree; `SUMS.md5` 4/4 = published); the off-site version shipped as its own kit, `S287_ASSETAPP_DRIVE`, gated on S286's pin, installed 10:00. **Rule, into `END_OF_SESSION_PROMPT_v15`: a kit is built in scratch and copied into `deploy_kits/` only when final and gated; once a kit could have been published, a change to it is a new kit.**

---

**Next free: F-513 · D534.**

**END OF FAULT → ACTION REGISTER v2.88 — F-512 is the last minted finding. The v2.87 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S263 close: the first 674356 bytes of v2.88 are byte-identical to v2.87 (md5 3b3c5637546ce17377bb925d60dfa627). If you cannot see this line, you are reading a truncated copy.**

---

## F-513 · A PIN THAT WAS A PREDICTION STOOD "DECLARED-PENDING" FOR THIRTY-THREE SESSIONS, AND THE BOOK CALLED A FILE UNBACKED ON ITS STRENGTH — S264 · THE ROUTINE'S, THE ASSISTANT'S (S258)

**Minted:** 17-Sep-2026, S264 (the Sanjeevni project). **Class:** canon / measurement. **Severity:** the Register carried a wrong hash for the live item spine from S229 to S264, and v1.2 of the Sanjeevni Book built a "no off-box copy" finding on it.

**What was found.** `/root/deploy/repo/deploy_kits/S229_ITEM_SPINE/marg_spine.py` is pinned in the Register at `9a08b2c4c9adc2283fb371f350bbb043` **DECLARED-PENDING** — S229's own words: *"the two VPS rows are PREDICTIONS from the published kit bytes and are DECLARED-PENDING until read back from the box at the S230 open."* No close read them back. The 17-Sep 01:35 bundle — the first to carry `/root/deploy` tools (S273) — holds that path at **`b5956f59464671f23dc376391f9e8bae`**, and `/root/finance/marg_spine.py`, the copy `spine_cadence.py` actually runs (`cd /root/finance`), at the same hash. **The repository's `deploy_kits/S229_ITEM_SPINE/marg_spine.py` is `b5956f59…` too.** The box and the repository agree; only the pin disagrees, and it was never anything but a guess. The sister row, `marg_spine_schema.sql` `a39b2fcc…` DECLARED-PENDING, matches the bundle at both paths and is upgraded to verified.

**The consequence.** At S258 the Book's §11.2b held every live pin against the bundle; `marg_spine.py` was not in it, so the row was held against GitHub instead — against the **pin**, which matched nothing — and the Book recorded *"the live item spine — the whole of §4 — exists byte-exact nowhere but the box."* S273 was then built to carry it. The file was in the repository the whole time. Nothing was lost; a kit was built for a gap that was a wrong number.

**Why it stood.** A DECLARED-PENDING row looks like a row. `verify_live_pins.py` and the S258 bulk check treat a row's hash as the claim; the word PENDING in the note column is read by no tool. Thirty-three closes passed it as *carried*.

**Action.** The Register (v5.104, this close) re-pins both S229 VPS rows to the bundle's bytes, visibly, with the prediction recorded beside them; the Sanjeevni Book v1.3 withdraws the §11.2b line. **Rule, into the routine at its next revision (A11b): a DECLARED-PENDING pin is a promise to read back at the next open, and a promise older than one close is a finding at every close until it is kept — the pin check states the count of PENDING rows beside its three counts.**

---

**Next free: F-514 · D534.**

**END OF FAULT → ACTION REGISTER v2.89 — F-513 is the last minted finding. The v2.88 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S264 close: the first 677818 bytes of v2.89 are byte-identical to v2.88 (md5 028cfa0fbfd66c9b925e3ecd3563d253). If you cannot see this line, you are reading a truncated copy.**

---

## §S265 — 17-Sep-2026 · F-514 … F-520 (the parent project; four the assistant's: F-514, F-515, F-516, F-520)

## F-514 · THE EXIT REGISTER'S LAST STEP WAS A TICK THAT MADE NOTHING TRUE — AND IT SPOKE IN THE JOINING FLOW'S WORDS — S265 · THE ASSISTANT'S (S207/S208 design)

**Minted:** 17-Sep-2026, S265. **Class:** workflow / record vs state. **Severity:** a leaver stayed active in the staff master, the attendance register and the code register while his exit read 6 of 7; the owner stopped on the last step, confused.

**What was found.** The owner walked Pravesh's exit (`EXIT-2026-0001`, recorded as *parvesh*) on 16-Sep and ticked steps 1–6. On 17-Sep he wrote *"i tried your flow for exit a leaver but was confused midway."* Read live the same hour: `/root/staff_master.csv` still carried **Pravesh, code 17, `active` Y** (13 people); `staff_register.db` still had him active (the S295 attendance map listed him as a staff member without a login); and **code 17 had never been retired** — `BIOMETRIC_REMOVED` retires the record's `emp_code`, and the record carried none. The seventh step's label came from `STEP_LABEL`, one dictionary for both flows: *"staff master rebuilt and the person appears"* — the joining wording, on a leaving.

**Why.** The register was designed (S207) as a list of ticks with names against them. Four of its seven exit steps describe a change to another store; none of them makes the change, and nothing reads the stores back. *A tick is a claim, and a claim with a name against it still describes nothing about the machine.*

**Action.** `S298_PRAVESH_EXIT` (built 17:32, **not yet published**): `joiner_app.py` `3e213e4d…` → `6305b05d…` gives an EXIT its own step-7 words — *"staff master and attendance register updated — the person no longer appears"* — and says the assistant does it; `exit_s298.py` makes the three changes for Pravesh (the staff master's `active` Y→N on his one row, one byte; the register's last working day 31-Aug-2026 and inactive, by the register's own resignation rule; code 17 retired), ticks step 7 and closes the record — **gated on August 2026 being locked** (read live: locked by manoj 13-Sep 12:44:38), because `att_month_report.py` drops an inactive row from every month it runs. **Rule:** a step that names a change in another store is done by whoever can make and read back the change, and is ticked only after the read-back; a register's words are per flow, never shared by lookup.

## F-515 · TWO OPEN CHATS BUILT TWO KITS UNDER ONE NUMBER — THE BOARD CLAIM WAS WRITTEN AFTER THE FOLDER, NOT BEFORE — S265 · THE ASSISTANT'S

**Minted:** 17-Sep-2026, S265. **Class:** numbering / concurrency (the F-509 family). **Severity:** caught before any publish; nothing reached GitHub under the doubled number.

**What happened.** The board's `nextFree` read *kit S297* at 17:10. S265 built `S297_PRAVESH_EXIT`, copied it into `deploy_kits\` and only then wrote its claim — and the write was refused: the document had moved to a newer version, carrying the Sanjeevni chat's `kit_S297: S297_STOCK_CHECK_HUB` claim, written while the kit was being built. The S265 kit was renumbered **S298** in scratch, re-gated (selftest 12, walk 6, rehearsal), placed as `S298_PRAVESH_EXIT`, and the S297-numbered copy moved out of the repository to `D:\dr-manoj-git\_to_delete\S297_PRAVESH_EXIT_renumbered_S298` — never published.

**Why.** Rule 14 (F-509) makes the board the lock for **session** numbers. Kit numbers were read from the board but claimed at the end, as a record, not at the start, as a lock.

**Rule:** a kit number is **claimed on the board before the scratch folder is named**, and a board write refused for its version is read first as a possible claim collision, not as noise.

## F-516 · THE STAFF SELF PAGE STILL LINKS "MY MONTH" AFTER THE OWNER RULED TODAY ONLY — S265 · THE ASSISTANT'S

**Minted:** 17-Sep-2026, S265. **Class:** ruling vs screen. **Severity:** low — a staff member can open a month grid the owner ruled out of their hands.

The owner ruled at S265 (D537): the staff PWA shows **today only**. The self page (`/register/me`, `staff_register.py` `13a26205…`) was extended the same session (Mark my exit) and still carries `📅 My month` at its head (S200). Noted to the owner the same evening; he then refined the ruling (D540: past days show **absents and leaves**, display only). **Rule:** when a ruling narrows a screen, the build that touches the screen next removes what the ruling excludes, in the same kit — a known contradiction is not carried past the kit that could fix it.

## F-517 · THE FRESHNESS LEG FOR DOCTERZ STILL SAYS "RUN BY HAND" — S265

**Minted:** 17-Sep-2026, S265. **Class:** stale watch configuration.

`/root/finance/freshness_legs.json` (`eaa97691…`) describes the Docterz leg as *"docterz_ingest.py is run by hand today … 200 h … Tighten to 26 h the day it gets a timer."* It has had a timer since S238 and runs every ten minutes all day since **S291** (17-Sep). The leg therefore stays quiet for eight days of silence. **Action:** the next kit that touches the health legs tightens it (26 h) and rewrites the note. **Rule:** a kit that gives a job a schedule rereads every watch that names that job.

## F-518 · THE NIGHTLY CODE BUNDLE CARRIES THE TWO MACHINE TOKENS OFF THE SERVER — S265

**Minted:** 17-Sep-2026, S265. **Class:** secrets at rest. **Severity:** two live tokens in two more stores.

Read from the bytes of the 17-Sep 01:35 bundle (`code_nightly.tar.gz` `10421813…`, extracted in the cloud workspace): `etc/systemd/system/clinic-finance.service` carries **`FINANCE_CRON_TOKEN` and `FINANCE_MARG_TOKEN` as `Environment=` lines with their values** (names read, values not printed). The bundle is written to Google Drive (`FinanceDB_Backups`) and copied nightly to `D:\Downloads\_kbtools\vps_code\` on manojz. F-465 (S255) named the unit file a token store; F-496 (S259) added the units to the bundle's purpose; neither counted the bundle itself. **Action:** `code_bundle.py` masks `Environment=` values whose names end in `TOKEN`, `KEY`, `SECRET` or `PASSWORD` (the name kept, the value replaced), and the F-456 rotation list gains both copies. **Rule:** a backup that copies configuration inherits every secret the configuration holds; the secret wall filters lines, not only files.

## F-519 · THE AUGUST LOCK DESK'S CARD TOTAL AND ITS TABLE TOTAL DISAGREE — OBSERVED ON THE PAGE, NOT YET IN THE STORE — S265 · OPEN

**Minted:** 17-Sep-2026, S265. **Class:** money display — **unconfirmed (A0: a page read as text is not the data).**

Read as text by a sub-agent (figures withheld from the conversation) at `https://attendance.dr-manoj.in/register/salary?ym=2026-08`: the LOCKED card (*"Official run for 2026-08 · locked by manoj on 2026-09-13 12:44:38"*) shows a total payout that is **not the figure in the TOTAL row of the staff table beneath it**. Candidates, none tested: the card reads `locked_run.total_payout` (frozen at lock) while the table is recomputed from today's engine and ledger (advances booked since 13-Sep, the S238 advance rule); or the two sum different columns. **Action:** the next session reads `locked_run` for 2026-08 against the engine's run for the same month from the database, names which surface is the paid figure, and says so on the page. No money moves on a page reading.

## F-520 · A READ-ONLY GIT COMMAND WAS RUN IN THE PC SHELL AT THE OPEN, BEFORE THE RULE WAS READ — S265 · THE ASSISTANT'S

**Minted:** 17-Sep-2026, S265. **Class:** the F-511 family.

At the S265 open a `git status` / `git log` ran in `device_bash` against `D:\dr-manoj-git\drmanoj-clinic-automation` before rule 15 (F-511) had been read. No lock file was left (checked by `find`, not git); git's index cache was rewritten at 13:03. Every later read used `.git/logs/HEAD` and the refs. **Rule restated:** the rules are read before the first command, not after it.

---

**Next free: F-521 · D541.**

**END OF FAULT → ACTION REGISTER v2.90 — F-520 is the last minted finding. The v2.89 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S265 close: the first 680789 bytes of v2.90 are byte-identical to v2.89 (md5 f3a0840bccd3ff35e8ef44f4d8184411). If you cannot see this line, you are reading a truncated copy.**


## F-521 · THE ASSISTANT WROTE ITS KIT NUMBER OVER THE OTHER SESSION'S BOARD CLAIM — S266 · THE ASSISTANT'S · CLOSED SAME HOUR

**Minted:** 17-Sep-2026, S266. **Class:** the F-509 family — the board is the lock, and it only works if it is read.

At 21:36 the parent session (S267) claimed **kit S303** on the System Board, writing `kit_S303` and moving `nextFree` to S304. At 21:37 this session wrote its own export-watch kit into the **same key**, from a board copy it had read at 21:3x **without looking at the key before writing** — so the parent's claim text was overwritten and two different kits carried one number, one of them already in `deploy_kits\`.

**Found** at 21:46, by diffing the board copy this session had read against the board as it then stood, before either kit was published. **Repaired in the same minutes:** the parent's `kit_S303` text was restored verbatim from the copy; this session's folder `deploy_kits\S303_EXPORT_WATCH_DOOR` was moved to `D:\dr-manoj-git\_to_delete\S303_EXPORT_WATCH_DOOR_renumbered_S305\` while `HEAD` still stood at `04ed587` (21:24:26) — **nothing had been published**; the kit was rebuilt as **S305_EXPORT_WATCH_DOOR**, its in-code tags renamed S303 → S305, and re-rehearsed. The owner was told in one line that the S303 line he had been given was withdrawn.

**Rule, restated and now obeyed in both directions:** *claim your number on the board as your FIRST act* (F-509) **and read the board's keys immediately before writing one** — `nextFree` alone is not the lock, because the other session may have claimed since the copy in hand was taken. Every later claim this session made (S307, S308, and the D/F numbers of this close) was preceded by a fresh read and a check that the key was absent.

## F-522 · THE STOCK CHECK'S STEP 7 READ "DONE" FROM THE COUNTING CLOSE — S266 · THE ASSISTANT'S · FIXED

**Minted:** 17-Sep-2026, S266. **Class:** a screen that announces a proof nobody made.

The hub built at S297 (live as S299) gave step 7, *"The proof — Marg agrees with the shelf"*, the state `done` whenever the count had been **closed** — and count #1 was closed on 06-Sep at 14:43 the moment its sheets were finished. Two different things wear the word *closed*: the counting is over, and Marg has been proved to agree with the shelf. The page read the first and said the second. **It stood wrong from S297 (17-Sep evening) until S304 (17-Sep 22:2x)** — no vouchers existed in that window, which is exactly why a green step was worth nothing.

**Fixed by `S304_COUNT_PROOF` (D543):** step 7 is computed from the vouchers entered and Marg's own next export, and reads *wait* until there is something to prove. **The general rule:** a step's state is derived from the evidence that step is about, never from a neighbouring fact that happens to be available.

## F-523 · A SECOND COMMIT TO THE SAME PATH LANDED THE OLD BYTES — S266 · TOOLING · RULE WRITTEN

**Minted:** 17-Sep-2026, S266. **Class:** a write that reports success and changes nothing (the F-383 family).

While building `S302_DAILY_PRINT` the installer was corrected in the cloud workspace and committed to the PC a second time, to the path it had already been written to. `device_commit_files` reported the file written, and its mtime on the PC was the moment of the commit — but **its bytes were the previous version** (`2fa8c332…` where `eb8e7f1a…` was expected), and stayed so through a re-read eight seconds later. The stale copy still carried the `py_compile` line the correction had removed, which is how it was caught: a `__pycache__` folder appeared inside the kit folder where the fixed installer could not have made one. Repaired by editing the file **in place on the PC** and re-reading its md5.

**Rule:** a file committed to the PC is **read back by md5 on the PC before it is used**, and a kit's sums are taken from those read-back bytes. When a path must be written twice, prefer editing it in place on the machine that holds it. *A commit tool's "written" is a report, not a proof — A0.*

## F-524 · THE DESK CALLED THE ALL-DIFFERENCES SHEET "HAND SHEET FOR DARPAN" — S264 post-close · FIXED, ROWED HERE

**Minted:** 17-Sep-2026, S266 (owed from the S264 post-close work). **Class:** a label that made a person act.

On the decision desk the footer link to the **all-differences** sheet — the owner's own copy, every line including surpluses — was worded as a hand sheet for Darpan. The owner printed it and it was the wrong sheet: Darpan's list is cut on the loss page, carries shortages only, and never shows a surplus or a decided line. **Fixed by `S294_DARPAN_RECONCILED`** (live 17-Sep ~16:10), whose footer now reads *"All differences — your copy (not for Darpan)"* beside *"Darpan's list — cut it on the loss page"*. Rowed here because a wording that sends a person to the wrong paper is a fault, not a preference: **a link's words are the only instruction a busy reader gets.**

---

**Next free: F-525 · D544.**

**END OF FAULT → ACTION REGISTER v2.91 — F-524 is the last minted finding. The v2.90 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S266 close: the first 689187 bytes of v2.91 are byte-identical to v2.90 (md5 89139788797869d8f9c1d37a31c7c938). If you cannot see this line, you are reading a truncated copy.**

## F-525 · AN INSTALLER'S HEALTH GATE DEMANDED 200 FROM A LOGIN-GATED ROUTE — S266 post-close · THE ASSISTANT'S · FIXED AND LIVE

**Minted:** 18-Sep-2026, 04:45 IST, S266 (re-opened after its close). **Class:** a green check turned into a false red — a gate that tested the wrong thing (the F-455 family: comparing a value in a form the system never produces).

`S307_SUPPLIER_CHECK_2` installed correctly on the box and was then rolled back by its own installer. Every step passed: the live pin was exact at `b3c20319`, the patcher produced the predicted `00c443cbce0b07ba763bbe655a3e3c58`, the file compiled, and the walk was green on a scratch copy of the live database (508 bills, 4 with a same-number twin at 0 same amount, 5 returns, 0 warned). Then, after the restart, the installer asked:
...
and got **302**. It called that red and restored `amir_day.py` byte-identically to `b3c20319`.

**The 302 was correct.** `finance_app.py` lists exactly two health routes in `PUBLIC_PATHS` — `/finance/healthz` and `/finance/purchase/api/healthz` (S224, by contract). `/finance/amir/api/healthz` is not among them, so the `before_request` gate redirects an unauthenticated `curl` to the login page, and has always done so. `S285_SUPPLIER_CHECK` probed the same route and **printed** the code for information without testing it; S266 copied the line into a new installer and made it a pass/fail. Nothing on the box was ever wrong, and nothing was harmed: the restore was byte-identical and the service came back on the S285 file.

**Why it matters beyond the one kit.** A red-walk that fires on a healthy box is worse than no check: it teaches the operator that red means nothing. It also cost a publish cycle of the owner's — his double-click is the scarce thing in this estate.

**Fixed by `S311_SUPPLIER_CHECK_2B`,** live 18-Sep from the owner's one line after publish `a8c6c8c`. S307's folder was already published and therefore frozen (F-512), so the repair is a new kit that carries **S307's payload byte-identically, under S307's own file names** (`patch_supplier_check2_s307.py`, md5 `c01a080c5bc8b2e0c1b0751ba21a07fe`) — the code that landed is the code S307 had already proven on the box. Only the installer differs:

```
c1=$(curl -s ... http://127.0.0.1:8106/finance/healthz)          # public: must be 200
c2=$(curl -s ... http://127.0.0.1:8106/finance/amir/step/4)      # gated: 200/302/401 all pass
```

The box answered `finance 200 · Amir's day 302 (302/401 = the login gate, expected)` and the install went 7/7 green to `00c443cb`.

**Rule, standing from now:** an installer's health gate **either** hits a route that is in `PUBLIC_PATHS` and requires 200, **or** accepts 302/401 on a gated page as the login gate it is. A probe that cannot distinguish "the app is down" from "the app asked me to log in" is not a health check. `S308_PURSUE_EVIDENCE` already did this correctly and is the model; the rule is written into the runbook at **v188**.

---

**Next free: F-526 · D544.**

**END OF FAULT → ACTION REGISTER v2.92 — F-525 is the last minted finding. The v2.91 and earlier end-markers above are retained in place as truncation-proofs. The file is pure-append: the first 694568 bytes are byte-identical to v2.91 (`8b1ee830f9b4b705d1492c9ecefa9a50`).**

## F-529 · A LOGIN THAT IS CASE-SENSITIVE WHILE ITS STORE IS LOWERCASE — S267 · OPEN

**Minted:** 18-Sep-2026, S267. **Class:** authentication trap. **Found by:** the owner — a staff member could not sign in after the owner reset his password twice.

`/portal/login` reads the username as `(request.form.get("user") or "").strip()` and hands it to `clinic_users.verify_password` **without lowercasing it**, while the reset path in the same file writes and reports the name as `user.strip().lower()`. The store holds one row, `awdhesh`, lowercase, active — read from the Manage Users page by a sub-agent, no duplicate and no capitalised variant. A single capital letter therefore reads as *"Wrong username or password"*, and there is no lock-out or last-sign-in column to tell anyone otherwise. On a phone keyboard this is the default failure. It was cleared by re-typing the name in lowercase with a password of at least six characters — the reset form refuses a shorter one and changes nothing.

**Action:** one line in the portal login (`.lower()` before verify) and the same in the staff register's local login. **Held, not built, at this close:** `portal.py` was being changed by the Sanjeevni chat all evening (`e13adbd6` at S299); the fix goes in on the first parent kit that owns that file again. **Rule:** a name that is stored lowercase is compared lowercase — anywhere a human types it.

## F-530 · THE LOGIN STORE'S OWN CODE IS IN NO STORE — S267

**Minted:** 18-Sep-2026, S267. **Class:** the F-518 family — a backup rule that protects a secret and loses the code with it.

`/root/portal/clinic_users.py` (Register pin `2e85a7c8`) is excluded from the nightly code bundle: the portal source entry drops any file whose name contains `users` (it exists to keep the password store out). The exclusion is right; its side effect is that **the code that decides whether a login succeeds exists nowhere but the box** — not in the bundle, not on GitHub, not in the encrypted state bundle. F-529 had to be diagnosed from `portal.py` alone for exactly this reason.

**Action:** name it in the *what no store holds* list at every close until the bundle carries the code file while still excluding the store's data (they are two different files); until then, treat any change to it as unrecoverable from backup.

## F-531 · A UNIT-FILE PIN THAT NOBODY RE-READ — S267

**Minted:** 18-Sep-2026, S267. **Class:** stale pin.

The Register records `clinic-finance.service` at `59c03bfa`. The 18-Sep 01:35 bundle carries the unit at `f0581a1e088ef87d87ee6663f454aabe` — hashed from the bundle's own bytes at this close, not predicted (F-472). The pin was simply never re-read after the unit last changed; nothing in the estate compares a unit file against the Register, because the pin list is generated from the Register and not from the box.

**Action:** re-pinned at this close to `f0581a1e…` from the bundle. **Rule:** a unit file is a pin like any other and is re-read from the bundle at every close that touches its service.

## F-532 · THE READ-ONLY GIT COMMAND, AGAIN — AND THIS TIME IT LEFT A LOCK — S267 · THE ASSISTANT'S

**Minted:** 18-Sep-2026, S267. **Class:** the F-511 / F-520 family, third occurrence.

At the S267 open a `git log` / `git status` / `git fetch --dry-run` ran in `device_bash` against `D:\dr-manoj-git\drmanoj-clinic-automation` before rule 15 was read. This time it left `.git/objects/maintenance.lock` (0 bytes), which the shell cannot unlink. It was moved out of the repository, and a `find` over `D:\dr-manoj-git` at this close shows no `*.lock` anywhere under it — the parked copy is gone too, which is the right end state and is recorded here rather than the parked path this finding first claimed.

**Action:** none outstanding. **Rule, restated because twice was not enough:** the session's rules are read before the first command, not after it; the repository is read through `.git/logs/HEAD` and the refs.

## F-533 · FROM TONIGHT THE BUNDLE'S COPY OF A UNIT WILL NOT MATCH ITS PIN — BY DESIGN — S267

**Minted:** 18-Sep-2026, S267. **Class:** a guard that will read as a fault if it is not written down first.

`S306_BUNDLE_MASK` (live today) masks secret-bearing `Environment=` values in `etc/systemd/system/*` before they enter the nightly code bundle, and records the untouched file's md5 on a `masked=<rel> original_md5=<md5> names=<...>` line in `BUNDLE_INFO`. `clinic-finance.service` carries `FINANCE_CRON_TOKEN` and `FINANCE_MARG_TOKEN`, so from the 19-Sep 01:35 run onward **the bundle's copy of that unit hashes differently from the live file, permanently and correctly**. A pin check that reads the masked bytes will report a mismatch that is not one.

**Action:** none on the code. **Rule, written before the first false red rather than after it:** a masked file is held against `BUNDLE_INFO`'s recorded `original_md5`, never against the masked copy; any pin check that reads the bundle must read that line first.

## F-534 · TWO CHATS APPENDED TO THE SAME CANON AND BOTH CALLED THE RESULT v1.104 — S267 · THE ASSISTANT'S · REPAIRED AT THIS CLOSE

**Minted:** 18-Sep-2026, S267. **Class:** the F-509 / F-515 family — parallel chats, one numbering space — and its worst instance so far, because it reached canon rather than a kit folder.

This close wrote `Fault_Action_Register_v2_91.md` and `KB_History_Archive_v1_104_S267close.md` at 01:24–01:25 by appending to v2.90 and v1.103, **the versions this chat had read at its own open**. The Sanjeevni chat had meanwhile closed S266 at 17:32 and a post-close at 23:46, taking v2.91/v2.92, Archive v1.104 and v1.105, Register v5.106 and v5.107, Runbook v187 and v188, `START_HERE_SESSION_268`, and minting **D541 … D543 and F-521 … F-525**. Every number this close had reserved was therefore already spent, and two files carried a version number that already existed on disk with different bytes. Nothing was published: the collision was found while writing the Register, by listing the canon folder rather than trusting the numbers read at the open.

**Repair, performed here:** both files rebuilt as pure appends onto the *current* parents (v2.92 and v1.105), every number in them advanced — D541 … D543 → **D545 … D547**, F-521 … F-524 → **F-529 … F-532** — and the two stale files moved out of the clone to `D:\dr-manoj-git\_to_delete_S267\` before any publish.

**And it happened a second time inside the same close.** The renumbered files had been written to D544 … D546 / F-526 … F-529 when the System Board was read for the A10c step: the Sanjeevni chat's **S268**, opened at 05:44 that morning, had by then taken **D544 and F-526 · F-527 · F-528** and kits S312 · S313. Everything was renumbered again, to D545 … D547 and F-529 … F-534, this time from the board itself — read at version 102 and claimed on it in the same minute, before another line of canon was written.

**Rule:** numbers come from the **board**, read immediately before the first canon file of a close is written and claimed there in the same minute — never from the session prompt, never from the numbers the session opened with, and never from a canon file read hours earlier. And the canon folder is listed, by `ls`, at that same moment: a parallel chat can spend a whole reserved block between one session's open and its close, and a second one while the close is being written.


---

**Next free: F-535 · D548.**

**END OF FAULT → ACTION REGISTER v2.93 — F-534 is the last minted finding. The v2.92 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S267 close: the first 698017 bytes of this file are byte-identical to v2.92 (md5 9b1ac63a7398d87bfab366e8828ef887).**

---

# §S268 — 18-Sep-2026 · SANJEEVNI · F-526 · F-527 · F-528 · F-535 · F-536 · F-537

*Six findings, five of them the assistant's. Three of the five are one mistake in three coats, and the
owner found the third himself.*

## F-526 · THE COUNT PROOF WINDOWED ON THE WRONG DAY — S268 · THE ASSISTANT'S

**Minted:** 18-Sep-2026, S268. **Class:** a figure computed from the date that was easy to reach rather than the date that means something.

Step 7 of the stock hub proves a count by holding the two reference days against each other. It selected the movements in that window by `stock_voucher_entered.entered_at` — **the moment Amir told the server he had keyed the voucher into Marg** — and not by the voucher's own date, the day the stock actually moved. Amir keys a day's vouchers when he gets to them; on any day he keys late, a movement falls outside the window that should contain it and inside one that should not. The proof was arithmetically correct on a set of rows chosen wrongly.

**Action:** recorded against the count-#2 work, **not repaired in this session**, because `stock_voucher_entered` is read by the 06-Sep count that is open on the owner's screen and D546 — now **D549** — forbids touching a table that count reads. The repair is named in the next session's brief.

**Rule:** a window over stock movement is cut on the movement's own date. When a table carries both *when it happened* and *when we were told*, the second is provenance and never the axis.

## F-527 · NO PURCHASE RETURN IS STORED ANYWHERE ON THIS BOX — S268

**Minted:** 18-Sep-2026, S268. **Class:** a capability the design assumed and the estate does not have.

`S312_CLAIM_QUEUE` was designed to close a claim by itself when the goods come back as a purchase return — the honest ending for a line that was never sold. Looking for the table to read, there is none: **`MargArchive\SALE_RETURN\` holds three files, the newest from April, and no purchase-return export exists at all** — not as an archive family, not as a signature, not as a row anywhere in `finance.db`. The estate can see a sale return and cannot see a purchase return.

**Action:** the self-closing half of the queue is **written and asleep** behind `_returns_available()`, a live capability test, rather than deleted or faked. When a purchase return can be read, the test passes and the code wakes with no kit. The next session's report work is told to watch for a purchase-return export Marg can produce; **that is the unlock, and it is worth a kit.**

**Rule:** code that depends on a store this box does not have is gated on a test of the store, never on a comment saying it will exist one day.

## F-528 · FOUR RULES ANSWER "WHICH SECTION IS THIS ITEM IN", AND NONE OF THEM IS STORED — S268

**Minted:** 18-Sep-2026, S268. **Class:** the same question answered in four places, each slightly differently, none of them writable by the owner.

`_voucher_section()` prints a section heading on Amir's Marg voucher. `_is_consumable()` decides whether a shortfall is consumption. The ortho lane has its own list. The dead-stock lane has a fourth. Each is a word list inside a Python file; **the owner cannot correct any of them, and nothing records what he would say.** A count restricted to orthotics — which he asked for in this session — has no definition of *orthotics* to restrict itself to.

**Action:** `S313_SECTION_MAP` (live). One table, `stock_item_section`, seeded from `_voucher_section()`'s own rule reduced to the item name, with the word lists **copied into the module rather than imported**, so the table cannot silently re-classify itself when somebody edits a pricing list months later. The owner reviewed it the same morning and moved seven items; Orthotics 68 → **69**. **21 of 373 items had been guessed wrongly by the word list**, measured against his own Marg category export.

**Action, second part:** the map is read by nothing yet, on purpose. **D548** settles why: it decides SCOPE only, never whether a shortfall is a loss. Re-pointing the loss lanes at it would change his loss figures and is a separate decision, separately put.

**Rule:** when the owner is the authority on an answer, the answer is a row he can change, not a list in a file he cannot open.

## F-535 · "TRUNCATION IS GONE" WAS TRUE OF THE PRINTED BILL AND FALSE OF THE EXPORT WE IMPORT — S268 · THE ASSISTANT'S

**Minted:** 18-Sep-2026, S268. *(Minted in this session as F-529 and advanced to F-535 at the close — see F-537.)* **Class:** a measurement taken on the wrong surface and then inherited without re-measurement.

S236 recorded on 09-Sep that Marg's item-name truncation was gone. It was gone **from the printed bill**. The exports this estate actually imports were never re-measured. Measured now across April–September, in every month: **the sale export clips at 20 characters · the purchase export at 27 · Marg's own item master at 29.**

The aggravating fact is not the original error. **S236's own paper instructed its successor to re-measure, and for nine sessions nobody did** — the sentence was read as a note rather than as an instruction, and the conclusion above it hardened into a fact.

**Action:** the three widths are now in the Sanjeevni Book v1.4 and in the next session's brief as measured figures with the months they were measured over. 29 is a hard ceiling on any rename — it had already broken two of this session's 22 rename proposals before they were caught.

**Rule:** an instruction to re-measure is a debt on the register, not a line in a paper. And a measurement records the surface it was taken on, because *the bill* and *the export* are two surfaces and only one of them is imported.

## F-536 · THE SALT IMPORTER READ MARG'S PAGE HEADING AS A SALT — S268 · **THE OWNER'S CATCH**

**Minted:** 18-Sep-2026, S268. *(Minted in this session as F-530 and advanced to F-536 at the close — see F-537.)* **Class:** identification by exclusion, in the reader that feeds the count.

The importer for the salt-wise item list identified a salt heading **by exclusion** — a row that is not an item, not a column header and not blank was taken to be a salt. Marg prints its shop name as a **page-break header roughly every 64 rows**. So `SANJEEVNI MEDICOS` was read as a salt, and every item under it until the next real heading inherited it. **19 of 373 items carried a wrong salt.**

The consequence is the reason this is the session's most serious finding. Same-salt pairing is how the stock check proposes a swap. **One of the pairs sitting on step 2 of the hub, waiting for the owner's answer, was a pair our own reader had invented.** He had been asked to adjudicate a match that existed only because of this bug. He caught it himself, from the wording of an earlier claim of mine, and said plainly that this was the only thing stopping him proceeding with the stock check.

**Action:** a strict reader was written — `margparse.py`, 153 lines, at `D:\Downloads\ClaudeCowork\03_WORKING_PAPERS\S268\reader\`, **installed nowhere**, with a `READ_ME.txt` saying so. It classifies every row **positively** (`ADVERT, CONTINUED, PAGENO, SHOPNAME, TITLE, COLHDR, STRAY, ITEM, HEADING, ANOMALY`, in that order) and carries an independent witness Marg gives free: **every item row is numbered and the numbering restarts at 1 under every heading**, so `parse()` sets `want = 1` on a heading and requires `r.n == want` on an item. A misread heading is contradicted by the very next row instead of silently becoming a salt. It was run over six exports and **made to fail on purpose at exactly the two rows where the old bug lived.**

**Action, second part:** nothing was re-populated from it. **D549** holds the stored tables still until count #1 closes. The corrections paper was reissued at v3 with the earlier claim — that six appliances carry the shop's own name as their salt — **retracted in full**, because that claim was this bug reading itself back.

**Rule, and it stands over the whole estate:**
...
**Corollary:** nothing in a report is identified by exclusion. Every row class is named positively, and a reader with no independent witness produces figures that do not enter canon — which is said plainly rather than papered over with an invented one.

## F-537 · THIS CLOSE OVERWROTE THE PARENT'S CANON, BECAUSE TWO CANON FILES CARRY NO SESSION IN THEIR NAME — S268 · THE ASSISTANT'S · REPAIRED BEFORE ANY PUBLISH

**Minted:** 18-Sep-2026, S268. **Class:** the F-509 / F-515 / F-534 family — parallel chats, one numbering space — but a **worse failure mode than F-534 records**, and the reason it deserves its own number.

This close wrote its canon by appending to the versions this chat had read at its open: Register v5.107, Archive v1.105, Fault v2.92, Runbook v188. The parent's **S267 close had landed at 01:46 that morning, in the other chat, while this session was working**, taking Register **v5.108**, Archive **v1.106**, Fault **v2.93**, Runbook **v189**, `START_HERE_SESSION_269`, and minting **D545 … D547 and F-529 … F-534**. F-534, written in that very close, describes the collision from the other side and instructs: list `KB_canon_all\` immediately before writing. **This close did not, and paid for it twice over.**

**And here is the part F-534 does not cover.** Four of the six canon files carry the session in their filename — `KB_Register_v5_108_S267close.md` and `KB_Register_v5_108_S268close.md` can sit side by side, wrong but both present, and the collision is a duplicated version number that a listing reveals. **Two do not: `Fault_Action_Register_v2_93.md` and `START_HERE_SESSION_269.md` carry no session and no distinguishing suffix at all.** Writing those did not collide with the parent's files. **It destroyed them** — silently, with no error, in a folder that publishes.

**Repair, performed here, before any publish.** The loss was found by reading the parent's `live_pins_S267close.txt` header, which names the Register version it was generated from. Both destroyed files were recovered from the published commit `5cfc938` by **cloning the repository in the cloud workspace** (F-511 forbids the git command in the PC shell) and restored byte-for-byte: `Fault_Action_Register_v2_93.md` md5 `0a6f6be88704fb8b95b97b091d659bc5`, `START_HERE_SESSION_269.md` md5 `7a5a40a8bf0e5117a7e6b93d7bde79d1`. This session's own canon was then **rebuilt onto the correct parents** — this file onto the parent's v2.93, the Archive onto its v1.106, the Register onto its v5.108 — every number advanced: D545 → **D548**, D546 → **D549**, F-529 → **F-535**, F-530 → **F-536**, and this session's handoff renamed to `START_HERE_SESSION_270` because the parent has reserved 269. The three wrongly-numbered files were deleted from the clone.

**That the recovery was possible at all is luck with a cause:** the owner's publish at 07:30:40 IST carried the parent's close to GitHub **before** this close overwrote it. Twenty minutes the other way and the parent's Fault Register and its next session's brief would both be gone, with no copy anywhere.

**Rule, and it is the operative one:**

> **Before writing any canon file whose name does not carry the session, read it back first and prove it is the parent you think it is.** A version-suffixed file collides loudly. An unsuffixed one — the Fault Register, `START_HERE_SESSION_###`, `OWNER_TODO_LIVE.md`, `CANONICAL_MANIFEST.md`, `MD5SUMS_ALL.txt` — is overwritten in silence.

**Rule, second part:** the recovery route is now known and is written here so the next session does not have to find it: **the published commit is the backup.** Clone in the cloud workspace, never in the PC shell, and take the file back.

**Standing, and repeated from F-534 because twice was not enough and this is the third time:** numbers come from the **board**, read immediately before the first canon file of a close is written, and `KB_canon_all\` is listed by `ls` in that same minute.

---

**Next free: F-538 · D550.**

**END OF FAULT → ACTION REGISTER v2.94 — F-537 is the last minted finding. The v2.93 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S268 close: the first 705804 bytes of this file are byte-identical to v2.93 (md5 0a6f6be88704fb8b95b97b091d659bc5).**

---

# §S268 POST-CLOSE — 18-Sep-2026 · SANJEEVNI · F-538

## F-538 · A PUBLISHED KIT FOLDER GREW A FILE AFTER IT WAS FROZEN, AND THE PUBLISH GATE REFUSED — S268 POST-CLOSE · THE ASSISTANT'S

**Minted:** 18-Sep-2026, at the S268 post-close. **Class:** F-512 (a frozen kit folder edited) meeting F-100's publish guard (a file silently excluded by `.gitignore`) — a near miss the guard caught, recorded because the guard caught it, not because anything reached the box.

The owner ran `PUBLISH_ALL.bat` on this close and it **refused**, correctly:

```
deploy_kits/S313_SECTION_MAP/__pycache__/section_map.cpython-310.pyc
REFUSING - at least one file was silently excluded by .gitignore.
Nothing committed or pushed.
```

**What had happened.** `S313_SECTION_MAP` was finalised at 01:00 and published at `57185e6`; its eight files and `SUMS.md5` are exactly what went live. At **02:03** a `__pycache__` appeared inside that folder — **`section_map.py` was imported from the kit folder while `S314_SECTION_SCOPE` was being rehearsed**, and Python wrote its bytecode beside the source. `.gitignore` ignores `__pycache__/` and `*.pyc`, so git would have carried the push while silently dropping a file that was sitting in a published kit. **That is precisely the shape F-100's guard exists to refuse, and it refused it.**

**Nothing was wrong with the kit.** The `.pyc` was never part of it: `SUMS.md5` has eight rows for eight files and still reads 8/8 green with the artifact gone. Deleting it restored the folder to byte-for-byte what published.

**Repair, performed here.** The `.pyc` and its `__pycache__` removed from the clone; **`find` over the whole repository now shows zero `__pycache__` and zero `*.pyc`**. All three of this session's kits re-gated from inside their own folders: **S312 9/9 · S313 8/8 · S314 6/6.** A repository-wide sweep for anything else `.gitignore` would swallow among today's files — `.json`, `.csv`, `.log`, `.tmp`, `.env`, `.pyc` — found **none**. The stray copy that had been carried into `ClaudeCowork\02_SESSION_KITS\S268\` was moved to that session's `_to_delete\`.

**Why this is the assistant's and not the tooling's.** F-512 says a kit folder that could have been published is frozen. A rehearsal that **imports a module out of `deploy_kits\`** writes into that folder, so the rehearsal broke the rule even though no line of the kit changed. The S302 close had already noted a `__pycache__` that *should not have existed* and treated it as a symptom; here it is the finding itself.

**Rule:** a rehearsal, walk or selftest **never imports from `deploy_kits\`** — it imports from the scratch build, or it copies the module to a scratch directory first. And before the publish is handed to the owner, the close runs `find <repo> -name __pycache__ -o -name '*.pyc'` and expects **zero**, the same way it already runs the `*.lock` check (A16).

**Standing, and it is F-100's rule restated because this is its fourth save:** an ignored file is silent by design, and silence is the failure. The guard refuses the whole push rather than drop one file, every time, and that is correct behaviour and not an obstacle to work around.

---

**Next free: F-539 · D550.**

**END OF FAULT → ACTION REGISTER v2.95 — F-538 is the last minted finding. The v2.94 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S268 post-close: the first 718357 bytes of this file are byte-identical to v2.94 (md5 383f02aa49aeec1c7db126e6218c36c2).**


---

## §7.29 — S269 (the parent, 18–19-Sep-2026): F-539 … F-547 — *five of the nine are the assistant's own, and four of those were found by the owner looking at a screen*

**Minted:** 19-Sep-2026 at the S269 close. **Class:** evidence-dating · owner-facing pages · backup coverage · publish gates · the canon's own linchpin. **Why they sit together:** this session put five kits on the box and three of them corrected the previous one — the corrections came from him tapping a button, not from a green gate.

---

## F-539 · I CONVICTED A PUBLISHED KIT OF NEVER HAVING BEEN INSTALLED, ON A SNAPSHOT THAT WAS HOURS TOO OLD — S269 · THE ASSISTANT'S

**What happened.** The first backlog item was to verify that S310's petty-photo freshness leg had switched on. I read `/root/finance/freshness_legs.json` out of the **18-Sep 01:35 code bundle**, found md5 `b02f846a` — 30 legs, `asset app archive` still at 200 h, no petty leg — and told the owner in writing that **S310 had been published but never installed**. It had been installed, that morning, after the bundle was taken. His re-run answered `RESULT ALREADY` on both rows and the live legs file reads `0e56aa9ead530a70b7977cbf237d13ea`.

**The mechanism.** The nightly bundle is a **snapshot at 01:35**. “The bundle does not show it” proves only *not before 01:35* — never *not done*. The board corroborated the wrong reading only because the board is as current as whoever last wrote to it, and no one had written `kit_S310_live`.

**What I should have done, and it was free.** The kit prints a read-only `--check`, and the collector's own page states the live bytes. Either costs the owner nothing.

**Action.** Retracted in his chat within the hour and on the board in my name. **Rule, into the routine as v16's first new A0 bullet:** *the nightly bundle is 01:35 evidence and nothing later; ask the live surface before convicting.*

---

## F-540 · A KIT SENT THE OWNER TO A URL THAT HAS NEVER EXISTED — S269 · THE ASSISTANT'S

S310's installer ends with `read next: https://followup.dr-manoj.in/finance/freshness`. He opened it and got **Not Found**; so did I. There is no such route: `freshness.py` writes `/root/finance/freshness.html` as a **file** and shouts over ntfy, and a grep of the whole 18-Sep bundle finds that name only inside `freshness.py` and `code_bundle.py`. The URL was invented in my own kit text at S267 and repeated to him at S269 without a test.

**Consequence beyond the dead link:** the freshness table — the one surface that dates all thirty watched legs, and where D525's never-fired checks live — **has never been readable in a browser at all.**

**Action.** Recorded; the page is on the next session's list (it also makes the printed line true). **Rule (v16, A0/A16):** a URL is a live route or it is not printed — check it against the app's own routes before it reaches a kit, a message or canon.

---

## F-541 · ELEVEN LIVE ENABLED UNITS WERE IN NO STORE AT ALL, AND NOTHING COULD SEE IT UNTIL THE BUNDLE LEARNED TO READ THE SYMLINKS — S269

**Found by machine, on S317's first run.** The nightly code bundle has carried unit FILES since S243 and the root crontab since S250, and neither answers *is this job switched on*. S317 (F-496's fix) added `unit_state.txt`: the `*.wants` enable symlinks read off the disk plus `systemctl is-enabled` / `is-active` per carried unit. Its first live output listed every enable symlink on the box — and **eleven enabled units matched none of the bundle's four patterns**:

```
assetapp.service · attendance-dashboard.service · attlistener.service
staff-register.service · staff-ledger.service · fitlog.service
gutlog.service · rxguard.service · email-agent.timer
call-recording-archive.timer · call-transcription.timer
```

So the **code** of the staff register and the asset app was backed up while **the unit file that starts them was nowhere**, and two `call-*` timers were missing although their services were carried.

**Action.** S318 carries seven of them the same evening and adds a self-auditing section — `[enabled, ours, and NOT carried by this bundle: N]` — which names ours and counts the rest, so the gap reports itself every morning. Four remain (`fitlog`, `gutlog`, `rxguard`, `email-agent.timer`) and **D555** rules that they are carried too. Carried units 20 → 27 on the live run.

---

## F-542 · TWO PUBLISH REFUSALS IN TWO DAYS, BOTH KNOWABLE BEFORE HIS DOUBLE-CLICK — S269 · THE ASSISTANT'S

1. **S316** carried two `.json` seed files into a repository whose `.gitignore` blocks `*.json` wholesale. `PUBLISH_ALL.bat` refused the **entire** run — *“at least one file was silently excluded by .gitignore. Nothing committed or pushed.”* — and he lost a click. The fix is one exact-path allow line per file, and it belongs in the same breath as the kit (F-300's family, fifth instance).
2. **S318** carried, as a test fixture, a sixteen-character quoted string assigned to a variable literally named `SECRET`. Not a real credential — but the value is deliberately NOT reproduced here, because writing it into canon is what turned this entry itself into a third refusal (F-549). Sixteen characters under a secret-shaped name — the exact shape of the WhatsApp token that sat readable for 84 days (F-365). The gate refused the whole publish, correctly. Replaced with `PUT-THE-TOKEN-HERE`.

**Both were findable in the cloud workspace before he was asked to click:** `deploy_kits/NO_PHONE_NUMBERS.py` is in the repository and takes one command. **Rule (v16, A11c/A16 and my routine):** run the repository's own gate over every new kit folder before naming the publish, and add the `!deploy_kits/…` allow line when the kit is copied in.

---

## F-543 · F-384 HAS RECURRED A THIRD TIME — FOUR FAMILIES CLAIMED “CURRENT” AT ONCE, AND NOTHING WAS WATCHING FOR IT — S269

F-384 (S236) and F-449 (S243) are one fault: two rows of one document family both claiming **CURRENT** in `CANONICAL_MANIFEST.md`, which matters because `gen_live_pins.py` keys on that bare word and can therefore generate the pin list from a superseded Register. Both were found by hand, months apart, by whoever happened to parse the manifest. Measured again at S269 on the published manifest `13d762c4`:

| family | rows claiming CURRENT |
|---|---|
| `END_OF_SESSION_PROMPT` | v13 (line 551) · v14 (2753) · v15 (2621) |
| `START_HERE_PROMPT` | v8 (559) · v10 (2691) |
| `SANJEEVNI_SYSTEM_BOOK` | v1_1 twice (2495, 2790) · v1_3 (2605) · v1_4 (2537) |
| `START_HERE_SESSION` | 268 (2557) stale; 269 and 270 are one per project and legitimate |

**Action.** The stale cells are corrected visibly at this close, and **S320 puts the check into the 03:10 nightly's canon gate** — read-only, named in the VERDICT line `CANON_SUMS_LATEST` already carries, with `CURRENT_ALLOW_MULTI` declaring the one legitimate exception (`START_HERE_SESSION` = 2, one entry point per project). No fourth report to remember; no change to the exit codes.

---

## F-544 · THE OWNER'S NEW PAGE HAD NEVER HAD A WORKING BUTTON, AND THE WALK COULD NOT SEE IT BECAUSE IT POSTED THE URLS ITSELF — S269 · THE ASSISTANT'S

**His words:** *“I tried to work in the X-ray and procedures page by adding words in the rename part and adding price. Then I clicked approve button and it gave a page not accessible.”*

Every form on `/finance/clinic/sheets` carried a **relative** action — `action="status"`, `"rename"`, `"price"`, `"add"`, `"item/add"` — and the page is served **without a trailing slash** (which is what the portal tile links to). A browser resolves a relative action against the *directory* of the current URL, so every button posted one level up:

```
page  /finance/clinic/sheets   +  action "status"   →   POST /finance/clinic/status   → 404
```

**S316's walk ran 33 green checks and could not have caught it:** it posted to the routes by their full URLs. Intact is not valid (S209's rule) — and a page's buttons are not tested until they are resolved the way a browser resolves them.

**Action.** S321 adds one `<base href>` taken from the blueprint's own `url_prefix`, which repairs all eleven forms and any added later. **Rule:** a page kit's walk renders the page, reads each `<form action>`, resolves it against the page URL and its base tag, and POSTs to what comes out. S321's walk does exactly that in 23 checks, and one of its negative controls **reproduces his 404** on the unpatched module.

---

## F-545 · A SAFETY RULE THAT SILENTLY DID NOTHING, AND PRINTED A ZERO WITH NO REASON BESIDE IT — S269 · THE ASSISTANT'S

S322 was to mark the limb X-rays as asking right / left. Its closing line read **“X-rays that now ask a side: 0”**. The cause is its own safety rule: it only touched rows still marked as the seed's own (`updated_by = 'seed'`), and by then he had **approved the whole X-ray list**, so every row counted as his. The rule is right — his edits must never be overwritten — but the flag is *additive*: it changes no name, no price and no approval, so it should never have been inside that rule.

**Only the owner's eye caught it.** The kit printed the number and no reason.

**Action.** S323 sets the flag regardless of who last touched the row (13 X-rays, confirmed live), while a **name** he typed himself is still never overwritten. **Rule:** when a kit's safety rule can make it do nothing, the closing line states **what it skipped and why**, not only what it did.

---

## F-546 · THE 03:10 NIGHTLY DIED INSIDE THE SSD MIRROR AND LEFT A `.new` BEHIND, AND NOTHING WATCHES THE PC NIGHTLY'S OWN COMPLETION — S269

**Measured at the close, 19-Sep 05:26 IST.** All three nightly reports carry **18-Sep 03:11 IST** — 26 hours old, which A0.2 calls a RED. But the run did fire: a catch-up started about **04:03 IST on 19-Sep**, wrote the paper shelf (`PAPERS.html`, 04:03) and began the mirror, and stopped — `F:\ClinicBackup\DrManojClinic_Automation\01_KB_MIRRORS\KB_mirror_ClaudeCowork_nightly_2026-09-19.zip.new` sits at 98,975,650 B, **unchanged in size and mtime across two listings 45 s apart**, so it is dead rather than still running. None of the three reports was rewritten.

**The newest complete mirror is 18-Sep** — 110,516,994 B, 3,616 files, every CRC tested — so nothing is lost; the day's snapshot is missing.

**And the second half of the finding:** `MAINTENANCE_REPORT_LATEST` of 18-Sep reads `VERDICT: WARN` with **all three steps RESULT OK** — a verdict whose reason is nowhere on its face.

**Action.** One double-click of `D:\Downloads\_kbtools\NIGHTLY.bat` is on his list — it is a GUI action on a machine no shell of mine can reach since the 8-Sep Windows update. Two follow-ups for the next session: the mirror step should remove or name a stale `.new` on its next run, and the WARN must state its cause.

---

## F-547 · THE WATCHER CHECK CANNOT GO RED IN ANY REALISTIC FAILURE, SO FOURTEEN DAYS OF GREEN THERE MEAN NOTHING — S269

Read out of `finance_app.py` while answering **D525** (the seven never-fired checks). `watcher` (lines 12037–12059) is red **only** when `_w["alive"] is False` *at the moment the page renders*. Its own comment says the agent restarts the watcher within 60 seconds, and the realistic failure — the medical PC off or unreachable — is deliberately routed to **`info`** by the S202 ruling. So the medical PC can be off all night, capture can be missing for hours, and this check says `info` and never `bad`. That is the **false green** S202's comment was written to condemn, built into the check on day one.

**The rest of D525, for the record:** `flags` and `margqueue` have no red branch at all, by the S195 ruling — informational by design, and **S319** stops the witness counting them (seven → five). `drawer` fires on 1-Oct (45 days after the 17-Aug cash count) and `renewals` on 20-Sep (seven days before the arms licence) — both prove themselves unaided. `backup` and `outbox` are honest guards whose condition has not occurred, and both are provable by injection because `FINANCE_DB`, `FINANCE_BACKUP_DIR` and `FINANCE_RENEWALS_STALE_H` are env-overridable. Evidence paper: `03_WORKING_PAPERS\_EVIDENCE\S269\S269_NEVER_FIRED_SEVEN.md`.

**Action.** **D554** rules it: red during clinic hours when the heartbeat is stale, info outside them. The kit is the next session's.

---

## F-548 · `device_bash` WORKS AGAIN ON manojz — AND THE PROMPT STILL SAYS IT IS DEAD — S269

The session-start prompt's §4 has said since 8-Sep-2026 that `device_bash` refuses to start because the
8-Sep Windows update left no Plan9 drive shares mounted, and that a session must try it once and then
not spend itself on it. At the **S269 close** it started on the first call and mounted **all three**
connected roots — `Downloads`, `dr-manoj-git` **and `ClinicBackup`** — `F:` included, which the same §4
says never mounts there. It reads, greps, writes and runs `python3` on both PCs.

Two limits are real and stay: a file **cannot be deleted** in a connected folder (`rm` returns
*Operation not permitted* until the owner grants it), so a stray file is renamed to `*.tmp` — which
`.gitignore` line 108 already ignores — rather than removed; and **F-511 stands unchanged** — no `git`
command is ever run in `device_bash` against a connected folder.

This is the **second** time a capability warning has outlived its fault in this prompt. The first cost
nineteen sessions of the assistant's browser (closed at S239 by AF-12, still warned against in v8).
The lesson is not about either tool: **a §4 warning is re-tested at the session that reads it, and a
capability that answers is recorded as answering that same session.**

**Action.** `START_HERE_SESSION_271.md` carries the correction so the next session inherits the working
tool, not the warning. The evergreen `START_HERE_PROMPT_v10` §4 is the owner's own custom-instructions
text and is the one line he may wish to strike when he next edits it — nothing waits on it.

---

## F-549 · THE PUBLISH GATE WAS RUN OVER THE KIT FOLDERS AND NOT OVER THE CANON THE SAME PUBLISH STAGES — S269

**THE ASSISTANT'S, and the third publish refusal in three days.** F-542's own rule, written hours earlier
in this same register, says: run `deploy_kits/NO_PHONE_NUMBERS.py` over every new kit folder before naming
the publish. It was run, over all eight kits, and it came back clean. The publish still refused — on
`deploy_kits/KB_canon_all/Fault_Action_Register_v2_96.md` line 6384, **this register**, at the very entry
that records F-542, because that entry QUOTED the fixture it was condemning.

**Three things went wrong at once, and only the third is new.**

1. The rule named **kit folders** and a publish stages **everything untracked** — at a close that is
   twelve canon files, which the gate had never been pointed at.
2. **A finding written in the words of the thing it found repeats the thing it found.** A register that
   quotes a token-shaped literal to explain a token-shaped literal is itself a carrier. The same is true
   of a patient number in a finding about patient numbers, and F-185 has always said so for numbers; it
   had never been said for credentials.
3. The gate was right on every count, and its refusal cost the owner a double-click — the second time
   that has happened in two days for the same reason: **the gate was available and was aimed too narrowly.**

**Repaired.** The quoted literal is gone from the entry above; the entry still says exactly what the fixture
was (sixteen characters, a variable named `SECRET`) without being one. Nothing needs rotating — it never was
a credential.

**Noted and deliberately NOT changed:** `deploy_kits/S317_UNIT_STATE/selftest_s317.py` holds the same fake
string in a *different* shape, `Environment=API_TOKEN=…` inside a fixture unit file, and the gate does not
flag it — correctly, since its job there is to be masked by the code under test and the test asserts it is
absent from the output. That kit is published and therefore frozen (**F-512**); editing it would break its
own `SUMS.md5` for no security gain.

**Action — the rule, corrected, and the method matters.** A bare folder run is NOT the publish gate: pointed
at `deploy_kits` it reports 1,628 hits and at `deploy_kits/KB_canon_all` 969, because the Archive, the Register
and the manifest are full of numbers that were masked years of sessions ago. `PUBLISH_ALL` does not scan folders
— it scans the **file list it is about to stage**, with the carried-forward allowance that prints
*canon carried forward — N known number(s), none new*. So the check must be run the same way, over a list of
exactly the files the close wrote — the twelve canon files plus every new kit folder:

```
python -B deploy_kits/NO_PHONE_NUMBERS.py --files-from <list> .
```

Run at the S269 close over its own 21 files: *clean — 21 staged file(s) checked (numbers and credentials)*,
*canon carried forward — 6 known number(s), none new.* That is the wording a close should be able to quote
before it names the publish; anything else is a folder scan wearing the gate's name.

And the standing writing rule: **a finding never reproduces the string it is about** — describe its shape,
never its value. Into `END_OF_SESSION_PROMPT` v17 at A11c and A16.

---

**Next free: F-550 · D557.**

**END OF FAULT → ACTION REGISTER v2.96 — F-549 is the last minted finding. The v2.95 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S269 close: the first 721890 bytes of this file are byte-identical to v2.95 (md5 1844aed175b496b52bfc195fee538cd4).**


---

## §7.30 · S271 (the parent, 19–20-Sep-2026) — F-550 … F-555

**F-550 — "Not in the patient master" was read as "new patient" (S324 → S325).** The master has no row for
some old IDs (2681, a patient of years). The tile marked him NAYA on the first live morning. **Action:** new
is decided by the number, never by the master's silence — see F-551 for where the number comes from.

**F-551 — A ceiling taken from the master's highest row sat above every genuine new ID (S325 → S326).** The
master carries one row far ahead of the series, never seen in any visit, so "above the highest known ID"
was never true for a new patient, and S325's data fix then marked that day's genuine new patients old.
**Action:** the ceiling is the highest ID **issued before the day** — Docterz day lines, visits, master rows
with a last-seen date — and every slip was re-decided by that rule (`fix_s326.py`). **Lesson: a maximum over
a table is only as good as its worst row; take the maximum over events, not over a register.**

**F-552 — The walk encoded an assumption about the live data (S324).** Check 79 counted rows at ₹500 in the
rate table as if only the walk's own row could be ₹500; the live table already priced most X-rays at ₹500,
so the installer refused on the box while the tile was correct. It refused safely — nothing was placed.
**Action:** a walk compares **before and after** of the rows it did not touch, never a count that depends on
the owner's data. Rewritten that way and green on live-shaped prices.

**F-553 — A coordinate click on a live owner page changed the wrong row.** Setting Knee studies to ₹1,000
through the assistant's browser, a click by screen position landed on Ankle (the page had shifted); Ankle
read ₹1,000 for about a minute and was restored, the full list read back afterwards. **Action:** on a live
page, change data through the page's own form located by its content (the form element of the row whose
name matches), never by pixel position; read the whole list back after.

**F-554 — A compile inside a kit folder left `__pycache__`, and the publish refused the whole run.** The
publish gate lists every ignored file under `deploy_kits` and refuses; `py_compile` and the installer
rehearsal ran inside the kit folders. **Action:** compile and rehearse from a copy; before naming the
publish, look for `__pycache__` under every new kit and move it out of the repository (the shell cannot
delete). Five such folders were moved to `D:\dr-manoj-git\_stray_S324\` this session.

**F-555 — The first X-ray/Proc start number was typed low, and the tile asked about 136 old numbers.**
Numbers ≤ 1136 belonged to earlier days. **Action:** voided once with the reason (his ruling: a one-time
request at the start); the rule that every skipped number is answered stands (D557). A first build that
treated any confirmed big jump as a fresh start was withdrawn on his word before it was installed —
**a rule the owner states is not relaxed to cure a one-off.**

---

**Next free: F-556 · D564.**

**END OF FAULT → ACTION REGISTER v2.97 — F-555 is the last minted finding. The v2.96 and earlier end-markers above are retained in place as truncation-proofs.**
...
## §7.31 · S270 (Sanjeevni, 18–20-Sep-2026, the carried increment) — F-556 … F-563

*S270 closed light on 20-Sep 01:06 IST and carried its findings to this close (S270_BUILD_BRIEF §6). Full text: `MARG_REPORT_CONTRACT_v1.md` §6 and `S270_CORE_DATA_VERIFICATION_REFERENCE.md` §5, both in canon from this close. F-527 (returns) and F-536 (the salt heading) already covered two of them.*

**F-556 — The live sale importer never checked a bill's lines against its bill.** The witness (every bill: gross = Σ rate × qty ÷ pack; grand total = Σ bills; footer count = bills read) exists in the S270 harness and holds on 2,806 lines; it was installed nowhere. **Action:** the certified sale reader of S331 carries it; the old importer is retired at the spine's rung 5.

**F-557 — The purchase item-wise report defeats a fixed-cell reader three ways:** the bill number fused to the item name in 29 lines (`EP001731MOTIF FORTE`), non-numeric bill numbers (`a000163`, `EP000476`), and supplier names cut across one to three grid cells. **Action:** the S331 reader classifies each row positively and splits the glued form; bill numbers are text with their zeros (reference §5 item 11).

**F-558 — 72 sale bills were never loaded** (every bill of 04-May and 27-May, and A000425 of 19-Apr — the May back-fill made no day for those dates), **and 8 sale lines were misread** (seven where Marg prints `***` in the stock column, one carrying an old name). **Action:** the spine holds all 3,888 (now 3,915) from the exports; `sale_line_item` is retired at rung 5.

**F-559 — The estate's "MRP" was never Marg's MRP:** `marg_item_fact.mrp_p` and `stock_app._mrp_p` are the median sold-at price per unit — 39 wrong, 3 absurd, 171 missing at S270; 43 differ from Marg by more than Rs 0.50 at S272. Marg prints the MRP on every salt list and it was thrown away. **Action:** the spine holds MRP, P.RATE and S.RATE from the salt list, dated; the median rule is retired at rung 5.

**F-560 — Identity by name alone:** Marg holds two `DOLOGESIC SP` (packing `1*10` and `1*10.`), and the shadow table held 172 sale bills twice. **Action:** identity is name + packing in the spine (D564); the shadow store is retired at rung 5.

**F-561 — Purchase bill numbers are not unique:** 16 numbers are used by more than one supplier, and two exports lost leading zeros (`045754`, `004625`). **Action:** a purchase bill is supplier + number + date, number as text (spine); F-494's line-owner rule stands until rung 4c.

**F-562 — Marg-side facts, recorded and carried, not repaired:** 11 negative stocks on 17-Sep; 10 sale bills whose lines do not add to the bill (5 May, 4 Jul, 1 Jun); 29 items whose stock moved between 01-Apr and 26-Aug with no sale or purchase in any export. **Action:** carried as Marg shows them; the 29 booked as dated, sourced reconciliation entries (D565); a new difference after the spine opened is a dated finding, never folded in.

**F-563 — Three reports are not data sources and must not be read as such:** the item master's rate columns are empty and it is stale (28-Aug); the daily print drops the rate on some lines; the stock ledger prints a negative balance as text. **Action:** the contract §1 names what to export and what never to; the spine reads none of the three for a figure.

---

## §7.32 · S272 (Sanjeevni, 20-Sep-2026) — F-564 … F-572

**F-564 — The S270 verification's witness sweep covered 70 of the 104 archive inputs its figures used.** Re-run at S272 over all 104, three unswept inputs fail their witness — the 31-Mar opening (twice) and the 12-Jun sale export. Every figure still held. **Action (rule):** the gate witnesses every input the figures use, from the same list the figures read — never a list kept beside them. The S331 builder does this by construction (`sp_export` is the only input list).

**F-565 — Marg's own 31-Mar-2026 closing prints a TOTAL 40 units below its 980 lines** (26,689 vs 26,729). The reader balances all 20 later closings to the unit. **The owner's re-export of 20-Sep (md5 cc608ca2) is row-for-row identical to the 27-Aug export**, so it is Marg's printed total, not a reading error. **Action:** the lines are taken and the printed total carried as a sourced exception in `spine_rules.json` (D565). *Lesson: an independent second export of the same report is the cheapest witness there is, and it cost him one click.*

**F-566 — The one door's store was read by no screen.** Every business table was still filled by the older push paths, and the estate ran two copies of the sale parser (843 and 748 lines). Measured from the 19-Sep bundle by lineage trace (S272 paper §2). **Action:** the spine is the one store; the second parser and the push paths retire at rung 5.

**F-567 — `stock_snapshot` and `stock_feed` can disagree on the same date:** the snapshot keeps an item a re-export dropped, the feed does not, and `/stock/now` reads one while the count reads the other. **Action:** both retire at rung 4b in favour of the spine's dated closings.

**F-568 — Five Marg sources were archived and never loaded:** the item master (company, packing), stock valuation (rates, value), the category list, the salt list's MRP / P.RATE / S.RATE, and SALE_RETURN. **Action:** the spine loads the first four as dated facts; SALE_RETURN stays PHI-deleted (credit notes come from the sale detail).

**F-569 — An audit did not keep a derived input it read** (`bf.pkl`, the S270 back-fill union). Rebuilt at S272 with S270's own parser and reproduced exactly (3,888). **Action (rule):** an audit keeps every derived input, or the script that makes it, in `evidence\`; `S272\rerun\make_bf_pkl.py` is that script.

**F-570 — Seven kits (S324 … S330) were built and published by the parent on 19-Sep without a board claim and without moving `nextFree`;** S272 claimed S324 from the board and had to withdraw it three minutes later after listing `deploy_kits\` on the PC. The parent owned it on the board the same night (F-509 / F-515 breach, its own). **Action (rule):** a claim reads the board AND lists `deploy_kits\`; the parent's next open verifies both.

**F-571 — An installer's check tested an import the way the check imports, not the way the code imports.** `S331`'s step 2 ran `import xlrd` bare and REFUSED on the box; the box vendors xlrd inside `/root/marg_ingest/`, and every existing reader borrows it from there after putting that folder on the path. Nothing was placed; one republish. **Action (rule):** a pre-flight import check puts the same paths the code puts, or calls the code's own import.

**F-572 — F-523 again: a second `device_commit_files` to a path already written this session landed the OLD bytes** (`S272_SPINE_ARCHITECTURE.md`, 30168286 instead of 29fd938e). Caught by md5 read-back; repaired by appending on the PC. **Action (rule, restated):** a committed file is read back by md5 before it is used; a second write to the same path this session goes through `device_bash` on the PC, not through a second commit.

---

**Next free: F-573 · D568.**

**END OF FAULT → ACTION REGISTER v2.98 — F-572 is the last minted finding. The v2.97 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S272 close: the first 742843 bytes of this file are byte-identical to v2.97 (md5 efaf75ad29c8c7bea5d835186170eb0f).**


## §7.33 · S274 (Sanjeevni, 20-Sep-2026) — F-573 … F-578

**F-573 — F-525 AGAIN: an installer's health gate asked a login-gated route for the kit's name and read the portal redirect as red.** `S334_SHAVEZ_MORNING`'s step 8 curled `/finance/reports/aaj/api/healthz`, which is behind `finance_app`'s `before_request` gate (not in `PUBLIC_PATHS`); the unauthenticated curl received the `/portal` redirect, the `grep` for the kit name failed, and the installer restored the S243 file byte-identically — after every real check had passed (selftest 40/0, the walk on the box 13/13, the service active, `/finance/healthz` 200). The box was never wrong; the owner lost a double-click and a line. Rebuilt as `S337_SHAVEZ_MORNING_2`, the same payload byte-identical, the gate reading `/finance/healthz` 200 · the page 302/401 · the placed module's own `KIT` imported by the service's python. **Action (rule, restating F-525 with the second half it lacked):** a health probe hits a `PUBLIC_PATHS` route or accepts 302/401; **and a kit's own `api/healthz` is proof of nothing to an unauthenticated curl — prove the placed module by importing it, not by fetching it.** The assistant's.

**F-574 — F-538 AGAIN, caught before the publish: the first copy of S334 placed in `deploy_kits\` carried a `__pycache__` written by its own installer's rehearsal.** Step 3 ran `py_compile` inside the kit folder on the PC; the folder was then copied into `deploy_kits\` with the bytecode inside. Found by the repository-wide `find` the close runs; the folder was moved out (never published) and re-placed clean; every installer of this session now compiles on copies in `/tmp`. **Action (rule):** an installer never runs `py_compile`, a selftest or a walk inside its own kit folder — copies first — and the `find` for `__pycache__` / `*.pyc` runs before a kit is copied into `deploy_kits\`, not only at the close. The assistant's.

**F-575 — Every Marg export carries two 10-digit numbers that are not a person's, and a PHI scan that does not know them refuses the whole estate.** The shop's header line `Phone : 93…` (rows 1–8 of every export) and Marg's advertisement footer (`MARG ERP NANO for Chemist … Call 63…, 73…, 98…`, or `Digital Purchase | … | Call MARG …`) sit in every file the door keeps. `phi_scan.py`'s first draft treated any 6–9 ten-digit run as a mobile and refused 96 of 98 closing-stock, salt, item, category, purchase, expiry and valuation exports; measured on the whole archive, the two furniture lines are the only such numbers in every one of them, and after exempting them (the header by position and prefix, the footer by the word MARG in the last three rows) 98/98 kept types pass and 44/44 sale-family exports are still refused. **Action (rule):** any scan for a person's number in a Marg export exempts exactly these two lines and nothing else; the exemption is stated in the scanner and measured against the archive, never widened by hand. Recorded so the next scanner does not rediscover it.

**F-576 — 27 batches seen sold, expiring within three months, with stock still on the item, are not in Marg's newest near-expiry export (28-Aug). OPEN.** `near_expiry.py`'s cross-check from the spine's sale lines (batch and expiry per sold line) against the 7 batches Marg's export lists. Either the export's *expire before* date was set nearer than three months when it was run, or those batches are finished (the spine holds stock per item, not per batch). The file will keep asking every night; the next *Stock expiry* export (Shavez, the 1st) answers it. **Action:** read `expiry/near_expiry_latest.txt` after the first October export; if the 27 shrink to a handful, the export window was the cause and the Marg setting is the fix; if not, the batches are finished and the cross-check needs per-batch stock, which only Marg's batch-wise closing carries. Also in that file: ASTOFEN R at −38 units in Marg (batch 1131603) against 44 in the spine — a Marg-side negative batch for the owner's eye.

**F-577 — D566 said "seven clean nights of gate 14/14"; the nightly gate is 13.** The S331 cron line runs `spine_build.py` without `--acceptance`; the acceptance flag adds the 14th blocking check (stock = Marg on every item at every checkable closing) and was on only for the install run. `spine_state.json` (S338) now records the count each build actually ran — `13/13` on the box at 09:06 and 09:40. The record, not the box, was wrong. **Action:** D572 restates the witness as seven nights of `13/13`; the Book §4A says which check is the acceptance one. The assistant's.

**F-578 — The S272 close's Register row and pin-list header carry the Register's pre-correction hash.** `CANONICAL_MANIFEST.md` row `KB_Register_v5_113_S272close` and `live_pins_S272close.txt` (`source_md5`, `manifest_current_register_pin`) say `98b5769f…`; the file in canon hashes `8c8c2346…` — the 08:10 IST visible correction (kit S332 → S334 in the file's footer) moved the bytes after the row and the pin list were written, and neither was regenerated. The gate (`MD5SUMS_ALL`) was rebuilt after the correction and is right; the two records above it are one hash behind. **Action:** corrected visibly at the S274 close in the superseded row's text; and the rule already in v16 A8 — *regenerate the pin list AFTER the last edit to the Register, and re-read the manifest row against the file* — gains the case that broke it: a correction to a canon file after its row is written is a new row. The assistant's (S272).

---

**Next free: F-579 · D573.**

**END OF FAULT → ACTION REGISTER v2.99 — F-578 is the last minted finding. The v2.98 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S274 close: the first 750247 bytes of this file are byte-identical to `Fault_Action_Register_v2_98.md` (md5 b536cc275a363351ecd52143e0867bbc).**


## §7.34 · S273 (the parent, 20-Sep-2026) — F-579 … F-584

**F-579 — The records page's first real Drive read failed live: the finance web app has no Google libraries.** S332's `records.py` imported `google.oauth2` inside the app, as `marg_ingest.py` does. But `clinic-finance` runs `/usr/bin/python3` (gunicorn under the system interpreter), and only `/root/wa/venv` has the Google libraries. The walk used a Drive stub and could not see it. The first real open answered *"Drive not reachable (ModuleNotFoundError)"*, read by the assistant from the live page minutes after the install. **Action:** S335 moves every Drive read into `records_drive.py`, run with the venv python in a subprocess. The walk exercises that subprocess path too (`RECORDS_HELPER_STUB`). **Rule:** a module mounted in `finance_app.py` may import only what `/usr/bin/python3` has; anything else goes through a venv helper, and the walk runs the helper, not an in-process stub. The assistant's.

**F-580 — S335's installer "proof" read a library warning as data.** The venv's Google library writes a Python-version warning to stderr. The installer's proof merged it with `2>&1`, so the JSON count and the PDF header printed a traceback and `/root…`, while the page itself worked. **Action:** S339 re-ran the proof with stderr kept apart: `X-ray test folder 0 file(s) · one filed report starts with %PDF-`. **Rule:** a proof step never merges stderr into what it parses. The assistant's.

**F-581 — Twice in one session, a live-shape walk asserted "exactly N" of something the live data already held.** S340's walk required exactly one unknown-ID lab report. The live box already held a real one (`lu:4`), so the walk went red and nothing was installed. S345's walk required exactly two WhatsApp items, but the live queue already held a real photo saved at 09:47. The same class of fault, one kit later, after the first was fixed only in its own walk. Each refusal was the gate working as intended: nothing reached the box. Each cost the owner a publish and a paste. **Action:** S342 and S346 re-issued the same payloads (S340 and S345 frozen, never installed) with every queue assertion scoped to the walk's own rows by key. Both were re-tested against a copy that deliberately held live-like rows of every kind, and on a clean copy. **Rule (standing, from S273):** *a walk that runs on a copy of live data asserts only about the rows it created itself: find them by their keys, never by count, emptiness or "the only one". And before a kit is placed, run its walk once on a copy seeded with a real-shaped row of every kind it counts.* The assistant's.

**F-582 — `live_pins_S271close.txt` carried a stale pin for `/root/state_backup/code_bundle.py`.** The open's bulk check (bundle 01:35, 20-Sep) held 135 VPS rows against the bundle's bytes: 134 matched, 1 moved. The moved row was `code_bundle.py` pinned `a70bf6e2…`. The Register's current row says `598e55a4…` (S317 · S318), and the live file hashes exactly that. The generator took an older row for the same path. The live machine was right; the pin list was one row behind. **Action:** recorded here. The next generator run is checked for duplicate paths, keeping the latest row. The assistant's (found at the S273 open).

**F-583 — A document existed only in project knowledge at the open: `claude/UNATTENDED_2026-09-20_Q5.md`.** The v10 open-time rescue check found it in no other store: not in the repository, not in `ClaudeCowork\MANIFEST.md5`. No tranche was taken this session, so nothing was put at risk. It was not transcribed to disk either, because a re-keying with no survivor is a transcription (F-504), not a rescue. **Action:** carried to the next open as the rescue check's first line. The unattended lane's own proposal (c), a repository home for the Q5 reports, is the durable answer. The assistant's.

**F-584 — The parent's numbering slipped twice in small ways, and the board caught both.** (1) A scratch folder was named `s336` before the claim was read. The fresh read of `board/_numbers` then showed that the Sanjeevni chat had already taken S336, so the kit became S339 before anything reached `deploy_kits/`. (2) The S339 claim line first recorded `1964ea9a`, a pre-rename build's hash, not the placed file's `b7b0e43c`; corrected in the next pinned write. Both were caught by the protocol, not by luck. **Rule, restated:** read `_numbers` before naming even a scratch folder; write a pin into a claim only after the file is final. The assistant's.

---

**Next free: F-585 · D578.**

**END OF FAULT → ACTION REGISTER v2.100 — F-584 is the last minted finding. The v2.99 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S273 close: the first 756179 bytes of this file are byte-identical to `Fault_Action_Register_v2_99.md` (md5 8828815ba13700d1e61c694c4b81afd8).**

## §7.35 · S276 (the parent, 20-Sep-2026) — F-585 … F-591

**F-585 — The pharmacy spine was live in no store: ten pins under `/root/finance/spine/` absent from the code bundle, the state backup and every other store.** The S276 open's bulk pin check (158 current VPS rows against the 20-Sep 01:35 bundle) listed the whole folder among the 23 absent rows; S331 had gone live at 07:19 that morning and its own README named the gap to the parent, as did the S274 close note. The 01:35 bundle's `root/finance` entries are non-recursive by design, so a new sub-folder is invisible until named; the 01:50 state bundle names its sources outright. **Action:** closed by `S347_SPINE_BACKUP` the same morning (code, rule files and witness text into the code bundle; `spine.db` + `readings/` into the encrypted state bundle, D578). **Rule:** a kit that creates a new folder under a backed-up tree names it to the two nightly stores in its own README *and* to the parent's board key the same session — the folder is not backed up until a kit says so. The Sanjeevni chat did both; the parent took two closes to act. The assistant's.

**F-586 — The mechanism behind F-582, read: the pin generator takes the first standard-shape row for a path and never parses a four-column row.** `KB_Register_v5_115` carries `code_bundle.py` in the S267 section as `| path | md5 | note |` (`a70bf6e2`, line 234) and in the S269 section as a four-column `| path | VPS | md5 | from | kits |` row (`598e55a4`, line 4245). `gen_live_pins.py` v1.3 walks the `CURRENT LIVE FILE VERSIONS` sections top-down and records the first standard row it meets, so the stale S267 row won and the S269 row was never seen — at S271 and again at S273. **Action:** this close writes the row in the standard shape in its own §S276 section, placed above all earlier ones, which corrects the pin by construction (`8200dcca`, S347). **Rule:** a pin row is written in the two-column shape the generator reads, in the newest section, or it does not exist to the generator; a section written in another shape is decoration. The assistant's.

**F-587 — Two unattended reports in a row existed only in project knowledge (the second instance of F-583).** The open-time rescue check found `UNATTENDED_2026-09-19_Q5.md` and `UNATTENDED_2026-09-20_Q5.md` in no repository commit, no `ClaudeCowork\MANIFEST.md5` row, not on the SSD. The S271 and S273 closes had run the same check; the 19-Sep report was in the store at both and was missed by neither check's logic but by the fact that only the *newest* report was carried forward as "the rescue check's first line". **Action:** both transcribed to `01_RESCUED_FROM_PROJECT_KNOWLEDGE\S276_RESCUE\` with a note saying they are transcriptions (F-504), read back byte-identical on the PC. The durable fix is the unattended lane's own proposal (c) — the queue and its reports into `KB_canon_all`, under the gate — which needs the owner's one line and is named in `OWNER_TODO_LIVE`. The assistant's.

**F-588 — The Daily Clinic Report said "Attendance report not received" for 49 days while the attendance mail arrived beside it, and nobody read the reason.** The ONtime biometric machine's mail (*ONtime Employee Manager Report*, an ABSENT csv attached) last arrived on **01-Aug-2026**; from 02-Aug the report's attendance section found nothing and printed its fallback line every day. The clinic's own attendance server has mailed *"Attendance — day summary"* at 21:00 daily into the same mailbox since S153, and the script did not know the subject. The unattended lane named the symptom on five nights from 14-Sep; the S269 close put it on the owner's build list as item 7; no close read `DailyClinicReports/Code.gs` until this one, and the cause took ten minutes to find (one `grep` for the printed string). **Action:** closed by S348 (D579). **Rule:** a daily surface that says "not received" for more than a week is read at its source at the next close, not carried as a backlog line — a fallback message printed every day is a fault, not a status. The assistant's.

**F-589 — The v6 parser was built on the Gmail API's *rendering* of the mail, not on the bytes the script reads, and the first live run refused it.** The offline test parsed the `PLAIN_TEXT` view the Gmail tool returns (a markdown rendering of the HTML, with `| N PRESENT |` pipes and `### ABSENT` headings). Apps Script's `getPlainBody()` on an HTML-only mail returns tag-stripped text with none of that shape, so `previewAttendance` said *"found but could not be read"*. v6.1 reads the HTML body first and the plain rendering second, and was proved live the same hour. **Rule:** the S258 rule again — a surface is not the store; a parser is tested on the bytes its runtime hands it, obtained through that runtime, and a read-only live run is part of every Apps Script placement (D577 now says so). The assistant's.

**F-590 — S349's freshness banner said "0 min ago" for a file four hours old, and its walk could not see it because the walk supplied the clock.** `_banner()` took *now* from `datetime.utcnow().timestamp()`, which on a box whose local clock is IST is 5.5 hours behind the epoch, so the age came out negative and was clamped to zero. `walk_s349.py` passed `now=` into `render()` for every check, so the defect lived in exactly the input the walk had taken over. Found by reading the page live in the owner's browser two minutes after the install; closed by S350 (`time.time()`), whose walk renders once on the real clock under `TZ=Asia/Kolkata` with the S349 bytes as a negative control. **Rule (F-581's family):** a walk that hands the code the very input a defect could live in — a clock, a path, a fixture row — proves nothing about that input; every walk keeps one check on the real thing. The assistant's.

**F-591 — `gas_export.py`'s `REPO_COPY` still points at the frozen S230 photograph, so from next Sunday it will report shape drift on `DailyClinicReports` that is true and expected.** The weekly export compares the live project against `deploy_kits/S230_GAS_EXPORT/` by line count and function names (the file is masked there). S348 placed v6.1 in Google and its masked photograph in `deploy_kits/S348_DAILY_REPORT_ATTENDANCE/`; S230's folder is frozen (F-512) and still holds v5 (964 lines against 1,050). **Action:** not repaired this close — named. The repair is a data change on the VPS (`REPO_COPY` in `gas_export.conf`, if the conf carries it, else one line in the script) pointing the comparison at the current photograph, and a rule that every Apps Script placement refreshes the photograph the weekly job reads. Carried to the next session. The assistant's.

---

**Next free: F-592 · D582** (mirror; the board's `_numbers` is the source).

**END OF FAULT → ACTION REGISTER v2.101 — F-591 is the last minted finding. The v2.100 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S276 close: the first 761083 bytes of this file are byte-identical to `Fault_Action_Register_v2_100.md` (md5 bed04921632891793ffa6842c3bbe182).**


## §7.36 · S277 (the parent, 20-Sep-2026) — F-593 … F-596

**F-593 — The X-ray plan read the clinic ID only at the start of the file name, and the staff write it at the end: the test page read 0 of 9 real files as matched.** `xray_plan()` (S333) took the ID from `_LEAD`, a leading run of digits, because the brief imagined `1234 NAME.jpg`. The first real files (19-Sep evening) were `NAME 1234.jpg`, the second view `NAME 1234 ..jpg`, and sometimes the second file with no number at all. Every one of the nine went to the check folder as *no clinic ID in the file name* while six of their trailing numbers were clinic IDs with X-ray slips on 19-Sep (measured on the 01:35 nightly database). The rule was written before any staff file existed and the brief's own instruction — read the real file-name shapes off the page first — is what caught it. **Action:** repaired by S351 (D585): the ID is the one standalone number anywhere in the name; a numberless file takes a same-day sister's ID and says so; two numbers are a question for a person. The page read 6 of 9 the same hour. **The lesson:** a name-shape rule is measured on the staff's own files before it is written, never on the example in a brief. The assistant's.

**F-594 — The count rule "files == X-rays on the slip" ignored that one study can be two films, and read a correct pair as a discrepancy.** After S351 a Knee-studies patient's two photos read *2 file(s), 1 X-ray(s) on the slip — numbered, not guessed*. The question was put to the owner in one line and answered at once (D584): AP and lateral are usually on a single film; Knee studies is the unique exception — both knees AP standing on one film, both knees lateral on a second. Nothing in the record had said how many photographs a study produces; the count rule had assumed one, silently. **Action:** repaired by S353: `study_films()` expands a study into its films and the day list counts films; the page read 8 of 9. **The lesson:** when a rule counts physical things, the physical habit is asked of the person who does it, not inferred from the slip's wording. The assistant's.

**F-595 — `gas_export.py`'s repository copy was behind on two projects, not one, and its Sunday report goes to a log nobody reads.** F-591 named DailyClinicReports (S348). Reading the job at its source this session: UPIReconciliation's S230 photograph lacked `VPS_Push_Lab.gs` (S330, 19-Sep) and `VPS_Lab_Files.gs` (S333–S346, 20-Sep), so the 13-Sep and 20-Sep 02:20 runs had already reported *in Google, absent from the repository copy* for a week — into `/root/state_backup/gas_export.log`, which no health leg, no page and no session reads. The comparison existed and was correct; its answer had no reader. **Action:** the copy is now the living `GAS_CURRENT` (S352, D583), so the report is true of the current photograph; the remaining half — the Sunday report onto a surface that is read (a freshness leg on the log's age and a line on the health page when it carries findings) — is named to the next parent session, not built. **The lesson:** a check whose only output is a log is a check that nobody has run. The assistant's.

**F-596 — `canon_sums.py`'s own backups accumulate inside `KB_canon_all` and get rows: forty-one `MD5SUMS_ALL.txt.bak_*` files, from S203 to this morning, three of them carried into the repository by the S276 publish.** The gate then verifies its own backups — 59 rows naming `.bak` — and the folder that is supposed to hold canon holds forty-one copies of its own checksum list. The S269 close saw two phantom rows and retired them; nothing retired the files. **Action:** at this close the forty-one files were moved to `D:\dr-manoj-git\_to_delete_S277\KB_canon_all_bak\` with `WHY_SAFE.txt` (every one is a superseded copy of a list that is rebuilt from the folder itself; the current `MD5SUMS_ALL.txt` and its nightly backup are the survivors), the sums list rebuilt and the gate re-run green. The durable repair — `canon_sums.py` writing its backup beside `_kbtools` rather than into canon — is one line in that tool and is named to the nightly's next kit. The assistant's.

---

**Next free: F-597 · D588** (mirror; the board's `_numbers` is the source).

**END OF FAULT → ACTION REGISTER v2.102 — F-596 is the last minted finding. The v2.101 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S277 close: the first 768196 bytes of this file are byte-identical to `Fault_Action_Register_v2_101.md` (md5 f27d142cddfb8552e204221270e2cc2e).**


## §7.37 · S278 (the parent, 20-Sep-2026) — F-597, F-598

**F-597 — The S277 close wrote D587's caller tile against the record twice: "the record says only outbound calls were ever captured" and "the finalised design … a notification tile shows …".** Both were false by the record itself. Incoming calls have been captured since S138 (`call_hook_capture.py` v3, `b8a1a293`, the live bytes equal the repository's), and the design in force was **D184** — the outcome tile at hang-up, ring-time deferred until a real `call.initiated` body had been seen. The misstatement reached `START_HERE_SESSION_278` as *"read it back from the record first, do not redesign it"*, and the S278 assistant then proposed a ring-time ntfy pop-up — the one channel the owner knew could not carry the clinic's call volume on the free tier. The owner caught it and asked for the plan to be re-read. **Action:** the record re-read (Archive §S129 D181–D188, §S138, §S140); the design re-made from the vendor's own collection (D590); F-597 recorded. **The lesson:** a brief that tells the next session *"read it back from the record"* must itself have read it — a paraphrase of a decision is not the decision (D172's family). The assistant's.

**F-598 — Vitals & Plan was never migrated to the VPS, while the owner believed it had been.** The S19x migration wave was locked as Case Pack → Vitals (under a D34 waiver) → CC→Tally; Case Pack moved, Vitals did not. Its portal tile is `pc_only` (`portal.py` L457, `http://localhost:5057`), shown only on a browser marked as the clinic PC, so on his phone and on an unmarked browser it simply does not appear — which reads to its owner as *the tile is missing*. The application and its ledgers live on manojz at `D:\clinic_writer\`; the repository holds byte-identical code (`vitals_page.html` `fcedae30`, `vitals_app.py` `ba29a558`); no Vitals code is in the VPS bundle. **Action:** the owner told how to use it today; the move put on the work list by his ruling (D589, last). **The lesson:** a migration wave that stops part-way must say so on the surface the owner reads; a hidden tile is indistinguishable from a lost one. The assistant's.

---

**Next free: F-599 · D591** (mirror; the board's `_numbers` is the source).


**END OF FAULT → ACTION REGISTER v2.103 — F-598 is the last minted finding. The v2.102 and earlier end-markers above are retained in place as truncation-proofs. Pure append proven at the S278 close: the first 772778 bytes of this file are byte-identical to `Fault_Action_Register_v2_102.md` (md5 c8b264c905060e534b58bf5b9be0202e).**

---

## §7.38 — S275 (20–21-Sep-2026, the Sanjeevni project): the approval figures, and the page that shows them

**F-599 · The autofile freezes a day's UPI at the moment the report arrives, and never looks again.** *Found: 20-Sep 22:0x, from the 01:35 nightly database, on the owner's report that the approval section showed wrong cash.* The D354 autofile (S210) builds a pharmacy day from Marg's net sale and the bank's UPI **as known at that instant**; Marg's report arrives in the evening or the next morning, the bank's MPR lands hours later. **Nine of sixteen September days were filed with UPI 0** — 01, 02, 05, 09, 12, 14, 16, 17, 18 — against ₹69,444 the bank had settled, so ₹69,444 of bank money was counted as cash in the drawer. Each day then opened a `upi_vs_statement` exception with the correct figures in its text, which nobody could close, because nothing in the estate re-read the day. **Repaired** S356: `day_resync.py`, every 30 minutes 07–23, re-reads the statement for unapproved, autofiled, never-corrected days. **Root cause not repaired:** the bank-statement handler inside `finance_app.py` (the parent's) could call the resync on arrival instead of waiting for the half-hour — named, not done.

**F-600 · `day_noncash_bill` has had no row since 21-Aug: the form that wrote it went, and nothing replaced it.** Darpan's typed day form carried home and procedure medicine into that table; D354's autofile replaced the form. The *kal* page reads the table and never writes it; the ingest's `home_med` tag (S194) has **zero rows ever**, because a bill made out to `HOME MEDICINE` has no clinic ID and no phone and is parked in `sale_item_review`, never reaching `sale_item`. So from 22-Aug to 20-Sep every home and procedure bill was counted as cash: **eleven bills in September, ₹15,960 home and ₹1,337 procedure**. Nobody noticed because the surface that would have said so was reading the empty tag (F-601). **Repaired** S356: the label bills become `day_noncash_bill` rows by word lists in the `setting` table. **Not repaired:** April–August, where the days are approved and the resync does not touch approved days — the owner's call.

**F-601 · The hub's "Billed to Home Medicine" card counted a tag with no rows, and said "none" for five months.** `api_home_medicine` (S194 ⭐2) counts `sale_item.home_med = 1`. That column has never held a row (F-600). The card has read *"No home-medicine bills in the last 30 days"* since it was built, while the days underneath carried them. The owner found it by hand: *"look at 14 Sept, the home med and proc are not showing."* Verified live before touching anything: the day page, Darpan's day card and the review queue all showed 14-Sep home ₹268 (A003581) and procedure ₹293 (A003601); only that card did not. **Repaired** S359: one anchored edit inside `api_home_medicine` reads `day_noncash_bill`, home and procedure apart, and the card is renamed *Home & procedure medicine*. **The lesson:** a card that reports "none" is making a claim, and a claim with no rows behind it is never checked by the thing that makes it.

**F-602 · A kit refused itself at a login-gated health probe — the third time.** S358's installer finished placing every file correctly, then curled `/finance/darpan/kal/api/healthz`. That route sits behind the app's login gate, so curl gets a redirect to the portal, not the module's JSON; the installer read RED, restored all four files byte-identically and installed nothing. **This is F-525 (S307, withdrawn and rebuilt as S311) and F-573 (S334, restored and rebuilt as S337) for the third time in seventeen sessions.** The rule was written down at S274 — *a placed module is proven by importing it, never by curling its own login-gated route* — and it was not applied to a route that had been written months earlier. **Repaired** S359: the probe is `import darpan_kal` and a check that the four new routes exist. **The standing rule, restated with teeth:** an installer may curl only a path listed in `PUBLIC_PATHS`; every other proof is an import or a database read.

**F-603 · OPEN · The Yes Bank statement upload refuses both PDF and CSV, and the tile's message is wrong.** *The owner, 21-Sep: "upload bank statement didnt accept the pdf and csv, both formats for yes bank, fix it also when you fix this page, also fix the message on its tile."* The bank-statement door takes the ICICI merchant MPR; the Yes Bank current-account statement is a different document in a different shape, and both the formats the bank actually gives him are refused. The tile still describes what it was built for, not what it does. **Not yet investigated** — the first job of S280, before the approvals page rebuild, because it is the one thing on the page he cannot work around.

**F-604 · The approvals page shows two different drawer figures a few inches apart.** *The drawer — cash register* reads ₹4,36,597 and *In Darpan's drawer* reads ₹2,29,459 on the same screen; both are computed, over different windows, and neither is labelled with its window. A third figure, *cash in the unit* ₹6,07,317, includes the ₹87,205 of previous-financial-year cash established by the 17-Aug physical count (D323) and never says so. **To repair** in the S280 rebuild: one drawer figure, one place, with the carried ₹87,205 shown as its own line.

**F-605 · "variance 0 ✓ within ₹2,000" has been structurally always zero since the autofile.** The line compares the day's typed total against Marg's total. Since D354 the day's total **is** Marg's total, so the comparison is a number against itself: it has printed 0 every day since and will print 0 forever, while looking like a check that passed. **To repair** in the S280 rebuild: the line goes; what replaces it is the bank-vs-Marg UPI comparison, which is two genuinely independent records.

**F-606 · `finance_upi` writes its resolution text as a literal format string.** A closed `upi_vs_statement` exception carries `bank statement agrees (settled %d p, %d txns)` — the `%` arguments were never applied. Cosmetic, visible on every day the resync closed tonight (nine of them). One line in the parent's `finance_upi.py`; named to the parent's backlog.

**Next free: F-607 · D597** (mirror; the board's `_numbers` is the source).


## §7.39 — S280 (21-Sep-2026, the Sanjeevni project): the cash made one calculation, the pool, the day panel

*Six findings, F-607 … F-612, claimed on the System Board at `_numbers` v48. Four are the assistant's own (F-608, F-609, F-610's display, F-611). One is open (F-612) and is the next Sanjeevni chat's first job.*

**F-607 · The account number, carried in by the bank's own file name, reached `data_flag`.** *Found: 21-Sep, reading the live `data_flag` rows the S186 upload door had written.* Yes Bank names its download after the full account number; the door stored the file name as given. **Repaired** by S360 for everything the module writes: `finance_yesbank.mask_name` cuts the number to its last four, idempotently, and the workbench page masks the name in the browser before it is sent. **Not repaired:** `api_yesbank_statement` inside `finance_app.py` (the parent's) still logs the raw name it receives — harmless now that the page masks it, wrong in principle. One line: `fname = finance_yesbank.mask_name(fname)`. **Named to the parent's backlog.** The historical rows were masked in place, backed up first.

**F-608 · "`pdftotext -layout` is on the box" was written into S360 and never measured.** *Found: 21-Sep ~08:1x, when the owner's install line REFUSED at its own dependency gate.* The installer was right to refuse — the gate did its job and nothing was placed — but the kit's README and the session's message both asserted the tool existed. The bundle carries files, not packages, so no store could have said so; one `command -v pdftotext` in the owner's paste would have. The owner installed `poppler-utils` from one line and S360 went live unchanged. **Rule:** a kit that needs a system binary asks for it in its own first step and says so in the README; the assistant never asserts a package is present on a box it has not asked.

**F-609 · The S275 close wrote S359 "LIVE ~23:0x" on 20-Sep; its publish commit is stamped 05:13:44 IST on 21-Sep.** *Found: 21-Sep at this close, reading `.git/logs/HEAD` (commit `6109a0c`, epoch 1789947824).* A kit cannot be live before it is published, so the time in the Register, the Runbook and the manifest's narrative for S359 is at least six hours early. F-510's family — a time felt, not read — and in a close written by the very routine that carries F-510. **Corrected visibly** in Register v5.120's §S280 table note; earlier text stays as written.

**F-610 · The day panel showed the till's cash as the day's cash.** *Found: 21-Sep, the owner's screenshot of 15-Sep — "the cash should be 11291".* The old panel printed ₹11,530 for 15-Sep: Marg's sale less UPI, with the ₹239 procedure-medicine bill still inside it, because the panel read the day line before the without-cash heads were taken off. Four drawer figures lived on the approvals page and Darpan's card, each by its own arithmetic (F-604's family, found wider). **Repaired** in two steps: S363 put every Sanjeevni cash screen onto `sanjeevni_cash.py`, one calculation proven against both physical counts; S365 rebuilt the day panel so it prints *Sale − Sale returns − Paid by UPI − Without cash = Cash received*, each line opening to its bills and medicines, 15-Sep = ₹11,291 — read live in the browser at the close.

**F-611 · A kit built under a number not yet claimed (F-515, again).** *Found: 21-Sep ~15:2x, when the board read showed the parent had claimed S364 at 15:0x.* This session's day-panel kit was built and rehearsed in scratch as S364 before the claim was written. The collision was caught before the kit left scratch — renamed S365, claimed at `_numbers` v47, nothing published under the wrong number — but it is exactly the fault F-515 wrote a rule for, in a session that had claimed five kits correctly that same day. **Rule unchanged; the lesson is that the rule applies to the fifth kit as much as the first.**

**F-612 · OPEN — the owner's Yes Bank CSV is still refused after S360.** *Reported: 21-Sep ~16:0x, the owner — "the Yes Bank statement is not picking up the CSV … giving an error … didn't try the PDF."* S360 refuses, **by design**, a CSV with no `Statement Period` line, with a message pointing at the PDF (F-112: a reconciler that does not know what it has not seen calls a missing deposit fine). Yes Bank's transaction download carries no period line, so the refusal is what he will have met — but a door that refuses the only file the owner reaches for is F-603 still open from his side. **For S281, first:** get his real CSV and PDF (from `D:\Downloads` or by asking once), read the exact message he saw, load the PDF for him; then make the CSV usable without breaking F-112 — the page asks for the From/To dates when the file does not state them, and the reconciler treats only that range as seen. Until then the pool's two deposits (03-Sep ₹3,00,000, 15-Sep ₹1,00,000) are recorded in `cash_pool_deposit` on the owner's word and read *unevidenced* on the workbench.

**Next free: F-613 · D602** (mirror; the board's `_numbers` is the source).


## §7.40 — S279 (21 → 25-Sep-2026, the parent): eleven findings, F-621 … F-631

*Claimed on the System Board `board/_numbers` at v86 (F-621 … F-630) and v87 (F-631). F-613 … F-620 belong to the Sanjeevni chat's S281, open beside this close; they are written by its own close. Six of the eleven are the assistant's own (F-621, F-622, F-623, F-627, F-628, F-625 in part).*

**F-621 · A token-less probe met the finance gate's 302, not the route's 401.** *Found: 23-Sep, `S373_XRAY_LIVE` went RED at its own installer's probe.* `finance_app`'s fail-closed `_gate` redirects every unauthenticated request to a non-`/finance/api/` path with a 302 before the route's own token check can answer 401; the installer expected 401 and restored. **Repaired** by `S374_XRAY_LIVE_2` (same payload, probe accepts 302 or 401). **Rule:** a token-gated door behind the finance gate is probed for *locked* (302 or 401), never for one code. S373 frozen, never live.

**F-622 · A post-place check that did not name itself, and a journal read too wide.** *Found: 24-Sep, `S383_REPORT_NAMES` restored after placing.* The installer said only *RED after placing* — the owner and the assistant had to guess which check; and its journal check counted any `Traceback` / `NOT mounted` line in clinic-finance's last two minutes, whoever logged it. **Repaired** by `S384_REPORT_NAMES_2`: every restore names its check, the journal is read only from the install's own restart, and only a fault inside the kit's own files restores — anything else is printed as information. The pattern is carried by every installer since (S391, S392, S393, S394).

**F-623 · The service worker was registered with scope `/portal/` while the page is `/portal`.** *Found: 22-Sep, from the card's own report on Shivani's OnePlus (S369's diagnostics): permission granted, then `sw-ready-timeout`.* `navigator.serviceWorker.ready` resolves only for a worker controlling the page, and a `/portal/` scope never controls `/portal`. The assistant's own. **Repaired** by `S370_SW_SCOPE`: scope `/portal` with `Service-Worker-Allowed: /`, and the code waits on the registration it holds, never on `.ready`. **Rule:** when a phone cannot be seen, make the page report its own state before guessing (S369).

**F-624 · A lab report mailed without a clinic ID in the subject was skipped silently by both mailbox scripts.** *Found: 24-Sep ~19:30, reading Gmail after the owner's report that Sukhveer's 16:30 mails never cleared.* `VPS_Push_Lab.gs` and `VPS_Lab_Files.gs` take the ID only from *"( 8118)"* at the subject's end and `continue` on anything else: no lab_report row, no PDF filed, no warning — eight of 24-Sep's reports and their 21/22-Sep originals. **Repaired** by `S391_LAB_NO_ID` (the server joins by name only when certain, lists the rest for the desk; the filer asks the server) and `S392` (F-625). **Rule (F-112's family):** a feed that skips what it cannot parse must say so somewhere a person reads.

**F-625 · The resend reached the server before the original.** *Found: 24-Sep, the first mailbox run after S391 — 8 of 9 lines cleared, then 9 ID-less originals were listed as unsure.* The mailbox reads newest mail first, so each resend took the open order, and S391's rule followed a copy only backwards in time. The assistant's own, in part: the replay walk had fed messages oldest first. **Repaired** by `S392_LAB_NO_ID_2` (either direction, within the lab window). **Rule:** a walk replays the order the real feeder uses.

**F-626 · The petty book's save had no guard against a repeated tap.** *Found: 24-Sep, from the owner's report that Bhati's paper book (₹14,420) and the system (₹15,980) disagreed.* One 9,000 top-up was saved at 21:29:26, :38 and :55 on 22-Sep, and a 20,000 receipt twice; the thin *Save ho gaya* line did not stop him re-entering. **Repaired:** the three entries cancelled on the owner's word; `S393`/`S394` refuse the same entry by the same person within 10 minutes and show a large Hindi card naming what was saved. **Rule:** a money form proves it saved by saying *what* it saved, in the user's language.

**F-627 · A reused scratch database contaminated a walk.** *Found: 24-Sep, building S391: the S383 walk went RED at check 4 on the unchanged live file.* A scratch copy used by earlier walks still held their `pend_mark` rows, so an orphan the walk expected to see had been answered already. The assistant's own. **Rule:** every walk starts from a fresh copy of the nightly database and deletes its own rows at the end (the S391–S394 walks do both, and each was run twice to prove it).

**F-628 · Two page-template faults caught before any install.** *Found: 24-Sep, building S393.* A `width:40%` inside the `%`-formatted page shell raised *unsupported format character* on every page (caught by the walk); a CSS `content:"\25B8"` written into a Python string became the control byte 0x15 (caught by a second reader looking at the rendered screen, not by any check). **Rule:** a `%` in a `%`-template is `%%`; a CSS escape inside a Python string is written as the character itself; and a screen change is read at phone size by a second reader before it is named to the owner.

**F-629 · The Callback Tracker's `Patient_Master` Diagnosis column is empty for all 7,918 patients.** *Found: 25-Sep, answering the owner's question about diagnosis labels.* The column exists and nothing fills it; the diagnosis lives only in Docterz's *Clinical Data Report*, which the daily flow does not export (one weekly export of 3–9 Sep is on the PC: ~60 % of visits carry a diagnosis). **To repair** in the contacts build (D614): one full Clinical Data Report export from the owner, mapped through the taxonomy; a weekly export thereafter.

**F-630 · The 25-Sep nightly did not run.** *Found: 25-Sep 09:02 IST at this close's A0.2.* All three reports carry 24-Sep 07:32 — a catch-up run that day — and none carries 25-Sep, although manojz was awake by 07:35 (the Docterz pickup wrote `_last_pass.txt` then). The 23-Sep nightly was also missed. So the *run as soon as possible after a missed start* setting is not firing reliably (S276's fix, `FIX_TASK_CATCHUP.bat`). **This close performed the steps by hand** (manifest rebuild, canon sums) through the PC shell, which works again. **To repair:** read the task's history and settings on manojz from the PC shell at the next open.

**F-631 · Five live files are in no nightly bundle.** *Found: 25-Sep at this close's A12b, holding the pins against the 25-Sep 01:35 bundle.* `/root/portal/portal_sw.js`, `/root/portal/http_ece.py`, `/etc/systemd/system/ring-hook.service` (S366), `/root/wa/casepack/casepack_page.html` (S385) and `/root/wa/fu_push_on_arrival.sh` (S376) are live and not carried; each comes back byte-identical only from its repository kit. The data files `tr_dict.json` and `push_subs.json` are carried by neither. **To repair:** widen `code_bundle.py`'s allow-list (a kit, named in the backlog).

**Next free: F-632 · D615** (mirror; the board's `_numbers` is the source).



## §7.41 — S281 (22 → 25-Sep-2026, the Sanjeevni project): F-613 … F-620

*Eight findings, claimed one by one on the System Board `board/_numbers` as each was found: F-613 (read v50), F-614 (v54), F-615 (v62), F-616 (v65, taken by the v66 write), F-617 (v67), F-618 (v72), F-619 (v75), F-620 (v81). Three carry a fault of the assistant's own inside them (F-615 whole; the S386 half of F-618; the S389.1 half of F-619). Clock times are read from the board's claim lines, the publish commits in the repository clone, the audit rows and file times in the 24-Sep nightly database and bundle, the medical PC's own heartbeat and watcher log as read from Drive, and the chat's own timestamps. F-620 is repaired for visibility; the stock half of it (S397) is built and proved and waits for the owner's OK.*

**F-613 · A day filed automatically is never refreshed when a later Marg export adds a bill to it — and the screen named the login, not the source.** *Found: 22-Sep, the first job S280 left open (the ₹200 of 04-Sep).* 04-Sep was filed by the machine at 09:03:05 on 05-Sep from Marg's 08:53 export — 17 bills, ₹23,675, nobody typed it. Marg's export of 06-Sep carried an 18th bill for 04-Sep, **A003396, ₹200 cash**. The bills on the server were refreshed; the filed day never was, so 04-Sep read ₹200 short against Marg, and the page showed "manoj" as though the owner had typed the figure. **Repaired** by `S367_DAY_TRUTH_4` (published 07:16:40 IST, `0971ba6`): `day_resync.py` v4 (`ddbb12eb`) adds a pass that makes an unapproved, autofiled, never-corrected day follow Marg's latest export, one `resync_marg` audit row each time, and names — never touches — an approved day that differs; `sanjeevni_day.py` v1.1 (`5d16eff5`) says how each day was filed (which export, what time, "nobody typed it"). 04-Sep had already been approved on 21-Sep, so it was corrected once, by hand, under **D602**: cash 11,066 → 11,266, sale 23,875, one `correct` audit row stamped 07:32:27 IST on 22-Sep. The login-as-source line left the page with `S368_APPROVALS_TREE`. Across 17-Aug … 21-Sep, 04-Sep was the only day whose filed sale differed from Marg's bills. **Rule:** a record the machine filed from a source follows the source until a person approves it; after approval, a difference is named on the page, never rewritten in silence; and a screen says where a figure came from, never who was signed in when it arrived.

**F-614 · A purchase bill marked WRONG could never be cleared once Marg's re-export agreed with it.** *Found: 22-Sep, on August's purchase month page — bills 148 (Kedar, ₹4,608) and 67025 (L.K. Drug House, ₹1,862).* A bill is marked Wrong with the amount believed right; when Marg is re-exported with that amount, bill-wise and supplier-wise agree again — and because the **Correct** button was shown only while they disagreed, it left the row with the disagreement. The verdict could not be cleared and August read *"marked Wrong and not yet resolved"* for ever, so it could not be finalised. The owner's 17:51 supplier-wise export reached the server at 18:01 with exactly the two amounts. **Repaired** by `S371_PURCHASE_WRONG_RESOLVE` (published 18:16:57 IST, `dc9975b`): `purchase_app.py` rev 13 (`8788962a`) resolves a WRONG bill by itself when the live Marg amount is the amount marked right (within ₹1), audited `verdict_resolved`, never in a FINAL month; a WRONG bill keeps its Correct button until the month is final. The installer's `resolve_now.py` cleared both bills at 18:17:38 IST (two `purchase_audit` rows by `resolve_now S371`). **Rule:** a verdict a person can set stays clearable by a person until the period is closed; a button's presence never depends on the very condition the verdict describes.

**F-615 · What ICICI "holds" was counted from zero at the 17-Aug anchor, and the count refused a transfer the bank really made.** *Found: 23-Sep, the owner — recording his real ICICI → Yes Bank transfer of ₹40,000 on 20-Sep, the page answered "more than the 31,551 it holds". He took it to the parent chat, which diagnosed it, drafted a patch and handed it back on the board ("this was a part of the Sanjeevni project").* The assistant's own: S375, built this session, counted only the POS settlements credited since 17-Aug less the transfers recorded, so the money the account already held was outside the count, and S375's over-balance rule turned a short count into a refusal. His own August statement proved it: closing ₹1,50,263.42 on 31-Aug plus ₹1,45,416 credited since came to ₹2,95,679 on 20-Sep, against which he moved ₹2,90,000 and left ₹5,679. A second fault sat beside it: his two 20-Sep transfers were being called *not in the statement* by a statement that ends on 20-Sep. **Repaired** by `S377_ICICI_COUNT` (published 09:49:18 IST, `916774c`): new table `bank_anchor`, seeded at 09:50:07 IST with the bank's own 31-Aug closing and its source named; position = that balance + settlements since − transfers since; a transfer larger than the count is **recorded with the shortfall named**, never refused (every other refusal stands); *NOT in the statement* is said only once a loaded statement reaches three days past the entry. His ₹40,000 went in at 09:53:44 IST. The parent's draft in `D:\dr-manoj-git\_handover\S377_ICICI_COUNT\` is superseded. **Rule:** a count we make is never allowed to refuse what the bank did — a balance starts from the bank's own printed figure, and a statement proves absence only when it reaches past the entry's date by the settlement lag.

**F-616 · The month's payment papers printed one sheet and five blanks, with no covering letter and no signature block.** *Found: 23-Sep, the owner — "I just finalized and printed the payment sheets for the month of August. Only the vendor payment sheet came out, remaining five pages were blank. The check letter did not get printed and in the payment details sheet in the lower part, my signature line, name, etc. part is also missing."* Read in the code and in his own six-page PDF: the pay page's print rule (`ADVICE_CSS_S265`) hid everything except the advice with `visibility:hidden`, which paints nothing but keeps the layout, so the page still filled six sheets; the covering letter printed only from its own page; no paper carried a signature block; and the cheque-number door refused every write once the month was FINAL, so it could not be typed after August was locked. **Repaired** by `S380_PAY_PACK` (published 15:39:52 IST, `f38d8b7`; `purchase_app.py` rev 14 `9ad50878`, file placed 15:40:48 IST; the owner pasted the installer's *walk 14/14 GREEN … DONE*): one bank pack, three papers on three sheets (letter portrait, annexure landscape, payment sheet portrait), a *For SANJEEVNI MEDICOS / Authorized Signatory* block with a typed signatory name, letter date / cheque number / signatory typable after FINAL and audited, and the pay page's own print collapsed with `display:none` to one landscape sheet. **D605.** **Rule:** a print path is proved by printing it to PDF and counting the pages, never by reading the stylesheet; hiding for print is `display:none`.

**F-617 · Marg began saving an export to the root of the pen drive, where nothing looked.** *Found: 24-Sep, the owner — "I just exported a report from Mark [Marg] and now it is saving to the pen drive which is connected to the medical PC in the root and not to the previous location … E:\, just check and confirm."* The medical PC's heartbeat of 07:36 IST: the watcher watched `D:\MARGERP\users`, `D:\MARG REPORTS` and `C:\Users\Public\MARG`; 0 captures that day. `E:\` is the Marg backup stick, and nothing read it. **Repaired** by `S381_STICK_WATCH`: `marg_watch.py` `9f0bf9c4` → `f39ce036` sweeps the **top level** of `E:\` on the 5-second poll — `.xls` / `.xlsx` / `.pdf` only, written in the last 24 hours only, never a subfolder, never a `.mbk` backup — delivered through the Drive `_kit` channel and installed by the agent (install time not recorded; every later watcher carries the rule, and the S395 watcher's start lines of 25-Sep name it). Published as a record at 08:11:02 IST (`6026435`). **Rule:** where Marg saves is not fixed; the watcher's list of places is the pipeline's blind edge, and the heartbeat names every place it watches so a new one is noticed the first morning, not the first week.

**F-618 · The medical PC's agent was started only by the SET account's own start-up folder.** *Found: 24-Sep afternoon, when the 23-Sep bill-wise sale was still missing at 14:50 IST; the owner — "PC was restarted hours ago … Chavez [Shavez], when doing work, will log in inside the Windows user named account in which the staff logs in …".* The agent (capture, sending, offsite backup) was launched by one file, `MargAgent.cmd` in SET's own Startup (CENSUS of 26-Aug, §17). On 24-Sep the PC restarted into SET at 07:31; SET was signed out about 08:14; the staff account "user", where Shavez exports, had no start-up entry. Heartbeat frozen at 08:13:56, last send 08:17:58, nothing captured after. **Repaired** in three kits. `S386_AGENT_ANY_USER` (a guard: one agent for the machine by a file lock, the others standing by) **refused itself at its own on-machine proof, 7/8**, when the owner ran it as SET about 15:0x IST — after stepping back for an agent from the old launcher the guard read its own stopped agent as gone and started it again, because Windows reports a stopped process as ended at once while the Linux build box had kept the unreaped child visible (the assistant's own; the test had passed there). Nothing changed; **S386 is FROZEN, never installed.** `S387_AGENT_ANY_USER_2` (the guard forgets its own agent when it steps back; the test waits for the hand-over and samples four seconds for exactly one heartbeat) **installed at 15:15:56 IST as SET**, proof 8/8 on the machine — in the per-account branch, because a double-clicked file is not elevated. `S388_ENABLE_USER_ACCOUNT` put the same starter into "user" (Drive is not signed in there, so it came through the kit manifest): run by the owner at 15:41, agent up 15:41:08, heartbeat 15:41:21. **Rule:** a Windows start-up entry belongs to one account — every account that works at the counter needs its own starter or an all-users one; and a process-supervision test is proved on the target machine before anything is placed, because the build box's process semantics are not Windows'.

**F-619 · With Office broken on the medical PC, Marg's Excel export failed — and nothing in the estate read Marg's one-click text export.** *Found: 24-Sep, the owner — "There is some issue with Microsoft Office in the medical PC. So Excel is not being exported. So either I export a text file or a PDF file as you deem to be okay."* The text export (`C:\Users\Public\MARG\<id>\REPORT.TXT`) met a watcher that took only `.xls` / `.xlsx` / `.pdf`, a door that refused by magic bytes, and readers that all want Marg's nine-column sheet; 23-Sep's bill-wise sale was missing for that reason. **Repaired** by `S389_MARG_TEXT` → `S390_MARG_TEXT_LIVE` (medical PC only; no VPS, manojz or parent change): NEW `marg_txt.py` turns the bill-wise text (item detail, CASH column, *End of Report*) into **the same .XLS Marg's Excel export is**, so every reader downstream is unchanged; held by default until `MARG_TXT_LIVE.txt`. S389 went in at 19:47:21 IST and took the owner's 22-Sep text on hold at 19:47:23; the parity run against Marg's own 22-Sep Excel matched all 27 bills and the grand total to the paisa but went **RED on the medicine lines** — a two-digit line number starts one column early and S389.1 lost its first digit (the assistant's own, caught by the parity gate before anything was sent). S389.2 (`76b5eb5d`) → **PARITY GREEN 10/10**; `marg_watch.py` `ce41fdaa` re-reads the reader when it changes; the live marker was sent only after the agent confirmed both installed. **LIVE 20:02:04 IST 24-Sep**; the 23-Sep text captured 20:56:31 and VERIFIED on the server 20:56:35. **D615.** **Rule:** a new reader goes live only after parity with the format it replaces, on the same day's real data; and the switch that turns it on travels only after the corrected reader is confirmed in place.

**F-620 · A text report the watcher would not take was neither kept nor explained, and the watcher's own messages went to DEVNULL.** *Found: 25-Sep ~05:30 IST, when the owner's 24-Sep bill-wise text export was not taken — heartbeat 05:30:55, 0 captures — and nothing anywhere said why.* `medical_agent.start_watcher` starts the watcher with stdout and stderr sent to DEVNULL, and S389's design discarded a refused text silently. With manojz's link down that morning, not even the mirror could be read. **Repaired for visibility** in three steps, all on the medical PC: `S395_WATCH_LOG` (`61414a5a`; watcher start 05:35:31 IST) writes every message to `D:\SendToClinic\marg_watch.log` and keeps a refused `report*.txt` once in `_captured_txt\refused\` with a `.why.txt` — its first line at 05:35:33 said *"the reader refused it: no GRAND TOTAL line"*; `S396_TEXT_CENSUS` (`6624df0b`, delivered through the Google Drive connector alone, upload hashed before the swap; live 05:54:42) judges **any** `.txt` by its content, not its name, and every ten minutes writes a census of recent text files and the log's tail to Drive `FromMedical`; S396.1 (`ce64bb31`, live 06:02:50) copies every refused report text and its reason to `FromMedical\refused_text`. What it showed at once: the 24-Sep export was the **SUMMARY** bill-wise report (24 bills, no medicine lines, no GRAND TOTAL). Re-exported with item detail from SET, it was captured at 07:27:00 — Marg writes `user_<id>.txt` first and then renames it `REPORT.TXT`, and the any-name rule took the first — and the server answered SALE_BILLWISE VERIFIED at 07:27:03. The closing stock he then exported as text was refused and **kept** at 07:36:58 — the same mechanism working as meant — and `S397_STOCK_TEXT` (built and proved 14/14, NOT installed; waits for the owner's OK) teaches the reader that report. **Rule (F-112's family):** a feed that refuses keeps what it refused and says why, somewhere a person — or the assistant — can read without the PC link; a Marg export is judged by what is inside it, never by its file name.

**Next free: F-632 · D616** (mirror; the board's `_numbers` is the source — D615 was taken at this close, the S281 Sanjeevni, read v89).
