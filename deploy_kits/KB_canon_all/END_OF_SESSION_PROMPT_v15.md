# END OF SESSION — full session close-out (v15) · SELF-CONTAINED
...
Confirm the header reads **`register_pin_verified: yes`**. **A close that rebuilds the manifest and
...
Then write `_claude_status` (session, `lastRead`, the close note). **A page that still announces work done
...
---
*v15 · 17-Sep-2026 · S263 · SELF-CONTAINED. Supersedes v14 whole (v14 · 15-Sep-2026 · S258 superseded v13 whole). If a step is not written in this
file, it is not part of the close. The four steps the nightly now owns are VERIFIED here, never
assumed — a missing report is a RED.*
