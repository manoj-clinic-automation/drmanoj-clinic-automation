...
_audit = None
UNIT = "petty"
APP_VERSION = "S300-PETTY-BOOK-1.1"
_schema_done = False
...
def _keeper_html(con, u, msg):
    ...
    out.append("<div class='card'><h2>Aaj aap aa rahe hain?</h2>%s%s</div>"
               % (avail_row(today, "Aaj %s" % today.strftime("%d-%b")), avail_row(tom, "Kal %s" % tom.strftime("%d-%b"))))
    # balance
    need = ("<p class='bad'>Aapko <b>₹ %s</b> chahiye — Dr Manoj / Dr Bhawna se lijiye.</p>" % _r(f["need_p"])
            if f["need_p"] else "<p class='ok'>Poora hai.</p>")
    out.append("<div class='card'><h2>Aapke haath mein</h2><p class='bignum'>₹ %s</p>"
               "<p class='mut'>Rehna chahiye ₹ %s</p>%s%s%s</div>"
               % (_r(f["hand_p"]), _r(f["hold_p"]), need,
                  ...
                  ("<p class='mut'>Loan baaki: ₹ %s</p>" % _r(f["loan_p"])) if f["loan_p"] else ""))
    # money in
    out.append("<div class='card'><h2>Paise mile</h2><div class='btns'>%s</div></div>"
    ...
    out.append("<div class='card'><h2>Diary check karke kami poori ki</h2><div class='btns'>%s</div></div>"
    ...
    out.append("<div class='card'><h2>Renovation — kisko diya</h2><div class='btns'>%s"
               "<a class='tile other' href='/finance/petty/new/pay/other'>Koi aur<br><span class='sm'>naam likhna hoga</span></a></div></div>"
    ...
    out.append("<div class='card'><h2>Har mahine</h2><div class='btns'>%s</div></div>"
               % "".join("<a class='tile' href='/finance/petty/new/pay/%s'>%s<br><span class='sm'>%s · ₹ %s · %s</span></a>"
                         ...
                         for p in mon))
    out.append("<div class='card'><h2>Apna loan</h2><p class='bignum'>Baaki ₹ %s</p>%s<div class='btns'>"
               "<a class='tile' href='/finance/petty/new/loan_out/self'>Loan liya</a>"
               "<a class='tile' href='/finance/petty/new/loan_back/self'>Loan wapas kiya</a></div></div>"
               % (_r(f["loan_p"]),
    ...
    if rows:
        ...
        out.append("<div class='card'><h2>Physiotherapy — reception ki entry sahi hai?</h2>"
    ...
    if recent:
        ...
        out.append("<div class='card'><h2>Pichhle 7 din</h2><table class='grid'><tbody>%s</tbody></table></div>" % "".join(body))
    return "".join(out)
...
def _owner_html(con, u, msg):
    ...
    out.append("<div class='card'><h2>Manoj Bhati today</h2><p class='bignum'>%s</p></div>"
               % {"yes": "Available", "no": "Not available"}.get(st, "Not marked yet"))
    # what needs a tap
    ...
    if pend:
        ...
        out.append("<div class='card'><h2>Needs your tap</h2><table class='grid'><tbody>%s</tbody></table></div>" % "".join(body))
    # the three
    last_top = {}
    ...
    for h in _holders(con):
        ...
        rows.append("<tr><td><b>%s</b><br><span class='mut'>holds ₹ %s</span></td><td>%s<br><span class='mut'>%s</span></td><td>%s</td></tr>"
                    % (_esc(h["name"]), _r(h["hold_p"]), state, counted, form))
    out.append("<div class='card'><h2>The three</h2><table class='grid'><tbody>%s</tbody></table></div>" % "".join(rows))
    # S300: the opening balances, in the cash and loan sections
    op = opening(con)
    ...
    def open_form(what, label, row):
        ...
        return ("<tr><td>%s</td><td>%s</td><td><form method='post' action='/finance/petty/opening' class='inline'>"
    ...
    out.append("<div class='card'><h2>Bhati's cash</h2><p class='bignum'>₹ %s in hand</p>"
               "<p class='mut'>opening ₹ %s + received ₹ %s − diaries ₹ %s − payments ₹ %s − loans out + loans back</p>"
               "<table class='grid'><tbody>%s</tbody></table></div>"
               % (_r(f["hand_p"]), _r(f["open_cash_p"]), _r(f["received_p"]), _r(f["topup_p"]), _r(f["paid_p"]),
                  open_form("cash", "Opening cash — what he held when the book started", op["cash"])))
    out.append("<div class='card'><h2>Bhati's loan</h2><p class='bignum'>₹ %s owed</p>"
               "<p class='mut'>opening ₹ %s + loans taken − loans repaid</p>"
               "<table class='grid'><tbody>%s</tbody></table></div>"
               % (_r(f["loan_p"]), _r(f["open_loan_p"]),
                  open_form("loan", "Opening loan — what he already owed", op["loan"])))
    # renovation and monthly
    body = []
    ...
    out.append("<div class='card'><h2>Payments</h2><table class='grid'><thead><tr><th>To</th><th class='r'>%s</th><th class='r'>All time</th></tr></thead>"
               "<tbody>%s</tbody></table></div>" % (today.strftime("%b %Y"), "".join(body)))
    # the month's entries
    ...
    out.append("<details class='card'%s><summary><b>%s — every entry</b> (%d)</summary><table class='grid'><tbody>%s</tbody></table></details>"
               % ("", today.strftime("%B %Y"), len(ents), "".join(body) or "<tr><td>None yet.</td></tr>"))
