...
EXPENSE_MENU = (
    ("purchase",            "Medicine purchase (stock)"),
    ("shop",                "Shop expense (tea, cleaning, stationery)"),
    ("transport",           "Transport / courier"),
    ("salary_advance_self", "My salary advance"),
    ("other",               "Other (write details)"),
)
            ...
            if "category" in e:
                ...
                if cat == "other":
                    ...
                    expenses.append(dict(amount_p=amt, category_fixed=None,
                                         staff_id=None, category_text=det[:200]))
                    continue
                expenses.append(dict(amount_p=amt, category_fixed=None, staff_id=None,
                                     category_text=EXPENSE_LABELS[cat]))
                continue
    ...
    try:
        ...
        for e in expenses:
            ...
            con.execute("INSERT INTO day_expense (day_entry_id, amount_p, amount_known, "
                        ...
                        (eid, e["amount_p"], e["category_fixed"], _sid, e["category_text"]))
        for m in movements:
...
def selftest():
    ...
    _d = fy_start()
    while _d <= today():
        if not sqlite3.connect(DB_PATH).execute(
                "SELECT 1 FROM day_entry WHERE unit=? AND business_date=?",
                (UNIT, _d.isoformat())).fetchone():
            _ed = _d.isoformat(); break
        _d += dt.timedelta(days=1)
    check("E1a: found a free day to rehearse the menu on", _ed is not None)
    if _ed:
        r = post({"business_date": _ed, "total": "900",
                  "expenses": [{"amount": "100", "category": "junk_value"}]})
        check("E1a: an off-menu category is refused",
              r.status_code == 400 and (r.get_json() or {}).get("error") == "choose_category")
        r = post({"business_date": _ed, "total": "900",
                  "expenses": [{"amount": "100", "category": None}]})
        check("E1a: a skipped choice is refused, never written as uncategorised",
              r.status_code == 400 and (r.get_json() or {}).get("error") == "choose_category")
        r = post({"business_date": _ed, "total": "900",
                  "expenses": [{"amount": "100", "category": "other", "details": "  "}]})
        check("E1a: Other without details is refused",
              r.status_code == 400 and (r.get_json() or {}).get("error") == "details_required")
        r = post({"business_date": _ed, "total": "900", "expenses": [
                  {"amount": "100", "category": "purchase"},
                  {"amount": "50",  "category": "other", "details": "cycle puncture"},
                  {"amount": "200", "category": "salary_advance_self", "staff_id": 424242}]})
        check("E1a: a menu save goes through", r.status_code == 200
              and (r.get_json() or {}).get("ok"))
        _ec = sqlite3.connect(DB_PATH)
        _rows = _ec.execute(
            "SELECT x.category_fixed, x.staff_id, x.category_text FROM day_expense x "
        ...
        if len(_rows) == 3:
            ...
            check("E1a: the menu category wrote its canonical label verbatim",
                  _rows[1][2] == EXPENSE_LABELS["purchase"] and _rows[1][0] is None)
            check("E1a: Other wrote the maker's own words",
                  _rows[2][2] == "cycle puncture" and _rows[2][0] is None)
        ...
        _ec.execute("DELETE FROM day_entry WHERE unit=? AND business_date=?", (UNIT, _ed))
