...
def api_cn_detail():
    """Every credit note of the month, verified THE RIGHT WAY (owner, 30-Aug):
    a CN in the SALE report is a SALES RETURN -- goods a patient brought back.
    So the back-audit finds the SAME PATIENT'S earlier sale bill(s) carrying
    that item. The CN row already knows its patient (patient_ref_id, written
    at ingest); every sale bill's item lines are in sale_line_item. Vendor
    purchases have nothing to do with it -- that framing was wrong and is
    gone."""
    u, err = _require("checker")
    if err:
        return err
    con = _db()
    ensure_schema(con)
    month = str(request.args.get("month") or "").strip() or         dt.date.today().isoformat()[:7]
    out = []
    for r in con.execute(
            "SELECT e.business_date d, si.source_ref bill, si.amount_p, "
            " si.patient_ref_id pid FROM sale_item si "
            "JOIN day_entry e ON e.id=si.day_entry_id "
            "WHERE e.unit=? AND si.service LIKE '%return%' "
            "AND substr(e.business_date,1,7)=? "
            "ORDER BY e.business_date DESC", (_unit, month)):
        patient = None
        if r["pid"]:
            try:
                p2 = con.execute("SELECT clinic_id, name FROM patient_ref "
                                 "WHERE id=?", (r["pid"],)).fetchone()
                if p2:
                    patient = dict(clinic_id=p2["clinic_id"], name=p2["name"])
            except sqlite3.OperationalError:
                pass
        lines = []
        try:
            for ln in con.execute(
                    "SELECT item_name, qty_raw, amount_p, batch, expiry_ym "
                    "FROM sale_line_item WHERE unit=? AND bill_no=? "
                    "AND is_return=1", (_unit, r["bill"])):
                # the back-audit: this patient's earlier sale bills that
                # carried this item, newest first
                matches = []
                if r["pid"]:
                    try:
                        for m in con.execute(
                                "SELECT si2.source_ref bill, e2.business_date d, "
                                " sl.qty_raw, sl.amount_p "
                                "FROM sale_item si2 "
                                "JOIN day_entry e2 ON e2.id=si2.day_entry_id "
                                "JOIN sale_line_item sl ON sl.unit=e2.unit "
                                " AND sl.bill_no=si2.source_ref AND sl.is_return=0 "
                                "WHERE si2.patient_ref_id=? "
                                " AND si2.service NOT LIKE '%return%' "
                                " AND e2.unit=? AND e2.business_date<=? "
                                " AND sl.item_name=? "
                                "ORDER BY e2.business_date DESC LIMIT 3",
                                (r["pid"], _unit, r["d"], ln["item_name"])):
                            matches.append(dict(bill=m["bill"], date=m["d"],
                                                qty=m["qty_raw"],
                                                amount_p=m["amount_p"]))
                    except sqlite3.OperationalError:
                        pass
                lines.append(dict(item=ln["item_name"], qty=ln["qty_raw"],
                                  amount_p=ln["amount_p"], batch=ln["batch"],
                                  expiry=ln["expiry_ym"],
                                  earlier_sales=matches,
                                  verified=bool(matches)))
        except sqlite3.OperationalError:
            pass
        needs = any(not l["verified"] for l in lines) or not lines
        appr = con.execute(
            "SELECT status, decided_by, decided_at, note FROM "
            "darpan_return_approval WHERE unit=? AND cn_bill=?",
            (_unit, r["bill"])).fetchone()
        if needs and not appr:
            # OWNER RULING (30-Aug): everything since 1-Apr-2026 is on this
            # server, so an untraceable return is not an innocent gap -- it is
            # not entertained without the owner's approval. Opening a pending
            # row IS the manual checking place.
            ...
            appr = con.execute(
                ...
                "darpan_return_approval WHERE unit=? AND cn_bill=?",
                (_unit, r["bill"])).fetchone()
        out.append(dict(date=r["d"], bill=r["bill"],
                        amount_p=int(r["amount_p"] or 0),
                        patient=patient, lines=lines,
                        needs_approval=needs,
                        approval=(dict(appr) if appr else None)))
    pending = sum(1 for x in out
                  if x["approval"] and x["approval"]["status"] == "pending")
    return jsonify(ok=True, month=month, count=len(out),
                   total_p=sum(x["amount_p"] for x in out),
                   pending_approval=pending, notes=out)
