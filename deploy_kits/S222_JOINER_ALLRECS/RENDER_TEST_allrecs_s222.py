...
def main():
    ...
    ck("the page carries the sab-record button", "Sab record" in html)
    ...
    with sync_playwright() as p:
        ...
        ck("it says the login exists", "ban chuka hai" in line)
        ck("it names the role from the store", "(staff)" in line)
        ck("it says the password ACTUALLY WORKS", "abhi chalta hai" in line, line[:120])
        ...
        time.sleep(1.5)
        line = pg.inner_text("#loginState")
        ck("it no longer claims the password works", "abhi chalta hai" not in line)
        ck("it warns that the printed password does NOT sign in",
           "NAHIN" in line and "hota" in line, line[:160])
        ck("it still says the login itself exists", "ban chuka hai" in line)
        ck("no javascript error", not errs, "; ".join(errs))
        ...
        ck("a joiner with no login still gets the warning, not credentials",
           "bana NAHIN hai" in line, line[:120])
        ck("no javascript error", not errs, "; ".join(errs))
