#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""slip_log.py -- S324 (session 271, 19-Sep-2026), S325/S326/S327 fixes the same day; S328 adds the X-ray EMR upload list: new = above the highest ID issued before the day. The OPD & X-ray/Proc slip tile, and the night
...
def _report_html(con, m, u):
    ...
    if n_emr:
        out.append('<p class="sm">X-rays waiting for upload to Docterz: <b>%d</b> (oldest %s) \u00b7 '
                   '<a href="/finance/slips/emr">open the upload list</a></p>' % (n_emr, _dmy(old_emr)))
...
def emr_items(con):
    ...
    for s in rows:
        ...
        out.append({"key": key, "day": s["day"], "session": _session_of((s["logged_at"] or "")[11:13]),
                    "clinic_id": s["clinic_id"], "name": nm, "what": _items_label(its),
                    "slip_no": s["slip_no"], "done": done.get(key)})
    try:
        dz = con.execute("SELECT business_date d, clinic_id, MAX(patient) patient, COUNT(*) n, MIN(shift) shift "
                         "FROM clinic_day_line WHERE section='xray' AND business_date>=? "
                         "GROUP BY business_date, clinic_id ORDER BY business_date", (EMR_FROM,)).fetchall()
    ...
    for r in dz:
        ...
        if (r["d"], cid) in covered:
            continue
        key = "dz:%s:%s" % (r["d"], cid)
        out.append({"key": key, "day": r["d"], "session": {"Morning": "Subah", "Evening": "Shaam"}.get(r["shift"] or "", ""),
                    "clinic_id": cid, "name": r["patient"] or "", "what": "X-ray%s (Docterz)" % (" ×%d" % r["n"] if r["n"] > 1 else ""),
...
def emr_page():
    ...
    con = _db()
    items = emr_items(con)
    return _shell("X-ray EMR upload", _emr_html(items, request.args.get("ok") or ""), "hi")
...
def emr_mark():
    ...
    keys = [k for k in request.form.getlist("key") if re.fullmatch(r"(slip:\d+|dz:\d{4}-\d{2}-\d{2}:[0-9A-Za-z\-/]{0,12})", k)]
    ...
    for k in keys:
        ...
        if act == "done" and not it["done"]:
            ...
            n += 1
        elif act == "undo" and it["done"]:
            con.execute("DELETE FROM emr_upload WHERE key=?", (k,))
            n += 1
    ...
    return redirect("/finance/slips/emr?ok=" + _q(msg), code=303)
