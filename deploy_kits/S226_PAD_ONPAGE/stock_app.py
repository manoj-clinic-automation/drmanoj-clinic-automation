...
def _pad_brief(con, cid, root_id, fam, R):
    ...
    return dict(count_id=cid, root_id=root_id, parts=len(fam) - 1,
...
def api_pad_upload():
    ...
    try:
        ...
        return jsonify(ok=False, error="unreadable", filename=fname,
    ...
    try:
        ...
        if prev:
            ...
            b.update(ok=True, repeat=True, filename=fname,
        ...
        if summary["blocked"]:
            return jsonify(ok=False, error="empty", filename=fname,
        ...
        if part_of:
            ...
            if not root:
                return jsonify(ok=False, error="no_root", filename=fname,
        ...
        return jsonify(ok=False, error="server", filename=fname,
...
def api_pad_commit():
    ...
    root_id, fam, R = _pad_family(con, cid)
    b = _pad_brief(con, cid, root_id, fam, R)
    b.update(ok=True, from_pad=dict(md5=md5, rows=len(parsed["rows"]),
