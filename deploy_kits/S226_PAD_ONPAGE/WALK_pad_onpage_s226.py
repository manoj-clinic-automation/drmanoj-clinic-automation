...
with sync_playwright() as pw:
    ...
    chk("the Excel panel is ON THIS PAGE, under the details", pg.locator("#xl").is_visible()
        and "Download the Excel pad" in pg.inner_text("#xl"))
    chk("...its download button is shut until the details are given", pg.locator("#padget").is_disabled())
    chk("...and it says what it wants", "Fill in who counted" in pg.inner_text("#padnote"))
    ...
    chk("THE PAD DOWNLOADS FROM THE BUTTON", os.path.getsize(PAD) > 10000 and dl.value.suggested_filename.startswith("STOCK_COUNT_PAD_"),
    ...
    chk("...still to do: 333 not counted, 3 to fix", "333 items not counted, 3 rows to fix" in out, out[:400])
    chk("...AND OFFERS THE SHEET FOR THE REMAINING WORK", pg.locator("#padout a.dl").count() == 1
        and "remaining work" in pg.locator("#padout a.dl").inner_text(), pg.locator("#padout a.dl").inner_text())
    chk("...and says what to do with it: the two tabs, upload again, repeat",
        "NOT COUNTED" in out and "SENT BACK TO FIX" in out and "upload it here" in out)
    ...
    chk("THE RECENT LIST NOW SHOWS COUNT #1 WITH ITS SHEET", "Count #1" in pg.inner_text("#recent")
        and "336 still to do" in pg.inner_text("#recent") and pg.locator("#recent a.dl").count() == 1, pg.inner_text("#recent")[:200])
    ...
    chk("the recent list shows one count with one follow-up sheet, 331 still to do",
        "Count #1 (+1 follow-up sheet)" in pg.inner_text("#recent") and "331 still to do" in pg.inner_text("#recent"), pg.inner_text("#recent")[:200])
    ...
    out = pg.inner_text("#padout")
    chk("THE LAST SHEET CLOSES THE LOOP: everything counted", "Everything is counted" in out, out[:300])
    chk("...the result sheet is offered, and the doctor is pointed to the finding", "Download the result sheet" in out and "finding" in out)
    chk("...whole count = the whole shop", "Whole count so far: %d of %d" % (len(ITEMS), len(ITEMS)) in out, out[:300])
    chk("the recent list says complete", "complete" in pg.inner_text("#recent") and "Result sheet" in pg.inner_text("#recent"))
    ...
    chk("the recent list is one call, newest first, roots only",
        [c["count_id"] for c in r["counts"]] == [4, 1] and r["counts"][1]["parts"] == 2, [c["count_id"] for c in r["counts"]])
