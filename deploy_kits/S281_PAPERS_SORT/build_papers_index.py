...
def main():
    ...
    if a.canon:
        ...
        if os.path.exists(cand):
            ...
            missing.append(cand)
        # S261 (F-506): sort by the NUMBER, not the string -- 'v5_99' sorted after
        # 'v5_100' and the shelf silently read a superseded Register from v5.100 on.
        regs = sorted((f for f in os.listdir(a.canon)
                       if re.match(r'KB_Register_v5_\d+.*\.md$', f)),
                      key=lambda f: int(re.search(r'v5_(\d+)', f).group(1))) if os.path.isdir(a.canon) else []
        if regs:
