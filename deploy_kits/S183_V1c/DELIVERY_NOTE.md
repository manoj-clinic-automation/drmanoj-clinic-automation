# S183_V1c — the F-97 structural fix (live-code pin verification)

**Session 183 · 16 Aug 2026 · read-only tool · nothing live is touched**

> **V1c — the pin list rebuilt from a Register corrected by the box.** V1b installed cleanly and
> its first run found the record wrong about nine rows: eight files recorded one directory too
> high (**F-101**), and one genuinely stale md5 — `call_hook_capture.py`, pinned at its S126 value
> while the live file had been replaced on 12 Jul 2026 (**F-102**, a second instance of F-97's
> class). Of the rows it could check, **31 of 31 matched with zero drift**, and the call-hook
> receiver was measured healthy at the same time (dual-key gate ON, 26 accepted / 0 refused, and
> the service's start time later than the file's mtime — so the running worker holds the current
> bytes).
>
> **KB Register v5.4 → v5.5** carries those nine corrections, proven zero-loss (11 lines removed,
> all 11 intended; nothing else changed). This kit's `live_pins.txt` is regenerated from v5.5
> (`ff509b01…`), and every corrected pin was asserted equal to what the box actually reported
> before the kit was built. **The tool itself is byte-identical to V1b** — only the pin list moved.
>
> Expect this run to be **GREEN on all 39**. The UNTRACKED count will rise, because
> `/root/wa/call-hook/` and `/root/wa/recordings-archive/` are now scanned directories that were
> invisible before. Untracked is a WARN and does not affect the verdict.

---

> **Re-issue history — S183_V1a, which the VPS gate correctly refused.** V1a named the pin list
> `live_pins.tsv`, and `.gitignore` line 32 is `*.tsv` — one of the blanket data-format
> guards (`*.csv`, `*.tsv`, `*.xlsx`, `*.jsonl` …) that keep patient data out of a public
> repo (F-31/F-49, sharpened by D320). Git dropped the file without a word, `push_kit.bat`
> printed *"pushed successfully"*, and the VPS `SUMS.md5` check refused the incomplete kit
> — touching nothing. **The guard was right; the file extension was wrong.** The pin list
> is renamed to `live_pins.txt`. No exception was carved into `.gitignore`: a blanket PHI
> rule with holes in it is how something eventually gets through.
>
> **F-100 (new finding, fixed in this kit).** `push_kit.bat` reported success while git had
> silently dropped a kit file — `git add <folder>` says nothing about ignored files inside
> it. The publishing record claimed a file that was not there: **the same shape as F-97,
> one layer up in the toolchain**. `push_kit.bat` **v4** now checks that every file in the
> kit is actually tracked after the add, names any that were excluded and the exact
> `.gitignore` rule that did it, and **refuses to commit** — so this fails on the PC in
> daylight instead of as a red at the VPS console at night. This applies to every future
...
| `live_pins.txt` | `/root/deploy/` | the pins, **generated from KB Register v5.4** (`9506a0fe…`) |
...
- The installer itself was rehearsed against a throwaway target before shipping, including
  a corrupted-SUMS run and a mismatched-KIT_ID run, to watch it refuse both times with
  nothing placed.
- **`push_kit.bat` v4's new guard was rehearsed against a throwaway git repo** carrying a
  deliberately ignored kit file: it named the file, named the `.gitignore` rule, and
  refused to commit.
...
```
bash /root/deploy/vps_deploy.sh S183_V1c
```
...
---
*S183_V1c · built offline, gated, read-only · Session 183*
