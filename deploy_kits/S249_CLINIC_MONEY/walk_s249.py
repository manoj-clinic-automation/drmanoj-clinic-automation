#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""walk_s249.py -- the LIVE-SHAPE walk for S249_CLINIC_MONEY.
...
its own _visible_sections is asked who is shown which tile.  Nothing live is touched.
...
for user, role in (("shavez", "maker"), ("alisha", "maker"), ("shivani", "maker"), ("manoj", "checker"), ("bhawna", "checker")):
...
ck("NEGATIVE CONTROL: the live file has no our_online_p (the card read the sheet bucket, 3,150)", "def our_online_p" not in live_src)
cx.close()
...
ck("staff card 200, no JavaScript, the takeaway line", s == 200 and "<script" not in h.lower() and "Nothing to do" in h and "every difference is explained" in h, (s, re.findall(r"verdict [^\"]*\">[^<]*", h)))
ck("the numbers table and 'Explained' card are on it; no Flags card", "Explained — nothing to do" in h and "Flags — these need a person" not in h and "3 paired, 1 bank-only, 0 ours-only" in h)
...
ck("D2: ONE flag -- counter 2,300 vs Docterz 1,700, +600, whole-fee hint 'one consultation ahead'", len(m["flags"]) == 1 and m["flags"][0]["code"] == "total_diff"
   and "counter is ₹600 higher" in m["flags"][0]["text"] and "one consultation ahead" in m["flags"][0]["text"] and "cash +₹600" in m["flags"][0]["text"], m["flags"])
...
ck("the flag persists in clinic_money_flag, open, with the answer form; 'First pass done' is disabled until answered",
   "1 thing needs a person" in h and "Cannot explain" in h and "disabled>First pass done" in h, re.findall(r"<button[^>]*>First pass[^<]*", h))
...
# ================================================================ 5 the float
print("\n-- 5 the float: not revenue, moves only the handover")
s, h, r = get(MNJ, "/finance/clinic/money")
ck("owner page 200: the float not issued yet; the queue shows the two owner items", s == 200 and "not issued yet" in h and "money that never landed" in h and "sheet unfilled after the day" in h, (s, re.findall(r"— [a-z ]*</p>", h)[:5]))
...
ck("no count yet: the handover expectation equals the collection", CR.expected_handover_p(cx, D) == 380000)
s, h, r = get(ALI, "/finance/clinic/register/%s" % D)
ck("the sheet now shows the float block, prefilled 5 / 10 / 10, opened with 2,500", 'id="float"' in h and "value='5'" in h and "value='10'" in h and "Opened today with &#8377; 2,500" in h)
s, h, r = post(ALI, "/finance/clinic/money/%s/float" % D, n200="5", n100="10", n50="10", need="")
ck("kept 2,500 aside: adjust 0, handover = collection = 3,800", s == 302 and CM.float_adjust_p(cx, D) == 0 and CR.expected_handover_p(cx, D) == 380000)
...
# the next day the float comes up short: kept 2,000 -> 500 released into the handover
D6 = "2026-09-13"
post(ALI, "/finance/clinic/register/%s" % D6, cons_cash_p="1000", cons_upi_p="", cons_card_p="", xray_cash_p="", xray_upi_p="", xray_card_p="",
     proc_cash_p="", proc_upi_p="", proc_card_p="", dress_cash_p="", dress_upi_p="", dress_card_p="", physio_cash_p="", physio_upi_p="", physio_handed_to="")
s, h, r = post(ALI, "/finance/clinic/money/%s/float" % D6, n200="5", n100="10", n50="0", need="10 x 50")
ck("D6: opened 2,500, kept 2,000 -> +500 released; handover 1,000 + 500 = 1,500; change asked", CM.float_open_p(cx, D6) == 250000 and CM.float_adjust_p(cx, D6) == 50000
   and CR.expected_handover_p(cx, D6) == 150000 and dbq("SELECT need FROM clinic_float_day WHERE business_date=?", D6)[0]["need"] == "10 x 50",
   (CM.float_open_p(cx, D6), CM.float_adjust_p(cx, D6), CR.expected_handover_p(cx, D6)))
s, h, r = get(ALI, "/finance/clinic/register/%s" % D6)
ck("the sheet says: 500 short, made up next time; change needed 10 x 50; handover line names the release",
   "500 short, made up next time" in h and "Change needed: <b>10 x 50</b>" in h and "released from the float" in h)
m = CM.match_day(cx, D)
...
s, h, r = post(MNJ, "/finance/clinic/money", act="float", kind="topup", n50="10", given_to="reception", date="2026-09-14")
ck("owner tops up 500 on 14-Sep -> the float opens at 2,500 again on the 14th", CM.float_open_p(cx, "2026-09-14") == 250000, CM.float_open_p(cx, "2026-09-14"))
s, h, r = get(MNJ, "/finance/clinic/money?m=2026-09")
ck("owner's month line: the float short on 1 day (₹ 500 made up); change asked on 1 day", "short on 1 day (₹ 500 made up); change asked on 1 day" in h, re.findall(r"The float</th>.*?</tr>", h, re.S))
cx.close()
...
ck("shavez (a maker) is refused the owner's page", s == 200 and "Not permitted" in h)
...
shutil.copyfile(os.path.join(KIT, "portal.py"), os.path.join(POR, "portal.py"))
...
try:
    ...
    ck("patched portal.py imports (every tile grouped -- its own assert); grants v15", PM._tile_grants().get("version") == 15)
