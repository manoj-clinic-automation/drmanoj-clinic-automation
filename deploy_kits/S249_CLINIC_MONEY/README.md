# S249_CLINIC_MONEY — the morning match · other UPI · the float · physiotherapy · F-459
...
`portal.py 06f1b378` · `tile_grants.json v14 0efad736`. Plan: `S249_CLINIC_MONEY_FINAL_PLAN`.
...
| **Tiles** | `portal.py` + grants v15 | *Morning match* (shavez, shivani, alisha by name; the doctor lands on his line) · *Physiotherapy* (bhati, bhawna by name). Bhati masked from Attendance / Staff Register / Forms & Downloads / Scan Purchase, as Amir is. |
...
- **`walk_s249.py` — 126 checks, 126 ok** — the REAL patched `finance_app.py` over a real sqlite
  ...
  front gate; then the patched `portal.py` imported with grants v15 and asked who is shown what.
...
```
cd /root/deploy/repo && git pull --ff-only && bash /root/deploy/repo/deploy_kits/S249_CLINIC_MONEY/install_S249_CLINIC_MONEY.sh
```
...
nothing live touched) → `.bak_S249_CLINIC_MONEY_<stamp>` beside every file and the db → place →
...
`tile_grants.json v15 1498911a8aa8b546acc158c62e489415`.
