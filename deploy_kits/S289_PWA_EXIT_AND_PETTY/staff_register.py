...
EXIT_REASONS = [("late_patients", "Late patients / work"),
...
<div class="wrap" style="max-width:480px">
  ...
  <div class="card" style="text-align:center">
    <div style="font-size:13px;color:var(--muted);margin-bottom:6px">Leaving late &mdash; machine not available?</div>
    {% if ex.row %}
      {% if ex.row.status == 'approved' %}
        <div class="msg ok">&#9989; Exit approved &mdash; <b>{{ ex.row.req_ts[11:16] }}</b> counts as your punch-out.</div>
      {% elif ex.row.status == 'rejected' %}
        <div class="msg err">&#10060; Your exit request was not approved.
          {% if ex.row.decide_note %}Reason: {{ ex.row.decide_note }}{% endif %}</div>
      {% else %}
        <div class="msg ok">&#8987; Exit sent at <b>{{ ex.row.req_ts[11:16] }}</b> &mdash; waiting for the doctor.
          If approved, {{ ex.row.req_ts[11:16] }} counts as your punch-out.</div>
      {% endif %}
    {% elif ex.can %}
      <form method="POST" action="{{ prefix }}/me/exit">
        <div style="font-size:13px;margin:4px 0">Why late?</div>
        {% for k, lbl in exit_reasons %}
          <label style="display:block;text-align:left;padding:10px;margin:4px 0;border:1px solid var(--blue);border-radius:10px">
            <input type="radio" name="reason" value="{{ k }}" required> {{ lbl }}</label>
        {% endfor %}
        <div style="font-size:13px;margin:10px 0 4px">Who did you tell?</div>
        {% for k, lbl in exit_told %}
          <label style="display:block;text-align:left;padding:10px;margin:4px 0;border:1px solid var(--blue);border-radius:10px">
            <input type="radio" name="told" value="{{ k }}" required> {{ lbl }}</label>
        {% endfor %}
        <button class="btn" type="submit" style="width:100%;font-size:16px;margin-top:10px"
                onclick="return confirm('Mark your exit NOW? The current time will be recorded as your punch-out.');">
          &#128682; Mark my exit (time now = punch-out)</button>
      </form>
    {% else %}
      <div class="note">Opens at {{ ex.opens }}, your shift end.</div>
    {% endif %}
  </div>
  {% endif %}
...
def request_exit(user, reason, told):
    ...
    told = (told or "").strip()
    if reason not in dict(EXIT_REASONS):
        raise PermissionError("choose a reason")
    if told not in dict(EXIT_TOLD):
...
def me_view():
    ...
    return render_template_string(
        ...
        ex=ex, exit_reasons=EXIT_REASONS, exit_told=EXIT_TOLD,
        msg=request.args.get("m", ""), msgcls=request.args.get("c", "ok"))
...
def me_exit():
    ...
    try:
        ...
        return redirect(APP_PREFIX + "/me?m=" + ("Exit+sent+at+%s" % ts[11:16]) + "&c=ok")
    except PermissionError as e:
        return redirect(APP_PREFIX + "/me?m=%s&c=err" % str(e).replace(" ", "+"))
...
<div class="wrap">
  ...
  {% if bd.xreq %}
  <div class="card">
    <h2 style="margin:0 0 8px;font-size:15px;color:#fff">&#128682; Exit requests
...
def _selftest():
    ...
    try:
        ...
        r = c6.get(APP_PREFIX + "/me")
        assert b"Opens at 18:00" in r.data and b"Mark my exit" not in r.data, "too early: no button"
        c6.post(APP_PREFIX + "/me/exit", data={"reason": "late_patients", "told": "manoj"})
        con = get_db()
        ...
        r = c6.get(APP_PREFIX + "/me")
        assert b"Mark my exit" in r.data and b"Doctor asked me to stay" in r.data
        ...
        r = c6.post(APP_PREFIX + "/me/exit", data={"reason": "late_patients", "told": "nobody"})
        assert "c=err" in r.headers.get("Location", ""), "told must be a choice"
        r = c6.post(APP_PREFIX + "/me/exit", data={"reason": "late_patients", "told": "bhawna"})
        assert "Exit+sent+at+22:40" in r.headers.get("Location", ""), r.headers.get("Location")
        c6.post(APP_PREFIX + "/me/exit", data={"reason": "other", "told": "manoj"})
        con = get_db()
        ...
        con.close()
        r = c6.get(APP_PREFIX + "/me")
        assert b"Exit sent at <b>22:40</b>" in r.data
        # sandip: present request rejected, no machine punch -> refused
        c7.post(APP_PREFIX + "/me/exit", data={"reason": "late_patients", "told": "manoj"})
        con = get_db()
        ...
        c5.post(APP_PREFIX + "/me/exit", data={"reason": "doctor_asked", "told": "manoj"})
        con = get_db()
        ...
        assert b"Exit requests" in r.data and b"Ranjeet K" in r.data
        ...
        con.close()
        r = c6.get(APP_PREFIX + "/me")
        assert b"Exit approved" in r.data and b"22:40" in r.data
        r = c5.get(APP_PREFIX + "/me")
        assert b"not approved" in r.data and b"left at 20:00" in r.data
        try:
    ...
    print("SELFTEST OK — all-clear, exceptions, Arjun (D276), late+approver, "
          ...
          "doctor decides from pending, approved exit = punch on the day picture, audited).")
    return True
