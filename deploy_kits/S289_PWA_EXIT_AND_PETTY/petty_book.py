...
_audit = None
UNIT = "petty"
APP_VERSION = "S289-PETTY-BOOK-1.0"
_schema_done = False
...
def _audit_safe(con, row_id, action, before, after, who):
    if not _audit:
        return
    try:
        _audit(con, "petty_entry", row_id, action, before=before, after=after, who=who)
        con.commit()
...
def figures(con):
    """The book's numbers, from the entries alone."""
    ...
    hand = recv - topup - pay - lout + lback
    return dict(hand_p=hand, hold_p=hold, need_p=max(0, hold - hand), loan_p=max(0, lout - lback),
                loan_raw_p=lout - lback, received_p=recv, topup_p=topup, paid_p=pay)
...
def _keeper_html(con, u, msg):
    ...
    out.append("<div class='card'><h2>Aapke haath mein</h2><p class='bignum'>₹ %s</p>"
               "<p class='mut'>Rehna chahiye ₹ %s</p>%s%s</div>"
    ...
    out.append("<div class='card'><h2>Har mahine</h2><div class='btns'>%s</div></div>"
               % "".join("<a class='tile' href='/finance/petty/new/pay/%s'>%s<br><span class='sm'>%s · ₹ %s · %s</span></a>"
                         ...
                         for p in mon))
    out.append("<div class='card'><h2>Apna loan</h2><div class='btns'>"
...
def _owner_html(con, u, msg):
    ...
    out = []
    if msg:
        out.append("<div class='%s'>%s</div>" % ("bad" if msg in ("bad", "amount", "notyours") else "ok", MSG_EN.get(msg, "")))
    ...
    if f["loan_p"]:
        out.append("<div class='card'><h2>Bhati's loan</h2><p class='bignum'>₹ %s owed</p></div>" % _r(f["loan_p"]))
