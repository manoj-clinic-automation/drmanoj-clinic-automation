...
S289 (17-Sep-2026) — APPROVED EXIT REQUESTS FOLD AS PUNCH-OUTS: load_exit_requests()
  reads the Staff Register's exit_request rows (status approved) read-only and
  fail-soft, exactly like present requests below; the out-time in the grid
  carries a '*'. A pre-S289 register DB (no table) changes nothing.
...
def load_exit_requests(ym):
    """{(uid, 'YYYY-MM-DD'): datetime} — APPROVED mark-my-exit requests (S289,
    owner 17-Sep-2026) for ym from the Staff Register DB, read-only. A staff
    member leaving late when the machine cannot be reached raises it at the
    moment of leaving; the server receipt time IS the punch-out. Folded exactly
    like load_present_requests(): a synthetic punch, so the departure side (OT
    candidate, early exit) reads it like a machine punch-out. Empty dict on ANY
    problem, including a register DB older than S289 with no exit_request table."""
    import sqlite3
    path = os.environ.get("ATT_REGISTER_DB", REGISTER_DB_DEFAULT)
    out = {}
    if not path or not os.path.exists(path):
    ...
    try:
        ...
        try:
            rows = con.execute(
                "SELECT staff_id, reg_date, req_ts FROM exit_request "
                "WHERE status='approved' AND reg_date LIKE ?",
                (ym + "-%",)).fetchall()
        finally:
            con.close()
...
def collect_month(ym, att_core, cfg, pol_all, upto_today=True):
    ...
    # S289: approved mark-my-exit requests are synthetic punch-outs, same rule.
    xreq = load_exit_requests(ym)
    for (uid, dstr), dt in xreq.items():
        punches.append((uid, dt))
    ...
    for d in range(1, ndays + 1):
        ...
        for uid, a in acc.items():
            ...
            if (uid, date.isoformat()) in xreq and xreq[(uid, date.isoformat())] == last:
                cell["xreq"] = True                   # S289: punch-out by approved exit request
            if last < sched_end:
...
def write_outputs(ym, rows, money_log, acc):
    ...
    def cellhtml(c):
        ...
        l2 = ""
        if c.get("out"):
            l2 = f'<br>{c["out"]}' + ("*" if c.get("xreq") else "")
            if c.get("early"):
...
* = present by APPROVED register request (the request time is the punch time); on the out-time, left by an APPROVED exit request.</p>
...
def selftest():
    ...
    try:
        ...
        # ---- S289: approved exit request = synthetic punch-OUT ----
        # No exit_request table yet (a pre-S289 register DB) -> nothing changes.
        assert load_exit_requests(ym6) == {}, "missing exit table must read as empty"
        _rc = _sq3.connect(reg_db)
        _rc.execute("CREATE TABLE exit_request (id INTEGER PRIMARY KEY, reg_date TEXT,"
                    " staff_id INTEGER, req_user TEXT, req_ts TEXT, reason TEXT, told TEXT,"
                    " status TEXT, verify_user TEXT, verify_ts TEXT, decide_user TEXT,"
                    " decide_ts TEXT, decide_note TEXT)")
        # TestB: APPROVED exit at 23:10 on the request-backed day; TestC: PENDING.
        _rc.execute("INSERT INTO exit_request(reg_date,staff_id,req_user,req_ts,reason,told,"
                    "status) VALUES('2026-12-01',2,'testb','2026-12-01 23:10:00','x','manoj','approved')")
        _rc.execute("INSERT INTO exit_request(reg_date,staff_id,req_user,req_ts,reason,told,"
                    "status) VALUES('2026-12-01',3,'testc','2026-12-01 23:00:00','x','manoj','pending')")
        _rc.commit(); _rc.close()
        rows8, ml8, _fp8, _rv8, acc8 = month_report(ym6, att_core, cfg)
        by8 = {r["Name"]: r for r in rows8}
        g8 = acc8[2]["grid"][1]
        assert g8["out"] == "23:10" and g8.get("xreq") is True and g8["ot"] > 0, g8
        assert by8["TestB"]["Present"] == 1 and by8["TestB"]["Late marks"] == 2, \
            "an exit request must not touch presence or the late side"
        assert by8["TestC"]["Present"] == 0 and by8["TestC"]["Absent"] == 1, \
            "a PENDING exit request must change nothing"
        assert by8["TestB"]["OT cand. minutes"] == g8["ot"], "exit feeds the OT candidate"
        _q1, _q2, q8 = write_outputs(ym6, rows8, ml8, acc8)
        assert "23:10*" in open(q8, encoding="utf-8").read(), "exit-backed out-time carries the *"
        _rc = _sq3.connect(reg_db); _rc.execute("DELETE FROM exit_request"); _rc.commit(); _rc.close()
