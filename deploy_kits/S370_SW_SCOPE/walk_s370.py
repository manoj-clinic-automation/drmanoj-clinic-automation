#!/usr/bin/env python3
"""walk_s370.py -- the S370 walk (portal side): the card reports, the doctor reads, staff cannot; the S366 portal.py is
the negative control. Usage: python walk_s370.py <app dir: portal.py portal_push.py ring_common.py portal_sw.js
...
tmp = tempfile.mkdtemp(prefix="s370walk_")
...
ok("scope:'/portal'" in h and "_swActive" in h and "sw-activate-timeout" in h and "DENIED_TXT" in h, "home carries the S370 card: scope /portal, own-registration wait, activate timeout")
...
if neg:
    ...
    ok("await _withTimeout(navigator.serviceWorker.ready" in nc.get("/portal").get_data(as_text=True) and "await _withTimeout(navigator.serviceWorker.ready" not in h, "negative control: the S369 page still waits on .ready; the S370 page never does")
