...
SALARY_EXCLUDED = {"ADVANCE_ISSUE", "LOAN_CAPITALISE", "LOAN_SKIP", "PERK", "SALARY_PAID",
                   # S199: match the ledger after D332/SL6 added these two
                   # Rs 0 marker rows (deferral + capacity hold; no money).
...
_COLS = [
    ...
    ("", "Extra duty +", "extra_rs"),
    ("", "Outstation +", "outst_rs"),
    ("", "Ledger money", "ledger_fold"),
    ("", "Incentive->pot", "incentive_pot"),
    ("", "Partial month adj.", "prorate_delta"),
    ("", "NET (new)", "final_net"),
]
# S199-C (owner): Old(shadow)/Delta columns removed — the retired ledger
# code's net had no attendance deductions, so the comparison misled (the
# scenario/flow pages own old-vs-new). shadow_* fields still computed, unused.
...
def render_html(ym, rows, problems, pot_total, embed=False):
    ...
    # S199-C: the parity headline is retired with the shadow columns.
    ...
    parts.append("<p class='note'>NET (new) is the standalone take-home the register "
                 ...
                 "to the annual pot (paid the following Diwali). Old-vs-new comparison lives on the "
                 "Deduction-scenario and Month-end-flow pages.</p>")
    return "\n".join(parts)
...
def _selftest():
    ...
    assert "standalone" in h and "Tester" in h and "NET (new)" in h and "Old (shadow)" not in h
