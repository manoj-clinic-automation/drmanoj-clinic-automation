...
> **Superseded on 23-Aug-2026 by `S195_Marg_decrypt_partial_key.md`**
> (md5 `3f83f1594fcb22e29b6aba0458e6574b`), which records a THOROUGH NEGATIVE:
> four independent attacks over all 27,246 records and all seven `*.c18` files
> failed, and the identical 19-byte header prefix across files of 809 B to 13 MB
> **falsifies** the "XOR of a standard DBF" hypothesis this note is built on.
> The `0x30` and `256` "confirmations" below are consistent with coincidence or
> with the wrapper, not with a real decrypt. **Remote decryption is RETIRED.**
> The standing answer to "why don't we just read Marg's database?" is now carried by
> **`MARG_AND_MEDICAL_PC_MASTER_REFERENCE_v3.md` §4.3** (md5 `579ea885e440e76af73de3ecc4542d71`),
> which cites both S195 documents so the supersession is visible.
> Label added at S203 after this note was mistakenly asserted as current in a
> master reference. Retained, not deleted (F-23).
