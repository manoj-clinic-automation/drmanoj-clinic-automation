...
REM push_kit.bat v3 — finds git even when it is not on PATH; never claims success unless the push succeeded.
...
if not defined GIT (
  echo !! git.exe not found on this PC.
  echo    Either install Git for Windows, or commit the folder
  echo    %REPO_DIR%\deploy_kits\%KIT_NAME%
  echo    with GitHub Desktop the way you committed the S180 kits.
  pause & exit /b 1
)
