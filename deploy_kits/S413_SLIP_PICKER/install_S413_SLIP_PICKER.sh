#!/bin/bash
# =============================================================================
#  install_S413_SLIP_PICKER.sh · kit S413_SLIP_PICKER (session 282, 26-Sep-2026)
#
#  The owner, 26-Sep-2026: the X-ray/Proc form gets ONE search box over X-rays and procedures (SBK / BK find the
#  right lines), the most-used lines as buttons with prices, every chosen line in full, a running total; the room
#  rows list each line with its price. Saving posts the same fields as before. Proved on two scratch copies of the
#  live database before anything is placed (the browser part of the walk ran in the build session; here it is skipped).
#  FILE:  /root/finance/slip_log.py  S401 36405348 -> S413 fa2d21bf.  Restarts clinic-finance only.
#
#  Run by (one line on the VPS):
#    cd /root/deploy/repo && git pull --ff-only && bash /root/deploy/repo/deploy_kits/S413_SLIP_PICKER/install_S413_SLIP_PICKER.sh
# =============================================================================
set -u
KIT="S413_SLIP_PICKER"; KDIR="$(cd "$(dirname "$0")" && pwd)"
...
STAMP="$(date +%Y%m%d_%H%M%S)"; WALK="/tmp/s413_walk_$STAMP"
S_FROM=364053483a5a4c4161663e77775f1a01; S_TO=fa2d21bfbf5fae297857e7da58a1ceae
...
[ "$(m5 "$FIN/slip_log.py")" = "$S_FROM" ] || { say "!! [2/6] slip_log.py is $(m5 "$FIN/slip_log.py"), not S401 - nothing installed"; exit 1; }
say "[2/6] live pin exact (slip_log S401)"
mkdir -p "$WALK" || exit 1
"$VPY" -B -c "import sys, hashlib; sys.argv = sys.argv[1:]; exec(open(sys.argv[0]).read().split('open(sys.argv[2]')[0]); assert hashlib.md5(s.encode()).hexdigest() == sys.argv[2]" \
  apply_s413.py "$FIN/slip_log.py" "$S_TO" 2>/dev/null || { say "!! [3/6] the kit file is not the live file plus S413 - nothing installed"; rm -rf "$WALK"; exit 1; }
"$VPY" -B -m py_compile slip_log.py walk_s413.py || { say "!! [3/6] compile failed - nothing installed"; rm -rf "$WALK"; exit 1; }
...
WOUT="$( cd /tmp && timeout 300 "$VPY" -B "$KDIR/walk_s413.py" "$KDIR/slip_log.py" "$FIN/slip_log.py" "$WALK/scratch_A.db" "$WALK/scratch_B.db" 2>&1 | tail -1 )"
rm -rf "$WALK"
echo "$WOUT" | grep -q "^WALK OK" || { say "!! [4/6] walk red: $WOUT - nothing installed"; exit 1; }
say "[4/6] $WOUT"
BS="$FIN/slip_log.py.bak_S413_36405348"
...
say "[6/6] $KIT: DONE -- open the X-ray/Proc form: https://followup.dr-manoj.in/finance/slips?s=xp"
