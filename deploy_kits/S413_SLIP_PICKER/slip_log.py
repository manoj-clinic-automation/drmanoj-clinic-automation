...
def log_slip(con, who, series, slip_no, clinic_id, no_id_name, items, confirm_gap=False, day=None):
    ...
    if clinic_id:
        ...
        if p:
            name_seen = p[0]
        if is_new_id(con, clinic_id, day):                     # S326: new is decided by the number, not by the name
            is_new = 1
    ...
    if day != today:                                      # S401: a late parchi never marks numbers as chhoota --
...
(function(){
  ...
  [].forEach.call(document.querySelectorAll('.pk'),function(pk){        /* S413: the picker */
    ...
    function norm(t){return (t||'').toLowerCase().replace(/[^a-z0-9\u0900-\u097f]+/g,'');}
...
.rrow.old .rl{color:var(--warn)}.il{display:block;font-size:15px;color:var(--ink)}.il em{font-style:normal;font-weight:700;color:var(--accent)}.rl{flex:1;min-width:0}.amt{font-weight:700;white-space:nowrap;font-size:15px;color:var(--ink);background:#eef2f4;border-radius:7px;padding:4px 8px}.rl{font-size:15px}.rb{display:flex;gap:4px;flex:0 0 auto}
