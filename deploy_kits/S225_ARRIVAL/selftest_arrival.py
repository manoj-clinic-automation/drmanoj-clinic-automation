...
# -*- coding: utf-8 -*-
"""
selftest_arrival.py -- S225 (rev 8): the rev-7 selftest as regression, then §16 the arrival flow. Base: the rev-6 selftest (rounding now in the engine), then §14 the phone book. Base: the S224 selftest UNCHANGED as regression, then §13 the staff order page. Base: the REAL blueprint, on a temp finance.db, fed the REAL
...
ck("the plan's ITEM 1 line grew by the shortfall and says 'carried', still a multiple of 10", any("carried" in w for w in it1["why"]) and it1["order_strips"] % 10 == 0 and it1["order_strips"] > names["ZZTEST ITEM 1"]["order_strips"])
