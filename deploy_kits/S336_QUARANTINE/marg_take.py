...
WHAT IT GUARANTEES
    ...
    3. NO RAW EXPORT WITH A PATIENT NUMBER AT REST (S186).  A sale report is read into PHI-free
       item lines and the file is deleted in the same call.  Stock, purchase and item files
       carry none and are kept.
       S336 (D567 item 3, Book s11.4 3d): a REFUSED or UNKNOWN file is no longer deleted on sight.
       It stays in the router's own quarantine (archive/_REFUSED, archive/_UNKNOWN, beside the .txt
       the router writes) ONLY when phi_scan.clean() finds no person's detail in it -- no mobile-shaped
       number in any cell, no sale-family word in its preamble, not a PDF, readable and non-empty.
       Otherwise it is deleted exactly as before, and the sidecar says so.  A kept file is re-judged
       by marg_rescan_vps.py whenever signatures.json changes, and rescued through THIS door.
...
EXT_MAGIC = {".xls":  (b"\xd0\xcf\x11\xe0", b"PK\x03\x04"),     # Marg emits both under .xls
             ".xlsx": (b"PK\x03\x04",),
             ".pdf":  (b"%PDF",)}
SOURCES = ("manual", "push", "drive", "pc", "test", "rescan")   # S336: rescan = a rescued quarantine file re-taken
...
def _quarantine_clean(dest):
    """S336: may a refused / unknown file rest in the router's quarantine?  phi_scan decides; any
    failure to decide is a no.  The sidecar the router wrote beside it records the answer."""
    try:
        import phi_scan                                          # noqa: PLC0415
        ok, why = phi_scan.clean(dest)
    except Exception as e:                                       # noqa: BLE001
        ok, why = False, "phi_scan could not run: %s" % str(e)[:80]
    _sidecar_note(dest, "kept: %s -- %s" % ("yes" if ok else "no",
                                            "no person's detail found (S336); re-judged by marg_rescan_vps.py when signatures.json changes"
                                            if ok else why))
    return ok


def _sidecar_note(dest, line):
    """Append one line to the router's .txt beside a quarantined file; never raises."""
    try:
        p = os.path.splitext(dest)[0] + ".txt"
    ...
    try:
        ...
        try:
            ...
            if dest and os.path.exists(dest):
                if not phi:
                    kept = 1
                elif verdict != "VERIFIED" and typ not in MI.PHI_TYPES and _quarantine_clean(dest):
                    kept = 1                               # S336: no person's detail found -- kept for the rescan
                else:
                    os.remove(dest)                        # S186: no raw export with a number at rest
                    if verdict != "VERIFIED":
                        _sidecar_note(dest, "kept: no -- deleted at the door (S186: a person's detail, or a PDF, "
                                            "or nothing could be established)")
