...
def _item_life(con, item, ps, since_iso, upto_iso):
    ...
    purchases, in_units, in_lines = [], 0, 0
    if names:
        q = ",".join("?" * len(names))
        vend = {}
        ...
        for r in con.execute(
                ...
                "loose_qty, rate_p, purchase_rate_p, direction FROM purchase_line WHERE item IN (%s) "
                "AND bill_date IS NOT NULL AND bill_date>=? AND bill_date<=? "
                "ORDER BY bill_date, bill_no" % q, tuple(names) + (since_iso, upto_iso)):
            qty, free, loose = float(r[7] or 0), float(r[8] or 0), float(r[9] or 0)
            units = int(round((qty + free) * ps + loose))
            ...
            purchases.append(dict(vendor=vend.get(r[0]) or (r[0] or "").upper() or "-",
                                  ...
                                  qty=qty, free=free, loose=loose, units=units,
    ...
    if len(marg) >= 2:
        a, b = marg[0], marg[-1]
        # documents between the two export days (after a, up to b)
        p_units = sum(x["units"] for x in purchases if a[0] < (x["bill_date"] or "") <= b[0])
        s_units = r_units = 0
        for r in con.execute("SELECT is_return, qty_raw FROM sale_line_item WHERE unit=? AND item_key IN (?, ?) "
                             "AND business_date>? AND business_date<=?", (_unit, item, sk, a[0], b[0])):
        ...
        det = dict(from_date=a[0], from_text=_r_dmy(a[0]), from_qty=a[1], to_date=b[0], to_text=_r_dmy(b[0]),
                   ...
                   docs_delta=docs_delta, residue=marg_delta - docs_delta,
                   sentence=("From %s to %s Marg's figure moved %+d; bills explain %+d "
                             "(bought %d, sold %d, returned %d); %s."
                             % (_r_dmy(a[0]), _r_dmy(b[0]), marg_delta, docs_delta, p_units, s_units, r_units,
                                ("no residue -- every unit is accounted for by a document" if marg_delta == docs_delta
                                 else "%+d units moved with NO document behind them" % (marg_delta - docs_delta)))))
    return dict(item=item, pack_size=ps, since=since_iso, since_text=_r_dmy(since_iso), upto=upto_iso,
                upto_text=_r_dmy(upto_iso), purchase_names=names,
...
def _purchases_since(con, since_iso, upto_iso):
    ...
    out = {}
    for r in con.execute("SELECT item, bill_date, qty, free, loose_qty, direction FROM purchase_line "
                         "WHERE bill_date IS NOT NULL AND bill_date>=? AND bill_date<=?", (since_iso, upto_iso)):
        k = _pad_norm(r[0])
...
def _lane_of(d, ctx):
    ...
    if diff > 0 and diff >= max(2 * pack, 20) and diff >= max(1, abs(marg)) * 0.5:
        return "recount", "", "counted far above Marg (+%d units, Marg %d) -- nobody has that much extra; an entry slip is likely" % (diff, marg)
    ...
    box = 10 * pack
    if abs(diff) >= box and abs(diff) % box == 0:
        if ctx["max_purchase_q"] * pack >= abs(diff):
            return "bill", "", "the gap is %d box%s of 10 strips and a purchase of that size sits in the window -- see the bill" % (abs(diff) // box, "" if abs(diff) == box else "es")
        return "recount", "", "the gap is a whole box (%d strips) but no purchase of that size is on the server -- count again" % (abs(diff) // pack)
    # 3 -- Marg moved without a document
    if res not in (None, 0) and abs(res) >= max(2, pack):
        return "marg", "", "Marg's figure moved %+d units more than the bills explain since %s -- an adjustment or a late bill; look in Marg first" % (res, ctx.get("res_from") or "the first export day")
    # 6 -- over
    if diff > 0:
        return "over", "", "more on the shelf than Marg (+%d) -- a wrong-item sale or an unentered purchase; never a loss" % diff
    # 5 -- within the allowance
    allow = ctx["allowance"]
    if -diff <= allow:
        return "allowance", "", "short %d unit%s, within the allowance of %d for this item (%d sold, %d loose) -- expected" % (-diff, "" if diff == -1 else "s", allow, sold, loose)
    # 4 -- real shelf loss
    return "loss", "", "Marg agrees with the bills; the shelf is short %d unit%s (allowance %d) -- breakage, unbilled sale or theft" % (-diff, "" if diff == -1 else "s", allow)
...
def _pad_report_data(con, root_id):
    ...
    def ctx_for(item, pack, need_residue):
        ...
        res, res_from = None, None
        if need_residue:
            try:
                life = _item_life(con, item, pack, since, upto)
                if life.get("detector"):
                    res, res_from = life["detector"]["residue"], life["detector"]["from_text"]
            except Exception:                                 # noqa: BLE001
                res = None
        return dict(sold=s[0], loose=s[1], last_sale=s[2], bought_fy=bool(p[0]), last_purchase=p[1],
                    max_purchase_q=p[2], unit_p=up, price_source=usrc, residue=res, res_from=res_from,
    ...
    for d_ in R["differences"]:
        ...
        d.update(lane=lane, lane_title=LANE_TITLE[lane], kind=kind, why=why, allowance=c["allowance"],
                 ...
                 last_purchase=c["last_purchase"], residue=c["residue"], salt=c["salt"],
