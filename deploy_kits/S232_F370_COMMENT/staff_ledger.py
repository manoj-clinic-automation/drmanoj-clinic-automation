...
# beside users.json (default 50 for EVERYONE -- D352 (S204) RETIRED Darpan's
# 75% exception, and D425 (S232) confirmed there is no grandfathering: 50%
# through August. D352's own rule is why this comment mattered -- an
# exception belongs in a settings row, never in a code fallback -- and this
# line was the last place that retired exception still lived (F-370, S232;
# the DATA was always right, the box has held {"Darpan": 50} throughout).
# Above the ceiling an advance is SPECIAL: it must say so, it is NEVER
# direct (even a checker's own entry goes PENDING), and it requires the
# signed written application (Dr Manoj / Dr Bhawna) against the row --
# but SINCE D374 (S225, the owner: "make it optional for me, I can add it
# later") the checker MAY APPROVE FIRST and attach later. The row then
# carries application_owed and every screen says so until save_application()
# clears it. This block said "approval refuses" until S232; decide() below
# has carried the correct D374 note since S225. Two records, one file.
