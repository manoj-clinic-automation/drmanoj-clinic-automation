...
def page_health():
    ...
    for c in h["checks"]:
        ...
        inner = ("<div class=dot style='background:%s'></div>"
                 "<div class=body><div class=lbl>%s</div><div class=det>%s</div>%s%s</div>"
                 % (cc, c["label"], c["detail"],
                    ("<div class=hint>%s</div>" % c["hint"]) if c["hint"] else "",
                    ("<div class=act>&#8594; %s</div>" % act)
                    if (act and c["state"] not in ("ok", "info")) else ""))
        if tgt:
            rows.append("<a class=row href='%s'>%s<div class=go>&#8594;</div></a>"
                        % (tgt, inner))
        else:
            rows.append("<div class=row>%s</div>" % inner)
    html = ("""<!DOCTYPE html><html lang=en><head><meta charset=utf-8>
<meta name=viewport content="width=device-width,initial-scale=1">
...
a.row{text-decoration:none;color:inherit;border-radius:8px}
a.row:hover{background:#f7f6f2}
.body{flex:1;min-width:0}
.go{margin-left:auto;color:#b9b4a8;font-size:16px;align-self:center;flex:none}
.act{color:var(--brand);font-size:12.5px;margin-top:4px;font-weight:600}
...
<div class=wrap>
 <div class=hero><div class=w>%s</div><div class=a>%schecked %s</div></div>
 <div class=card>%s</div>
 <div class=foot>Refresh to re-check. This page asks one question: is
 anything wrong right now? &mdash; and every row is a door to the place that
 fixes it.</div>
</div></body></html>""" % (wash, col, col, word,
       (("&#8594; " + " \u00b7 ".join(h.get("culprits") or []) + "<br>")
