#!/usr/bin/env python3
# S193_CASHPOS4 - restore the LOUD stat tiles for each Cash-position line
# (drawer/reserve/Manoj/banked) while keeping them tap-to-expand.
# In-place html patch, fail-loud, idempotent.
import sys
...
NEW=r"""function loadCashPos(){
  fetch("/finance/api/cash-position?_="+(new Date()).getTime(),{cache:"no-store"}).then(function(r){return r.json()}).then(function(j){
    ...
    function row(label,val,detailHtml){
      return '<div class="custPerson" style="border-top:1px solid #eee">'+
        '<div onclick="tog(this)" class="stat" style="cursor:pointer">'+
        '<span class="lbl">'+label+' <span class="mut" style="font-weight:400">▸ tap</span></span>'+
...
def main():
    ...
    if h.count(OLD)!=1: print("*** PREFLIGHT FAILED: anchor %d times, expected 1."%h.count(OLD)); sys.exit(2)
    open(TARGET,"w",encoding="utf-8").write(h.replace(OLD,NEW,1))
    print("      loud stat tiles restored (still tap-to-expand).")
