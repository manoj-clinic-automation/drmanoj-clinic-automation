#!/bin/bash
# =============================================================================
#  install_p2a.sh · kit S187_P2a — the Sanjeevni tile lands on the hub and
...
#
#  EXPECT: finance selftest ~392/392 · portal gate 18/18 · both services
#  answering · pins 43 / 0 / 0 AMBER (pending until the close).
#
#  Rehearsal: P1A_FIN=... P1A_PORTAL=... P1A_DEPLOY=... P1A_NOSVC=1 bash install_p2a.sh
# =============================================================================
set -u
KIT_NAME="S187_P2a"
...
OLD_APP="fe92a74dda3b374c813cab3c98ad2897"; NEW_APP="04088c31df9e4ed3913121d6945406aa"
OLD_PG="e37cb13b225316980b3566e8f6321120";  NEW_PG="8ec5ff700529c9758b2d62f4cc447040"
OLD_POR="34dbeef7292bc98752398a26c021a224"; NEW_POR="bd4ed0a3b89659676e7e193998eeb1a9"
...
&& [ "$(cur finance_app_P2a.py)" = "$NEW_APP" ] \
&& [ "$(cur finance_approvals.html)" = "$NEW_PG" ] \
&& [ "$(cur portal_P2a.py)" = "$NEW_POR" ] \
&& echo "-- payload hashes match the Register v5.18 pins" \
...
"$PY" smoke_portal_P2a.py portal_P2a.py "$POR/portal.py" || { echo "RED - portal gate. Nothing changed."; exit 1; }
...
# ---- finance first ----------------------------------------------------------
cp -f finance_app_P2a.py "$FIN/finance_app.py"
...
# ---- portal second ----------------------------------------------------------
cp -f portal_P2a.py "$POR/portal.py"
echo "-- portal placed"
if [ "$NOSVC" != "1" ]; then
...
cp -f live_pins_P2a.txt "$DEP/live_pins.txt" && echo "-- pin list updated"
if [ "$NOSVC" != "1" ]; then
