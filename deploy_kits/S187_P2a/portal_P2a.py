...
TILES = [
    ...
    {"icon": "\U0001F4CB", "name": "Surgical Case Pack",
     "desc": "Estimate \u00b7 OT list \u00b7 consent \u00b7 case history", "live": True,
     "url": "/portal/casepack",
     "roles": ["doctor"]},
    ...
    {"icon": "\U0001F4CB", "name": "Case Pack \u00b7 PC fallback",
     "desc": "Emergency fallback \u2014 use the portal tile", "live": True,
...
_TILE_GROUP = {
    ...
    "Surgical Case Pack": "Clinic", "Send WhatsApp": "Clinic", "Follow-up WhatsApps": "Clinic",
    "Follow-up Tracker": "Clinic PC tools", "Vitals & Plan": "Clinic PC tools",
    "Case Pack \u00b7 PC fallback": "Clinic PC tools", "CC Statements \u2192 Tally": "Clinic PC tools",
...
<div class="wrap">
  ...
  {% for t in items %}
    {% if t.live %}
      <a class="tile live" href="{{ t.url }}" target="_blank" rel="noopener"{% if t.clinic_meta %} data-clinic-tile{% endif %}>
        <div class="ic">{{ t.icon }}</div>
        <div class="nm">{{ t.name }}</div>
        <div class="ds"{% if t.review_counts %} data-review-counts{% endif %}{% if t.gist %} data-gist-summary{% endif %}{% if t.sanjeevni_counts %} data-sanjeevni-counts{% endif %}{% if t.daily_sale_counts %} data-daily-sale-counts{% endif %}>{{ t.desc }}</div>
...
(function(){
  ...
  fetch('/finance/api/tile-summary',{credentials:'same-origin'})
   ...
   .then(function(d){
     ...
     if(d.marg_pushes)p.push('📄 '+d.marg_pushes+' Marg report'+(d.marg_pushes>1?'s':''));
     if(d.missing_marg)p.push('📭 '+d.missing_marg+' day'+(d.missing_marg>1?'s':'')+' no Marg');
