...
F-185: this module reads patient names and clinic IDs off the sheet because they are in the
columns it walks past. It NEVER returns them, stores them, or logs them. Only counts and money.
"""
import re
...
def parse_day_revenue(ws):
    ...
    for key in ("revisit", "concession"):
        streams[key] = {"count": sum(1 for r in block_rows(key) if _s(r[2]) != ""),
                        "amount_p": 0}
