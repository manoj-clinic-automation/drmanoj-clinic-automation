# FAULT → ACTION REGISTER — v2.23 (CONSOLIDATED, SELF-CONTAINED)
...
*Next free finding: F-130.* *(This line read "F-115" through v2.20 and v2.21 while the index rows above it were current — the F-45/F-108 family; corrected at v2.22, not silently, and advanced with the index ever since.)*
...
**The through-line of the three.** F-127 is *a boundary mistaken for a different boundary*. F-128 is
*the F-106 family inside the harness built to catch the F-106 family*. F-129 is *a true record
pointed at the wrong subject* (F-118). All three are the same underlying habit — **a check that
looks like it covers something it does not** — and in each case what exposed it was building the
next thing on top and finding the ground would not hold.

---

**END OF FAULT → ACTION REGISTER v2.23 — §7, §7.1 and the CHANGELOG are the last sections. If any of the three is absent, this file is truncated and must not be used as canonical.**
