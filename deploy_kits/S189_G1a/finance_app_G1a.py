...
def api_where_is_the_cash():
    """F-132/F-133: what the maker is entitled to know about cash that is NOT
    in front of him -- and nothing else.

    He is told two things: how much has been handed to Dr Manoj or Dr Bhawna
    and not yet come back, and how long since the last bank trip. He is NOT
    told the unit balance, month-to-date revenue, or any other day's money.

    THE ZERO IS THE POINT. As at S188 this answers Rs 0 for both, because
    `cash_movement` contains nothing but the 15 Yes Bank deposits -- not one
    handover to either doctor has ever been recorded, though the entry has
    existed on this page since S179 (F-133). That is also WHY the drawer reads
    over two lakh: the money left the room and never left the books. So a zero
    here is rendered as an INSTRUCTION, never as a fact -- see the page.

    Parked totals are scoped to the current financial year (1 April). The bank
    clock deliberately is NOT: 'days since the last deposit' must stay true
    across an April boundary."""
    u, err = require("maker", "checker")
    if err:
        return err
    con = db()
    ...
    net = {}
    for r in con.execute(
            "SELECT m.party p, m.direction d, SUM(m.amount_p) s "
            "FROM cash_movement m JOIN day_entry e ON e.id = m.day_entry_id "
            "WHERE e.unit=? AND e.business_date >= ? AND m.party <> 'bank' "
            "GROUP BY m.party, m.direction", (UNIT, since)):
        net[r["p"]] = net.get(r["p"], 0) + (r["s"] if r["d"] == "out" else -r["s"])

    parked = [dict(party=p, name=PARTY_NAMES.get(p, p), amount=rupees(net.get(p, 0)),
                   amount_p=net.get(p, 0))
              for p in ("dr_manoj", "dr_bhawna")]
    for p in sorted(k for k in net if k not in ("dr_manoj", "dr_bhawna")):
...
def selftest():
    ...
    # the financial-year boundary, proven with a real row rather than argued
    _fx = sqlite3.connect(DB_PATH)
    _fy = fy_start().isoformat()
    _before = (parse_iso_date(_fy) - dt.timedelta(days=17)).isoformat()
    _fx.execute("INSERT OR IGNORE INTO day_entry (unit,business_date,status,source,"
                "entered_by,entered_at) VALUES (?,?,'approved','legacy_sheet','smoke',?)",
                (UNIT, _before, _before + "T10:00:00"))
    _eid = _fx.execute("SELECT id FROM day_entry WHERE unit=? AND business_date=?",
                       (UNIT, _before)).fetchone()[0]
    _fx.execute("INSERT INTO cash_movement (day_entry_id,direction,party,amount_p,reference) "
                "VALUES (?,'out','dr_manoj',1234500,'last-FY handover')", (_eid,))
    _fx.commit()
    _wj2 = c.get("/finance/api/where-is-the-cash").get_json() or {}
    _manoj = [p for p in (_wj2.get("parked") or []) if p["party"] == "dr_manoj"]
    check("F-132: a handover dated in the PREVIOUS financial year is excluded",
          bool(_manoj) and _manoj[0]["amount"] in ("0.00", "0"))
    # and an identical movement INSIDE the year is counted -- on its own free
    # date, found rather than assumed (the seed already occupies 1 April).
    _inside = None
    ...
    check("F-132: found a free in-year date to rehearse on", _inside is not None)
    if _inside:
        ...
        _fx.execute("INSERT INTO cash_movement (day_entry_id,direction,party,amount_p,"
                    "reference) VALUES (?,'out','dr_manoj',1234500,'this-FY handover')",
                    (_eid2,))
        _fx.commit()
        _wj3 = c.get("/finance/api/where-is-the-cash").get_json() or {}
        _m3 = [p for p in (_wj3.get("parked") or []) if p["party"] == "dr_manoj"]
        check("F-132: the same handover inside this year IS counted",
              bool(_m3) and _m3[0]["amount"] == "12,345.00")
        check("F-132: and it lifts the parked total off zero",
              (_wj3.get("parked_total") or "") == "12,345.00"
              and _wj3.get("any_recorded") is True)
        _fx.execute("DELETE FROM cash_movement WHERE day_entry_id=?", (_eid2,))
        _fx.execute("DELETE FROM day_entry WHERE id=?", (_eid2,))
    _fx.execute("DELETE FROM cash_movement WHERE day_entry_id=?", (_eid,))
    _fx.execute("DELETE FROM day_entry WHERE id=?", (_eid,))
