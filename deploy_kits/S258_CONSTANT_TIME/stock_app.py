...
PAGE_AMIR = os.path.join(HERE, "stock_amir.html")          # S227 STAFF: Amir's board -- Marg's jobs, uploads, Darpan's lists
...
# S227 STAFF -- the owner, 06-Sep-2026: "blade, gloves, fibre casts are never tracked and
# billed to patients ... we regularly don't make patients buy procedure consumables."
# A consumable's shelf shortfall is CONSUMPTION, never a loss to chase.
...
_BRAND_WORDS = ("TYNOR", "UNISON", "HOSPIK", "FLAMINGO", "REMEDE", "VISSCO", "DYNA", "BAMBOO", "LP", "MEDTRIX",
                "SAMSON", "ORTHOLIFE", "DR", "PLUS", "PREMIUM", "DELUXE", "NEO", "NEOPRENE", "ELAS", "ELASTIC",
                "ADJ", "ADJUSTABLE", "LF", "LIGHT", "SOFT", "HARD", "REGULAR", "SPL", "SPECIAL", "NEW", "OLD")


def _is_consumable(item, st=None):
    u = " " + (item or "").upper() + " "
    extra = tuple(w.strip().upper() for w in str((st or {}).get("consumable_words") or "").split(",") if w.strip())
    return any(w in u for w in _CONSUMABLE_WORDS + extra)


def _is_orthotic(item):
    u = " " + (item or "").upper() + " "
    return (not _is_consumable(item)) and any(w in u for w in _ORTHOTIC_WORDS)


def _ortho_family(item):
    """THE ORTHOTICS FAMILY -- the owner, 06-Sep-2026: 'same items from different
    companies; bought one, sold another'. The item without its brand and filler
    words: KNEE SUPPORT HINGED M (TYNOR) and KNEE SUPPORT HINGED M UNISON are one
    family; sizes stay (an M is not an L)."""
    toks = [t for t in _re221.sub(r"[^A-Z0-9]+", " ", (item or "").upper()).split() if t and t not in _BRAND_WORDS]
    return " ".join(toks)
...
def _stock_settings_words(con):
    try:
        r = con.execute("SELECT value FROM setting WHERE key='stock.consumable_words'").fetchone()
...
def _stock_settings(con):
    return dict(margin_med=_stock_setting(con, "stock.margin_med", 0.20),
                ...
                consumable_words=_stock_settings_words(con))
...
def page_count():
    ...
    _g = stock_gate(con, as_on)                              # S240 STOCK GATE
    if _g["verdict"] == "red" and not (_may_decide(u) and request.args.get("override") == "1"):
        return (_gate_page(_g, _may_decide(u), request.path + "?override=1"), 409,
...
def api_snapshot():
    ...
    con.commit()
    # S243: a difference closes when MARG's export agrees with the count -- never
...
def api_count():
    ...
    _g = stock_gate(_db(), b["marg_as_on"].strip())          # S240 STOCK GATE
    if _g["verdict"] == "red" and not _may_decide(u):
        return jsonify(ok=False, error="stock_export_stale", reasons=_g["reasons"],
                       message="Marg ka stock export purana hai -- naya STOCK CLOSING export karwaiye. "
                               "The stock export is older than entries keyed after it; nothing was recorded."), 409
...
# ---- S240 STOCK GATE: "a count refuses when its Marg export is older than the day's last entry" ----
# S228 Phase 1, item 2. The owner's rule: no count is measured against a closing-stock export that
# was taken BEFORE entries it should contain were keyed in Marg -- a residue across an un-keyed day is
# not stock movement (Kedar's bill 160, dated 01-09, keyed after the 05-09 closing: S228).
#
# The server cannot see Marg's keying times. It CAN see the order of Marg's own exports, by their
# capture stamps, and that is enough for the two rules below. Nothing here writes anything.
#
#   R1  BILLS KEYED AFTER THE STOCK -- LISTED, NOT A REFUSAL (rev 2, the owner's correction of
#       11-Sep). An item-wise purchase export taken AFTER the stock export carries a bill, dated inside
#       what the earlier item-wise exports already covered, that none of them carried. The invoice date
#       is not the day the goods arrived: "Ravi bill date is 6 but it arrived late, don't confuse, the
#       physical stock was the actual inventory at that time." So such a bill is either goods that were
#       on the shelf and keyed late (the stock export is short) OR goods that came after (the export is
#       right). The server cannot tell which -- AMBER, with the bills named for a person to look at.
#   R2  PURCHASES EXPORTED TO THE STOCK DAY. The item-wise purchase exports reach at least the day
#       before the stock's as-on date (D463: 1st -> date, every punch day). Otherwise RED.
#   Where R1 has nothing earlier to compare with, it says so (amber) -- never a silent pass.
#
# A red gate stops a STAFF count (the count page refuses, in Hinglish; the submit answers 409).
# The checker sees the same reasons and may open the count anyway (?override=1) -- his decision,
# recorded in the readiness frozen beside the finding. Every figure here is guarded: a gate that can
# take the count page down is worse than no gate (S209).

_GATE_LINE_TYPES = ("ITEMWISE", "BILLITEMWISE")
# The gate can be switched off without a code change: STOCK_GATE=off in the service environment.
# Only for an emergency the owner calls -- it is on by default, and the walks run with it on.
GATE_ON = os.environ.get("STOCK_GATE", "on").strip().lower() != "off"


def _gate_stamp(ts):
    """'2026-09-06T09:01:01' -> '20260906-090101' (the export-stamp shape). None if unreadable."""
    s = str(ts or "")
...
def stock_gate(con, as_on=None):
    """{verdict: green|amber|red, reasons:[...], notes:[...], as_on, stock_at}. Reads only."""
    out = dict(verdict="green", reasons=[], notes=[], late_bills=[], as_on=None, stock_at=None)
    if not GATE_ON:
        out["notes"].append("The count gate is switched off (STOCK_GATE=off).")
        out["verdict"] = "amber"
        return out
    try:
        if not as_on:
            as_on, _rows = _newest_snapshot(con)
        out["as_on"] = as_on
        day = _iso(as_on)
        if not day:
            out["notes"].append("No stock export to judge.")
            out["verdict"] = "amber"
            return out
        dmy = "%s-%s-%s" % (day[8:10], day[5:7], day[0:4])
        r = con.execute("SELECT MIN(received_at) FROM stock_feed WHERE as_on IN (?,?) AND source LIKE "
                        "'push_snapshot%'", (dmy, day)).fetchone()
        t = (r[0] if r else None)
        out["stock_at"] = t
        if not t:
            # R3: the figures for this day came from OUR computation (push_expected), not from a Marg
            # closing-stock export -- a count against them would measure us against ourselves (F-350).
            out["reasons"].append(
                "There is no Marg STOCK CLOSING export as on %s -- the figures for that day are our own "
                "computed ones. Export STOCK CLOSING from Marg before counting." % _r_dmy(day))
        ts = _gate_stamp(t)
        exps = con.execute("SELECT md5, period_from, period_to, export_stamp FROM purchase_export "
                           "WHERE type IN (%s)" % ",".join("?" * len(_GATE_LINE_TYPES)),
                           _GATE_LINE_TYPES).fetchall()
        prev_day = (dt.date.fromisoformat(day) - dt.timedelta(days=1)).isoformat()
        known_to = max([str(e[2] or "")[:10] for e in exps] or [""])
        if not known_to or known_to < prev_day:
            out["reasons"].append(
                "Purchases (item-wise) are exported only up to %s, but this stock is as on %s. "
                "Export PURCHASE BILL ITEM WISE (1st to today) before counting."
                % (_r_dmy(known_to) if known_to else "nothing", _r_dmy(day)))
        if ts:
            before = [e for e in exps if (e[3] or "") < ts]
            after = [e for e in exps if (e[3] or "") >= ts]
            month0 = day[:8] + "01"
            covered = max([str(e[2] or "")[:10] for e in before
                           if str(e[1] or "")[:10] <= month0] or [""])
            if covered and covered < month0:
                covered = ""
            if not covered:
                out["notes"].append("No item-wise purchase export was taken before this stock export "
                                    "this month, so late-keyed bills cannot be checked.")
            elif after:
                def bills(ms):
                    if not ms:
                        return {}
                    # keyed on (bill number, bill date): the two item-wise layouts do not both carry the
                    # supplier, so a supplier in the key would call the same bill "new" (a false red)
                    q = ("SELECT bill_no, bill_date, MAX(supplier_norm) FROM purchase_line WHERE source_md5 IN (%s) "
                         "GROUP BY bill_no, bill_date" % ",".join("?" * len(ms)))
                    return {(str(x[0] or ""), str(x[1] or "")[:10]): (x[2] or "") for x in con.execute(q, ms)}
                b0 = bills([e[0] for e in before])
                b1 = bills([e[0] for e in after])
                late = sorted((k[1], k[0], sup) for k, sup in b1.items()
                              if k not in b0 and k[1] and month0 <= k[1] <= covered and k[1] <= day)
                if late:
                    out["late_bills"] = [dict(supplier=sup, bill_no=bno, bill_date=d) for d, bno, sup in late]
                    out["notes"].append(
                        "%d purchase bill%s dated on or before %s %s keyed into Marg after this stock export was "
                        "taken (e.g. bill %s%s of %s). If those goods were on the shelf before the export, take a "
                        "fresh STOCK CLOSING; if they arrived later, the export is right."
                        % (len(late), "" if len(late) == 1 else "s", _r_dmy(covered),
                           "was" if len(late) == 1 else "were", late[0][1],
                           (" of " + late[0][2]) if late[0][2] else "", _r_dmy(late[0][0])))
        else:
            out["notes"].append("The stock export's arrival time is unknown, so late-keyed bills cannot be checked.")
    except Exception as e:                                          # noqa: BLE001
        out["notes"].append("The gate could not be read (%s) -- judge the data by eye." % e.__class__.__name__)
        out["verdict"] = "amber"
        return out
    out["verdict"] = "red" if out["reasons"] else ("amber" if out["notes"] else "green")
    return out


def _gate_page(g, checker, override_url):
    """The page a red gate serves instead of the count. Hinglish for the counter; English for the owner."""
    rs = "".join("<li>%s</li>" % _html_esc(x) for x in g["reasons"])
    if checker:
        body = ("<h2 style='color:#b00'>This count should not start yet</h2><p>Stock export as on <b>%s</b>, "
                "reached the server %s.</p><ul>%s</ul><p><a href='%s'>Open the count anyway</a> "
                "(your decision; it is recorded with the count).</p>"
                % (_html_esc(g.get("as_on") or "?"), _html_esc(_r_stamp(g.get("stock_at"))), rs, override_url))
    else:
        body = ("<h2 style='color:#b00'>Abhi stock count shuru mat kijiye</h2><p>Marg ka stock export (as on "
                "<b>%s</b>) purana hai. Pehle Amir se naya STOCK CLOSING export karwaiye, phir yeh page dobara "
                "kholiye.</p><details><summary>Kyon?</summary><ul>%s</ul></details>"
                % (_html_esc(g.get("as_on") or "?"), rs))
    return ("<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' "
            "content='width=device-width,initial-scale=1'><title>Stock count</title></head>"
            "<body style='font-family:system-ui,sans-serif;margin:16px;max-width:640px'>%s</body></html>" % body)
...
def readiness(con):
    ...
    gate = stock_gate(con)                                   # S240 STOCK GATE
    for x in gate.get("reasons") or []:
        warn.append("COUNT GATE: " + x)
    if gate.get("late_bills"):
        for x in gate.get("notes") or []:
            if "keyed into Marg after" in x:
                warn.append("COUNT CHECK: " + x)
    return dict(ok=True, headline="Read first -- know the data.",
                ...
                lines=lines, warnings=warn, gate=gate)
...
CREATE TABLE IF NOT EXISTS stock_marg_answer (
  id        INTEGER PRIMARY KEY,
  count_id  INTEGER NOT NULL,
  item      TEXT,
  note      TEXT,
  file      TEXT NOT NULL,           -- name inside pad_uploads/marg_answers/
  orig      TEXT NOT NULL,           -- the file's own name
  md5       TEXT NOT NULL,
  bytes     INTEGER NOT NULL,
  by_user   TEXT,
  at        TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS stock_tranche (
  id        INTEGER PRIMARY KEY,
  count_id  INTEGER NOT NULL,
  no        INTEGER NOT NULL,           -- list 1, 2, 3 ... per count and kind
  kind      TEXT NOT NULL,              -- 'med' | 'ortho'
  items     TEXT NOT NULL,              -- JSON list of item names
  issued_by TEXT,
...
def _pad_brief(con, cid, root_id, fam, R):
    ...
    return dict(count_id=cid, root_id=root_id, parts=len(fam) - 1, closed=R.get("closed"),
                ...
                amir="/finance/stock/page/amir?count=%d" % root_id)
...
def _qwd(units, ps):
    """With the direction in words: '60 strips OUT', '48 strips IN' -- the owner,
    06-Sep-2026: 'moved in, or out? clarify inline'."""
    n = int(round(units or 0))
    return _qw(n, ps) + (" IN" if n > 0 else (" OUT" if n < 0 else ""))
...
def _item_life(con, item, ps, since_iso, upto_iso):
    ...
    if len(marg) >= 2:
        ...
        for a, b in zip(marg, marg[1:]):
            ...
            md = b[1] - a[1]; dd = pu - su + ru
            spans.append(dict(from_date=a[0], from_text=_r_dmy(a[0]), from_qty=a[1], to_date=b[0], to_text=_r_dmy(b[0]), to_qty=b[1],
                              marg_delta=md, purchased=pu, sold=su, returned=ru, docs_delta=dd, residue=md - dd, merged=1,
                              explained=None, lookup=None, noise=None))
        # S227 STAFF -- the owner's first live lookup list: 'TYRO BR 74 strips 3 tabs OUT, then 34 strips 3 tabs IN'.
        # An export is taken in the middle of a day and a bill is keyed after it, so neighbouring spans show the
        # same move twice with opposite signs. MERGE such neighbours until none is left; the merged span is where the
        # real move sits. Then a few tabs at a boundary are noise, and only what remains is worth a question.
        changed = True
        while changed and len(spans) > 1:
            changed = False
            for i in range(len(spans) - 1):
                x, y = spans[i], spans[i + 1]
                if x["residue"] and y["residue"] and (x["residue"] > 0) != (y["residue"] > 0):
                    m = dict(from_date=x["from_date"], from_text=x["from_text"], from_qty=x["from_qty"], to_date=y["to_date"], to_text=y["to_text"], to_qty=y["to_qty"],
                             marg_delta=x["marg_delta"] + y["marg_delta"], purchased=x["purchased"] + y["purchased"], sold=x["sold"] + y["sold"],
                             returned=x["returned"] + y["returned"], docs_delta=x["docs_delta"] + y["docs_delta"], residue=x["residue"] + y["residue"],
                             merged=x["merged"] + y["merged"], explained=None, lookup=None, noise=None)
                    spans[i:i + 2] = [m]; changed = True
                    break
        a, b = marg[0], marg[-1]
        ...
        for span in spans:
            res = span["residue"]
            span["moved_text"] = _qws(span["marg_delta"], ps); span["residue_text"] = _qws(res, ps)
            if not res:
                continue
            if raw == 0 or (res > 0) != (raw > 0):
                span["noise"] = "cancels out across the window -- an export taken mid-day; nothing moved"
                continue
            if abs(res) < (ps if ps > 1 else 1):
                span["noise"] = "%s at an export boundary -- the export is taken mid-day; not worth a question" % _qw(res, ps)
                continue
            if res > 0:
                # a purchase entry keyed in Marg AFTER its bill date -- the bill is dated on or before the span's first
                # day (its units were never in this span's documents, or sat in an earlier span that showed them missing)
                lo = (dt.date.fromisoformat(span["from_date"]) - dt.timedelta(days=45)).isoformat()
                cands = sorted([x for x in purchases if lo <= (x["bill_date"] or "") <= span["from_date"] and x["bill_no"] not in used and x["units"] > 0],
                               key=lambda x: x["bill_date"], reverse=True)
            else:
                cands = sorted([x for x in purchases if (x["bill_date"] or "") <= span["to_date"] and x["bill_no"] not in used and x["units"] < 0],
                               key=lambda x: x["bill_date"], reverse=True)
            ...
            if not hit:
                for m_ in range(len(cands)):
                    ...
                    if pair: break
            found = ([hit[0]] if hit else (list(pair) if pair else []))
            if found:
                for x in found: used.add(x["bill_no"])
                span["explained"] = dict(
                    bills=[dict(bill_no=x["bill_no"], bill_date_text=x["bill_date_text"], vendor=x["vendor"], qty_text=x["qty_text"], direction=x["direction"]) for x in found],
                    sentence="%s is %s -- a purchase entry keyed in Marg after the %s export; the documents agree, nothing to chase."
                             % (span["residue_text"], " and ".join("%s %s of %s (%s%s)" % ("return" if x["direction"] == "RETURN" else "bill", x["bill_no"], x["bill_date_text"], x["qty_text"],
                                                                                          (", %g free on the bill" % x["free"]) if x["free"] else "") for x in found), span["from_text"]))
                explained_total += res
            else:
                span["lookup"] = ("Marg > item ledger > %s > %s to %s: which voucher took %s? No bill, return or sale on the server for it."
                                  % (item, span["from_text"], span["to_text"], _qwd(res, ps)))
        residue = raw - explained_total
        open_spans = [x for x in spans if x["lookup"]]
        expl = [x for x in spans if x["explained"]]
        noise_only = raw != 0 and not open_spans and not expl
        if noise_only:
            residue = 0
        if raw == 0 and not expl:
            kind_, conclusion = "clean", "Marg and the documents agree from %s to %s. Nothing to chase in Marg." % (_r_dmy(a[0]), _r_dmy(b[0]))
        elif noise_only:
            kind_, conclusion = "clean", "Only %s across the whole window -- boundary noise of exports taken mid-day. Nothing to chase in Marg." % _qw(raw, ps)
        elif residue == 0:
            kind_, conclusion = "explained", "Nothing to chase in Marg: %s" % " ".join(x["explained"]["sentence"] for x in expl)
        else:
            kind_ = "lookup"
            conclusion = "%s of Marg with no document on the server -- %s. Amir looks it up in Marg's item ledger." % (
                " and ".join("%s between %s and %s" % (_qwd(x["residue"], ps), x["from_text"], x["to_text"]) for x in open_spans) or _qwd(residue, ps),
                "the one thing to chase" if len(open_spans) == 1 else "the things to chase")
            if expl:
                conclusion += " (Also settled: %s)" % " ".join(x["explained"]["sentence"] for x in expl)
        details = ["Window %s to %s: Marg's figure %s -> %s (%s). Documents on the server: bought %s, sold %s, returned %s (%s). Difference %s%s."
                   % (_r_dmy(a[0]), _r_dmy(b[0]), a[1], b[1], _qws(marg_delta, ps), _qw(p_units, ps), _qw(s_units, ps), _qw(r_units, ps), _qws(docs_delta, ps), _qws(raw, ps),
                      (", of which %s explained" % _qws(explained_total, ps)) if explained_total and raw != explained_total else (" -- explained" if explained_total else ""))]
        for x in spans:
            details.append("%s -> %s%s: Marg %d -> %d (%s); documents %s; residue %s%s" % (
                x["from_text"], x["to_text"], (" (%d export days merged)" % x["merged"]) if x["merged"] > 1 else "", x["from_qty"], x["to_qty"], x["moved_text"], _qws(x["docs_delta"], ps),
                _qwd(x["residue"], ps) if x["residue"] else "none",
                (" -- " + x["explained"]["sentence"]) if x["explained"] else ((" -- " + x["lookup"]) if x["lookup"] else ((" -- " + x["noise"]) if x["noise"] else ""))))
        det = dict(from_date=a[0], from_text=_r_dmy(a[0]), from_qty=a[1], to_date=b[0], to_text=_r_dmy(b[0]),
                   ...
                   spans=spans, lookups=[x["lookup"] for x in open_spans], kind=kind_, conclusion=conclusion, details=details,
                   sentence=conclusion)
...
LANES = (
    ("consume", "Procedure consumables", "used in the clinic, never billed to patients -- the shortfall is consumption; write off as used"),
    ("ortho", "Orthotics", "same article, different brands -- bought one, sold another; the family, not the line, tells the story"),
...
def _lane_of(d, ctx):
    ...
    # 0 -- a procedure consumable: the shelf shortfall is what the clinic used
    if ctx.get("consumable"):
        if diff < 0:
            return "consume", "", "a procedure consumable -- used in the clinic, never billed to patients; %s used since Marg last knew. Write off as consumption; Amir posts one stock-issue voucher." % _qw(-diff, pack)
        if diff > 0:
            return "consume", "", "a procedure consumable with %s MORE on the shelf than Marg -- a purchase not entered or a consumption voucher too large; Amir corrects Marg to the shelf." % _qw(diff, pack)
    # 0b -- an orthotic: read the FAMILY (same article, other brands) before the line
    fam = ctx.get("family")
    if fam and diff != 0:
        net, members = fam["net"], fam["members"]
        if len(members) > 1 and net == 0:
            return "ortho", "", "the family '%s' nets to zero across %d brands (%s) -- one brand was sold and another billed; Amir swaps the figures in Marg, nothing is lost." % (fam["key"], len(members), fam["text"])
        if len(members) > 1 and abs(net) < abs(diff):
            return "ortho", "", "the family '%s' is %s net across %d brands (%s) -- part is a brand swap, the rest %s. Amir swaps the brands in Marg; the remainder is decided on the family." % (
                fam["key"], _qwd(net, 1).replace(" IN", " over").replace(" OUT", " short"), len(members), fam["text"], "is missing" if net < 0 else "is a purchase not entered")
        if diff > 0:
            return "ortho", "", "an orthotic with %s more on the shelf than Marg -- an unentered purchase or a clinic-billed sale not keyed; Amir corrects Marg to the shelf." % _qw(diff, pack)
        return "ortho", "", "an orthotic short %s; no other brand of the same article is over -- fitted in the clinic and not keyed, or missing. Darpan rechecks; then write off or pursue." % _qw(-diff, pack)
    ...
    if res not in (None, 0) and abs(res) >= max(2, pack):
        return "marg", "", "%s of Marg %s with no document on the server -- an adjustment, a correction or a late entry. %s" % (
            _qwd(res, pack), ctx.get("res_where") or ("since " + (ctx.get("res_from") or "the first export day")),
            "Amir looks up that span in Marg's item ledger before the shelf is blamed." if ctx.get("res_where") else "Look in Marg first.")
    # 6 -- over
    if diff > 0:
...
def _pad_report_data(con, root_id):
    ...
    # S228 THREE-WAY: the shelf, Marg and our own system, and WHICH of the two
    # pushes the count's own figures actually came from.
    _tw = _three_way(con, R["as_on"], {k: v[0] for k, v in packs.items()})
    _ours = _tw["ours"]
    # THE ORTHOTICS FAMILIES -- every orthotic in the shop (differing, matched or uncounted) by family
    families = {}
    _dm = {d_[0]: dict(item=d_[0], marg=d_[3], counted=d_[4], diff=(d_[4] or 0) - (d_[3] or 0)) for d_ in R["differences"]}
    _mt = {m[0]: m for m in R["matched"]}
    for r in con.execute("SELECT item, pack_size, qty FROM stock_snapshot WHERE as_on=?", (R["as_on"],)):
        it = r[0]
        if not _is_orthotic(it):
            continue
        k = _ortho_family(it)
        f = families.setdefault(k, dict(key=k, members=[], net=0, short=0, over=0, counted_n=0))
        if it in _dm:
            dd = _dm[it]; diff = int(dd["diff"]); st_ = "diff"
            marg_, cnt_ = int(dd["marg"]), int(dd["counted"])
        elif it in _mt:
            diff = 0; st_ = "match"; marg_ = cnt_ = int(_mt[it][3])
        else:
            diff = 0; st_ = "uncounted"; marg_ = int(r[2] or 0); cnt_ = None
        f["members"].append(dict(item=it, marg=marg_, counted=cnt_, diff=diff, state=st_))
        f["net"] += diff
        if diff < 0: f["short"] += -diff
        if diff > 0: f["over"] += diff
        if st_ != "uncounted": f["counted_n"] += 1
    for f in families.values():
        f["members"].sort(key=lambda m: (m["diff"], m["item"]))
        f["text"] = ", ".join("%s %s" % (m["item"], ("short %d" % -m["diff"]) if m["diff"] < 0 else (("over %d" % m["diff"]) if m["diff"] > 0 else ("= Marg" if m["state"] == "match" else "not counted"))) for m in f["members"])
        f["verdict"] = ("brand swap -- Amir swaps the figures in Marg, nothing lost" if (len(f["members"]) > 1 and f["net"] == 0 and (f["short"] or f["over"]))
                        else ("part swap, %s short net" % f["net"] if (len(f["members"]) > 1 and f["net"] < 0 and f["over"])
                        else ("part swap, %s over net" % f["net"] if (len(f["members"]) > 1 and f["net"] > 0 and f["short"])
                        else ("short %d -- fitted and not keyed, or missing" % -f["net"] if f["net"] < 0
                        else ("over %d -- unentered purchase or a clinic-billed sale not keyed" % f["net"] if f["net"] > 0
                        else "all agree")))))
    ...
    def ctx_for(item, pack, need_residue):
        ...
        if need_residue:
            try:
                ...
                if dd:
                    ...
                    ex = [x for x in dd.get("spans") or [] if x.get("explained")]
                    if ex and not res:
                        bills_ = sorted({b_["bill_no"] for x in ex for b_ in x["explained"]["bills"]})
                        if bills_ and dd.get("explained_units"):
                            res_note = "its %s move is %s keyed after an export" % (_qwd(dd["explained_units"], pack), ", ".join("bill %s" % b_ for b_ in bills_))
                        elif bills_:
                            res_note = "%s was keyed after an export day and shows in the next -- same entry, net zero" % ", ".join("bill %s" % b_ for b_ in bills_)
                        else:
                            res_note = "an entry made and reversed in Marg, net zero"
            except Exception:                                 # noqa: BLE001
                res = None
        return dict(sold=s[0], loose=s[1], last_sale=s[2], bought_fy=bool(p[0]), last_purchase=p[1],
                    ...
                    consumable=_is_consumable(item, st), family=families.get(_ortho_family(item)) if _is_orthotic(item) else None,
    ...
    for d_ in R["differences"]:
        ...
        d.update(lane=lane, lane_title=LANE_TITLE[lane], kind=kind, why=why, allowance=c["allowance"],
                 ...
                 consumable=bool(c.get("consumable")), family=(c["family"]["key"] if c.get("family") else None),
                 family_verdict=(c["family"]["verdict"] if c.get("family") else None),
    ...
    for m in R["matched"]:
        ...
        if not c["sold"] and not c["last_sale"] and qty != 0:
            ...
            dead_matched.append(dict(item=item, packing=packing, pack=ps, marg=qty, counted=qty, diff=0,
                                     ...
                                     cost_p=(None if cp_ is None else cp_ * qty),
                                     last_purchase=c["last_purchase"], word=words.get(item)))
    # the owner, 06-Sep-2026: "first candidate for write off, for my eyes after Darpan and Amir confirm" --
    # the ones with stock (value) first; Marg 0 lines are not on this list at all
    dead_matched.sort(key=lambda x: (-(x["cost_p"] or x["mrp_p"] or 0), x["item"]))
    # lane totals
    lanes = []
    ...
    return dict(count_id=_root, parts=len(fam) - 1, family=fam, when=R["when"], when_iso=R.get("when_iso"),
                ...
                ortho_families=sorted(families.values(), key=lambda f: (-(f["short"] + f["over"]), -len(f["members"]), f["key"])),
                marg_answers=_marg_answers(con, _root), marg_answer_url="/finance/stock/api/pad/marg_answer/%d" % _root,
                ...
                matched=[dict(item=m[0], packing=m[1] or "", pack=int(m[2] or 1), qty=m[3],
                ...
                links=dict(final_sheet="/finance/stock/api/pad/followup/%d.xlsx" % _root,
                           ...
                           ortho="/finance/stock/api/pad/ortho/%d.xlsx" % _root,
                           amir_page="/finance/stock/page/amir?count=%d" % _root,
...
def _purchase_audit(con, since_iso=None):
    ...
    for r in con.execute("SELECT id, supplier_norm, bill_no, bill_date, item, packing, batch, qty, free, "
        ...
        if row["free"] and row["qty"] and row["free"] > max(2.0, row["qty"] * 0.5):
            odd_free.append(dict(row, why="free %g against %g paid -- more than half; check the bill"
                                 % (row["free"], row["qty"])))
    # a purchase line whose packing disagrees with the stock item's packing
    conflicts = []
    try:
        days = [r[0] for r in con.execute("SELECT DISTINCT as_on FROM stock_snapshot")]
        latest = max(days, key=lambda d_: _dmy_to_iso(d_) or "") if days else None
        stock_pack = {r[0]: (r[1] or "") for r in con.execute("SELECT item, packing FROM stock_snapshot WHERE as_on=?", (latest,))} if latest else {}
    except Exception:                                         # noqa: BLE001
        stock_pack = {}
    if stock_pack:
        norm_pack = {_pad_norm(k): v for k, v in stock_pack.items()}
        seen_c = set()
        for r in con.execute("SELECT id, supplier_norm, bill_no, bill_date, item, packing, batch, qty, free, loose_qty FROM purchase_line "
                             "WHERE bill_date IS NOT NULL AND bill_date>=? ORDER BY bill_date DESC, id DESC", (since_iso,)):
            sp = norm_pack.get(_pad_norm(r[4]))
            if sp is None or not r[5]:
                continue
            a_, b_ = _pack_units(r[5]) or 0, _pack_units(sp) or 0
            if a_ and b_ and a_ != b_ and _pad_norm(r[4]) not in seen_c:
                seen_c.add(_pad_norm(r[4]))
                conflicts.append(dict(id=r[0], vendor=vend.get(r[1]) or (r[1] or "").upper() or "-", bill_no=r[2] or "-", bill_date=r[3], bill_date_text=_r_dmy(r[3]),
                                      item=r[4], packing=r[5], stock_packing=sp, batch=r[6] or "", qty=float(r[7] or 0), free=float(r[8] or 0), loose=float(r[9] or 0)))
    ...
    return dict(since=since_iso, since_text=_r_dmy(since_iso), lines=n,
                ...
                loose_is_total=total_loose, superseded=dead, odd_free=odd_free, packing_conflicts=conflicts,
                needs_amir=len(odd_free) + len(conflicts),
...
def api_pad_audit_xlsx():
    ...
    # the owner, 06-Sep-2026: "show only the ones that need Amir and the exact reason". Double entries, loose-is-
    # total lines and old exports are the SERVER's business and already handled -- they stay in the JSON, not here.
    rows = []
    for x in a["odd_free"]:
        rows.append(["FREE TOO HIGH", x["vendor"], x["bill_no"], x["bill_date_text"], x["item"], x["packing"], x["batch"],
                     x["qty"], x["free"], x["loose"], "open this bill in Marg: is the free quantity really %g on %g paid? Correct the entry if not." % (x["free"], x["qty"]), ""])
    for x in a.get("packing_conflicts") or []:
        rows.append(["PACKING DIFFERS", x["vendor"], x["bill_no"], x["bill_date_text"], x["item"], x["packing"], x["batch"],
                     x["qty"], x["free"], x["loose"], "the purchase says packing %s but the stock item is %s -- one of the two is wrong in Marg; fix the item master or the bill line." % (x["packing"] or "-", x["stock_packing"] or "-"), ""])
    return _xlsx_response(_xlsx_rows(
        "PURCHASE DATA AUDIT -- since %s -- %d purchase lines checked, %d need Amir" % (a["since_text"], a["lines"], len(rows)),
        "Only what needs a hand in Marg is on this sheet; the reason is exact. (Double entries from overlapping exports and Marg's loose-column arithmetic are the server's business and are already handled -- %d double entr%s counted once, %d line%s read as Marg means them.) Tick the last column when done." % (len(a["duplicates"]), "y" if len(a["duplicates"]) == 1 else "ies", len(a["loose_is_total"]), "" if len(a["loose_is_total"]) == 1 else "s"),
...
@bp.route("/api/pad/marg_answer/<int:cid>", methods=["POST"])
def api_pad_marg_answer(cid):
    """MARG'S ANSWER, UPLOADED -- the owner, 06-Sep-2026: 'a section for me and Amir to
    generate these from Marg, for you -- better as upload to the VPS than in chat'.
    Any file (a Marg export, a PDF, a photo of the ledger) is kept byte for byte in
    pad_uploads/marg_answers/ with a row that says which count, which item, who, when.
    The next build reads them; nothing else moves."""
    u, err = _require("checker", "maker", "viewer")
    if err:
        return err
    con = _db()
    ...
    if f is None or not f.filename:
        return jsonify(ok=False, error="no_file", message="Choose the file first."), 400
    raw = f.read()
    if not raw:
        return jsonify(ok=False, error="empty", message="That file is empty."), 400
    if len(raw) > 25 * 1024 * 1024:
        return jsonify(ok=False, error="too_big", message="Over 25 MB -- send a smaller export or a photo of the page."), 400
    import hashlib                                            # noqa: PLC0415
    md5 = hashlib.md5(raw).hexdigest()
    orig = os.path.basename(f.filename)[:120]
    safe = _re221.sub(r"[^A-Za-z0-9._-]+", "_", orig).strip("_") or "file"
    d = os.path.join(_pad_archive_dir(con), "marg_answers")
    os.makedirs(d, exist_ok=True)
    now = dt.datetime.now()
    name = "%d_%s_%s_%s" % (cid, now.strftime("%Y%m%d-%H%M%S"), md5[:8], safe)
    with open(os.path.join(d, name), "wb") as fh:
        fh.write(raw)
    item = (request.form.get("item") or "").strip()[:120]
    note = (request.form.get("note") or "").strip()[:400]
    con.execute("INSERT INTO stock_marg_answer (count_id, item, note, file, orig, md5, bytes, by_user, at) VALUES (?,?,?,?,?,?,?,?,?)",
                (cid, item, note, name, orig, md5, len(raw), (u or {}).get("user") or "", now.isoformat(timespec="seconds")))
    con.commit()
    return jsonify(ok=True, file=name, orig=orig, bytes=len(raw), md5=md5,
                   message="Kept on the server: %s (%d KB). It is on the list below." % (orig, len(raw) // 1024 or 1))


def _marg_answers(con, cid):
    _pad_ensure(con)
    return [dict(id=r[0], item=r[1] or "", note=r[2] or "", orig=r[3], bytes=r[4], by=r[5] or "", at=r[6], at_text=_r_stamp(r[6]) + " IST")
            for r in con.execute("SELECT id, item, note, orig, bytes, by_user, at FROM stock_marg_answer WHERE count_id=? ORDER BY id DESC", (cid,))]


# ---------------------------------------------------------------------------
# S227 STAFF -- DARPAN'S LISTS IN TURNS. The owner, 06-Sep-2026: "he gets the highest
# value ones, 5, with 5 routine ones at a time; he submits, he gets another; he gets
# only quantities, no rates, to spare him the fear of the loss being pinned on him.
# Keep orthotics separate."
TRANCHE_TOP, TRANCHE_ROUTINE = 5, 5
...
def _tranches(con, cid):
    _pad_ensure(con)
    out = []
    for r in con.execute("SELECT id, no, kind, items, issued_by, issued_at, returned_at, returned_by FROM stock_tranche WHERE count_id=? ORDER BY kind, no", (cid,)):
        out.append(dict(id=r[0], no=r[1], kind=r[2], items=json.loads(r[3] or "[]"), issued_by=r[4] or "", issued_at=r[5],
                        issued_text=_r_stamp(r[5]) + " IST", returned_at=r[6], returned_text=(_r_stamp(r[6]) + " IST") if r[6] else "",
                        returned_by=r[7] or "", pdf="/finance/stock/api/pad/tranche/%d/%d.pdf" % (cid, r[0])))
    return out


def _tranche_next(con, cid, kind, u):
    """Cut the next list: 5 highest-value open lines + 5 routine ones (smallest
    differences, so the list is not all bad news), none already on a list."""
    d = _pad_report_data(con, cid)
    if d is None:
        return None, "No count #%d." % cid
    had = _tranches(con, cid)
    if any(t["kind"] == kind and not t["returned_at"] for t in had):
        return None, "Darpan still has list %d -- mark it returned first." % [t["no"] for t in had if t["kind"] == kind and not t["returned_at"]][0]
    given = {it for t in had for it in t["items"]}
    pool = [x for x in _tranche_pool(d, kind) if x["item"] not in given]
    if not pool:
        return None, "Nothing left for Darpan on the %s side." % ("orthotics" if kind == "ortho" else "medicines")
    by_value = sorted(pool, key=lambda x: (x["mrp_p"] is None, -abs(x["mrp_p"] or 0), -abs(x["diff"])))
    top = by_value[:TRANCHE_TOP]
    rest = [x for x in by_value if x not in top]
    routine = sorted(rest, key=lambda x: (abs(x["diff"]), x["item"]))[:TRANCHE_ROUTINE]
    items = [x["item"] for x in top + routine]
    no = 1 + max([t["no"] for t in had if t["kind"] == kind] or [0])
    con.execute("INSERT INTO stock_tranche (count_id, no, kind, items, issued_by, issued_at) VALUES (?,?,?,?,?,?)",
                (cid, no, kind, json.dumps(items), (u or {}).get("user") or "", dt.datetime.now().isoformat(timespec="seconds")))
    con.commit()
    return [t for t in _tranches(con, cid) if t["kind"] == kind and t["no"] == no][0], ""


@bp.route("/api/pad/tranche/<int:cid>/next", methods=["POST"])
def api_pad_tranche_next(cid):
    u, err = _require("checker", "maker", "viewer")
    if err:
        return err
    con = _db()
    ensure_schema(con)
    kind = (request.get_json(silent=True) or {}).get("kind") or "med"
    if kind not in ("med", "ortho"):
        return jsonify(ok=False, error="bad_kind", message="kind must be med or ortho"), 400
    t, msg = _tranche_next(con, cid, kind, u)
    if t is None:
        return jsonify(ok=False, error="none", message=msg), 400
    return jsonify(ok=True, tranche=t, message="List %d cut: %d items. Print it and hand it to Darpan." % (t["no"], len(t["items"])))


@bp.route("/api/pad/tranche/<int:cid>/<int:tid>/returned", methods=["POST"])
def api_pad_tranche_returned(cid, tid):
    u, err = _require("checker", "maker", "viewer")
    if err:
        return err
    con = _db()
    ensure_schema(con)
    _pad_ensure(con)
    con.execute("UPDATE stock_tranche SET returned_at=?, returned_by=? WHERE id=? AND count_id=? AND returned_at IS NULL",
                (dt.datetime.now().isoformat(timespec="seconds"), (u or {}).get("user") or "", tid, cid))
    con.commit()
    return jsonify(ok=True, tranches=_tranches(con, cid))


@bp.route("/api/pad/tranche/<int:cid>/<int:tid>.pdf")
def api_pad_tranche_pdf(cid, tid):
    """THE PAPER LIST FOR DARPAN: quantities only, no rates, no values."""
    u, err = _require("checker", "maker", "viewer")
    if err:
        return err
    con = _db()
    ensure_schema(con)
    t = [x for x in _tranches(con, cid) if x["id"] == tid]
    if not t:
        return jsonify(ok=False, error="not_found", message="No such list."), 404
    t = t[0]
    d = _pad_diffs_data(con, cid)
    if d is None:
        return jsonify(ok=False, error="not_found", message="No count #%d." % cid), 404
    ...
    import pad_receipt                                        # noqa: PLC0415
    from flask import Response                                # noqa: PLC0415
    pdf = pad_receipt.render_tranche(d)
    return Response(pdf, mimetype="application/pdf",
                    headers={"Content-Disposition": 'inline; filename="DARPAN_LIST_%d_%s_count%d.pdf"' % (t["no"], t["kind"], cid)})


@bp.route("/api/pad/answers/<int:cid>", methods=["POST"])
def api_pad_answers(cid):
    """AMIR'S TYPING BOARD: Darpan's paper answers, one line per item -- reason (a
    number from the legend or a key), an optional recount (strips, loose), a note.
    Recounts join the count as a correction row on the S221 answer layer only (the
    count itself never changes); a real recount goes through a sheet as before."""
    u, err = _require("checker", "maker", "viewer")
    if err:
        return err
    con = _db()
    ...
    for a in b.get("answers") or []:
        ...
        reason = _pad_reason_key(a.get("reason"))
        note = (a.get("note") or "").strip()[:300]
        if not item or (not reason and not note):
            continue
        d = con.execute("SELECT id FROM stock_diff WHERE item=? AND count_id IN (%s) ORDER BY count_id DESC, id DESC LIMIT 1" % q, (item,) + tuple(fam)).fetchone()
        if not d:
            continue
        con.execute("INSERT INTO stock_diff_answer (diff_id, reason, note, answered_by, answered_at) VALUES (?,?,?,?,?)",
                    (int(d[0]), reason or "dont_know", (note or None) if reason else ("(no reason given) " + note), (u or {}).get("user") or "", now_iso()))
        n += 1
    con.commit()
    return jsonify(ok=True, recorded=n, message="%d answer%s recorded." % (n, "" if n == 1 else "s"))


def _amir_board(con, cid):
    """Everything Amir's page shows, in one dict."""
    d = _pad_report_data(con, cid)
    if d is None:
        return None
    # 1 -- what Marg still owes the server: one question per item (spans joined), then the exports
    look = []
    for x in d["differences"]:
        if x.get("lookups"):
            look.append(dict(item=x["item"], packing=x["packing"], marg=x["marg"], counted=x["counted"], diff=x["diff"], pack=x["pack"],
                             questions=x["lookups"], n=len(x["lookups"])))
    look.sort(key=lambda r: (-abs(r["diff"] * 1.0 / max(1, r["pack"])), r["item"]))
    ...
    for x in d["differences"]:
        ...
        if w["action"] == "WRITE_OFF":
            (vouchers["consume"] if x["lane"] == "consume" else vouchers["issue"]).append(x)
        elif w["action"] in ("MARG_FIX", "EXPLAINED"):
            vouchers["fix"].append(x)
    # 3 -- Darpan's lists
    tr = _tranches(con, cid)
    pool_med = len([x for x in _tranche_pool(d, "med") if x["item"] not in {i for t in tr for i in t["items"]}])
    pool_ortho = len([x for x in _tranche_pool(d, "ortho") if x["item"] not in {i for t in tr for i in t["items"]}])
    return dict(count_id=d["count_id"], day=d["day"], lookups=look, vouchers=vouchers, tranches=tr,
                pool=dict(med=pool_med, ortho=pool_ortho), marg_answers=d["marg_answers"], marg_answer_url=d["marg_answer_url"],
                links=d["links"], reasons=[dict(n=i + 1, key=k, en=REASON_ENGLISH[k], hi=STAFF_REASONS.get(k, "")) for i, k in enumerate(REASON_NUMBERS)],
                differences=[dict(item=x["item"], packing=x["packing"], pack=x["pack"], marg=x["marg"], counted=x["counted"], diff=x["diff"], lane=x["lane"],
                                  word=(x["word"]["label"] if x.get("word") else ""), answer=(x["answer"] or {}).get("label") if x.get("answer") else "") for x in d["differences"]],
                ortho_families=d.get("ortho_families") or [])


@bp.route("/api/pad/amir/<int:cid>")
def api_pad_amir_board(cid):
    u, err = _require("checker", "maker", "viewer")
    if err:
        return err
    con = _db()
    ensure_schema(con)
    b = _amir_board(con, cid)
    if b is None:
        return jsonify(ok=False, error="not_found", message="No count #%d." % cid), 404
...
def api_pad_ortho_xlsx(cid):
    """THE ORTHOTICS CLEANUP -- every family, its members, the net, the verdict."""
    u, err = _require("checker", "maker", "viewer")
    if err:
        return err
    con = _db()
    ...
    if d is None:
        return jsonify(ok=False, error="not_found", message="No count #%d." % cid), 404
    rows = []
    for f in d.get("ortho_families") or []:
        for m in f["members"]:
            rows.append([f["key"], m["item"], m["marg"], ("" if m["counted"] is None else m["counted"]), m["diff"] or "", f["net"], f["verdict"], ""])
    return _xlsx_response(_xlsx_rows(
        "ORTHOTICS CLEANUP -- stock count #%d (%s)" % (d["count_id"], d["day"]),
        "One block per FAMILY (the article without its brand). Read the family's NET and VERDICT, not one line: a family that nets to zero was sold as one brand and billed as another -- swap the figures in Marg. Tick the last column when done.",
        ["FAMILY", "ITEM (brand)", "MARG", "SHELF", "DIFF", "FAMILY NET", "VERDICT", "DONE (tick)"],
        rows, {0: 30, 1: 34, 2: 8, 3: 8, 4: 8, 5: 11, 6: 60, 7: 10}),
        "STOCK_COUNT_%s_ORTHOTICS_CLEANUP.xlsx" % (d["day"] or ""))
...
def api_pad_lookups(cid):
    ...
    for x in d["differences"]:
        qs = x.get("lookups") or []
        if qs:
            rows.append([x["item"], x["packing"], x["marg"], x["counted"], x["diff"], " | ".join(qs), "", ""])
...
@bp.route("/page/amir")
def page_amir():
    """AMIR'S BOARD -- the owner, 06-Sep-2026: "all uploads and downloads to be placed
    on the staff page who is meant for it". What Marg still owes the server (one
    question per item, the exports), the upload box, the vouchers to post, and
    Darpan's lists in turns with the typing board for his answers."""
    u, err = _require("checker", "maker", "viewer")
    if err:
        return err
    from flask import Response                                # noqa: PLC0415
    ...
    if not cid.isdigit():
        ...
        cid = str(r[0]) if r else "0"
    try:
        with io.open(PAGE_AMIR, "r", encoding="utf-8") as fh:
            html = fh.read()
    except IOError:
        return jsonify(ok=False, error="missing", message="stock_amir.html is not beside stock_app.py"), 503
    boot = json.dumps(dict(count_id=int(cid), user=(u or {}).get("user") or "", checker=_may_decide(u)))
    return Response(html.replace("/*__BOOT__*/", "window.BOOT=" + boot + ";"), mimetype="text/html")
