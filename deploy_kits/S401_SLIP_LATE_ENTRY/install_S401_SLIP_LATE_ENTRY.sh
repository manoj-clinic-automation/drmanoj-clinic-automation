#!/bin/bash
# =============================================================================
#  install_S401_SLIP_LATE_ENTRY.sh · kit S401_SLIP_LATE_ENTRY (session 282, 25-Sep-2026)
#
#  The owner, 25-Sep-2026: a parchi missed on its day is written afterwards (up to 7 days) ON ITS OWN DAY, from a
#  new 'Chhooti parchi' screen or the 'Din' choice on every form; Bhati and reception do it as Shavez does; the
#  Report link shows for Bhati. Proved on two scratch copies of the live database before anything is placed.
#  FILE:  /root/finance/slip_log.py  S398 d208f57a -> S401 36405348.  Restarts clinic-finance only.
#
#  Run by (one line on the VPS):
#    cd /root/deploy/repo && git pull --ff-only && bash /root/deploy/repo/deploy_kits/S401_SLIP_LATE_ENTRY/install_S401_SLIP_LATE_ENTRY.sh
# =============================================================================
set -u
KIT="S401_SLIP_LATE_ENTRY"; KDIR="$(cd "$(dirname "$0")" && pwd)"
...
STAMP="$(date +%Y%m%d_%H%M%S)"; WALK="/tmp/s401_walk_$STAMP"
S_FROM=d208f57a4070b5acc02e3e91c7cb86bd; S_TO=364053483a5a4c4161663e77775f1a01
...
[ "$(m5 "$FIN/slip_log.py")" = "$S_FROM" ] || { say "!! [2/6] slip_log.py is $(m5 "$FIN/slip_log.py"), not S398 - nothing installed"; exit 1; }
say "[2/6] live pin exact (slip_log S398)"
mkdir -p "$WALK" || exit 1
"$VPY" -B -c "import sys, hashlib; sys.argv = sys.argv[1:]; exec(open(sys.argv[0]).read().split('open(sys.argv[2]')[0]); assert hashlib.md5(s.encode()).hexdigest() == sys.argv[2]" \
  apply_s401.py "$FIN/slip_log.py" "$S_TO" 2>/dev/null || { say "!! [3/6] the kit file is not the live file plus S401 - nothing installed"; rm -rf "$WALK"; exit 1; }
"$VPY" -B -m py_compile slip_log.py walk_s401.py || { say "!! [3/6] compile failed - nothing installed"; rm -rf "$WALK"; exit 1; }
...
WOUT="$( cd /tmp && timeout 240 "$VPY" -B "$KDIR/walk_s401.py" "$KDIR/slip_log.py" "$FIN/slip_log.py" "$WALK/scratch_A.db" "$WALK/scratch_B.db" 2>&1 | tail -1 )"
rm -rf "$WALK"
echo "$WOUT" | grep -q "^WALK OK" || { say "!! [4/6] walk red: $WOUT - nothing installed"; exit 1; }
say "[4/6] $WOUT"
BS="$FIN/slip_log.py.bak_S401_d208f57a"
...
say "[6/6] $KIT: DONE -- menu shows Chhooti parchi; read next: https://followup.dr-manoj.in/finance/slips?s=late"
