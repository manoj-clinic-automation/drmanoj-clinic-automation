#!/bin/bash
# =============================================================================
#  install_S405_BANK_SMS_DOOR.sh · kit S405_BANK_SMS_DOOR (session 283, Sanjeevni, 26-Sep-2026, D621, F-634)
#
...
#
#  THE OWNER (25/26-Sep-2026): the phone posts the ICICIPOS SMS every morning (nine times, HTTP 200) yet the Bank SMS
#  page reads "none yet" -- the strict regex missed and the door kept nothing of a miss. He has cloned the macro for
#  Yes Bank. The bulk supplier NEFT is one Yes Bank debit whose SMS amount equals the pay month's NEFT portion.
#
#  PATCHED ON THE BOX from the live bytes (make_s405.py; every anchor exactly once; FROM -> TO pins checked):
#          /root/finance/bank_sms.py                        3a8f0a88 -> a70d6d96   never silent · tolerant ladder · Yes Bank · the provisional · the page
#          /root/finance/sanjeevni_approvals.py             675aab4a -> 126f90fc   the Needs-you line + the two POST routes
#          /root/finance/finance_ui/finance_approvals.html  c319bb56 -> 928a25ef   OK / Not this on the line (clinic -- declared)
#  Restarts clinic-finance only. finance.db backed up first; the seed adds ONE setting (neft.sms_tolerance_p = 0).
#  The walk (this kit's), then S404's, S403's, S400's and S402's own walks re-run on the patched files -- all on scratch copies.
# =============================================================================
set -u
KIT="S405_BANK_SMS_DOOR"
...
WALK="/tmp/s405_walk_$STAMP"
declare -A FROM=( [bank_sms.py]=3a8f0a8863942cde3fce2d0294a86769 [sanjeevni_approvals.py]=675aab4a46ab8792f05b17cab15d17ee
                  [finance_approvals.html]=c319bb56d30ac960d49d4a67083a55b2 )
declare -A TO=( [bank_sms.py]=a70d6d96e700380d08b8e379ea0720e5 [sanjeevni_approvals.py]=126f90fc9912092378e817f101a8f74e
                [finance_approvals.html]=928a25ef503267ce8e518f126306c236 )
declare -A DEST=( [bank_sms.py]="$FIN/bank_sms.py" [sanjeevni_approvals.py]="$FIN/sanjeevni_approvals.py" [finance_approvals.html]="$FIN/finance_ui/finance_approvals.html" )
ORDER=(bank_sms.py sanjeevni_approvals.py finance_approvals.html)
...
clean() { find "$KDIR" "$K400" "$K402" "$K403" "$K404" "$WALK" -name __pycache__ -prune -exec rm -rf {} + 2>/dev/null; }
...
for k in "$K400/walk_s400.py" "$K400/seed_s400.py" "$K402/walk_s402.py" "$K403/walk_s403.py" "$K403/seed_s403.py" "$K404/walk_s404.py" "$K404/seed_s404.py"; do
  [ -f "$k" ] || { say "!! [1/10] $k must sit beside this kit (the S400 / S402 / S403 / S404 walks re-run) - nothing installed"; exit 1; }
done
say "[1/10] kit gates green (S400, S402, S403 and S404 walks found)"
ALL=1; for f in "${ORDER[@]}"; do [ "$(m5 "${DEST[$f]}")" = "${TO[$f]}" ] || ALL=0; done
if [ "$ALL" = 1 ]; then say "-- ALREADY INSTALLED. Seed re-checked:"; "$SPY" -B seed_s405.py "$DBF"; exit 0; fi
...
mkdir -p "$WALK/built" "$WALK/finance/finance_ui" "$WALK/finance/spine" "$WALK/old/finance_ui" "$WALK/old/spine" "$WALK/old404/finance_ui" "$WALK/old404/spine" \
         "$WALK/old403/finance_ui" "$WALK/old403/spine" "$WALK/old0/finance_ui" "$WALK/old2/finance_ui" "$WALK/marg_ingest" "$WALK/marg_old" \
         "$WALK/assetapp_live" "$WALK/assetapp_old3" "$WALK/plive" "$WALK/p403" "$WALK/p404" "$WALK/pold" "$WALK/uploads" "$WALK/stub" || exit 1
"$SPY" -B make_s405.py --finance "$FIN" --out "$WALK/built" | sed 's/^/   /' || { say "!! [3/10] the build from the live bytes stopped - nothing installed"; rm -rf "$WALK"; exit 1; }
...
PYS="$WALK/built/bank_sms.py $WALK/built/sanjeevni_approvals.py"
( "$SPY" -m py_compile seed_s405.py make_s405.py walk_s405.py $PYS && "$VPY" -m py_compile seed_s405.py $PYS 2>/dev/null ) \
...
# the walk's copies of the box: finance (new) · old (as it is) · old404 (pre-S404) · old403 (pre-S403) · old0 (pre-S400) · old2 (pre-S402)
for side in finance old old404 old403 old0 old2; do
...
for side in finance old old404 old403; do cp -p "$FIN"/spine/*.py "$FIN"/spine/*.json "$WALK/$side/spine/" 2>/dev/null; done
[ -f "$FIN/spine/spine.db" ] && for side in finance old old403; do cp -p "$FIN/spine/spine.db" "$WALK/$side/spine/"; done
cp -p "$WALK/built/bank_sms.py" "$WALK/built/sanjeevni_approvals.py" "$WALK/finance/"
...
# old403 = the box BEFORE S403 (its .bak_S403 files), the negative control S403's own walk expects
...
# the portals: live (S403, v28) · before S403 = as S404 left it (v27; S404's walk reads it as its 'new') · before S404 · before S400
...
for i in 1 2 3 4 5; do copydb "$DBF" "$WALK/scratch$i.db" || { say "!! [5/10] no scratch copy of the database - nothing installed"; rm -rf "$WALK"; exit 1; }; done
...
WOUT="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch1.db" timeout 900 "$VPY" -B "$KDIR/walk_s405.py" --app "$WALK/finance" --old "$WALK/old" --db "$WALK/scratch1.db" 2>&1 )"
echo "$WOUT" | sed 's/^/   /'
echo "$WOUT" | grep -q "^WALK_S405 GREEN" || { say "!! [5/10] walk red - nothing installed"; clean; rm -rf "$WALK"; exit 1; }
clean
say "[5/10] walk_s405 green on a scratch copy of the live database (above)"
W404="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch2.db" timeout 1500 "$VPY" -B "$K404/walk_s404.py" --app "$WALK/finance" --old "$WALK/old404" --marg-new "$WALK/marg_ingest" --marg-old "$WALK/marg_old" \
         --db "$WALK/scratch2.db" --portal-new "$WALK/p403" --portal-old "$WALK/p404" 2>&1 )"
...
W403="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch3.db" timeout 1500 "$VPY" -B "$K403/walk_s403.py" --app "$WALK/finance" --old "$WALK/old403" \
         --assets-new "$WALK/assetapp_live" --assets-old "$WALK/assetapp_old3" --db "$WALK/scratch3.db" --assets-db "$WALK/assets_scratch.db" \
...
# S400's frozen walk asserts Needs you 'unchanged except its own line'; S403's three lines break that by the owner's brief, so that
# re-run alone gets NEEDS_YOU_WITHOUT_S403=1 (an env switch the service never sets) -- exactly as S403's installer ran it.
W400="$( cd "$WALK" && env $WENV NEEDS_YOU_WITHOUT_S403=1 FINANCE_DB="$WALK/scratch4.db" timeout 900 "$VPY" -B "$K400/walk_s400.py" --app "$WALK/finance" --old "$WALK/old0" --db "$WALK/scratch4.db" --portal-new "$WALK/p404" --portal-old "$WALK/pold" 2>&1 )"
...
W402="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch5.db" timeout 900 "$VPY" -B "$K402/walk_s402.py" --app "$WALK/finance" --old "$WALK/old2" --db "$WALK/scratch5.db" 2>&1 )"
...
clean
say "[6/10] S404's, S403's, S400's and S402's own walks re-run on the patched files: still green (above)"
copydb "$DBF" "$FIN/finance.db.bak_S405_$STAMP" || { say "!! [7/10] database backup failed - nothing installed"; rm -rf "$WALK"; exit 1; }
declare -A BAK
for f in "${ORDER[@]}"; do BAK[$f]="${DEST[$f]}.bak_S405_$(m5 "${DEST[$f]}" | cut -c1-8)"; \cp -p "${DEST[$f]}" "${BAK[$f]}" || { say "!! [7/10] backup failed - nothing placed"; rm -rf "$WALK"; exit 1; }; done
say "[7/10] finance.db.bak_S405_$STAMP made; .bak_S405_<from8> beside each of the three files"
restore() {
  ...
  say "   healthz $(curl -s -o /dev/null -m 8 -w '%{http_code}' http://127.0.0.1:8106/finance/healthz) · the database backup stays: $FIN/finance.db.bak_S405_$STAMP"
...
"$SPY" -B seed_s405.py "$DBF" || restore
say "[9/10] the setting seeded (neft.sms_tolerance_p)"
...
c4=$(curl -s -o /dev/null -m 8 -w '%{http_code}' -X POST http://127.0.0.1:8106/finance/api/bank-sms)
c5=$(curl -s -o /dev/null -m 8 -w '%{http_code}' http://127.0.0.1:8106/finance/approvals)
say "health : finance healthz $c2 · /finance/bank-sms without a login $c3 · the door without a key $c4 (401 = the key gate, expected) · /finance/approvals without a login $c5 (302 = a login gate, expected)"
[ "$c2" = 200 ] && { [ "$c3" = 302 ] || [ "$c3" = 401 ] || [ "$c3" = 403 ]; } && [ "$c4" = 401 ] && { [ "$c5" = 302 ] || [ "$c5" = 401 ]; } || restore
journalctl -u clinic-finance --since "-2 min" --no-pager 2>/dev/null | grep -q "NOT mounted" && restore
say "[10/10] clinic-finance active, healthz 200, the door answers 401 to a keyless post, nothing 'NOT mounted'"
clean; rm -rf "$WALK"
for f in "${ORDER[@]}"; do md5sum "${DEST[$f]}"; done
say "$KIT: DONE -- https://followup.dr-manoj.in/finance/bank-sms (the Ignored card, the Yes Bank table) · https://followup.dr-manoj.in/finance/approvals (Needs you: OK / Not this)"
