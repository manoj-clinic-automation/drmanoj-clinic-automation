...
<div class="btnbar">
  ...
  <a class="pill" href="{{ prefix }}/salary/scenario?ym={{ ym }}">&#128202; Deduction scenario</a>
  <a class="pill" href="{{ prefix }}/salary/flow?ym={{ ym }}">&#128203; Month-end flow</a>
</div>
...
def _flow_doc(kind):
    ...
    try:
        ...
        if kind == "sheet1":
            ...
            doc = P.sheet2_html(res, doors=True, back=back)
        else:
            st = pack_status(ym)
            doc = P.sheets34_html(res, back=back,
...
def salary_lock():
    ...
    # S199: the month-end pack must be approved before a lock (owner flow).
    _Pol, _perr = _policy_module()
    if _Pol is not None and int(_Pol.load_settings().get("require_pack_approval", 1)):
        _st = pack_status(ym)
