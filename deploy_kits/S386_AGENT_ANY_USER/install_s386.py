# -*- coding: utf-8 -*-
r"""install_s386.py -- kit S386_AGENT_ANY_USER (F-618). Run ON THE MEDICAL PC by INSTALL_S386.bat.
...
ONE copy of it. Stops at the first thing that is not as expected, and puts everything back.
  ...
  4  start-up: with administrator rights, MargAgent.cmd goes into the ALL-USERS start-up folder
     and every account's own old MargAgent.cmd is renamed .replaced_S386.bak; D:\SendToClinic is
...
FILES = {"agent_guard.py": "cfa99ffaf5894a83dc114b9832a73f60",
         "START_AGENT.cmd": "3a57650df28763997d61708b86d0ffec"}
...
def main():
    say("S386 -- the medical agent, for whichever account signs in")
    ...
    for n, h in FILES.items():
        ...
        try:
            if old:
                shutil.copy2(dst, "%s.bak_S386_%s" % (dst, old[:8]))
    ...
    if "SELFTEST OK" not in out:
        for dst, old in placed:
            try:
                if old:
                    shutil.copy2("%s.bak_S386_%s" % (dst, old[:8]), dst)
                else:
                    os.remove(dst)
            except OSError:
                pass
    ...
    if admin:
        ...
        for old in glob.glob(r"C:\Users\*\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\%s" % ENTRY):
            try:
                os.replace(old, old + ".replaced_S386.bak")
        ...
        try:
            ...
            if os.path.exists(dst) and open(dst, "rb").read() != body:
                os.replace(dst, dst + ".replaced_S386.bak")
