...
# --------------------------------------------------------------------------
def rejudge(path, sigs, archive=None, seen=None):
    """Returns a result dict shaped like marg_router.process()'s, or None if the
    file cannot be opened at all. `archive` and `seen` (index rows by md5) let
    the S224 stock-variant rule see the same universe the router sees."""
    ...
    res["rows"] = sh.nrows
    # S229 (F-351): read_preamble gained a fourth return value at S228 -- c0, the
    # column the preamble actually starts in, because Marg's BATCH WISE STOCK
    # report indents the whole sheet by one. This file still unpacked three and
    # so raised ValueError on the FIRST quarantined file of every run: the rescue
    # tool had been dead since that fix, which is why three item ledgers and two
    # salt lists sat in quarantine while the registry already knew them.
    # ONE JUDGEMENT, NOT TWO: c0 is passed on to dates_from exactly as the router
    # passes it, so an indented report is dated here the way it is dated there.
    title, header, hrow, c0 = R.read_preamble(sh)
    sig, status, why = R.identify(title, header, sigs)
    d_from, d_to, b_from, b_to = R.dates_from(title, sh, hrow, c0)
    ...
    if status == "IDENTIFIED":
        ...
        if (verdict == "VERIFIED" and hasattr(R, "stock_variant")
                and sig["type"] in getattr(R, "STOCK_TYPES", ())):
            # S224 (F-235): the router's own variant rule, not a copy of it.
            res["variant"], res["notes"] = R.stock_variant(
                path, sh, sig, header, hrow, d_from, res["md5"],
                archive, seen or {}, write_cache=False)
    elif status == "REFUSED":
...
def run(args):
    ...
    seen_rows = {(r.get("md5") or "").strip().lower(): r for r in rows}
    rescued, still, changed_rows = [], [], 0
    for path in files:
        res = rejudge(path, sigs, archive, seen_rows)
        if res is None:
...
def selftest(_args):
    ...
    ck("S224: the router exposes the stock-variant rule this tool reuses",
       hasattr(R, "stock_variant") and "notes" in R.INDEX_COLS)
