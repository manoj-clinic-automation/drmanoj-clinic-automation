...
SR_DIR="$T/sr" bash "$K/install_sheets_v5.sh" > "$T/o1" 2>&1; rc=$?
...
chk "backups exist" "ls $T/sr/*.bak_S238_SHEETS_V5_* >/dev/null 2>&1"
echo "== 2 re-run"; SR_DIR="$T/sr" bash "$K/install_sheets_v5.sh" > "$T/o2" 2>&1; rc=$?
chk "re-run exit 0" "[ $rc = 0 ]"; chk "re-run says already" "grep -q 'already installed' $T/o2"
echo "== 3 red after install"; fresh; echo 500 > "$T/health"
SR_DIR="$T/sr" bash "$K/install_sheets_v5.sh" > "$T/o3" 2>&1; rc=$?
...
SR_DIR="$T/sr" bash "$K/install_sheets_v5.sh" > "$T/o4" 2>&1; rc=$?
...
SR_DIR="$T/sr" bash "$T/k2/install_sheets_v5.sh" > "$T/o5" 2>&1; rc=$?
