#!/usr/bin/env python3
"""probe_sheets5.py -- kit S238_SHEETS_V5 (the F-414 fix: the copy under test reads the
...
difference must be the hold threshold alone; renders every sheet from the new result.
   v1.12 verifies cover days by the out-punch for cover staff: only overtime, the extra-duty
   credit and the net may move, and the net by exactly (overtime paid + extra-duty credit):
   every field must match except the new OT fields, and every net must be identical.
   usage: probe_sheets5.py <new salary_policy.py> <live salary_policy.py> <YYYY-MM>
   (internal: probe_sheets5.py --one <salary_policy.py> <YYYY-MM> <render 0|1>)"""
...
fails = []
NEWF = {"ot_min", "ot_rs", "ot_paid", "extra_rs", "extra_days", "duty_credits"}
print("  %-11s %11s %11s   %7s %11s   %9s %9s" % ("staff", "net NOW", "net NEW", "OT min",
                                                  "OT paid", "Cover NOW", "Cover NEW"))
for n, s in new["staff"].items():
    ...
    _exp = (s.get("ot_paid", 0) - o.get("ot_paid", 0)) + (s.get("extra_rs", 0) - o.get("extra_rs", 0))
    if abs((s.get("net", 0) - o.get("net", 0)) - _exp) > 0.01:
        diff.append("net (by more than overtime + extra duty)")
    flag = ""
    if diff:
        flag = "   <-- CHANGED: " + ", ".join(diff); fails.append(n)
    print("  %-11s %11.2f %11.2f   %7d %11.2f   %9.2f %9.2f%s" % (
        n, o.get("net", 0), s.get("net", 0), s.get("ot_min", 0), s.get("ot_paid", 0),
        o.get("extra_rs", 0), s.get("extra_rs", 0), flag))
