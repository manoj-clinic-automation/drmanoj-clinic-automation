#!/usr/bin/env bash
# install_sheet2_cover.sh · kit S239_SHEET2_COVER1      Run by:  bash /root/deploy/vps_deploy.sh S239_SHEET2_COVER1
# ONE file: /root/staff_register/salary_policy.py  v1.13 (c4238f0c) -> v1.14
# The owner, 11-Sep-2026: Cover duty in ONE column -- the rupees with the days in brackets, 600 (3).
# Display only: the probe proves every computed figure is identical before anything is replaced.
set -u
KIT="S239_SHEET2_COVER1"
KDIR="$(cd "$(dirname "$0")" && pwd)"
SPF="/root/staff_register/salary_policy.py"; B1=c4238f0cbf4fca65df116fc23fc3bc58; N1=07cb3ddd30d92545ed23e0e55836a704
...
[ "$c1" = "$B1" ] || { echo "!! [2/6] CURRENCY GATE -- $SPF is $c1, expected v1.13 $B1. Nothing installed."; exit 1; }
echo "[2/6] live file is v1.13"
PR=/tmp/s239_cov_probe; rm -rf "$PR"; mkdir -p "$PR" && chmod 700 "$PR"; cp salary_policy.py probe_sheet2_cover.py "$PR/"
...
( cd "$PR" && "$PY" -B probe_sheet2_cover.py "$PR/salary_policy.py" "$SPF" "$MONTH" ) || { echo "!! [4/6] probe RED -- nothing installed"; rm -rf "$PR"; exit 1; }
rm -rf "$PR"
