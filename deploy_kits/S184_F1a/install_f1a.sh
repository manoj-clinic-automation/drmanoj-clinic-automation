#!/bin/bash
# =============================================================================
#  install_f1a.sh · kit S184_F1a — D322 missing-day classifier in finance_app.py
#
...
cd "$KIT_DIR" \
&& md5sum -c SUMS.md5 \
&& [ "$(awk 'NR==1{print $1}' KIT_ID.txt)" = "S184_F1a" ] \
...
&& cp finance_app.py finance_app.py.bak_S184F1 \
&& cp finance.db finance.db.bak_S184F1 \
&& touch .S184F1_touched \
...
&& echo "" \
&& echo "=============================================================" \
&& echo " S184_F1a INSTALLED — D322 missing-day classifier live" \
...
&& echo "   backups: finance_app.py.bak_S184F1 · finance.db.bak_S184F1" \
&& echo "   The dashboard reclassifies the Sunday/holiday shouts on next load." \
&& echo "=============================================================" \
|| { echo ""; echo "RED — install did not complete."; \
     if [ -f "$LIVE/.S184F1_touched" ]; then \
        echo "   live files WERE touched — restoring:"; \
        cp -f "$LIVE/finance_app.py.bak_S184F1" "$LIVE/finance_app.py" && echo "   finance_app.py restored"; \
        cp -f "$LIVE/finance.db.bak_S184F1" "$LIVE/finance.db" && echo "   finance.db restored"; \
        rm -f "$LIVE/.S184F1_touched"; \
