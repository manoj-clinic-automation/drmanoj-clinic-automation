#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""selftest_s359.py -- S359_CASH_LOG_2: the doctors' cash log and the month table, on a scratch database.
...
with Flask stubbed, files four counter days the way the autofile does, and proves:
  ...
  grants edit producing v24 with Bhawna's tile.
    python3 -B selftest_s359.py [--python PY]
...
def load_views():
    ...
    p = os.path.join(HERE, "views_s359.sql")
...
def main():
    ...
    a = ap.parse_args()
    tmp = tempfile.mkdtemp(prefix="s359_")
    ok = n = 0
    ...
    try:
        ...
        r = subprocess.run([a.python, "-B", os.path.join(HERE, "grant_kal_s359.py"), g, out], capture_output=True, text=True)
        ...
        check("grants: v23 -> v24, bhawna gains 'Kal ka hisaab', darpan untouched",
              ...
              and j["users"]["darpan"]["extra"] == ["Kal ka hisaab"] and "v24 (S359" in j["_note"])
        r2 = subprocess.run([a.python, "-B", os.path.join(HERE, "grant_kal_s359.py"), out, out + ".x"], capture_output=True, text=True)
