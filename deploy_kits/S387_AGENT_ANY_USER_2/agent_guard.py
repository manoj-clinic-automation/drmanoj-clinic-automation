# -*- coding: utf-8 -*-
r"""agent_guard.py -- ONE medical agent for the whole machine, whichever Windows account signs in.  (S386, S387)

S387 (24-Sep-2026): S386 was refused at the medical PC by its own proof -- after stepping back for an
agent from the old launcher, the guard read its own stopped agent as "gone" and started it again at
once, and the two flapped (Windows reports a stopped process as ended immediately; the build box
hid it). Now a guard that steps back forgets its own agent and stands aside for as long as the other
one's heartbeat is fresh. The selftest samples for four seconds that exactly one agent beats.
...
def reap(p):
    """Collect a stopped agent's exit so it reads as stopped at once -- on Windows it already does;
    elsewhere an unreaped child would look alive and hide a fault only Windows shows (S387)."""
    try:
        p.wait(timeout=10)
    except Exception:                                           # noqa: BLE001
        pass
...
def run_once():
    ...
    while True:
        ...
        if starts_since(t0) >= 2:
            kill_tree(p.pid)
            reap(p)
            write_child(0)          # S387: the fresh heartbeat is now the OTHER agent's -- stand aside for it
...
def selftest():
    ...
    try:
        a = guard("A")
        ck("the first account's guard starts the agent", beats_from("A", 15.0))
        b = guard("B")
        time.sleep(2.5)
        ...
        if c:                                                     # its guard AND its agent end with it
            kill_tree(c)
        ck("when that account signs out, the other takes over within seconds", beats_from("B", 15.0))
        ...
        def stepped_back():
            try:
                return "stepped back" in open(os.path.join(d, "agent_guard.log"), encoding="utf-8").read()
            except OSError:
                return False
        settled = wait_until(stepped_back, 15.0)
        if settled:
            time.sleep(1.0)                                      # let the stopped one's last beat pass
        seen = pids_beating(4.0) if settled else set()
        ck("an agent from the old launcher appears -> the guarded one steps back and STAYS back (one agent, not two)",
           settled and len(seen) == 1, "agents seen beating over 4 s: %d" % len(seen))
        kill_tree(leg.pid)
        ...
        ck("AGENT_OFF.txt stops the agent", wait_until(lambda: heartbeat_age_in(d) > 1.2, 15.0))
        os.remove(os.path.join(d, "_off", "AGENT_OFF.txt"))
        ck("deleting AGENT_OFF.txt brings it back", beats_from("B", 15.0))
