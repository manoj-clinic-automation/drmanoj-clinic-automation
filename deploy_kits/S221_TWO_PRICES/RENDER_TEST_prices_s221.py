...
# -*- coding: utf-8 -*-
"""
RENDER_TEST_prices_s221.py -- a REAL BROWSER on the corrected document.

Carries the S221_STOCK_FINDING browser gate forward and adds what this kit is
for: the money column that was mislabelled now says what it is, a real MRP
column sits beside it, the coverage of both is on the page, and the drift page
renders honestly when it has nothing to say.
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # THE CORRECTION, on the glass
        ck("the document now says where the MRP figure comes from",
           "selling rate" in body and "printed strip rate" in body, body[:900])
        ck("and where the cost figure comes from",
           "last purchase rate" in body, body[:900])
        ck("the old claim that cost is unavailable is GONE",
           "purchase prices are not in the system" not in body)
        ck("the coverage of the two prices is stated on the page",
           "could not be priced" in body or "priced both ways" in body, body[:1200])
        ck("the shortage at cost still shows (Rs 20.00 for 8 units at Rs 2.50)",
           "20.00" in body, body[:900])
        ck("an item with NEITHER price has its own block, outside every total",
           "No price at all" in body and "ZZR NORATE" in body, body[:900])
        ck("and the block says why -- never bought, never sold",
           "never been bought" in body and "never been sold" in body)
        ...
        pg3.goto("http://127.0.0.1:%d/finance/stock/page/drift" % port)
        pg3.wait_for_timeout(1500)
        dbody = pg3.inner_text("body")
        ck("the drift page renders", "Stock drift" in dbody)
        ck("no javascript error on it", not e3, "; ".join(e3[:2]))
        ck("with nothing to compare it SAYS so, rather than showing an empty table",
           "nothing can be compared" in dbody or "Nothing has been pushed" in dbody
           or "Nothing comparable yet" in dbody, dbody[:400])
        ck("it explains the difference between a bug and a shelf, in words",
           "arithmetic" in dbody and "shelf" in dbody)
        pg3.close()
        b.close()
