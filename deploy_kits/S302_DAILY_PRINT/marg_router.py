...
RE_RANGE = re.compile(r"FROM\s+(\d{2}-\d{2}-\d{4})(?:\s*(?:TO|-|\u2013)\s*(\d{2}-\d{2}-\d{4}))?", re.I)   # S228: the item ledger writes "FROM a - b", not "FROM a TO b"
...
def stock_universe(archive, seen, as_on, exclude_md5, write=True):
    ...
    for md5, row in seen.items():
        ...
        if not ent:
            ...
            try:
                sh = open_sheet(p)
                _t, hdr, hrow, _c0 = read_preamble(sh)
...
def read_preamble(sh, look=25):
    ...
    for r in range(min(sh.nrows, look)):
        ...
        if len(filled) >= 3:
            ...
            break
    # THE PREAMBLE IS NOT ALWAYS IN COLUMN 0. Marg's BATCH WISE STOCK report
    # indents the whole sheet by one column, and reading the title from a fixed
    # column 0 found nothing, so the file matched no signature AND carried no
    # data dates -- refused twice over, for the same one reason. Read from the
    # first column the header row actually starts in. (S228)
    c0 = 0
    if header_row is not None:
        for c in range(min(sh.ncols, 6)):
            if cell(sh, header_row, c):
                c0 = c
                break
    # S302: A PRINT-LAYOUT REPORT. Marg can also write a report as ONE text column -- the
    # owner's daily summary sale report does -- where no row carries 3+ cells: the column
    # heads are a single line boxed between two rules of dashes. Only when no ordinary
    # header row exists, and only in a sheet of at most two columns, that boxed line is the
    # header (as one cell) and the rules are not titles. Every report read before is read
    # exactly as before.
    text_layout = False
    if header_row is None and sh.ncols <= 2:
        lines = [(r, cell(sh, r, 0)) for r in range(min(sh.nrows, look))]
        lines = [(r, t) for r, t in lines if t]
        for i in range(1, len(lines) - 1):
            r, t = lines[i]
            if (not _is_rule(t) and len(t.split()) >= 4 and _is_rule(lines[i - 1][1])
                    and _is_rule(lines[i + 1][1])):
                header_row, header, c0, text_layout = r, [t], 0, True
                break
    title = ""
    upto = header_row if header_row is not None else min(sh.nrows, look)
    for r in range(upto):
        t = cell(sh, r, c0)
        if t and not (text_layout and _is_rule(t)):
            title = t          # last non-empty single-cell line wins
    return title, header, header_row, c0


def _is_rule(t):
    """S302: a line drawn with dashes or equals signs, as Marg's print layout draws them."""
    s = (t or "").replace(" ", "")
    return len(s) >= 8 and set(s) <= set("-=_")


def dates_from(title, sh, header_row, c0=0):
...
def ends_with_row(sh, words, tail=40):
    """S240 (F-429). True when one of the last `tail` rows BEGINS with these words, in
    order -- whether Marg prints them in one cell ('GRAND TOTAL') or splits them across
    cells ('GRAND' | 'TOTAL'). Matched on the ROW's leading words, not on any cell's
    substring, so an item description that merely contains 'GRAND' cannot pass it.
    Used where Marg's own footer line is an advert that it rotates (S219, 09-Sep)."""
    if not words:
        return True
    want = [str(w).upper() for w in words]
    for r in range(max(0, sh.nrows - tail), sh.nrows):
        toks = []
        for c in range(sh.ncols):
            v = cell(sh, r, c).upper()
...
def verify(path, sh, sig, title, header, header_row, d_from, d_to, b_from, b_to):
    ...
    if sig and sig.get("end_row") and not ends_with_row(sh, sig.get("end_row")):
        problems.append("TRUNCATED — the closing row %r is missing; the export "
                        "stopped early" % " ".join(sig.get("end_row")))
    if problems:
...
def process(path, sigs, cfg, seen, out):
    ...
    try:
        ...
        return _archive_and_index(path, res, cfg, digest, "REFUSED", _why,
                                  None, None, None, "", [], out)
    res["rows"] = sh.nrows
    title, header, hrow, c0 = read_preamble(sh)
    sig, status, why = identify(title, header, sigs)
    d_from, d_to, b_from, b_to = dates_from(title, sh, hrow, c0)
...
def learn(path, out):
    sh = open_sheet(path)
    title, header, _, _c0 = read_preamble(sh)
    block = {
...
def selftest(out):
    ...
    # S302: the owner's daily summary sale report -- Marg's print layout, one text column
    class _T(object):
        def __init__(self, rows):
            self.rows = rows; self.nrows = len(rows); self.ncols = 1
        def cell_value(self, r, c):
            return self.rows[r] if c == 0 else ""
    _rule = "-" * 60
    _txt = _T(["BILL WISE SALES STATEMENT AS ON", "", _rule, "",
               "BILL NO.   DESCRIPTION       D.R.  GROSS AMT.  DISCOUNT   TAX  DR/CR  NET AMT.", "", _rule, "",
               "16-09-2026", "", "X0001 ########## A CUSTOMER", "", "1 4 SOME ITEM 1*15 1:0 165.00 10/28", "",
               "DAY TOTAL : 165.00 0.00 0.00 0.00 165.00", "", _rule, "",
               "Total No. of Bills: 1 GRAND TOTAL : 165.00 0.00 0.00 0.00 165.00", "", _rule, "", "*** End of Report ***"])
    _t, _h, _hr, _c0 = read_preamble(_txt)
    ck("print layout: the boxed line is the header, the rule is not the title",
       _hr == 4 and _t == "BILL WISE SALES STATEMENT AS ON" and len(_h) == 1 and _h[0].startswith("BILL NO."))
    _s5, _st5, _ = identify(_t, _h, sigs)
    ck("print layout: identified as SALE_DAILY_PRINT, never as SALE_BILLWISE",
       _st5 == "IDENTIFIED" and _s5["type"] == "SALE_DAILY_PRINT")
    ck("SALE_DAILY_PRINT is never uploadable", _s5 and _s5.get("uploadable") is False)
    _d5 = dates_from(_t, _txt, _hr, _c0)
    ck("print layout: dated from its day line", _d5[0] == "2026-09-16" and _d5[1] == "2026-09-16")
    ck("print layout: verifies with its end marker",
       verify("", _txt, _s5, _t, _h, _hr, _d5[0], _d5[1], _d5[2], _d5[3])[0] == "VERIFIED")
    _cut = _T(_txt.rows[:-1])
    ck("print layout: a cut-off print is refused as truncated",
       verify("", _cut, _s5, _t, _h, _hr, _d5[0], _d5[1], _d5[2], _d5[3])[0] == "REFUSED")
    _s6, _st6, _ = identify("BILL WISE SALES STATEMENT AS ON 19-08-2026",
        ["BILL NO.","DESCRIPTION","D.R.","GROSS AMT.","DISCOUNT","TAX","DR/CR","NET AMT.","CASH"], sigs)
    ck("the DETAIL export is still SALE_BILLWISE DETAIL", _st6 == "IDENTIFIED" and _s6["type"] == "SALE_BILLWISE" and _s6["variant"] == "DETAIL")
    _grid = _T(["NOT A TEXT REPORT", "", _rule, "", "ONE TWO THREE FOUR", "", "just text", "no rule"])
    _grid.ncols = 3
    ck("a sheet wider than two columns never takes the print-layout reading", read_preamble(_grid)[2] is None)
