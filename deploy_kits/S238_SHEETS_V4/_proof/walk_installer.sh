...
SR_DIR="$T/sr" bash "$K/install_sheets_v4.sh" > "$T/o1" 2>&1; rc=$?
...
chk "ot_pay is 1" "grep -q '\"ot_pay\": 1' $T/sr/salary_policy_settings.json"
chk "backups exist" "ls $T/sr/*.bak_S238_SHEETS_V4_* >/dev/null 2>&1"
echo "== 2 re-run"; SR_DIR="$T/sr" bash "$K/install_sheets_v4.sh" > "$T/o2" 2>&1; rc=$?
chk "re-run exit 0" "[ $rc = 0 ]"; chk "re-run says already" "grep -q 'already installed' $T/o2 && grep -q 'already 1' $T/o2"
echo "== 3 red after install"; fresh; echo 500 > "$T/health"
SR_DIR="$T/sr" bash "$K/install_sheets_v4.sh" > "$T/o3" 2>&1; rc=$?
...
chk "settings restored" "! grep -q ot_pay $T/sr/salary_policy_settings.json"
echo "== 4 wrong live file"; fresh; echo 200 > "$T/health"; echo "# changed" >> "$T/sr/salary_policy.py"; cp "$T/sr/salary_policy.py" "$T/before"
SR_DIR="$T/sr" bash "$K/install_sheets_v4.sh" > "$T/o4" 2>&1; rc=$?
chk "refused exit 1" "[ $rc = 1 ]"; chk "CURRENCY GATE printed" "grep -q 'CURRENCY GATE' $T/o4"
chk "nothing touched" "cmp -s $T/before $T/sr/salary_policy.py && cmp -s $OLD_SR $T/sr/staff_register.py"
echo "== 5 tampered kit"; cp -r "$K" "$T/k2"; echo x >> "$T/k2/salary_policy.py"; fresh
SR_DIR="$T/sr" bash "$T/k2/install_sheets_v4.sh" > "$T/o5" 2>&1; rc=$?
