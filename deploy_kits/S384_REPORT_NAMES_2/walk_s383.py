#!/usr/bin/env python3
"""walk_s383.py -- kit S383_REPORT_NAMES (S382's walk + the name on the report + the counts). The real slip_log.py over a SCRATCH COPY of the real finance.db, on the walk's
...
slip_log.py (S382) is the negative control. Prints IDs and counts only.
...
# ---- S383: the name on the lab report against the ID's name
con.execute("DELETE FROM record_file WHERE clinic_id LIKE '9092%' AND kind='blood'")
con.executemany("INSERT INTO record_file(clinic_id, kind, day, drive_id, file_name, mime, bytes, source, source_ref, note, added_by, added_at) "
                "VALUES (?,?,?,?,?,'application/pdf',0,'lab_mail',?,'','walk','')", [
    ("909201", "blood", D, "walkDRIVEB00001", "%s \u00b7 909201 \u00b7 WALK 909201 \u00b7 Blood report.pdf" % D, "walkB1"),
    ("909202", "blood", D, "walkDRIVEB00002", "%s \u00b7 909202 \u00b7 Mrs QQQQ ZZZZ \u00b7 Blood report.pdf" % D, "walkB2"),
    ("909203", "blood", D, "walkDRIVEB00003", "%s \u00b7 909203 \u00b7 Smt Walkk \u00b7 Blood report.pdf" % D, "walkB3")])
con.commit()
nm = {r["clinic_id"]: r for r in S.name_mismatches(con) if r["clinic_id"].startswith("9092")}
ok(set(nm) == {"909202"}, "the report whose name is nothing like the ID's is flagged; the same name and a spelling variant are not", sorted(nm))
ok(S.same_name("Ram Kishor", "Ramkishore") and S.same_name("Smt Sunita Devi", "SUNITA") and not S.same_name("Rahul Verma", "Pooja Gupta"),
   "the name rule: spelling and titles forgiven, different people not")
USER["u"] = {"user": "shivani", "roles": ["maker"]}
at(D2 + "T12:00:00")
pg = c.get("/finance/slips/pending").get_data(as_text=True)
ok("Report par naam alag" in pg and "Wahi mareez" in pg, "reception sees the name mismatch with its two buttons")
USER["u"] = {"user": "sukhveer", "roles": ["viewer"]}
ok("Report par naam alag" not in c.get("/finance/slips/pending").get_data(as_text=True), "Sukhveer does not get the mismatch lines")
USER["u"] = {"user": "manoj", "roles": ["checker"]}
j = c.get("/finance/slips/api/pending-counts").get_json()
ok(j["ok"] and j.get("name_mismatch", 0) >= 1 and {"blood", "blood_red", "xray", "xray_red", "orphans"} <= set(j), "the doctors' counts carry the mismatch", j)
USER["u"] = {"user": "shivani", "roles": ["maker"]}
c.post("/finance/slips/pending/act", data={"key": nm["909202"]["key"], "act": "nm_ok"})
ok(not [r for r in S.name_mismatches(con) if r["clinic_id"] == "909202"], "Wahi mareez closes it")
USER["u"] = {"user": "manoj", "roles": ["checker"]}
menu_live = None
...
ok(not hasattr(S0, "name_mismatches") and "name_mismatch" not in S0.pending_counts(con), "negative control: the live slip_log has no name check")
ok({r.rule for r in A.url_map.iter_rules()} - {r.rule for r in A0.url_map.iter_rules()} ==
   set() and not ({r.rule for r in A0.url_map.iter_rules()} - {r.rule for r in A.url_map.iter_rules()}), "routes identical")
