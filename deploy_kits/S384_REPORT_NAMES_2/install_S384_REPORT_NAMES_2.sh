#!/bin/bash
# =============================================================================
#  install_S384_REPORT_NAMES_2.sh · kit S384_REPORT_NAMES_2 (session 279, 24-Sep-2026) -- step 3 of the owner's plan
#
#  S384 = S383's payload, byte-identical. S383 went red after placing without saying which check failed; its journal
#  check counted ANY Traceback / NOT mounted in the last two minutes, whoever logged it. Now every failing check says
#  its name, the journal is read only from this install's own restart, and only a slip_log / portal fault restores;
#  anything else found there is printed as information.
#
...
#    cd /root/deploy/repo && git pull --ff-only && bash /root/deploy/repo/deploy_kits/S384_REPORT_NAMES_2/install_S384_REPORT_NAMES_2.sh
# =============================================================================
set -u
KIT="S384_REPORT_NAMES_2"; KDIR="$(cd "$(dirname "$0")" && pwd)"
...
STAMP="$(date +%Y%m%d_%H%M%S)"; WALK="/tmp/s384_walk_$STAMP"
...
say "[5/7] $POUT"
BS="$FIN/slip_log.py.bak_S384_3e6cb9b0"; BP="$PD/portal.py.bak_S384_f9c7e306"
...
OTHER="$(echo "$J" | grep -c "Traceback\|NOT mounted")"
[ "$OTHER" != 0 ] && say "   (information: $OTHER other Traceback / NOT mounted line(s) in clinic-finance since the restart, not from slip_log -- not this kit's; first: $(echo "$J" | grep -m1 "NOT mounted\|Error" | cut -c1-160))"
\cp -p portal.py "$PD/portal.py" && [ "$(m5 "$PD/portal.py")" = "$P_TO" ] || restore "portal.py did not read back"
...
PH=$(curl -s -m 8 "http://127.0.0.1:$PPORT/portal/health"); echo "$PH" | grep -q '"status": *"ok"' || restore "portal health answered: $(echo "$PH" | cut -c1-120)"
