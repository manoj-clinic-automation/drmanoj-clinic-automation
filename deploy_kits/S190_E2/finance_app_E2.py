...
def api_day(date_iso):
    ...
    _adv_mtd = advance_month_to_date_p(con, d.isoformat())
...
def api_save_day():
    ...
    if adv_new_p:
        ...
        _prior_p = advance_month_to_date_p(
            con, iso, exclude_day_entry_id=(_existing_row["id"] if _existing_row else None))
...
def selftest():
    ...
    check("D330c: the clinic page carries the attach-the-bill control",
          "Attach the bill (required to file)" in _chh2)
    if _cd:
    ...
    if _ed:
        ...
        check("D330: the page carries the attach-the-bill control",
              "Attach the bill (required to file)" in _eh2)
