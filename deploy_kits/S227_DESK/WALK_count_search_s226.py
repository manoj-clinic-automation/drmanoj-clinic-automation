...
with sync_playwright() as pw:
    ...
    # ---- S226 ZEROFIX. THE ONE THAT MUST NEVER COME BACK.
    #      An entry made by pressing OK, before the breakdown was recorded,
    #      holds a total and nothing else. Opening it filled nothing in, calc()
    #      read two empty boxes, computed 0, and SAVED IT. Searching for an item
    #      you had already counted destroyed it, in silence, mid-count.
    pg.evaluate("""() => { const K='sanj-stock-live-v1';
        const S=JSON.parse(localStorage.getItem(K));
        S.e['TYRO BR']={c:700, cby:'Darpan', eby:'Amir', at:'2026-09-06T11:00:00'};
        S.e['ARM SLING XL']=undefined; delete S.e['ARM SLING XL'];
        localStorage.setItem(K, JSON.stringify(S)); }""")
    pg.reload(); pg.wait_for_timeout(600); pg.locator("#start").click(); pg.wait_for_timeout(300)
    before = pg.evaluate("JSON.stringify(JSON.parse(localStorage.getItem('sanj-stock-live-v1')).e['TYRO BR'])")
    pg.locator("#find").fill("tyro"); pg.wait_for_timeout(700)
    after = pg.evaluate("JSON.stringify(JSON.parse(localStorage.getItem('sanj-stock-live-v1')).e['TYRO BR'])")
    chk("AN OLD ENTRY IS NOT DESTROYED BY LOOKING AT IT", after == before, (before, after))
    chk("...its figure is still 700, not 0",
        pg.evaluate("JSON.parse(localStorage.getItem('sanj-stock-live-v1')).e['TYRO BR'].c") == 700)
    vals = pg.locator("#list .entry input").evaluate_all("e=>e.map(x=>x.value)")
    chk("...and it opens FILLED, derived from the total it does have",
        vals[:2] == ["70", "0"], vals)           # TYRO BR is 1*10: 700 = 70 strips
    pg.locator("#list .entry .qs").fill("47"); pg.wait_for_timeout(400)
    chk("...while a real edit still saves",
        pg.evaluate("JSON.parse(localStorage.getItem('sanj-stock-live-v1')).e['TYRO BR'].c") == 470,
        pg.evaluate("JSON.stringify(JSON.parse(localStorage.getItem('sanj-stock-live-v1')).e['TYRO BR'])"))
    pg.locator("#findclear").click(); pg.wait_for_timeout(250)
