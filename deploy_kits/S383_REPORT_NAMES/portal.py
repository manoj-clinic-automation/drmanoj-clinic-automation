...
Session 200 (S200_R8): the two Staff-Ledger tiles go same-origin (/ledger) —
the followup vhost proxies /ledger since this kit (phase 2 of the unification).
Session 200 (S200_R2b): the two Staff-Register tiles + the Register health chip
go SAME-ORIGIN (/register/...) so taps stay inside the installed portal PWA —
the followup vhost proxies /register since S200_R2a. No other tile touched.
...
TILES = [
    ...
    {"icon": "\U0001F552", "name": "Meri attendance",
     # S295 NEW (owner, 17-Sep): every staff member's own page -- today's punch only, "Mark me
     # present" in the morning, "Mark my exit" when leaving late (S289). The server shows each login
     # its OWN row and nothing else (/register/me). Given by ROLE so a new staff login gets it without
     # a grants edit; masked by name for the physiotherapist, who is not on the biometric.
     "desc": "Aaj ki punch \u00b7 present / exit request",
     "live": True,
     "url": "/register/me",
     "roles": ["staff", "manager"]},
    ...
    {"icon": "\U0001F9FE", "name": "Stock Check",
     ...
     "desc": "\u0938\u094d\u091f\u0949\u0915 \u0917\u093f\u0928\u0924\u0940 \u2014 every step, in order",
     "live": True,
    ...
    {"icon": "\U0001F4CB", "name": "Aaj ki reports",
     ...
     "roles": ["doctor"]},
    {"icon": "\U0001F58A\uFE0F", "name": "Corrections",
    ...
    {"icon": "\U0001F9D1\u200D\u2695\uFE0F", "name": "Physiotherapy",
     ...
     "url": "/finance/physio",
     "roles": ["doctor"]},
    {"icon": "\U0001F4D4", "name": "Petty book",
     # S289 NEW (owner, 17-Sep). Manoj Bhati's petty book: availability, the Shavez and Darpan
     # diaries topped up, renovation payees, Rahul's monthly, his own loan -- tap and number pad,
     # Hinglish for him; the doctors' check page in English. Its own server unit ('petty'),
     # separate from Sanjeevni and clinic money. Granted by name to bhati and bhawna in v18.
     "desc": "Bhati \u2014 diary, renovation, loan \u00b7 your check",
     "live": True,
     "url": "/finance/petty",
     "roles": ["doctor"]},
    {"icon": "\U0001F9FE", "name": "OPD & X-ray/Proc Slips",
     ...
     "url": "/finance/slips",
     "roles": ["doctor"]},
    {"icon": "\U0001F4C2", "name": "Patient records",
    ...
    {"icon": "\u2705", "name": "Check karein",
     ...
     "url": "/finance/checks",
     "roles": ["doctor"]},
    {"icon": "\U0001F9EA", "name": "Report baaki",
     # S382 NEW (owner, 24-Sep-2026). Blood reports the lab has not mailed and X-ray photos not yet filed: date,
     # clinic ID, name; one tap when mailed / put; a report that arrives clears its own line; mismatches flagged.
     # Served by slip_log.py (unit 'slips'; Sukhveer is its viewer). Granted by name in tile_grants.json to
     # sukhveer, alisha, shivani, shavez.
     "desc": "Blood \u00b7 X-ray report pending \u00b7 ek tap",
     "live": True,
     "url": "/finance/slips/pending",
     "roles": ["doctor"]},
    {"icon": "\U0001F9D1\u200D\u2695\uFE0F", "name": "Bhati aaj",
     # S289 NEW. Reception's one line: is the physiotherapist coming today. Same address; the
     # server shows a 'petty' viewer that line and nothing else. Granted by name in v18.
     "desc": "Physiotherapist aaj aayenge?",
     "live": True,
     "url": "/finance/petty",
     "roles": []},
    {"icon": "\U0001F4C8", "name": "Docterz Revenue",
     ...
     "desc": "Clinic collection by day \u2014 from Docterz",
     "live": True,
     "url": "/finance/clinic/day",
     "roles": ["doctor"]},
    {"icon": "\U0001F4D2", "name": "Docterz daily collection",
     ...
     "desc": "Reception's day totals \u2014 cash, UPI, card",
     "live": True,
...
_TILE_GROUP = {
    ...
    "Attendance": "Staff", "Staff Register": "Staff", "Meri attendance": "Staff",
    ...
    "Vaapsi Desk": "Money & Accounts", "Stock Check": "Money & Accounts",
    "Corrections": "Money & Accounts", "Docterz Revenue": "Money & Accounts",
    "Morning match": "Money & Accounts", "Physiotherapy": "Money & Accounts",
    "Petty book": "Money & Accounts", "Bhati aaj": "Clinic",
    "OPD & X-ray/Proc Slips": "Clinic",
    "Patient records": "Clinic",
    "Check karein": "Clinic",
    "Report baaki": "Clinic",
    "Docterz daily collection": "Money & Accounts", "Marg Purchases": "Money & Accounts", "Order Medicines": "Money & Accounts", "Vendor payments": "Money & Accounts",
...
# S179 (STRUCK at S223): "darpan is role=staff, which is shared. He only needs Daily
...
<div id="pushCard" class="card" style="display:none;margin:0 0 12px;padding:12px 14px;border:1px solid var(--line);border-radius:12px;background:var(--card)">
  ...
  <div class="strip">
    ...
    <a class="chipbox" href="/register/review" target="_blank" rel="noopener">
...
(function(){
  ...
  if(!el)return;
  /* S383: the reports line (Report baaki) joins the calls line; either half may be missing, never shown as zero */
  var calls=fetch('/portal/gist-data',{credentials:'same-origin'})
   .then(function(r){return r.ok?r.json():null;})
   .then(function(d){
  ...
  var reps=fetch('/finance/slips/api/pending-counts',{credentials:'same-origin'})
   .then(function(r){return r.ok?r.json():null;})
   .then(function(p){
     if(!p||!p.ok||typeof p.blood!=='number')return '';
     var s='\U0001F9EA '+p.blood+' blood'+(p.blood_red?' ('+p.blood_red+' late)':'')+' \u00b7 '+p.xray+' X-ray';
     var m=(p.orphans||0)+(p.name_mismatch||0);
     if(m)s+=' \u00b7 '+m+' mismatch';
     return s;
   }).catch(function(){return '';});
  Promise.all([calls,reps]).then(function(v){
     var s=v.filter(function(x){return x;}).join('  \u2502  ');
...
function _swActive(reg){ return new Promise(function(res){ if(reg.active){ res(reg); return; } var w=reg.installing||reg.waiting; if(!w){ res(reg); return; } w.addEventListener('statechange', function(){ if(w.state==='activated'||w.state==='redundant') res(reg); }); }); }
...
async function _pushSubscribe(){
  ...
  /* S370: scope '/portal' (no slash) covers this page at /portal; and we wait on THIS registration, never on
     navigator.serviceWorker.ready, which never resolves when the page is outside the scope (Shivani's phone, 22-Sep) */
  var reg=await _withTimeout(navigator.serviceWorker.register('/portal/sw.js',{scope:'/portal'}), 15000, 'sw-register-timeout');
  await _withTimeout(_swActive(reg), 20000, 'sw-activate-timeout');
  try{ (await navigator.serviceWorker.getRegistrations()).forEach(function(r){ if(r!==reg && /\/portal\/$/.test(r.scope)) r.unregister(); }); }catch(e){}   /* S370: retire the S366 '/portal/' registration */
  ...
  <div class="ucard">
    ...
    <div class="acts">
      <a class="ibtn go" style="text-decoration:none;display:inline-block"
         href="https://followup.dr-manoj.in/finance/staff?flow=join">&#10133; Add a new joiner</a>
      <a class="ibtn" style="text-decoration:none;display:inline-block"
         href="https://followup.dr-manoj.in/finance/staff?flow=exit">&#128682; Exit a leaver</a>
      <a class="ibtn" style="text-decoration:none;display:inline-block"
         href="https://followup.dr-manoj.in/finance/staff">&#128203; All staff records</a>
    </div>
  </div>
...
TRACKER_URL = "https://script.google.com/macros/s/AKfycbyoQ5R3yvFC0B8arOnVWo4002BFfBGIVM2cBwpaMwUM4GaYw7d89jk1U_g38Ht0omcF/exec"
...
<div class="wrap">
  ...
  <a class="gcard" id="gReports" href="/finance/slips/pending" hidden
     style="display:block;text-decoration:none;color:inherit;margin:0 0 14px">
    <div class="glabel">Reports pending \u00b7 blood &amp; X-ray</div>
    <div class="gbig" id="gRepBig">\u2014</div><div class="gsub" id="gRepSub">tap to open the list</div></a>
  <script>
  /* S383: the doctors' line for Report baaki; hidden when the counts cannot be read -- never shown as zero */
  fetch('/finance/slips/api/pending-counts',{credentials:'same-origin'}).then(function(r){return r.ok?r.json():null;})
   .then(function(p){ if(!p||!p.ok||typeof p.blood!=='number')return;
     document.getElementById('gRepBig').textContent=p.blood+' blood \u00b7 '+p.xray+' X-ray';
     var s=[]; if(p.blood_red)s.push(p.blood_red+' blood late'); if(p.xray_red)s.push(p.xray_red+' X-ray late');
     if(p.orphans)s.push(p.orphans+' report with no test written'); if(p.name_mismatch)s.push(p.name_mismatch+' name not matching');
     document.getElementById('gRepSub').textContent=(s.join(' \u00b7 ')||'nothing late')+' \u2014 tap to open';
     document.getElementById('gReports').hidden=false; }).catch(function(){});
  </script>
  ...
  {% else %}
    ...
    <div class="gmetrics">
      ...
      <div class="gcard">
        <div class="glabel">Judgment funnel (7d)</div>
        {% if g.funnel %}
          <div class="gbig">{{ g.funnel.answered_7d }} \u2192 {{ g.funnel.transcribed_7d }} \u2192 {{ g.funnel.judged_7d }}</div>
          <div class="gsub">answered \u2192 transcribed \u2192 judged{% if g.funnel.unjudged_open %} \u00b7 {{ g.funnel.unjudged_open }} unjudged{% if g.funnel.top_unjudged_reason %} (mostly: {{ g.funnel.top_unjudged_reason }}){% endif %}{% endif %}</div>
        {% else %}<div class="gbig">\u2014</div><div class="gsub">unavailable</div>{% endif %}
      </div>

      <div class="gcard">
        <div class="glabel">Staff vs AI (7d)</div>
        {% if g.staff_ai %}
          {% if g.staff_ai.mismatch_7d %}
            <div class="gbig">\u26A0 {{ g.staff_ai.mismatch_7d }} mismatch</div>
          {% else %}
            <div class="gbig">\u2713 0 mismatch</div>
          {% endif %}
          <div class="gsub">{{ g.staff_ai.filed_7d }}/{{ g.staff_ai.total_7d }} filed{% if g.staff_ai.filed_pct is not none %} ({{ g.staff_ai.filed_pct }}%){% endif %} \u00b7 <a href="/portal/console?view=staff" style="color:var(--blue)">coach \u2192</a></div>
        {% else %}<div class="gbig">\u2014</div><div class="gsub">unavailable</div>{% endif %}
      </div>

      <div class="gcard">
        <div class="glabel">New leads (7d)</div>
        {% if g.leads %}
          <div class="gbig">{{ g.leads.new_7d }}</div>
          <div class="gsub">{{ g.leads.answered_7d }} answered \u00b7 {{ g.leads.unanswered_7d }} never reached \u00b7 <a href="/portal/console?view=leads" style="color:var(--blue)">open \u2192</a></div>
        {% else %}<div class="gbig">\u2014</div><div class="gsub">unavailable</div>{% endif %}
      </div>
