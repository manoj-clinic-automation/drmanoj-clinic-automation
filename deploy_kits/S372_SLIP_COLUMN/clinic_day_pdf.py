...
def day_data(con, date):
    ...
    return {"day": day, "lines": lines, "sections": sections, "people": _people(lines),
            "tender": _tender(day), "splits": sorted(sby.items()), "slips": slips}
...
_ROW = 13.0
# x positions of the seven columns: Slip | # | Patient | Clinic ID | Amount | Mode | Shift   (S372: Slip first)
_COL = {"slip": 68, "sn": 92, "pat": 100, "pat_w": 168, "cid": 315, "amt": 395, "mode": 462, "sh": 536}
...
class _Doc(object):
    ...
    def table_head(self, title, cont=False):
        ...
        yy = self.y - _ROW + 6.5
        p.text(_COL["slip"], yy, "Slip", 8.5, bold=True, align="r")
    ...
    def table_row(self, i, l, title, slip=""):
        if self.need(_ROW + 2):
            self.table_head(title, cont=True)
        p = self.pdf
        yy = self.y - _ROW + 6.5
        p.text(_COL["slip"], yy, _latin(slip) if slip else "", 9, align="r")      # S372: blank when no slip
...
def render_pdf(d, who=""):
    ...
    slips = d.get("slips") or {}
    for title, rows, key in d["sections"]:
        doc.table_head(title)
        tot = 0
        for i, l in enumerate(rows, 1):
            tot += l["amount_p"] or 0
            doc.table_row(i, l, title, slip=(slip_lookup.slip_no(slips, key, l["clinic_id"]) if slip_lookup else ""))
