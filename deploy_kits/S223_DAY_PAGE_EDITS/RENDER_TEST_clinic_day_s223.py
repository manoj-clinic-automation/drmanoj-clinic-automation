...
print("\n-- 7b  the owner's changes of 04-Sep ---------------------------------")
dh = cl.get("/finance/clinic/day/%s" % newest_date).get_data(as_text=True)
ck("the title is his clinic's name", "Dr. Manoj Agarwal Clinic" in dh
   and "Advanced Orthopaedic Surgery Centre" not in dh)
ck("'every entry' is gone from the heading", "every entry" not in dh)
ck("the day page has no footer paragraph", 'class="foot"' not in dh)
ck("the month page KEEPS its footer (it says how much history exists)",
   'class="foot"' in cl.get("/finance/clinic/day?m=%s" % month).get_data(as_text=True))
for sec in ("FREE REVISITS", "FREE / CONCESSION CASES"):
    if sec in dh:
        blk = dh.split(sec, 1)[1].split("</div>", 1)[0]
        ck("%s now carries Amount and Mode" % sec, ">Amount<" in blk and ">Mode<" in blk)
ck("every section uses the same six columns",
   dh.count(">Clinic ID<") == dh.count(">Amount<") == dh.count(">Mode<") == dh.count(">Shift<"))
ck("columns are fixed-width, so nothing sprawls", "table-layout:fixed" in dh)
# morning before evening, inside each section, in the delivered HTML
import re as _re
ok_order = True
for key, title in (("consult", "PAID CONSULTATIONS"), ("xray", "X-RAY")):
    rows = [l for l in lines if l["section"] == key]
    if not rows or not any((l["shift"] or "").lower() == "morning" for l in rows):
        continue
    blk = dh.split(title, 1)[1].split("</table>", 1)[0]
    seq = _re.findall(r">(Morning|Evening)<", blk)
    if seq != sorted(seq, key=lambda x: 0 if x == "Morning" else 1):
        ok_order = False
ck("morning entries come before evening, in every section", ok_order)

print("\n-- 8  the split-payment breakup --------------------------------------")
# The PARSER that recovers these legs is proven on the 79 real raw exports on the clinic PC
# (103 split bills, Rs 1,77,999). Those exports are not in this container, so what is tested HERE
# is the PAGE: a small fixture, built from clinic IDs that really are on the day under test, is
# inserted into clinic_day_tender and the block is read back out of the delivered HTML.
w = _db()
ids = [l["clinic_id"] for l in lines if l["clinic_id"]][:2]
if len(ids) < 2:
    ck("skipped: not enough clinic ids on this day to build the fixture", True)
else:
    w.execute("DELETE FROM clinic_day_tender WHERE business_date=?", (newest_date,))
    w.executemany("INSERT INTO clinic_day_tender (business_date, clinic_id, invoice_no, tender, "
                  "amount_p, source_file) VALUES (?,?,?,?,?,?)", [
        (newest_date, ids[0], "TESTINV1", "Wallet", 60000, "fixture"),
        (newest_date, ids[0], "TESTINV1", "Online Payment", 50000, "fixture"),
        (newest_date, ids[1], "TESTINV2", "Cash", 60000, "fixture"),
        (newest_date, ids[1], "TESTINV2", "Debit Card", 100000, "fixture")])
    w.commit()
    dh2 = cl.get("/finance/clinic/day/%s" % newest_date).get_data(as_text=True)
    ck("the split block appears when legs exist", "SPLIT PAYMENTS" in dh2)
    ck("each leg is named with its own amount",
       "Wallet ₹600" in dh2 and "Online Payment ₹500" in dh2 and "Debit Card ₹1,000" in dh2)
    ck("the legs of one bill are added up (600 + 500 = 1,100)", "1,100" in dh2)
    ck("the block totals the split bills (1,100 + 1,600 = 2,700)", "2,700" in dh2)
    ck("two bills are counted as two", "2 bills" in dh2)
    ck("F-185 still holds on the split block: no mobile-shaped number",
       not re.search(r"[6-9]\d{9}", dh2))
    w.execute("DELETE FROM clinic_day_tender WHERE source_file='fixture'"); w.commit()
    dh3 = cl.get("/finance/clinic/day/%s" % newest_date).get_data(as_text=True)
    ck("with no legs stored the block is absent, not empty", "SPLIT PAYMENTS" not in dh3)
