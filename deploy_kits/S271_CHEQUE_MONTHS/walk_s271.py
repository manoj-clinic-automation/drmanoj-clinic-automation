# -*- coding: utf-8 -*-
"""S271 LIVE-SHAPE WALK -- run on the box, against a COPY of the real database.
...
v2, after v1 failed on the box and taught two things (F-483):
  ...
  0  S271 ADDS ONE SECTION, 4b: a month that has NO cheque but DOES owe money on
     the cheque lane must be reachable from the register. Before S271 it was not,
     and that is the register's most useful state.
...
def con_count(con, month):
    try:
        return con.execute("SELECT COUNT(*) FROM purchase_cheque WHERE month=?",
                           (month,)).fetchone()[0]
    except Exception:
        return 0
...
def main(argv=None):
    ...
    h, ok = need200(c1, P + "/page/cheques", "the register")
    if ok:
        n_cheques = con_count(dbf1(), month)
        ck("this walk starts with no cheque logged for %s" % month, n_cheques == 0, n_cheques)
        ck("and the month is offered on the register anyway",
           "/page/cheques/%s" % month in h,
           "the strip must come from the purchase book, not from the cheques in it")
        hb, okb = need200(c0, P + "/page/cheques", "the register BEFORE S271")
        if okb:
            ck("before this kit it was not offered", "/page/cheques/%s" % month not in hb)
        h2, ok2 = need200(c1, "%s/page/cheques/%s" % (P, month), "that month's register")
        if ok2:
            ck("and opening it shows what the sheet still owes",
               "Against the sheet" in h2 and ("still owed" in h2 or "Nothing is owed" in h2))
