#!/usr/bin/env python3
"""probe_sheet2_ot.py -- kit S239_SHEET2_OT. Run by the installer BEFORE anything is replaced.
...
ONLY: every number the engine computes must be identical; the table must lose 'Leave days
counted' and gain Late minutes, Overtime (minutes, Rs) and Cover duty (days, Rs); every other cell must match.
   usage: probe_sheet2_ot.py <new salary_policy.py> <live salary_policy.py> <YYYY-MM>"""
...
if len(sys.argv) > 1 and sys.argv[1] == "--one":
    ...
    print("@@" + json.dumps({"staff": {s["name"]: num(s) for s in r["staff"]},
                             "ldc": bool(sec and "Leave days counted" in sec),
                             "ot": bool(sec and "Overtime (+)" in sec and "Cover duty (+)" in sec and "Late minutes" in sec),
                             "rows": rows}))
    sys.exit(0)
...
if not old["ldc"] or old["ot"]:
    fails.append("the LIVE table is not the shape this kit was built against")
if new["ldc"] or not new["ot"]:
    fails.append("the NEW table did not drop 'Leave days counted' / gain Overtime + Cover duty")
if len(old["rows"]) != len(new["rows"]):
    fails.append("row count differs")
print("  %-12s %9s %9s %9s %9s %9s" % ("staff", "late min", "OT min", "OT Rs", "cover d", "cover Rs"))
for a, b in zip(old["rows"], new["rows"]):
    if a[:5] + a[6:] != b[:6] + b[7:-4]:
        fails.append("%s: an existing cell changed" % a[0])
    print("  %-12s %9s %9s %9s %9s %9s" % tuple([b[0], b[6]] + b[-4:]))
if fails:
    print("PROBE RED:"); [print("   " + f) for f in fails]; sys.exit(1)
print("PROBE GREEN: display only -- every figure identical; Leave days counted gone; late minutes, OT + cover shown")
