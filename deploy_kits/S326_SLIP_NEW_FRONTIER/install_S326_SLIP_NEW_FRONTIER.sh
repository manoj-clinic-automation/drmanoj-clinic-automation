#!/bin/bash
# =============================================================================
#  install_S326_SLIP_NEW_FRONTIER.sh · kit S326_SLIP_NEW_FRONTIER (session 271, 19-Sep-2026, the first live morning)
#
#  Run by (one line on the VPS):
#    cd /root/deploy/repo && git pull --ff-only && bash /root/deploy/repo/deploy_kits/S326_SLIP_NEW_FRONTIER/install_S326_SLIP_NEW_FRONTIER.sh
#
#  THE OWNER, the same morning after S325: "is new patient also it is giving the same prompt purana
#  mariz". S325 took the ceiling from the patient master, which carries one stray row far ahead of
#  the series, so every genuine new ID sat under it. The ceiling is now the highest ID actually issued
#  BEFORE the day (Docterz day lines, visits, master rows seen). NEW is decided by the number; a known
#  name still autofills.
#
#  FILES:  /root/finance/slip_log.py  121174ae -> (TO below)  full-file replacement, backed up beside itself
#          /root/finance/finance_app.py must be 41e0ffb4 (S324) -- checked, not changed
#  DATA:   fix_s326.py -- every slip's NEW mark re-decided by the rule above. slip.is_new only.
#  Gates: SUMS + KIT_ID -> pins -> compile -> THE WALK ON THIS BOX (84 checks, scratch copy) -> backup ->
#  place -> fix -> restart clinic-finance -> health. Any red after placing: the file restored, restarted.
# =============================================================================
set -u
KIT="S326_SLIP_NEW_FRONTIER"
...
WALK="/tmp/s326_walk_$STAMP"
FROM=121174aeb311c6bd8255f9d34998dbb2
TO=36cb57671a730c868b2371625aa70810
...
if [ "$(m5 "$DEST")" = "$TO" ]; then say "-- ALREADY INSTALLED. Re-running the fix (idempotent):"; "$SPY" -B fix_s326.py "$DBF"; exit 0; fi
...
"$SPY" -m py_compile slip_log.py fix_s326.py || { say "!! [3/7] compile failed - nothing installed"; exit 1; }
...
WOUT="$( cd "$WALK/app" && FINANCE_DB="$WALK/walk.db" FINANCE_ALLOW_HEADER_AUTH=1 SLIP_NOW="$(date +%Y-%m-%dT%H:%M:%S)" \
         FINANCE_SSO_DIR="$POR" PETTY_UPLOAD_DIR="$WALK/uploads" timeout 170 "$SPY" -B "$KDIR/walk_s326.py" "$WALK/app" 2>&1 | tail -1 )"
echo "$WOUT" | grep -q "^WALK OK" || { say "!! [4/7] walk red: $WOUT - nothing installed"; rm -rf "$WALK"; exit 1; }
rm -rf "$WALK"
say "[4/7] $WOUT"
BAK="$DEST.bak_S326_${FROM:0:8}"
