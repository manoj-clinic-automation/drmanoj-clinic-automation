...
# -*- coding: utf-8 -*-
"""
walk_jaankari_s221_v2.py -- THE LIVE-SHAPE WALK for S221_JAANKARI, corrected.

v2, 03-Sep-2026. v1 went RED on the live box at install, on one check of
thirty-one: "with nothing to ask, every list is empty". The CODE was never in
question -- both pins came back exactly as predicted. The CHECK was wrong: to
test "nothing to ask" it copied the live database and pushed returns.act_from
forward, which empties the identity list and leaves the other two full, because
a live database legitimately holds open disputes and due spot counts (yours held
2 and 9). It asserted a property of my workspace copy, not of the code -- the
F-195 family, a check that passes for a reason other than the one it names.

v1 is left exactly as it was installed (the C-S216-1 precedent: correct going
forward, sealed kits untouched). This file supersedes it; run this one.
...
def main():
    ...
    c2.execute(SPOT_DDL)
    # v2 FIX -- found by this walk going RED on the live box at install, 03-Sep.
    # A LIVE database legitimately holds open disputes and due spot counts, so
    # pushing returns.act_from forward empties one list and leaves the other two
    # full. The scenario under test is "nothing to ask", and on a copy of live
    # that has to be MADE, not assumed. The first version asserted a property of
    # my own workspace copy rather than of the code -- the F-195 family.
    c2.execute("DELETE FROM identity_dispute")
    c2.execute("DELETE FROM stock_spot_check")
