...
/** ==========================================================================
 *  v2 (S196, Task #8): push the renewals picture to the clinic VPS daily, so
 *  the /finance/health page (and through it the portal tile) can show
 *  "N renewals inside 30 days · nearest: ...".
 *
 *  Independent of the email pacing above: the EMAIL nags only when something
 *  is close; the PUSH runs every day so the health card can also say "feed
 *  stale" the day this script stops running — a reminder system whose own
 *  death is silent is the thing this estate keeps refusing to build.
 *
 *  SETUP (once):
 *    1. On the VPS, generate the token (never in chat):  openssl rand -hex 24
 *       and set FINANCE_RENEWALS_TOKEN in clinic-finance.service (the
 *       installer note shows the two commands).
 *    2. Here: Project Settings -> Script properties -> add
 *       RENEWALS_PUSH_TOKEN = <the same value>
 *    3. Run pushRenewalsToVPS() once by hand — expect "pushed: N item(s)".
 *  The daily nagRenewals trigger then pushes automatically (it calls this at
 *  the end of every run, quiet or not).
 *  ========================================================================== */

var PUSH_URL         = 'https://followup.dr-manoj.in/finance/api/renewals-push';
var PUSH_WINDOW_DAYS = 35;    // send a little beyond the card's 30-day window
var PUSH_OVERDUE_KEEP= 45;    // and everything overdue the nag still tracks

function pushRenewalsToVPS() {
  var now = new Date(), items = [];
  RENEWALS.forEach(function (r) {
    if (!r.dateISO || r.dateISO.indexOf('TODO') === 0) return;
    var d = new Date(r.dateISO + 'T09:00:00');
    var days = Math.floor((d - now) / 864e5);
    if (days > PUSH_WINDOW_DAYS || days < -PUSH_OVERDUE_KEEP) return;
    items.push({ vendor: r.vendor, dateISO: r.dateISO, note: r.note || '' });
  });
  var token = PropertiesService.getScriptProperties()
                .getProperty('RENEWALS_PUSH_TOKEN');
  if (!token) return 'NOT PUSHED: RENEWALS_PUSH_TOKEN script property not set';
  try {
    var resp = UrlFetchApp.fetch(PUSH_URL, {
      method: 'post',
      contentType: 'application/json',
      headers: { 'X-Finance-Renewals': token },
      payload: JSON.stringify({ window_days: 30, items: items }),
      muteHttpExceptions: true
    });
    var code = resp.getResponseCode();
    PropertiesService.getScriptProperties().setProperty(
      'renewals_last_push', code + ' @ ' + now.toISOString());
    return code === 200 ? 'pushed: ' + items.length + ' item(s)'
...
/* v2: the trigger target. The email logic is untouched in nagRenewalsCore();
   the push happens on EVERY daily run, quiet days included. */
function nagRenewals() {
  var out = nagRenewalsCore();
  var pushed = pushRenewalsToVPS();
  return out + ' | ' + pushed;
}
