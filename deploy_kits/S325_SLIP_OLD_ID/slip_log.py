#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""slip_log.py -- S324 (session 271, 19-Sep-2026), S325 fix the same morning: an old ID the master lacks is NOT new. The OPD & X-ray/Proc slip tile, and the night
...
def is_new_id(con, clinic_id):
    """S325 -- the owner's rule, 19-Sep-2026: "system knows till which number the clinic id exist".
    NEW means ABOVE the highest ID the master knows. An ID at or below it is an old patient even when
    the master has no row for him (2681 was read as NAYA on the first live morning)."""
    n = _int(clinic_id)
    hi = highest_id(con)
...
def highest_id(con):
    try:
        r = con.execute("SELECT MAX(CAST(clinic_id AS INTEGER)) m FROM patient_ref "
                        "WHERE clinic_id GLOB '[0-9]*'").fetchone()
        return int(r["m"] or 0)
    except Exception:                                    # noqa: BLE001
        return 0
...
def api_patient():
    ...
    p = patient(con, cid)
    if p:
        return jsonify(ok=True, known=True, name=p[0], last4=p[1])
    hi = highest_id(con)
    n = _int(cid)
    far = bool(hi and n is not None and n > hi + ID_JUMP)
    return jsonify(ok=True, known=False, far=far, highest=hi, old=not is_new_id(con, cid))
...
def _tile_html(con, u):
    ...
    sec_room = _sec("room", "Room: Paid / Done", "%d baaki" % (len(open_old) + len(open_today)), room_body)
    ...
    for series, label in SERIES:
        ...
        bl.append('<p><b>%s</b>: book %d–%d · agla %s</p>' % (label, b["first_no"], b["last_no"], nx_ or "—"))
        bl.append('<details><summary class="sm">badlein</summary>%s</details>' % _book_form(con, series, "Naya book / sudhaar"))
    if "checker" in _roles(u):
        bl.append('<p><a href="/finance/slips/report">Report (English)</a></p>')
    sec_book = _sec("book", "Book", "", "".join(bl))
  ...
  [].forEach.call(document.querySelectorAll('form.slipf'),function(f){
    ...
    function look(){var v=(cid.value||'').trim();who.textContent='';who.className='who';
      ...
      .then(function(r){return r.json();}).then(function(j){
        if(!j.ok)return;
        if(j.known){who.textContent=j.name+(j.last4?'  · ••••'+j.last4:'');who.className='who known';}
...
.rrow.old .rl{color:var(--warn)}.rl{font-size:15px}.rb{display:flex;gap:4px;flex:0 0 auto}
