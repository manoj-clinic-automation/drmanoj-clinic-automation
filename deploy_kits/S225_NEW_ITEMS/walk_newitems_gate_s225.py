...
# -*- coding: utf-8 -*-
"""
walk_newitems_gate_s225.py -- THE LIVE-SHAPE WALK for rev 10 (the new-items log), run on a COPY in a probe folder (F-321): the REAL finance_app.py,
...
On the box (after the copy, BEFORE the restart):
    FIN_DIR=/root/finance /root/wa/venv/bin/python3 -B /root/finance/walk_newitems_gate_s225.py
"""
import os
import shutil
import sqlite3
...
ck("purchase_app beside finance_app is rev 10 (…, new-items log)", hasattr(PA, "page_staff") and hasattr(PA, "_rematch_if_changed") and hasattr(PA, "_log_new_items") and hasattr(PA, "_new_items_card"))
