...
# ---------------------------------------------------------------- S260 (C.3)
# ONE SETTINGS FILE PER MACHINE.  D:\Downloads\margsync\_config\machine.conf
# Plain KEY=VALUE lines, # starts a comment. Every value below is the one this
# script has always used, so a missing file, a missing key or a blank value
# changes nothing at all. The file is NOT in the repository (F-185): it says
# where THIS machine is, and that is not the code's business.
CONF_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                         "_config", "machine.conf")


def conf(key, default, path=None):
    """The value for `key`, or `default` if the file or the key is not there."""
    try:
        with open(path or CONF_FILE, "r", encoding="utf-8-sig", errors="replace") as fh:
            for line in fh:
                line = line.strip()
...
MEDICAL_HOST = conf("MEDICAL_HOST", "100.119.151.40")
MEDICAL_SHARE = conf("MEDICAL_SHARE", "DDrive")
SERVER_BASE = conf("SERVER_BASE", "https://followup.dr-manoj.in").rstrip("/")
...
DEF_URL       = SERVER_BASE + "/finance/api/pipeline-status"
DEF_TOKEN     = conf("TOKEN_LOCAL", r"D:\Downloads\margsync\SendToClinic\token.txt")
DEF_TOKEN_UNC = r"\\%s\%s\SendToClinic\token.txt" % (MEDICAL_HOST, MEDICAL_SHARE)
