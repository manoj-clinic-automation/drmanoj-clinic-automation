...
# -*- coding: utf-8 -*-
"""
casepack_portal.py — Surgical Case Pack, VPS edition · v2 (S215, CP-1 · D359) · S385: /portal/casepack/trdict
=============================================================================
...
def register(app, guard, get_user):
    ...
    # --- S385 (24-Sep-2026): his own Hindi spellings, remembered ------------------------------------------------
    # The owner corrected a name and an address in one place of a consent and the print kept the machine spelling
    # everywhere else. The page now corrects every place at once and remembers the spelling HERE, so the next
    # consent spells it right by itself. One small JSON file beside the case ledger (PHI stays in CASEPACK_DIR,
    # never in git); this module is its only writer; nothing is ever deleted from it, a newer spelling replaces.
    TR_DICT = os.path.join(CASEPACK_DIR, "tr_dict.json")

    def _tr_read():
        try:
            with open(TR_DICT, "r", encoding="utf-8") as f:
                d = json.load(f)
            return d if isinstance(d, dict) else {}
        except Exception:
            return {}

    @app.route("/portal/casepack/trdict")
    @guard
    def casepack_trdict():
        return jsonify({"ok": True, "dict": _tr_read()})

    @app.route("/portal/casepack/trdict", methods=["POST"])
    @guard
    def casepack_trdict_save():
        try:
            pairs = (request.get_json(silent=True) or {}).get("pairs") or {}
            if not isinstance(pairs, dict):
                return jsonify({"ok": False, "error": "pairs must be an object"}), 400
            d = _tr_read(); n = 0
            for k, v in list(pairs.items())[:40]:
                k = " ".join(str(k or "").lower().split())[:80]; v = " ".join(str(v or "").split())[:120]
                if not k or not v or not re.search(r"[a-z]", k) or not re.search(u"[\u0900-\u097F]", v):
                    continue
                d[k] = v; n += 1
            if n:
                tmp = TR_DICT + ".tmp"
                ...
                os.replace(tmp, TR_DICT)
            return jsonify({"ok": True, "saved": n, "size": len(d)})
        except Exception as e:
            return jsonify({"ok": False, "error": str(e)}), 500
