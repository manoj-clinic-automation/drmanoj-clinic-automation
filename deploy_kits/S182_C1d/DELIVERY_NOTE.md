...
---
## C1d addendum (supersedes C1c — F-88)
The C1c live red was the system WORKING: the smoke ran against a copy of the real store and hit
the bank arbiter. D1 resolved to 14-Aug (last medical day +1); the live `upi_statement` table
holds a real clinic settlement for that date (the MPR push stores all three units since S179);
`reconcile_upi` opened a `upi_vs_statement` exception; approve correctly refused without
acknowledgment — a path the synthetic db could never exercise because it holds no statements.
Roles were verified IDENTICAL live vs seeded (owner ran the read-only probe), eliminating the
placeholder hypothesis by evidence, not argument.
C1d changes (smoke only; app routes untouched except none):
 * the clinic smoke PLANTS a clinic statement for D1 (INSERT OR IGNORE — live keeps its real one),
   making the mismatch path deterministic in every environment;
 * approve is tested BOTH ways: refuse without `acknowledge_upi` (409), land with it (200);
 * the three formerly-red checks now print the status and body they actually saw (a check must
   say what it saw), so any future red self-diagnoses.
Offline gate: 228/238 — the same 10 seeding artefacts, zero new failures, +1 new check.
CANDIDATE RULE for the register: a smoke suite must be run against a live-shaped store — bank
statements, real history — before its first live gate; a synthetic db proves logic, not life.
