#!/bin/bash
# =============================================================================
#  install_c1d.sh · kit S182_C1d — clinic daily entry (C1 slice 1)
...
cd "$KIT_DIR" \
&& md5sum -c SUMS.md5 \
&& [ "$(awk 'NR==1{print $1}' KIT_ID.txt)" = "S182_C1d" ] \
...
&& rm -f .S182_touched \
&& echo "" && echo "S182_C1d INSTALLED — smoke green, migration in, service restarted" \
