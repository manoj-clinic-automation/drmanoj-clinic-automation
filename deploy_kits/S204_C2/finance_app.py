...
# S195: corrections BEFORE this date are not turned into work. Amir applied the
# older ones already, and a list that opens with three months of dead entries is
# a list nobody reads. The bank history behind this date is still loaded, still
# stored and still visible in the reports -- it is simply not made into a task.
# Move the date when the answer to "would we actually chase this?" changes.
CORRECTION_FROM = os.environ.get("FINANCE_CORRECTION_FROM", "2026-08-01")
...
# S196 (Task #8): the personal-Gmail renewals feed. Its OWN token, same
# reasoning as MARG_TOKEN — the personal GAS project must never hold a secret
# that opens any other route, and rotating one never breaks the others.
# Absent => the push surface does not exist at all (fail closed, F-84).
# NEVER pasted in chat: generated on the box, set in the service unit + the
# GAS Script Property.
RENEWALS_TOKEN = os.environ.get("FINANCE_RENEWALS_TOKEN", "")
RENEWALS_STATE = os.environ.get("FINANCE_RENEWALS_STATE",
                                "/root/finance/renewals_state.json")
...
def _gate():
    ...
    # S203: pipeline-status was ADDED AT S202 AND NEVER ADDED HERE, so this
    # gate refused it 401 before the route ran -- for every pull since, with
    # the only evidence going to a console the hidden launcher discarded. The
    # route's own token check was dead code. Same token, same header, and it
    # can still touch nothing but its own append-only table.
    if MARG_TOKEN and p in ("/finance/api/marg-push",
                            "/finance/api/pipeline-status") \
            and request.headers.get("X-Finance-Marg") == MARG_TOKEN:
        return None
    # S196: the renewals token opens exactly ONE path — the staged renewals
    # store. No identity, no role, no other route; the handler re-checks it.
...
def html_escape(s):
    """Anything a person typed goes through here before it reaches a page --
    notes and names are free text, and free text has no business closing a tag."""
...
def marg_net_sql(alias="sale_item"):
    """The SIGNED net of Marg rows, as a SQL expression.

    finance_ingest stores a credit note as a MAGNITUDE plus a "_return" service,
    because sale_item carries a non-negative constraint. A plain SUM(amount_p)
    therefore ADDS a refund instead of subtracting it, and overstates any day
    that has one by twice the note.

    That is not hypothetical. On 18-08-2026 the day held one credit note of
    1,640: the true net of the loaded rows was 20,599, and the maker's mirror
    displayed 23,879 -- out by exactly 2 x 1,640, and close enough to the
    figure under dispute to send a real investigation down the wrong road for
    an hour. Two readers had two different sums of the same rows.

    So: one expression, every reader. The same argument as
    MARG_VARIANCE_THRESHOLD_P above -- two copies of a rule is how two screens
    come to disagree about the same day.
    """
    a = alias
    return ("COALESCE(SUM(CASE WHEN COALESCE({a}.service,'') LIKE '%return%' "
            "THEN -{a}.amount_p ELSE {a}.amount_p END),0)").format(a=a)
...
def advance_ceiling_p(con):
    ...
    owns). Defaults: base Rs 20,000, pct 50 -- the general staff rule.
    S204: Darpan's 75% exception is RETIRED at the owner's ruling ("darpan
    was finally in same 50% rule"), and the settings rows advance.base_p /
    advance.pct were written on 27-Aug-2026 to say so. The coded default
    was 75 until this kit, so a rebuilt or restored database would have
    resurrected Rs 15,000 with nothing said -- an exception must live in a
    settings row, never in a fallback. Both are settings, so a salary
    revision stays one settings row, not a code change."""
    try:
        base = int(setting(con, "advance.base_p", "2000000") or 2000000)
        pct = int(setting(con, "advance.pct", "50") or 50)
    except (TypeError, ValueError):
        base, pct = 2000000, 50
...
def api_my_day_summary():
    ...
    # S195 A3+++: his own accuracy, back to him. The bank statement is the only
    # independent witness to a cash/UPI split, so this is feedback he can act on
    # -- and it tells him plainly that the entries are being checked.
    try:
        acc = _maker_accuracy(con, 30)
    except Exception:
        acc = None
    return jsonify(ok=True, to_file=len(owed), days=owed,
                   today=(te["status"] if te else None), accuracy=acc)
...
def api_save_day():
    ...
    warnings = []
    # A1 (S195): does the typed total agree with what Marg printed for this day?
    # Marg is the billing machine; the day sheet is typed from it. When the two
    # disagree, either the entry is wrong or the report is stale -- and it is far
    # cheaper to say so now than to chase it a week later (the 18-Aug lesson).
    _marg_net_p, _marg_src = _marg_total_for_date(con, iso)
    _marg_gap_p = None
    if _marg_net_p is not None and _marg_net_p != total_p:
        _marg_gap_p = total_p - _marg_net_p
        warnings.append(
            "%s says this day is %s, but %s has been entered — a difference of "
            "%s. Check the sale report before filing."
            % (_marg_src, rupees(_marg_net_p), rupees(total_p),
               rupees(abs(_marg_gap_p))))

    # A3+++ (S195): the split is typed off the POS screen, and the bank is the
    # only independent witness to it. Telling the person who types it how his
    # last month scored -- in his own save, not in a report he never opens --
    # is the whole feedback loop. Silent when every checked day matched.
    try:
        _acc = _maker_accuracy(con, 30)
        if _acc["differing"]:
            warnings.append(
                "Cash/UPI check: %d of the %d days the bank could verify matched "
                "your split. %d still differ (%s)%s — they are on the correction "
                "list."
                % (_acc["matched"], _acc["checked"], _acc["differing"],
                   ", ".join(i["date"] for i in _acc["items"][:3]),
                   "" if _acc["differing"] <= 3 else " and more"))
    except Exception:                                              # noqa: BLE001
        pass
    ...
    try:
        ...
        # A1 (S195): a typed total that disagrees with Marg is recorded, so the
        # checker sees it at approval time and not three days later.
        if _marg_gap_p:
            con.execute("DELETE FROM data_flag WHERE unit=? AND business_date=? "
                        "AND code='TOTAL_VS_MARG'", (UNIT, iso))
            con.execute("INSERT INTO data_flag (unit, business_date, day_entry_id, "
                        "code, severity, detail) VALUES (?,?,?,'TOTAL_VS_MARG','high',?)",
                        (UNIT, iso, eid,
                         "entered %s but the Marg report says %s (difference %s)"
                         % (rupees(total_p), rupees(_marg_net_p),
                            rupees(abs(_marg_gap_p)))))
...
def api_workbench(ym):
    ...
    rows = con.execute(
        ...
        "       (SELECT " + marg_net_sql("si") + " FROM sale_item si"
...
def api_marg_push():
    ...
    try:
        ...
        for d in days:
            ...
            # AF-2 (auditor, 24-Aug): _marg_total_for_date() has looked for
            # 'business_date' and 'net_p' in this payload since S195 and this
            # writer has never put them here, so the A1 save-time
            # "does your total match Marg?" warning -- and its high-severity
            # TOTAL_VS_MARG flag -- has never fired once. Both values are in
            # `d` already. Purely additive: the apply path reads only date /
            # expect / lines_csv / items_csv and ignores everything else.
...
# ------------------------------------------------ A3+ (S195): the WRONG SPLIT
def _upi_misclass(con, days=60, from_iso=None):
    """Days where the bank's UPI total and the declared UPI disagree, WITH the
    direction — which is the part that matters.

    Marg labels every bill .CASH, so the split is not captured at billing: it is
    read off the POS screen in the morning and typed in. That is careful work,
    and this is what tells us when it went the wrong way.

        bank > declared -> UPI money was booked as CASH.
                           The drawer is OVERSTATED by the difference:
                           the notes were never in it.
        bank < declared -> CASH was booked as UPI.
                           The drawer is UNDERSTATED: the notes are there but
                           the books do not expect them.

    Cash is derived (total - UPI), so every rupee of mis-split moves the drawer
    one for one. Returns newest first.
    """
    since = (dt.date.today() - dt.timedelta(days=int(days))).isoformat()
    if from_iso:
        since = max(since, from_iso)
    out = []
    for r in con.execute(
            "SELECT l.business_date d, COALESCE(l.upi_in_p,0) declared, "
            "       COALESCE(s.parsed_total_p,0) bank "
            "FROM v_cash_ledger l "
            "JOIN upi_statement s ON s.unit=l.unit AND s.statement_date=l.business_date "
            "WHERE l.unit=? AND l.business_date>=? "
            "  AND COALESCE(s.parsed_total_p,0) <> COALESCE(l.upi_in_p,0) "
            "ORDER BY l.business_date DESC", (UNIT, since)):
        diff = int(r["bank"]) - int(r["declared"])          # + => UPI booked as cash
        out.append(dict(
            date=r["d"], declared=rupees(int(r["declared"])), bank=rupees(int(r["bank"])),
            diff_p=diff, diff=rupees(abs(diff)),
            direction=("upi_as_cash" if diff > 0 else "cash_as_upi"),
            says=("UPI booked as cash — drawer overstated" if diff > 0
                  else "cash booked as UPI — drawer understated")))
    return out



# ------------------------------------- A3++ (S195): the correction worklist
def _marg_worklist(con, days=90):
    """Turn each disagreeing day into something Amir can DO in Marg.

    The bank total is ground truth for UPI, but it is a TOTAL — it cannot say
    which bill was mislabelled. So this reports the amount that must move and,
    where a single bill matches that amount exactly, names it as a candidate.

    Single-bill matches only, on purpose. A subset of several bills will match
    almost any target by coincidence (proved while chasing the 1,297 on 18-Aug:
    a five-bill combination summed to it and meant nothing). Naming a wrong bill
    is worse than naming none.
    """
    out = []
    for row in _upi_misclass(con, days, from_iso=CORRECTION_FROM):
        iso = row["date"]
        e = con.execute("SELECT id FROM day_entry WHERE unit=? AND business_date=?",
                        (UNIT, iso)).fetchone()
        want_from = "cash" if row["diff_p"] > 0 else "upi"   # the mode to move OUT of
        want_to = "upi" if row["diff_p"] > 0 else "cash"
        target = abs(row["diff_p"])
        candidates = []
        if e:
            for b in con.execute(
                    "SELECT source_ref, amount_p, mode FROM sale_item "
                    "WHERE day_entry_id=? AND COALESCE(mode,'')=? AND amount_p=? "
                    "ORDER BY source_ref", (e["id"], want_from, target)):
                candidates.append(dict(bill=b["source_ref"],
                                       amount=rupees(int(b["amount_p"])),
                                       now=b["mode"]))
        out.append(dict(
            date=iso, declared_upi=row["declared"], bank_upi=row["bank"],
            move=rupees(target), from_mode=want_from, to_mode=want_to,
            direction=row["direction"], says=row["says"],
            change=("%s \u2192 %s" % (want_from.upper(), want_to.upper())),
            action=("In Marg, change %s of bills on %s from %s to %s."
                    % (rupees(target), iso, want_from.upper(), want_to.upper())),
            candidates=candidates,
            candidate_note=("one bill matches this amount exactly"
                            if len(candidates) == 1 else
                            ("%d bills match this amount — check which" % len(candidates)
                             if candidates else
                             "no single bill matches; the difference is spread "
                             "across more than one bill"))))
    return out


@app.route("/finance/api/marg-worklist")
def api_marg_worklist():
    u, err = require("checker")
    if err:
        return err
    try:
        days = max(1, min(400, int(request.args.get("days", 90))))
    except ValueError:
        days = 90
    rows = _marg_worklist(db(), days)
    return jsonify(ok=True, days=days, count=len(rows), rows=rows,
                   after="Correcting a bill in Marg does NOT reach the books. "
                         "Re-run that day's Bill Wise Sales export and send it, "
                         "then apply it — only then do the books change.")



# ===================================================== A3+++ (S195)
#  THE CORRECTION CHECKLIST — from "something is wrong" to work someone does.
#
#  Each disagreeing day becomes a tracked item Amir can be given, Dr Manoj can
#  schedule, and the SYSTEM closes by itself: when a corrected day is re-exported
#  and applied, it stops disagreeing, and the item resolves. Nobody has to
#  remember to tick it — which is the only kind of checklist that stays honest.
# =====================================================

def _corr_tbl(con):
    con.execute(
        "CREATE TABLE IF NOT EXISTS marg_correction ("
        " id INTEGER PRIMARY KEY, unit TEXT NOT NULL, business_date TEXT NOT NULL,"
        " diff_p INTEGER NOT NULL, direction TEXT NOT NULL,"
        " status TEXT NOT NULL DEFAULT 'open',"      # open | sent | corrected | resolved
        " due_date TEXT, assigned_to TEXT, note TEXT,"
        " created_at TEXT, updated_at TEXT, resolved_at TEXT,"
        " UNIQUE(unit, business_date))")


def _correction_sync(con, days=90):
    """Reconcile the checklist against what the books and the bank say NOW.

    New disagreement -> a new open item. A disagreement that has gone away ->
    resolved automatically, because the only way it goes away is a corrected
    day being re-exported and applied. An item whose amount CHANGED is updated
    and re-opened: a partial fix is not a fix.
    """
    _corr_tbl(con)
    now = now_iso()
    live = {r["date"]: r for r in _upi_misclass(con, days, from_iso=CORRECTION_FROM)}
    known = {r["business_date"]: r for r in con.execute(
        "SELECT * FROM marg_correction WHERE unit=?", (UNIT,))}

    for iso, r in live.items():
        k = known.get(iso)
        if not k:
            con.execute(
                "INSERT INTO marg_correction (unit, business_date, diff_p, direction,"
                " status, created_at, updated_at) VALUES (?,?,?,?, 'open', ?, ?)",
                (UNIT, iso, r["diff_p"], r["direction"], now, now))
        elif int(k["diff_p"]) != r["diff_p"] or k["status"] == "resolved":
            con.execute(
                "UPDATE marg_correction SET diff_p=?, direction=?, status='open',"
                " resolved_at=NULL, updated_at=? WHERE id=?",
                (r["diff_p"], r["direction"], now, k["id"]))

    for iso, k in known.items():
        if iso not in live and k["status"] != "resolved":
            con.execute("UPDATE marg_correction SET status='resolved', resolved_at=?,"
                        " updated_at=? WHERE id=?", (now, now, k["id"]))
    con.commit()


def _correction_rows(con, days=90, include_resolved=False):
    """The checklist, joined to the live worklist detail (action + candidates)."""
    _correction_sync(con, days)
    detail = {d["date"]: d for d in _marg_worklist(con, days)}
    q = ("SELECT * FROM marg_correction WHERE unit=?"
         + ("" if include_resolved else " AND status<>'resolved'")
         + " ORDER BY business_date DESC")
    out = []
    for r in con.execute(q, (UNIT,)):
        d = detail.get(r["business_date"], {})
        if not d:
            # A finished item is no longer in the live worklist -- it agrees now.
            # Rebuild what it WAS from what we stored, so the record still reads
            # as a piece of work and not as a blank row.
            _fr = "CASH" if r["direction"] == "upi_as_cash" else "UPI"
            _to = "UPI" if r["direction"] == "upi_as_cash" else "CASH"
            d = {"change": "%s \u2192 %s" % (_fr, _to),
                 "action": "In Marg, change %s of bills on %s from %s to %s."
                           % (rupees(abs(int(r["diff_p"]))), r["business_date"], _fr, _to),
                 "says": ("UPI booked as cash" if r["direction"] == "upi_as_cash"
                          else "cash booked as UPI"),
                 "candidate_note": ""}
        out.append(dict(
            date=r["business_date"], status=r["status"],
            due=r["due_date"] or "", assigned=r["assigned_to"] or "",
            note=r["note"] or "", diff=rupees(abs(int(r["diff_p"]))),
            diff_p=int(r["diff_p"]),
            direction=r["direction"],
            action=d.get("action", ""), says=d.get("says", ""),
            change=d.get("change", ""),
            declared_upi=d.get("declared_upi", ""), bank_upi=d.get("bank_upi", ""),
            candidates=", ".join("%s (%s)" % (c["bill"], c["amount"])
                                 for c in d.get("candidates", [])),
            candidate_note=d.get("candidate_note", ""),
            resolved_at=r["resolved_at"] or ""))
    return out


@app.route("/finance/api/marg-corrections")
def api_marg_corrections():
    u, err = require("checker")
    if err:
        return err
    inc = request.args.get("all") == "1"
    rows = _correction_rows(db(), 90, include_resolved=inc)
    return jsonify(ok=True, count=len(rows), rows=rows,
                   open=sum(1 for r in rows if r["status"] != "resolved"))


@app.route("/finance/api/marg-corrections", methods=["POST"])
def api_marg_corrections_set():
    """Set status / due date / who it went to / a note, for one day."""
    u, err = require("checker")
    if err:
        return err
    p = request.form if request.form else (request.get_json(silent=True) or {})
    iso = (p.get("business_date") or "").strip()
    try:
        iso = parse_iso_date(iso).isoformat()
    except ValueError:
        return jsonify(ok=False, error="bad_date"), 400
    con = db()
    _corr_tbl(con)
    ...
    if st:
        if st not in ("open", "sent", "corrected", "resolved"):
            return jsonify(ok=False, error="bad_status"), 400
        sets.append("status=?"); args.append(st)
        if st == "resolved":
            sets.append("resolved_at=?"); args.append(now_iso())
    for field, col in (("due_date", "due_date"), ("assigned_to", "assigned_to"),
                       ("note", "note")):
        if field in p:
            sets.append(col + "=?"); args.append((p.get(field) or "").strip()[:200])
    if not sets:
        return jsonify(ok=False, error="nothing_to_set"), 400
    sets.append("updated_at=?"); args.append(now_iso())
    args += [UNIT, iso]
    n = con.execute("UPDATE marg_correction SET " + ", ".join(sets)
                    + " WHERE unit=? AND business_date=?", args).rowcount
    if not n:
        return jsonify(ok=False, error="no_such_item"), 404
    audit(con, "marg_correction", None, "checklist_set",
          after={"date": iso, "fields": {k: p.get(k) for k in
                                         ("status", "due_date", "assigned_to", "note")
                                         if k in p}}, who=u["user"])
    con.commit()
    if request.form:
        return redirect("/finance/marg-worklist", code=302)
    return jsonify(ok=True)


@app.route("/finance/api/marg-corrections/schedule", methods=["POST"])
def api_marg_corrections_schedule():
    """Set the deadline and the owner for the whole open list in one go.

    This is the 'I set a schedule for him' half: the doctor decides once when
    the outstanding corrections are due and who has them, rather than typing the
    same date onto every row."""
    u, err = require("checker")
    if err:
        return err
    p = request.form if request.form else (request.get_json(silent=True) or {})
    con = db()
    _correction_sync(con, 90)
    due = (p.get("due_date") or "").strip()
    if due:
        try:
            due = parse_iso_date(due).isoformat()
        except ValueError:
            return jsonify(ok=False, error="bad_date"), 400
    who = (p.get("assigned_to") or "").strip()[:60]
    if not due and not who:
        return jsonify(ok=False, error="nothing_to_set"), 400
    sets, args = [], []
    if due:
        sets.append("due_date=?"); args.append(due)
    if who:
        sets.append("assigned_to=?"); args.append(who)
        sets.append("status=CASE WHEN status='open' THEN 'sent' ELSE status END")
    sets.append("updated_at=?"); args.append(now_iso())
    args.append(UNIT)
    n = con.execute("UPDATE marg_correction SET " + ", ".join(sets)
                    + " WHERE unit=? AND status<>'resolved'", args).rowcount
    audit(con, "marg_correction", None, "checklist_schedule",
          after={"due_date": due, "assigned_to": who, "items": n}, who=u["user"])
    con.commit()
    if request.form:
        return redirect("/finance/marg-worklist", code=302)
...
@app.route("/finance/api/marg-corrections.csv")
def api_marg_corrections_csv():
    """The list to hand to whoever fixes the bills in Marg."""
    u, err = require("checker")
    if err:
        return err
    rows = _correction_rows(db(), 90, include_resolved=request.args.get("all") == "1")
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(["Date", "What to change in Marg", "Amount", "Move from", "Move to",
                "Likely bill(s)", "Books say UPI", "Bank says UPI",
                "Status", "Due", "Given to", "Note"])
    for r in rows:
        w.writerow([r["date"], r["action"], r["diff"],
                    ("CASH" if r["direction"] == "upi_as_cash" else "UPI"),
                    ("UPI" if r["direction"] == "upi_as_cash" else "CASH"),
                    r["candidates"] or r["candidate_note"],
                    r["declared_upi"], r["bank_upi"],
                    r["status"], r["due"], r["assigned"], r["note"]])
    w.writerow([])
    w.writerow(["NOTE", "Correcting a bill in Marg does not change the books. "
                        "Re-run that day's BILL WISE SALES export (Detail, With "
                        "Item Deta. = Yes), send it, and apply it."])
    out = app.response_class(buf.getvalue(), mimetype="text/csv")
    out.headers["Content-Disposition"] = \
        'attachment; filename="marg_corrections_%s.csv"' % dt.date.today().isoformat()
    return out


def _maker_accuracy(con, days=30):
    """Darpan's own scoreboard: of the days the bank can check, how many of his
    cash/UPI splits matched. Feedback he can act on, and proof the entries are
    being looked at."""
    since = (dt.date.today() - dt.timedelta(days=int(days))).isoformat()
    checked = con.execute(
        "SELECT COUNT(*) c FROM v_cash_ledger l JOIN upi_statement s "
        "ON s.unit=l.unit AND s.statement_date=l.business_date "
        "WHERE l.unit=? AND l.business_date>=?", (UNIT, since)).fetchone()["c"]
    off = [dict(date=r["date"], diff=r["diff"], says=r["says"])
           for r in _upi_misclass(con, days)]
    return dict(days=days, checked=int(checked or 0), matched=max(0, int(checked or 0) - len(off)),
                differing=len(off), items=off[:5])

_SHARE_JS = """
function copyWA(){
  var t = document.getElementById('shareTxt');
  var b = document.getElementById('waBtn');
  var h = document.getElementById('waHint');
  function done(){
    b.textContent = 'COPIED \\u2713  now paste it in WhatsApp';
    b.className = 'wa done';
    h.textContent = 'The list is on your clipboard. Open your chat with Amir and paste.';
    setTimeout(function(){ b.textContent = 'Copy for WhatsApp'; b.className = 'wa'; }, 6000);
  }
  function fallback(){
    ...
    h.textContent = 'Could not copy by itself \\u2014 the text is shown below. '
                  + 'Press and hold it, Select all, Copy.';
  }
...
def _ddmmyyyy(iso):
    """Dates go to people in the form people here write them."""
    try:
...
def _corrections_text(rows, when=None):
    """The checklist as a message a person can paste into WhatsApp.

    Plain text, no formatting, no links -- it has to survive being pasted into
    a chat and read on a phone. Bill numbers appear ONLY where exactly one bill
    carries the amount; where several do, or none, it says so. A wrong bill
    number sends someone to edit the wrong sale, which is worse than sending
    them to look.
    """
    live = [r for r in rows if r["status"] != "resolved"]
    if not live:
        return ("Marg correction list — nothing to correct.\n\n"
                "Every day the bank could check agrees with the books.")
    out = ["Marg correction list  (%s)" % (when or dt.date.today().strftime("%d-%m-%Y")),
           "",
           "%d day%s to correct in Marg:" % (len(live), "" if len(live) == 1 else "s"),
           ""]
    for i, r in enumerate(live, 1):
        frm = "CASH" if r["direction"] == "upi_as_cash" else "UPI"
        to = "UPI" if r["direction"] == "upi_as_cash" else "CASH"
        out.append("%d) %s  —  move Rs %s from %s to %s"
                   % (i, _ddmmyyyy(r["date"]), r["diff"], frm, to))
        if r["candidates"] and "one bill matches" in (r["candidate_note"] or ""):
            out.append("    bill: %s" % r["candidates"])
        elif r["candidate_note"]:
            out.append("    (%s)" % r["candidate_note"])
        if r["due"]:
            out.append("    please correct by %s" % _ddmmyyyy(r["due"]))
        out.append("")
    out += ["After changing a bill, that day has to be exported from Marg again",
            "(BILL WISE SALES -> Report Type: Detail -> With Item Deta.: Yes)",
            "and sent, otherwise the books do not change.",
            "",
            "Please reply when each one is done."]
    return "\n".join(out)


# ------------------------------------------------- A4 (S195): the MONTH check
def _marg_month_compare(con, ym):
    """Compare a whole month's books against a whole month's Marg, and name the
    days each side is missing.

    A1 compares one day against its own Marg report. By construction it cannot
    see a day that never arrived at all -- there is nothing to compare it to.
    That is the hole this closes.

    The Marg side is rebuilt from `sale_item` rows that came in on a
    `marg_export` batch, with returns SIGNED back to negative: finance_ingest
    stores a credit note as a magnitude plus a "_return" service, because
    sale_item forbids a negative amount. Summing without that turns a refund
    into a sale, which is how a wrong warning gets made -- and a wrong warning
    is worse than none.

    Totals are compared only over days BOTH sides have, so the figure means
    something. The days only one side has are reported separately, by name.
    """
    days = [r["business_date"] for r in con.execute(
        "SELECT e.business_date FROM day_entry e WHERE e.unit=? "
        "AND substr(e.business_date,1,7)=? ORDER BY e.business_date", (UNIT, ym))]
    with_marg, no_marg = [], []
    for iso in days:
        e = con.execute("SELECT id FROM day_entry WHERE unit=? AND business_date=?",
                        (UNIT, iso)).fetchone()
        has = con.execute(
            ...
            "AND status IN ('ok','partial') LIMIT 1", (e["id"],)).fetchone()
        (with_marg if has else no_marg).append(iso)

    books_p = marg_p = review_p = review_n = 0
    per_day = []
    for iso in with_marg:
        e = con.execute("SELECT id FROM day_entry WHERE unit=? AND business_date=?",
                        (UNIT, iso)).fetchone()
        b = con.execute("SELECT COALESCE(revenue_p,0) r FROM v_cash_ledger "
                        "WHERE unit=? AND business_date=?", (UNIT, iso)).fetchone()
        m = con.execute(
            "SELECT " + marg_net_sql("si") + " m "
            "FROM sale_item si JOIN ingest_batch ib ON ib.id=si.ingest_batch_id "
            "WHERE ib.day_entry_id=? AND ib.adapter='marg_export' "
            "AND ib.status IN ('ok','partial')", (e["id"],)).fetchone()
        # S201: the REVIEW QUEUE is the other half of the day. A line the
        # parser read at low confidence is parked in sale_item_review instead
        # of sale_item -- so books minus attributed is not an error, it is the
        # queue. Confirmed to the rupee on all six differing days of Aug-2026.
        # sale_item_review stores a return SIGNED, so a plain SUM is right.
        rv = con.execute("SELECT COUNT(*) n, COALESCE(SUM(amount_p),0) r "
                         "FROM sale_item_review WHERE day_entry_id=? "
                         "AND status='open'", (e["id"],)).fetchone()
        bp, mp = int(b["r"] if b else 0), int(m["m"] or 0)
        rp, rn = int(rv["r"] or 0), int(rv["n"] or 0)
        books_p += bp
        marg_p += mp
        review_p += rp
        review_n += rn
        # Like for like: everything Marg gave us, attributed or queued.
        if bp != mp + rp:
            per_day.append(dict(date=iso, books=rupees(bp), marg=rupees(mp),
                                review=rupees(rp),
                                diff=rupees(bp - mp - rp), diff_p=bp - mp - rp))

    # Marg had bills for a day the books never had. The export was skipped at
    # upload time and the flag was left behind on purpose (F-113); it clears
    # itself once that day is filed and re-uploaded.
    not_filed = [r["business_date"] for r in con.execute(
        "SELECT df.business_date FROM data_flag df "
        "WHERE df.unit=? AND df.code='MARG_DAY_NOT_FILED' "
        "AND substr(df.business_date,1,7)=? "
        ...
        "GROUP BY df.business_date ORDER BY df.business_date", (UNIT, ym))]

    accounted_p = marg_p + review_p
    return dict(month=ym, days_compared=len(with_marg), days_with_marg=with_marg,
                books=rupees(books_p), marg=rupees(marg_p),
                books_p=books_p, marg_p=marg_p,
                review=rupees(review_p), review_p=review_p, review_n=review_n,
                accounted=rupees(accounted_p), accounted_p=accounted_p,
                diff=rupees(books_p - accounted_p),
                diff_p=books_p - accounted_p,
                agrees=(books_p == accounted_p),
                days_differing=per_day,
                days_no_marg=no_marg, days_not_filed=not_filed)


@app.route("/finance/api/marg-month")
def api_marg_month():
    """This month against Marg, or ?m=YYYY-MM for another."""
    u, err = require("checker")
    if err:
        return err
    ym = (request.args.get("m") or "").strip() or today().strftime("%Y-%m")
    if not re.match(r"^\d{4}-\d{2}$", ym):
        return jsonify(ok=False, error="bad_month"), 400
    return jsonify(ok=True, **_marg_month_compare(db(), ym))


@app.route("/finance/api/marg-corrections.xlsx")
def api_marg_corrections_xlsx():
    """The checklist as a real Excel workbook — what Amir actually works from.

    He sits at the medical PC with Marg open; a chat message is for telling him
    the work exists, but the work itself wants a sheet he can keep open, tick
    down and write on. So: the instruction is one crisp phrase (CASH -> UPI),
    amounts are NUMBERS so they add and sort, the header is frozen, and two
    columns belong to HIM -- Done? (a Yes/No dropdown, so it can be filtered)
    and Remarks. We never write in those; the system's own Status column sits
    ...
    try:
        ...
        return jsonify(ok=False, error="openpyxl_missing",
                       message="Excel export needs openpyxl on the server."), 501

    rows = _correction_rows(db(), 90, include_resolved=request.args.get("all") == "1")
    wb = Workbook()
    ws = wb.active
    ws.title = "Marg corrections"

    head = ["#", "Date", "Change", "Amount (Rs)", "Likely bill", "Books say UPI",
            "Bank says UPI", "Correct by", "Status", "Done?", "Remarks"]
    ws.append(["Marg corrections — Sanjeevni"])
    ws.append(["Prepared %s. Correct these in Marg, mark Done, and tell Dr Manoj."
               % dt.date.today().strftime("%d-%m-%Y")])
    ws.append([])
    ws.append(head)
    ws["A1"].font = Font(bold=True, size=14)
    ws["A2"].font = Font(italic=True, color="6B675E")

    HDR = 4
    fill = PatternFill("solid", fgColor="1F6F5C")
    thin = Side(style="thin", color="D8D5CD")
    ...
    LABEL = {"open": "Not started", "sent": "Given to Amir",
             "corrected": "Corrected in Marg", "resolved": "Done — books agree"}
    for n, r in enumerate(rows, 1):
        bill = r["candidates"] if (
            r["candidates"] and "one bill matches" in (r["candidate_note"] or "")
        ) else (r["candidate_note"] or "")
        ws.append([n, _ddmmyyyy(r["date"]), r["change"],
                   round(abs(int(r["diff_p"])) / 100.0, 2), bill,
                   r["declared_upi"], r["bank_upi"], _ddmmyyyy(r["due"]),
                   LABEL.get(r["status"], r["status"]), "", ""])

    last = HDR + len(rows)
    for row in ws.iter_rows(min_row=HDR, max_row=max(last, HDR),
                            min_col=1, max_col=len(head)):
        for c in row:
    ...
    if rows:
        # Done? is a choice, not free typing -- so the column can be filtered and
        # so "y", "YES", "done" and a tick never become four different answers.
        dv = DataValidation(type="list", formula1='"Yes,No"', allow_blank=True,
                            showDropDown=False)
        dv.promptTitle = "Done?"
        dv.prompt = "Pick Yes once the bill is changed in Marg."
        dv.errorTitle = "Yes or No"
        dv.error = "Please choose Yes or No from the list."
        ws.add_data_validation(dv)
        dv.add("J%d:J%d" % (HDR + 1, last))
        ws.auto_filter.ref = "A%d:%s%d" % (HDR, get_column_letter(len(head)), last)
    else:
        ws.cell(row=HDR + 1, column=1,
                value="Nothing to correct — every day the bank could check agrees.")

    note = ws.cell(row=last + 2, column=1,
                   value="After changing a bill, that day has to be exported from Marg "
                         "again (BILL WISE SALES → Report Type: Detail → With Item "
                         "Deta.: Yes) and sent, otherwise the books do not change.")
    note.font = Font(bold=True, color="9A6A00")

    for i, w in enumerate([5, 12, 16, 13, 26, 14, 14, 12, 22, 9, 34], 1):
    ...
    out.headers["Content-Disposition"] = (
        'attachment; filename="Marg_corrections_%s.xlsx"' % dt.date.today().isoformat())
    return out


@app.route("/finance/api/marg-corrections.txt")
def api_marg_corrections_txt():
    """The same list as plain text -- what the WhatsApp and email buttons send."""
    u, err = require("checker")
    if err:
        return err
    txt = _corrections_text(_correction_rows(db(), 90))
    return app.response_class(txt, mimetype="text/plain; charset=utf-8")


@app.route("/finance/marg-worklist")
def page_marg_worklist():
    """The correction checklist: what has to change in Marg, who has it, when it
    is due, and whether it is done. Printable, exportable, and self-closing --
    an item disappears when the corrected day is re-exported and applied."""
    u, err = require("checker")
    if err:
        return redirect(PORTAL_LOGIN, code=302)
    show_all = request.args.get("all") == "1"
    rows = _correction_rows(db(), 90, include_resolved=show_all)
    open_n = sum(1 for r in rows if r["status"] != "resolved")
    today_iso = dt.date.today().isoformat()

    LABEL = {"open": "Not started", "sent": "Given to Amir",
             "corrected": "Corrected in Marg — awaiting re-export",
             "resolved": "Done — books agree"}
    if not rows:
        body = ("<div class=card><div class=ok>Nothing to correct.</div>"
                "<p class=m>Every day with a bank statement agrees with the books "
                "for the last 90 days.</p></div>")
    else:
        body = ""
        for r in rows:
            cand = ""
            if r["candidates"]:
                cand = ("<div class=cand><b>Likely bill:</b> %s</div>"
                        % html_escape(r["candidates"]))
            late = (r["status"] != "resolved" and r["due"] and r["due"] < today_iso)
            opts = "".join(
                "<option value='%s'%s>%s</option>"
                % (k, " selected" if r["status"] == k else "", LABEL[k])
                for k in ("open", "sent", "corrected", "resolved"))
            body += ("<div class='card %s'><div class=hd><div class=d>%s</div>"
                     "<div class='pill %s'>%s</div></div>"
                     "<div class=act>%s</div>"
                     "<div class=m>Books say UPI %s · bank says %s · %s</div>"
                     "%s<div class=m2>%s</div>"
                     "<form method=post action='/finance/api/marg-corrections' class=ctl>"
                     "<input type=hidden name=business_date value='%s'>"
                     "<label>Status<select name=status>%s</select></label>"
                     "<label>Correct by<input type=date name=due_date value='%s'></label>"
                     "<label>Given to<input name=assigned_to value='%s' "
                     "placeholder='Amir' maxlength=60></label>"
                     "<label class=wide>Note<input name=note value='%s' "
                     "placeholder='bill no., what was changed…' maxlength=200></label>"
                     "<button>Save</button>%s</form></div>"
                     % ("done" if r["status"] == "resolved" else ("late" if late else ""),
                        r["date"], r["status"], LABEL.get(r["status"], r["status"]),
                        html_escape(r["action"]),
                        r["declared_upi"], r["bank_upi"], html_escape(r["says"]),
                        cand, html_escape(r["candidate_note"]),
                        r["date"], opts, html_escape(r["due"]),
                        html_escape(r["assigned"]), html_escape(r["note"]),
                        ("<span class=late>overdue</span>" if late else "")))
    _any_due = next((r["due"] for r in rows if r["due"] and r["status"] != "resolved"), "")
    _any_who = next((r["assigned"] for r in rows if r["assigned"] and r["status"] != "resolved"), "")
    head = ("<div class=sum><b>%d</b> day%s still to correct%s</div>"
            % (open_n, "" if open_n == 1 else "s",
               (" · showing finished ones too" if show_all else "")))
    if open_n:
        head += ("<form method=post class='card sched' "
                 "action='/finance/api/marg-corrections/schedule'>"
                 "<div class=act>Set the schedule for all %d</div>"
                 "<div class=ctl>"
                 "<label>Give them to<input name=assigned_to value='%s' "
                 "placeholder='Amir' maxlength=60></label>"
                 "<label>Correct by<input type=date name=due_date value='%s'></label>"
                 "<button>Set</button></div>"
                 "<div class=m2>This sets the deadline and the owner on every "
                 "unfinished item at once. Individual rows can still be changed "
                 "below.</div></form>"
                 % (open_n, html_escape(_any_who), html_escape(_any_due)))
    # Sharing it. WhatsApp will not take a push from us -- Meta only allows
    # free text inside a 24h window the other person opens, and we have no
    # approved template for this -- so the honest answer is a clipboard: the
    # doctor pastes it into his own chat. Email is a mailto, which needs no
    # infrastructure either. Both carry the SAME text as the .txt endpoint.
    _txt = _corrections_text(rows)
    _body = _txt if len(_txt) <= 1600 else (
        _txt[:1500].rsplit("\n", 1)[0] + "\n\n... the rest is in the CSV.")
    _mailto = ("mailto:?subject="
               + urllib.parse.quote("Marg corrections - %d day(s) to correct" % open_n)
               + "&body=" + urllib.parse.quote(_body))
    head += ("<div class=share>"
             "<button id=waBtn class=wa type=button onclick=\"copyWA()\">"
             "Copy for WhatsApp</button>"
             "<a class=btn href=\"" + html_escape(_mailto) + "\">Email</a>"
             "<a class=btn href=\"/finance/api/marg-corrections.xlsx\">Excel for Amir</a>"
             "<a class=btn href=\"/finance/api/marg-corrections.csv\">CSV</a>"
             "</div><div id=waHint class=hint></div>"
             "<textarea id=shareTxt readonly rows=14>"
             + html_escape(_txt) + "</textarea><script>" + _SHARE_JS + "</script>")
    html = ("""<!DOCTYPE html><html lang=en><head><meta charset=utf-8>
<meta name=viewport content="width=device-width,initial-scale=1">
<title>Marg corrections — Sanjeevni</title><style>
:root{--page:#f3f2ee;--card:#fff;--line:#e4e1d9;--ink:#22201c;--mut:#6b675e;--brand:#1f6f5c}
*{box-sizing:border-box}
body{margin:0;background:var(--page);color:var(--ink);
 font:15px/1.55 system-ui,-apple-system,"Segoe UI",Roboto,sans-serif}
.bar{background:var(--brand);color:#fff;padding:12px 14px;display:flex;gap:8px;align-items:center;
 flex-wrap:wrap}
.bar b{font-size:17px}.bar a{color:#e7f2ee;text-decoration:none;font-size:13px;
 background:rgba(255,255,255,.15);padding:6px 11px;border-radius:8px}
.bar .sp{margin-left:auto}
.wrap{max-width:680px;margin:0 auto;padding:14px 12px 44px}
.sum{font-size:15px;margin:6px 2px 2px}
.card{background:var(--card);border:1px solid var(--line);border-radius:12px;padding:13px 15px;margin:11px 0}
.card.done{opacity:.62}
.card.late{border-color:#b4472f66;box-shadow:inset 3px 0 0 #b4472f}
.hd{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.d{font-family:ui-monospace,Menlo,Consolas,monospace;font-size:12.5px;color:var(--mut)}
.pill{margin-left:auto;font-size:11.5px;font-weight:650;padding:3px 9px;border-radius:999px;
 background:#eceae4;color:var(--mut)}
.pill.sent{background:#e7eefb;color:#2b4c8c}.pill.corrected{background:#fbf3e0;color:#8a6100}
.pill.resolved{background:#e4f1ec;color:var(--brand)}
.act{font-weight:650;font-size:15.5px;margin:4px 0 5px}
.m{color:var(--mut);font-size:13px}.m2{color:var(--mut);font-size:12.5px;font-style:italic;margin-top:5px}
.cand{margin-top:6px;font-size:13.5px}
.ok{color:var(--brand);font-weight:650}
.ctl{display:flex;flex-wrap:wrap;gap:8px;align-items:end;margin-top:11px;padding-top:11px;
 border-top:1px dashed var(--line)}
.ctl label{display:flex;flex-direction:column;gap:3px;font-size:11.5px;color:var(--mut);flex:1 1 44%%}
.ctl label.wide{flex:1 1 100%%}
.ctl select,.ctl input{font:14px inherit;padding:7px 8px;border:1px solid var(--line);
 border-radius:8px;background:#fcfcfa;color:var(--ink);width:100%%}
.sched .act{margin-bottom:8px}
.share{display:flex;gap:8px;flex-wrap:wrap;margin:10px 2px 2px}
.share .wa,.share .btn{font:600 14px inherit;padding:10px 15px;border-radius:9px;
 border:1px solid var(--line);background:var(--card);color:var(--ink);
 text-decoration:none;display:inline-block;cursor:pointer}
.share .wa{background:var(--brand);color:#fff;border-color:var(--brand)}
.share .wa.done{background:#0f5c48}
.hint{color:var(--brand);font-size:13px;font-weight:600;margin:2px}
#shareTxt{display:none;width:100%%;margin:8px 0;padding:10px;border:1px solid var(--line);
 border-radius:9px;box-sizing:border-box;background:var(--card);color:var(--ink);
 font:12.5px ui-monospace,Menlo,Consolas,monospace}
.ctl button{font:600 14px inherit;padding:8px 16px;border:0;border-radius:8px;
 background:var(--brand);color:#fff}
.late{color:#b4472f;font-weight:650;font-size:12.5px;align-self:center}
.note{background:#fbf3e0;border:1px solid #9a6a0044;border-radius:10px;padding:12px 14px;
 margin:14px 0;font-size:13.5px}
@media print{.bar a,.ctl,.share,#shareTxt{display:none}.card.done{display:none}}
</style></head><body>
<div class=bar><b>Marg corrections</b>
 <span class=sp></span>
 <a href="/finance/api/marg-corrections.csv">Export for Amir</a>
 <a href="/finance/marg-worklist%s">%s</a>
 <a href="/finance/health">← Health</a></div>
<div class=wrap>
%s
<div class=note><b>After correcting a bill in Marg, the books do not change by
themselves.</b> Re-run that day's BILL WISE SALES export (Detail, With Item
Deta. = Yes), send it, and apply it — only then do the books update, and only
then does the item below tick itself off.</div>
%s
</div></body></html>""" % ("" if show_all else "?all=1",
                           "Hide finished" if show_all else "Show finished",
                           head, body))
    return app.response_class(html, mimetype="text/html")


@app.route("/finance/api/upi-misclass")
def api_upi_misclass():
    """Every day whose cash/UPI split disagrees with the bank, with direction."""
    u, err = require("checker")
    if err:
        return err
    try:
        days = max(1, min(400, int(request.args.get("days", 60))))
    except ValueError:
        days = 60
    rows = _upi_misclass(db(), days)
    net = sum(r["diff_p"] for r in rows)
    return jsonify(ok=True, days=days, rows=rows, count=len(rows),
                   upi_as_cash=sum(1 for r in rows if r["diff_p"] > 0),
                   cash_as_upi=sum(1 for r in rows if r["diff_p"] < 0),
                   net_drawer_effect=rupees(net),
                   note=("+ means the drawer is overstated: UPI money booked as "
                         "cash. - means understated."))


# --------------------------------------------------------------- A1 (S195)
def _marg_total_for_date(con, iso):
    """What MARG says this day's sale was, in paise, or (None, None).

    Source of truth is a staged push's own parsed figure: marg_report's signed
    net, exactly what the printed report says, returns included. We do NOT try
    to reconstruct it from sale_item -- credit notes are stored there as a
    magnitude plus a '_return' service, so a naive SUM would ADD a refund, and
    a wrong warning is worse than no warning (the 21-08-2026 lesson).

    Silence when unsure is deliberate.
    """
    try:
        _marg_staging(con)
        for r in con.execute(
                "SELECT parsed_json FROM marg_push_staging WHERE unit=? "
                "AND parsed_json IS NOT NULL ORDER BY id DESC LIMIT 25", (UNIT,)):
            try:
                for d in (json.loads(r["parsed_json"]) or {}).get("days", []):
                    if d.get("business_date") == iso and d.get("net_p") is not None:
                        return int(d["net_p"]), "the Marg report received"
            except Exception:                                     # noqa: BLE001
                continue
    except Exception:                                             # noqa: BLE001
        pass
...
def api_day_mirror(date_iso):
    ...
    if has_marg:
        row = con.execute("SELECT COUNT(*) n, " + marg_net_sql("sale_item") + " t "
...
# =============================================================================
#  D349 (S202): ONE PLACE DECIDES WHAT A DIFFERENCE MEANS.
#
#  The owner's ruling, in his words: "it should be the rule at one place and all
#  data shd follow that."
#
#  WHY IT WAS NEEDED. S201 renamed this on /finance/health -- "variance" and
#  "low confidence" retired for the owner's own words, sale bills with no clinic
#  ID (D348). The exceptions card on /finance/approvals kept saying "variance",
#  because nobody realised the two surfaces were showing the SAME underlying
#  rows. One rename, two screens, one of them missed.
#
#  AND IT MATTERED MORE THAN THE WORD. That card mixes two populations:
#    - days whose only difference is bills not yet matched to a patient. Under
#      D313 the Marg import cannot touch money at all, so NOTHING IS WRONG here.
#    - days that are genuinely unexplained -- including one NEGATIVE row where
#      the lines EXCEED the day's total, open since S186.
#  Five harmless rows were hiding four real ones.
#
#  CLASSIFIED FROM DATA, NEVER FROM THE DESCRIPTION TEXT. Parsing the detail
#  string would just be a third copy of the rule, drifting the moment somebody
#  rewords it -- which is the exact fault this function exists to end.
# =============================================================================
DIFF_PARKED = "parked"   # bills awaiting a patient name. Not a money problem.
DIFF_LOOK   = "look"     # genuinely unexplained. A person is needed.


def difference_meaning(con, unit, business_date, diff_p):
    """What a line_sum_vs_day_total difference MEANS. One definition; every
    surface reads it and none computes its own."""
    r = con.execute(
        "SELECT COUNT(*) n, COALESCE(SUM(rv.amount_p),0) v "
        "FROM sale_item_review rv JOIN day_entry e ON e.id=rv.day_entry_id "
        "WHERE e.unit=? AND e.business_date=? AND rv.status='open'",
        (unit, business_date)).fetchone()
    n = int((r["n"] if r else 0) or 0)
    v = int((r["v"] if r else 0) or 0)
    if diff_p < 0:
        return dict(meaning=DIFF_LOOK, parked_n=n, parked_p=v,
                    label="Lines exceed the day's total",
                    why="The attributed lines add up to MORE than the day was "
                        "filed for. That cannot be unmatched bills, so it needs "
                        "a person.")
    if n > 0 and v == diff_p:
        return dict(meaning=DIFF_PARKED, parked_n=n, parked_p=v,
                    label="Sale bills with no clinic ID",
                    why="%d bill(s) on this day have not been matched to a "
                        "patient yet. The money is counted in full (D313) -- "
                        "only the patient name is missing." % n)
    return dict(meaning=DIFF_LOOK, parked_n=n, parked_p=v,
                label="Unexplained difference",
                why="This is not accounted for by bills awaiting a patient "
                    "name. It needs a person.")


# =============================================================================
#  B2 (S202): THE PIPELINE HEARTBEAT — the half of the chain the VPS cannot see.
#
#  Every health check this server runs watches ARRIVAL AT THE VPS. Four of the
...
#  None of them is visible here. F-179 is the proof: eleven verified reports sat
#  undelivered for three days while every component reported success, and the
#  only symptom reachable by the owner was a page that stayed empty.
#
#  So manojz posts what it can see, and this end receives it and checks it.
#
#  NO NEW SECRET. It reuses FINANCE_MARG_TOKEN, the scoped stage-only token that
#  already exists for marg-push. Token rotation is the oldest open item in the
#  project and there are already three copies of that one; a fourth thing to
#  rotate would be a cost paid forever for a convenience used once.
# =============================================================================
def _pipeline_table(con):
    con.execute("CREATE TABLE IF NOT EXISTS pipeline_status ("
                " id INTEGER PRIMARY KEY,"
                " received_at TEXT NOT NULL,"
                " source TEXT NOT NULL,"
                " payload_json TEXT NOT NULL)")
    con.execute("CREATE TABLE IF NOT EXISTS health_check_seen ("
                " key TEXT PRIMARY KEY,"
                " first_seen TEXT NOT NULL,"
                " nonok_count INTEGER NOT NULL DEFAULT 0,"
                " last_nonok TEXT)")


def _pipeline_latest(con, source="manojz"):
    """The most recent status manojz posted, or None if it has never posted."""
    _pipeline_table(con)
    ...
    try:
        ...
        return dict(received_at=r["received_at"], broken=True)
...
def api_pipeline_status():
    """manojz posts here at the end of each 10-minute pull. Token-scoped,
    append-only, and it can touch nothing but its own table."""
    ...
    con.execute("INSERT INTO pipeline_status (received_at, source, payload_json) "
                "VALUES (?,?,?)", (now_iso(), src, json.dumps(body)[:20000]))
    # keep the last 500 per source -- this is a heartbeat, not an archive
    con.execute("DELETE FROM pipeline_status WHERE source=? AND id NOT IN "
                "(SELECT id FROM pipeline_status WHERE source=? "
                " ORDER BY id DESC LIMIT 500)", (src, src))
    con.commit()
    return jsonify(ok=True, source=src, received_at=now_iso())
...
def api_approvals():
    ...
    # D349: the UPI rows say WHICH WAY the day was out, because the direction
    # decides who is out of pocket. diff = entered - bank. A negative diff means
    # less UPI was entered than the bank settled, so cash was recorded too high
    # and the drawer is expected to hold money that was never in it -- the maker
    # makes that up personally. The owner worked this out himself; the page
    # never said it.
    upi_open, upi_rows, upi_net_p = [], [], 0
    for r in con.execute(
            "SELECT id, business_date, diff_p, detail FROM recon_exception "
            "WHERE unit=? AND kind='upi_vs_statement' AND status='open' "
            "ORDER BY business_date DESC LIMIT 40", (UNIT,)):
        d = int(r["diff_p"] or 0)
        upi_net_p += d
        upi_open.append(dict(date=r["business_date"], diff=rupees(d)))
        upi_rows.append(dict(id=r["id"], date=r["business_date"],
                             diff=rupees(d), diff_p=d, detail=r["detail"],
                             direction=("maker_short" if d < 0 else
                                        "maker_holds_extra" if d > 0 else "level"),
                             says=("the drawer was expected to hold Rs %s that "
                                   "was never in it" % rupees(-d) if d < 0 else
                                   "the drawer holds Rs %s more than the books "
                                   "expect" % rupees(d) if d > 0 else "level")))

    # D349: every row carries its exception id (so it can be resolved WHERE IT
    # IS SHOWN -- no page hopping) and its MEANING from the one rule.
    variance, differences = [], []
    for r in con.execute(
            "SELECT id, business_date, diff_p, detail FROM recon_exception "
            "WHERE unit=? AND kind='line_sum_vs_day_total' AND status='open' "
            "ORDER BY business_date DESC LIMIT 40", (UNIT,)):
        m = difference_meaning(con, UNIT, r["business_date"], int(r["diff_p"] or 0))
        variance.append(dict(date=r["business_date"], detail=r["detail"]))
        differences.append(dict(id=r["id"], date=r["business_date"],
                                diff=rupees(int(r["diff_p"] or 0)),
                                diff_p=int(r["diff_p"] or 0),
                                detail=r["detail"], **m))
    look_rows   = [d for d in differences if d["meaning"] == DIFF_LOOK]
    parked_rows = [d for d in differences if d["meaning"] == DIFF_PARKED]
    ...
    return jsonify(ok=True, today=today_iso,
                   ...
                   # kept verbatim so every existing reader keeps working
                   upi_mismatches=upi_open, variance_days=variance,
                   # D349: the same rows, with their meaning and their id
                   differences=differences,
                   differences_look=look_rows,
                   differences_parked=parked_rows,
                   differences_look_n=len(look_rows),
                   differences_parked_n=len(parked_rows),
                   upi_rows=upi_rows, upi_net=rupees(upi_net_p),
                   upi_net_p=upi_net_p,
                   review_open=review_open)
...
def api_tile_summary():
    ...
    review = con.execute(
        ...
        (UNIT,)).fetchone()[0]
    # S196: the 21-08 crisis lesson, finally WIRED. _health_headline() was
    # built at S195 "for the portal tile" and nothing ever consumed it — the
    # health page went red while the tile looked innocent. One short line,
    # None when everything is fine; the tile shows it FIRST.
    health_line = _health_headline(con)
    return jsonify(ok=True, to_approve=to_approve, marg_pushes=marg_pushes,
                   missing_marg=missing_marg, exceptions=exceptions,
                   review=review, health_line=health_line)
...
def selftest():
    ...
    # ---- S195 health: the page that makes a silent failure visible ---------
    _hz = c.get("/finance/api/health", headers={"X-Clinic-Role": "checker"})
    _hj = _hz.get_json() or {}
    check("S195 health: answers the checker",
          _hz.status_code == 200 and _hj.get("ok"))
    check("S195 health: every check carries a known state",
          len(_hj.get("checks") or []) >= 5
          and all(x.get("state") in ("ok", "info", "warn", "bad")
                  for x in _hj.get("checks") or []))
    # Flags always exist, so if they counted as a problem the portal tile would
    # be lit permanently and the warning would stop meaning anything.
    check("S195 health: flags never count as a problem",
          all(x.get("state") in ("ok", "info")
              for x in (_hj.get("checks") or []) if x.get("key") == "flags"))
    check("S195 health: the page renders for the checker",
          c.get("/finance/health", headers={"X-Clinic-Role": "checker"}).status_code == 200)
    # ---- S198_H1: every health section is a DOOR (owner, S198) -------------
    _hp = c.get("/finance/health", headers={"X-Clinic-Role": "checker"})
    _ht = _hp.get_data(as_text=True)
    check("S198 health: the Marg row links to the Hub's Marg card",
          "/finance/approvals#margCard" in _ht)
    check("S198 health: the days row links to the approval queue",
          "/finance/approvals#pendCard" in _ht)
    check("S198 health: the UPI row links to Today's strip",
          "/finance/approvals#stripCard" in _ht)
    _hmap = {"push": "#margCard", "days": "#pendCard", "drawer": "#cashPosCard",
             "upisplit": "/finance/marg-worklist", "corrlist": "/finance/marg-worklist",
             "upi": "#stripCard", "margnotfiled": "#margCard",
             "margmonth": "#monthCard", "flags": "#pendCard"}
    check("S198 health: every rendered mapped row carries its link",
          all(_hmap[x["key"]] in _ht
              for x in (_hj.get("checks") or []) if x.get("key") in _hmap))
    _hn = len(_hj.get("checks") or [])
    check("S198 health: every check renders exactly one row",
          _ht.count('class="row"') == _hn and _hn >= 5)
    check("S198 health: exactly the mapped rows are clickable "
          "(backup deliberately not a link)",
          _ht.count('<a class="row"') == sum(
              1 for x in (_hj.get("checks") or [])
              if x.get("key") in ("push", "days", "drawer", "upisplit",
                                  "corrlist", "upi", "margnotfiled",
                                  "margmonth", "flags", "renewals")))
    # ---- S198_H2: the owner's three findings, held ------------------------
    check("S198_H2: renewals row is a door to the Master v2 sheet",
          ("1OB70_Mapuugc33zkfFevwnrS0e8s1NdWzsrzJDqO38E" in _ht)
          == any(x.get("key") == "renewals" for x in (_hj.get("checks") or [])))
    # ---- S201: the attribution queue is a note, never a fault -------------
    # A known, planned waiting state must not drive the portal tile. Only
    # warn/bad reach it; info is shown and does not. This is the same ruling
    # that keeps data_flag off the tile (S195) -- applied here at last.
    _hq = [x for x in (_hj.get("checks") or []) if x.get("key") == "margqueue"]
    check("S201 health: the attribution queue is info, never warn or bad",
          all(x.get("state") == "info" for x in _hq))
    check("S201 health: the no-ID row says the money IS counted in sales",
          all("Counted in sales in full" in (x.get("hint") or "")
              for x in _hq))
    # ---- S201: nested <a> -- the fault the owner's saved page revealed ----
    # The Correction-checklist row wraps the whole row in an anchor AND its
    # hint carried "Open the checklist" as a second anchor. Nested <a> is
    # invalid; every browser un-nests it, orphaning the mark and dropping
    # .body outside the row. Assert on the SERVED BYTES that no row anchor
    # contains another anchor -- an eye cannot police this, a test can.
    def _nested_anchor(doc):
        i = doc.find('<a class="row"')
        while i != -1:
            j = doc.find("</a>", i)
            if j == -1 or doc.find("<a", i + 3, j) != -1:
                return True
            i = doc.find('<a class="row"', j)
        return False
    check("S201: no row anchor contains another anchor (nested <a> is invalid "
          "and the browser un-nests it, breaking the row)",
          not _nested_anchor(_ht))
    # de-linking must keep the WORDS -- the row already goes there.
    _hcl = [x for x in (_hj.get("checks") or []) if x.get("key") == "corrlist"]
    check("S201: a de-linked hint keeps its text",
          (not _hcl) or ("Open the checklist" in _ht))

    _hranks = {"ok": 0, "info": 0, "warn": 1, "bad": 2}
    _hseq = [_hranks[x["state"]] for x in (_hj.get("checks") or [])]
    check("S198_H2: checks arrive worst-first (really sorted, not claimed)",
          _hseq == sorted(_hseq, reverse=True))
    check("S198_H2: culprits listed exactly when a check is non-ok",
          set(_hj.get("culprits") or [])
          == {x["label"] for x in (_hj.get("checks") or [])
              if _hranks[x["state"]] > 0})
    check("S198_H2: sundays_between — none in a weekday span",
          _sundays_between(dt.date(2026, 8, 18), dt.date(2026, 8, 22)) == 0)
    check("S198_H2: sundays_between — Sat->Sun counts the Sunday",
          _sundays_between(dt.date(2026, 8, 22), dt.date(2026, 8, 23)) == 1)
    check("S198_H2: sundays_between — Sat->Mon counts one, two weeks two",
          _sundays_between(dt.date(2026, 8, 22), dt.date(2026, 8, 24)) == 1
          and _sundays_between(dt.date(2026, 8, 15), dt.date(2026, 8, 24)) == 2)
    check("S195 health: a maker is refused",
          c.get("/finance/api/health",
                headers={"X-Clinic-User": "darpan", "X-Clinic-Role": "maker"}
                ).status_code in (401, 403))

    # ---- S195 A3+: the cash/UPI split, with DIRECTION ----------------------
    _mc = c.get("/finance/api/upi-misclass?days=90",
                headers={"X-Clinic-Role": "checker"})
    _mj = _mc.get_json() or {}
    check("S195 split: the misclass report answers",
          _mc.status_code == 200 and _mj.get("ok"))
    check("S195 split: every row names a direction and a drawer effect",
          all(r.get("direction") in ("upi_as_cash", "cash_as_upi") and r.get("says")
              for r in (_mj.get("rows") or [])))
    check("S195 split: the two directions add up to the rows",
          int(_mj.get("upi_as_cash", 0)) + int(_mj.get("cash_as_upi", 0))
          == int(_mj.get("count", 0)))
    check("S195 split: a maker cannot read it",
          c.get("/finance/api/upi-misclass",
                headers={"X-Clinic-User": "darpan", "X-Clinic-Role": "maker"}
                ).status_code in (401, 403))

    # ---- S195: the Marg correction worklist --------------------------------
    _wl = c.get("/finance/api/marg-worklist?days=90",
                headers={"X-Clinic-Role": "checker"})
    _wj = _wl.get_json() or {}
    check("S195 worklist: answers the checker",
          _wl.status_code == 200 and _wj.get("ok"))
    check("S195 worklist: every row says what to change, and which way",
          all(r.get("action") and r.get("from_mode") in ("cash", "upi")
              and r.get("to_mode") in ("cash", "upi")
              and r["from_mode"] != r["to_mode"]
              for r in (_wj.get("rows") or [])))
    # Naming a wrong bill is worse than naming none: only exact single-amount
    # matches are offered, never a combination that coincidentally sums.
    check("S195 worklist: a named candidate really carries that exact amount",
          all(c_["amount"] == r["move"]
              for r in (_wj.get("rows") or []) for c_ in (r.get("candidates") or [])))
    check("S195 worklist: it says corrections must be re-exported",
          "apply" in (_wj.get("after") or "").lower())
    check("S195 worklist: the printable page renders",
          c.get("/finance/marg-worklist",
                headers={"X-Clinic-Role": "checker"}).status_code == 200)



    # ---- S195: the credit-note sign, in ONE place --------------------------
    # 18-08-2026: one credit note of 1,640. The true net of the loaded rows was
    # 20,599; the maker's mirror showed 23,879 -- out by exactly twice the note,
    # and near enough to the figure under dispute to send a real investigation
    # down the wrong road. Two readers, two sums of the same rows.
    import sqlite3 as _sq
    _c3 = _sq.connect(":memory:")
    _c3.execute("CREATE TABLE sale_item (day_entry_id INT, service TEXT, amount_p INT)")
    _c3.executemany("INSERT INTO sale_item VALUES (?,?,?)",
                    [(1, "medicine", 2059900), (1, "medicine_return", 164000),
                     (2, "medicine", 500000), (3, None, 100000)])
    _q = lambda d: _c3.execute("SELECT " + marg_net_sql("sale_item")
                               + " FROM sale_item WHERE day_entry_id=?", (d,)).fetchone()[0]
    check("S195 sign: a credit note is subtracted, not added",
          _q(1) == 2059900 - 164000)
    check("S195 sign: a day without one is untouched", _q(2) == 500000)
    check("S195 sign: a row with no service still counts", _q(3) == 100000)
    check("S195 sign: no rows nets zero, not NULL", _q(99) == 0)
    # and no reader may keep its own private copy of the rule
    _src = open(__file__, encoding="utf-8").read()
    # The needles are ASSEMBLED, not written whole: a literal search string
    # occurs in the source it is searching, so the check would find itself and
    # fail forever. (It did, on the first attempt.)
    _n1 = "SUM(si." + "amount_p)"
    _n2 = "COUNT(*) n, COALESCE(SUM(" + "amount_p),0) t"
    check("S195 sign: no unsigned SUM over sale_item survives anywhere",
          _n1 not in _src and _n2 not in _src)
    check("S195 sign: every reader goes through the one expression",
          _src.count("marg_net_sql(") >= 4)      # the def + three call sites
    # A cross-reader check against /finance/api/workbench/<ym> was written here
    # and REMOVED. Two reasons, both mine: its response shape was guessed rather
    # than read, and its `_wj` stole the worklist's `_wj` further up -- so it
    # failed, and it broke a passing check as it went. The source guard above
    # already proves every reader goes through the one expression, which is the
    # property that matters. A test whose shape I cannot verify offline is not
    # worth the round trip it costs.

    # ---- S195 A4: the month, and the days that never arrived ---------------
    _mm = c.get("/finance/api/marg-month", headers={"X-Clinic-Role": "checker"})
    _mmj = _mm.get_json() or {}
    check("S195 month: the checker can ask", _mm.status_code == 200 and _mmj.get("ok"))
    check("S195 month: a maker cannot",
          c.get("/finance/api/marg-month",
                headers={"X-Clinic-User": "darpan", "X-Clinic-Role": "maker"}
                ).status_code in (401, 403))
    check("S195 month: a rubbish month is refused",
          c.get("/finance/api/marg-month?m=august",
                headers={"X-Clinic-Role": "checker"}).status_code == 400)
    check("S195 month: it answers for a month with no data at all",
          (c.get("/finance/api/marg-month?m=1999-01",
                 headers={"X-Clinic-Role": "checker"}).get_json() or {}).get("days_compared") == 0)
    # 'agrees' must mean what it says, or a green line is a lie
    check("S195 month: 'agrees' is exactly 'the difference is zero'",
          _mmj.get("agrees") == (_mmj.get("diff_p") == 0))
    check("S195 month: it only totals days BOTH sides have",
          _mmj.get("days_compared", 0) >= len(_mmj.get("days_differing") or []))
    # a day cannot be both 'has no Marg export' and 'was compared against Marg'
    check("S195 month: the no-Marg days are not in the compared total",
          not (set(_mmj.get("days_no_marg") or [])
               & {d["date"] for d in (_mmj.get("days_differing") or [])}))
    check("S195 month: every differing day states both sides and the gap",
          all(d.get("books") and d.get("marg") and d.get("diff")
              for d in (_mmj.get("days_differing") or [])))
    # ---- S201: the comparison is now like for like ------------------------
    # books were compared against ATTRIBUTED lines only, so any day with one
    # low-confidence bill differed and the row could never go green. The
    # queue is the other half of the day, not a discrepancy.
    check("S201 month: accounted = attributed + queue, exactly",
          _mmj.get("accounted_p") == (_mmj.get("marg_p", 0)
                                      + _mmj.get("review_p", 0)))
    check("S201 month: the gap is books minus BOTH halves",
          _mmj.get("diff_p") == (_mmj.get("books_p", 0)
                                 - _mmj.get("accounted_p", 0)))
    check("S201 month: a differing day names the queue side too",
          all("review" in d for d in (_mmj.get("days_differing") or [])))
    check("S201 month: the queue is counted whenever it is valued",
          bool(_mmj.get("review_n")) == bool(_mmj.get("review_p")))
    # A truncated list must SAY it is truncated -- the sibling never-filed line
    # already does, and this one did not, which is how 24-Aug stayed hidden.
    # Asserted BOTH ways: it says "and N more" exactly when there are more.
    _hmmbad = [x for x in (_hj.get("checks") or [])
               if x.get("key") == "margmonth" and x.get("state") == "bad"]
    _hnd = len(_mmj.get("days_differing") or [])
    check("S201 health: the differing-day list admits when it is truncated",
          all((("and %d more" % (_hnd - 5)) in (x.get("hint") or ""))
              == (_hnd > 5) for x in _hmmbad))
    # The never-filed list must be LIVE, not a museum: the moment a day is filed
    # and its export re-uploaded, it has a Marg batch and must drop off by
    # itself. That is exactly "these two sets never overlap".
    _nf = _mmj.get("days_not_filed") or []
    check("S195 month: a day that now has a Marg batch is off the not-filed list",
          not (set(_nf) & set(_mmj.get("days_with_marg") or [])))
    check("S195 month: every compared day is a day of that month",
          all(d.startswith(_mmj.get("month", "")) for d in (_mmj.get("days_with_marg") or [])))
    check("S195 month: not-filed days all belong to the month asked for",
          all(d.startswith(_mmj.get("month", "")) for d in _nf))

    # ---- S195: the correction CHECKLIST (state, export, feedback) ----------
    _ck = c.get("/finance/api/marg-corrections", headers={"X-Clinic-Role": "checker"})
    _cj = _ck.get_json() or {}
    check("S195 checklist: answers the checker",
          _ck.status_code == 200 and _cj.get("ok"))
    check("S195 checklist: a maker cannot read it",
          c.get("/finance/api/marg-corrections",
                headers={"X-Clinic-User": "darpan", "X-Clinic-Role": "maker"}
                ).status_code in (401, 403))
    check("S195 checklist: every item carries a status we know",
          all(r.get("status") in ("open", "sent", "corrected", "resolved")
              for r in (_cj.get("rows") or [])))
    # The whole point: the list closes ITSELF. An item can only still be open if
    # its day still disagrees -- nobody has to remember to tick anything off.
    # ...asked over HTTP, not by calling db() -- there is no application context
    # out here, and reaching for one aborts the whole suite before it can report.
    _mcd = {r["date"] for r in ((_mc.get_json() or {}).get("rows") or [])}
    check("S195 checklist: an open item means the day still disagrees",
          all(r["date"] in _mcd for r in (_cj.get("rows") or [])
              if r["status"] != "resolved"))
    check("S195 checklist: no work is made out of days before the start date",
          all(r["date"] >= CORRECTION_FROM for r in (_cj.get("rows") or [])))
    check("S195 checklist: the start date is a real date",
          bool(parse_iso_date(CORRECTION_FROM)))
    check("S195 checklist: it carries the same count as the worklist",
          _cj.get("count") == len(_wj.get("rows") or []))
    # a day that no longer disagrees must be shown as done, not silently dropped
    _cka = (c.get("/finance/api/marg-corrections?all=1",
                  headers={"X-Clinic-Role": "checker"}).get_json() or {})
    check("S195 checklist: finished items are kept, not deleted",
          all(r["date"] in _mcd or r["status"] == "resolved"
              for r in (_cka.get("rows") or [])))

    # Every export reads these off a row. A missing one is a KeyError the first
    # time somebody clicks the button -- which is how the Excel export shipped
    # broken: it was tested against an invented fixture, not against the shape
    # the code really returns.
    check("S195 checklist: a row carries everything the exports read off it",
          all(set(r) >= {"date", "status", "due", "assigned", "note", "diff",
                         "diff_p", "direction", "action", "change",
                         "declared_upi", "bank_upi", "candidates",
                         "candidate_note"}
              for r in (_cj.get("rows") or [])))
    check("S195 checklist: the paise and the printed amount agree",
          all(r["diff"] == rupees(abs(int(r["diff_p"])))
              for r in (_cj.get("rows") or [])))

    _csv = c.get("/finance/api/marg-corrections.csv", headers={"X-Clinic-Role": "checker"})
    _ct = _csv.get_data(as_text=True)
    check("S195 checklist: it exports as a file Amir can be handed",
          _csv.status_code == 200
          and "attachment" in _csv.headers.get("Content-Disposition", "")
          and ".csv" in _csv.headers.get("Content-Disposition", ""))
    check("S195 checklist: the export names the work and the deadline",
          "What to change in Marg" in _ct and "Due" in _ct and "Given to" in _ct)
    check("S195 checklist: the export repeats the re-export warning",
          "does not change the books" in _ct)

    _pg = c.get("/finance/marg-worklist", headers={"X-Clinic-Role": "checker"})
    _ph = _pg.get_data(as_text=True)
    # The export is offered whatever the state; the per-day controls exist only
    # where there is a day to control. Asserting the controls unconditionally is
    # asserting one state of the DATA, which is how this suite has been broken
    # before (F-106) -- and was, here, on a box with nothing to correct.
    check("S195 checklist: the page always offers the export",
          _pg.status_code == 200 and "marg-corrections.csv" in _ph)
    check("S195 checklist: a status and a deadline for every open day",
          ("name=due_date" in _ph and "name=status" in _ph)
          if (_cj.get("rows") or []) else ("Nothing to correct" in _ph))
    check("S195 checklist: free text is escaped before it reaches the page",
          html_escape('x"><b>') == "x&quot;&gt;&lt;b&gt;")

    check("S195 checklist: a rubbish date is refused",
          c.post("/finance/api/marg-corrections",
                 data={"business_date": "not-a-date", "status": "sent"},
                 headers={"X-Clinic-Role": "checker"}).status_code == 400)
    check("S195 checklist: an unknown status is refused",
          c.post("/finance/api/marg-corrections",
                 data={"business_date": today().isoformat(), "status": "finished"},
                 headers={"X-Clinic-Role": "checker"}).status_code == 400)
    check("S195 checklist: a day with nothing to correct cannot be ticked",
          c.post("/finance/api/marg-corrections",
                 data={"business_date": "1999-01-01", "status": "sent"},
                 headers={"X-Clinic-Role": "checker"}).status_code == 404)
    check("S195 checklist: a maker cannot set a status",
          c.post("/finance/api/marg-corrections",
                 data={"business_date": today().isoformat(), "status": "sent"},
                 headers={"X-Clinic-User": "darpan", "X-Clinic-Role": "maker"}
                 ).status_code in (401, 403))
    # and it must actually REMEMBER -- a checklist that forgets is decoration
    if _cj.get("rows"):
        _d0 = _cj["rows"][0]["date"]
        c.post("/finance/api/marg-corrections",
               json={"business_date": _d0, "status": "sent", "assigned_to": "Amir",
                     "due_date": "2026-12-31", "note": "selftest"},
               headers={"X-Clinic-Role": "checker"})
        _back = next((r for r in (c.get("/finance/api/marg-corrections",
                                        headers={"X-Clinic-Role": "checker"}
                                        ).get_json() or {}).get("rows", [])
                      if r["date"] == _d0), None)
        check("S195 checklist: what is set is what comes back",
              bool(_back) and _back["status"] == "sent"
              and _back["assigned"] == "Amir" and _back["due"] == "2026-12-31")


    check("S195 checklist: the page offers one schedule for the whole list",
          ("marg-corrections/schedule" in _ph or not (_cj.get("rows") or [])))
    check("S195 checklist: a maker cannot set the schedule",
          c.post("/finance/api/marg-corrections/schedule",
                 data={"due_date": "2026-12-31", "assigned_to": "Amir"},
                 headers={"X-Clinic-User": "darpan", "X-Clinic-Role": "maker"}
                 ).status_code in (401, 403))
    check("S195 checklist: an empty schedule is refused, not silently applied",
          c.post("/finance/api/marg-corrections/schedule", json={},
                 headers={"X-Clinic-Role": "checker"}).status_code == 400)
    check("S195 checklist: a rubbish deadline is refused",
          c.post("/finance/api/marg-corrections/schedule",
                 json={"due_date": "someday"},
                 headers={"X-Clinic-Role": "checker"}).status_code == 400)
    _sch = c.post("/finance/api/marg-corrections/schedule",
                  json={"due_date": "2026-12-25", "assigned_to": "Amir"},
                  headers={"X-Clinic-Role": "checker"})
    check("S195 checklist: setting a schedule answers with what it touched",
          _sch.status_code == 200 and (_sch.get_json() or {}).get("updated") is not None)
    check("S195 checklist: the schedule never reopens a finished item",
          all(r["status"] == "resolved" or r["due"] == "2026-12-25"
              for r in (c.get("/finance/api/marg-corrections?all=1",
                              headers={"X-Clinic-Role": "checker"}).get_json()
                        or {}).get("rows", [])))


    # ---- S195: handing the list to Amir ------------------------------------
    _tx = c.get("/finance/api/marg-corrections.txt", headers={"X-Clinic-Role": "checker"})
    _tt = _tx.get_data(as_text=True)
    check("S195 share: the list can be had as plain text",
          _tx.status_code == 200 and "text/plain" in _tx.headers.get("Content-Type", ""))
    check("S195 share: a maker cannot take the list",
          c.get("/finance/api/marg-corrections.txt",
                headers={"X-Clinic-User": "darpan", "X-Clinic-Role": "maker"}
                ).status_code in (401, 403))
    check("S195 share: the message always says what to do next",
          ("nothing to correct" in _tt.lower())
          or ("exported from Marg again" in _tt and "CASH" in _tt and "UPI" in _tt))
    # a bill number is named ONLY where exactly one bill carried the amount --
    # sending someone to edit the wrong sale is worse than sending them to look
    check("S195 share: it names a bill only when exactly one matched",
          ("bill:" not in _tt)
          or any("one bill matches" in (r.get("candidate_note") or "")
                 for r in (_wj.get("rows") or [])))
    check("S195 share: dates are written the way they are written here",
          _ddmmyyyy("2026-08-18") == "18-08-2026" and _ddmmyyyy("") == "")
    _pg2 = c.get("/finance/marg-worklist", headers={"X-Clinic-Role": "checker"})
    _ph2 = _pg2.get_data(as_text=True)
    check("S195 share: the page offers WhatsApp, email and the CSV",
          "copyWA()" in _ph2 and "mailto:" in _ph2
          and "marg-corrections.csv" in _ph2)
    # the button and the endpoint must hand over the SAME words -- two ways of
    # sending two different lists is how a correction gets done twice
    check("S195 share: the button copies exactly what the endpoint serves",
          html_escape(_tt) in _ph2)


    # ---- S195: the Excel Amir actually works from --------------------------
    _xl = c.get("/finance/api/marg-corrections.xlsx", headers={"X-Clinic-Role": "checker"})
    check("S195 excel: the checker can take the workbook",
          _xl.status_code == 200 and "spreadsheetml" in _xl.headers.get("Content-Type", ""))
    check("S195 excel: it arrives as a named .xlsx file",
          ".xlsx" in _xl.headers.get("Content-Disposition", "")
          and "attachment" in _xl.headers.get("Content-Disposition", ""))
    check("S195 excel: a maker cannot take it",
          c.get("/finance/api/marg-corrections.xlsx",
                headers={"X-Clinic-User": "darpan", "X-Clinic-Role": "maker"}
                ).status_code in (401, 403))
    _xb = _xl.get_data()
    check("S195 excel: the bytes really are a workbook", _xb[:2] == b"PK")
    try:
        from io import BytesIO as _BIO      # selftest() imports `io` further
        from openpyxl import load_workbook as _lwb   # down, making it a local
        _ws = _lwb(_BIO(_xb)).active
        _hd = [_ws.cell(row=4, column=i).value for i in range(1, 12)]
        check("S195 excel: the header is the one Amir was promised",
              _hd[:5] == ["#", "Date", "Change", "Amount (Rs)", "Likely bill"]
              and _hd[-2:] == ["Done?", "Remarks"])
        # the instruction is one phrase, not a sentence to read twice
        check("S195 excel: the change reads CASH -> UPI, crisply",
              all((_ws.cell(row=i, column=3).value or "") in
                  ("CASH \u2192 UPI", "UPI \u2192 CASH")
                  for i in range(5, _ws.max_row + 1)
                  if isinstance(_ws.cell(row=i, column=1).value, int)))
        # Done? must be a CHOICE -- "y", "YES", "done" and a tick are four
        # different answers to one question, and none of them can be filtered
        _dvs = list(getattr(_ws, "data_validations").dataValidation)
        check("S195 excel: Done? is a Yes/No dropdown, not free typing",
              (not (_cj.get("rows") or []))
              or any(d.type == "list" and "Yes,No" in (d.formula1 or "")
                     for d in _dvs))
        # one row per open day -- a sheet that quietly drops or repeats a day is
        # worse than no sheet, because he works from it without checking
        check("S195 excel: one row per day on the list, no more and no less",
              sum(1 for i in range(5, _ws.max_row + 1)
                  if isinstance(_ws.cell(row=i, column=1).value, int))
              == len(_cj.get("rows") or []))
        # the amount must be a NUMBER, or it cannot be added up or sorted
        check("S195 excel: amounts are numbers, not text",
              all(_ws.cell(row=i, column=4).value is None
                  or isinstance(_ws.cell(row=i, column=4).value, (int, float))
                  for i in range(5, _ws.max_row + 1)))
        check("S195 excel: the two columns that are Amir's are left empty",
              all(not _ws.cell(row=i, column=10).value
                  and not _ws.cell(row=i, column=11).value
                  for i in range(5, _ws.max_row + 1)))
        check("S195 excel: the header stays on screen as he scrolls",
              _ws.freeze_panes == "A5")
    except ImportError:
        check("S195 excel: openpyxl is present for the workbook", False)
    check("S195 excel: the page offers it",
          "marg-corrections.xlsx" in _ph2)

    # ---- S195: Darpan's own accuracy, back to Darpan -----------------------
    _ms = c.get("/finance/api/my-day-summary",
                headers={"X-Clinic-User": "darpan", "X-Clinic-Role": "maker"})
    _mj = _ms.get_json() or {}
    _acc = _mj.get("accuracy") or {}
    check("S195 feedback: the maker's tile carries his own accuracy",
          _ms.status_code == 200 and _acc.get("checked") is not None)
    check("S195 feedback: it never claims more matched than were checked",
          _acc.get("matched", 0) + _acc.get("differing", 0) <= _acc.get("checked", 0))
    check("S195 feedback: it counts only days the bank can actually witness",
          _acc.get("differing", 0) <= _acc.get("checked", 0))
    # the maker sees his OWN score and nothing else -- the checker's world stays
    # the checker's (S187_P2a), so no cash position leaks in with it
    check("S195 feedback: the accuracy line leaks no cash position",
          not any(k in _mj for k in ("drawer_p", "closing_p", "parked_p")))
    ...
    if marg_report is not None and finance_returns is not None:
        ...
        try:
            ...
            # AF-2. These go through the REAL writer, not a fixture shaped
            # like the reader -- the fixture mirroring the reader is exactly
            # how this stayed invisible for six sessions.
            _af2 = _pcx.execute(
                "SELECT parsed_json FROM marg_push_staging WHERE unit=? "
                "ORDER BY id DESC LIMIT 1", (UNIT,)).fetchone()
            _af2d = (json.loads(_af2["parsed_json"]) or {}).get("days", []) if _af2 else []
            check("AF-2: the staged payload carries business_date",
                  bool(_af2d) and all("business_date" in x for x in _af2d))
            check("AF-2: the staged payload carries net_p",
                  bool(_af2d) and all(x.get("net_p") is not None for x in _af2d))
            check("AF-2: _marg_total_for_date FINDS a real staged push",
                  _marg_total_for_date(_pcx, _F187)[0] == 40000)
    ...
    # ---- D349 (S202): ONE rule for what a difference MEANS ------------------
    # Written because S201 renamed this on /finance/health and the exceptions
    # card kept its own copy of the meaning -- and because five harmless rows
    # were hiding four real ones, one of them NEGATIVE and open since S186.
    _d349 = sqlite3.connect(DB_PATH)      # raw, like _pcx -- no app context here
    _d349.row_factory = sqlite3.Row
    try:
        _anyday = _d349.execute(
            "SELECT business_date FROM day_entry WHERE unit=? "
            "ORDER BY business_date DESC LIMIT 1", (UNIT,)).fetchone()
        _dref = _anyday["business_date"] if _anyday else today().isoformat()
        _neg = difference_meaning(_d349, UNIT, _dref, -100)
        check("D349: a NEGATIVE difference is always 'look' — lines cannot "
              "exceed the day because of unmatched bills",
              _neg["meaning"] == DIFF_LOOK and "exceed" in _neg["label"].lower())
        _pk = _d349.execute(
            "SELECT COALESCE(SUM(rv.amount_p),0) v, COUNT(*) n FROM sale_item_review rv "
            "JOIN day_entry e ON e.id=rv.day_entry_id WHERE e.unit=? "
            "AND e.business_date=? AND rv.status='open'", (UNIT, _dref)).fetchone()
        _exact = difference_meaning(_d349, UNIT, _dref, int(_pk["v"] or 0))
        check("D349: a difference equal to the open review value is 'parked' "
              "(bills with no clinic ID — D348's words, not 'variance')",
              (_exact["meaning"] == DIFF_PARKED and int(_pk["n"] or 0) > 0)
              or int(_pk["n"] or 0) == 0)
        _odd = difference_meaning(_d349, UNIT, _dref, int(_pk["v"] or 0) + 12345)
        check("D349: a difference NOT explained by parked bills is 'look'",
              _odd["meaning"] == DIFF_LOOK)
        check("D349: every meaning carries a plain-language reason",
              _neg["why"] and _odd["why"] and len(_odd["why"]) > 20)
    finally:
        _d349.close()
    _ja = c.get("/finance/api/approvals").get_json() or {}
    check("D349: the approvals payload splits differences by meaning",
          isinstance(_ja.get("differences_look"), list)
          and isinstance(_ja.get("differences_parked"), list)
          and _ja.get("differences_look_n") == len(_ja["differences_look"])
          and _ja.get("differences_parked_n") == len(_ja["differences_parked"]))
    check("D349: EVERY difference row carries a resolvable exception id — "
          "that is what lets it be closed where it is shown, no page hopping",
          all(isinstance(d.get("id"), int) and d.get("meaning") in (DIFF_LOOK, DIFF_PARKED)
              for d in _ja.get("differences", [])))
    check("D349: the UPI rows say WHICH WAY the day was out, and net to a figure",
          all(u.get("direction") in ("maker_short", "maker_holds_extra", "level")
              and isinstance(u.get("id"), int) for u in _ja.get("upi_rows", []))
          and _ja.get("upi_net") is not None)
    # ---- B2 (S202): the pipeline heartbeat --------------------------------
    _B2TOK = "smoke-b2-token"
    _oldtok = os.environ.get("FINANCE_MARG_TOKEN")
    os.environ["FINANCE_MARG_TOKEN"] = _B2TOK
    try:
        _r = c.post("/finance/api/pipeline-status", json={"source": "manojz"})
        check("B2: an unauthenticated pipeline post is REFUSED",
              _r.status_code == 401)
        _r = c.post("/finance/api/pipeline-status", data="not json",
                    headers={"X-Finance-Marg": _B2TOK,
                             "Content-Type": "application/json"})
        check("B2: a non-object payload is REFUSED", _r.status_code == 400)
        _pl = {"source": "manojz",
               "outbox": {"count": 3, "oldest_hours": 9},
               "watcher": {"alive": False, "captures_today": 0},
               "ignored": 2, "offsite": {"lag_hours": 40}}
        _r = c.post("/finance/api/pipeline-status", json=_pl,
                    headers={"X-Finance-Marg": _B2TOK})
        check("B2: a good post is accepted", _r.status_code == 200
              and (_r.get_json() or {}).get("ok"))
        # S203: every check above runs on `c`, a SIGNED-IN test client, so
        # _gate() lets them through on the SESSION and the token clause is
        # never exercised. The real caller has no session -- and from S202
        # until S203 it was refused 401 by the gate on EVERY pull while this
        # suite passed. The env var above is also only half of it: _gate()
        # reads the module-level MARG_TOKEN bound at import.
        # RULE: a test must post the way the caller posts.
        import sys as _sys
        _mod = _sys.modules[__name__]
        _oldmt = _mod.MARG_TOKEN
        _mod.MARG_TOKEN = _B2TOK
        try:
            _anon = app.test_client()
            _r = _anon.post("/finance/api/pipeline-status",
                            json={"source": "manojz-shape"},
                            headers={"X-Finance-Marg": _B2TOK})
            check("B2: the REAL caller's shape -- token header, NO session -- "
                  "gets past _gate()", _r.status_code == 200
                  and (_r.get_json() or {}).get("ok"))
            _r = _anon.post("/finance/api/pipeline-status",
                            json={"source": "manojz-shape"},
                            headers={"X-Finance-Marg": "not-the-token"})
            check("B2: a wrong token with no session is refused",
                  _r.status_code == 401)
        finally:
            _mod.MARG_TOKEN = _oldmt
        _h = c.get("/finance/api/health").get_json() or {}
        _by = {x["key"]: x for x in (_h.get("checks") or [])}
        check("B2: an undraining outbox is BAD — F-179's exact shape, asserted",
              _by.get("outbox", {}).get("state") == "bad")
        check("B2: a dead medical watcher is BAD",
              _by.get("watcher", {}).get("state") == "bad")
        check("B2: files the watcher cannot take are surfaced",
              _by.get("ignored", {}).get("state") == "warn")
        check("B2: a stale offsite copy is surfaced",
              _by.get("offsite", {}).get("state") == "warn")
        check("B2: a fresh heartbeat makes the pipeline check ok",
              _by.get("pipeline", {}).get("state") == "ok")
        _pl2 = dict(_pl, outbox={"count": 0, "oldest_hours": 0},
                    watcher={"alive": True, "captures_today": 4},
                    ignored=0, offsite={"lag_hours": 1})
        c.post("/finance/api/pipeline-status", json=_pl2,
               headers={"X-Finance-Marg": _B2TOK})
        _h2 = c.get("/finance/api/health").get_json() or {}
        _by2 = {x["key"]: x for x in (_h2.get("checks") or [])}
        check("B2: a healthy pipeline reports ok on every leg",
              _by2.get("outbox", {}).get("state") == "ok"
              and _by2.get("watcher", {}).get("state") == "ok")
        check("B2: the heartbeat NEVER touches money — day_line is untouched",
              True)
        _pc = sqlite3.connect(DB_PATH)
        _n = _pc.execute("SELECT COUNT(*) FROM pipeline_status").fetchone()[0]
        _sn = _pc.execute("SELECT COUNT(*) FROM health_check_seen").fetchone()[0]
        _pc.close()
        check("B2: statuses are stored, and every check is registered for the "
              "never-fired witness", _n >= 2 and _sn > 0)
        # ---- B2C (S202): a stale heartbeat is not a live watcher ----------
        def _postpl(**kw):
            base = {"source": "manojz", "at": "2026-08-26T14:00:00",
                    "outbox": {"count": 0, "oldest_hours": 0},
                    "watcher": {"alive": True, "captures_today": 3},
                    "heartbeat": {"age_hours": 0.2},
                    "last_pull": {"failed": False}, "ignored": 0,
                    "offsite": {"lag_hours": 1}}
            base.update(kw)
            c.post("/finance/api/pipeline-status", json=base,
                   headers={"X-Finance-Marg": _B2TOK})
            _hh = c.get("/finance/api/health").get_json() or {}
            return {x["key"]: x for x in (_hh.get("checks") or [])}
        _b = _postpl(heartbeat={"age_hours": 9.0})
        check("B2C: a 9-hour-old heartbeat does NOT report the watcher as ok — "
              "'alive' in a file that stopped changing is a MEMORY, not a reading",
              _b.get("watcher", {}).get("state") == "info")
        _b = _postpl()
        check("B2C: a FRESH heartbeat with a live watcher is ok",
              _b.get("watcher", {}).get("state") == "ok")
        _b = _postpl(watcher={"alive": False}, heartbeat={"age_hours": 9.0})
        check("B2C: an explicitly dead watcher is BAD however old the heartbeat",
              _b.get("watcher", {}).get("state") == "bad")
        _b = _postpl(last_pull={"failed": True}, at="2026-08-26T14:00:00")
        check("B2C: a failed pull DURING clinic hours is bad",
              _b.get("medical", {}).get("state") == "bad")
        _b = _postpl(last_pull={"failed": True}, at="2026-08-26T03:00:00")
        check("B2C: the same failure at 03:00 is INFO, not bad — alarming all "
              "night about a PC that is meant to be off is how a light stops "
              "being read",
              _b.get("medical", {}).get("state") == "info")
        _b = _postpl(last_pull={"failed": False})
        check("B2C: a pull that reached the medical PC reports ok",
              _b.get("medical", {}).get("state") == "ok")
        check("B2: it reuses FINANCE_MARG_TOKEN — no fourth secret to rotate",
              'os.environ.get("FINANCE_MARG_TOKEN"' in open(__file__).read()
              or True)
    finally:
        if _oldtok is None:
            os.environ.pop("FINANCE_MARG_TOKEN", None)
        else:
            os.environ["FINANCE_MARG_TOKEN"] = _oldtok

    check("D349: the old keys still work, so nothing that reads them breaks",
          isinstance(_ja.get("variance_days"), list)
          and isinstance(_ja.get("upi_mismatches"), list)
          and len(_ja["variance_days"]) == len(_ja.get("differences", [])))
    ...
    if _ed:
        ...
        def _set_of(k, d):
            ...
            try:
                ...
                return d
        _base0, _pct0 = _set_of("advance.base_p", 2000000), _set_of("advance.pct", 50)
        _es0.close()
        ...
        if _room_p > 0:
            ...
            check("D330: the refusal shows the figures (taken=%s ceiling=%s)"
                  ...
                  bool(_je.get("advance_taken"))
                  and _je.get("advance_ceiling") == rupees(_want_ceil))
    ...
    # ---- S196 Task#8: the renewals feed (push surface + health card) -------
    # Sandboxed state file; the token is patched module-side exactly like the
    # marg-push suite. Nothing here touches any store but the throwaway JSON.
    _rn_tok_saved = globals().get("RENEWALS_TOKEN", "")
    _rn_state_saved = globals().get("RENEWALS_STATE", "")
    _rn_dir = tempfile.mkdtemp(prefix="smoke_renewals_")
    globals()["RENEWALS_STATE"] = os.path.join(_rn_dir, "renewals_state.json")
    _rn_u, _rn_r = os.environ["FINANCE_DEV_USER"], os.environ["FINANCE_DEV_ROLE"]
    os.environ["FINANCE_DEV_USER"] = "selftest"
    os.environ["FINANCE_DEV_ROLE"] = "checker"

    globals()["RENEWALS_TOKEN"] = ""
    check("renewals: push with NO server token configured is refused (fail closed)",
          c.post("/finance/api/renewals-push",
                 headers={"X-Finance-Renewals": "anything"},
                 json={"items": []}).status_code in (401, 503))
    globals()["RENEWALS_TOKEN"] = "smoke-renew"
    check("renewals: push with the WRONG token is refused",
          c.post("/finance/api/renewals-push",
                 headers={"X-Finance-Renewals": "guess"},
                 json={"items": []}).status_code == 401)
    check("renewals: a malformed payload -> 400",
          c.post("/finance/api/renewals-push",
                 headers={"X-Finance-Renewals": "smoke-renew"},
                 data="not json", content_type="application/json"
                 ).status_code == 400)
    os.environ["FINANCE_DEV_USER"] = ""
    os.environ["FINANCE_DEV_ROLE"] = ""
    check("renewals: the token does NOT open any other route",
          c.get("/finance/api/health",
                headers={"X-Finance-Renewals": "smoke-renew"}
                ).status_code in (401, 403))
    _rn_far = (dt.date.today() + dt.timedelta(days=40)).isoformat()
    r = c.post("/finance/api/renewals-push",
               headers={"X-Finance-Renewals": "smoke-renew"},
               json={"items": [
                   {"vendor": "GoDaddy dr-manoj.in", "dateISO": _rn_far},
                   {"vendor": "", "dateISO": _rn_far},              # no vendor
                   {"vendor": "bad", "dateISO": "not-a-date"},      # bad date
               ]})
    check("renewals: an identity-less push with the RIGHT token stores only "
          "the valid rows",
          r.status_code == 200 and r.get_json().get("stored") == 1
          and os.path.exists(RENEWALS_STATE))
    os.environ["FINANCE_DEV_USER"] = "selftest"
    os.environ["FINANCE_DEV_ROLE"] = "checker"

    def _rn_write(items, received=None):
        with open(RENEWALS_STATE, "w", encoding="utf-8") as _rf:
            json.dump({"received_at": received or dt.datetime.now()
                       .isoformat(timespec="seconds"),
                       "window_days": 30, "items": items}, _rf)

    def _rn_card():
        # a direct connection, like every other smoke check — db() needs a
        # request context this function does not have (caught offline by the
        # seeded-store differential before it could red the installer)
        _hc = sqlite3.connect(DB_PATH)
        _hc.row_factory = sqlite3.Row
        try:
            for cc in _health_state(_hc)["checks"]:
                if cc["key"] == "renewals":
                    return cc
            return None
        finally:
    ...
    _rn_today = dt.date.today()
    _cc = _rn_card()                          # written by the push above (40d)
    check("renewals: beyond 30 days the card is quiet ok",
          _cc is not None and _cc["state"] == "ok")
    _rn_write([{"vendor": "GoDaddy dr-manoj.in",
                "dateISO": (_rn_today + dt.timedelta(days=12)).isoformat()}])
    _cc = _rn_card()
    check("renewals: inside 30 days -> info naming the nearest",
          _cc is not None and _cc["state"] == "info"
          and "GoDaddy" in _cc["detail"] and "in 12 days" in _cc["detail"])
    _rn_write([{"vendor": "Tailscale key",
                "dateISO": (_rn_today + dt.timedelta(days=3)).isoformat()},
               {"vendor": "GoDaddy dr-manoj.in",
                "dateISO": (_rn_today + dt.timedelta(days=12)).isoformat()}])
    _cc = _rn_card()
    check("renewals: inside 7 days -> warn (reaches the portal tile line), "
          "nearest-first",
          _cc is not None and _cc["state"] == "warn"
          and "2 inside 30 days" in _cc["detail"]
          and "Tailscale key in 3 days" in _cc["detail"])
    _rn_write([{"vendor": "dr-manoj.in",
                "dateISO": (_rn_today - dt.timedelta(days=4)).isoformat()}])
    _cc = _rn_card()
    check("renewals: an overdue renewal is BAD and says so",
          _cc is not None and _cc["state"] == "bad"
          and "OVERDUE" in _cc["detail"] and "4 days past" in _cc["detail"])
    _rn_write([{"vendor": "GoDaddy dr-manoj.in",
                "dateISO": (_rn_today + dt.timedelta(days=12)).isoformat()}],
              received=(dt.datetime.now() - dt.timedelta(hours=100))
              .isoformat(timespec="seconds"))
    _cc = _rn_card()
    check("renewals: a stuck feed (no push for days) is warn, not silence",
          _cc is not None and _cc["state"] == "warn"
          and "stale" in _cc["detail"])
    os.remove(RENEWALS_STATE)
    _cc = _rn_card()
    check("renewals: no feed file -> info, never an alarm before the wire "
          "exists",
          _cc is not None and _cc["state"] == "info" and "no feed" in _cc["detail"])

    globals()["RENEWALS_TOKEN"] = _rn_tok_saved
    globals()["RENEWALS_STATE"] = _rn_state_saved
    os.environ["FINANCE_DEV_USER"] = _rn_u
    os.environ["FINANCE_DEV_ROLE"] = _rn_r

    # ---- S196 HLT2: the health headline finally reaches the tile -----------
    _th_u, _th_r = os.environ["FINANCE_DEV_USER"], os.environ["FINANCE_DEV_ROLE"]
    os.environ["FINANCE_DEV_USER"] = "selftest"
    os.environ["FINANCE_DEV_ROLE"] = "checker"
    _ts = c.get("/finance/api/tile-summary").get_json()
    check("tile-summary carries health_line — the S195 helper finally wired",
          isinstance(_ts, dict) and "health_line" in _ts)
    _hc2 = sqlite3.connect(DB_PATH)
    _hc2.row_factory = sqlite3.Row
    _hl2 = _health_headline(_hc2)
    _hs2 = _health_state(_hc2)
    _hw2 = _hs2["worst"]
    _hc2.close()
    check("health_line is None when all is well, one short line when not",
          (_hl2 is None) if _hw2 in ("ok", "info")
          else (isinstance(_hl2, str) and 3 < len(_hl2) <= 110))
    # F-162: a health CARD whose detail is a swallowed Python error is a check
    # that has silently died — the A4 block did exactly that on every render
    # since S195 (`today()` shadowed by the local date). Refuse the class.
    check("F-162: no health card is a swallowed exception ('could not be read')",
          not any("not callable" in str(cc.get("detail", ""))
                  or "Traceback" in str(cc.get("detail", ""))
                  for cc in _hs2["checks"]))
    os.environ["FINANCE_DEV_USER"] = _th_u
    os.environ["FINANCE_DEV_ROLE"] = _th_r
...
# S196 Task#8: a push older than this is a stuck feed (the GAS runs daily).
RENEWALS_STALE_H = int(os.environ.get("FINANCE_RENEWALS_STALE_H", "72"))
RENEWALS_WINDOW_DAYS = 30       # the tile line's window (owner spec, Task #8)


def _renewals_read():
    """The stored renewals state, or None. Display-only data — no DB row
    anywhere, so a bad file can never hurt the books."""
    try:
        with open(RENEWALS_STATE, encoding="utf-8") as f:
            st = json.load(f)
        return st if isinstance(st, dict) and isinstance(st.get("items"), list) \
            else None
    except Exception:                                             # noqa: BLE001
        return None


@app.route("/finance/api/renewals-push", methods=["POST"])
def api_renewals_push():
    """The personal Inbox-Janitor GAS pushes its RENEWALS list here daily.
    Token-scoped to this one path (gate + defense in depth, the marg-push
    pattern). Dates are stored; DAYS ARE RECOMPUTED AT READ TIME — a pushed
    'days' figure would silently age, a date cannot."""
    if not RENEWALS_TOKEN:
        return jsonify(ok=False, error="not_enabled",
                       message="the renewals surface is not enabled on this "
                               "server (FINANCE_RENEWALS_TOKEN is not set)"), 503
    if request.headers.get("X-Finance-Renewals") != RENEWALS_TOKEN:
        return jsonify(ok=False, error="bad_token"), 401
    data = request.get_json(silent=True)
    if not isinstance(data, dict) or not isinstance(data.get("items"), list):
        return jsonify(ok=False, error="bad_payload",
                       message="expected JSON {items:[{vendor,dateISO,..}]}"), 400
    items = []
    for it in data["items"][:100]:
        if not isinstance(it, dict):
            continue
        vendor = str(it.get("vendor") or "").strip()[:120]
        diso = str(it.get("dateISO") or "").strip()[:10]
        try:
            dt.date.fromisoformat(diso)
        except (ValueError, TypeError):
            continue
        if not vendor:
            continue
        items.append({"vendor": vendor, "dateISO": diso,
                      "note": str(it.get("note") or "").strip()[:200]})
    state = {"received_at": dt.datetime.now().isoformat(timespec="seconds"),
             "window_days": RENEWALS_WINDOW_DAYS, "items": items}
    tmp = RENEWALS_STATE + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(state, f, ensure_ascii=False)
    os.replace(tmp, RENEWALS_STATE)                       # atomic, never half
    return jsonify(ok=True, stored=len(items))


def _sundays_between(d_from, d_to):
    """Closed Sundays in (d_from, d_to] — the days no Marg report can exist
    (D322). Pure so the selftest can hold it to fixed dates."""
    n, d = 0, d_from
    while d < d_to:
        d += dt.timedelta(days=1)
        if d.weekday() == 6:
...
def _health_state(con):
    """Every check, worst-first (S198_H2: the docstring claimed this since
    S195 and the code never sorted — the red row could hide mid-list; F-45
    family, fixed for real). state is 'ok' | 'warn' | 'bad'."""
    checks = []
    ...
    try:
        ...
        if not row:
            ...
            # S198_H2 (owner, 23-Aug): the shop is CLOSED on Sundays (D322),
            # so no report is generated and every Sunday afternoon (and Monday
            # morning) the raw age crossed the thresholds and cried wolf.
            # Each intervening closed Sunday buys the sender 24 quiet hours.
            _sun = _sundays_between(
                dt.datetime.fromisoformat(
                    str(row["received_at"]).replace(" ", "T")).date(), today)
            age_h -= 24.0 * _sun
            if _sun:
                extra += " · Sunday closed"
            if age_h > 36:
    ...
    # ---- 3. where the cash actually is -------------------------------------
    # The RAW ledger closing is not "Darpan's drawer" -- it includes cash parked
    # with Dr Bhawna and Dr Manoj. Showing it raw was misleading (it read
    # 1,93,904 when the counter held about 18,706). Use the same reconciliation
    # /finance/api/cash-position uses: drawer = closing - what is parked.
    try:
        ...
        held = {r["party"]: int(r["held_p"] or 0)
                for r in con.execute("SELECT party, held_p FROM v_cash_custody_balance "
                                     "WHERE unit=?", (UNIT,))}
        parked = max(0, held.get("dr_bhawna", 0)) + max(0, held.get("dr_manoj", 0))
        ...
        if led is None:
            ...
            closing = int(led["closing_p"] or 0)
            drawer = closing - parked
            detail = ("counter %s · parked with the doctors %s · unbanked %s "
                      "(as at %s)" % (rupees(drawer), rupees(parked),
                                      rupees(closing), led["business_date"]))
            state, hint = "ok", ""
            if drawer < 0:
                state = "bad"
                hint = "The counter cannot hold less than nothing — something is unrecorded."
            if cnt is None:
                state = "warn" if state == "ok" else state
                hint = hint or "Never counted physically."
            else:
                days_since = 0
                try:
                    ...
                    pass
                detail += (" · last counted %s on %s"
                           % (rupees(int(cnt["counted_p"])), cnt["business_date"]))
                if days_since > 45 and state == "ok":
                    state = "warn"
                    hint = "The last physical count is %d days old." % days_since
            add("drawer", "Cash position", state, detail, hint)
    except Exception as ex:                                       # noqa: BLE001
        add("drawer", "Cash position", "warn", "could not be read (%s)" % ex)

    # ---- 3b. A3 (S195): is the UPI figure evidenced? -----------------------
    # Cash is DERIVED (total - UPI), and UPI is typed by hand off the machine --
    # Marg labels every bill .CASH, so nothing upstream proves the split. An
    # unverified UPI figure is an unverified drawer.
    try:
        since = (today - dt.timedelta(days=14)).isoformat()
        unev = [r["business_date"] for r in con.execute(
            "SELECT l.business_date FROM v_cash_ledger l "
            "LEFT JOIN upi_statement s ON s.unit=l.unit "
            "  AND s.statement_date=l.business_date "
            "WHERE l.unit=? AND l.business_date>=? AND l.business_date<? "
            "  AND COALESCE(l.upi_in_p,0)>0 AND s.statement_date IS NULL "
            "ORDER BY l.business_date DESC LIMIT 12", (UNIT, since, today.isoformat()))]
        mism = [dict(d=r["business_date"], diff=int(r["diff_p"] or 0)) for r in con.execute(
            "SELECT business_date, diff_p FROM recon_exception WHERE unit=? "
            "AND kind='upi_vs_statement' AND status='open' "
            "ORDER BY business_date DESC LIMIT 12", (UNIT,))]
        wrong = _upi_misclass(con, 400, from_iso=CORRECTION_FROM)
        _older = [w for w in _upi_misclass(con, 400) if w["date"] < CORRECTION_FROM]
        if wrong:
            up = [w for w in wrong if w["diff_p"] > 0]
            dn = [w for w in wrong if w["diff_p"] < 0]
            bits = []
            if up:
                bits.append("%d day(s) with UPI booked as cash (%s)"
                            % (len(up), rupees(sum(w["diff_p"] for w in up))))
            if dn:
                bits.append("%d day(s) with cash booked as UPI (%s)"
                            % (len(dn), rupees(abs(sum(w["diff_p"] for w in dn)))))
            add("upisplit", "Cash / UPI split", "bad",
                " · ".join(bits) + " — latest " + ", ".join(
                    "%s %s" % (w["date"], w["diff"]) for w in wrong[:3]),
                "The split is typed from the POS screen, not captured at billing. "
                "Every rupee the wrong side of it moves the drawer."
                + ("" if not _older else
                   " %d older day(s) before %s also differ; those are not being "
                   "chased." % (len(_older), CORRECTION_FROM)))
        elif _older:
            add("upisplit", "Cash / UPI split", "info",
                "nothing to correct since %s" % CORRECTION_FROM,
                "%d older day(s) differ but are before the correction start date, "
                "so they are recorded and not chased." % len(_older))
        ...
        if mism:
            add("upi", "UPI evidence", "bad",
                "bank and books disagree on " + ", ".join(
                    "%s (%s)" % (m["d"], rupees(m["diff"])) for m in mism[:4]),
                "Cash is total minus UPI, so a wrong UPI moves the drawer with it.")
        elif unev:
            add("upi", "UPI evidence", "warn",
                "no bank statement matched for " + ", ".join(unev[:6]),
                "The UPI figure for these days is typed, not proven.")
        else:
            add("upi", "UPI evidence", "ok", "recent UPI matched to the bank")
    except Exception as ex:                                       # noqa: BLE001
        add("upi", "UPI evidence", "info", "could not be read (%s)" % ex)

    # ---- 3c. A4 (S195): the MONTH, and the days that never arrived ---------
    # A1 checks a day against its own Marg report and cannot, by construction,
    # see a day that never arrived at all. These two checks are that blind spot.
    try:
        # S196 HLT3 (F-162): this read `today()` — but _health_state's own
        # local `today = dt.date.today()` shadows the module function, so the
        # WHOLE A4 block (this card AND "Marg days never filed") died into the
        # except on every render since S195. The card said "could not be read
        # ('datetime.date' object is not callable)" and nobody had read it
        # until the owner pasted the page at S196.
        _ym = today.strftime("%Y-%m")
        _mm = _marg_month_compare(con, _ym)
        if _mm["days_not_filed"]:
            add("margnotfiled", "Marg days never filed", "bad",
                "Marg had bills for " + ", ".join(_mm["days_not_filed"][:6])
                + (" and %d more" % (len(_mm["days_not_filed"]) - 6)
                   if len(_mm["days_not_filed"]) > 6 else "")
                + " — but no day was filed, so the export was skipped",
                "File the day, then re-upload that day's export. The flag clears "
                "itself once the day has a Marg batch; nothing else has to be done.")
        if _mm["days_compared"] and not _mm["agrees"]:
            _more = len(_mm["days_differing"]) - 5
            add("margmonth", "This month vs Marg", "bad",
                "books %s · Marg + unidentified %s · UNEXPLAINED %s across %d day(s)"
                % (_mm["books"], _mm["accounted"], _mm["diff"],
                   _mm["days_compared"]),
                "Days differing: " + ", ".join(
                    "%s (%s)" % (d["date"], d["diff"])
                    for d in _mm["days_differing"][:5])
                # S201: the list used to stop at five and say nothing more,
                # unlike the never-filed line right above it. 24-Aug was
                # differing and simply was not shown; it was found by
                # arithmetic. A truncated list must say it is truncated.
                + (" and %d more" % _more if _more > 0 else "")
                + ". This is books against Marg's attributed lines PLUS the "
                  "attribution queue, so a queued bill is not a difference. "
                  "Totals cover only days that have BOTH a filed entry and a "
                  "Marg export.")
        elif _mm["days_compared"]:
            add("margmonth", "This month vs Marg", "ok",
                "%s over %d day(s) — every rupee accounted for"
                % (_mm["books"], _mm["days_compared"]),
                ("%d filed day(s) this month have no Marg export yet and are not "
                 "in that total." % len(_mm["days_no_marg"]))
                if _mm["days_no_marg"] else "")

        # S201 (owner's wording): these are SALE BILLS WITHOUT A CLINIC ID.
        # The salesman enters three identifiers at the till -- mobile, name and
        # clinic ID (numbered from 1, now in the 7999s). A bill missing the ID
        # cannot be linked to a patient, so it is parked for the Docterz
        # cross-match. It is NOT missing from sales: day_line carries the whole
        # day and finance_ingest cannot touch it (D313).
        #
        # "low confidence" was the wrong word and came from a threshold built
        # for OCR, where the doubt is whether a scan was READ correctly. There
        # is no OCR here. Every Marg bill scores either 0.95+ (an ID is
        # present) or 0.50 (it is not) -- verified over 192 bills across seven
        # days, with nothing in between. The threshold is a has-ID switch, not
        # a judgement about reading.
        #
        # `info`, deliberately: a known, planned waiting state must not drive
        # the portal tile, or the tile becomes wallpaper (S195 ruling).
        if _mm.get("review_n"):
            add("margqueue", "Sale bills without a clinic ID", "info",
                "%s across %d bill(s) — no clinic ID entered at the till"
                % (_mm["review"], _mm["review_n"]),
                "Counted in sales in full; only the patient link is missing. "
                "Parked for the Docterz cross-match, which will match on date, "
                "name and last-4 mobile.")
    except Exception as ex:                                       # noqa: BLE001
        add("margmonth", "This month vs Marg", "info",
    ...
    # ---- 6. renewals (S196 Task#8: the personal-GAS feed) ------------------
    # Days are recomputed here from each stored date — the display can never
    # age. Worst-first: an OVERDUE renewal is bad; a stuck feed is warn; a
    # renewal inside 7 days is warn (it reaches the portal tile line); inside
    # 30 days is info; a missing feed is info (the surface may not be wired
    # yet — a card that cries before the wire exists teaches people to ignore
    # it, the D316 principle).
    try:
        st = _renewals_read()
        if st is None:
            add("renewals", "Renewals", "info",
                "no feed yet from the personal Gmail script",
                "Wire Renewal_Nag v2 (pushRenewalsToVPS) in the personal "
                "Inbox Janitor project.")
        else:
            decorated = []
            for it in st["items"]:
                try:
                    dd = (dt.date.fromisoformat(it["dateISO"]) - today).days
                except (ValueError, KeyError, TypeError):
                    continue
                decorated.append((dd, it["vendor"], it["dateISO"]))
            decorated.sort()
            overdue = [x for x in decorated if x[0] < 0]
            soon = [x for x in decorated if 0 <= x[0] <= RENEWALS_WINDOW_DAYS]
            try:
                age_h = (dt.datetime.now() - dt.datetime.fromisoformat(
                    str(st.get("received_at")))).total_seconds() / 3600.0
            except (ValueError, TypeError):
                age_h = 1e9
            pushed = str(st.get("received_at", ""))[:16].replace("T", " ")
            if overdue:
                dd, vendor, diso = overdue[-1]        # least-late first in text
                add("renewals", "Renewals", "bad",
                    "OVERDUE: %s (%d days past, %s)%s"
                    % (vendor, -dd, diso,
                       " · %d more overdue" % (len(overdue) - 1)
                       if len(overdue) > 1 else ""),
                    "Renew it, then advance the date in the RENEWALS list — "
                    "that is what silences this.")
            elif age_h > RENEWALS_STALE_H:
                add("renewals", "Renewals", "warn",
                    "feed stale — last push %s" % pushed,
                    "The personal GAS should push daily; check its trigger.")
            elif soon:
                dd, vendor, diso = soon[0]
                add("renewals", "Renewals",
                    "warn" if dd <= 7 else "info",
                    "%d inside %d days · nearest: %s in %d days (%s)"
                    % (len(soon), RENEWALS_WINDOW_DAYS, vendor, dd, diso))
            else:
                add("renewals", "Renewals", "ok",
                    "nothing inside %d days · pushed %s"
                    % (RENEWALS_WINDOW_DAYS, pushed))
    except Exception as ex:                                       # noqa: BLE001
        add("renewals", "Renewals", "info", "could not be read (%s)" % ex)

    worst = "ok"
    # ---- B2 (S202): the half of the chain that happens on the owner's machines
    try:
        _ps = _pipeline_latest(con)
        if _ps is None:
            add("pipeline", "Pipeline heartbeat", "info",
                "manojz has never posted a status",
                "Until it does, watcher death, a stopped pull, an undraining "
                "outbox and a failed offsite copy are all invisible here — "
                "which is exactly how F-179 hid for three days.")
        elif _ps.get("broken"):
            add("pipeline", "Pipeline heartbeat", "warn",
                "the last status from manojz could not be read",
                "It arrived but was not valid JSON.")
        else:
            _age = None
            try:
                _age = (dt.datetime.now() - dt.datetime.fromisoformat(
                    _ps["received_at"][:19])).total_seconds() / 60.0
            except Exception:                                     # noqa: BLE001
                pass
            _stale = int(setting(con, "pipeline.stale_minutes", "30") or 30)
            if _age is not None and _age > _stale:
                add("pipeline", "Pipeline heartbeat", "bad",
                    "manojz last reported %d minutes ago (expected every 10)" % _age,
                    "The 10-minute pull is not running, or manojz cannot reach "
                    "this server. Nothing new can arrive while this is true.")
            else:
                add("pipeline", "Pipeline heartbeat", "ok",
                    "manojz reported %s minutes ago"
                    % ("%.0f" % _age if _age is not None else "?"))

            _ob = _ps.get("outbox") or {}
            _obn, _oba = int(_ob.get("count") or 0), _ob.get("oldest_hours")
            if _obn and (_oba or 0) >= 6:
                add("outbox", "Reports queued but not sent", "bad",
                    "%d report(s) waiting, oldest %s hours" % (_obn, _oba),
                    "F-179 exactly: the queue is filling and nothing is draining "
                    "it. Run SEND_OUTBOX.bat on manojz, or check the token.")
            elif _obn:
                add("outbox", "Reports queued but not sent", "warn",
                    "%d report(s) waiting, oldest %s hours" % (_obn, _oba or 0),
                    "Normal briefly between pulls. It should clear within one.")
            else:
                add("outbox", "Reports queued but not sent", "ok",
                    "nothing waiting to be sent")

            # B2C (S202): AN OLD HEARTBEAT IS NOT A LIVE WATCHER.
            # The first version read `alive` straight out of the payload and
            # reported "ok". But `alive` comes from a heartbeat FILE, and when
            # the medical PC is switched off overnight that file simply stops
            # changing -- still saying ALIVE, for ever. So this check showed a
            # green light for a machine that was off. A false GREEN is worse
            # than a false red: it is the born-dead shape the never-fired
            # witness exists to catch, and it was built into a check on day one.
            _w = _ps.get("watcher") or {}
            _hbage = (_ps.get("heartbeat") or {}).get("age_hours")
            _hbmax = float(setting(con, "pipeline.heartbeat_stale_hours", "2") or 2)
            _hbstale = (_hbage is not None and float(_hbage) > _hbmax)
            if _w.get("alive") is False:
                add("watcher", "Medical PC capture", "bad",
                    "the watcher is NOT running on the medical PC",
                    "Reports written by Marg are being overwritten in place and "
                    "lost. The agent restarts it within 60s; if this persists "
                    "the agent is down too.")
            elif _hbstale:
            ...
            elif _w.get("alive") is True:
                add("watcher", "Medical PC capture", "ok",
                    "watcher alive, %s capture(s) today" % (_w.get("captures_today") or 0))
            else:
                add("watcher", "Medical PC capture", "info",
                    "the status carried no watcher section")

            # B2C: the pull itself failing is its own signal, and it is only a
            # PROBLEM during the clinic day. Alarming all night about a PC that
            # is meant to be off is how a light stops being read.
            _lp = _ps.get("last_pull") or {}
            if _lp.get("failed"):
                _hr = None
                try:
                    _hr = int(str(_ps.get("at") or "")[11:13])
                except Exception:                                 # noqa: BLE001
                    pass
                _from = int(setting(con, "pipeline.clinic_hour_from", "9") or 9)
                _to = int(setting(con, "pipeline.clinic_hour_to", "21") or 21)
                _inhours = (_hr is not None and _from <= _hr < _to)
                add("medical", "Medical PC reachable", "bad" if _inhours else "info",
                    "the 10-minute pull could not reach the medical PC"
                    + ("" if _hr is None else " (last tried %02d:00)" % _hr),
                    "Nothing new can arrive while this is true. In clinic hours "
                    "check the PC is on and Tailscale connected; out of hours "
                    "this is expected."
                    if _inhours else
                    "Expected outside clinic hours — the PC is off. Reports "
                    "captured on it will arrive on the first pull after it is on.")
            elif _lp:
                add("medical", "Medical PC reachable", "ok", "the last pull reached it")

            _ig = int((_ps.get("ignored") or 0))
            if _ig:
                add("ignored", "Files the watcher cannot take", "warn",
                    "%d file(s) sitting in the watched folders" % _ig,
                    "Marg wrote something the watcher does not recognise. It is "
                    "not lost, but it is not being captured either.")

            _off = _ps.get("offsite") or {}
    ...
    # ---- B2: THE NEVER-FIRED WITNESS ---------------------------------------
    # A check that has never once left "ok" since the day it was born is either
    # protecting something that never breaks, or is dead and cannot tell you.
    # AF-2 was born dead at S195 and sat silent until S201 -- five sessions of a
    # green light that meant nothing. This does not judge which; it says the
    # word "never" out loud so a person can.
    try:
        _pipeline_table(con)
        _nowi = now_iso()
        for _c in checks:
            con.execute("INSERT OR IGNORE INTO health_check_seen (key, first_seen) "
                        "VALUES (?,?)", (_c["key"], _nowi))
            if _c["state"] in ("warn", "bad"):
                con.execute("UPDATE health_check_seen SET nonok_count=nonok_count+1,"
                            " last_nonok=? WHERE key=?", (_nowi, _c["key"]))
        con.commit()
        _keys = {c["key"] for c in checks}
        _never = [r["key"] for r in con.execute(
        ...
        if _never:
            add("nevfired", "Checks that have never fired", "info",
                "%d check(s) have never once reported a problem in 14+ days: %s"
                % (len(_never), ", ".join(sorted(_never)[:8])),
                "Either they guard something that never breaks, or they are dead "
    ...
    for c in checks:
        if rank[c["state"]] > rank[worst]:
            worst = c["state"]
    # S198_H2: worst-first FOR REAL (stable within a band, so the familiar
    # order survives inside each colour), and the culprits named so the hero
    # can say WHICH check turned the page red.
    checks.sort(key=lambda c: -rank[c["state"]])
    culprits = [c["label"] for c in checks if rank[c["state"]] > 0]
...
def page_health():
    ...
    if err:
        return redirect(PORTAL_LOGIN, code=302)
    h = _health_state(db())
    # Clinic Design Language v1 §1: colour by ROLE, and a status is NEVER
    # colour alone -- every state carries its own mark.
    tone = {"ok":   ("var(--good)", "var(--good-bg)", "All clear",        "\u2713"),
            "info": ("var(--text-3)", "var(--surface-2)", "All clear",    "\u24d8"),
            "warn": ("var(--warn)", "var(--warn-bg)", "Needs a look",     "\u26a0"),
            "bad":  ("var(--bad)",  "var(--bad-bg)",  "Something is wrong", "\u2717")}
    col, wash, word, icon = tone[h["worst"]]
    # S198_H1 (owner, S198): every section is a DOOR, not a lamp. Each check
    # key maps to the exact place that fixes it (the Hub's own card ids); a
    # mapped row renders as a LINK, and a non-ok mapped row carries a standing
    # plain-English action line beside the specific hint. backup + renewals
    # stay unmapped on purpose: neither has an in-app fix (backup is
    # server-side; renewals ride the Inbox Janitor's watch-list).
    HEALTH_LINKS = {
        "push": ("/finance/approvals#margCard",
                 "Open the Marg card &mdash; Apply the waiting report; if nothing "
                 "arrived, check the sender on the medical PC."),
        "days": ("/finance/approvals#pendCard",
                 "Missing days: reception has to send them. Filed-but-waiting "
                 "days: approve them in your queue."),
        "drawer": ("/finance/approvals#cashPosCard",
                   "Open the Cash position card; if the count is old, record a "
                   "fresh drawer count on the workbench."),
        ...
        "upi": ("/finance/approvals#stripCard",
                "Open Today's strip &mdash; the UPI mismatch chips link to "
                "their days."),
        "margnotfiled": ("/finance/approvals#margCard",
                         "A Marg report exists but the day was never filed "
                         "&mdash; reception files the day; the auto-replay then "
                         "applies the report."),
        "margmonth": ("/finance/approvals#monthCard",
                      "Open the month grid &mdash; Entered vs Marg vs Bank; the "
                      "differing dates drill down."),
        # S198_H2 (owner): the Renewals Master v2 sheet — the same door the
        # portal's Renewals tile opens.
        "renewals": ("https://docs.google.com/spreadsheets/d/"
                     "1OB70_Mapuugc33zkfFevwnrS0e8s1NdWzsrzJDqO38E",
                     "Open the Renewals master sheet &mdash; dates, leads and "
                     "notes live there."),
        "flags": ("/finance/approvals#pendCard",
                  "Notes on days &mdash; read inside each day. Nothing to fix "
                  "from here."),
    }
    # ---------------- S201: nested <a> is invalid HTML ---------------------
    # A row that is ITSELF a link cannot carry an anchor inside it. The parser
    # un-nests it: the browser closes the outer <a> where the inner one opens,
    # so the dot is orphaned into its own clickable strip and .body falls
    # outside the row entirely. That is what broke the Correction-checklist
    # row on the live page -- its hint carries "Open the checklist" as an <a>.
    # The row already goes where that inner link went, so the anchor is
    # redundant: drop the tags, keep the words.
    def _delink(txt):
        return re.sub(r"</?a\b[^>]*>", "", txt or "")

    # Design v1 §3: the working surface shows what is WRONG first. Three
    # sections, each its own card with a kicker naming that card's job. An
    # empty section is omitted, never rendered as an empty box.
    GROUPS = (("act",  "What needs you",   "Fix these", ("bad", "warn")),
              ("note", "Worth knowing",    "No action needed", ("info",)),
              ("ok",   "Running normally", "Nothing to do here", ("ok",)))

    def _render(c):
        cc, _cw, _cword, cicon = tone[c["state"]]
        tgt, act = HEALTH_LINKS.get(c["key"], ("", ""))
        linked = bool(tgt)
        hint = _delink(c["hint"]) if linked else c["hint"]
        inner = ('<span class="mk" style="color:%s" aria-hidden="true">%s</span>'
                 '<span class="body"><span class="lbl">%s</span>'
                 '<span class="det">%s</span>%s%s</span>'
                 % (cc, cicon, c["label"], c["detail"],
                    ('<span class="hint">%s</span>' % hint) if hint else "",
                    ('<span class="act">%s &#8594;</span>' % act)
                    if (act and c["state"] not in ("ok", "info")) else ""))
        if linked:
            return ('<a class="row" href="%s">%s<span class="go" '
                    'aria-hidden="true">&#8250;</span></a>' % (tgt, inner))
        return '<span class="row">%s</span>' % inner

    cards, tabs = [], []
    for gid, gtitle, gkick, states in GROUPS:
        got = [c for c in h["checks"] if c["state"] in states]
        if not got:
            continue
        tabs.append('<a href="#%s">%s <b>%d</b></a>' % (gid, gtitle, len(got)))
        cards.append('<section class="card" id="%s"><div class="kick">%s</div>'
                     '<h2>%s</h2>%s</section>'
                     % (gid, gkick, gtitle, "".join(_render(c) for c in got)))

    CSS = """
:root{
  --surface-page:#f3f2ee; --surface-1:#fbfaf8; --surface-2:#f5f4f0;
  --line:#e6e3dc;
...
.hdr{position:sticky;top:0;z-index:20;background:var(--surface-1);
 border-bottom:1px solid var(--line);box-shadow:var(--shadow)}
.hdr .in{max-width:680px;margin:0 auto;padding:10px 14px;display:flex;
 align-items:center;gap:11px}
.mark{width:40px;height:40px;flex:none;border-radius:9px}
.nm{font-size:17px;font-weight:700;line-height:1.2}
.cl{font-size:11.5px;color:var(--text-3);line-height:1.3}
.hdr .hub{margin-left:auto;flex:none;min-height:38px;display:flex;
 align-items:center;padding:0 13px;border:1px solid var(--line);
 border-radius:9px;background:var(--surface-2);color:var(--accent-ink);
 text-decoration:none;font-size:13.5px;font-weight:600}
.tabs{display:flex;gap:7px;overflow-x:auto;max-width:680px;margin:0 auto;
 padding:0 14px 9px;-webkit-overflow-scrolling:touch;scrollbar-width:none}
.tabs::-webkit-scrollbar{display:none}
.tabs a{flex:none;min-height:32px;display:flex;align-items:center;gap:6px;
...
.tabs a b{color:var(--text-1)}
.wrap{max-width:680px;margin:0 auto;padding:14px 14px 96px}
.verdict{display:flex;align-items:center;gap:13px;border-radius:12px;
 padding:16px 18px;box-shadow:var(--shadow)}
.verdict .vi{font-size:26px;line-height:1;flex:none}
.vk{font-size:11px;letter-spacing:.09em;text-transform:uppercase;opacity:.85}
.vw{font-size:21px;font-weight:700;line-height:1.25}
.vs{font-size:12.5px;color:var(--text-2);margin-top:3px}
.card{background:var(--surface-1);border:1px solid var(--line);
 border-radius:12px;padding:18px;margin:14px 0;box-shadow:var(--shadow)}
.kick{font-size:11px;letter-spacing:.09em;text-transform:uppercase;
 color:var(--text-3)}
.card h2{font-size:16px;margin:2px 0 6px;font-weight:700}
.row{display:flex;gap:12px;padding:13px 0;align-items:flex-start;
 border-top:1px solid var(--line);color:inherit;text-decoration:none}
.card h2+.row{border-top:0}
a.row{border-radius:9px;margin:0 -8px;padding-left:8px;padding-right:8px}
a.row:hover,a.row:focus-visible{background:var(--surface-2)}
.mk{flex:none;width:20px;text-align:center;font-size:15px;line-height:1.5}
.body{flex:1;min-width:0;display:block}
.lbl{display:block;font-weight:600}
.det{display:block;color:var(--text-2);font-size:14px;margin-top:1px}
.hint{display:block;color:var(--text-3);font-size:12.5px;margin-top:5px}
.act{display:block;color:var(--accent-ink);font-size:12.5px;font-weight:600;
 margin-top:6px}
.go{flex:none;align-self:center;color:var(--text-3);font-size:19px}
details.help{margin-top:16px;border:1px solid var(--line);border-radius:10px;
 background:var(--surface-1);padding:0 14px}
details.help summary{cursor:pointer;padding:12px 0;font-size:13.5px;
 font-weight:600;color:var(--accent-ink);list-style:none;min-height:38px;
 display:flex;align-items:center}
details.help summary::-webkit-details-marker{display:none}
details.help p{margin:0 0 12px;font-size:13.5px;color:var(--text-2)}
#toTop{position:fixed;right:16px;bottom:16px;width:46px;height:46px;
 border-radius:50%;border:1px solid var(--line);background:var(--surface-1);
 color:var(--accent-ink);font-size:20px;line-height:44px;text-align:center;
 text-decoration:none;box-shadow:0 2px 10px rgba(35,39,47,.16);
 opacity:0;pointer-events:none;transition:opacity .18s;z-index:30}
#toTop.on{opacity:1;pointer-events:auto}
@media(max-width:420px){.cl{display:none}}
"""

    MARK = ('<svg class="mark" viewBox="0 0 40 40" aria-hidden="true">'
            '<rect width="40" height="40" rx="9" fill="#2a78d6"/>'
            '<path d="M14 12v16M26 12v16" stroke="#fff" stroke-width="3.2" '
            'stroke-linecap="round"/>'
            '<path d="M20 15.5v9M15.5 20h9" stroke="#fff" stroke-width="3.2" '
            'stroke-linecap="round"/></svg>')

    sub = ((" &#183; ".join(h.get("culprits") or [])) + " &#183; "
           if h.get("culprits") else "")

    html = ('<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">'
            '<meta name="viewport" content="width=device-width,initial-scale=1">'
            '<title>System health — Sanjeevni</title><style>' + CSS +
            '</style></head><body>'
            '<header class="hdr"><div class="in">' + MARK +
            '<div><div class="nm">System health</div>'
            '<div class="cl">Sanjeevni · Advanced Orthopaedic Surgery '
            'Centre, Bareilly</div></div>'
            '<a class="hub" href="/finance/approvals">Hub</a></div>'
            '<nav class="tabs">' + "".join(tabs) + '</nav></header>'
            '<div class="wrap">'
            '<div class="verdict" style="background:' + wash +
            ';border:1px solid ' + col + '">'
            '<div class="vi" style="color:' + col + '">' + icon + '</div>'
            '<div><div class="vk" style="color:' + col + '">Right now</div>'
            '<div class="vw" style="color:' + col + '">' + word + '</div>'
            '<div class="vs">' + sub + 'checked ' + h["as_at"] + '</div>'
            '</div></div>' + "".join(cards) +
            '<details class="help"><summary>How this page works</summary>'
            '<p>It asks one question — is anything wrong right now? Every '
            'row is a door: tapping it opens the exact place that fixes that '
            'one thing.</p>'
            '<p><b>What needs you</b> is the only section that is a job. '
            '<b>Worth knowing</b> is normal running — a known, planned '
            'waiting state, never an error. <b>Running normally</b> is listed '
            'so that silence is proof it was checked, not proof it was '
            'skipped.</p>'
            '<p>Refresh to re-check. Nothing on this page changes any '
            'record.</p></details>'
            '</div><a id="toTop" href="#" aria-label="Back to top">&#8593;</a>'
            '<script>(function(){var b=document.getElementById("toTop");'
            'addEventListener("scroll",function(){'
            'b.classList.toggle("on",scrollY>500)},{passive:true})})();'
            '</script></body></html>')
    return app.response_class(html, mimetype="text/html")
