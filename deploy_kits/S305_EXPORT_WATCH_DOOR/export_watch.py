...
S305 (D467 phase 2c) -- THE SERVER'S OWN EVIDENCE. The exports also reach this box through its one door
(marg_take -> mi_file), pushed straight from the medical PC. A day is judged on either route: the clinic
PC's parsed feed (purchase_export / stock_feed) OR the door's own VERIFIED record (mi_file). And "unknown"
now means neither route has reported since the day ended -- when the clinic PC is asleep but the medical PC
pushed, the day is judged, not shrugged at. The "have" JSON names the route ("via": pc / door).

READS ONLY: /root/punches.csv, /root/staff_register/staff_register.db (to find Amir's machine id),
finance.db purchase_export / stock_feed / purchase_feed / emp_code / mi_file (S305). WRITES ONLY export_watch.
"""
import argparse
import csv
...
def evidence(con, day):
    ...
    for typ, pf, pt, st in con.execute(
        ...
        if key in ("BILLWISE", "LINES"):
            prev = have.get(key)
            if ok or not prev:
                have[key] = {"stamp": st, "period": "%s..%s" % (pf, pt), "covers": ok, "via": "pc"}
    if day.day <= MONTH_END_DAYS:
        ...
        if r:
            have["SUPPLIERWISE"] = {"stamp": r[0], "period": "%s..%s" % (r[1], r[2]), "covers": True, "via": "pc"}
    ...
    if r and r[1]:
        have["STOCK"] = {"stamp": r[0], "period": as_on, "covers": True, "via": "pc"}
    else:
        ...
        if r and r[1] and (r[0] or "9") <= by:
            have["STOCK"] = {"stamp": r[0], "period": nxt.strftime("%d-%m-%Y") + " (next morning)", "covers": True, "via": "pc"}
    _door_evidence(con, day, have, first, need_to)             # S305: the server's own door
    return have


DOOR_LINES = ("PURCHASE_ITEMWISE", "PURCHASE_BILLITEMWISE")
DOOR_STOCK_VARIANTS = ("DEFAULT", "TOTALS")                   # a whole-store closing -- never an orthotics subset
...
def _door_evidence(con, day, have, first, need_to):
    """S305 (D467 phase 2c): what the door itself VERIFIED for this day (mi_file), added only where the
    clinic PC's feed has not already shown it covered. Stamps are the capture moment (yyyymmdd-HHMMSS);
    mi_file dates are ISO; received_at carries +05:30."""
    if not _has_table(con, "mi_file"):
        return
    ymd = day.strftime("%Y%m%d")
    for typ, df, dt_to, st in con.execute(
            "SELECT type, date_from, date_to, stamp FROM mi_file WHERE verdict='VERIFIED' AND substr(stamp,1,8)=? "
            "AND type IN ('PURCHASE_BILLWISE','PURCHASE_ITEMWISE','PURCHASE_BILLITEMWISE') ORDER BY stamp", (ymd,)):
        key = "LINES" if typ in DOOR_LINES else "BILLWISE"
        ok = (df or "9") <= first and (dt_to or "") >= need_to
        prev = have.get(key)
        if not prev or (ok and not prev.get("covers")):
            have[key] = {"stamp": st, "period": "%s..%s" % (df, dt_to), "covers": ok, "via": "door"}
    if day.day <= MONTH_END_DAYS and not (have.get("SUPPLIERWISE") or {}).get("covers"):
        last = day.replace(day=1) - dt.timedelta(days=1)
        r = con.execute("SELECT stamp, date_from, date_to FROM mi_file WHERE verdict='VERIFIED' AND type='PURCHASE_SUPPLIERWISE' "
                        "AND date_from<=? AND date_to>=? AND substr(stamp,1,8)>=? ORDER BY stamp DESC LIMIT 1",
                        (last.replace(day=1).isoformat(), last.isoformat(), day.replace(day=1).strftime("%Y%m%d"))).fetchone()
        if r:
            have["SUPPLIERWISE"] = {"stamp": r[0], "period": "%s..%s" % (r[1], r[2]), "covers": True, "via": "door"}
    if not (have.get("STOCK") or {}).get("covers"):
        q = ("SELECT MIN(received_at) FROM mi_file WHERE verdict='VERIFIED' AND type='STOCK_CLOSING' AND variant IN (%s) "
             "AND date_from=?" % ",".join("?" * len(DOOR_STOCK_VARIANTS)))
        r = con.execute(q, DOOR_STOCK_VARIANTS + (day.isoformat(),)).fetchone()
        if r and r[0]:
            have["STOCK"] = {"stamp": r[0], "period": day.strftime("%d-%m-%Y"), "covers": True, "via": "door"}
        else:
            nxt = day + dt.timedelta(days=1)
            by = dt.datetime.combine(nxt, STOCK_BY).strftime("%Y-%m-%dT%H:%M:%S")
            r = con.execute(q, DOOR_STOCK_VARIANTS + (nxt.isoformat(),)).fetchone()
            if r and r[0] and r[0][:19] <= by:
                have["STOCK"] = {"stamp": r[0], "period": nxt.strftime("%d-%m-%Y") + " (next morning)", "covers": True, "via": "door"}


def pc_reported_after(con, when_iso):
    """Has the chain reported since this moment? manojz (a purchase feed row or any purchase export) or,
    S305, the door itself (any file the medical PC or anyone pushed -- mi_file.received_at, +05:30 cut)."""
    ...
    r3 = con.execute("SELECT MAX(substr(received_at,1,19)) FROM mi_file").fetchone() if _has_table(con, "mi_file") else None
    latest = max([x for x in ((r or [None])[0], (r2 or [None])[0], (r3 or [None])[0]) if x] or [""])
...
def judge(punched, have, pc_ok, day=None):
    ...
    if not pc_ok:
        return "unknown", missing, "neither the medical PC's push nor the clinic PC has reported since the day ended -- a machine may be asleep"
...
def selftest():
    ...
    # S305: the door's own evidence
    con3 = sqlite3.connect(":memory:")
    con3.executescript("CREATE TABLE purchase_export (md5, type, file, period_from, period_to, export_stamp, received_at,"
                       " n_rows, grand_amount_p, superseded_by); CREATE TABLE stock_feed (id, as_on, source, item, qty,"
                       " received_at); CREATE TABLE purchase_feed (id, at, host, pull_last, pull_age_min, state);"
                       " CREATE TABLE emp_code (code, person, issued_on, retired_on, source, note);"
                       " CREATE TABLE mi_file (md5, type, variant, verdict, stamp, date_from, date_to, received_at, source);")
    r = check(con3, day, punches=pcsv, staff_db=sdb, now=now)
    ck("S305: both routes silent -> unknown", r["verdict"] == "unknown")
    con3.execute("INSERT INTO mi_file VALUES ('d1','PURCHASE_BILLWISE','DEFAULT','VERIFIED','20260911-134235','2026-09-01','2026-09-10','2026-09-11T13:42:38+05:30','push')")
    con3.execute("INSERT INTO mi_file VALUES ('d2','PURCHASE_ITEMWISE','DEFAULT','VERIFIED','20260911-140731','2026-09-01','2026-09-11','2026-09-11T22:31:00+05:30','push')")
    r = check(con3, day, punches=pcsv, staff_db=sdb, now=now)
    ck("S305: clinic PC asleep, the door has both -> judged ok, via the door", r["verdict"] == "ok" and r["have"]["BILLWISE"]["via"] == "door" and r["have"]["LINES"]["via"] == "door")
    con3.execute("INSERT INTO mi_file VALUES ('d3','STOCK_CLOSING','SUBSET','VERIFIED','20260911-221844','2026-09-11','2026-09-11','2026-09-11T22:18:47+05:30','push')")
    r = check(con3, day, punches=pcsv, staff_db=sdb, now=dt.datetime(2026, 9, 12, 10, 45))
    ck("S305: an orthotics-only closing is not the stock closing", r["verdict"] == "red" and r["missing"] == ["STOCK"])
    con3.execute("INSERT INTO mi_file VALUES ('d4','STOCK_CLOSING','TOTALS','VERIFIED','20260912-080057','2026-09-12','2026-09-12','2026-09-12T08:01:00+05:30','push')")
    r = check(con3, day, punches=pcsv, staff_db=sdb, now=dt.datetime(2026, 9, 12, 10, 45))
    ck("S305: a whole-store closing through the door the next morning before 10:30 counts", r["verdict"] == "ok" and r["have"]["STOCK"]["via"] == "door")
    con3.execute("INSERT INTO mi_file VALUES ('d5','PURCHASE_BILLWISE','DEFAULT','REFUSED','20260913-093257','2026-09-01','2026-09-12','2026-09-13T09:33:00+05:30','push')")
    r = check(con3, dt.date(2026, 9, 13), punches=pcsv, staff_db=sdb, now=dt.datetime(2026, 9, 13, 23, 40))
    ck("S305: a REFUSED file is never evidence", "BILLWISE" not in r["have"])
    ck("S305: a database without mi_file still judges as before", evidence(con, day).get("BILLWISE", {}).get("via") == "pc")
    print("export_watch selftest: %d checks, %d failures" % (21, len(fails)))
    return 1 if fails else 0


# --- S274 (Club 3h) -- the OFF switch ----------------------------------------
...
_OFF_NAMES = ("ALL_OFF", "EXPORT_WATCH_OFF")
...
def main(argv=None):
    ...
    _off = _off_marker()
    if _off:
        print("export_watch: switched off (%s exists)" % _off)
        return 0
