#!/usr/bin/env python3
"""probe_sheets3.py -- kit S238_SHEETS_V3 (the F-414 fix: the copy under test reads the
...
difference must be the hold threshold alone; renders every sheet from the new result.
   v1.10 adds overtime (shown, not paid while ot_pay = 0) and moves nothing else:
   every field must match except the new OT fields, and every net must be identical.
   usage: probe_sheets3.py <new salary_policy.py> <live salary_policy.py> <YYYY-MM>
   (internal: probe_sheets3.py --one <salary_policy.py> <YYYY-MM> <render 0|1>)"""
...
for n, s in new["staff"].items():
    o = old["staff"].get(n, {})
    diff = sorted(k for k in set(s) | set(o) if k not in NEWF and abs(s.get(k, 0) - o.get(k, 0)) > 0.01)
    flag = ""
    if diff:
