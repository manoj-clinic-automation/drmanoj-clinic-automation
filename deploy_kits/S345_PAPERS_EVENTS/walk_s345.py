#!/usr/bin/env python3
"""walk_s345.py -- (S345 adds papers + events) (S344 adds the WhatsApp reports) (S342 = S340 with the walk finding its own rows by key) (S332 walk + S333 X-ray test run + S335 Drive helper + S340 Check karein) the live-shape walk of the patient-record page: THE REAL finance_app.py (patched) over
...
def main():
    ...
    os.environ["FINANCE_CRON_TOKEN"] = "walk-token-s345"
    ...
    T = {"X-Finance-Cron": "walk-token-s345"}
    ...
    ups = con.execute("SELECT id, local_name FROM record_upload ORDER BY id").fetchall()
    ...
    check("the box's copies are gone", os.listdir(inbox) == [])
    check("nothing left to carry", c.get("/finance/records/api/upload-pending", headers=T).get_json()["items"] == [])
    t = get("manoj", "/finance/records/p/%s" % pc).get_data(as_text=True)
    ids3 = [r[0] for r in con.execute("SELECT id FROM record_file WHERE source='upload' ORDER BY id")]
