#!/usr/bin/env python3
"""walk_s346.py -- (S346 = S345 with every queue check scoped to the walk's own rows) (S345 adds papers + events) (S344 adds the WhatsApp reports) (S342 = S340 with the walk finding its own rows by key) (S332 walk + S333 X-ray test run + S335 Drive helper + S340 Check karein) the live-shape walk of the patient-record page: THE REAL finance_app.py (patched) over
...
def main():
    ...
    os.environ["FINANCE_CRON_TOKEN"] = "walk-token-s346"
    ...
    T = {"X-Finance-Cron": "walk-token-s346"}
    ...
    check("a second sweep adds nothing of the walk's", not [i for i in c.get("/finance/records/api/wa-pending", headers=T).get_json()["items"]
                                                         if i["msg_id"].startswith("wa")])
    t = get("alisha", "/finance/checks").get_data(as_text=True)
    # the LIVE queue may already hold real WhatsApp items (S345 was refused on the box for counting "exactly
    # two"): every assertion below looks only at the walk's own messages.
    forms = dict((m.group(1), m.group(0)) for m in re.finditer(r'<form class="ck"[^>]*><input type="hidden" name="key" value="(wa:[^"]+)">.*?</form>', t, re.S))
    check("two WhatsApp items for reception", "wa:waA" in forms and "wa:waB" in forms and "File dekhein" in forms["wa:waA"]
          and "Report nahi hai" in forms["wa:waA"], sorted(forms))
    check("the family is offered, not chosen", forms["wa:waA"].count('name="cid"') == 2 and " checked" not in forms["wa:waA"])
    ...
    t = get("alisha", "/finance/checks").get_data(as_text=True)
    check("both WhatsApp items leave the queue", 'value="wa:waA"' not in t and 'value="wa:waB"' not in t)
    h = get("manoj", "/finance/records").get_data(as_text=True)
    check("the doctors' trail says it", "filed as X-ray" in h and "not a report" in h)

    # ---- S345: papers and events
    import io as _io
    inbox = os.path.join(os.path.dirname(dbp), "records_inbox")
    os.environ["RECORDS_INBOX"] = inbox
    pc = fam[0]
    UP = lambda u, path, d: c.post(path, data=d, headers=H(u), content_type="multipart/form-data")   # noqa: E731
    fl = lambda b, n: (_io.BytesIO(b), n)                                                               # noqa: E731
    t = get("alisha", "/finance/checks").get_data(as_text=True)
    check("Kagaz jodein is on the queue page", "/finance/checks/add" in t)
    check("awdhesh cannot add papers", get("awdhesh", "/finance/checks/add").status_code in (302, 403))
    r = UP("alisha", "/finance/checks/add", {"cid": "99999994", "kind": "scan", "day": today, "file": fl(b"%PDF x", "m.pdf")})
    check("an unknown ID is refused", "list mein nahi" in r.get_data(as_text=True))
    r = UP("alisha", "/finance/checks/add", {"cid": pc, "kind": "scan", "day": today, "file": fl(b"MZ", "m.exe")})
    check("a non-photo/PDF is refused", "Sirf photo ya PDF" in r.get_data(as_text=True))
    r = UP("alisha", "/finance/checks/add", {"cid": pc, "kind": "scan", "day": "2099-01-01", "file": fl(b"%PDF x", "m.pdf")})
    check("a future date is refused", "Tareekh" in r.get_data(as_text=True))
    r = UP("alisha", "/finance/checks/add", {"cid": pc, "kind": "scan", "day": today, "file": fl(b"%PDF-1.4 mri", "mri.pdf")})
    check("reception adds an MRI", r.status_code == 303 and "Jud gaya" in c.get(r.headers["Location"], headers=H("alisha")).get_data(as_text=True))
    ups = con.execute("SELECT id, local_name FROM record_upload WHERE clinic_id=? AND uploaded_by='alisha' ORDER BY id", (pc,)).fetchall()
    ups = [u2 for u2 in ups if os.path.exists(os.path.join(inbox, u2[1]))]
    check("kept on the box, private", len(ups) == 1 and oct(os.stat(os.path.join(inbox, ups[0][1])).st_mode & 0o777) == "0o600")
    r = UP("manoj", "/finance/records/p/%s/add" % pc, {"kind": "other", "day": today, "file": fl(b"\xff\xd8 discharge", "d.jpg")})
    check("the doctor adds a paper", r.status_code == 303 and "Added" in r.headers["Location"].replace("%20", " "))
    r = UP("manoj", "/finance/records/p/%s/add" % pc, {"what_kind": "event", "day": today, "hospital": "Walk Hospital",
                                                      "what": "ORIF walk", "note": "n"})
    check("an event with no paper", r.status_code == 303 and "Event%20added" in r.headers["Location"])
    r = UP("bhawna", "/finance/records/p/%s/add" % pc, {"what_kind": "event", "day": today, "hospital": "Walk H2",
                                                       "what": "Admission walk", "file": fl(b"%PDF-1.4 ds", "ds.pdf")})
    check("an event with its paper", r.status_code == 303)
    r = UP("manoj", "/finance/records/p/%s/add" % pc, {"what_kind": "event", "day": today, "what": ""})
    check("an event must say what was done", "Say%20what" in r.headers["Location"])
    check("shavez cannot add on the doctors' page", UP("shavez", "/finance/records/p/%s/add" % pc, {"kind": "other"}).status_code in (302, 403))
    t = get("manoj", "/finance/records/p/%s" % pc).get_data(as_text=True)
    check("waiting papers are said", "On its way to Drive" in t and "on its way to Drive" in t)
    check("events show", "Walk Hospital" in t and "ORIF walk" in t and "Admission walk" in t and "Add event" in t and "Add a paper" in t)
    check("upload-pending refuses no token", c.get("/finance/records/api/upload-pending").status_code in (401, 302))
    items = c.get("/finance/records/api/upload-pending", headers=T).get_json()["items"]
    items = [i for i in items if i["file_name"].startswith(today + " \u00b7 " + pc)]
    check("three papers wait", len(items) == 3 and {i["folder"] for i in items} == {"Scans", "Papers"}, items)
    check("the Drive name carries date and ID", all(i["file_name"].startswith(today + " · " + pc) for i in items))
    check("upload-file refuses no token", c.get("/finance/records/api/upload-file/%d" % items[0]["id"]).status_code in (401, 302))
    r = c.get("/finance/records/api/upload-file/%d" % items[0]["id"], headers=T)
    check("upload-file gives the bytes", r.status_code == 200 and r.data == b"%PDF-1.4 mri")
    files = []
    for k, i in enumerate(items):
        did = "upDriveId%06d" % k
        open(os.path.join(stub, did), "wb").write(b"%PDF up")
        files.append({"id": i["id"], "drive_id": did, "file_name": i["file_name"]})
    r = c.post("/finance/records/api/upload-done", headers=T, json={"files": files + [{"id": 99999, "drive_id": "zzzzzzzzzzzz"}]})
    check("upload-done files three", r.get_json()["stored"] == 3, r.get_json())
    check("the box's copies are gone", not [n for n in os.listdir(inbox) if n in {u2[1] for u2 in ups}])
    check("nothing of the walk's left to carry", not [i for i in c.get("/finance/records/api/upload-pending", headers=T).get_json()["items"]
                                                      if i["id"] in {x["id"] for x in items}])
    t = get("manoj", "/finance/records/p/%s" % pc).get_data(as_text=True)
    ids3 = [r[0] for r in con.execute("SELECT id FROM record_file WHERE source='upload' AND clinic_id=? AND added_at>='' "
                                      "AND source_ref IN (%s) ORDER BY id" % ",".join("'up:%d'" % x["id"] for x in items), (pc,))]
    check("three new files on the patient", len(ids3) == 3 and all("/finance/records/file/%d" % i in t for i in ids3))
    check("the event's paper sits on the event", "on its way to Drive" not in t)
