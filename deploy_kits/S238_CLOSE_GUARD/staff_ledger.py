...
v3.1 .. v3.6 (S157-S225): D331 ceiling + signed application · SL2-SL7 (quota lane,
...
def make_entry(users, maker, staff, cat, date_from, date_to, days, manual_amount,
    ...
    if cat == "ADVANCE_ISSUE":
        ...
        special = False
        sched = []
    # S238 (F-400): a live row with the same person, category, date and amount.
...
<p><small>v{APP_VERSION} · append-only · corrections by contra entry only</small></p>
    ...
    def salary():
        ...
        # step 5 — ledger close (S238/D447: only after the month, only by hand)
        if closed:
...
def selftest():
    ...
    r32 = make_entry(users, "doc", "Alpha", "ADVANCE_ISSUE",
                     "2027-03-06", "2027-03-06", 0, "4000", "special test (S238: narration now required)",
                     special=True)
    ...
    r33 = make_entry(users, "doc", "Beta", "ADVANCE_ISSUE",
                     "2027-03-07", "2027-03-07", 0, "16000", "special test (S238: narration now required)", special=True)
    ...
    # ======== S238 (D447): the close guard, the report, duplicates, SPECIAL narration ====
    ...
    print(f"SELFTEST PASSED — {ok[0]} maker-checker, rate-card, advance, loan, "
          f"skip, statement, salary, report, F-51, D332/SL5+SL6+SL7 and S238 checks OK")
