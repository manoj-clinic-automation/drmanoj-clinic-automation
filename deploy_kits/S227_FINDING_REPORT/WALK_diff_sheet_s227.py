...
def unit_mrp(nm):
    return int(round(STRIP_P[nm] / float(PS[nm]))) if nm in STRIP_P else None
...
with sync_playwright() as pw:
    ...
    chk("THE BOX SAYS THE VALUE OF THE MISMATCH AT MRP: short, over, and the lines without an MRP",
        ...
        and ("%d line%s with no MRP on record" % (exp_unpriced, "" if exp_unpriced == 1 else "s")) in out, out[:600])
    ...
    chk("...the downloads in the box, in order: proof, the remaining sheet, the report, the differences sheet",
        [a.get_attribute("class").split()[-1] for a in pg.locator("#padout a.dl").all()] == ["proof", "dl", "report", "diffs"],
    ...
    chk("...an item the shop never sold has no MRP and says so (None), never a cost wearing an MRP label",
        len(unp) == exp_unpriced and all(x["item"] not in STRIP_P for x in unp))
    ...
    chk("...%d numbered rows, largest MRP value first, the unpriced ones last saying 'no MRP'" % len(DIFF_ITEMS),
        len(lines) == len(DIFF_ITEMS) and DIFF_ITEMS and first["item"] in lines[0] and "no MRP" in lines[-1], (lines[0], lines[-1]))
    ...
    chk("...AT MRP is a rupee figure where the shop sells the item and 'no MRP on record' where it never has",
        df.cell(row=rows[first["item"]], column=6).value.startswith("Rs ") and df.cell(row=rows[unp[0]["item"]], column=6).value == "no MRP on record")
