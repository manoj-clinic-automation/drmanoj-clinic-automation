    ...
    for item, (m, cnt) in items.items():
        if cnt != m:
            ...
            diffs.append((item, packing.get(item, ""), packs.get(item, 1), m, cnt,
                          _value_p(con, item, cnt - m), _mrp_value_p(con, item, cnt - m)))
        else:
...
def _pad_mismatch_totals(diffs):
    ...
    t = dict(lines=len(diffs), short_lines=0, over_lines=0,
...
def _pad_diffs_data(con, root_id):
    ...
    rows = []
    for (item, packing, ps, marg, counted, cost_p, mrp_p) in R["differences"]:
...
def _pad_report_data(con, root_id):
    ...
    the frozen readiness lines, each part's seal, the mismatch, the differences
    with their explanation layer, the not-counted with the value sitting
    unchecked, the matched (collapsed), the rows sent back."""
    _root, fam, R = _pad_family(con, root_id)
    if R is None:
        return None
    _f_ensure(con)
    q = ",".join("?" * len(fam))
    ans = _pad_answers_by_item(con, fam)
    # cause and decision, newest per item across the family
    cause, dec = {}, {}
    ...
    for (item, packing, ps, marg, counted, cost_p, mrp_p) in R["differences"]:
        d = dict(item=item, packing=packing or "", pack=int(ps or 1), marg=marg, counted=counted,
                 diff=counted - marg, cost_p=cost_p, mrp_p=mrp_p, answer=ans.get(item),
                 cause=cause.get(item), decision=dec.get(item),
                 life="/finance/stock/api/pad/item/%d/%s" % (_root, item))
        d["major"] = _pad_major(d)
        diffs.append(d)
    diffs.sort(key=lambda d: (d["mrp_p"] is None, -abs(d["mrp_p"] or 0), d["cost_p"] is None,
                              -abs(d["cost_p"] or 0), -abs(d["diff"])))
    # not counted: Marg's figure, and its value at MRP sitting unchecked
    snapq = {}
    for r in con.execute("SELECT item, qty FROM stock_snapshot WHERE as_on=?", (R["as_on"],)):
        snapq[r[0]] = int(r[1] or 0)
    notc, unchecked_p, unchecked_unpriced = [], 0, 0
    for (item, packing, ps) in R["not_counted_rows"]:
        qn = snapq.get(item, 0)
        v = _mrp_value_p(con, item, qn)
        if v is None:
            unchecked_unpriced += 1
        else:
            unchecked_p += abs(int(v))
        notc.append(dict(item=item, packing=packing or "", pack=int(ps or 1), marg=qn, mrp_p=v))
    ...
    return dict(count_id=_root, parts=len(fam) - 1, family=fam, when=R["when"], when_iso=R.get("when_iso"),
                ...
                items_in_shop=R["items_in_shop"], counted=R["counted"], agreed=R["agreed"],
                differed=R["differed"], not_counted=R["not_counted"], sent_back=R["sent_back"],
                explained=len(ans), mismatch=R["mismatch"], basis="MRP",
                fy_from=since, fy_from_text=_r_dmy(since), upto=upto, upto_text=_r_dmy(upto),
                major=sum(1 for d in diffs if d["major"]), differences=diffs,
