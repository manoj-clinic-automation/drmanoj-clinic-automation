#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""day_resync.py -- S399_DAY_TRUTH_5 (S355 -> S356 -> S357 -> S367 -> this): an unapproved pharmacy day follows
...
def approved_label_missing(con, since, home_w, proc_w):
...
def main(argv=None):
    ...
    for e in rows2:
        ...
        if v != "NONE":
            print("  %-9s %s" % (v, line))
    for line in approved_label_missing(con, a.since, home_w, proc_w):
        print("  %-9s %s" % ("APPROVED", line))
