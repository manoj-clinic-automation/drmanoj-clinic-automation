#!/bin/bash
# install_S311_SUPPLIER_CHECK_2B.sh -- the wrong-supplier check, second stage, on Amir's step 4: a purchase return
...
set -u
KIT="S311_SUPPLIER_CHECK_2B"; KDIR="$(cd "$(dirname "$0")" && pwd)"
...
STAMP="$(date +%Y%m%d_%H%M%S)"; WALK="/tmp/s311_walk_$STAMP"
...
rm -f "$F.S311new"*; \cp -p "$F" "$F.S311new" || { echo "!! [3/7] copy failed"; exit 1; }
"$PY" -B patch_supplier_check2_s307.py --file "$F.S311new" --from "$FROM" || { rm -f "$F.S311new"*; echo "!! [3/7] patch refused - nothing placed"; exit 1; }
rm -f "$F.S311new.bak_"*   # the patcher keeps the S307 payload names (F-525): match any suffix
[ "$(m5 "$F.S311new")" = "$TO" ] || { rm -f "$F.S311new"; echo "!! [3/7] patched to an unpredicted md5 - nothing placed"; exit 1; }
"$PY" -c "import sys, os, tempfile, py_compile; py_compile.compile(sys.argv[1], cfile=os.path.join(tempfile.mkdtemp(), 'x.pyc'), doraise=True)" "$F.S311new" \
  || { rm -f "$F.S311new"; echo "!! [3/7] compile failed - nothing placed"; exit 1; }
echo "[3/7] patched on the box to the predicted md5; compiles"
if [ "${NOWALK:-0}" != "1" ]; then
  mkdir -p "$WALK" && \cp -p "$F.S311new" "$WALK/amir_new.py" && \cp -p "$F" "$WALK/amir_live.py"
  "$PY" -c "import sqlite3,sys; s=sqlite3.connect('file:%s?mode=ro'%sys.argv[1],uri=True); d=sqlite3.connect(sys.argv[2]); s.backup(d); d.close(); s.close()" "$DBF" "$WALK/walk.db" \
    || { echo "!! [4/7] scratch copy of the database failed - nothing placed"; rm -rf "$WALK"; rm -f "$F.S311new"; exit 1; }
  WOUT="$( cd "$FIN" && timeout 170 "$PY" -B "$KDIR/walk_s307.py" "$WALK/amir_live.py" "$WALK/amir_new.py" "$WALK/walk.db" 2>&1 | tail -2 )"
  echo "$WOUT" | sed 's/^/       /'
  echo "$WOUT" | tail -1 | grep -q "^WALK OK" || { echo "!! [4/7] walk red - nothing placed"; rm -rf "$WALK"; rm -f "$F.S311new"; exit 1; }
  rm -rf "$WALK"; echo "[4/7] walk green on a scratch copy of the live database"
else echo "[4/7] walk skipped (NOWALK=1, test only)"; fi
BAK="$F.bak_S311_${FROM:0:8}"; \cp -p "$F" "$BAK" || { rm -f "$F.S311new"; echo "!! [5/7] backup failed - nothing placed"; exit 1; }; echo "backup : $BAK"
restore() { echo "!! RED - restoring amir_day.py byte-identically"; \cp -p "$BAK" "$F"; rm -f "$F.S311new"
  if [ "${NORESTART:-0}" != "1" ]; then systemctl restart clinic-finance || true; sleep 3; fi
  echo "   $F $(m5 "$F")"; exit 1; }
mv -f "$F.S311new" "$F" || restore
[ "$(m5 "$F")" = "$TO" ] || restore
echo "[5/7] placed"
if [ "${NORESTART:-0}" != "1" ]; then
  ...
  # F-525: the service check hits a PUBLIC_PATHS route, which must answer 200.
  # Amir's own page is behind the login gate, so 302/401 there is CORRECT, not a fault.
...
rm -f "$F.S307new" "$F".S307new.bak_* 2>/dev/null   # tidy what the withdrawn S307 run left behind
