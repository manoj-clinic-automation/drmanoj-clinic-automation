#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""slip_log.py -- S324 (session 271, 19-Sep-2026), S325/S326/S327 fixes the same day; S328 adds the X-ray EMR upload list; S329: the tile opens a menu, one tap per X-ray upload; S330: blood tests: new = above the highest ID issued before the day. The OPD & X-ray/Proc slip tile, and the night
...
def home():
    ...
    return _shell("Parchi & upload", _tile_html(con, u), "hi")
...
def _menu_html(con, u, nx, nx2, room_n, today_n, miss_n):
    ...
    items = {
        ...
        "emr": ('<a class="mi" href="/finance/slips/emr"><b>Docterz upload</b><span class="%s">%d baaki</span></a>'
    ...
    if user == "awdhesh":
        ...
        order = ["opd", "xp", "blood", "room", "emr", "list"]
...
def _report_html(con, m, u):
    ...
    if n_emr:
        pend = [i for i in emr_items(con) if not i["done"]]
        rows = "".join('<li>%s \u00b7 %s %s \u2014 %s</li>' % (_dmy(i["day"])[:6], _esc(i["clinic_id"] or "no ID"),
                                                                _esc(i["name"] or ""), _esc(i["what"])) for i in pend[:200])
        out.append('<details class="sec"><summary><span>X-rays not yet uploaded to Docterz</span><span class="badge red">%d</span>'
                   '</summary><div class="body"><p class="sm">Oldest %s. Only what is still pending is listed.</p><ul>%s</ul>'
                   '</div></details>' % (n_emr, _dmy(old_emr), rows))
    try:
...
def emr_items(con):
    """Every X-ray from EMR_FROM on: from the slip tile (X-ray/Proc slips carrying an X-ray) and, for a
    patient-day no slip covers, from Docterz's own X-ray lines -- so the list is whole even for days or
    patients the tile missed. Read-only on both sources; the tick lives in emr_upload."""
    ensure(con)
    emr_ensure(con)
    done = {r["key"]: dict(r) for r in con.execute("SELECT * FROM emr_upload")}
    out, covered = [], set()
    rows = con.execute("SELECT * FROM slip WHERE series='xp' AND state='ok' AND day>=? ORDER BY day, slip_no",
                       (EMR_FROM,)).fetchall()
    for s in rows:
        ...
        if not its:
            continue
        s = dict(s)
        covered.add((s["day"], s["clinic_id"]))
        nm = s["no_id_name"] or s["name_seen"] or ((patient(con, s["clinic_id"]) or ("", ""))[0])
        old = done.get("slip:%d" % s["id"])               # S328 ticked a whole slip; it still counts
        for it in its:                                    # S329: one line, one tap, per X-ray (owner)
            key = "xi:%d" % it["id"]
            rec = done.get(key) or old
            out.append({"key": key, "done_key": (key if done.get(key) else ("slip:%d" % s["id"] if old else "")),
                        "day": s["day"], "session": _session_of((s["logged_at"] or "")[11:13]),
                        "clinic_id": s["clinic_id"], "name": nm, "what": _items_label([it]),
                        "slip_no": s["slip_no"], "done": rec, "kind": "xray"})
    try:
        dz = con.execute("SELECT business_date d, clinic_id, sn, patient, shift FROM clinic_day_line "
                         "WHERE section='xray' AND business_date>=? ORDER BY business_date, sn", (EMR_FROM,)).fetchall()
    except Exception:                                    # noqa: BLE001 -- no Docterz table, slips alone
        dz = []
    for r in dz:
        cid = str(r["clinic_id"] or "")
        if (r["d"], cid) in covered:
            continue
        key = "dz:%s:%s:%s" % (r["d"], cid, r["sn"])
        old = done.get("dz:%s:%s" % (r["d"], cid))
        rec = done.get(key) or old
        out.append({"key": key, "done_key": (key if done.get(key) else ("dz:%s:%s" % (r["d"], cid) if old else "")),
                    "day": r["d"], "session": {"Morning": "Subah", "Evening": "Shaam"}.get(r["shift"] or "", ""),
                    "clinic_id": cid, "name": r["patient"] or "", "what": "X-ray (Docterz)",
                    "slip_no": None, "done": rec, "kind": "xray"})
    try:                                                  # S330: blood reports join the same list
        out.extend(blood_state(con)[1])
    except Exception:                                    # noqa: BLE001 -- the X-rays never wait on the lab
        pass
    out.sort(key=lambda x: (x["day"], 0 if x["session"] == "Subah" else 1, x["slip_no"] or 0))
    return out


def emr_pending_count(con):
    try:
        items = emr_items(con)
    except Exception:                                    # noqa: BLE001
        return 0, None
    p = [i for i in items if not i["done"]]
    return len(p), (p[0]["day"] if p else None)
...
def emr_page():
    ...
    try:
        ...
        orders = []
    _BLOOD_ASKING[0] = any(not o["outcome"] for o in orders)
    return _shell("Docterz upload", _blood_open_html(orders) + _emr_html(items, request.args.get("ok") or ""), "hi",
                  back="/finance/slips", back_label="\u2190 Menu")
...
def emr_mark():
    ...
    keys = [k for k in request.form.getlist("key") if re.fullmatch(r"(xi:\d+|dz:\d{4}-\d{2}-\d{2}:[0-9A-Za-z\-/]{0,12}:\d+|lr:\d{4}-\d{2}-\d{2}:[0-9A-Za-z\-/]{1,12})", k)]
    valid = {i["key"]: i for i in emr_items(con)}
    ...
    for k in keys:
        ...
        if not it:
            continue
        if act == "done" and not it["done"]:
            con.execute("INSERT OR IGNORE INTO emr_upload(key, kind, day, clinic_id, uploaded_by, uploaded_at) "
                        "VALUES (?, ?, ?, ?, ?, ?)", (k, it.get("kind", "xray"), it["day"], it["clinic_id"], who, now))
            n += 1
        elif act == "undo" and it["done"] and it.get("done_key"):
            con.execute("DELETE FROM emr_upload WHERE key=?", (it["done_key"],))
            n += 1
    _audit_safe(con, 0, "emr_upload_" + (act or "?"), None, json.dumps({"keys": keys[:50], "n": n}), who)
    con.commit()
    msg = ("%d X-ray upload ho gaye." % n) if act == "done" else ("%d wapas baaki mein." % n)
    return redirect("/finance/slips/emr?ok=" + _q(msg), code=303)   # stays inside the tile's menu
...
def _emr_html(items, ok):
    pend = [i for i in items if not i["done"]]
    today = _today()
    donetoday = [i for i in items if i["done"] and (i["done"]["uploaded_at"] or "")[:10] == today]
    out = []
    if ok:
        out.append('<div class="ok">%s</div>' % _esc(ok))
    if pend:
        nx_ = len([i for i in pend if i.get("kind") != "path"])
        out.append('<div class="rem">Baaki: <b>%d X-ray, %d blood report</b> · sabse purana %s. Docterz mein mareez kholein, '
                   'uploads mein daalein, phir yahan “Upload ho gaya” dabayein.</div>'
                   % (nx_, len(pend) - nx_, _dmy(pend[0]["day"])[:6]))
    else:
        out.append('<div class="ok">Kuch baaki nahi — sab Docterz mein upload ho chuke.</div>')
    days = []
    for i in pend:
    ...
    first = True
    for d in days:
        rows = [i for i in pend if i["day"] == d]
        body = []
        for sess in ("Subah", "Shaam", ""):
            rs = [i for i in rows if i["session"] == sess]
            if not rs:
                continue
            if sess:
                body.append('<h3>%s</h3>' % sess)
            for i in rs:
                body.append('<div class="lrow"><div class="rl"><span class="no">%s</span> %s<br><span class="sm">%s%s</span></div>'
                            '<form method="post" action="/finance/slips/emr/mark" class="rb"><input type="hidden" name="key" value="%s">'
                            '<button name="act" value="done">Upload ho gaya</button></form></div>' % (
                                _esc(i["clinic_id"] or "ID nahi"), _esc(i["name"] or "—"), _esc(i["what"]),
                                (" · parchi %d" % i["slip_no"]) if i["slip_no"] else "", _esc(i["key"])))
        allkeys = "".join('<input type="hidden" name="key" value="%s">' % _esc(i["key"]) for i in rows)
        body.append('<form method="post" action="/finance/slips/emr/mark" class="allf">%s'
                    '<button name="act" value="done" class="more">Is din ke sab %d upload ho gaye</button></form>' % (allkeys, len(rows)))
        sec = _sec("d" + d.replace("-", ""), "%s%s" % (_dmy(d)[:6], " (aaj)" if d == today else ""), "%d baaki" % len(rows), "".join(body))
        if first and not _BLOOD_ASKING[0]:
            sec = sec.replace('<details class="sec"', '<details class="sec" open', 1)
        first = False
        out.append(sec)
    dl = []
    for i in donetoday:
        dl.append('<div class="lrow"><span class="no">%s</span> %s <span class="sm">%s · %s %s</span>'
                  '<form method="post" action="/finance/slips/emr/mark" class="vf"><input type="hidden" name="key" value="%s">'
                  '<button name="act" value="undo" class="x">Galti se — wapas</button></form></div>' % (
                      _esc(i["clinic_id"] or "ID nahi"), _esc(i["name"] or "—"), _esc(i["what"]),
                      _esc(i["done"]["uploaded_by"]), _esc((i["done"]["uploaded_at"] or "")[11:16]), _esc(i["key"])))
    out.append(_sec("donetoday", "Aaj upload hue", "%d" % len(donetoday), "".join(dl) or '<p class="sm">Abhi koi nahi.</p>'))
    out.append(_TILE_JS.replace("form.slipf", "form.none").replace(
        "if(b&&b.value==='void'&&!confirm('Yeh parchi hatayein?'))e.preventDefault();",
        "if(b&&b.value==='void'&&!confirm('Yeh parchi hatayein?'))e.preventDefault();"))
    out.append("<script>[].forEach.call(document.querySelectorAll('form.allf'),function(f){f.addEventListener('submit',"
               "function(e){if(!confirm('Kya is din ke sab X-ray Docterz mein upload ho chuke hain?'))e.preventDefault();});});</script>")
    return "".join(out)
...
def blood_outcome():
    ...
    if r and act in ("not_tested", "lab_missed", "undo"):
        ...
        con.commit()
    return redirect("/finance/slips/emr?ok=" + _q("Theek."), code=303)
