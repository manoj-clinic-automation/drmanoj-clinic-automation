...
def absent_worklist(con, ym):
    ...
    reqs = {}
    try:
        for r in con.execute("SELECT reg_date, staff_id, status FROM present_request "
                             "WHERE reg_date LIKE ?", (ym + "-%",)):
            reqs[(r["reg_date"], r["staff_id"])] = r["status"]
    except Exception:
        pass
    acc = {}
    ...
    return [b for b in blocks if b["rows"] or b["corrected"]], True
...
def fixabsents():
    ...
    return render_template_string(
        ...
        feed_ok=feed_ok, reason_default=request.args.get("r", ""),
...
<div class="wrap">
  ...
  <form method="POST" action="{{ prefix }}/fixabsents/apply">
    ...
    <div style="margin:18px 0 40px"><button class="btn" type="submit">Mark the ticked days present</button></div>
  </form>
  {% endif %}
</div></body></html>
"""
...
def _selftest():
    ...
    assert b"2026-08-04|6" not in rF.data, \
        "a staff-day already corrected must not be offered again"
    assert b"2026-08-04|1" in rF.data, "an uncorrected absence must be offered"
    con = get_db()
