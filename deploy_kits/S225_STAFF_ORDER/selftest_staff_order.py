...
# -*- coding: utf-8 -*-
"""
selftest_staff_order.py -- S225 (rev 6): the S224 selftest UNCHANGED as regression, then §13 the staff order page. Base: the REAL blueprint, on a temp finance.db, fed the REAL
...
if True:
    ...
    ck("staff page shows the OUT item with a rounded quantity", "ZZTEST ITEM 1" in sp and "<b>%d</b> strips" % PA._staff_qty(names["ZZTEST ITEM 1"]["order_strips"], "strip") in sp)
    ...
    ck("one new order, its lines in strips with pack_size and units", q1("SELECT COUNT(*) FROM purchase_order") == before + 1
       ...
       and q1("SELECT packs FROM purchase_order_line WHERE order_id=? AND item='ZZTEST ITEM 1'", soid) == PA._staff_qty(names["ZZTEST ITEM 1"]["order_strips"], "strip"))
