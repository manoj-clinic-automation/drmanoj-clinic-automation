...
# -*- coding: utf-8 -*-
"""
purchase_app.py -- S224: Marg's purchases, on the box.  (rev 6, S225, 04-Sep-2026)
...
def _page(title, body, extra_js=""):
    ...
    nav = ('<nav class="noprint muted"><a href="%s/page/hub">Hub</a>'
           ...
           '<a href="%s/page/staff">Order medicines</a>'
           '<a href="/finance/stock/page/drift">Stock check</a></nav>' % (prefix, prefix, prefix, prefix))
...
def _staff_plan(con):
    ...
    for v in plan["vendors"]:
        ...
        for l in v["lines"]:
            ...
            unit = _unit_word(s.get("packing"), l["pack_size"])
            qty = _staff_qty(l["order_strips"], unit)
...
def page_staff():
    ...
    for i, v in enumerate(vendors):
        ...
        if v["no_vendor"]:
            ...
            phone = _phone_for(con, v["vendor"])
            btns = ('<button class="p" onclick="sendWA(%d)">Send on WhatsApp</button> '
                    '<a href="tel:%s"><button>Call</button></a>' % (i, _esc(_wa_digits(phone))))
        else:
