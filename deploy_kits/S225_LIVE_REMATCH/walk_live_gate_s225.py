...
# -*- coding: utf-8 -*-
"""
walk_live_gate_s225.py -- THE LIVE-SHAPE WALK for rev 9 (the live cross-check), run on a COPY in a probe folder (F-321): the REAL finance_app.py,
...
On the box (after the copy, BEFORE the restart):
    FIN_DIR=/root/finance /root/wa/venv/bin/python3 -B /root/finance/walk_live_gate_s225.py
"""
import os
import shutil
import sqlite3
...
ck("purchase_app beside finance_app is rev 9 (staff page, phone book, arrival, live re-match)", hasattr(PA, "page_staff") and hasattr(PA, "_arrive") and hasattr(PA, "_rematch_if_changed") and hasattr(PA, "_in_transit"))
