# CLINIC CALLBACK TRACKER — APPS SCRIPT PROJECT AUDIT — v1.7
...
response, not under audit (D180 undisturbed). Next free finding number: **F-36**.*
...
- **F-10 (two blind escapers → stop embedding data in markup)** — unchanged, and the audit's own
  prescription stands: **own commit.** Two S136 additions were built inside the existing discipline
  (`esc(jsq(…))` where applicable) and add no new fragile sites.
