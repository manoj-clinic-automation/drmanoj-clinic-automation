# HANDOFF RUNBOOK — 09 July 2026 — Session 131 — v69
...
> **v69 — the last two threads closed.** The **Fault → Action Register was KEPT, not retired** — Diagnostics §M1 records **D114**: *"the single brain = Fault_Action_Register."* The recommendation to fold it away would have overturned a decision nobody had read (D112–D115 all sit in the D1–D120 hole). **That would have been the fourth lineage error of this session.** Re-based to **v2.0**, status-true, zero lines lost. **F-24:** the Register describes an auto-responder that **does not exist** — the live watchman is read-only and *"never starts/stops/changes a service."* **Not one row is live-and-acting.** **F-25:** six `CALLHOOK_*` codes detecting since S125 with **no lane and no procedure**. **D203** states the detection/response boundary. **D112–D115 restored.** Seven historical documents archived out of the working set.
>
...
**A-000 — THE FAULT REGISTER PROMISES A RESPONDER THAT DOES NOT EXIST (F-24).** Nine faults marked `AUTO→ESC` — *"System does: `systemctl restart call-api`"*. **The live watchman never restarts anything.** **D113 must be re-stated as intent, or the Lane-1 responder (Deliverable 2) must be scheduled.** Until then, treat every alert as detect-and-tell. **During an outage, do not wait for a restart.**
...
- **Fault → Action Register → `Fault_Action_Register_v2_0.md` (NEW).** **Kept, not retired (D114).** Self-contained, §1/§2/§3/§4/§5 verbatim. **Per-row status column: not one row is 🟢.** §2.5 added — the six `CALLHOOK_*` codes, laned at last. Source-of-truth line was twenty-five KB versions dead.
...
**END OF RUNBOOK v69 — §5 is the last section. If §5 is absent, this file is truncated.**
