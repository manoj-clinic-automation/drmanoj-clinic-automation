...
def spool_has_files(spool):
    """True if anything at all is sitting in the spool."""
    try:
        for n in os.listdir(spool):
            if os.path.isfile(os.path.join(spool, n)):
                return True
    except OSError:
        pass
    return False
...
def watch(roots, spool, once, do_route, out, route_extra=None, poll_s=SAFETY_POLL_S):
    ...
    # S201: route whenever the spool HAS anything, not only when this run
    # captured something. Previously a routing run that died (an index write
    # that failed, a copy that raised) left its files in the spool, and no
    # later run would touch them until an unrelated new file happened to
    # arrive. Reports could sit unrouted indefinitely with nothing saying so.
    # The router skips anything already indexed, so routing an idle spool is
    # cheap and idempotent.
    if do_route and (new or spool_has_files(spool)):
