...
STATE_NAME = "spine_state.json"      # S338: the health page's leg reads last_success_iso from here


def write_state(out_db, B, passed):
    """S338 (D567 item 4). One small JSON beside the spine for the health page (freshness_legs.json, kind
    state_json, field last_success_iso): last_success_iso moves ONLY when the gate passed, so a failing
    gate lets the leg go stale and red by itself; the other fields carry the one-line status for whoever
    reads the file.  Written atomically; a failure here never fails the build."""
    path = os.path.join(os.path.dirname(os.path.abspath(out_db)), STATE_NAME)
    now = dt.datetime.now(IST).isoformat(timespec="seconds")
    prev = {}
    try:
    ...
    ok = sum(1 for r in rows if r[1] and r[2])
    failed = [r[0] for r in rows if r[1] and not r[2]]
    status = ""
    if passed:
        try:
            import spine_read                                 # noqa: PLC0415
            status = spine_read.Spine(out_db).status()
        except Exception as e:                                # noqa: BLE001
            status = "built %s · gate %d/%d (status line unavailable: %s)" % (now[:16], ok, nb, str(e)[:60])
    else:
        status = "GATE FAILED %d/%d at %s -- %s; the last good spine is still in place" % (ok, nb, now[:16], "; ".join(failed)[:200])
    state = dict(at=now, gate="%d/%d" % (ok, nb), passed=bool(passed), failed=failed, status=status,
                 last_success_iso=now if passed else prev.get("last_success_iso", ""),
                 last_failure_iso=(prev.get("last_failure_iso", "") if passed else now), kit="S338_SPINE_HEALTH")
    try:
        with open(path + ".tmp", "w") as fh:
            json.dump(state, fh, indent=1)
        os.replace(path + ".tmp", path)
    except OSError as e:
        print("spine_build: could not write %s: %s" % (path, e))
    return state
