#!/bin/bash
# =============================================================================
#  install_S407_NEFT_MESSAGES.sh · kit S407_NEFT_MESSAGES (session 283, Sanjeevni, 26-Sep-2026, D623)
#
...
#
#  THE OWNER (25/26-Sep-2026): "A provisional entry visible to Amir and the staff; suppliers told by WhatsApp with the full
#  account number and IFSC. All messages from the reception mobile. I log the provisional once; then maximum automated."
#
#  NEW:    /root/finance/supplier_msg.py (the event, the queue, the token doors, the cards, the setup page)
#  PATCHED ON THE BOX from the live bytes (make_s407.py; every anchor exactly once; FROM -> TO pins checked):
#          /root/finance/purchase_app.py          7896eae4 -> fdec7ec0   init mounts the module; the pay page's NEFT card + chips
#          /root/finance/amir_day.py              00c443cb -> 59a51d47   Amir's card on every step
#          /root/finance/sanjeevni_approvals.py   74fe5437 -> f6fc90d5   two Needs-you lines
#          /root/finance/finance_app.py           8055b0de -> d19c2046   the two token doors on the public list (declared)
#  Restarts clinic-finance only. finance.db backed up first; the seed adds two settings and makes the phone token once.
#  The walk (this kit's), then S406's, S405's, S404's, S403's, S400's and S402's own walks re-run on the patched files.
# =============================================================================
set -u
KIT="S407_NEFT_MESSAGES"
...
WALK="/tmp/s407_walk_$STAMP"
declare -A FROM=( [purchase_app.py]=7896eae4dff4427a2ebdb4f5023fd686 [amir_day.py]=00c443cbce0b07ba763bbe655a3e3c58
                  [sanjeevni_approvals.py]=74fe5437afd646851d0c28200592177d [finance_app.py]=8055b0deddcb65234f1b9d818d56fda9 )
declare -A TO=( [purchase_app.py]=fdec7ec01d2ceb449aaa0a62956a7a0f [amir_day.py]=59a51d471cb30702dfd0cb8b6b6f3a44
                [sanjeevni_approvals.py]=f6fc90d5d31badd3c5eec3691a78c4fb [finance_app.py]=d19c2046b190a4ec00745dc4121152e8 )
declare -A DEST=( [purchase_app.py]="$FIN/purchase_app.py" [amir_day.py]="$FIN/amir_day.py" [sanjeevni_approvals.py]="$FIN/sanjeevni_approvals.py" [finance_app.py]="$FIN/finance_app.py" )
ORDER=(purchase_app.py amir_day.py sanjeevni_approvals.py finance_app.py)
NEWF=(supplier_msg.py)
...
clean() { find "$KDIR" "$K400" "$K402" "$K403" "$K404" "$K405" "$K406" "$WALK" -name __pycache__ -prune -exec rm -rf {} + 2>/dev/null; }
...
for k in "$K400/walk_s400.py" "$K400/seed_s400.py" "$K402/walk_s402.py" "$K403/walk_s403.py" "$K403/seed_s403.py" "$K404/walk_s404.py" "$K404/seed_s404.py" "$K405/walk_s405.py" "$K405/seed_s405.py" "$K406/walk_s406.py" "$K406/seed_s406.py"; do
  [ -f "$k" ] || { say "!! [1/10] $k must sit beside this kit (the earlier walks re-run) - nothing installed"; exit 1; }
done
say "[1/10] kit gates green (S400, S402, S403, S404, S405 and S406 walks found)"
ALL=1; for f in "${ORDER[@]}"; do [ "$(m5 "${DEST[$f]}")" = "${TO[$f]}" ] || ALL=0; done
for f in "${NEWF[@]}"; do [ "$(m5 "$FIN/$f")" = "$(m5 "$f")" ] || ALL=0; done
if [ "$ALL" = 1 ]; then say "-- ALREADY INSTALLED. Seed re-checked:"; FINANCE_DIR="$FIN" "$SPY" -B seed_s407.py "$DBF"; exit 0; fi
...
mkdir -p "$WALK/built" "$WALK/finance/finance_ui" "$WALK/finance/spine" "$WALK/old/finance_ui" "$WALK/old/spine" "$WALK/old406/finance_ui" "$WALK/old406/spine" \
         ...
         "$WALK/old0/finance_ui" "$WALK/old2/finance_ui" "$WALK/marg_ingest" "$WALK/marg_old" "$WALK/assetapp_live" "$WALK/assetapp_old3" \
         "$WALK/plive" "$WALK/p403" "$WALK/p404" "$WALK/pold" "$WALK/uploads" "$WALK/stub" || exit 1
"$SPY" -B make_s407.py --finance "$FIN" --out "$WALK/built" | sed 's/^/   /' || { say "!! [3/10] the build from the live bytes stopped - nothing installed"; rm -rf "$WALK"; exit 1; }
...
say "[3/10] live bytes + anchored edits give exactly the kit's four files (pins match)"
PYS="$WALK/built/purchase_app.py $WALK/built/amir_day.py $WALK/built/sanjeevni_approvals.py $WALK/built/finance_app.py"
( "$SPY" -m py_compile supplier_msg.py seed_s407.py make_s407.py walk_s407.py $PYS 2>/dev/null && "$VPY" -m py_compile supplier_msg.py seed_s407.py $PYS 2>/dev/null ) \
...
for side in finance old old406 old405 old404 old403 old0 old2; do
...
for side in finance old old406 old405 old404 old403; do cp -p "$FIN"/spine/*.py "$FIN"/spine/*.json "$WALK/$side/spine/" 2>/dev/null; done
[ -f "$FIN/spine/spine.db" ] && for side in finance old old406 old405 old403; do cp -p "$FIN/spine/spine.db" "$WALK/$side/spine/"; done
cp -p "$WALK/built/purchase_app.py" "$WALK/built/amir_day.py" "$WALK/built/sanjeevni_approvals.py" "$WALK/built/finance_app.py" "$WALK/finance/"
cp -p supplier_msg.py "$WALK/finance/"
# old406 = the box BEFORE S406 (its .bak_S406 files), the negative control S406's own walk expects
...
for i in 1 2 3 4 5 6 7; do copydb "$DBF" "$WALK/scratch$i.db" || { say "!! [5/10] no scratch copy of the database - nothing installed"; rm -rf "$WALK"; exit 1; }; done
...
WOUT="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch1.db" timeout 900 "$VPY" -B "$KDIR/walk_s407.py" --app "$WALK/finance" --old "$WALK/old" --db "$WALK/scratch1.db" 2>&1 )"
echo "$WOUT" | sed 's/^/   /'
echo "$WOUT" | grep -q "^WALK_S407 GREEN" || { say "!! [5/10] walk red - nothing installed"; clean; rm -rf "$WALK"; exit 1; }
clean
say "[5/10] walk_s407 green on a scratch copy of the live database (above)"
W406="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch2.db" timeout 900 "$VPY" -B "$K406/walk_s406.py" --app "$WALK/finance" --old "$WALK/old406" --db "$WALK/scratch2.db" 2>&1 )"
...
W405="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch3.db" timeout 900 "$VPY" -B "$K405/walk_s405.py" --app "$WALK/finance" --old "$WALK/old405" --db "$WALK/scratch3.db" 2>&1 )"
...
W404="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch4.db" timeout 1500 "$VPY" -B "$K404/walk_s404.py" --app "$WALK/finance" --old "$WALK/old404" --marg-new "$WALK/marg_ingest" --marg-old "$WALK/marg_old" \
         --db "$WALK/scratch4.db" --portal-new "$WALK/p403" --portal-old "$WALK/p404" 2>&1 )"
...
W403="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch5.db" timeout 1500 "$VPY" -B "$K403/walk_s403.py" --app "$WALK/finance" --old "$WALK/old403" \
         --assets-new "$WALK/assetapp_live" --assets-old "$WALK/assetapp_old3" --db "$WALK/scratch5.db" --assets-db "$WALK/assets_scratch.db" \
...
W400="$( cd "$WALK" && env $WENV NEEDS_YOU_WITHOUT_S403=1 NEEDS_YOU_WITHOUT_S406=1 FINANCE_DB="$WALK/scratch6.db" timeout 900 "$VPY" -B "$K400/walk_s400.py" --app "$WALK/finance" --old "$WALK/old0" --db "$WALK/scratch6.db" --portal-new "$WALK/p404" --portal-old "$WALK/pold" 2>&1 )"
...
W402="$( cd "$WALK" && env $WENV FINANCE_DB="$WALK/scratch7.db" timeout 900 "$VPY" -B "$K402/walk_s402.py" --app "$WALK/finance" --old "$WALK/old2" --db "$WALK/scratch7.db" 2>&1 )"
...
clean
say "[6/10] S406's, S405's, S404's, S403's, S400's and S402's own walks re-run on the patched files: still green (above)"
copydb "$DBF" "$FIN/finance.db.bak_S407_$STAMP" || { say "!! [7/10] database backup failed - nothing installed"; rm -rf "$WALK"; exit 1; }
declare -A BAK
for f in "${ORDER[@]}"; do BAK[$f]="${DEST[$f]}.bak_S407_$(m5 "${DEST[$f]}" | cut -c1-8)"; \cp -p "${DEST[$f]}" "${BAK[$f]}" || { say "!! [7/10] backup failed - nothing placed"; rm -rf "$WALK"; exit 1; }; done
say "[7/10] finance.db.bak_S407_$STAMP made; .bak_S407_<from8> beside each of the four files"
restore() {
  ...
  say "   healthz $(curl -s -o /dev/null -m 8 -w '%{http_code}' http://127.0.0.1:8106/finance/healthz) · the database backup stays: $FIN/finance.db.bak_S407_$STAMP"
...
FINANCE_DIR="$FIN" "$SPY" -B seed_s407.py "$DBF" || restore
say "[9/10] settings seeded; the reception phone's token made once (never printed)"
...
c3=$(curl -s -o /dev/null -m 8 -w '%{http_code}' http://127.0.0.1:8106/finance/purchase/page/pay)
c4=$(curl -s -o /dev/null -m 8 -w '%{http_code}' http://127.0.0.1:8106/finance/api/supplier-msg/next)
c5=$(curl -s -o /dev/null -m 8 -w '%{http_code}' http://127.0.0.1:8106/finance/purchase/page/phone-setup)
c6=$(curl -s -o /dev/null -m 8 -w '%{http_code}' http://127.0.0.1:8106/finance/amir)
say "health : finance healthz $c2 · /finance/purchase/page/pay without a login $c3 · the phone's door without a token $c4 (401 = the token gate, expected) · phone-setup $c5 · /finance/amir $c6 (302/403 = login gates, expected)"
[ "$c2" = 200 ] && { [ "$c3" = 302 ] || [ "$c3" = 401 ]; } && [ "$c4" = 401 ] && { [ "$c5" = 302 ] || [ "$c5" = 401 ]; } && { [ "$c6" = 302 ] || [ "$c6" = 401 ] || [ "$c6" = 403 ]; } || restore
journalctl -u clinic-finance --since "-2 min" --no-pager 2>/dev/null | grep -q "NOT mounted" && restore
say "[10/10] clinic-finance active, healthz 200, the phone's door answers 401 to a tokenless call, nothing 'NOT mounted'"
clean; rm -rf "$WALK"
for f in "${ORDER[@]}"; do md5sum "${DEST[$f]}"; done
for f in "${NEWF[@]}"; do md5sum "$FIN/$f"; done
say "$KIT: DONE -- https://followup.dr-manoj.in/finance/purchase/page/pay (NEFT done · the suppliers' messages) · https://followup.dr-manoj.in/finance/purchase/page/phone-setup (Shavez, once) · https://followup.dr-manoj.in/finance/amir"
