...
V1 SCOPE -- only what can be re-checked by the ORIGINAL rule (D349):
  ...
  4. recon_exception kind='line_sum_vs_day_total', status='open' (v2, 02-Sep)
     -> healed when the day now has ZERO open review rows AND the live
        identity check (finance_daily_gaps.identity_gaps -- THE rule) finds
        no unmatched or ambiguous bill. The owner's "12 days waiting" fix.
...
def run(con, dry=False, now=None):
    ...
    # 1c -- line_sum_vs_day_total (v2): review empty + identity clean -> healed
    try:
        import finance_daily_gaps                            # noqa: PLC0415
        _idg = finance_daily_gaps.identity_gaps
    except ImportError:
        _idg = None
    if _idg is not None:
        rows = con.execute("SELECT id, unit, business_date FROM recon_exception "
                           "WHERE kind='line_sum_vs_day_total' AND status='open'").fetchall()
        for r in rows:
            out["rechecked"] += 1
            rev = con.execute(
                "SELECT COUNT(*) c FROM sale_item_review v JOIN day_entry e "
                "ON e.id=v.day_entry_id WHERE e.unit=? AND e.business_date=? "
                "AND v.status='open'", (r[1], r[2])).fetchone()[0]
            if rev:
                out["still_open"] += 1
                continue
            try:
                gaps, tally = _idg(con, r[2], r[1], exclude_returns=True)
            except Exception:                                # noqa: BLE001
                out["still_open"] += 1
                continue
            if tally.get("unmatched") or tally.get("ambiguous"):
                out["still_open"] += 1
                continue
            out["healed"] += 1
            out["items"].append("line_sum_vs_day_total %s healed (review empty, "
                                "every bill resolved)" % r[2])
            if not dry:
                con.execute("UPDATE recon_exception SET status='resolved', "
                            "resolution='healed: review queue empty and every bill "
                            "resolved to a patient (finance_heal v2)', "
                            "closed_by='finance_heal', closed_at=? WHERE id=?",
                            (now, r[0]))
                con.execute("INSERT INTO heal_log (healed_at,kind,unit,business_date,"
                            "source_table,source_id,detail,healed_by) VALUES (?,?,?,?,?,?,?,?)",
                            (now, "line_sum_vs_day_total", r[1], r[2],
                             "recon_exception", r[0],
                             "review empty; identity check clean", "finance_heal"))
