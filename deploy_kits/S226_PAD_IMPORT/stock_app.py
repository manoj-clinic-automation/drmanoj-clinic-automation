...
# tablet -- REFUSES the import and is named, with its row number. A count that
# quietly loses eleven lines is worse than one that will not load.
#
# Nothing is written until a person has seen exactly what was read and pressed
# the button, and the bytes that were previewed are the bytes that commit: the
# file's own md5 has to come back with the confirmation.

def _pad_read(fs):
    """(parsed, md5, bytes). Raises ValueError with a sentence a person can act on."""
    import hashlib                                            # noqa: PLC0415
    raw = fs.read()
    if not raw:
    ...
    return (padreader.read_pad(io.BytesIO(raw)), hashlib.md5(raw).hexdigest(), len(raw))
...
def pad_fresh():
    ...
    return _xlsx_response(padwriter.fresh_pad(items, as_on, today),
                          "STOCK_COUNT_PAD_%s.xlsx" % today)
...
def pad_followup(cid):
    ...
    R = dict(count_id=cid, part_of=(root_id if root_id != cid else None),
             ...
             when=_r_stamp(c[3]) + " IST", counted_by=", ".join(who_c) or "",
             entered_by=", ".join(who_e) or "", recorded_by=c[4] or "",
             as_on=as_on, as_on_note=as_on_note,
             bill="%s dated %s" % (c[1], c[2]), readiness_lines=lines,
             ...
             fixes=fixes, matched=matched)
    today = dt.date.today().strftime("%d-%m-%Y")
    return _xlsx_response(padwriter.result_workbook(R),
                          "STOCK_CHECK_RESULT_count%d_%s.xlsx" % (root_id, today))
...
def page_pad():
    ...
    if err:
        return err
    try:
        with io.open(PAGE_PAD, "r", encoding="utf-8") as fh:
            t = fh.read()
    except OSError:
        return jsonify(ok=False, error="template_missing",
                       message="stock_pad.html is not beside stock_app.py"), 503
    return t, 200, {"Content-Type": "text/html; charset=utf-8",
                    "Cache-Control": "no-store"}
...
def api_pad_preview():
    ...
    ensure_schema(con)
    try:
        return jsonify(**_pad_summary(con, parsed, md5, n))
    except Exception as e:                                    # noqa: BLE001
        # An HTML 500 reaches the page as "Unexpected token '<'", which tells a
        # counter nothing at all. Whatever breaks, it breaks in words.
...
def api_pad_commit():
    ...
    bill = (request.form.get("bill_no") or "").strip().upper()
    bdate = (request.form.get("bill_date") or "").strip()
    cby = (request.form.get("counted_by") or "").strip()
    eby = (request.form.get("entered_by") or "").strip()
    if not (bill and bdate and cby and eby):
        return jsonify(ok=False, error="bad_request",
    ...
    items = [dict(item=m["item"], marg_qty=m["marg"], counted_qty=m["counted"],
                  ...
                  counted_by=cby, entered_by=eby) for m in matched]
    out = _record_count(dict(marg_as_on=as_on, bill_no=bill, bill_date=bdate,
                             items_total=len(snap), items=items), u)
    if not out.get("ok"):
        return jsonify(**out), 200
    cid = out["count_id"]
    # What could not be used is KEPT, against the count, so the follow-up sheet
    # can be handed back now and again later -- a sheet a person has to ask for
    # twice is a sheet that gets lost.
    for pr in parsed["problems"]:
    ...
    if part_of:
        ...
        # A row corrected on the SENT BACK TO FIX tab carries the ORIGINAL sheet
        # row it came from. That, not the name -- which is exactly what was
        # wrong -- is what says the issue is closed. Nothing is deleted: the
        # issue keeps what was first written, and gains which part settled it.
    ...
    con.commit()
    out["from_pad"] = dict(md5=md5, rows=len(parsed["rows"]),
