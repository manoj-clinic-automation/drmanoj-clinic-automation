...
def _head(sh, title, sub, date_str, part_line=None):
    sh.text(1, 0, title, TITLE)
    sh.text(2, 0, sub, NOTE)
    sh.merges.append("A2:H2")
    sh.text(4, 0, "Date", BOLD); sh.text(4, 1, date_str, INPUT)
    sh.text(5, 0, "Counted by", BOLD); sh.text(5, 1, "", INPUT)
    sh.text(6, 0, "Entered by", BOLD); sh.text(6, 1, "", INPUT)
...
def fresh_pad(items, as_on_text, today=None):
