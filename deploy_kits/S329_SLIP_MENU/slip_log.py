#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""slip_log.py -- S324 (session 271, 19-Sep-2026), S325/S326/S327 fixes the same day; S328 adds the X-ray EMR upload list; S329: the tile opens a menu, one tap per X-ray upload: new = above the highest ID issued before the day. The OPD & X-ray/Proc slip tile, and the night
...
def _menu_html(con, u, nx, nx2, room_n, today_n, miss_n):
    ...
    items = {
        ...
        "emr": ('<a class="mi" href="/finance/slips/emr"><b>X-ray upload (Docterz)</b><span class="%s">%d baaki</span></a>'
    ...
    if user == "awdhesh":
        order = ["room", "xp", "opd", "emr", "list"]
    elif user in ("alisha", "shivani"):
        order = ["emr", "opd", "xp", "room", "list"]
    else:
        order = ["opd", "xp", "room", "emr", "list"]
...
def emr_page():
    ...
    con = _db()
    items = emr_items(con)
    return _shell("X-ray upload (Docterz)", _emr_html(items, request.args.get("ok") or ""), "hi",
...
def emr_mark():
    ...
    keys = [k for k in request.form.getlist("key") if re.fullmatch(r"(xi:\d+|dz:\d{4}-\d{2}-\d{2}:[0-9A-Za-z\-/]{0,12}:\d+)", k)]
    ...
    for k in keys:
        ...
        if act == "done" and not it["done"]:
            con.execute("INSERT OR IGNORE INTO emr_upload(key, kind, day, clinic_id, uploaded_by, uploaded_at) "
                        "VALUES (?, 'xray', ?, ?, ?, ?)", (k, it["day"], it["clinic_id"], who, now))
            n += 1
...
def _emr_html(items, ok):
    ...
    if pend:
        out.append('<div class="rem">Baaki: <b>%d X-ray</b> · sabse purana %s. Pen drive se reception PC par laa kar, '
                   'Docterz mein mareez kholein, uploads mein X-ray daalein, phir yahan “Upload ho gaya” dabayein.</div>'
                   % (len(pend), _dmy(pend[0]["day"])[:6]))
    else:
        out.append('<div class="ok">Koi X-ray baaki nahi — sab Docterz mein upload ho chuke.</div>')
    days = []
    for i in pend:
