...
# ---------------------------------------------------------------------------
# S227 STAFF -- DARPAN'S LIST IN TURNS: quantities only, no rates, no values. The
# owner, 06-Sep-2026: "he gets only quantities, no rates, to spare him the fear of
# the loss being attributable to him." Same boxes as the hand sheet, so the typing
# board reads it column for column.
...
class _TDoc(_LDoc):
    def _header(self):
        p = self.pdf
        p.text(_LL, _LTOP + 14, "%s - %s - LIST FOR DARPAN" % (CLINIC, STORE), 9, gray=0.35)
        p.text(_LR, _LTOP + 14, self.head_right, 9, bold=True, align="r", gray=0.35)
        p.line(_LL, _LTOP + 9, _LR, _LTOP + 9, 0.4, 0.6)
    ...
    def thead(self, cont=False):
        ...
        self.y -= 6
        if cont:
            p.text(_LL, self.y - 9, "(continued)", 9, bold=True)
            self.y -= 13
        p.rect(_LL, self.y - 14 + 3, _LR - _LL, 14, 0.9)
        yy = self.y - 14 + 6
...
def render_tranche(d):
    """d -- _pad_diffs_data() narrowed to one list, plus d['tranche']. No rates, no values."""
    cid = d["count_id"]; t = d["tranche"]
    kind = "ORTHOTICS" if t["kind"] == "ortho" else "MEDICINES"
    doc = _TDoc("Stock count #%d - list %d for Darpan" % (cid, t["no"]), "Count #%d - %s - LIST %d (%s)" % (cid, d.get("day") or "", t["no"], kind))
    p = doc.pdf
    p.text(_LL, doc.y - 15, "%s - %s" % (CLINIC, STORE), 14, bold=True)
    p.text(_LR, doc.y - 15, "LIST %d FOR DARPAN - %s - COUNT AGAIN" % (t["no"], kind), 12, bold=True, align="r", gray=0.15)
    doc.y -= 22
    p.line(_LL, doc.y, _LR, doc.y, 0.8)
    doc.y -= 6
    ...
    return doc.finish("Count #%d - list %d for Darpan - PDF made %s IST" % (cid, t["no"], dt.datetime.now().strftime("%d-%m-%Y %H:%M")))
