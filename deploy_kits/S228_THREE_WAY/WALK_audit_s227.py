...
with sync_playwright() as pw:
    ...
    chk("AUDIT WORKBOOK for Amir: only what needs him -- the one FREE TOO HIGH line, with the exact thing to do; the server's own business (double entries, loose arithmetic) counted in the note only",
        len(rows) == 1 and kinds == ["FREE TOO HIGH"] and "open this bill in Marg" in str(rows[0][10]) and "1 double entr" in ws.cell(row=2, column=1).value and "6 lines read as Marg means them" in ws.cell(row=2, column=1).value, (len(rows), kinds, ws.cell(row=2, column=1).value[:300]))
    ...
    chk("...PINPOINTED to the span 27-08 -> 02-09 and EXPLAINED: bill 2767 of 27-08-2026 (48 strips) keyed after that day's export; residue left 0",
        ...
        and "48 strips" in sp[0]["explained"]["sentence"] and "a purchase entry keyed in Marg after the 27-08-2026 export" in sp[0]["explained"]["sentence"] and t["residue"] == 0 and not t["lookups"], sp)
    ...
    chk("ROSIKA (the owner's confusing case): -60 strips in span 1 and +60 in span 2 are MERGED into one span -- residue 0, no lookup, CLEAN",
        t["residue"] == 0 and t["raw_residue"] == 0 and not t["lookups"] and any(x["merged"] == 2 and x["residue"] == 0 for x in t["spans"]) and t["kind"] == "clean", t["spans"])
    chk("...its ONE conclusion: Marg and the documents agree, nothing to chase", t["conclusion"] == "Marg and the documents agree from 27-08-2026 to 06-09-2026. Nothing to chase in Marg.", t["conclusion"])
    ...
    chk("LANES: ROSIKA is not a Marg-lane line either; short 1 strip 2 tabs is the loss", lane[ROS]["lane"] in ("loss", "allowance"), (lane[ROS]["lane"], lane[ROS]["why"]))
