...
with sync_playwright() as pw:
    ...
    # S228 (D389): Darpan's LISTS moved to the owner's loss desk -- "move from here
    # to a place where I own it and get it done from Darpan". Amir keeps the TYPING
    # BOARD, so box 3 is now his typing, not his handing out. Updated here, not in
    # the product: the S227 shape it asserted is the one the owner superseded.
    chk("AMIR'S BOARD: the four boxes -- Marg's reports (with the upload box), vouchers, typing Darpan's answers, files; and the orthotics families",
        "Reports the server still needs from Marg" in body and "Vouchers to post in Marg" in body and "Type Darpan's answers" in body and "Files on the server" in body and "Orthotics — by family" in body
        and pg.locator("#margform #margfile").count() == 1, body[:300])
    chk("...and the lists themselves are NOT handed out from here any more (D389)",
        pg.locator("button[data-next]").count() == 0 and pg.locator("button[data-ret]").count() == 0)
