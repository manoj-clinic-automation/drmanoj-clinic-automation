...
def units_words(n, ps):
    n = abs(int(n)); st, tb = divmod(n, ps); out = []
    if st: out.append("%d strip%s" % (st, "" if st == 1 else "s"))
    if tb: out.append("%d tab%s" % (tb, "" if tb == 1 else "s"))
...
with sync_playwright() as pw:
    ...
    chk("THE BOX OFFERS THE STOCK CHECK REPORT", pg.locator("#padout a.dl.report").count() == 2 and "Open the STOCK CHECK REPORT" in out
        and pg.locator("#padout a.dl.report").first.get_attribute("href") == "/finance/stock/page/report?count=1")
    ...
    chk("...each with vendor, date, bill number, item as written, qty + free + loose, units, rate",
        P[0] == dict(vendor="ANMOL AGENCIES", bill_date="2026-04-15", bill_date_text="15-04-2026", bill_no="AN/1301", item=X, packing="1*15",
                     batch="", expiry="", qty=4.0, free=1.0, loose=0.0, units=75, basis="strips x pack + loose remainder", qty_text="5 strips", on_bill="4 + 1 free", rate_p=9100, purchase_rate_p=None, direction="PURCHASE"), P[0])
    ...
    chk("THE DETECTOR: Marg moved %+d, the documents explain %+d (bought %d, sold %d, returned %d), residue %+d" % (MARG_DELTA, DOCS_DELTA, WIN_P_UNITS, WIN_SOLD, WIN_RET, RESIDUE),
        ...
        and t["docs_delta"] == DOCS_DELTA and t["residue"] == RESIDUE and (("with no document on the server" in t["conclusion"] and t["kind"] == "lookup") if RESIDUE else "Nothing to chase" in t["conclusion"]), t)
    ...
    chk("SECTION 1 -- Differences, %d items, in lanes, with the FY window in words" % len(DIFF_ITEMS),
        ("1 · Differences — %d items" % len(DIFF_ITEMS)) in body and "01-04-2026" in body and "06-09-2026" in body)
    pg.evaluate("()=>document.querySelectorAll('.lane .lb[hidden]').forEach(e=>e.hidden=false)"); pg.wait_for_timeout(200)
    chk("...%d rows across the lanes, each with Marg -> counted, the difference in strips and tabs, MAJOR where it is, 'no word yet'" % len(DIFF_ITEMS),
        pg.locator("#s1 .row").count() == len(DIFF_ITEMS) and pg.locator("#s1 .badge.major").count() == j["major"]
        and pg.locator("#s1 .badge", has_text="no word yet").count() == len(DIFF_ITEMS))
    rowX = pg.locator('#s1 .row .h[data-item="%s"]' % X)
    ...
    lt0 = life_el.inner_text()
    chk("TAP X: THE CONCLUSION FIRST -- one line, the workings folded", "CONCLUSION" in lt0.upper() and ("Amir looks it up" in lt0 if RESIDUE else "Nothing to chase" in lt0) and "Show how this was decided" in lt0 and "AN/1301" not in lt0, lt0[:300])
    life_el.locator("button.tg").click(); pg.wait_for_timeout(200)
    ...
    chk("TAP X: THE INLINE SECTION OPENS with the financial year and the four purchase rows -- vendor, date, bill no, item, qty",
        ...
        and "5 strips" in lt and "(4 + 1 free on the bill)" in lt and "BHARAT MEDICAL" in lt and "BM-7741" in lt and "6 strips 3 tabs" in lt and "(6 + 3 loose on the bill)" in lt and "PO/2210" in lt and "(return)" in lt and "AN/1188" not in lt, lt[:500])
    chk("...bought / sold / returned totals in strips and tabs, Marg's figures by export day",
        ("Bought: " in lt and "in 4 bill lines" in lt and "units" not in lt.split("Sold:")[0]) and "Sold:" in lt and "in 6 bills" in lt and "Returned by patients:" in lt and "01-08-2026 = 100" in lt and "06-09-2026 = %d" % MARG[X] in lt, lt[500:1000])
    chk("...THE RESIDUE, in one sentence, and what a residue means",
        (("%s IN between" % units_words(RESIDUE, PSX) in lt and "Marg > item ledger > " + X in lt) if RESIDUE else "Nothing to chase" in lt) and "A residue is a stock change no document on the server explains" in lt)
    ...
    chk("X sits in the 'Marg moved without a document' lane, its residue in the reason", [d for d in j["differences"] if d["item"] == X][0]["lane"] == "marg")
    ...
    chk("SECTION 3 -- Matched, collapsed to one line", ("3 · %d items matched exactly" % (40 - len(DIFF_ITEMS))) in body and pg.locator("#s3 .lb").get_attribute("hidden") is not None)
    pg.locator("#s3 button.tg").click(); pg.wait_for_timeout(200)
    chk("...opens on a tap to the list", pg.locator("#s3 .lb").get_attribute("hidden") is None and pg.locator("#s3 table tr").count() == 41 - len(DIFF_ITEMS))
    ...
    pg.evaluate("()=>document.querySelectorAll('.lane .lb[hidden]').forEach(e=>e.hidden=false)"); pg.wait_for_timeout(200)
    rt = pg.locator('#s1 .row .h[data-item="%s"]' % X).inner_text()
    chk("A STAFF REASON SHOWS ON THE LINE, and 'explained so far' counts it",
        "staff: breakage — one strip crushed" in rt and "no word yet" not in rt and "1 / %d" % len(DIFF_ITEMS) in pg.inner_text("#head").replace("\n", " "), rt)
