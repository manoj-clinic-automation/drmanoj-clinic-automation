...
# -*- coding: utf-8 -*-
"""
walk_book_inline_s225.py -- THE LIVE-SHAPE WALK for rev 12 (the inline phone book), run on a COPY in a probe folder (F-321): the REAL finance_app.py,
...
On the box (after the copy, BEFORE the restart):
    FIN_DIR=/tmp/s225bi_probe FIN_DB=/root/finance/finance.db /root/wa/venv/bin/python3 -B /tmp/s225bi_probe/walk_book_inline_s225.py
"""
import os
import shutil
import sqlite3
...
ck("purchase_app beside finance_app is rev 12 (…, salt list, inline book)", hasattr(PA, "page_staff") and hasattr(PA, "_log_new_items") and hasattr(PA, "page_salts") and hasattr(PA, "_store_salts") and hasattr(PA, "_bank_name") and "IFSC_BANKS" in dir(PA))
...
rbt = rb.get_data(as_text=True)
ck("phone book: the doctor -> 200 through the real gate", rb.status_code == 200 and "Stockist phone book" in rbt, str(rb.status_code))
ck("rev 12: the book is the inline one (bookForm, inline slots, BOOKV) and the popup functions are gone", "function bookForm" in rbt and "const BOOKV=" in rbt and "bookBank(" not in rbt and "bookPhones(" not in rbt)
_bjs = rbt[rbt.index("function bookMsg"):rbt.index("const BOOK=")] if "function bookMsg" in rbt else "prompt("
ck("rev 12: no prompt()/confirm()/alert() in the book's own script", "prompt(" not in _bjs and "confirm(" not in _bjs and "alert(" not in _bjs)
ck("rev 12: on the COPY of the live db, every row has its inline slots (count only, nothing printed)", rbt.count('id="inl-bank-') == rbt.count('id="inl-phones-') and rbt.count('id="inl-bank-') >= 1)
