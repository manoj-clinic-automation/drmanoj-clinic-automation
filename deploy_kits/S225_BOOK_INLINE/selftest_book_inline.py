...
# -*- coding: utf-8 -*-
"""
selftest_book_inline.py -- S225 (rev 12): the rev-11b selftest as regression (two book assertions updated for the owner's
rev-12 ruling: full account numbers on the page, inline forms), then §20 the inline phone book. Base: the rev-10 selftest, then §19 Amir's salt work list. Base: the rev-6 selftest (rounding now in the engine), then §14 the phone book. Base: the S224 selftest UNCHANGED as regression, then §13 the staff order page. Base: the REAL blueprint, on a temp finance.db, fed the REAL
...
ck("darpan's book shows UNVERIFIED, 'waits for Dr Manoj'; rev 12: the TABLE shows the FULL account number (the owner, 05-Sep: this page is the NEFT source)", "UNVERIFIED" in bk2 and "waits for Dr Manoj" in bk2 and ACC in _tbl and ("…" + ACC[-4:]) not in _tbl)
ck("rev 12: rows are addressed by index, not by a quoted name in the attribute (no quoting hazard)", "bookPhones(" not in bk2 and "bookBank(" not in bk2 and _re.search(r'onclick="bookForm\(\d+,\'phones\'\)"', bk2) is not None and _re.search(r'onclick="bookForm\(\d+,\'bank\'\)"', bk2) is not None)
as_("manoj", "doctor", {"checker"})
...
# ------------------------------------------------------------- 18. S225 rev 10: new items
PA._db, PA._require, PA._assets_db = _db, _require, ASSETS
as_("manoj", "doctor", {"checker"})
ym = today.strftime("%Y-%m")
with app.test_request_context():
    logged0 = PA._log_new_items(_db(), ym)
ck("the ZZTEST items (in the one and only snapshot = the baseline) are NOT logged as new", not any("ZZTEST ITEM" in x for x in logged0))
# a second, newer snapshot with a brand-new item; and a purchase line this month for an item never seen
c = sqlite3.connect(DB)
nd = (today + dt.timedelta(days=1)).strftime("%d-%m-%Y")
for name, qty in (("ZZTEST ITEM 0", 12), ("ZZTEST ITEM 1", 0), ("ZZTEST ITEM 2", 500), ("ZZNEW SHELF ITEM", 7)):
    c.execute("INSERT INTO stock_snapshot (as_on,item,qty,packing,pack_size,loaded_at,source) VALUES (?,?,?,?,?,?,?)", (nd, name, qty, "1*10", 10, "t", "push_snapshot"))
md5x = os.urandom(16).hex()
c.execute("INSERT INTO purchase_export (md5,type,file,period_from,period_to,export_stamp,received_at,n_rows,grand_amount_p,superseded_by) VALUES (?,?,?,?,?,?,?,?,?,NULL)",
          (md5x, "ITEMWISE", "zz_new.xls", ym + "-01", today.isoformat(), "20260904-120000", "t", 1, 0))
cols = {x[1] for x in c.execute("PRAGMA table_info(purchase_line)")}
vals = dict(source_md5=md5x, line_type="ITEMWISE", supplier_norm=PA.supplier_key("ZZTEST STOCKIST"), bill_no="NEW1", bill_date=today.isoformat(),
            month=ym, item="ZZNEW BOUGHT ITEM", packing="1*10", qty=5, amount_p=50000, net_amount_p=50000, rate_p=10000)
use = {k: v for k, v in vals.items() if k in cols}
c.execute("INSERT INTO purchase_line (%s) VALUES (%s)" % (",".join(use), ",".join("?" * len(use))), tuple(use.values()))
c.commit(); c.close()
with app.test_request_context():
    logged1 = PA._log_new_items(_db(), ym)
ck("a brand-new shelf item and a first-bought item are logged; the old ZZTEST items are not", "ZZNEW SHELF ITEM" in logged1 and "ZZNEW BOUGHT ITEM" in logged1 and not any(x.startswith("ZZTEST ITEM") for x in logged1), str(logged1))
with app.test_request_context():
    logged2 = PA._log_new_items(_db(), ym)
ck("running again logs nothing more (idempotent, a row is written once)", logged2 == [])
rows = q("SELECT item, seen_in, supplier FROM purchase_new_item ORDER BY item")
ck("the log says where each was first seen, with the stockist for the bought one", any(r["item"] == "ZZNEW BOUGHT ITEM" and r["seen_in"] == "purchase" and "ZZTEST STOCKIST" in (r["supplier"] or "") for r in rows) and any(r["item"] == "ZZNEW SHELF ITEM" and r["seen_in"] == "stock" for r in rows))
hb = cl.get(P + "/page/hub").get_data(as_text=True)
ck("the hub shows the New items card with the count and both names", "New items" in hb and "new medicines this month" in hb and "ZZNEW SHELF ITEM" in hb and "ZZNEW BOUGHT ITEM" in hb)

# ------------------------------------------------------------- 19. S225 rev 11: the salt WORK LIST
PA._db, PA._require, PA._assets_db = _db, _require, ASSETS
import push_salts as PS
FX = os.path.expanduser("~/mnt/Downloads/Sanjeevni_Salt_Fix_for_Amir.xlsx"); NX = os.path.expanduser("~/mnt/Downloads/margsync/_analysis/Sanjeevni_Salt_Corrections.xlsx")
...
as_("amir", "staff", {"viewer"})
ck("amir is refused before the setting names him (fail-closed)", cl.get(P + "/page/salts").status_code == 403)
c = sqlite3.connect(DB); c.execute("INSERT OR REPLACE INTO setting (key, value) VALUES (?,?)", (PA.SALT_USERS_KEY, "amir")); c.commit(); c.close()
...
wait_id = q1("SELECT id FROM purchase_salt_task WHERE section='waiting' ORDER BY seq LIMIT 1")
ck("amir cannot answer a waiting row (doctor only)", cl.post(P + "/api/salt_task", json=dict(action="answer", id=wait_id, answer="X")).status_code == 403)
as_("manoj", "doctor", {"checker"})
sp2 = cl.get(P + "/page/salts").get_data(as_text=True)
ck("the doctor sees the answer box with the salt datalist on the waiting rows", ('id="ans%d"' % wait_id) in sp2 and 'list="salts"' in sp2 and 'id="salts"' in sp2)
r = cl.post(P + "/api/salt_task", json=dict(action="answer", id=wait_id, answer="ZZ DOCTOR SALT"))
ck("the doctor answers a waiting row: saved with his name, the name joins the list", r.status_code == 200 and q("SELECT answer, answer_by FROM purchase_salt_task WHERE id=?", wait_id)[0][:] == ("ZZ DOCTOR SALT", "manoj") and q1("SELECT COUNT(*) FROM purchase_salt_name WHERE salt='ZZ DOCTOR SALT'") == 1)
ck("an answer on a non-waiting row is refused (409); an empty answer 400; unknown id 404",
   cl.post(P + "/api/salt_task", json=dict(action="answer", id=first_change, answer="X")).status_code == 409 and cl.post(P + "/api/salt_task", json=dict(action="answer", id=wait_id, answer="")).status_code == 400
   and cl.post(P + "/api/salt_task", json=dict(action="done", id=999999, done=True)).status_code == 404)
...
try:
    ...
    wb = openpyxl.load_workbook(_io.BytesIO(x))
    ck("  openpyxl opens it: 5 sheets, the change sheet has header + 77 rows, the DONE mark is in it, the waiting sheet carries the doctor's answer",
       len(wb.sheetnames) == 5 and wb[wb.sheetnames[2]].max_row == 78 and any("DONE" in [str(v) for v in row] for row in wb[wb.sheetnames[2]].iter_rows(values_only=True))
       and any("ZZ DOCTOR SALT" in [str(v) for v in row] for row in wb[wb.sheetnames[3]].iter_rows(values_only=True)), str(wb.sheetnames) + " " + str(wb[wb.sheetnames[2]].max_row))
except ImportError:
    ck("  (openpyxl not here; structure check only)", True)
sys.path.insert(0, os.path.join(KITS, "S224_DAY_REVENUE_PDF"))
r = cl.get(P + "/salts.pdf"); pdfb = r.get_data()
ck("A4 PDF renders with the five sections and [x] / [ ] marks", r.status_code == 200 and pdfb.startswith(b"%PDF-1.4") and pdfb.rstrip().endswith(b"%%EOF") and b"Rename these salts first" in pdfb and b"[x]" in pdfb and b"[ ]" in pdfb)
as_("darpan", "staff", {"maker"})
ck("darpan (not named) cannot open the list, the Excel or the PDF", cl.get(P + "/page/salts").status_code == 403 and cl.get(P + "/salts.xlsx").status_code == 403 and cl.get(P + "/salts.pdf").status_code == 403)
as_("", "", set())
ck("signed out -> 401", cl.get(P + "/page/salts").status_code == 401)


# ------------------------------------------------------------- 20. rev 12: the inline phone book (the owner, 05-Sep-2026)
as_("manoj", "doctor", {"checker"})
ck("_bank_name reads the bank off the IFSC; unknown -> ''", PA._bank_name("HDFC0001234") == "HDFC Bank" and PA._bank_name("sbin0004567") == "State Bank of India" and PA._bank_name("ZZZZ0000001") == "" and PA._bank_name("") == "")
c = sqlite3.connect(DB)
c.execute("UPDATE purchase_vendor_contact SET source='neft_import', added_by='neft_import_s225' WHERE vendor_norm=?", (PA.supplier_key("ZZTEST STOCKIST"),))
c.execute("INSERT INTO purchase_vendor_contact (vendor_norm, vendor, phone, acct_no, ifsc, bank_status, bank_verified_by, bank_verified_at, source, added_by, updated_at) "
          "VALUES (?,?,?,?,?,?,?,?,?,?,?)", (PA.supplier_key("ZZ NEFT ONLY"), "ZZ NEFT ONLY", "", "3" * 14, "KARB0000524", "VERIFIED", "owner (NEFT import)", PA.now_iso(), "neft_import", "neft_import_s225", PA.now_iso()))
c.commit(); c.close()
bk4 = cl.get(P + "/page/book").get_data(as_text=True)
_tbl4 = bk4[:bk4.index("<script>")]
_js4 = bk4[bk4.index("function bookMsg"):bk4.index("const BOOK=")]
ck("the book page carries no popup at all: no prompt(), confirm() or alert() in the book's own script", "prompt(" not in _js4 and "confirm(" not in _js4 and "alert(" not in _js4 and "function bookForm" in _js4 and "function bookVerify" in _js4)
ck("every row has its three inline slots (phones, bank, verify), hidden until tapped", _tbl4.count('id="inl-phones-') == _tbl4.count('id="inl-bank-') == _tbl4.count('id="inl-verify-') >= 3 and 'id="inl-bank-0" hidden' in _tbl4)
ck("the doctor's Verify is an inline yes/no, not a confirm()", "bookVerify(" in bk4 and ">Verify<" in bk4 and "Yes, verified" in _js4)
ck("the add form reports inline (msg-add), not alert()", 'id="msg-add"' in _tbl4 and "bookMsg(m, 'Name and first phone are required.')" in _js4)
ck("full account number and IFSC in the table, the bank named from the IFSC", ("3" * 14) in _tbl4 and "KARB0000524" in _tbl4 and "Karnataka Bank" in _tbl4 and ACC in _tbl4 and "HDFC Bank" in _tbl4)
ck("a row from the NEFT import says so, and a row with no phone says 'no number yet' + 'add numbers'", "from the NEFT records (04-Sep)" in _tbl4 and "no number yet" in _tbl4 and ">add numbers<" in _tbl4 and ">edit numbers<" in _tbl4)
ck("the page counts the stockists without a number", "1 stockist without a phone number yet" in _tbl4)
ck("BOOKV (the index -> name list) is served beside BOOK", "const BOOKV=" in bk4 and "ZZ NEFT ONLY" in bk4[bk4.index("const BOOKV="):])
as_("darpan", "staff", {"maker"})
bk5 = cl.get(P + "/page/book").get_data(as_text=True)
ck("darpan also sees the full account number (the three editors are the NEFT preparers), but no Verify button", ("3" * 14) in bk5[:bk5.index("<script>")] and ">Verify<" not in bk5)
r = cl.post(P + "/api/book", json=dict(action="phones", vendor="ZZ NEFT ONLY", phone=FAKE2, phone2=""))
ck("the inline phones form's payload (action=phones) still lands through the unchanged API for a NEFT-only row", r.status_code == 200 and q1("SELECT phone FROM purchase_vendor_contact WHERE vendor_norm=?", PA.supplier_key("ZZ NEFT ONLY")) == FAKE2)
ck("...and that row keeps its VERIFIED bank status (phones and bank are separate actions)", q1("SELECT bank_status FROM purchase_vendor_contact WHERE vendor_norm=?", PA.supplier_key("ZZ NEFT ONLY")) == "VERIFIED")
as_("amir", "staff", {"viewer"})
ck("amir is still refused the book (rev 12 changed the page, not the gate)", cl.get(P + "/page/book").status_code == 403)
ck("no full account number in any audit row after rev 12 either", not any(("3" * 14) in (d or "") or ACC in (d or "") for (d,) in q("SELECT detail FROM purchase_audit WHERE action LIKE 'book_%'")))
