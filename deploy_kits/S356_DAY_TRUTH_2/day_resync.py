#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""day_resync.py -- S356_DAY_TRUTH_2: an autofiled pharmacy day follows the bank.
...
WHAT THIS DOES
    ...
    day_noncash_bill row (the row his old form used to write), once.
...
WORD_SETTINGS = (("noncash.home_words", HOME_DEFAULT,
                  "S356: substrings (comma-separated, any case) of a Marg bill's customer text that mean HOME MEDICINE"),
                 ("noncash.proc_words", PROC_DEFAULT,
                  "S356: substrings that mean PROCEDURE MEDICINE (Darpan's spellings; add more here, never in code)"))
...
def noncash_day(con, e, home_w, proc_w, apply):
    ...
    for bill, bdate, head, amt, where in bills:
        ...
        if ex is not None:
            ...
            continue
        if amt <= 0:
            notes.append("%s is a %s CREDIT NOTE of %s -- the table cannot hold a negative bill; the day's net already carries it, "
                         "so the drawer reads low by that amount (a person's line)" % (bill, head.replace("_", " "), rupees(-amt)))
            continue
    ...
    con.execute("INSERT INTO audit_log (table_name, row_id, action, after_json, by_whom, at) "
                ...
                (eid, json.dumps(dict(date=iso, added=[dict(bill=b, head=h, amount_p=a, source=w)
                                                          for b, d, h, a, w in added], kit=KIT)), WHO, now))
    con.commit()
    return "ADDED", line
