...
# S298 (17-Sep-2026): on an EXIT the last step used the JOIN wording ("the person appears"), and the
# owner, walking Pravesh's exit, stopped there confused. It is also not a tick that makes anything true:
# the staff master and the attendance register have to be changed. The words now say so.
EXIT_LABEL = {
    "STAFF_MASTER":     "staff master and attendance register updated -- the person no longer appears",
}
EXIT_WHY = {
    "STAFF_MASTER":     ("Claude does this step: marks the person inactive in the staff master and "
                         "the attendance register from the last working day, retires the Emp Code, "
                         "and ticks it. Tell Claude who left and the last working day."),
}


def label_for(kind, step):
    if kind == "EXIT" and step in EXIT_LABEL:
        return EXIT_LABEL[step]
    return STEP_LABEL.get(step, step)


def why_for(kind, step):
    if kind == "EXIT" and step in EXIT_WHY:
        return EXIT_WHY[step]
    return STEP_WHY.get(step)
...
def blocked_by(kind, done, step):
    ...
    if not missing:
        ...
        # S208.1 -- ONE-LINE FIX, found by walking the EXIT flow against the
        # live require() signature: HARD_REQUIRES was applied regardless of
        # kind, so an EXIT's final STAFF_MASTER demanded BIOMETRIC -- a step
        # an exit does not have -- and no exit could EVER complete. A hard
        # requirement only binds when the required step exists in this flow.
        order_set = set(order)
        missing = [s for s in HARD_REQUIRES.get(step, ())
                   if s in order_set and s not in done]
    if missing:
        return False, ("Do '%s' first. %s" % (label_for(kind, missing[0]),
                                              why_for(kind, missing[0]) or "")).strip()
    return True, ""
...
def pending(con):
    ...
    for r in con.execute("SELECT id,ref,kind,person,role,status,opened_on,username,emp_code "
        ...
        out.append({
            ...
            "waiting_label": label_for(kind, waiting[0]) if waiting else None,
            "still_to_do": waiting,
...
def api_open():
    ...
    kinds = employment_kinds(con)                       # S222 SECTIONS
...
def api_step():
    ...
    return jsonify(ok=True, ref=ref, step=step, label=label_for(kind, step),
                   complete=complete, still_to_do=remaining,
                   next=(remaining[0] if remaining else None),
                   next_label=(label_for(kind, remaining[0]) if remaining else None))
...
def api_record():
    ...
    steps = [{"step": s, "label": label_for(kind, s), "why": why_for(kind, s),
...
def _portal_status(user, pw=None):
...
def api_portal_user():
    """Does this joiner's login actually exist, and does its password work? READ ONLY.
    ...
    st = _portal_status(user, default_password(person))
