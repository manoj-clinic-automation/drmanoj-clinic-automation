#!/usr/bin/env python3
"""walk_s340.py -- (S332 walk + S333 X-ray test run + S335 Drive helper + S340 Check karein) the live-shape walk of the patient-record page: THE REAL finance_app.py (patched) over
...
def main():
    ...
    os.environ["FINANCE_CRON_TOKEN"] = "walk-token-s340"
    ...
    T = {"X-Finance-Cron": "walk-token-s340"}
    ...
    uk = [k for k in keys if k.startswith("lu:")]
    npk = [k for k in keys if k.startswith("np:")]
    check("unknown ID is asked", len(uk) == 1 and "99999997" in t, keys)
    check("the e-mail with no PDF is asked", len(npk) == 1)
    check("the walk's earlier no-PDF e-mail is closed by the PDFs beside it", len([k for k in keys if k.startswith("np:")]) == 1)
