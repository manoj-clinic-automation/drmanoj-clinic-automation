...
class _XlsxSheet(object):
    """Wraps an openpyxl worksheet to expose the SAME three things the parser
    uses from an xlrd sheet — nrows, ncols, cell_value(r, c) — with identical
    semantics. Built at S183 because staff open a Marg .xls in Excel and save
    it back as .xlsx; xlrd 2.x reads legacy .xls only, so a perfectly good
    export was being refused. Nothing else in the parser had to change: every
    money and identity column is proven byte-identical to the .xls path by the
    round-trip regression test (an .xls converted to .xlsx parses to the same
    day_totals and bill count).

    A date-typed cell is rendered dd-mm-yyyy — the one shape the report body and
    iso() already expect — so a cell Excel stored as a real date reads exactly
    as the same cell stored as text in the .xls."""

    def __init__(self, ws):
        rows = [list(r) for r in ws.iter_rows(values_only=True)]
        # trim wholly-empty trailing rows so nrows matches xlrd's view
    ...
    def cell_value(self, r, c):
        ...
        if v is None:
            return ""
        # openpyxl hands back datetime for date-formatted cells; the .xls path
        # sees these as text. Normalise to the report's own dd-mm-yyyy so iso()
        # parses them the same either way.
        if isinstance(v, (datetime.datetime, datetime.date)):
            return v.strftime("%d-%m-%Y")
        # xlrd stores EVERY number as a float, so `_cell` renders it with a
        # trailing '.0' (a batch '260117' becomes '260117.0'). openpyxl keeps
        # whole numbers as int. Coerce int -> float here so an Excel-saved file
        # reads byte-identically to the original .xls it came from — the whole
        # point is faithfulness, and consistent batch/qty strings are what the
        # return-correlation match depends on. (bool is an int subclass; leave
        # it alone.)
...
def _open_sheet(path):
    # Identify by CONTENT, not by extension — a file is not its name (D188).
    # A Marg export saved through Excel becomes a real .xlsx (ZIP: 'PK\\x03\\x04')
    # while the native export is a legacy .xls (OLE2: '\\xd0\\xcf\\x11\\xe0').
    try:
        with open(path, "rb") as fh:
            magic = fh.read(8)
    except (OSError, IOError) as ex:
        raise MargReportError("cannot read the file (%s)" % ex)
    ...
    if magic[:4] == b"PK\x03\x04":
        try:
            ...
            raise MargReportError(
                "this is an .xlsx file (a Marg export re-saved through Excel) and "
                "openpyxl is not installed on this machine to read it.  Install "
                "with:  pip install openpyxl   — or re-export from Marg as .xls")
        try:
            wb = openpyxl.load_workbook(path, read_only=True, data_only=True)
        except Exception as ex:                                # noqa: BLE001
            raise MargReportError("not a readable .xlsx file (%s)" % ex)
        return _XlsxSheet(wb[wb.sheetnames[0]])

    # default: legacy .xls via xlrd (unchanged behaviour)
    try:
        import xlrd
    except ImportError:
        raise MargReportError(
    ...
    for day in report["days"]:
        ...
        for b in day["bills"]:
            w.writerow({
                ...
                "mode": b["mode"] or "",
                "gross": "%.2f" % (b["gross_p"] / 100.0),
                "disc": "%.2f" % (b["disc_p"] / 100.0),
            })
            n += 1
    return n
