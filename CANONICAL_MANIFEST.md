...
| `START_HERE_SESSION_202` | **S201 close** | `d5e6e96fda8c6c978f215601446d5a09` | **CURRENT** entry point. Phase 0 = manifest md5 · F-88 · F-107 inverse · F-119 · F-123 · A8 (against Register **v5.46**) · `verify_live_pins.py` (expect GREEN against `live_pins_S201close.txt`; the S200 list shows drift on `finance_app.py`, which the box has right — F-134 shape). Carries the S201 narrative, the ⭐0 owner items (**pin-list copy · token rotation, THREE copies, never hand-copy** · Darpan SPECIAL · Pravesh 31-Aug · the July cash items · AF-3 · R9), **⭐1 the owner-prioritised `/ops` runbook surface with B2 as its honest prerequisite**, and next free **D349 · F-184**. |
...
| `MARG_PIPELINE_REFERENCE_v1` | S201 · **how it works** | *(md5 owed — a real hash, not a placeholder: D172/D188/F-116)* | the capture → route → archive → send chain end to end; the two Marg output trees (D: and **C:**); the three token copies (§4). **Project knowledge; repo-filing OWED at the S202 open — the F-107 condition, recorded rather than left to be discovered.** |
| `MARG_INGESTION_REFERENCE_v1` | S201 · **what the server does** | *(md5 owed)* | `finance_ingest` end to end: the confidence tiers, `marg_net_sql()` signed net, credit notes as magnitude + `_return`, and **D313 — money is the maker's typed entry; `sale_item` is attribution only.** Same F-107 condition. |
| `S201_UI_Health_Redesign_Record` | S201 · **the F-181/F-182 record** | *(md5 owed)* | the nested-`<a>` fault as found in the owner's own saved SERVED bytes, the Clinic Design Language v1 migration, and the F-130 registration. Same F-107 condition. |
...
`START_HERE_SESSION_202.md` (`d5e6e96fda8c6c978f215601446d5a09`) ·
