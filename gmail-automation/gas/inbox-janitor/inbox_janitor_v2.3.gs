/**
 * INBOX JANITOR v2.3 (22-Aug-2026: REPORTS rule made domain-proof) (DEPLOYED STATE: 17-entry verified RENEWALS array) — drmanojkragarwal@gmail.com  (23-Jul-2026)
 *
 * FUNCTIONS & TRIGGERS:
...
var RENEWALS = [
  // v2.3 (owner, 22-Aug-2026): drmanojagarwal.in is being RETIRED, not
  // transferred -- the JustDial->Hostinger ACTION entry is withdrawn, and so
  // is its 2027 renewal below. dr-manoj.in is renewed and on AUTO-RENEW
  // (GoDaddy login = the CLINIC account, drmka.ortho) -- entry kept so the
  // yearly date still surfaces as a check that auto-renew actually fired.
  { vendor: 'GoDaddy dr-manoj.in', dateISO: '2027-08-29', cycleMonths: 12,
    note: 'AUTO-RENEW is ON (set Aug-2026). Login = CLINIC Google account. ' +
          'Entire VPS estate + short links ride on this domain: on this date ' +
          'just VERIFY the auto-renewal charged.' },
...
var RULE_REPORTS = {
  ...
  // v2.3: 'MERCHANTSOLUTIONS@icicibank.com' went blind when ICICI moved to
  // @icici.bank.in (the CC saver was built domain-proof for this exact
  // migration; this rule was not). Username-only now, like the CC saver --
  // observed cost: ~5 MPR mails/day piling up unread since the move.
  query: '(from:(merchantsolutions OR mail.track360.co.in OR ' +
