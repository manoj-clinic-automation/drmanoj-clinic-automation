# DIAGNOSTICS & SURVEILLANCE SYSTEM SPEC — v2.3 (CONSOLIDATED, SELF-CONTAINED)
...
| v2.2 | 13 Jul 2026 (Session 144) | **Flag Investigator (D239) LIVE** as a detection + self-heal mechanism (`flag_investigator.py` v1.1, cron `*/30 9-19`+`0 20` IST): diagnoses lost conversations against MyOperator `/search`, self-heals same-day recoverables via one pipeline kick, counts provider never-recorded, `escalate_lokesh` at ≥3/7d → raise with Lokesh. **F-42 QUANTIFIED** (42 never-recorded/7d, provider-side). **F-43** raised+fixed (kick-gate keyed off an outcome transition, not the durable `kicked` flag). |
| v2.3 | 14 Jul 2026 (Session 145) | **F-44 raised + fixed + PROVEN.** The Investigator + the digest classifier had counted provider-MISSED calls as never-recorded, because they read talk-seconds instead of the top-level `status`. Fixed to key off `status` (`bridged` vs `missed`/`voicemail`): `flag_investigator.py` v1.2 (`is_lost_candidate()` drops missed/voicemail; `diagnose()` adds `missed_no_conversation` and restricts `never_recorded` to status 1 + blank filename) and `daily_digest.py` v1.4 (`classify_pending()` labels missed rows "missed — no recording expected", not an alert). **The S144 "42 never-recorded" collapsed 42 → 0 on re-baseline — all 42 were `missed (status 2)`; the escalate-to-Lokesh was a FALSE alarm and is VOID.** Decision **D244**; the detection semantics below updated accordingly. |
...
## FLAG INVESTIGATOR (D239) — DETECTION + SELF-HEAL MECHANISM (S144)

`flag_investigator.py` v1.1 (`/root/wa/recordings-archive/`, run with the venv python; cron `*/30 9-19 * * *` + `0 20 * * *` IST) is the first surveillance mechanism that not only DETECTS a fault class but self-heals the recoverable part of it.

- **Detects:** F-42-class lost conversations — connected calls with real talk time and no recording — read from `Call_Durations`, past a 25-min grace. **v1.2 (S145):** provider-MISSED rows are excluded before candidacy — a missed call's ring/hold seconds are not talk time (F-44/D244), and even if one slips through, a `/search` `status "2"` is classified `missed_no_conversation`, not `never_recorded`.
- **Adjudicates against provider ground truth:** MyOperator `/search`; a call has audio ONLY when status `"1"` + non-empty filename. Outcomes: `recoverable` / `never_recorded` / `no_provider_log`.
- **Self-heals** same-day recoverables by dropping one ordinary pipeline kick (the worker re-runs archive→verdict idempotently); gated on the durable per-call `kicked` flag so a call is kicked at most once.
- **Escalation routing:** `flag_investigator_results.json` carries `escalate_lokesh=true` when provider never-recorded ≥ 3 in a rolling 7 days (D239 default). The daily digest reads this file. As of S144 the count is 42/7d → standing escalation to Lokesh.
- **Fault codes touched:** **F-42** (now quantified, provider-side; open pending Lokesh) and **F-43** (kick-gate keyed off an outcome transition instead of the durable action-taken flag — raised + FIXED in v1.1; lesson: a gate must depend on what was actually done, not a derivable state — the F-41 family). **F-44** (S145) — the detector had counted provider-MISSED calls as never-recorded; the "42" was 42 missed calls and collapsed to **0** on re-baseline. Fixed to key off the top-level `status` (D244); missed calls now get the non-alert outcome `missed_no_conversation`, and `never_recorded` means only status-1 + no-recording. **The Lokesh escalation was a FALSE alarm and is VOID.**
- **Fallback:** the tool's failure modes are delays, never data loss — recordings persist on MyOperator's cloud indefinitely; the 02:00/03:40 nightly sweeps remain the guaranteed floor beneath it.

**END OF DIAGNOSTICS & SURVEILLANCE SYSTEM SPEC v2.3 — "WHAT THIS DOCUMENT STILL OWES" is the last section. If it is absent, this file is truncated and must not be used as canonical.**
