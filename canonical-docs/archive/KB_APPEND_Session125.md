...
> **Rejected-at-the-door (Category 5) — IMPLEMENTED** in the receiver (D163), live 08-Jul 14:49. **`callhook_watchdog.py` v1.0 — BUILT**, on the VPS, manual runs only. Blocked on two defects before scheduling: a coverage guard (it currently reports "MyOperator is not delivering at all" for any date its access log doesn't cover) and `unquote()` normalisation (D165).

**ADD** to the §12 systems list:

> **`callhook_watchdog.py`** — `/root/wa/call-hook/callhook_watchdog.py`, 25,042 bytes. Read-only detector for the `call-hook` fault family. `--selftest` (37/37) · bare run (today) · `--json` · `--date YYYY-MM-DD` (**untrustworthy until the coverage guard lands**) · `--state <path>` (the only thing it ever writes). Exit 0 OK / 1 WARN / 2 CRITICAL / 3 could-not-run. Six fault codes: `CALLHOOK_SECRET_MISMATCH_403`, `CALLHOOK_RAWLOG_MISSING`, `CALLHOOK_403_EARLIER_TODAY`, `CALLHOOK_MULTIPLE_KEYS`, `CALLHOOK_NO_ACCEPTED_TODAY`, `CALLHOOK_SILENT`.

---

## §4 — CHANGELOG: one row, at the top of the table

| v1.49 | 08 Jul 2026 (Session 125) | The `CALLHOOK_SECRET_MISMATCH_403` **detector is built** (`callhook_watchdog.py` v1.0, read-only, 37/37) and **Category 5 is implemented** in the receiver (`call_hook_capture.py` v2, reject log, 43/43, live 14:49:12). Its first run settled the S124 forensics in one line: **one key label carrying both the 635 rejections and the 115 acceptances** — MyOperator never changed what it sent, so no second webhook subscription was delivering on 08 Jul. **The outage was TWO faults, not one.** *Window 1* (06 Jul 13:41 → 07 Jul 16:28): the 61-character run-on line, **dormant** until a worker respawn re-read `.env`. *Window 2* (07 Jul ~16:35 → 08 Jul 10:28): after S94's repair demonstrably worked (4 × 200), the panel and the VPS came apart again — **how, is still unknown**. `7e17f7` resolved as the run-on, and identified from §94 above: a **lost newline** merging `CALLHOOK_SECRET` with `FU_UPLOAD_SECRET=…`, reconstructing to the byte (12+17+32=61, non-alnums `@__=` in order). **`sed -i '17s'` was S94's repair, not the cause** — incident v1 §6 inverted this, S124 §3.2 inspected `.env` after the repair, and S124 §4's "the VPS was correct throughout" is false for Window 1. All three retracted. **D162** dual-key acceptance (a shared secret must be rotatable without a synchronised cutover — the worker reads `.env` once at import, so a mismatch is dormant until an unpredictable respawn). **D163** a gate writes down its refusals before it refuses. **D164** `.env` is never edited by line number, and its contents are validated at startup; WinSCP Binary, never Text. **D165** key labels must be encoding-normalised before comparison (wire `%40` ≠ literal `@`). Diagnostics Spec → v1.7 (new **Category 6: absence of coverage is not absence of events**, learned from a defect in the detector itself). |
