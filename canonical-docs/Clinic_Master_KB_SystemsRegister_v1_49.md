# Clinic Master KB / Systems Register — v1.49 (CONSOLIDATED)
...
**`CALLHOOK_SECRET` rotation — STEPS 1 AND 2 COMPLETE, STEPS 3 AND 4 OPEN.**
...
- **Step 3** ⬜ **OPEN. The MyOperator panel still sends the old key `key_db8972`.** Requires a clinic day with hours in front of it: update the panel, place one real call, confirm `on PREVIOUS key/30min` falls to `0` and `refused today: none` — **then re-check ≥1 hour later on the same clinic day.** An incident is closed by a successful re-test, not a successful test.
- **Step 4** ⬜ **BLOCKED ON STEP 3, AND MUST STAY BLOCKED.** Clearing `CALLHOOK_SECRET_PREV` while the panel holds the old key reconstructs the 06-Jul outage by hand. The command is deliberately absent from `rotate_callhook.sh`, from this KB, and from the runbook. See **D173**.

**The old 12-character `@` key (`key_db8972`) is still live and still exposed in a chat transcript.** It dies at step 4 and not before. The exposure is now *bounded and unhurried* rather than *unfixable without an outage* — which is what steps 1 and 2 bought.
...
- **Next free decision number for new work: D176.**
...
**END OF MASTER KB v1.49. The CHANGELOG is the last section, and the `v1.38` entry is its last line. If either is absent, this file is truncated and must not be used as canonical.**
